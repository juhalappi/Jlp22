...
\addcontentsline{toc}{subsection}{\textbf{J} in Luke} 
...
the software. This meant e.g. that Luke planned to publish the manual where I would be the 
first author without my involvement. 
...
I described in previous versions some details of the process and listed all the ten 
...
jexamples.txt. 
Version March 30, 2025   Paper Juha Lappi (2024) 'From Finnish Assortment Pricing to Market 
Economy Using Prices for Sawn Wood and 
Chips in Reference Bucking' is now available at DOI: 10.4236/ojf.2024.143015 
github.com/juhalappi/jlp22  contains scripts for doing all computations of the paper. I have not 
tested if everything works with the new version. 
A dangerous property of matrix computations is removed. If A was a matrix, then code A=7 put 
each element of A equal to 7. Now A=7 means that A is made \textcolor{teal}{REAL} object with value 7. Using 
A=0 can be used instead \textcolor{VioletRed}{delete\_o}(A) which was needed before to make a \textcolor{teal}{MATRIX} object \textcolor{teal}{REAL}. All 
elements can be made equal to 7 with code A()=7. If \textcolor{teal}{x} is \textcolor{teal}{REAL}, then the value can be used also with 
\textcolor{teal}{x}(1) ,and the value can be set using \textcolor{teal}{x}(1)= or \textcolor{teal}{x}(1,1)=.This is useful when working with matrices 
with varying sizes, including matrix with one element. The new version has function bspline() wich allows 
to use bspile basis functions as regressor variables in linear regression. The manual now describes 
now function \textcolor{VioletRed}{nonlin}() for nonlinear regression and function \textcolor{VioletRed}{varcomp}() which can be used to 
compute variance and covariance components. 
 
Version 6.11.2024. Some bugs are corrected and some additions made. it is not tested whether all exaples can be run. 
This version is put into Github so that Reetta can start testing with newest version. 
Version 8.8.2024 A bug in making active set was corrected. This bug caused that some schedules which should be 
...
Version 17.6, 2024 Github folder Stem contains data and the script needed to compute results in paper: Juha Lappi (2024) 'From Finnish Assortment Pricing to Market 
Economy Using Prices for Sawn Wood and 
...
Version 5. 11. 2023. Functions stemopt and stemcurve are reported. Stemopt can be used to optimize bucking or to write 
...
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
\subsection{\textcolor{VioletRed}{system}() executes Windows commands}\index{system()} 
\label{systemcom} 
...
\end{note} 
\subsection{\textcolor{VioletRed}{macro}() makes shortcuts for macros}\index{macro()} 
\label{macro} 
...
\label{inpuwhere} 
\subsection{\textcolor{Red}{;now}() is done immediately}\index{;now} 
\label{inpunow} 
...
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
\subsubsection{\textcolor{Red}{;list}(part\%?) \textcolor{teal}{LIST} of objects having common part in name}\index{;list} 
\label{inpusub2} 
...
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
\subsection{\textcolor{VioletRed}{protect}() protects object from being deleted}\index{protect()} 
\label{protect} 
...
\subsection{\textcolor{VioletRed}{unprotect}() unprotects ptotected objects.}\index{unprotect()} 
\label{unprotect} 
...
\begin{itemize} 
If argument is \textcolor{teal}{REAL} then mean and sum are eqaul to the argument, ans sd and var are zero. 
...
\label{sum} 
	! 
\section{ Derivatives \textcolor{VioletRed}{der}()}\index{der()} 
\label{der} 
...
\begin{changemargin}{3cm}{0cm} 
\noindent  \textcolor{teal}{Arg1} is the argument),\textcolor{teal}{Arg2} is the mean (default 0), \textcolor{teal}{Arg3} is the standard deviation 
...
\subsection{\textcolor{VioletRed}{interpolate}() Linear or quadratic interpolation}\index{interpolate()} 
\label{interpolate} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
...
\subsubsection{\textcolor{VioletRed}{list2sym}() makes \textcolor{teal}{LIST} of symbolic constants from another \textcolor{teal}{LIST}.}\index{list2sym()} 
\label{list2sym} 
...
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
\subsubsection{\textcolor{VioletRed}{namenum}() and namenum\%() take numbers from object name}\index{namenum()} 
\label{namenum} 
...
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
\subsection{\textcolor{VioletRed}{difference}() Difference of LISTs or removes an object from \textcolor{teal}{LIST}}\index{difference()} 
\label{difference} 
...
\item  \textcolor{teal}{arg} is STEMCURVE => len=the number of height diamter pairs 
\end{itemize} 
...
\begin{changemargin}{3cm}{0cm} 
\noindent  Values to be put into \textcolor{teal}{ILIST}, or the dimension 
...
\begin{changemargin}{3cm}{0cm} 
\noindent  Values to be put into \textcolor{teal}{ILIST} when dimension is determined as the 
only argument 
\end {changemargin} 
\hrule 
\vspace{0.2cm} 
...
\subsection{\textcolor{VioletRed}{close}() Closes  files}\index{close()} 
\label{close} 
...
the arguments. 
\end{note} 
\subsection{\textcolor{teal}{Printresult} controls printing of the result} 
\label{printresult} 
...
\end{itemize} 
\begin{note} 
After the first version in 2025 all elements of a \textcolor{teal}{MATRIX} can be se using A()= value. 
Earlier this could be done using A=value. Now A=value makes A into \textcolor{teal}{REAL}. 
\end{note} 
\begin{note} 
Note 
...
\end{note} 
\subsection{\textcolor{VioletRed}{vector}() Makes a column vector of arguments which can be \textcolor{teal}{REAL} or \textcolor{teal}{MATRIX} or \textcolor{teal}{LIST}.}\index{vector()} 
\label{vector} 
\subsection{flip() Reversers order of matrix element} 
\label{flip} 
...
\label{cumsum} 
\textcolor{VioletRed}{cumsum}(\textcolor{teal}{MATRIX}) generates a \textcolor{teal}{MATRIX} with the same dimensions as the argument, 
...
\end{note} 
\subsection{covmatrix() Covariance matrix from correlation matrix} 
\label{covmatrix} 
This is inverse function of \textcolor{VioletRed}{corrmatrix}() making covariance matrix from correlation 
...
\noindent Output \tabto{3cm}  1 \tabto{5cm}   \textcolor{teal}{MATRIX} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  matrix having variances on the diagonal 
...
\noindent Args  \tabto{3cm} 1 \tabto{5cm}   \textcolor{teal}{MATRIX} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  symmetric correlation matrix. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{sd} \tabto{3cm} N|1  \tabto{5cm}   \textcolor{teal}{MATRIX}  \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent Vector of standard deviations 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{var} \tabto{3cm} N|1  \tabto{5cm}  \textcolor{teal}{MATRIX} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Vector of variances. 
\end {changemargin} 
\hrule 
\vspace{0.2cm} 
\begin{note} 
Either \textcolor{blue}{sd->} or \textcolor{blue}{var->} is required 
\end{note} 
\begin{note} 
Correlations greater than 1 are turned into 1 and correaltions less than -1 
are turned into -1 
\end{note} 
\begin{note} 
symmetry of the correaltion matrix is not tested 
\end{note} 
\begin{note} 
The diagonal of the correlation matrix is ignored. 
\end{note} 
\begin{note} 
Output can be the same as the input 
\end{note} 
\begin{note} 
\textcolor{blue}{sd->} and \textcolor{blue}{var->} require \textcolor{teal}{MATRIX} argument. Numeric values can be given 
using \textcolor{blue}{sd->}\textcolor{VioletRed}{vector}(1,3,4) 
\end{note} 
\subsection{matrixborder) puts row above a \textcolor{teal}{MATRIX} or column on the left} 
\label{matrixborder} 
...
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
\subsection{\textcolor{VioletRed}{fill}() fills empty (zero) cells in a matrix}\index{fill()} 
\label{fill} 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{extra} \tabto{3cm} 0-999 \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  extra variables put into the data, e.g., \textcolor{teal}{Regf} and \textcolor{teal}{Resid} which will be 
updated automatically after regf() or \textcolor{VioletRed}{nonlin}(). 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
...
\noindent \textcolor{blue}{filter} \tabto{3cm} 1 \tabto{5cm}  Code \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent The condition used to accept observations into the new data. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{reject} \tabto{3cm} 1 \tabto{5cm}  Code \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent The condition used to rject observations from the new data. 
\end {changemargin} 
\hrule 
\vspace{0.2cm} 
\begin{note} 
...
\begin{changemargin}{3cm}{0cm} 
\noindent The number of observations in class is stored in vector Class\%nobsw if 
expand is not present and in Class\%\%nobsw if \textcolor{blue}{expand->} is present 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{w} \tabto{3cm} -1|0 \tabto{5cm}    \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Deviations from the class means are computed to w vectors. 
\end {changemargin} 
\hrule 
\vspace{0.2cm} 
\begin{note} 
...
, and a \textcolor{teal}{DATA} is gone through obsevation by observation. It is possible to store 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{keep} \tabto{3cm} N|1-  \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Variables in the data matrix whose values are changed 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
...
\vspace{0.2cm} 
\begin{note} 
Before March 2025 the function contained option maketrans which could make nonintended 
changes in the data matrix as new values of all output variables were put in to the  data matrix if the 
variables were there already. Now \textcolor{blue}{keep->} specifies waht variables are changed in the data matrix 
\end{note} 
\color{Green} 
\begin{verbatim} 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{rmse}  \tabto{3cm} -1,99 \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  rmses are stored 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
...
\label{regr} 
Ordinary or stepwise linear regression can be computed using \textcolor{VioletRed}{regr}(). 
...
\begin{changemargin}{3cm}{0cm} 
\noindent  if \textcolor{blue}{corr->} is present \textcolor{VioletRed}{regr}() generated matrix ]output\%corr[ for 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{extra} \tabto{3cm}  \tabto{5cm}   \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  If option is given, then the values of the regression function are not stored in to the data matrix columns 
\textcolor{teal}{Regf} and \textcolor{teal}{Resid} even if these are present. This allows making several new regressions for the residuals of 
an earlier regression. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{any} \tabto{3cm} -1|1 \tabto{5cm}   \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  If data contains \textcolor{teal}{Regf} and \textcolor{teal}{Resid} then these variables are computed into data matrix also for 
observations rejected due to \textcolor{blue}{filter->} or \textcolor{blue}{reject->} (and not due to missing values indicated 
...
coluns can reserved with \textcolor{blue}{extra->} option in \textcolor{VioletRed}{data}() or in \textcolor{VioletRed}{newdata}() 
\end{note} 
\begin{note} 
...
\end{note} 
\begin{note} 
Even if the correlation matrix of parameter estimated would be singular, 
...
\begin{itemize} 
\item   \textcolor{VioletRed}{coef}(\textcolor{teal}{re},xvar) = coefficient of variable xvar. If xvar is 
...
\item  \textcolor{VioletRed}{se}(\textcolor{teal}{re},xvar) standard error of a coefficient. If xvar is All, 
...
\end{itemize} 
\subsection{bspline() Definiton of bspline} 
\label{bspline} 
The function makes a BSPLINE object which can generate bspline basis functions. The bspline basis functions 
are generated by giving output(\textcolor{teal}{x}), where \textcolor{teal}{x} is the argument wich must be between the first and last knot (icnlusive). 
The bspline functions can be used as regressor arguments of \textcolor{VioletRed}{regr}() by including 
@output\%list to the argument list. \textcolor{blue}{noint->} option must be included. 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent output \tabto{3cm} 1 \tabto{5cm}  BSPLINE \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  BSPLINE object. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{arg} \tabto{3cm}  3-N \tabto{5cm}   \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Knots of the bspline. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent Degree \tabto{3cm} N|1 \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  degree of the spline. Default is 2, maximum is 3. 
\end {changemargin} 
\hrule 
\vspace{0.2cm} 
\begin{note} 
If the same value is repeated, the continuity is dropped with one, 
...
\end{note} 
\begin{note} 
The basis functions add up to one, which means that the intercept is implicitly included in the model, 
Thus the \textcolor{VioletRed}{regr}() function needs to have \textcolor{blue}{noint->} option. If there are more than one bplines as 
arguments some functions must be dropped in a way to be explained later. 
\end{note} 
\begin{note} 
If there are knot sections which do not contain any data points, no error occurs, but \textbf{Jlp22} 
tells that some regressors are always zero and are dropped automatically. 
\end{note} 
\singlespacing 
\begin{example}[bsplineex]x\\ 
\label{bsplineex} 
\noindent \textcolor{teal}{Window}='400,400'\\ 
font=\,']Arial,12['\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{draw}(\textcolor{blue}{func->}(10*\textcolor{teal}{x}/(15+2*\textcolor{teal}{x}+0.5*\textcolor{teal}{x}*\textcolor{teal}{x})+5+2*(\textcolor{teal}{x}.gt.40)),\textcolor{teal}{x}->\textcolor{teal}{x},\textcolor{blue}{xrange->}(0,100),\textcolor{blue}{append->},\textcolor{blue}{color->}\textcolor{teal}{Green},\textcolor{blue}{width->}2,\textcolor{blue}{sho 
 
xm=\textcolor{VioletRed}{matrix}(100,\textcolor{blue}{do->})\\ 
tr=\textcolor{VioletRed}{trans}()\\ 
f=(10*\textcolor{teal}{x}/(15+2*\textcolor{teal}{x}+0.5*\textcolor{teal}{x}*\textcolor{teal}{x})+5+2*(\textcolor{teal}{x}.gt.40))\\ 
\textcolor{teal}{y}=f+\textcolor{VioletRed}{rann}(0,0.3)\\ 
/                                                       \\ 
dat=\textcolor{VioletRed}{newdata}(xm,\textcolor{blue}{read->}\textcolor{teal}{x},\textcolor{blue}{maketrans->}tr)\\ 
\textcolor{VioletRed}{stat}(\textcolor{blue}{min->},\textcolor{blue}{max->})\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{plotyx}(\textcolor{teal}{y},\textcolor{teal}{x},\textcolor{blue}{show->}0,\textcolor{blue}{append->})\\ 
cl=\textcolor{VioletRed}{classify}(\textcolor{teal}{y},\textcolor{teal}{x}->\textcolor{teal}{x},\textcolor{blue}{dx->}5)\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{drawclass}(cl,\textcolor{blue}{se->},\textcolor{blue}{color->}\textcolor{teal}{Blue},\textcolor{blue}{append->},\textcolor{blue}{width->}2,\textcolor{blue}{show->}0)\\ 
fig2=\textcolor{teal}{fig}\\ 
bs=bspline(0,5,15,40,100)\\ 
tr=\textcolor{VioletRed}{trans}()\\ 
bs(\textcolor{teal}{x})\\ 
xge40=\textcolor{teal}{x}.gt.40\\ 
\textcolor{VioletRed}{if}(done)f=re()\\ 
/                                    \\ 
done=0\\ 
re=\textcolor{VioletRed}{regr}(\textcolor{teal}{y},@bs\%list,xge40,\textcolor{blue}{noint->},\textcolor{blue}{trans->}tr)\\ 
done=1\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{draw}(\textcolor{blue}{func->}tr(f),\textcolor{teal}{x}->\textcolor{teal}{x},\textcolor{blue}{xrange->}(0,100),\textcolor{blue}{append->},\textcolor{blue}{show->}0,\textcolor{blue}{color->}\textcolor{teal}{Red},\textcolor{blue}{width->}2)\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{drawline}(45,55,6.4,6.4,\textcolor{blue}{color->}\textcolor{teal}{Green},\textcolor{blue}{width->}2,\textcolor{blue}{append->},\textcolor{blue}{show->}0,\textcolor{blue}{label->}'\,true"font"')\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{drawline}(45,55,6.1,6.1,\textcolor{blue}{color->}\textcolor{teal}{Blue},\textcolor{blue}{width->}2,\textcolor{blue}{append->},\textcolor{blue}{show->}0,\textcolor{blue}{label->}'\,classified"font"')\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{drawline}(45,55,5.8,5.8,\textcolor{blue}{color->}\textcolor{teal}{Red},\textcolor{blue}{width->}2,\textcolor{blue}{append->},\textcolor{blue}{show->}0,\textcolor{blue}{label->}'\,estimated"font"')\\ 
\textcolor{VioletRed}{show}(\textcolor{teal}{fig},\textcolor{blue}{xlabel->}'\textcolor{teal}{x}"font"',\textcolor{blue}{title->}'Smooth\,bspline\,+\,jump"font"',\textcolor{blue}{continue->}fcont)\\ 
{\color{ForestGreen}!***spline is not continuous at 40}\\ 
bs2=bspline(0,5,15,40,40,40,100)\\ 
tr2=\textcolor{VioletRed}{trans}()\\ 
bs2(\textcolor{teal}{x})\\ 
\textcolor{VioletRed}{if}(done)f=re2()\\ 
/                                     \\ 
done=0\\ 
re2=\textcolor{VioletRed}{regr}(\textcolor{teal}{y},@bs2\%list,\textcolor{blue}{noint->},\textcolor{blue}{trans->}tr2)\\ 
done=1\\ 
fig2=\textcolor{VioletRed}{draw}(\textcolor{blue}{func->}tr2(f),\textcolor{teal}{x}->\textcolor{teal}{x},\textcolor{blue}{xrange->}(0,100),\textcolor{blue}{append->},\textcolor{blue}{show->}0,\textcolor{blue}{color->}\textcolor{teal}{Red},\textcolor{blue}{width->}2)\\ 
fig2=\textcolor{VioletRed}{drawline}(45,55,6.4,6.4,\textcolor{blue}{color->}\textcolor{teal}{Green},\textcolor{blue}{width->}2,\textcolor{blue}{append->},\textcolor{blue}{show->}0,\textcolor{blue}{label->}'\,true"font"')\\ 
fig2=\textcolor{VioletRed}{drawline}(45,55,6.1,6.1,\textcolor{blue}{color->}\textcolor{teal}{Blue},\textcolor{blue}{width->}2,\textcolor{blue}{append->},\textcolor{blue}{show->}0,\textcolor{blue}{label->}'\,classified"font"')\\ 
fig2=\textcolor{VioletRed}{drawline}(45,55,5.8,5.8,\textcolor{blue}{color->}\textcolor{teal}{Red},\textcolor{blue}{width->}2,\textcolor{blue}{append->},\textcolor{blue}{show->}0,\textcolor{blue}{label->}'\,estimated"font"')\\ 
\textcolor{VioletRed}{show}(fig2,\textcolor{blue}{xlabel->}'\textcolor{teal}{x}"font"',\textcolor{blue}{title->}'Bpline\,not\,continuous\,at\,40"font"',\textcolor{blue}{continue->}fcont)\\ 
\end{example} 
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
...
Ordinary least squares regression with modified Gauss-Newton method can be computed using \textcolor{VioletRed}{nonlin}(). 
...
\noindent output \tabto{3cm} 1 \tabto{5cm}   \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Name used to name associated objects generated. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{arg} \tabto{3cm}  2 \tabto{5cm}  REALV \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  \textcolor{teal}{y}-variable and name of the function in \textcolor{teal}{TRANS} given in \textcolor{blue}{trans->} 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{par} \tabto{3cm} 1-  \tabto{5cm}  REALV \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  parameters of the function. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{initial} \tabto{3cm} -1|\#of par \tabto{5cm}   \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent Initial values of parameters. Default the current values. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{parmin} \tabto{3cm} -1|\#of par \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Optional minimum values of parameters. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{parmax} \tabto{3cm} -1|\#of par \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Optional maximum values of parameters. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{variance} \tabto{3cm} 1|0 \tabto{5cm}  CODE \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  If present the value of the code option gives the assumed 
residual variance for each observation. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{missing} \tabto{3cm}  \tabto{5cm}   \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent If present then observations having \textcolor{teal}{y}-value greater than or eqaul to 1.7e19 are rejected. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{step} \tabto{3cm} -1|1  \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Initial step length. Default 1. 
intercept is not considered. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{maxiter} \tabto{3cm} -1|1 \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Maximum number of iterations, default 100. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{slow} \tabto{3cm} -1|1 \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Criterion for stopping iterations due to slow improvement, default -0.1. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
...
\begin{changemargin}{3cm}{0cm} 
\noindent  Tolerance for checkking improvement, deafault 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{var} \tabto{3cm} -1|0 \tabto{5cm}    \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  if \textcolor{blue}{var->} is present \textcolor{VioletRed}{nonlin}() generated matrix ]output\%var[ for 
the variance-covariance matrix of the coeffcient estimates. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
...
\noindent \textcolor{blue}{corr} \tabto{3cm} -1|0 \tabto{5cm}    \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  if \textcolor{blue}{corr->} is present \textcolor{VioletRed}{nonlin}() generated matrix ]output\%corr[ for 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{loglike} \tabto{3cm} -1|1 \tabto{5cm}  CODE \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Log likelihood function whose value is printed but not otherwise utlized. 
The user can use it to make loglikelihood tests manually. 
\end {changemargin} 
\hrule 
\vspace{0.2cm} 
\begin{note} 
The transforamtion given in \textcolor{blue}{trans->} needs to have two consecutive rows \\ 
\textcolor{VioletRed}{der}(par1,...,parn)\\ 
f= function\_definition\\ 
All the parameters given in \textcolor{blue}{par->} need to be included in \textcolor{VioletRed}{der}(), but \textcolor{VioletRed}{der}() can include also 
other parameters. Thus it possible to estimate a closely overparameterized model in phases 
just modifying \textcolor{blue}{par->} 
\end{note} 
\begin{note} 
\textcolor{VioletRed}{der}(par1,...,parn) before f= produces derivative variables f\par1, ..., f\parn whose 
behavior can be studies using the transformation given in \textcolor{blue}{trans->} 
\end{note} 
\begin{note} 
Even if the correlation matrix of parameter estimated would be singular, 
e.g. due to a corelation one, the estimation may succeed if the intercept is not 
...
\end{note} 
\begin{note} 
\textcolor{blue}{step->} needs to be given only if the the deafult value one produces arithmetic errors or 
...
\end{note} 
\begin{note} 
If p is the probability of a binary variable and \textcolor{teal}{y} is the observed valu, either 1 or 0, 
then \textcolor{blue}{loglike->}(\textcolor{teal}{y}*\textcolor{VioletRed}{log}(p)+(1-\textcolor{teal}{y})*\textcolor{VioletRed}{log}(1-p)) could be used. 
\end{note} 
\singlespacing 
\begin{example}[nonlinex]\\ 
\label{nonlinex} 
\noindent {\color{ForestGreen}***generate vector 1...100}\\ 
xm=\textcolor{VioletRed}{matrix}(100,\textcolor{blue}{do->})\\ 
tr=\textcolor{VioletRed}{trans}()\\ 
f=10*\textcolor{teal}{x}/(\textcolor{teal}{x}+5)-6\\ 
\textcolor{teal}{y}=f+\textcolor{VioletRed}{rann}(0,2)\\ 
/                                                     \\ 
dat=\textcolor{VioletRed}{newdata}(xm,\textcolor{blue}{read->}\textcolor{teal}{x},\textcolor{blue}{maketrans->}tr)\\ 
	!                                                                                                                \\ 
\textcolor{VioletRed}{stat}(\textcolor{blue}{min->},\textcolor{blue}{max->})\\ 
\textcolor{teal}{x}\%min,\textcolor{teal}{x}\%max;\,\,{\color{ForestGreen}!these are utilized in classify}\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{plotyx}(\textcolor{teal}{y},\textcolor{teal}{x},\textcolor{blue}{show->}0)\\ 
cl=\textcolor{VioletRed}{classify}(\textcolor{teal}{y},\textcolor{teal}{x}->\textcolor{teal}{x},\textcolor{blue}{dx->}5)\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{drawclass}(cl,\textcolor{blue}{se->},\textcolor{blue}{sd->},\textcolor{blue}{color->}\textcolor{teal}{Blue},\textcolor{blue}{append->},\textcolor{blue}{show->}0)\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{draw}(\textcolor{blue}{func->}(10*\textcolor{teal}{x}/(\textcolor{teal}{x}+5)-6),\textcolor{teal}{x}->\textcolor{teal}{x},\textcolor{blue}{xrange->}(0,100),\textcolor{blue}{append->},\textcolor{blue}{width->}2,\textcolor{blue}{show->}0)<\\ 
 
tr=\textcolor{VioletRed}{trans}()\\ 
\textcolor{VioletRed}{der}(a,\textcolor{teal}{b},c)\\ 
f=a*\textcolor{teal}{x}/(\textcolor{teal}{x}+\textcolor{teal}{b})-c\\ 
/                                                                  \\ 
a,\textcolor{teal}{b},c=6,3,-3\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{draw}(\textcolor{blue}{func->}(a*\textcolor{teal}{x}/(\textcolor{teal}{x}+\textcolor{teal}{b})-c),\textcolor{teal}{x}->\textcolor{teal}{x},\textcolor{blue}{xrange->}(0,100),\textcolor{blue}{append->},\textcolor{blue}{width->}2,\textcolor{blue}{color->}\textcolor{teal}{Green},\textcolor{blue}{show->}0)\\ 
nl=\textcolor{VioletRed}{nonlin}(\textcolor{teal}{y},f,\textcolor{blue}{trans->}tr,\textcolor{blue}{corr->},\textcolor{blue}{var->},\textcolor{blue}{par->}(a,\textcolor{teal}{b},c))\\ 
nl\%corr;\\ 
nl\%var;\\ 
\textcolor{teal}{fig}=\textcolor{VioletRed}{draw}(\textcolor{blue}{func->}(a*\textcolor{teal}{x}/(\textcolor{teal}{x}+\textcolor{teal}{b})-c),\textcolor{teal}{x}->\textcolor{teal}{x},\textcolor{blue}{xrange->}(0,100),\textcolor{blue}{append->},\textcolor{blue}{width->}2,\textcolor{blue}{color->}\textcolor{teal}{Red},\textcolor{blue}{show->}0)\\ 
\textcolor{teal}{Window}='500,500'\\ 
font=']Arial,12['\\ 
\textcolor{VioletRed}{show}(\textcolor{teal}{fig},\textcolor{blue}{xlabel->}'\textcolor{teal}{x}"font"',\textcolor{blue}{ylabel->}'f"font"',\textcolor{blue}{title->}'Estimation\,of\,rectangular\,hyperbola"font"')\\ 
\end{example} 
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
...
\noindent output \tabto{3cm} 1 \tabto{5cm}   \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Name used to name associated output object 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
...
\noindent \textcolor{blue}{class} \tabto{3cm} 1 \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Class variable. Class changes when varaible gets different value. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{cov} \tabto{3cm} -1|0 \tabto{5cm}    \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  if \textcolor{blue}{cov->} is present \textcolor{VioletRed}{varcomp}() generates matrix ]output\%varb[ for 
...
\noindent \textcolor{blue}{corr} \tabto{3cm} -1|0 \tabto{5cm}    \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  if \textcolor{blue}{corr->} is present \textcolor{VioletRed}{varcomp}() generates matrix ]output\%corrb[ for 
...
\noindent \textcolor{blue}{func} \tabto{3cm} -1|1 \tabto{5cm}  CODE \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  If there are no arguments, the variance components are computed for the variable 
generated with \textcolor{blue}{func->}. 
\end {changemargin} 
\hrule 
\vspace{0.2cm} 
\begin{note} 
Using \textcolor{VioletRed}{eigen}() for the generated variance matrices, we can do principal component analysis both for 
the between class and within class variance matrices. Sere the example. 
\end{note} 
\singlespacing 
\begin{example}[varcompex]Variance components\\ 
\label{varcompex} 
\noindent classes=50\\ 
bef=\textcolor{VioletRed}{matrix}(classes,4)\\ 
col=\textcolor{VioletRed}{matrix}(classes)\\ 
col=\textcolor{VioletRed}{rann}()\\ 
bef(All,1)=col\\ 
col=\textcolor{VioletRed}{rann}()\\ 
...
\textcolor{VioletRed}{mean}(bef);\\ 
\textcolor{VioletRed}{sd}(bef);\\ 
nobw=\textcolor{VioletRed}{matrix}(classes)\\ 
tr=\textcolor{VioletRed}{trans}()\\ 
\textcolor{VioletRed}{do}(i,1,classes)\\ 
nobw(i)=\textcolor{VioletRed}{int}(\textcolor{VioletRed}{max}(\textcolor{VioletRed}{rann}(5,2),1))\\ 
\textcolor{VioletRed}{enddo}\\ 
/                           \\ 
\textcolor{VioletRed}{call}(tr)\\ 
nobtot=\textcolor{VioletRed}{sum}(nobw);\\ 
datm=\textcolor{VioletRed}{matrix}(nobtot,5)\\ 
tr=\textcolor{VioletRed}{trans}()\\ 
iob=0\\ 
\textcolor{VioletRed}{do}(i,1,classes)\\ 
 
\textcolor{VioletRed}{do}(j,1,nobw(i))\\ 
iob=iob+1\\ 
datm(iob,1)=i\\ 
e1=0.5*\textcolor{VioletRed}{rann}()\\ 
datm(iob,2)=bef(i,1)+e1\\ 
e2=0.4*e1+0.2*\textcolor{VioletRed}{rann}()\\ 
datm(iob,3)=bef(i,2)+e2\\ 
e3=0.1*e1+0.2*e2+0.2*\textcolor{VioletRed}{rann}()\\ 
datm(iob,4)=bef(i,3)+e3\\ 
e4=0.1*e1+0.2*e2+0.1*e3+0.2*\textcolor{VioletRed}{rann}()\\ 
datm(iob,5)=bef(i,4)+e4\\ 
\textcolor{VioletRed}{enddo}\\ 
\textcolor{VioletRed}{enddo}\\ 
/                           \\ 
\textcolor{VioletRed}{call}(tr)\\ 
dat=\textcolor{VioletRed}{newdata}(datm,\textcolor{blue}{read->}(stand,\textcolor{teal}{y1}...y4))\\ 
\textcolor{VioletRed}{stat}()\\ 
vc=\textcolor{VioletRed}{varcomp}(\textcolor{teal}{y1}...y4,\textcolor{blue}{class->}stand,\textcolor{blue}{cov->},\textcolor{blue}{corr->})\\ 
vclist=\textcolor{Red}{;list}(vc\%?);\\ 
@vclist;\\ 
eib=\textcolor{VioletRed}{eigen}(vc\%varb)\\ 
eib\%values;\\ 
eib\%matrix;\\ 
eiw=\textcolor{VioletRed}{eigen}(vc\%varw)\\ 
eiw\%values;\\ 
eiw\%matrix;\\ 
\end{example} 
\vspace{-7mm} \rule{5cm}{0.1pt} 
...
\noindent \textcolor{blue}{any} \tabto{3cm} -1|0 \tabto{5cm}    \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent Indicates that each value of the \textcolor{teal}{x}-variables forms a separate class. 
...
arguments or is not given then variables \textcolor{teal}{x}\%min and \textcolor{teal}{x}\%max are used if they exist. If they do not exist an error occurs. 
Note that these variables can be 
generate with \textcolor{VioletRed}{stat}(\textcolor{blue}{min->},\textcolor{blue}{max->}). 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{min}  \tabto{3cm}  -1|0|1  \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  \textcolor{blue}{min->} indicates that row vector Output\%min is generated. First elements 
tell the class wise minimums, and the last element the total minimum. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{max}  \tabto{3cm}  -1|0|1  \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  \textcolor{blue}{min->} indicates that row vector Output\%max is generated. First elements 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{err}  \tabto{3cm}  \tabto{5cm}   \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  If \textcolor{blue}{err->} is present, classisyy interprets that the classified variable is residual an print 
...
\url{https://\textcolor{teal}{gnuplot}.sourceforge.net/docs_4.2/node356.html}\\ 
...
\label{plotyxex} 
\noindent {\color{ForestGreen}** plotyx() (line  16019 file c:/jlp22/j.f90)}\\ 
...
\begin{changemargin}{3cm}{0cm} 
\noindent  Range for the \textcolor{teal}{x}-variable. If the option does not exist or there are no arguments \textbf{Jlp22} tries to 
...
fi=\textcolor{VioletRed}{draw}(\textcolor{blue}{func->}\textcolor{VioletRed}{pdf}(\textcolor{teal}{x},0,\textcolor{VioletRed}{rmse}(reg)),\textcolor{teal}{x}->\textcolor{teal}{x},\textcolor{blue}{xrange->},\textcolor{blue}{append->},\textcolor{blue}{continue->}fcont)\,{\color{ForestGreen}! xrange comes from stat()}\\ 
\end{example} 
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
\begin{note} 
...
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{sort} \tabto{3cm} N|1 \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  Option indicates taht \textcolor{teal}{x}-values are sorted in increasing order. 
\end{changemargin} 
\vspace{0.3cm} 
\hrule 
\vspace{0.3cm} 
\noindent \textcolor{blue}{until} \tabto{3cm} |1 \tabto{5cm}  \textcolor{teal}{REAL} \tabto{7cm} 
\begin{changemargin}{3cm}{0cm} 
\noindent  \textcolor{teal}{x}-\textcolor{teal}{y} pairs are process only up the number given in the option. 
\end {changemargin} 
\hrule 
\vspace{0.2cm} 
\begin{note} 
...
\end{note} 
\begin{note} 
If \textcolor{teal}{x}-values are sorted, the same figure would ppear if \textcolor{teal}{x}-values would be sorted in decreasing order. 
\end{note} 
\begin{note} 
...
\section{Stems: stem curves and optimization of bucking} 
\label{spline} 
...
\label{laaspoly} 
To be reported later. 
\subsection{\textcolor{VioletRed}{stemcurve}() defines a stem curve}\index{stemcurve()} 
\label{stemcurve} 
...
the number of used height diameter pairs can be obtained with \textcolor{VioletRed}{len}() 
...
\end{note} 
\subsection{\textcolor{VioletRed}{stemopt}() optimizes bucking or writes bucking schedules}\index{stemopt()} 
\label{stemopt} 
...
\textcolor{blue}{func->} code option determining the share of top cross-sectional are going to sawn wood.\\ 
...
\singlespacing 
\begin{example}[stemoptex]moptex\\ 
\label{stemoptex} 
...
\vspace{-7mm} \rule{5cm}{0.1pt} 
\onehalfspacing 
\subsection{\textcolor{VioletRed}{info}() print some information of current parameters.}\index{info()} 
\label{info} 
\begin{note} 
