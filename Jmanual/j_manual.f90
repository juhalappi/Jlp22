...
!use koemod,only: koe2
	...
	use j_mod, only: j_nfunctions_,j_noptions_,j_functions,j_options
...
!	logical,dimension(maxsections)::subsection
	...
	character*200,dimension(maxsec,maxsections)::sheaders
	character*200,dimension(maxhead,j_noptions_)::macroheaders
	integer,dimension(j_noptions_)::lmacroheaders=0
	character*180,dimension(maxlfunc)::body
...
!	logical ::inbox=.false.
	...
	character*20 labe	
	integer p1
	character*180 optline
	character*1 ::ch
	logical test
	logical saa
	logical::frommacro=.false.
	integer::lfrommacro
	character*100 begintabular
	logical intabular
...
103 write(6,*)'<54>',line(ifirst(2):ilast(2))
...
!		isinput=.false.
	...
	elseif(index(line(1:le),'outfile').gt.0)then
		 write(6,*)'deleting',line(ifirst(2):le),' abort if you want'
		! read(5,*)ii
		if(open20)close(20)
...
!	open(30,file='jexamples.txt',status='REPLACE',action='WRITE')
	...
	subroutine writeincl()
		write(20,*)'exfile=thisfile()'
		do ine=1,nexample
			write(20,'(a)')exlabel(ine)(1:lexlabel(ine))//"=';incl(exfile,from->~"// &
			exlabel(ine)(1:lexlabel(ine))//"~)'"
		enddo
		write(20,*)"AGAIN=';incl(exfile)'"
		write(20,*)';if(.not.EVERYTHING);return'
		do ine=1,nexample
			write(20,'(a)')' '
...
!inmacro
	...
	if(isoption)then
	 ...
	 if(.not.inlatex)then
		...
		call putsec('\begin{tabular}{ m{.10\textwidth}  m{.05\textwidth}m{.10\textwidth}p{.70\textwidth}}')
...
!		call words(line,ifirst(2),le,nwords,ifirst,ilast,iss)
	...
	if(isexample)then
		...
		write(6,*)'<88>',exlabel(nexample)(1:lelabe),'**',exlabel(nexample)(lelabe+1:le)
...
integer function j_len_trim(inp)   !like len-trim but returns zero aslo when line consists of tabs
	...
	subroutine call(iob,io)
		write(6,*)'type2'
	end
	subroutine result(iob,io)
		write(6,*)'type3'
	end
	...
	subroutine which(iob,io)
		write(6,*)'type6'
	end
	subroutine goto(iob,io)
		write(6,*)'type7'
	end
	subroutine min(iob,io)
		write(6,*)'type8'
	end
	subroutine abs(iob,io)
		write(6,*)'type9'
	end
	subroutine sign(iob,io)
		write(6,*)'typea'
	end
	subroutine nint(iob,io)
		write(6,*)'typeb'
	end
	subroutine sqrt(iob,io)
		write(6,*)'typec'
	end
	subroutine len(iob,io)
		write(6,*)'typed'
	end
	subroutine read(iob,io)
		write(6,*)'typee'
	end
	subroutine data(iob,io)
		write(6,*)'typedta'
	end