...
subroutine jlp(iob,io)   ! %%jlp  !!!!****************************************************
	...
	if(p_intapp.and.p_nz.gt.0)then
		...
		j_err=.true.;return
	endif !if(p_intapp.and.p_nz.gt.0)   4788
...
!	write(6,*)'p_ibaunit, bef istart',p_ibaunit
	...
	if(p_nshow.gt.0)then
		...
		enddo !j=1,p_nshowin   4883
		if(j_err)return
 
	endif !if(p_nshow.gt.0)   4880
	...
	endif !if(p_fpresent.or..true.)   4907
	...
	if(p_nz.gt.0.and. .not.p_zopt)then
		...
		j_err=.true.
		return
	endif !if(p_nz.gt.0.and. .not.p_zopt)   4952
...
!write(16,*)'newstart'
	...
	do i=1,p_nrow
		...
		if((p_ix(i).ne.0).or.p_fpresent)then   ! there is a x-variable in the row
			...
			if(p_xps(i).gt.p_rhs2(i).or.p_rhs(i).eq.j_ninf)then
				...
				p_rhscur(i)=p_rhs(i)
				p_lower(i)=.true.
			endif !if(p_xps(i).gt.p_rhs2(i).or.p_rhs(i).eq.j_ninf)   5023
			...
			if(p_lbou(i))then
				...
				p_rhscur(i)=p_rhs2(i)
				p_lower(i)=.false.
			endif !if(p_lbou(i))   5035
			p_rhsw(i)=p_rhscur(i)
		endif !if((p_ix(i).ne.0).or.p_fpresent)   5021
		if(p_rhs2(i).lt.p_rhs(i))then
			write(6,*)'*constraint ',i,' upper limit ',p_rhs2(i),' less than lower limit ',p_rhs(i)
			j_err=.true.
		endif !if(p_rhs2(i).lt.p_rhs(i))   5044
	enddo !i=1,p_nrow   5019
	if(j_err)return
	!	write(6,*)'rhscur',p_rhscur
	!	write(6,*)'rhsw',p_rhsw
	...
	if(p_nrow.eq.0.and..not.p_fpresent)then   ! no constraints, was optimized directly
		...
		return
	endif !if(p_nrow.eq.0.and..not.p_fpresent)   5084
	j_v(p_ivstartedjlp)=j_1 !!!!
	...
	if(p_idebug.eq.p_pivot.or.j_err)then
		...
		if(idebug1.ne.idebug2)then
			...
			p_idebug=p_idebug+1
		endif !if(idebug1.ne.idebug2)   5260
 
	endif !if(p_idebug.eq.p_pivot.or.j_err)   5248
	if(j_err)return
	...
	if(j_err)then
		...
		if(idebug1.ne.idebug2)then
			...
			p_idebug=p_idebug+1
		endif !if(idebug1.ne.idebug2)   5390
...
580	call zenter()
		...
		endif !if(p_ienter.ne.0)   5401
 
	endif !if(p_nz.gt.0)   5398
	...
	!	write(6,*)'kier ',p_kier
		...
		endif !if(p_kier.eq.p_kierout)   5449
		...
		unitloop: do p_unit=1,p_nunits
			...
			!	write(6,*)'ibaunit4 ',p_ibaunit
				goto 900
			endif !if(p_pivot.ne.ipivotstart)   5473
			ipivotstart=p_pivot
...
!write(6,*)'<57hei2',goto401
			...
			if(p_feasible.and.p_feasible1)then
				...
				p_feasible1=.false.
			endif !if(p_feasible.and.p_feasible1)   5504
	...
	778				p_ienter=0
					...
					if(p_ienter.ne.0)then
						call leaving()
						goto 778
					endif !if(p_ienter.ne.0)   5561
 
 
 
 
				endif !if(p_fpresent)   5553
 
				call senter()
 
 
			endif !if(p_activeunit(p_unit))   5552
			...
			if(p_ienter.eq.0)then
				if(p_unit.eq.p_unitprev)then
					...
					goto 900
				endif !if(p_unit.eq.p_unitprev)   5578
 
				cycle unitloop
 
			endif !if(p_ienter.eq.0)   5577
			!			if(p_ienter.ne.0)then
			...
			if(p_lr0.eq.p_nrow.and.p_pivot.gt.p_pivotold)then !due to a bug in Fletcher
				...
				p_pivotold=p_pivot
			endif !if(p_lr0.eq.p_nrow.and.p_pivot.gt.p_pivotold)   5594
	380				call updatejlp()
			if(j_err)then
				idebug1=j_v(j_ivdebug)
				call pullout(LWSLL1)
				...
				if(idebug1.ne.idebug2)then
					...
					p_idebug=p_idebug+1
				endif !if(idebug1.ne.idebug2)   5617
			...
			if(p_ienter.gt.0)then
				...
				if(p_lr0.eq.p_nrow.and.p_pivot.gt.p_pivotold)then !due to a bug in Fletcher
					...
					p_pivotold=p_pivot
				endif !if(p_lr0.eq.p_nrow.and.p_pivot.gt.p_pivotold)   5636
				!		endif !if(.not.cancel)   5005
				goto 380
				!	cycle inunitloop
 
			endif !if(p_ienter.gt.0)   5632
			!				cycle unitloop
		...
		enddo unitloop !tloop: do p_unit=1,p_nunits   5472
		...
		if(p_fpresent.and.p_feasible)then
			if(.not.p_factnow)then
				...
				if(nofact.gt.iterxkf)then
					...
					p_factnow=.true.
					nofact=0
				endif !if(nofact.gt.iterxkf)   5814
			else !if(.not.p_factnow)then
				!	write(6,*)'round,p_factnow put to false',p_kier
				!		p_factnow=.false.
			endif !if(.not.p_factnow)   5812
		endif !if(p_fpresent.and.p_feasible)   5811
 
		!write(17,*)'aftunitloop',kier,p_unit
	enddo kierloop !rloop: do p_kier=0,p_maxrounds   5431
...
subroutine tulostele3()
	...
	if(ipivot9.ne.p_pivot)then  !vois katsoa muutkin muutokset
		...
		if(p_feasible)then
			...
			endif !if(p_objfprev.ne.p_small)   6035
	...
	endif !if(ipivot9.ne.p_pivot)   6014
	!write(6,*)'<654exitkier'
...
subroutine commonopt(iob,io)
	...
	if(p_ivpopar.eq.0)then
		...
		endif !if(p_nrow.gt.0)   6144
		! else
		! j_v(j_o(p_ivpopar)%i2)=j0
	endif !if(p_ivpopar.eq.0)   6134
	...
	if(noptarg.eq.1)then
		...
		endif !if(p_nureport.le.0)   6209
 
	else !if(j_nargopt(iob,io,j_mreport).eq.1)then
		p_nureport=6
	endif !if(noptarg.eq.1)   6204
	!	write(6,*)'tassanyt'
	!p_p9=p_iprint.eq.9
	!	sparse=.false.
	...
	if(j_linkoption(iob,io,j_mrefac).gt.0)then
		...
		p_nrefac=1000  !
	endif !if(j_linkoption(iob,io,j_mrefac).gt.0)   6239
...
7777 format(a16,i5)
	...
	do i=1,p_nrow
		...
		endif !if(p_rhs(i).gt.p_rhs2(i))   6296
 
	enddo !i=1,p_nrow   6295
	if(j_err)return
...
subroutine getnvarz()
	...
	if(j_linkoption(iob,io,j_mdata).ge.0)then
		...
		j_err=.true.;return
	endif !if(j_linkoption(iob,io,j_mdata).ge.0)   6344
...
subroutine initjlp2(iob,io)
	...
	if(p_nnotareavars.gt.0)then
		...
		if(p_ivarea.lt.0)then
			...
			j_err=.true.
			return
		endif !if(p_ivarea.lt.0)   6622
	endif !if(p_nnotareavars.gt.0)   6620
...
subroutine initxdatawx()
	...
	do i=1,p_nterm
		...
		endif !if(p_istermx(i))   6735
	enddo !i=1,p_nterm   6734
	...
	do ival=1,p_nterm
		...
		endif !if(p_istermx(ival).and.iro.ne.irov)   6753
		irov=iro
	enddo !ival=1,p_nterm   6750
...
rowloop:	do iro=0,p_nrow
	...
	row2:		do iro2=0,iro-1
						...
						do jj2=1,p_ninrowx(iro)
							...
							if(p_xtermvar(iba+jj).eq.p_xtermvar(iba2+jj2))then
								if(p_xcoef(iba+jj).eq.p_xcoef(iba2+jj2))then
									...
									cycle row2
								endif !if(p_xcoef(iba+jj).eq.p_xcoef(iba2+jj2))   6801
							else
								cycle row2
							endif !if(p_xtermvar(iba+jj).eq.p_xtermvar(iba2+jj2))   6800
 
						enddo !jj2=1,p_ninrowx(iro)   6798
 
					enddo outer !er:	do jj=1,p_ninrowx(iro)   6797
					!!! now all variables and coefficients are equal
					p_ix(iro)=p_ix(iro2)
					cycle rowloop !write(6,*)'samma x',iro,p_irow0,'ix',p_itemp0
				endif !if(p_ninrowx(iro).eq.p_ninrowx(iro2))   6795
				iba2=iba2+p_ninrowx(iro2)
			enddo row2 !2:		do iro2=0,iro-1   6794
			p_ntemp0=p_ntemp0+1
		...
		endif !if(p_ninrowx(iro).gt.0)   6789
 
		iba=iba+p_ninrowx(iro)
	enddo rowloop !loop:	do iro=0,p_nrow   6787
	...
	if(p_ivarea.gt.0)then   ! area-> option present
		...
		if(p_nnotareavars.gt.0)then
			...
			endif !if(p_nxvararea.gt.0)   6900
...
subroutine initdomain()
	...
	if(p_ivdomaintrans.gt.0)then
		...
		enddo !j=1,j_o(ivinl)%i(1)   7005
	endif !if(p_ivdomaintrans.gt.0)   6998
	if(j_err)return
	...
	endif !if(p_domvars(1).eq.j_ivall)   7065
	...
	do ido=1,p_ndomvars
		...
		do iro=0,p_nrow
			if(p_ninrowx(iro).gt.0)then
				...
				endif !if(idomv.eq.p_domvars(ido))   7084
			endif !if(p_ninrowx(iro).gt.0)   7082
 
		enddo !iro=0,p_nrow   7081
		lkm=lkm+p_nixcu(ido)
	enddo !ido=1,p_ndomvars   7079
	...
	enddo !ido=p_ido1,p_ndomvars   7126
	...
	do j=0,p_nrow
		...
		endif !if(p_domvars(p_rowdomnum(j)).eq.j_ivall.and.p_ix(j).ne.0)   7178
	enddo !j=0,p_nrow   7177
	!	 p_row0=1
	!	if(p_rowcurx(1).eq.0)p_row0=2
...
subroutine initopt()
	...
	if(p_nz.gt.0)then  !p_nz=number of z-variables
		...
		do ii=1,p_nterminrow(0)
			...
			if(.not.p_istermx(ival))then
				...
				endif !if(p_maxo)   7290
			endif !if(.not.p_istermx(ival))   7288
		enddo !ii=1,p_nterminrow(0)   7286
		...
		do irow=1,p_nrow
			...
			do ii=1,p_nterminrow(irow)
				...
				endif !if(.not.p_istermx(ival))   7307
			enddo !ii=1,p_nterminrow(irow)   7305
 
		enddo !irow=1,p_nrow   7301
		!	else !P_xpreset
	...
	if(iobjtype.gt.0)then
		...
		if(p_nonlin)then
			...
			j_err=.true.
			return
		endif !if(p_nonlin)   7411
		p_maxo=.false.
		p_coefmax=-j_1
 
	endif !if(iobjtype.gt.0)   7407
...
subroutine initoptz()
	...
	if(p_zmatrix)then
		...
		do ii=1,p_nterminrow(0)
			...
			endif !if(p_maxo)   7559
			!endif !if(.not.p_istermx(ival))   5316
		enddo !ii=1,p_nterminrow(0)   7551
		...
		do irow=1,p_nrow
			...
			enddo !ii=1,p_nterminrow(irow)   7572
			iba=iba+p_nz
		enddo !irow=1,p_nrow   7568
	endif !if(p_zmatrix)   7512
	...
	do i=1,p_nrow
		if(p_lbou(i))then
			...
			p_rhscur(i)=p_rhs2(i)
			p_lower(i)=.false.
		endif !if(p_lbou(i))   7595
		!	p_rhsw(i)=p_rhscur(i)
	enddo !i=1,p_nrow   7594
	!		p_rhsw(i)=p_rhscur(i)
	p_lowerJ=p_lower
...
subroutine preopt()
		...
		do i=1,p_nunits
			...
			do k=1,p_ns(i)
				...
				do jj=p_i1,p_nrowcurx
					...
					endif !if(p_xps(j)+curx-curk.gt.p_rhs2(j))   7658
 
				enddo !jj=p_i1,p_nrowcurx   7654
				if(p_value.lt.p_valueopt)then
					p_valueopt=p_value
					kopt=k
 
				endif !if(p_value.lt.p_valueopt)   7665
				if(k.eq.p_keys(i))p_valuek=p_value
			enddo !k=1,p_ns(i)   7642
			if(p_valuek.gt.p_valueopt)then
				...
				enddo !jj=1,p_nrowcurx   7676
				p_keys(i)=kopt
				p_apusum=p_apusum+p_valuek-p_valueopt
 
			endif !if(p_valuek.gt.p_valueopt)   7672
		enddo !i=1,p_nunits   7630
	...
	if(p_iprint.ge.0)then
		...
		enddo !jj=1,p_nrowcurx   7702
	endif !if(p_iprint.ge.0)   7700
	p_objf=p_small
...
subroutine initxdatjlp()
	...
	use jmod, only: p_datamatx
	use jmod, only: p_nkeepx
	use jmod, only: p_keepc
	use jmod, only: p_datamatc
	use jmod, only: p_nkeepc
	use jmod, only: p_isdomain
	use jmod, only: p_ivunit
	use jmod, only: p_ivdomaintrans
	use jmod, only: p_ido1
	...
	do p_unit=1,p_nunits
		...
		if(p_isdomain)then  ! ndom is number of domains
			...
			do ido=p_ido1,p_ndomvars
				...
				endif !if(j_v(p_domvars(ido)).ne.j_0)   7811
 
 
			enddo !ido=p_ido1,p_ndomvars   7808
 
			!	if(p_p8)write(6,*)'<11446 domainbits',p_domainbits
			p_idomba=p_idomba+1
 
 
		endif !if(p_isdomain)   7796
		...
		if(p_ns(p_unit).le.0)then
			...
			j_err=.true.;return
		endif !if(p_ns(p_unit).le.0)   7837
		...
		do is=1,p_ns(p_unit)
			...
			if(p_filter_)then         !subfilter->
				...
				if(testcode.eq.0.)then   ! filter->False, reject
					...
					goto 776
				endif !if(testcode.eq.0.)   7856
			endif !if(p_filter_)   7848
			if(p_reject)then            ! reject->
				...
				if(testcode.ne.0.)then    !reject->True
					...
					goto 776
				endif !if(testcode.ne.0.)   7868
			endif !if(p_reject)   7863
			776     continue
			!		write(6,*)'<8767'
		!	if(j_xmatinmemory)then
			...
			if(p_xpresent)then
				...
				endif !if(p_nxvararea.gt.0)   7884
		...
		if(p_filre)then
			...
			endif !if(nrej.ge.p_ns(p_unit))   7925
		endif !if(p_filre)   7924
		if(j_err)return
		!		write(6,*)'nuni ',p_nunits,p_unit,ibxmatx
	enddo !p_unit=1,p_nunits   7759
...
j1loop:	 do j1=1,j2-1
				...
				if(p_xpresent)then
					...
					enddo !k=1,p_ntemp0   7951
				endif !if(p_xpresent)   7949
				if(p_fpresent)then
	...
	elseif(size(p_svalue).lt.p_maxns)then
		...
		allocate(p_svalue(1:p_maxns),p_basreject(1:p_maxns))
	endif !if(.not.allocated(p_svalue))   8009
	...
	if(p_ivunit.le.0.and.p_nstot.ne.p_lopp)then
		...
		j_err=.true.
		return
	endif !if(p_ivunit.le.0.and.p_nstot.ne.p_lopp)   8029
	call cpu_time(p_time00)
	...
	if(p_isdomain)then
		...
		enddo !i=1,min(p_ndom,20)   8052
		if(nempt.gt.0)then
			j_err=.true.;write(6,*)'**empty domains:',nempt; return
		end if !if(nempt.gt.0)   8062
		...
		if(p_filre)then
			if(p_rejects(p_ibaunit(i)+1))then
				...
				do jj=1,p_nrowcurx
					...
					p_xmin(j)=j_inf
					p_xmax(j)=j_ninf
				enddo !jj=1,p_nrowcurx   8125
				goto 745
			endif !if(p_rejects(p_ibaunit(i)+1))   8119
		endif !if(p_filre)   8118
		...
		do k=2,p_ns(i)
			if(p_filre)then
				if(p_rejects(p_ibaunit(i)+k))then
					nrej=nrej+1
					cycle  !do k=2,nsch(i)
				endif !if(p_rejects(p_ibaunit(i)+k))   8152
			endif !if(p_filre)   8151
			...
			if(.not.p_fpresent)then
				...
				do jj=1,p_nrowcurx
					...
					if(curx.lt.p_xmin(j))then
						p_xmin(j)=curx
					endif !if(curx.lt.p_xmin(j))   8165
					if(curx.gt.p_xmax(j))then
						p_xmax(j)=curx
						keyopt=k   ! for the case where there are no constraints
					endif !if(curx.gt.p_xmax(j))   8168
				enddo !jj=1,p_nrowcurx   8162
			else
				keyopt=p_keys(i)
 
			endif !if(.not.p_fpresent)   8160
 
		enddo !k=2,p_ns(i)   8150
 
		do jj=1,p_nrowcurx
			j=p_rowcurx(jj)
			p_xsmin(j)=p_xsmin(j)+p_xmin(j)
			p_xsmax(j)=p_xsmax(j)+p_xmax(j)
		enddo !jj=1,p_nrowcurx   8180
...
4689    key=key+1
		...
		endif !if(p_nrow.eq.0.or.j_v(j_ivdollar2).eq.237.d0)   8186
		...
		if(p_warm.and.p_nrow.gt.0)then
			...
			if(p_keys(i).gt.p_ns(i).or.p_rejectnow)then
				...
				key=p_keys(i)
			endif !if(p_keys(i).gt.p_ns(i).or.p_rejectnow)   8202
		endif !if(p_warm.and.p_nrow.gt.0)   8199
		p_keys(i)=key
		...
		enddo !jj=1,p_nrowcurx   8214
 
		if(p_fpresent)call factxps(i,p_keys(i))
 
 
	enddo !i=1,p_nunits   8100
	...
	do j=0,p_nrow  !nrowp = number of rows with x-variables
		...
		endif !if(p_ix(j).ne.0)   8283
	enddo !j=0,p_nrow   8277
...
subroutine checkinfeas()
	...
	if(p_rowcurx(1).eq.0)then
		...
		p_i1=1   ! there is no  "
	endif !if(p_rowcurx(1).eq.0)   8370
	...
	!write(6,*)'<66p_nz ',p_nz
		...
		if(p_nz>0) then
			...
			enddo !inz_=1,p_nz   8382
		endif !if(p_nz>0)   8381
		if(p_fpresent) then
			if(p_nfxinrow(j)>0.or.p_nfyinrow(j)>0) &
				cycle rowloop
		endif !if(p_fpresent)   8387
		if(p_xsmin(j).gt.p_rhs2(j).or.p_xsmax(j).lt.p_rhs(j))then
			...
			endif !if(p_xsmin(iunf).gt.p_rhs2(iunf))   8401
			...
			endif !if(p_xsmax(iunf).lt.p_rhs(iunf))   8407
		endif !if(p_xsmin(j).gt.p_rhs2(j).or.p_xsmax(j).lt.p_rhs(j))   8391
	enddo rowloop !loop:			do jj=1,p_nrowcurx   8377
...
subroutine printz()
	...
	if(.not.p_xpresent)then
		...
		enddo !i=1,p_npvar   8423
		return
	endif !if(.not.p_xpresent)   8422
	do i=1,p_npvar
		!fact
		...
		if(.not.p_isvarx(i).and..not.p_isvarf(i))then
			...
			if(nnz.ge.10.and.nnz.lt.p_nz)then
				write(6,*)' ...etc...'
				exit
			endif !if(nnz.ge.10.and.nnz.lt.p_nz)   8436
		endif !if(.not.p_isvarx(i).and..not.p_isvarf(i))   8433
 
	enddo !i=1,p_npvar   8428
...
subroutine isfeasible()
	...
	if(.true.)then !.not.j_feasible.or.p_unit.eq.1)then !
			...
			if(p_ubou(irow)) then
				if(p_x(irow)+p_tole(irow).lt.p_rhscur(irow)-p_rhs2(irow))then
					...
					if(p_lower(irow))then
						...
						endif !if(p_xpresent)   8505
					endif !if(p_lower(irow))   8499
					p_value=p_value+p_x(irow)
					if(p_p)write(6,*)'infer',irow,p_x(irow)
					p_tolecur=p_tolecur+p_tole(irow)
				endif !if(p_x(irow)+p_tole(irow).lt.p_rhscur(irow)-p_rhs2(irow))   8492
			endif !if(p_ubou(irow))   8491
			if(p_lbou(irow)) then
				if(p_x(irow)-p_tole(irow).gt.p_rhscur(irow)-p_rhs(irow))then
					...
					if(.not.p_lower(irow))then
						...
						endif !if(p_xpresent)   8528
					endif !if(.not.p_lower(irow))   8522
					...
					p_tolecur=p_tolecur+p_tole(irow)
				endif !if(p_x(irow)-p_tole(irow).gt.p_rhscur(irow)-p_rhs(irow))   8515
			endif !if(p_lbou(irow))   8514
 
			! common part of the nonfeasible
			! compute prices of basic variables
			! irow<=nrow -> objr:tä ei käytetä
			! test decreasing
		enddo !i=1,p_lr0   8472
		...
		if(p_nnf.gt.0)then
			...
			if(p_feasible)then
				...
				endif !if(p_nnf.gt.1)   8553
				!	p_again=.true.
				j_err=.true.;return
				!p_nagain=p_nagain+1
			endif !if(p_feasible)   8552
			p_feasible=.false.
			if(.not.p_isobj2)then
				p_objr=>p_objr2
				p_isobj2=.true.
				p_isobj0=.false.
			endif !if(.not.p_isobj2)   8575
			...
			if(p_nnf.ne.p_nnfold)then
				p_objfv=p_small
				p_justkey=.false.
			endif !if(p_nnf.ne.p_nnfold)   8583
			p_ix0=0
					...
					if(p_nagain.le.1)then
						...
						if(p_objf.gt.p_againsol+p_tolecur)then
							...
							if(p_nagain.gt.7)then
								...
								endif !if(p_nrecover.ge.20)   8648
							...
							endif !if(p_nagain.gt.7)   8643
 
						endif !if(p_objf.gt.p_againsol+p_tolecur)   8639
 
					endif !if(p_nagain.le.1)   8636
 
				endif !if(p_again)   8635
			endif !if(.not.p_feasible)   8622
			...
			if(p_xpresent)then
				if(p_lx0.eq.0.and.p_lz0.eq.0)then
					...
					return
				endif !if(p_lx0.eq.0.and.p_lz0.eq.0)   8671
			endif !if(p_xpresent)   8670
	...
	if(p_objf.le.p_objfv-100*p_tolecur.and.p_tmax.gt.j_0)then !100j_0*j_tolecur)then  ! 100 added Feb. 2011 cnhaged by JL 9.9.2018
		...
		return
	endif !if(p_objf.le.p_objfv-100*p_tolecur.and.p_tmax.gt.j_0)   8768
...
subroutine updatejlp()
	...
	endif !if(p_unit.eq.p_unit55)   8826
	...
	if(p_fpresent)then
		...
		enddo !ii_=p_mxd+1,p_lf0   8871
 
 
	endif !if(p_fpresent)   8870
	!end new
	...
	if(p_xpresent)then
		...
		do j=1,p_lx0   !nrow
			...
			endif !if(p_x(p_lcur+p_nrowz).lt.p_wminerr)   8898
			...
			if(p_lunit(nex).ne.p_lunit(p_lcur))then
				if(p_wsu.gt.p_oneps)then
					...
					endif !if(p_wsu.gt.p_wmaxerr)   8962
					...
					do ilr=p_lr0+1,p_nrow
						...
						if(abs(r(p_lcur0+p_nrowz)).gt.p_vcmax)then
							if(p_lr(ilr).ne.p_icolold)then
								...
								p_cycling=.true.
							endif !if(p_lr(ilr).ne.p_icolold)   9007
						endif !if(abs(r(p_lcur0+p_nrowz)).gt.p_vcmax)   9006
 
					enddo !ilr=p_lr0+1,p_nrow   8993
					if(p_vcmax.gt.p_tole(p_lr(p_ilrmax)))then
						p_newc=p_lr(p_ilrmax)
					...
					endif !if(p_vcmax.gt.p_tole(p_lr(p_ilrmax)))   9017
 
				endif !if(p_wsu.gt.p_oneps)   8961
 
				p_wsu=p_zero
 
				p_lcur0=p_next(p_lcur)
 
			endif !if(p_lunit(nex).ne.p_lunit(p_lcur))   8960
 
			p_lcur=nex
		enddo !j=1,p_lx0   8893
 
	endif !if(p_xpresent)   8888
...
901	continue
	...
	if(p_nrow.gt.0)then  !there are constraints
		...
		end do !i=p_nrow+1,p_ncol+p_nrow   9121
		if(.not.p_maxo)then  ! poistettu bugia metsästettäessä
			p_vc=-p_vc                        !;objf=-objf done earlier
			p_vx=-p_vx
 
		end if !if(.not.p_maxo)   9125
 
	endif !if(p_nrow.gt.0)   9120
...
877 continue !nrow=0
	...
	endif !if(p_nrow>0.and.p_nz.eq.0)   9169
	...
	do irow=0,p_nrow
		...
		endif !if(p_xpresent.and.p_isdomain)   9207
		if(idom.ne.idomv)then
			...
			endif !if(p_isdomain)   9217
			write(nureport,*)' ',('_',kk=1,78)
			write(nureport,'(a)')p_buf(1:79)
 
		endif !if(idom.ne.idomv)   9214
...
6606 format(t50,'price', t61,'bound',t70, 'bound')
		...
		if(idom2.ne.idom.or.irow.eq.p_nrow)then
			...
			if(p_iprint.ge.1)then
				...
				do jx=1,p_nxvar  !p_nxvartot !+p_noutsubtrans !p_ncvar,available variables
							...
							do ii=1,p_nrowx2(item)
								...
								if(ivxvar.eq.p_irowvarx(iba+ii))then
									if(iro.eq.0)then
										...
										indi=1
									endif !if(iro.eq.0)   9329
								endif !if(ivxvar.eq.p_irowvarx(iba+ii))   9328
								!endif !if(p_yes)   9419
							enddo !ii=1,p_nrowx2(item)   9313
 
						endif !if(j_yes.and.p_ix(iro).ne.0)   9307
 
					enddo !iro=0,p_nrow   9300
					!	if(jx.le.p_nxvartot)then
					...
					if(p_nrow.gt.0)then
						...
						if(p_intapp)	p_buf(67:76)=j_chr10(p_sumxi((idom-1)*p_nsumx+ixkeep)-p_sumx((idom-1)* &
							p_nsumx+ixkeep))
					endif !if(p_nrow.gt.0)   9377
					write(nureport,'(a)')p_buf(1:79)
 
				end do !jx=1,p_nxvar   9285
				write(nureport,*)' '
				do jx=1,p_nxvartot
					...
					endif !if(p_domainname(1:p_ldomainname).eq.'All')   9393
					j_v(ivoo)=p_sumx((idom-1)*p_nsumx+jx)
				...
				enddo !jx=1,p_nxvartot   9387
 
 
			endif !if(p_iprint.ge.1)   9265
		end if !if(idom2.ne.idom.or.irow.eq.p_nrow)   9263
 
 
		!		end do domloop !domloop: do j=1,j_nsetd(i)
		idomv=idom
	end do !irow=0,p_nrow   9203
	if(p_nshow.gt.0)then
		write(6,*)' '
		write(6,*)'showdomains'
		...
		do j=1,p_nshow
			...
			do jx=1,p_nxvartot
				...
				if(p_domainname(1:p_ldomainname).eq.'All')then
					...
					!	call j_getname(ivoo)
					!	write(6,*)j_oname(1:j_loname),' =',j_v(ivoo)
				endif !if(p_domainname(1:p_ldomainname).eq.'All')   9445
				!	if(ipe.gt.0)cycle
				j_v(ivoo)=p_sumx((idom-1)*p_nsumx+jx)
			...
			enddo !jx=1,p_nxvartot   9441
 
 
 
 
		enddo !j=1,p_nshow   9432
	endif !if(p_nshow.gt.0)   9427
	...
	endif !if(p_nrow.gt.0.and.p_isdomain)   9509
...
subroutine repoepilog(nureport,isx)
	...
	if(j_v(p_ivunbounded)>0) then
		...
		write(nureport,'(a)')p_buf(1:79)
	endif !if(j_v(p_ivunbounded)>0)   9570
	...
	endif !if(j_v(p_ivfeasible)>0)   9578
	...
	endif !if(j_v(p_ivfeasible)>0)   9587
	write(nureport,'(a)')p_buf(1:79)
	...
	if(j_v(p_ivoptimal)>0) then
		...
		endif !if(p_kier.ge.p_maxrounds.and.isx)   9597
	else !if(j_v(p_ivoptimal)>0) then
		p_buf='Solution is not optimal'
	endif !if(j_v(p_ivoptimal)>0)   9596
	write(nureport,'(a)')p_buf(1:79)
...
901	continue
	...
	do i=p_nrow+1,p_ncol+p_nrow
		p_x(p_ls(i))=j_0
		!	if(p_ls(i).eq.2)write(6,*)'<4774putzero'
	end do !i=p_nrow+1,p_ncol+p_nrow   9634
...
!	if(p_nz.gt.0) then
	...
	DO 66778 I=1,p_nz
		...
		p_buf(36:46)=j_chr10(p_redcost(i))
		write(nureport,'(a)')'     '//p_buf(1:46)
66778 continue !66778 I=1,p_nz   9780
...
subroutine writerow(nureport,irow)
	...
	if(irow.gt.0)then
		...
		if(j_v(p_ivfeasible)>0)then
			...
			le=33
			!	le=
		endif !if(j_v(p_ivfeasible)>0)   9859
 
	endif !if(irow.gt.0)   9850
	...
	do while(le.gt.34)
		...
		enddo !while(iplus.le.34.and.iplus.le.le)   9885
		if(iplus.gt.le)then
		...
		endif !if(iplus.gt.le)   9889
	enddo !while(le.gt.34)   9881
	...
	if(irow.ge.1)then
		...
		end if !if(p_rhs(irow).eq.p_rhs2(irow))   9916
...
subroutine renter()
	...
	if(p_niter.ge.10)then  !!!!
		...
		if(p_nnf.eq.p_nnfiter.and.p_objf-p_valiter.le.p_tolecur)then
			...
			p_niter=0
			p_valiter=p_objf
		endif !if(p_nnf.eq.p_nnfiter.and.p_objf-p_valiter.le.p_tolecur)   9962
	endif !if(p_niter.ge.10)   9959
	...
	do ilr=p_lr0+1,p_nrow   !!!!! nonbasic residuals <B1>
		...
		if(p_rhs2(ico).gt.p_rhs(ico))then
			...
			if(p_vc(ico).gt.p_vcmax.and.p_lower(ico).and.ico.ne.p_icolold)then
				...
				endif !if(p_ubou(ico).and.p_lbou(ico))  10019
				...
				if(p_ubou(ico).and.p_lbou(ico)) then
					...
					p_tmax=j_inf ;p_rcur=p_tmax
				endif !if(p_ubou(ico).and.p_lbou(ico))  10040
				p_vcmax=-p_vc(ico)
				p_post=.true.
				!is(p_p) write(p_n16,*)'posit resid could enter',ico  !!!!
				!is(p_p.and.p_fpresent) write(p_n16,*)'**fact** KANTAAN >> residual, p_ienter = 1'
			end if !if(p_vc(ico).gt.p_vcmax.and.p_lower(ico).and.ico.ne.p_icol  10011
		end if !if(p_rhs2(ico).gt.p_rhs(ico))  10006
	end do !ilr=p_lr0+1,p_nrow   9999
...
subroutine zenter()
	...
	do ilz=p_lz0+1,p_nz
		...
		if(p_val.gt.p_vcmax)then
			...
			p_vcmax=p_val
		endif !if(p_val.gt.p_vcmax)  10094
	enddo !ilz=p_lz0+1,p_nz  10066
...
doloop:	do j=1,p_nrowcurx
		...
		if(p_vx(iro).ne.j_0)then
			do ip=1,p_nxbas-1
				if( p_ix(iro).eq.p_ixpack(ip))then
					...
					cycle doloop
				endif !if( p_ix(iro).eq.p_ixpack(ip))  10171
			enddo !ip=1,p_nxbas-1  10170
			p_nxbas=p_nxbas+1
			p_vxpack(p_nxbas)=p_vx(iro)
			p_ixpack(p_nxbas)=p_ix(iro)   ! result (1*ind1,2*ind2,....)
 
		endif !if(p_vx(iro).ne.j_0)  10169
	enddo doloop !oop:	do j=1,p_nrowcurx  10167
	...
	endif !if(p_nxbas.le.0.and..not.p_fpresent)  10185
	...
	!$OMP PARALLEL DO
		...
		if(p_fastnow)then
			...
			endif !if(p_fastreject(iobs).and.i.ne.p_keys(p_unit))  10236
		endif !if(p_fastnow)  10235
		if(p_filre)then
			if(p_rejects(iobs))then
				...
				p_svalue(i)=j_ninf
				cycle
			endif !if(p_rejects(iobs))  10242
		endif !if(p_filre)  10241
		...
		endif !if(p_fastmake)  10303
 
	enddo nschloop !hloop:	do i=1,p_ns(p_unit)  10227
	!$OMP END PARALLEL DO
...
subroutine tulostele()
	...
	endif !if(p_feasible)  10364
 
	call defsolu()
	if(p_xpresent)then
 
		call getsolx() !p_nunits,ibaunit,keys,lunit,nsch,isch,
	endif !if(p_xpresent)  10371
...
subroutine tulostele2(iob) !prints and checkes if finished
	...
	if(p_feasible)then
		if(p_kier.le.10.or.int(p_kier/10)*10.eq.p_kier)then
			...
			endif !if(p_objfprev.ne.p_small)  10436
			...
			do i=1,p_nunits
				if(j_o(p_ivvaluedif)%d(i).gt.j_0)then
					...
					!	ndi=ndi+1
				endif !if(j_o(p_ivvaluedif)%d(i).gt.j_0)  10457
			enddo !i=1,p_nunits  10456
			...
			endif !if(p_isstop.or.p_isfastp)  10472
			if(p_isstop)then
				if(j_codevalue(iob,p_stoplink).ne.j_0)then
					write(6,*)'iteration stops due to stop->'
					p_goto900=.true.
					return
				endif !if(j_codevalue(iob,p_stoplink).ne.j_0)  10481
				...
				if(pros.lt.0.01.and.p_kier.ge.10)then
					...
					p_goto900=.true.
					return
				endif !if(pros.lt.0.01.and.p_kier.ge.10)  10489
 
			endif !if(p_isstop)  10480
		...
		endif !if(p_kier.le.10.or.int(p_kier/10)*10.eq.p_kier)  10400
		if(p_fast.and.p_feasible.and.p_kier.ge.20)then
			...
			endif !if(p_fastusedsame.gt.p_fastusesame)  10517
 
		endif !if(p_fast.and.p_feasible.and.p_kier.ge.20)  10513
	else !if(j_feasible)then
		...
		do i=1,p_lr0 !p_lr0 = number of basic residuals
			...
			elseif(.not.p_lower(p_lr(i)).and.p_x(p_lr(i)).lt.-p_tole(p_lr(i))) then !if(j_lower(j_lr(i)).and.j_x(j_lr(i)).gt.j_tole(j_lr(i))) then
				numn=numn+1
				p_value=p_value+p_x(p_lr(i))
			endif !if(p_lower(p_lr(i)).and.p_x(p_lr(i)).gt.p_tole(p_lr(i)))  10573
		enddo !i=1,p_lr0  10571
		if(p_kier.le.10.or.int(p_kier/10)*10.eq.p_kier)then
			p_as=100.
		...
		endif !if(p_kier.le.10.or.int(p_kier/10)*10.eq.p_kier)  10581
	endif !if(p_feasible)  10399
...
termloop:		do while(iterm.gt.0)
				...
				if(itable.gt.0)then
					...
					endif !if(irow.eq.0)  10639
 
				endif !if(itable.gt.0)  10628
				jterm=jterm+1
				iterm=p_logfactterm(ilogfactbas+jterm)
			enddo termloop !mloop:		do while(iterm.gt.0)  10623
			if(ifact.eq.p_keyfact(p_keyfactbas+ifact))then
				p_keyfval=j_dapu
			elseif(j_dapu.gt.p_fopt)then
				p_optfact=ifact
				p_fopt=j_dapu
			endif !if(ifact.eq.p_keyfact(p_keyfactbas+ifact))  10649
 
			jne=jne+1
			ine=p_neig(p_inebas+jne)
		enddo neigloop !gloop:	do while(ine.gt.0)  10616
		if(p_fopt.gt.p_keyfval)then
			p_ienter=4
			return
		endif !if(p_fopt.gt.p_keyfval)  10659
 
 
		p_ilog=p_ilog+1
		p_logfactbas=p_logfactbas+p_nlogfact
	enddo logloop !loop: do while(p_ilog.le.p_nlog)  10612
	return
...
subroutine entercol()
	...
	if(p_fpresent) then
		...
		endif !if((p_nfrowx.gt.0.and.p_frowx(1).eq.0).or.(p_nfyrow.gt.0.a  10708
	endif !if(p_fpresent)  10706
	...
	enddo !iro=p_row0,p_nrowcurx  10742
	!do iro=1,p_nrow
...
subroutine leaving()
	...
	if(p_ienter.eq.p_ienterv.and.	p_objf.lt.p_oldsol+p_tolecur)then
		...
		p_oldsol=p_objf
		p_nsame=0
	endif !if(p_ienter.eq.p_ienterv.and.	p_objf.lt.p_oldsol+p_tolecur  10808
	...
	if(.not.p_post)then
		do jj=1,p_nrow
			r(p_ls(jj))=-r(p_ls(jj))
		enddo !jj=1,p_nrow  10867
		p_post=.true.   !!!!
	endif !if(.not.p_post)  10866
...
2244	if(p_tmax.lt.p_tmaxmin)then  !!!!! jos mukaan, pitää tsekata keyt ensin
	...
	endif !2244	if(p_tmax.lt.p_tmaxmin)  10942
...
!	if(p_p)	write(p_n16,*)' p_leave, p_ienter ',p_leave,p_ienter
	...
	if(p_pivot.eq.p_idebug)then
		...
		!			close(p_n16)
	endif !if(p_pivot.eq.p_idebug)  11042
	...
	if(p_leave.eq.0)then
		...
		endif !if(p_xpresent.and.abs(p_rcur).le.1.d-9)  11095
	...
	if(p_ienter.eq.1)then
			...
			if(p_fpresent)then
				do ilu_ = 1,p_lunits0
					...
					if (p_lunw(ilu_)==p_lcursame) then
						p_lunw(ilu_)=p_id
					endif !if (p_lunw(ilu_)==p_lcursame)  11230
				enddo !ilu_ = 1,p_lunits0  11228
			endif !if(p_fpresent)  11227
		...
		endif !if(p_lcursame.gt.0)  11214
		! edelleed d-enters
		p_lx0=p_lx0+1
		p_lunit(p_id)=p_unit
		p_isch(p_id)=p_iopt
	...
	endif !if(p_leavec.le.p_nrow)  11354
	...
	do j=1,p_lz0  !!!! z leaving
		...
		if(r(p_leaz).gt.p_tiny78)then
			if(p_x(p_leaz).lt.p_tmax*r(p_leaz))then
				...
				p_leavec=p_leaz
			endif !if(p_x(p_leaz).lt.p_tmax*r(p_leaz))  11465
		endif !if(r(p_leaz).gt.p_tiny78)  11464
	enddo !j=1,p_lz0  11461
...
subroutine sleavtmax()
	...
	do j=1,p_lx0  !!!!sched leaving
		...
		if(r(p_lead).gt.p_tiny78)then     !
			...
			endif !if(p_x(p_lead).lt.p_tmax*r(p_lead))  11482
 
		endif !if(r(p_lead).gt.p_tiny78)  11480
	enddo !j=1,p_lx0  11475
...
subroutine fleavtmax()
	...
	do j=p_mxd+1,p_lf0
		...
		if(r(leaf).gt.p_tiny78)then
			...
			endif !if(p_x(leaf).lt.p_tmax*r(leaf))  11498
 
		endif !if(r(leaf).gt.p_tiny78)  11497
	enddo !j=p_mxd+1,p_lf0  11494
			...
			if(p_ubou(p_lr(j)))then
				...
				if(p_lower(p_lr(j)).and.p_x(p_lr(j))+ &
					...
					if(p_x(p_lr(j)).lt.p_tmax*r(p_lr(j)) )then
						...
						p_leavec=p_lr(j)
					end if !if(p_x(p_lr(j)).lt.p_tmax*r(p_lr(j)) )  11594
				...
				end if !if(p_lower(p_lr(j)).and.p_x(p_lr(j)  11578
			end if !if(p_ubou(p_lr(j)))  11566
			...
			if(p_lbou(p_lr(j)))then
				...
				elseif(p_x(p_lr(j)).le.j_0.and.p_lower(p_lr(j)))then !if(.not.j_lower(j_lr(j)).and.j_x(j_lr(j))+j_rhs(j_lr(j))-j_rhscur(j_lr(j)).gt.r(j_lr(j))*j_tm
					...
					if(p_x(p_lr(j)).gt.p_tmax*r(p_lr(j)) )then  !-x/-r <tmax-tole -x<-r*(tmax-tole)
						...
						p_leavec=p_lr(j)
					end if !if(p_x(p_lr(j)).gt.p_tmax*r(p_lr(j)) )  11668
				...
				elseif(p_x(p_lr(j)).gt.j_0.and.p_lower(p_lr(j)).and.p_feasible)then !if(.not.j_lower(j_lr(j)).and.j_x(j_lr(j))+j_rhs(j_lr(j))-j_rhscur(j_lr(j)).g
					...
					return
					!	goto 2244
				endif !if(.not.p_lower(p_lr(j)).an  11653
 
			end if !if(p_lbou(p_lr(j)))  11651
 
		end if !if(r(p_lr(j)).gt.p_tiny78)  11525
 
	end do !j=1,p_lr0  11510
...
subroutine skeyleav()
	...
	do i=1,p_lx0
		...
		if(p_lunit(nex).ne.p_lunit(p_lcur))then
			...
			endif !if(p_lunit(p_lcur).eq.p_unit.and.p_ienter.eq.3)  11741
			...
			if(p_rs.lt.p_tiny78n.and.p_wsu.gt.p_tmax*p_rs)then !!!!
				...
				!					end if
			endif !if(p_rs.lt.p_tiny78n.and.p_wsu.gt.p_tmax*p_rs)  11749
 
			p_wsu=p_onen   !opposite sign of ws as in old JLP
			p_rs=p_zero
			p_lcur0=p_next(p_lcur)   ! p_lcur0 is again the first col in the next new unit
			! last
		endif !if(p_lunit(nex).ne.p_lunit(p_lcur))  11738
		p_lcur=nex
	enddo !i=1,p_lx0  11731
...
subroutine fkeyleav()
	...
	do p_ixk_=1,p_nlog
		...
		do while (p_lcur0_unit > p_mxd)
			...
			endif !if ((p_ienter==4).and.(p_unit==p_lunit_).and.(ixk_==ixkent  11800
			...
			enddo !while (p_lunit(p_lcur_) == p_lunit(p_lcur0_unit))  11810
			...
			endif !if ((p_ienter==3).and.(p_unit==p_lunit_))  11818
			...
			do while((p_lunit(p_lunw(lun_))/=p_lunit_).and.(lun_<=p_lunits0))
				lun_=lun_+1
			enddo !while((p_lunit(p_lunw(lun_))/=p_lunit_).and.(lun_<=p_lunit  11838
			...
			if(lun_ <= p_lunits0) then
				...
				enddo !while (p_lunit(lcur_w) == p_lunit(lcur0_w))  11851
 
			endif !if(lun_ <= p_lunits0)  11843
			...
			endif !if(p_rs>p_tiny78.and.p_wsu.lt.p_tmax*p_rs)  11895
 
			p_lcur0_unit = p_lcur_
		enddo !while (p_lcur0_unit > p_mxd)  11787
	enddo !p_ixk_=1,p_nlog  11784
...
subroutine jotainf()
	...
	if ((p_ienter==4).and.(p_lcursamef==0)) then
		...
		do while((p_lunit(p_lunw(lun_))/=p_unit).and.(lun_<=p_lunits0))
			lun_=lun_+1
		enddo !while((p_lunit(p_lunw(lun_))/=p_unit).and.(lun_<=p_lunits0  11941
		...
		if(lun_ <= p_lunits0) then
			...
			enddo !while (p_lunit(p_lcur_) == p_lunit(lcur0_w))  11955
 
		endif !if(lun_ <= p_lunits0)  11947
		...
		endif !if((p_rs>p_tiny78).and.(p_wsu.lt.p_tmax*p_rs))  11973
 
	endif !if ((p_ienter==4).and.(p_lcursamef==0))  11930
	...
	endif !if ((p_ienter==4).and.(p_leavkf==0).and.(p_leave==0).and.(  11985
...
subroutine leaveskeys()
	...
	if(p_unit.eq.p_lunit(p_leavk).and.p_ienter.eq.3)then
		...
		enddo !j=1,p_nrow  12070
		!		endif !if(sparse)  20028
	endif !if(p_unit.eq.p_lunit(p_leavk).and.p_ienter.eq.3)  12060
	...
	if(p_lunit(p_lcur).eq.p_lunit(nex))then
		...
		endif !if(p_pivot.eq.p_idebug)  12212
		...
		if(p_fpresent)then
			...
			do ilu_ = 1,p_lunits0
				if (p_lunw(ilu_)==nex) then
					p_lunw(ilu_)=p_id
				endif !if (p_lunw(ilu_)==nex)  12258
			enddo !ilu_ = 1,p_lunits0  12257
		endif !if(p_fpresent)  12255
		...
		p_lcur=p_id
		goto 17
	endif !if(p_lunit(p_lcur).eq.p_lunit(nex))  12111
...
subroutine leave0ienter3()
	...
	enddo !jj=1,p_nrowcurx  12360
 
	do jj=1,p_nrow
		p_rhsw(jj)=p_rhscur(jj)-p_xps(jj)
	enddo !jj=1,p_nrow  12366
...
subroutine leave0ienter1()
	...
	if((p_lower(p_newc).and..not.p_ubou(p_newc)).or.(.not.p_lower(p_newc).and..not.p_lbou(p_newc))) then
		...
		endif !if(p_maxo)  12391
		j_err=.true.
 
		return
	endif !if((p_lower(p_newc).and..not.p_ubou(p_newc)).or.(.not.p_lo  12385
	...
	endif !if((p_ix(p_newc).ne.0).or.p_fpresent)  12407
	...
	if(.not.p_feasible)then
		do jj=1,p_lr0
			if(r(p_lr(jj)).gt.p_tiny78)then
				...
				endif !if(.not.p_lbou(p_lr(jj)).or..not.p_lower(p_lr(jj)))  12442
				p_tmax=p_x(p_lr(jj))/r(p_lr(jj))
				p_rcur=r(p_lr(jj))
				p_leavec=p_lr(jj)
				p_leave=p_lsi(p_leavec)
				p_goto8888=.true.
			endif !if(r(p_lr(jj)).gt.p_tiny78)  12441
			if(r(p_lr(jj)).lt.p_tiny78n)then
				if(.not.p_ubou(p_lr(jj)).or.p_lower(p_lr(jj))) then
					!virheilmoitus ja häivytään
				endif !if(.not.p_ubou(p_lr(jj)).or.p_lower(p_lr(jj)))  12452
				p_tmax=p_x(p_lr(jj))/r(p_lr(jj)) ;p_rcur=r(p_lr(jj))
				p_leavec=p_lr(jj)
				p_leave=p_lsi(p_leavec)
				p_goto8888=.true.
			endif !if(r(p_lr(jj)).lt.p_tiny78n)  12451
		enddo !jj=1,p_lr0  12440
	endif !if(.not.p_feasible)  12439
	...
	if(p_route67)then
		if(p_newc.gt.p_nrow)then    !jl 20160608
			...
			if(abs(p_x(p_ls(p_leave))).lt.p_tiny6)then
				...
				j_err=.true. ;return
			endif !if(abs(p_x(p_ls(p_leave))).lt.p_tiny6)  12493
			...
			if(abs(p_x(p_ls(p_leave))).lt.p_tiny6)then
				...
				! j_err=.true.;return
			endif !if(abs(p_x(p_ls(p_leave))).lt.p_tiny6)  12512
 
		endif !if(p_newc.gt.p_nrow)  12485
	endif !if(p_route67)  12484
...
subroutine printproblem()
	...
	do irow=1,p_nrow
		...
		end if !if(p_xpresent.and.p_isdomain)  12548
		!write(nureport,*)' ',('_',kk=1,78)
		!		write(j_buf,66061)
		!66061 format('row',t38,'value',t50,'shadow',t61,  'lower',t70,'upper')
		...
		if(irow.gt.1)then
			...
			end if !if(p_rhs(irow).eq.p_rhs2(irow))  12591
			...
			if(p_maxo)then
				...
				j_tempchar(1:5)=' min'
			end if !if(p_maxo)  12605
			!else !if(j_v(p_ivfeasible)>0)then
			!	j_buf(1:5)=' '
			!	j_buf(6:33)='Infeasible, temporary object'
			!endif !if(j_v(p_ivfeasible)>0)then
		end if !if(irow.gt.1)  12578
...
logical function isxkzero(ixk)
	...
	if(j_o(p_ivdatamatx)%d(ibxdatkey_+ikeepxk_).ne.0.)then
		isxkzero=.false.
		return
	endif !if(j_o(p_ivdatamatx)%d(ibxdatkey_+ikeepxk_).ne.0.)  12638
	do k_=1,p_lx0
		if(p_lunit(p_lx(k_)).ne.p_unit) cycle
		...
		if(j_o(p_ivdatamatx)%d(ibxdatkey_+ikeepxk_).ne.0.)then
			isxkzero=.false.
			return
		endif !if(j_o(p_ivdatamatx)%d(ibxdatkey_+ikeepxk_).ne.0.)  12646
	enddo !k_=1,p_lx0  12642
...
function value_f(iobs)
	...
	do jjj = 1,p_nrowxkfkey
		...
		endif !if (p_rowxkfkey(jjj)%jcurix == 0)  12668
	enddo !jjj = 1,p_nrowxkfkey  12666
	...
	do jj=1,p_nrowykfkey
		...
		endif !if (p_rowykfkey(jj)%jcurix.eq.0)  12686
	enddo !jj=1,p_nrowykfkey  12684
...
subroutine lcursam()
	...
	do i=1,p_lx0
		...
		if(p_lunit(p_lcur).eq.p_unit)then
			p_lcursame=p_lcur
			return
		end if !if(p_lunit(p_lcur).eq.p_unit)  12711
	end do !i=1,p_lx0  12709
	return
...
766		nex=p_next(nex)
		...
		if(jj.gt.p_lx0.or.nex.eq.0)then
			...
			j_err=.true.
			return
		end if !if(jj.gt.p_lx0.or.nex.eq.0)  12727
		jj=jj+1
		goto 766
767		continue
	end do !i=1,p_lx0  12721
	! end logical testing
...
subroutine testcol()
	...
	do i=0,p_nrow
		...
		if(i.gt.0)then
			if(p_ls(i).gt.p_nrowz)then
				...
				!        return
	788		  continue
			endif !if(p_ls(i).gt.p_nrowz)  12752
		end if !if(i.gt.0)  12751
	end do !i=0,p_nrow  12749
	p_idomba=0
	do i=1,p_nunits
		...
		end do !j=0,p_nrow  12779
	end do !i=1,p_nunits  12767
	...
	do i=1,p_lx0
		...
		if(p_x(p_lx(i)+p_nrowz).LT.0.OR.p_x(p_lx(i)+p_nrowz).GT.1.)then
			...
			j_err=.true.
		end if !if(p_x(p_lx(i)+p_nrowz).LT.0.OR.p_x(p_lx(i)+p_nrowz).GT.1.  12811
	end do !i=1,p_lx0  12786
...
!      22       20      22
	...
	if(p_lunit(nex).ne.lunv)then
		...
		if(wss.gt.1.)then
			...
			write(6,*)'*nex* return'
			j_err=.true. ;return
		end if !if(wss.gt.1.)  12840
	end if !if(p_lunit(nex).ne.lunv)  12835
...
subroutine testxpssub(iunit_)
	...
	do i_=1,p_nunits  !***********************
		...
		enddo !jj_=1,p_nrowcurx  12868
 
		!tehdasmuuttujien xps-laskenta
		if(p_fpresent) then
 
 
		endif !if(p_fpresent)  12876
 
 
	enddo !i_=1,p_nunits  12859
	...
	if(iunit_>0) then
		...
		if(p_fpresent) then
			...
			if (p_nfy.gt.0) then
				...
				enddo !jjj=1,j_o(p_ivkeepc)%i(1)  12898
				!	do jjj=1,p_nutiltrans
				call dotrans(p_ivutiltrans,1)
				if(j_err)then
					write(6,*)'error for trans ',j
					stop 741
				endif !if(j_err)  12903
				!	enddo !jjj=1,p_nutiltrans  11758
			endif !if (p_nfy.gt.0)  12897
		endif !if(p_fpresent)  12893
	endif !if(iunit_>0)  12885
	...
	do iii=1,p_nrow
		...
		endif !if(abs(p_testxps(iii)-p_xps(iii)).gt.0.001)  12915
	enddo !iii=1,p_nrow  12914
...
subroutine testobjr0()
	...
	do ldj_=1,p_lx0
		...
		do it_=0,p_nrow
			...
			endif !if(p_ixcur(it_))  12943
		enddo !it_=0,p_nrow  12942
 
		if (p_fpresent) then
 
 
		endif !if (p_fpresent)  12948
 
		write(p_n16,*)'ld(j)',p_lx(ldj_),'test(0:nrow)',(p_test(jjj),jjj=0,p_nrow)
 
	enddo !ldj_=1,p_lx0  12929
	...
	do iuni=1,p_nunits
		if(.not.p_isunit)then
			...
			if(j_err)then
				...
				return
			endif !if(j_err)  12973
			if(p_nshow.gt.0)then
				call dotrans(p_ivshowtrans,1)
				if(j_err)return
			endif !if(p_nshow.gt.0)  12977
			if(p_ivtrans.gt.0)then
				call dotrans(p_ivtrans,1)
				if(j_err)stop 770
			endif !if(p_ivtrans.gt.0)  12981
		endif !if(.not.p_isunit)  12970
		...
		if(p_isdomain)then
			...
			do id=1,p_ndom
				...
				end if !if(indomain2(id,iuni))  13009
				iba=iba+p_nxvartot !+p_noutsubtrans !+p_ncvar
			enddo !id=1,p_ndom  13006
		...
		if(p_nshow.gt.0)then
			...
			do id=1,p_nshow
				if(j_o(p_ivshowmatrix)%d(id).ne.j_0)then
					...
					do jx=1,p_nxvartot
						p_sumx(iba+jx)=p_sumx(iba+jx)+ j_v(j_o(p_ivkeepx)%i2(jx))
					end do !jx=1,p_nxvartot  13053
				endif !if(j_o(p_ivshowmatrix)%d(id).ne.j_0)  13051
				iba=iba+p_nxvartot
			enddo !id=1,p_nshow  13050
		endif !if(p_nshow.gt.0)  13048
 
	end do !iuni=1,p_nunits  12969
...
subroutine getsolx() !
	...
	if(p_nz.eq.0)then
		...
		do i=1,p_nunits  !***********************
			...
			enddo !jj=1,p_nrowcurx  13089
		enddo !i=1,p_nunits  13077
	endif !if(p_nz.eq.0)  13072
	...
	if(p_ivarea.gt.0.and.p_nnotareavars.gt.0)then
		...
		if(p_nxvarareatot.lt.p_nxvartot)then !  .or.notc.gt.0)then
			...
			ipart=1
		endif !if(p_nxvarareatot.lt.p_nxvartot)  13123
	...
	endif !if(p_ivarea.gt.0.and.p_nnotareavars.gt.0)  13119
	...
	do iuni=1,p_nunits
		if(.not.p_isunit)then
			...
			if(j_err)then
				...
				stop 512
			endif !if(j_err)  13146
			if(p_ivtrans.gt.0)then
				call dotrans(p_ivtrans,1)
				if(j_err)stop 651
 
			endif !if(p_ivtrans.gt.0)  13150
		endif !if(.not.p_isunit)  13144
		iobs=p_ibaunit(iuni)+integerschedw(iuni)   !+j_ibaunitbas
		call j_getobsiv(iobs,p_ivdatamatx,p_ivkeepx,0)! ,p_ivtransx,0)  !getsolx
		if(j_err)then
			write(6,*)'error for observation ',iobs
		endif !if(j_err)  13158
		...
		if(p_ivarea.gt.0)then
			if(ipart.eq.0)then
				...
				j_v(p_varxareatot)=j_v(p_ivarea)*j_v(p_varxareatot)
			endif !if(ipart.eq.0)  13166
		endif !if(p_ivarea.gt.0)  13165
		if(p_intapp)then
			if(p_isdomain)then
				iba=0
				!	p_idomba=(iuni-1)*p_ndomv
				do id=1,p_ndom
					...
					end if !if(indomain2(id,iuni))  13182
 
					iba=iba+p_nxvartot !+p_noutsubtrans !+p_ncvar+
				end do !id=1,p_ndom  13178
			else !if(j_ndom.gt.0)then
			...
			end if !if(p_isdomain)  13175
		endif !if(p_intapp)  13174
	end do !iuni=1,p_nunits  13143
	...
	do i=1,p_lx0
		...
		endif !if(.not.p_isunit)  13226
		if(p_nshow.gt.0)then
			call dotrans(p_ivshowtrans,1)
			if(j_err)return
		endif !if(p_nshow.gt.0)  13230
		iobs=p_ibaunit(iuni)+p_keys(iuni)
		call j_getobsiv(iobs,p_ivdatamatx,p_ivkeepx,0) !,p_ivtransx,0) !getsolx
		!	if(p_ivsubtrans.gt.0)call dotrans(p_ivsubtrans,1)
		if(j_err)return
		...
		if(p_ivarea.gt.0)then
			if(ipart.eq.0)then
				...
				j_v(p_varxareatot)=j_v(p_ivarea)*j_v(p_varxareatot)
			endif !if(ipart.eq.0)  13240
		endif !if(p_ivarea.gt.0)  13239
		if(p_isdomain)then
			iba=0
			!	p_idomba=(iuni-1)*p_ndomv
			do id=1,p_ndom
				...
				end if !if(indomain2(id,iuni))  13252
 
				iba=iba+p_nxvartot  !+p_noutsubtrans
			end do !id=1,p_ndom  13249
		else !if(j_ndom.gt.0)then
			do jx=1,p_nxvartot
				p_sumx(jx)=p_sumx(jx)-wei* j_v(j_o(p_ivkeepx)%i2(jx))
			end do !jx=1,p_nxvartot  13267
			...
			do id=1,p_nshow
				if(j_o(p_ivshowmatrix)%d(id).ne.j_0)then
					...
					do jx=1,p_nxvartot
						p_sumx(iba+jx)=p_sumx(iba+jx)-wei* j_v(j_o(p_ivkeepx)%i2(jx))
						!	p_sumx(iba+jx)=p_sumx(iba+jx)+ j_v(j_o(p_ivkeepx)%i2(jx))
					end do !jx=1,p_nxvartot  13283
				endif !if(j_o(p_ivshowmatrix)%d(id).ne.j_0)  13281
				iba=iba+p_nxvartot
			enddo !id=1,p_nshow  13280
 
		endif !if(p_nshow.gt.0)  13278
		...
		if(p_ivarea.gt.0)then
			if(ipart.eq.0)then  !if(p_nxvarareatot.eq.nxvartot)then
				...
				j_v(p_varxareatot)=j_v(p_ivarea)*j_v(p_varxareatot)
			endif !if(ipart.eq.0)  13299
		endif !if(p_ivarea.gt.0)  13298
		...
		if(p_isdomain)then
			...
			do id=1,p_ndom
				...
				end if !if(indomain2(id,iuni))  13312
 
				iba=iba+p_nxvartot+p_noutsubtrans
			end do !id=1,p_ndom  13309
		else !if(j_ndom.gt.0)then
			do jx=1,p_nxvartot
				p_sumx(jx)=p_sumx(jx)+wei* j_v(j_o(p_ivkeepx)%i2(jx))
			end do !jx=1,p_nxvartot  13327
			...
			do id=1,p_nshow
				if(j_o(p_ivshowmatrix)%d(id).ne.j_0)then
					...
					do jx=1,p_nxvartot
						p_sumx(iba+jx)=p_sumx(iba+jx)+wei* j_v(j_o(p_ivkeepx)%i2(jx))
						!			p_sumx(iba+jx)=p_sumx(iba+jx)+ j_v(j_o(p_ivkeepx)%i2(jx))
					end do !jx=1,p_nxvartot  13343
				endif !if(j_o(p_ivshowmatrix)%d(id).ne.j_0)  13341
				iba=iba+p_nxvartot
			enddo !id=1,p_nshow  13340
		endif !if(p_nshow.gt.0)  13338
 
 
 
	end do !i=1,p_lx0  13222
...
logical function indomain(ido)
	...
	elseif(btest(p_domainbits(p_idomba+p_icurint(ido)),p_icurbit(ido)))then
		...
		indomain=.false.
	endif !if(p_domvars(ido).eq.j_ivall)  13483
	p_idomba=p_idomba+p_ndomv
	return
end function
...
logical function indomain2(ido,iuni)
	...
	if(p_domvars(ido).eq.j_ivall)then
		indomain2=.true.
		return
	endif !if(p_domvars(ido).eq.j_ivall)  13496
	idomba=(iuni-1)*p_ndomv
	if(btest(p_domainbits(idomba+p_icurint(ido)),p_icurbit(ido)))then
		indomain2=.true.
	else
		indomain2=.false.
	endif !if(btest(p_domainbits(idomba+p_icurint(ido)),p_icurbit(ido  13501
	return
end function
...
subroutine zerocap()
	...
	do irowj_ = 2,p_nrow
		...
		if(p_nfxinrow(irowj_).eq.1.and. p_nfyinrow(irowj_).eq.0.and. &
			...
			endif !if(p_zeroc)  13530
 
		endif !if(p_nfxinrow(irowj_).eq.1.and. p_nfyinrow(irowj_).eq.0.an  13516
	enddo !irowj_ = 2,p_nrow  13513
	return
end subroutine
...
subroutine jlp00(iob,io)
	...
	do ival=1,p_nterm
		...
		if(ikeepx0.gt.0)then
			...
			ikeepx(nx)=ikeepx0
			itermx(nx)=ival
		endif !if(ikeepx0.gt.0)  13582
		if(ikeepx0.le.0.and.ikeepc0.le.0)then
			call j_getname(i)
			write(6,*)j_oname(1:j_loname),' is not in cdata or in xdata'
			j_err=.true.
		endif !if(ikeepx0.le.0.and.ikeepc0.le.0)  13588
	enddo !ival=1,p_nterm  13571
		...
		do i=1,p_nunits
			...
			do j=2,ns
				...
				if(j_dapu2.gt.j_dapu)then
					p_keys(i)=j
					j_dapu=j_dapu2
				endif !if(j_dapu2.gt.j_dapu)  13619
				ibax=ibax+nkeepx
		...
		do i=1,p_nunits
			...
			do j=1,ns
				...
				if(j_dapu2.gt.j_dapu)then
					p_keys(i)=j
					j_dapu=j_dapu2
				endif !if(j_dapu2.gt.j_dapu)  13642
				ibax=ibax+nkeepx
			...
			ibac=ibac+nkeepc
		enddo !i=1,p_nunits  13631
 
	endif !if(nc.eq.0)  13605
	write(6,*)'objf',p_objf
...
subroutine initdata0(iob,io)
	...
	if(p_ivarea.eq.0)then   !option area->,
		...
		if(p_ivarea.le.0)then
			...
			j_err=.true.
			return
		endif !if(p_ivarea.le.0)  13713
	endif !if(p_ivarea.eq.0)  13711
	...
	do ival=1,p_nterm
		...
		endif !if(ikeep.gt.0)  13830
	enddo !ival=1,p_nterm  13824
	...
	if(p_isunit)then
		...
		endif !if(allocated(j_itempvector))  13857
		...
		do i=2,j_o(p_ivdatamatx)%i(1)
			...
			if(j_o(p_ivdatamatx)%d(ibas+inde).ne.j_dapu)then
				...
				iprev=i
			endif !if(j_o(p_ivdatamatx)%d(ibas+inde).ne.j_dapu)  13872
			ibas=ibas+p_nkeepx
		enddo !i=2,j_o(p_ivdatamatx)%i(1)  13870
		j_itempvector(p_nunits)=j_o(p_ivdatamatx)%i(1)-iprev
		p_maxns=max(p_maxns,j_itempvector(p_nunits))
		!	write(6,*)'shed',p_ns(1:p_nunits)
	else
		p_nunits=j_o(p_ivdatamatc)%i(1)
	endif !if(p_isunit)  13855
	...
	do i=1,p_nunits  !***********************
		...
		p_ibaunit(i+1)=p_ibaunit(i)+p_ns(i)
		iba=iba+p_nkeepc
		!	write(6,*)'here',i,p_isunit,p_ns(i),iba,nsindex,p_ibaunit(i+1),p_nkeepc
	enddo !i=1,p_nunits  13928
...
subroutine fterms(iob,io)
	...
	write(6,*)'factorries ar at this point'
	j_err=.true.
	return
	nlog2=0
	nfact2=0
			...
			do ifact=1,nfact
				...
				do j=1,p_nterm
					if(ilofa.eq.p_termvar(j))then
						...
						p_isvarf(jif)=.true.
						p_istermf(j)=.true.
					endif !if(ilofa.eq.p_termvar(j))  14072
					if(ivtable.eq.p_termvar(j))then
						p_tabpart(j)=i
						p_istermf(j)=.true.
						jif=j_inlistobject(p_termvar(j),p_ivvars)
						p_isvarf(jif)=.true.
					endif !if(ivtable.eq.p_termvar(j))  14080
				enddo !j=1,p_nterm  14071
 
			enddo !ifact=1,nfact  14064
		enddo !ilog=1,nlog  14049
		if(j_err)return
		nlog2=nlog2+nlog
		nfact2=nfact2+nfact
	...
	do i=1,p_nterm
		...
		if(j_otype(ivterm).eq.j_iptable)then
			...
			enddo !i2=1,p_nterm-1  14121
 
		endif !if(j_otype(ivterm).eq.j_iptable)  14120
		p_ndupmax=max(p_ndupmax,ndup)
	enddo !i=1,p_nterm  14117
		...
		do ip=1,p_ntable
			...
			do j=1,nlog
				...
				do k=1,p_knn
					...
					endif !if(j_dapu.gt.j_0)  14288
				enddo !k=1,p_knn  14285
			...
			enddo !j=1,nlog  14280
 
		enddo !ip=1,p_ntable  14255
		...
		do k=1,p_ns(i8)
			...
			do it=1,p_nterminrow(0)
				...
				elseif(itable.ne.0)then
					...
					enddo !ilog=1,p_tablenlog(itable)  14331
 
				elseif(p_factpart(it).ne.0)then
					p_value=p_value+p_coef(it)*p_datamatx(ibadatamatx+p_keeplog(ilog))
				endif !if(p_isvarx(it).and.j_yes)  14320
 
			enddo !it=1,p_nterminrow(0)  14318
			write(6,*)p_value
			if(p_value.gt.p_valueopt)then
				p_keys(i8)=k
				p_valueopt=p_value
			endif !if(p_value.gt.p_valueopt)  14348
			!		write(6,*)'k,value,valueop',k,p_value,p_valueopt,p_keys(i8)
			ibadatamatx=ibadatamatx+p_nkeepx
		enddo !k=1,p_ns(i8)  14314
 
		do ir=0,p_nrow
 
 
		enddo !ir=0,p_nrow  14356
		!	j_err=.true.
		!	return
