...
subroutine jlpgetcol(icol)
	...
	end do !i=jlp_lavecsp(jlp_lavecsp(0)+icol),last(icol)    101
	return
...
subroutine jlpsubcol(ic1,ic2,icr)   !!!!
	...
	if(i1.gt.last(ic1))then
		...
		viel1=.true.
	end if !if(i1.gt.last(ic1))    114
	if(i2.gt.last(ic2))then
		viel2=.false.
	else !if(i2.gt.last(ic2))then
		viel2=.true.
	end if !if(i2.gt.last(ic2))    119
	ie=0
	ie1=0
	ie2=0
		...
		if(i2.eq.last(ic2))then
			...
			i2=i2+1
			goto 100
		end if !if(i2.eq.last(ic2))    133
	end if !100 if(viel2.and.(jlp_lavecsp(i2).lt.jlp_lavecsp(i1).or..n    128
	...
	200 if(viel1.and.(jlp_lavecsp(i1).lt.jlp_lavecsp(i2).or..not.viel2))then
		...
		if(i1.eq.last(ic1))then
			...
			i1=i1+1
		end if !if(i1.eq.last(ic1))    147
		goto 100
	end if !200 if(viel1.and.(jlp_lavecsp(i1).lt.jlp_lavecsp(i2).or..n    142
...
300 if(viel1.and.viel2.and.jlp_lavecsp(i1).eq.jlp_lavecsp(i2))then
		...
		if(i1.eq.last(ic1))then
			...
			i1=i1+1
		end if !if(i1.eq.last(ic1))    162
		if(i2.eq.last(ic2))then
			viel2=.false.
		else !if(i2.eq.last(ic2))then
			i2=i2+1
		end if !if(i2.eq.last(ic2))    167
		goto 100
	end if !300 if(viel1.and.viel2.and.jlp_lavecsp(i1).eq.jlp_lavecsp(    155
...
subroutine problem(iob,io)   !new version old version is problem2  %%jlp  !!!!
	...
	! NNote 4: It is not necessary to define problem() function if the problem includes only zvariables. In jlp() function you can use zmatrix-> option instead of
	...
	integer, dimension(:), allocatable:: nsetr,isetd  !nsetd pois
	...
	allocate(nsetr(0:500),isetd(1:1000)) !nsetd(1:500),
	allocate(ivars(0:1000))
	allocate(irowvars(1:5000),coef(1:5000),nvars(1:500))
	!if(.not.allocated(jlp_ivdomains))allocate(jlp_ivdomains(1:500))
	newprice=.true.
	newc=0
	inprint=0
	!	write(6,*)'PROBLEM'
	...
	iper=j_putlistobject(ivdomainvars,single=j_ivall)
	call j_deflistobject(iout,'%rowdomain',ivrowdomain,nres=500)
	if(ivdefrow.le.0)ivdefrow=j_object('DefRows')   ! poista
	if(ivdefrow.gt.0)then
		ndefrow=j_v(ivdefrow)
	else !if(ivdefrow.gt.0)then
		ndefrow=200
	end if !if(ivdefrow.gt.0)    362
	call j_getname(iout)
	probname=j_oname
	lprobname=j_loname
	...
	if(ivdefdomain.gt.0)then
		...
		ndefdomain=100
	end if !if(ivdefdomain.gt.0)    371
	...
	call j_deftext(0,'$domaintext',ndefdomain,80*ndefdomain,ivdomaintext)
	!                                                               text object
...
! write(6,*)'<77inp:',j_linp,j_inp(1:j_linp)
	...
	ip=j_nextlim0(j_inp,ialdom,j_linp,':')  !Mela
	!	write(6,*)'<6464>ip,ipv,ialdom',ip,ipv,ialdom,j_incline(),':',j_inp(1:j_linp)
	!	write(6,*)ip.gt.0,j_inp(1:j_linp).eq.'/',ipv.gt.1,(ip.gt.0.or.j_inp(1:j_linp).eq.'/').and.ipv.gt.1
	if((ip.gt.0.or.j_inp(1:j_linp).eq.'/').and.ipv.gt.1)then
		call j_inclinegoto(idomres)
		!ialdom=ipv+1
		!		write(6,*)'<3443>ialdom,ipv',ialdom,ipv,idomres
		ipv=0
		if(j_err)return
		goto 1
 
	endif !if((ip.gt.0.or.j_inp(1:j_linp).eq.'/').and.ipv.gt.1)    402
	...
	if(j_inp(1:j_linp).ne.'/')then
		...
		if(ip.gt.0)then   !domains
			...
			17    call j_putnewcleantext(ivdomain,j_inp(ialdom:ip-1),iline)  !cleantext
				!		ndo=j_o(ivdomain)%i(0)
		!	write(6,*)'putting domain ',iline,'/',j_inp(ialdom:ip-1),'/',' ndom ',ndom
			...
			if(iline.gt.ndom)then  ! new domain definition ?
				...
				idom=j_object(j_inp(ialdom:ip-1))
				!		write(6,*)'<37 ',idom
				if(idom.le.0)then
					idom=j_object('$Dom'//domnum(1:le))
					!				write(6,*)'<38 ',idom
					if(idom.le.0)call j_getobject(0,'$Dom'//domnum(1:le),j_ipreal,idom)
					!		write(6,*)'here>','$Dom'//domnum(1:le)//'='//j_inp(ialdom:ip-1)
 
					call j_puttext(ivdomaintext,&
						'$Dom'//domnum(1:le)//'='//j_inp(ialdom:ip-1))
				endif !if(idom.le.0)    435
 
			else if(.not.repeatdom)then !if(iline.gt.ndom)then
				write(6,*)'**problem: domain ',j_inp(ialdom:ip-1), ' repeated without repeatdomains->'
				j_err=.true.
				return
			endif !if(iline.gt.ndom)    426
			...
			ivdomains(iline)=idom
			iperk=j_putlistobject(ivdomainvars,single=idom)
			! return
			! endif !if(j_err)then
			! using same transform as used in jcompil to work with loops/if then
			!	write(6,*)'<6363>iline,iperk',iline,iperk
			ndoms=ndoms+1  !domain occurences
			call j_puti(isetd,ndoms,iline)
			!		call j_puti(nsetd,nset,nsetd(nset)+1)
			ial=ip+1
			if(ial.gt.j_linp)then
				idomres=0
				goto 1
			endif !if(ial.gt.j_linp)    471
			ipv=j_nextlim0(j_inp,ial,j_linp,':') !Mela
			if(ipv.gt.ial)then !
				idomres=j_incline()
				ialdom=ial
				!	write(6,*)'<458>ipv,idomres,ialdom',ipv,idomres,ialdom
				goto 1
			else !if(ip.gt.ial)then
				write(*,*)'not legal domainrow:',j_inp(1:j_linp)
				j_err=.true.
				goto 900
			end if !if(ipv.gt.ial)    476
			...
			end if !if(irow.le.0)    489
			irow=irow+1
			ipe=j_putlistobject(ivrowdomain,single=idom,append=.true.)
			...
			if(ipe.gt.0)then
				...
				if(ipg+ipl.gt.0)then
					...
					if(ipg.gt.0)then
						...
						call j_putd(rhs_,irow,j_ninf )
					end if !if(ipg.gt.0)    518
					if(ipl.gt.0)then
						...
						call j_putd(rhs2_,irow,j_inf)
					end if !if(ipl.gt.0)    528
					if(ipg.gt.0.and.ipl.gt.0)then
						ip=min(ipg,ipl)
					else !if(ipg.gt.0.and.ipl.gt.0)then
						ip=ipg+ipl
					end if !if(ipg.gt.0.and.ipl.gt.0)    541
 
				else !if(ipg+ipl.gt.0)then
					ipmin=index(j_inp(1:j_linp+1),'==min')
					if(ipmin.gt.0)then
						call j_putd( rhs_,irow,j_0)
						call j_putd(rhs2_,irow,j_ninf)
						ip=ipmin
						irowobj=irow
						...
						if(ipmax.le.0)then
							...
							j_err=.true.
							goto 900
						end if !if(ipmax.le.0)    556
						call j_putd( rhs2_,irow,j_0)
						call j_putd( rhs_,irow,j_inf)
						irowobj=irow
						ip=ipmax
					end if !if(ipmin.gt.0)    549
 
				end if !if(ipg+ipl.gt.0)    515
			end if !if(ipe.gt.0)    508
		...
		!	if(j_err)write(6,*)'<36366ertas'
			...
			if(j_inp(1:1).eq.'#')then
				...
				if(iendp_.le.0.or.iendp_.eq.le.or.iendp_.eq.2)then
					...
					if(iperiod_.le.0.or.j_err)then
						...
						j_err=.true.
						return
					endif !if(iperiod_.le.0.or.j_err)    584
 
				endif !if(iendp_.le.0.or.iendp_.eq.le.or.iendp_.eq.2)    578
 
				ialrow=iendp_+1
				if(.not.allocated(iperiods))then
					allocate(iperiods(1:500))
					iperiods=0
				endif !if(.not.allocated(iperiods))    593
				If(.not.isperiod)iperiods=0
				isperiod=.true.
				call j_puti(iperiods,irow,iperiod_)
			else !if(j_inp(1:1).eq.'#')then
				ialrow=1
			endif !if(j_inp(1:1).eq.'#')    575
			...
			if(j_inp(ial7:ial7).eq.'(')then
				...
				endif !if(irp.eq.ial7+1.or.irp.ge.le.or.j_inp(irp+1:irp+1).ne.'*'    618
				...
				if(ivcoe_.gt.0)then
					...
					elseif(j_otype(ivcoe_).eq.j_ipmatrix)then !if(j_otype(ivcoe_).eq.j_ipreal)then
						...
						j_err=.true.
						return
					endif !if(j_otype(ivcoe_).eq.j_ipreal)    625
				...
				if(ine.lt.ine2)then   !explicit coefficient
					...
					if(ivcoe_.gt.0)then
						...
						elseif(j_otype(ivcoe_).eq.j_ipmatrix)then !if(j_otype(ivcoe_).eq.j_ipreal)then
							...
							j_err=.true.
							return
						endif !if(j_otype(ivcoe_).eq.j_ipreal)    661
				...
				end if !if(ine.lt.ine2)    654
			endif !if(j_inp(ial7:ial7).eq.'(')    616
			...
			if(varlist_) then
				...
				if(coevec_) then
					...
					endif !if(lenlist_ /= nrows_ .or. ncols_ /= 1)    716
				endif !if(coevec_)    713
			endif !if(varlist_)    707
			...
			goto 1
		end if !if(ip.gt.0)    415
	end if !if(j_inp(1:j_linp).ne.'/')    413
	!write(6,*)'hep2'
	...
	endif !if(isperiod)    767
	!	write(6,*)'ndom ',ndom,'source'
	...
	endif !if(ndom.gt.1)    779
	!write(6,*)'dhhd'
	call j_getobject(iout,' ',j_ipproblem,ivout)   !miksei iout käy suoraan, kunhan typen määrittelsi?
	!	write(6,*)'ivout,iout',ivout,iout
	ih= 16                  !6+nset+1+nset+1+ndoms+irow+nval+ivdomainvars
	ih2=2*nset+ndoms+irow+nval+nval+ndom !last is for ivdomains
	...
	! h(15)
	j_o(ivout)%i(1)=ivrhs;j_o(ivout)%i(2)=ivrhs2
	j_o(ivout)%i(3)=ivdomain;j_o(ivout)%i(4)=ivrow
	...
	j_o(ivout)%i(15)=0
	if(ndom.gt.0)then
		j_o(ivout)%i(15)=j_object(probname(1:lprobname)//'%domtrans')
		j_o(ivout)%i(16)=ivrowdomain
	endif !if(ndom.gt.0)    832
	!	write(6,*)j_o(ivout)%i(15),'<rui'
	!	write(6,*)'<ivtrans ',j_o(ivout)%i(15),probname(1:lprobname)//'%domtrans'
	!	j_otitle(ivout)=ivtitle_
 
	!nsetr number of rows for each domain, domain 0 is nothing
	! nsetd =number of doamins in each set
	!	write(6,*)'nset,ndoms,irow,nval,jlp_ndom',nset,ndoms,irow,nval,ndom
 
	!write(6,*)'ndom',ndom,'nset ',nset,'nsetr',nsetr(1:nset)
	j_o(ivout)%i2(1:nset)=nsetr(1:nset)
 
	j_o(ivout)%i2(nset+1:2*nset)=1  !nsetd(1:nset)
 
	!	write(6,*)'nsetd',nsetd(1:nset)
	j_o(ivout)%i2(nset+1:nset+ndoms)=isetd(1:ndoms) !mitkä domainit seteissä
	!write(6,*)'isetd',isetd(1:nset)
	j_o(ivout)%i2(nset+ndoms+1:nset+ndoms+irow)=nvars(1:irow)
	!write(6,*)'nvars',nvars(1:irow)
	j_o(ivout)%i2(nset+ndoms+irow+1:nset+ndoms+irow+nval)=irowvars(1:nval)
	if(ndom.gt.0)	j_o(ivout)%i2(nset+ndoms+irow+nval+1:nset+ndoms+irow+nval+ndom)=ivdomains(1:ndom)
	!	if(ndom.gt.0)write(6,*)'ivdomains ',ivdomains(1:ndom)
	j_o(ivout)%d(1:nval)=coef(1:nval)
 
	nrow=nsetr(0)
	do i=1,nset
		nrow=nrow+nsetr(i)   !nsetd(i)*nsetr(i)
	end do !i=1,nset    860
	j_o(ivout)%i(11)=nrow
	...
	deallocate(nsetr,isetd,ivars,irowvars,coef,nvars) !nsetd,
...
subroutine jlpcurix(iuni)   !!!!
	...
	if(jlp_ndom.le.0)then
		do jcurix=0,jlp_nrow
			...
			endif !if(jlp_ix(jcurix).ne.0)    900
		enddo !jcurix=0,jlp_nrow    898
		return !do jcurix=0,j_nrow;j_ixcur(jcurix)=j_ix(jcurix)
	endif !if(jlp_ndom.le.0)    897
	idombas=(iuni-1)*jlp_ndomv
	!write(6,*)'<45 ',iunit,idomba
	do jcurix=0,jlp_nrow
		if(jlp_ix(jcurix).gt.0)then   !is there x var
 
			icurint=(jlp_irowdomain(jcurix)-1)/32+1  !integer part
 
			icurbit=jlp_irowdomain(jcurix)-(icurint-1)*32-1  !bit part
			!	write(6,*)'<4554icurint,icurbit,idombas,iuni,jlp_ndomv ',icurint,icurbit,idombas,iuni,jlp_ndomv
			!			if(btest(jlp_domainbits(icurint,iuni),icurbit))then
			if(btest(jlp_domainbits(idombas+icurint),icurbit))then
				jlp_ixcur(jcurix)=jlp_ix(jcurix)
				jlp_nrowp=jlp_nrowp+1
				jlp_ixcurrows(jlp_nrowp)=jcurix  !luetellaan
			else !if(btest(j_domainbits(icurint,iuni),icurbit))then
				jlp_ixcur(jcurix)=0
			endif !if(btest(jlp_domainbits(idombas+icurint),icurbit))    917
		else !if(j_ix(jcurix).gt.0)then
			jlp_ixcur(jcurix)=0
		endif !if(jlp_ix(jcurix).gt.0)    910
	enddo !jcurix=0,jlp_nrow    909
	!	write(6,*)'jlp_nrow,jlpnrowp''jlp_nrow,jlpnrowp'
...
subroutine jlpfcurix(iuni)   !!!!
	...
	if(jlp_ndom.le.0)then
		do jcurix=0,jlp_nrow
			...
			endif !if(jlp_nfxrow(jlp_irowrow(jcurix)).ne.0)    948
		enddo;return !jcurix=0,jlp_nrow    946
	endif !if(jlp_ndom.le.0)    945
	...
	do jcurix=0,jlp_nrow
		...
		if(jlp_nfxrow(jlp_irowrow(jcurix)).ne.0)then
			...
			if(btest(jlp_domainbits(idombas+icurint),icurbit))then
				...
				jlp_ifxcurrows(jlp_nrowpf)=jcurix
				jlp_ixcurfact(jcurix)=.true.
			endif !if(btest(jlp_domainbits(idombas+icurint),icurbit))    964
		endif !if(jlp_nfxrow(jlp_irowrow(jcurix)).ne.0)    960
	enddo !jcurix=0,jlp_nrow    958
	...
	if(jlp_ndom.le.0)then
		do jcurix=0,jlp_nrow
			if(jlp_nfyrow(jlp_irowrow(jcurix)).ne.0)then
				...
				jlp_ixcurfact(jcurix)=.true.
			endif !if(jlp_nfyrow(jlp_irowrow(jcurix)).ne.0)    983
		enddo !jcurix=0,jlp_nrow    982
		return !do jcurix=0,j_nrow
	endif !if(jlp_ndom.le.0)    981
	...
	do jcurix=0,jlp_nrow
		if(jlp_nfyrow(jlp_irowrow(jcurix)).ne.0)then
			...
			if(btest(jlp_domainbits(idombas+icurint),icurbit))then
				...
				jlp_ifycurrows(jlp_nrowpfy)=jcurix
				jlp_ixcurfact(jcurix)=.true.
			endif !if(btest(jlp_domainbits(idombas+icurint),icurbit))    998
		endif !if(jlp_nfyrow(jlp_irowrow(jcurix)).ne.0)    995
	enddo !jcurix=0,jlp_nrow    994
...
subroutine jlpfirow2curix(iuni)   !!!!
	...
	if(jlp_ndom.le.0)then
		do jcurix=0,jlp_nrow
			...
			endif !if((jlp_nfxrow(irow_).ne.0).or.(jlp_nfyrow(irow_).ne.0))   1018
		enddo !jcurix=0,jlp_nrow   1016
		return !do jcurix=0,j_nrow
	endif !if(jlp_ndom.le.0)   1015
	...
	do jcurix=0,jlp_nrow
		...
		if((jlp_nfxrow(irow_).ne.0).or.(jlp_nfyrow(irow_).ne.0))then
			...
			if(btest(jlp_domainbits(idombas+icurint),icurbit))then
				jlp_irow2curix(0,irow_)=jlp_irow2curix(0,irow_)+1
				jlp_irow2curix(jlp_irow2curix(0,irow_),irow_)=jcurix
			endif !if(btest(jlp_domainbits(idombas+icurint),icurbit))   1035
		endif !if((jlp_nfxrow(irow_).ne.0).or.(jlp_nfyrow(irow_).ne.0))   1032
	enddo !jcurix=0,jlp_nrow   1029
...
subroutine priceunit(iob,io)  !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1114
	...
	if(iunit.gt.jlp_nunits.or.iunit.le.0)then
		...
		j_err=.true. ;return
	endif !if(iunit.gt.jlp_nunits.or.iunit.le.0)   1124
	...
	ibxmatx=(iobs-1)*jlp_ntemp0  !xmatiba(iobs) !,1)
	do jj=1,jlp_nrowp
		j=jlp_ixcurrows(jj)
		!	j_value=j_value+j_vx(j)*j_xmat(j_ix(j),iobs) !v(ix(j))
		jlp_value=jlp_value+jlp_vx(j)*jlp_xmat(ibxmatx+jlp_ix(j)) !v(ix(j))
	enddo !jj=1,jlp_nrowp   1133
	j_v(iout)=jlp_value
...
subroutine priceschedw(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1146
	narg=j_o(iob)%i(io+1)
	iout=j_o(iob)%i(io+2+narg)
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	iunit=j_v( j_o(iob)%i(io+2) )
	...
	do jj=1,jlp_nrowp
		j=jlp_ixcurrows(jj)
		jlp_value=jlp_value+jlp_vx(j)*jlp_xmat(ibxmatx+jlp_ix(j)) ! ,iobs) !v(ix(j))
	enddo !jj=1,jlp_nrowp   1169
	j_v(iout)=jlp_value
...
subroutine priceschedcum(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1181
	narg=j_o(iob)%i(io+1)
	iout=j_o(iob)%i(io+2+narg)
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	...
	if(is.gt.jlp_lopp.or.is.le.0)then
		...
		j_err=.true. ;return
	endif !if(is.gt.jlp_lopp.or.is.le.0)   1190
	ibxmatx=(is-1)*jlp_ntemp0  !xmatiba(is) !,1)
	call jlpcurix(iunit)
	jlp_value=j_0
	do jj=1,jlp_nrowp
		j=jlp_ixcurrows(jj)
		!j_value=j_value+j_vx(j)*j_xmat(j_ix(j),is) !,is) !v(ix(j))
		jlp_value=jlp_value+jlp_vx(j)*jlp_xmat(ibxmatx+jlp_ix(j)) !,is) !v(ix(j))
	enddo !jj=1,jlp_nrowp   1197
	j_v(iout)=jlp_value
...
subroutine weights(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1214
	...
	if(narg.eq.0)then
		...
		do i=1,jlp_ndiv
			...
			elseif(jlp_iunitdiv(i).gt.iunit)then !if(j_iunitdiv(i).eq.iunit)then
				goto 77
			endif !if(jlp_iunitdiv(i).eq.iunit)   1229
		end do !i=1,jlp_ndiv   1228
		ns=1
		!
		77  j_v(iout)=ns
	end if !if(narg.eq.0)   1223
	!	io=io+j_o(iob)%i(io+1)+3
	return
...
function nweights()  !number of nonzero weights tehty Markulle  !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1244
...
subroutine partweights(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1261
	! if narg=1 then total number of weights, if narg=2 then weight in unit
	narg=j_o(iob)%i(io+1)
	iout=j_o(iob)%i(io+2+narg)
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	...
	if(narg.eq.0)then
		...
		do i=1,jlp_ndiv
			if(jlp_iunitdiv(i).eq.iunit)then
				ns=ns+1
			endif !if(jlp_iunitdiv(i).eq.iunit)   1277
		enddo !i=1,jlp_ndiv   1276
		77  j_v(iout)=ns
	endif !if(narg.eq.0)   1271
	!	io=io+narg+3
	return
...
subroutine jlpunit(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1290
	...
	if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ndiv)then
		...
		do j=1,jlp_ndiv
			...
			if(ib.ge.ibas)then
				iunit=iunit-(ib-ibas)
				goto 77
			endif !if(ib.ge.ibas)   1311
			iunitv=iunit
		enddo !j=1,jlp_ndiv   1305
		! all
		iunit=ibas-niv
		77  j_v(j_o(iob)%i(io+j_o(iob)%i(io+1)+2))=iunit
 
	endif !if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ndiv)   1296
...
function lunit_(ibas)    !!!!
	...
	do j=1,jlp_ndiv
		...
		if(ib.ge.ibas)then
			iunit=iunit-(ib-ibas)
			goto 77
		endif !if(ib.ge.ibas)   1341
		iunitv=iunit
	enddo !j=1,jlp_ndiv   1335
	! all
	iunit=ibas-niv
	77 lunit_=iunit
...
subroutine partunit(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1356
	narg=j_o(iob)%i(io+1)
	iout=j_o(iob)%i(io+2+narg)
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	...
	if(idiv.le.0.or.idiv.gt.jlp_ndiv)then
		...
		! all
	endif !if(idiv.le.0.or.idiv.gt.jlp_ndiv)   1365
	!	90 io=io+j_o(iob)%i(io+1)+3
	return
...
subroutine weight(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1377
	...
	if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ndiv)then
		...
		do j=1,jlp_ndiv
			...
			if(ib.eq.ibas)then
				...
				wei=1.   !are
				goto 77
			endif !if(ib.eq.ibas)   1394
			iunitv=iunit
		enddo !j=1,jlp_ndiv   1390
		! all
		wei=1.
		77  j_v(j_o(iob)%i(io+j_o(iob)%i(io+1)+2))=wei
 
	endif !if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ndiv)   1383
	!	90 io=io+j_o(iob)%i(io+1)+3
	return
...
function we_(ibas)    !!!!
	...
	do j=1,jlp_ndiv
		...
		if(ib.eq.ibas)then
			...
			!  iunit=ibas-nivv
			goto 77
		endif !if(ib.eq.ibas)   1423
		iunitv=iunit
	enddo !j=1,jlp_ndiv   1418
	! all
	wei=1.
	77  we_=wei
	return
...
subroutine schedweight(ibas,is,wei) !returns for ibas'th schedule twith nonzero weight the   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1441
	...
	if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ndiv)then
		...
		do j=1,jlp_ndiv
			...
			if(ib.eq.ibas)then
				...
				wei=1.   !are
				goto 77
			end if !if(ib.eq.ibas)   1459
			iunitv=iunit
		end do !j=1,jlp_ndiv   1454
		! all
		wei=1.
		iunit=ibas-niv;is=jlp_ibaunit(iunit)+jlp_keys(iunit)
		77  continue !  v(o(iob)%i(io+o(iob)%i(io+1)+2))=wei
 
	end if !if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ndiv)   1447
...
subroutine weightschedcum(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1483
	...
	if(isc.le.0.or.isc.gt.jlp_lopp)then
		...
		return
	endif !if(isc.le.0.or.isc.gt.jlp_lopp)   1489
	...
	if(isc.le.jlp_ibaunit(iunit0))then
		...
		enddo !iunit=iunit0-1,1,-1   1497
	else !if(isc.le.j_ibaunit(iunit0))then
		do iunit=iunit0,jlp_nunits
			if(isc.le.jlp_ibaunit(iunit+1))exit
		enddo !iunit=iunit0,jlp_nunits   1501
	endif !if(isc.le.jlp_ibaunit(iunit0))   1496
	...
	if(j_linkoption(iob,io,j_minteger).ge.0)then
		...
		do j=1,jlp_ndiv
			...
			if(jlp_iunitdiv(j).eq.iunit)then
				...
				enddo !j2=j+1,jlp_ndiv   1513
				if(is.eq.jlp_isdiv(jmax))then
					wei=1.
				else !if(is.eq.j_isdiv(jmax))then
					wei=0.
					goto 90
				endif !if(is.eq.jlp_isdiv(jmax))   1517
			endif !if(jlp_iunitdiv(j).eq.iunit)   1511
		enddo !j=1,jlp_ndiv   1509
			...
			if(jlp_iunitdiv(j).eq.iunit)then
				do j2=j,jlp_ndiv
					...
					if(is.eq.jlp_isdiv(j2))then
						wei=jlp_wdiv(j2)
						goto 90
					endif !if(is.eq.jlp_isdiv(j2))   1533
				enddo !j2=j,jlp_ndiv   1531
				wei=0.
				goto 90
			endif !if(jlp_iunitdiv(j).eq.iunit)   1530
		enddo !j=1,jlp_ndiv   1528
	endif !if(j_linkoption(iob,io,j_minteger).ge.0)   1507
 
	if(is.eq.jlp_keys(iunit))then
		wei=1.
	else !if(is.eq.j_keys(iunit))then
		wei=0.
	endif !if(is.eq.jlp_keys(iunit))   1544
	90 continue
	...
	do j=1,jlp_ndiv
		...
		if(jlp_iunitdiv(j).eq.iunit)then
			...
			do j2=j+1,jlp_ndiv
				if(jlp_iunitdiv(j2).ne.iunit)exit
				if(jlp_wdiv(j2).gt.jlp_wdiv(jmax))jmax=j2
			enddo !j2=j+1,jlp_ndiv   1575
			wei=jlp_isdiv(jmax)
			goto 90
		endif !if(jlp_iunitdiv(j).eq.iunit)   1573
	enddo !j=1,jlp_ndiv   1571
...
subroutine weightschedw(iob,io)  !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1599
	...
	if(iunit.le.0.or.iunit.gt.jlp_nunits)then
		...
		write(6,*)'**weight%schedw: trying to access unit',iunit, 'maximum is:',jlp_nunits
		return
	endif !if(iunit.le.0.or.iunit.gt.jlp_nunits)   1606
	...
	if(is.le.0.or.is.gt.jlp_nsch(iunit))then
		...
		return
	endif !if(is.le.0.or.is.gt.jlp_nsch(iunit))   1612
		...
		do j=1,jlp_ndiv
			...
			if(jlp_iunitdiv(j).eq.iunit)then
				...
				do j2=j+1,jlp_ndiv
					if(jlp_iunitdiv(j2).ne.iunit)exit
					if(jlp_wdiv(j2).gt.jlp_wdiv(jmax))jmax=j2
				enddo !j2=j+1,jlp_ndiv   1624
				if(is.eq.jlp_isdiv(jmax))then
					wei=1.
				else !if(is.eq.j_isdiv(jmax))then
					wei=0.
					goto 90
				endif !if(is.eq.jlp_isdiv(jmax))   1628
			endif !if(jlp_iunitdiv(j).eq.iunit)   1622
		enddo !j=1,jlp_ndiv   1620
			...
			if(jlp_iunitdiv(j).eq.iunit)then
				do j2=j,jlp_ndiv
					...
					if(is.eq.jlp_isdiv(j2))then
						wei=jlp_wdiv(j2)
						goto 90
					endif !if(is.eq.jlp_isdiv(j2))   1644
				enddo !j2=j,jlp_ndiv   1642
				wei=0.
				goto 90
			endif !if(jlp_iunitdiv(j).eq.iunit)   1641
		enddo !j=1,jlp_ndiv   1639
 
	endif !if(j_linkoption(iob,io,j_minteger).ge.0)   1618
 
	if(is.eq.jlp_keys(iunit))then
		wei=1.
	else !if(is.eq.j_keys(iunit))then
		wei=0.
	endif !if(is.eq.jlp_keys(iunit))   1656
	90 continue
...
subroutine partweight(iob,io)  !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1672
	narg=j_o(iob)%i(io+1)
	iout=j_o(iob)%i(io+2+narg)
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	idiv=j_v(j_o(iob)%i(io+2))  ! for which the unit is wanted
	if(idiv.le.0.or.idiv.gt.jlp_ndiv)then
		...
		! all
	endif !if(idiv.le.0.or.idiv.gt.jlp_ndiv)   1681
	!	90 io=io+j_o(iob)%i(io+1)+3
	return
...
subroutine schedw(iob,io)  !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1693
	ibas=j_v(j_o(iob)%i(io+2))  ! for which the weight is wanted
	if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ld0)then
		...
		do j=1,jlp_ndiv
			...
			if(ib.eq.ibas)then
				...
				goto 77
			end if !if(ib.eq.ibas)   1711
			iunitv=iunit
		end do !j=1,jlp_ndiv   1705
		! all
		iunit=ibas-niv;is=jlp_keys(iunit) ! o(isol)%i(iunit)
		77 j_v(j_o(iob)%i(io+j_o(iob)%i(io+1)+2))=is
 
	end if !if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ld0)   1699
...
function ls_(ibas)   !!!!
	...
	do j=1,jlp_ndiv
		...
		if(ib.eq.ibas)then
			...
			is=jlp_keys(iunit) ! o(isol)%i(iunit)
			goto 77
		end if !if(ib.eq.ibas)   1745
		iunitv=iunit
	end do !j=1,jlp_ndiv   1739
	! all
	iunit=ibas-niv;is=jlp_keys(iunit) ! o(isol)%i(iunit)
	77 ls_=is
...
subroutine schedcum(iob,io)  !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1765
	...
	if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ld0)then
		...
		do j=1,jlp_ndiv
			...
			if(ib.eq.ibas)then
				...
				goto 77
			end if !if(ib.eq.ibas)   1783
			iunitv=iunit
		end do !j=1,jlp_ndiv   1778
		! all
		iunit=ibas-niv;is=jlp_ibaunit(iunit)+jlp_keys(iunit) !o(isol)%i(iunit)+ o(isol)%i2(4+iunit)
 
		77  j_v(j_o(iob)%i(io+j_o(iob)%i(io+1)+2))=is
 
	end if !if(ibas.le.0.or.ibas.gt.jlp_nunits+jlp_ld0)   1772
...
subroutine partschedw(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1808
	narg=j_o(iob)%i(io+1)
	...
	endif !if(idiv.le.0.or.idiv.gt.jlp_ndiv)   1819
	return
	!io=io+j_o(iob)%i(io+1)+3
	!	return
...
subroutine partschedcum(iob,io)   !!!!
	...
	if(j_v(j_ivstartedjlp).eq.0)then
		...
		j_err=.true.
		return
	endif !if(j_v(j_ivstartedjlp).eq.0)   1834
	narg=j_o(iob)%i(io+1)
	isol=j_o(iob)%i(io+2)
	...
	endif !if(idiv.le.0.or.idiv.gt.jlp_ndiv)   1848
	return
	!	90 io=io+narg+3
	!	return
...
subroutine xkf(iob,io) !!!!
	...
	elseif(j_otype(ivfile).eq.j_ipchar)then !if(ivfile.eq.j_ivdollar)then
		...
		if(nu.gt.0)then
			...
			if(faction(1:4).eq.'READ')then
				call j_closeunit(nu)
				nu=0
			endif !if(faction(1:4).eq.'READ')   1904
		endif !if(nu.gt.0)   1902
		if(nu.le.0)then
			bin_ = .false.
	...
	if(.not.textform)then
		...
		do i=1,j_o(jlp_ivxkyk)%i(1)
			...
			len1=max(len1,le)
		enddo !i=1,j_o(jlp_ivxkyk)%i(1)   1931
 
		len2=0
		do i=1,j_o(jlp_ivfact)%i(1)
			le=j_leno(j_o(jlp_ivfact)%i2(i))
			len2=max(len2,le)
		enddo !i=1,j_o(jlp_ivfact)%i(1)   1937
	endif !if(.not.textform)   1921
	...
	do iuni_ = 1,jlp_nunits
		...
		endif !if(jlp_ivtrans.gt.0)   1949
		dividedunit_ = .false.
		...
		if(jlp_ndiv>0) then
			...
			do while((jlp_iunitdiv(jj)/=iuni_).and.(jj<jlp_ndiv))
				jj = jj + 1
			enddo !while((jlp_iunitdiv(jj)/=iuni_).and.(jj<jlp_ndiv))   1961
			dividedunit_= jlp_iunitdiv(jj) == iuni_
		endif !if(jlp_ndiv>0)   1958
		...
		if (dividedunit_) then
			...
			do while(iuniw_==iuni_)
				...
				endif !if(jlp_ivsubtrans.gt.0)   1979
				...
				enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)   1987
				...
				if(jj<jlp_ndiv) then
					...
					iuniw_ = -1
				endif !if(jj<jlp_ndiv)   1994
			enddo !while(iuniw_==iuni_)   1971
			...
			endif !if(jlp_ivsubtrans.gt.0)   2010
			...
			enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)   2019
 
		endif !if (dividedunit_)   1968
		...
		if(icol_<=jlp_lf0_xkfsol) then
			do while ((icol_<jlp_lf0_xkfsol).and.(jlp_xkfsol(icol_)%iunit/= iuni_))
				icol_=icol_+1
			enddo !while ((icol_<jlp_lf0_xkfsol).and.(jlp_xkfsol(icol_)%iunit   2035
			...
			if (jlp_xkfsol(icol_)%iunit == iuni_) then
				...
				enddo !while(icol_>jlp_mxd)   2044
			endif !if (jlp_xkfsol(icol_)%iunit == iuni_)   2040
		endif !if(icol_<=jlp_lf0_xkfsol)   2034
		...
		do if_= 1,j_o(jlp_ivfact)%i(1)
			do  ixk_= 1,j_o(jlp_ivxkyk)%i(1)
				...
				endif !if (sumxkf_(ixk_,if_)> amount0)   2065
			enddo ! ixk_= 1,j_o(jlp_ivxkyk)%i(1)   2064
		enddo !if_= 1,j_o(jlp_ivfact)%i(1)   2063
		if(p.and.kier.eq.0)then
			kier=1
			goto 100
		endif !if(p.and.kier.eq.0)   2084
	enddo !iuni_ = 1,jlp_nunits   1945
 
	if(ivfile /= j_ivdollar) then
		call j_closeunit(nu) ! JL 29.3. 2016
	endif !if(ivfile /= j_ivdollar)   2090
	deallocate(sumxkf_)
	j_v(ivout)=nrec
	return
...
integer function integerschedw(iunit)  ! %%integer solution   !!!!
	...
	if(iunit.le.0.or.iunit.gt.jlp_nunits)then
		...
		return
	endif !if(iunit.le.0.or.iunit.gt.jlp_nunits)   2103
	...
	do j=1,jlp_ndiv
		...
		if(jlp_iunitdiv(j).eq.iunit)then
			...
			do j2=j+1,jlp_ndiv
				if(jlp_iunitdiv(j2).ne.iunit)exit
				if(jlp_wdiv(j2).gt.jlp_wdiv(jmax))jmax=j2
			enddo !j2=j+1,jlp_ndiv   2113
			wei=jlp_isdiv(jmax)
			goto 90
		endif !if(jlp_iunitdiv(j).eq.iunit)   2111
	enddo !j=1,jlp_ndiv   2109
...
1 do j=1,i-1 ;name(j:j)=j_o(j_ivnames)%ch(ibas+j) ;end do
	...
	if(np.eq.2)then
		...
		if(iv2.le.0)then
			...
			j_err=.true.
		endif !if(iv2.le.0)   2173
		if(iv3.le.0)then
			call j_printname('**second part unknown ',iv,' ');
			j_err=.true.
		endif !if(iv3.le.0)   2177
		if(j_err)return
...
subroutine jlp(iob,io)   ! %%jlp  !!!!****************************************************
	...
	! both.
	! rhs Vector containing lower bound for each constraint row. Value j_inf is used to
	! indicate the absence of the lower bound in a row. Either or both of the bound
	! options (rhs->, rhs2->) has to be defined.
	! rhs2 Vector containing upper bound for each constraint row. Value j_inf is used to
	...
	double precision,dimension(:),allocatable:: xvars1,xvars2 !to delete redundant
	!cycle
	...
	logical ,dimension(:),allocatable::basreject !basic schedules in unit
	integer,dimension(1) ::loco
	double precision,dimension(:),allocatable:: svalue
	double precision,dimension(:),allocatable:: fastvalues  !the values of schedules in the current unit when setting up the
	...
	logical ::isunit
	logical ::isdomain
	integer, dimension (:), pointer :: domainvars
	...
	common/refactorc/nup,nfreq
	xmatiba(iobs)=(iobs-1)*jlp_ntemp0  !(j_memobs(iobs)-1)*j_ntemp0
	...
	if(noptarg.gt.0)then
		if(j_v(j_optarg0(1)).le.j_0)echo=.false.
	endif !if(noptarg.gt.0)   2783
	!	j_getoption_index(iob,io,moption,minarg,maxarg,iptype,expand,needsarg,noptarg,optarg,istart) ! %%option
	!	write(6,*)'<57',iob
	call j_getoption_index(iob,io,j_mnoint,-1,0,j_ipreal,.true.,noptarg,j_optarg0)
	!	call j_getoption(iob,j_mslow,-1,1,j_ipreal,.true.,nslow,slowopt)
	intapp=noptarg.lt.0
	...
	if(jlp_ivtrans.gt.0)then
		if(j_otype(jlp_ivtrans).ne.j_iptrans)then
			...
			j_err=.true. ;return
		endif !if(j_otype(jlp_ivtrans).ne.j_iptrans)   2823
	endif !if(jlp_ivtrans.gt.0)   2822
	...
	if(jlp_ivsubtrans.gt.0)then
		if(j_otype(jlp_ivsubtrans).ne.j_iptrans)then
			...
			j_err=.true. ;return
		endif !if(j_otype(jlp_ivsubtrans).ne.j_iptrans)   2831
	endif !if(jlp_ivsubtrans.gt.0)   2830
	...
	if(ivarea.eq.0)then   !option area->,
		...
		if(ivarea.le.0)then
			...
			j_err=.true.
			return
		endif !if(ivarea.le.0)   2844
	endif !if(ivarea.eq.0)   2842
	...
	if(notareav.gt.0)then
		...
		if(ivarea.lt.0)then
			...
			j_err=.true.
			return
		endif !if(ivarea.lt.0)   2853
	endif !if(notareav.gt.0)   2851
	...
	if(noptarg.eq.1)then
		...
		endif !if(nureport.le.0)   2869
 
	else !if(j_nargopt(iob,io,j_mreport).eq.1)then
		nureport=6
	endif !if(noptarg.eq.1)   2864
	...
	if(j_linkoption(iob,io,j_mtest).ge.0)then
		...
		end if !if(j_o(iob)%i(j_linkoption(iob,io,j_mtest)).eq.0)   2899
	else !if(j_linkoption(iob,io,j_mtest).gt.0)then
		jlp_testl=.false.
	end if !if(j_linkoption(iob,io,j_mtest).ge.0)   2898
	!debug
	p=.false.
	p2=.false.
	!	ivclass=0
	...
	if(j_linkoption(iob,io,j_mdebug).ge.0)then
		...
		if(ndebug.eq.0)then
			...
			if(idebug.le.0)then
				...
				if(ndebug.ge.2)then           ! pelaakohan tämä niinkuin pitäisi
					...
					jdebug=2
				endif !if(ndebug.ge.2)   2939
			endif !if(idebug.le.0)   2936
 
		endif !if(ndebug.eq.0)   2930
		...
		idebug=huge(1)
	endif !if(j_linkoption(iob,io,j_mdebug).ge.0)   2926
	ipivot=0
	...
	if(j_linkoption(iob,io,j_mzmatrix).gt.0)then
		if(j_nargopt(iob,io,j_mzmatrix).le.0)then
			...
			j_err=.true.;goto 990
		endif !if(j_nargopt(iob,io,j_mzmatrix).le.0)   2957
		zmatrix=.true.
		if(j_linkoption(iob,io,j_mdata).gt.0.or.j_linkoption(iob,io,j_mproblem).gt.0.or. &
			...
			j_err=.true.;goto 990
		endif !if(j_linkoption(iob,io,j_mdata).gt.0.or.j_linkoption(iob,i   2962
	...
	if (zobj) then
		...
		if((nzodim1*nzodim2).ne.nz) then
			...
			j_err=.true.;goto 990
		endif !if((nzodim1*nzodim2).ne.nz)   3016
	endif !if (zobj)   3010
	...
	if(j_linkoption(iob,io,j_mrhs).gt.0)then
		...
		if(j_otype(ivrhs).ne.j_ipmatrix) then
			...
			j_err=.true.;goto 990
		endif !if(j_otype(ivrhs).ne.j_ipmatrix)   3035
		...
		if((nrhsoptdim1*nrhsoptdim2).ne.jlp_nrow) then
			...
			j_err=.true.;goto 990
		endif !if((nrhsoptdim1*nrhsoptdim2).ne.jlp_nrow)   3040
	else !if(j_linkoption(iob,io,j_mrhs).gt.0)then
		rhsopt=.false.
	endif !if(j_linkoption(iob,io,j_mrhs).gt.0)   3024
 
	if(j_linkoption(iob,io,j_mrhs2).gt.0)then
		if(.not.zmatrix)then
			write(6,*)'*jlp: zmatrix-option missing'
			j_err=.true.;goto 990
		endif !if(.not.zmatrix)   3051
		if(j_nargopt(iob,io,j_mrhs2).le.0)then
			write(6,*)'*jlp: rhs-> should have argument'
			j_err=.true.;goto 990
		endif !if(j_nargopt(iob,io,j_mrhs2).le.0)   3055
		rhs2opt=.true.
		...
		if(j_otype(ivrhs2).ne.j_ipmatrix) then
			...
			j_err=.true.;goto 990
		endif !if(j_otype(ivrhs2).ne.j_ipmatrix)   3061
		...
		if((nrhs2optdim1*nrhs2optdim2).ne.jlp_nrow) then
			...
			write(6,*)'Elements in rhs->:',nrhsoptdim1*nrhsoptdim2
			j_err=.true.;goto 990
		endif !if((nrhs2optdim1*nrhs2optdim2).ne.jlp_nrow)   3066
	else !if(j_linkoption(iob,io,j_mrhs2).gt.0)then
		rhs2opt=.false.
	endif !if(j_linkoption(iob,io,j_mrhs2).gt.0)   3050
	...
	if (zmatrix) then
		...
		if(rhsopt) then
			...
			do i_=1,jlp_nrow
				if(jlp_rhs(i_).eq.j_ninf) then
					jlp_rhs(i_)=j_ninf
					jlp_lbou(i_)=.false.
				endif !if(jlp_rhs(i_).eq.j_ninf)   3113
			enddo !i_=1,jlp_nrow   3112
		endif !if(rhsopt)   3109
		if(rhs2opt) then
			jlp_ubou=.true.
			jlp_rhs2(1:jlp_nrow) = j_o(ivrhs2)%d(1:jlp_nrow)
			do i_=1,jlp_nrow
				if(jlp_rhs2(i_).eq.j_inf) then
					jlp_rhs2(i_)=j_inf
					jlp_ubou(i_)=.false.
				endif !if(jlp_rhs2(i_).eq.j_inf)   3123
			enddo !i_=1,jlp_nrow   3122
		endif !if(rhs2opt)   3119
	endif !if (zmatrix)   3089
	...
	if(jlp_ivprob.gt.0)then
		if(j_otype(jlp_ivprob).ne.j_ipproblem)then
			...
			goto 990
		endif !if(j_otype(jlp_ivprob).ne.j_ipproblem)   3147
		jlp_ndom=j_o(jlp_ivprob)%i(10)
 
		ivdomainvars=j_o(jlp_ivprob)%i(13)
		if(ivdomainvars.gt.0)then
			jlp_ndomainvars=j_o(ivdomainvars)%i(1)
			domainvars=>j_o(ivdomainvars)%i2(1:jlp_ndomainvars)
			ivdomtrans=j_o(jlp_ivprob)%i(15)
			ivrowdomain=j_o(jlp_ivprob)%i(16)  !index of row in domainvars
			isdomain=ivdomtrans.ne.0
		else !if(j_ivprob.gt.0)then
			if(.not.zmatrix) then
				write(6,*)'**no problem given';j_err=.true.;goto 990
			endif !if(.not.zmatrix)   3162
		endif !if(ivdomainvars.gt.0)   3155
	endif !if(jlp_ivprob.gt.0)   3146
	...
	if(jlp_xpresent)then
		if(isunit)then
			...
			if(ivxdata.le.0.or.j_otype(max(1,ivxdata)).ne.j_ipdata)then
				if(j_o(ivcdata)%i(5).ne.0.)then
					...
					endif !if(j_otype(j_o(ivcdata)%i(11)).ne.j_ipdata)   3191
					ivxdata=ivcdata
				...
				endif !if(j_o(ivcdata)%i(5).ne.0.)   3189
			endif !if(ivxdata.le.0.or.j_otype(max(1,ivxdata)).ne.j_ipdata)   3188
			!xdisk testaus
			! ivtestmat=0
		...
		endif !if(isunit)   3176
 
		ivxdatmat=j_o(ivxdata)%i(1)
		!	j_xdatinmemory=j_o(ivxdatmat)%i(4).gt.0
 
		!	j_xdatnu=-j_o(ivxdatmat)%i(4) !if xdat is not in file this is not used
		!		write(6,*)'ivxdatmat j_xdatnu',ivxdatmat,j_xdatnu
		...
		!	write(6,*)'dgdgu',ivxdatmat,jlp_ivkeepx,keepx,ivcmat,jlp_ivkeepc,keepc
 
	else !if(j_linkoption(iob,io,j_mdata).gt.0)then
		ivcdata=0
	endif !if(jlp_xpresent)   3175
	...
	if(j_linkoption(iob,io,j_mmaxiter).gt.0)then
		...
		mxiter=2000
	endif !if(j_linkoption(iob,io,j_mmaxiter).gt.0)   3238
	...
	if(j_linkoption(iob,io,j_mrefac).gt.0)then
		...
		nrefac=1000  !
	endif !if(j_linkoption(iob,io,j_mrefac).gt.0)   3246
...
7777 format(a16,i5)
		...
		if(jlp_nrow.le.1.and.sparse)then
			...
			sparse=.false.
		endif !if(jlp_nrow.le.1.and.sparse)   3350
		!nsetr number of rows in each domainset
		!nsetd=nubber of domains in each domainset
		...
		write(6,*)'nset,ndoms,irowp,nval,jlp_ndom',nset,ndoms,irowp,nval,jlp_ndom
 
		jlp_nsetr=>j_o(jlp_ivprob)%i2(1:nset)  !i+1 will match nsetd(i)
		!		jlp_nsetd=>j_o(jlp_ivprob)%i2(nset+1:2*nset)
		jlp_isetd=>j_o(jlp_ivprob)%i2(nset+1:nset+ndoms)
		jlp_nvars=>j_o(jlp_ivprob)%i2(nset+ndoms+1:nset+ndoms+irowp)
		jlp_irowvars=>j_o(jlp_ivprob)%i2(nset+ndoms+irowp+1:nset+ndoms+irowp+nval)
		if(jlp_ndom.gt.0)jlp_ivdomains=>j_o(jlp_ivprob)%i2(nset+ndoms+irowp+nval+1:nset+ndoms+irowp+nval+jlp_ndom)
		!	if(jlp_ndom.gt.0)write(6,*)'<34ivdom',jlp_ivdomains
			...
			do k=1,jlp_nsetr(i) ! number of rows in set i
				...
				if(irow0.ne.irowobj)then
					...
					if(j_o(ivrhs2)%d(irow0).eq.0.)then
						...
						jlp_maxo=.false.
						coefmax=-j_1
					end if !if(j_o(ivrhs2)%d(irow0).eq.0.)   3427
				end if !if(irow0.ne.irowobj)   3410
			end do !k=1,jlp_nsetr(i)   3408
			!			end do !do j=1,j_nsetd(i)
		end do !i=1,nset   3404
		jlp_lbou=jlp_rhs.gt.j_ninf   ! is there lower bound
		jlp_ubou=jlp_rhs2.lt.j_inf   ! is there upper bound
	endif !if (.not.zmatrix)   3392
	!write(6,*)'<768'
	...
	if(jlp_xpresent)then   !xpresent means that data-> , even if all variables  !!!!
			...
			if(iv2.ne.0)then	! tehdasmuuttuja
				if(.not.jlp_fpresent) then
					...
					if(ivoutresult.eq.j_ivresult) then
						...
						j_err=.true.
						goto 930
					endif !if(ivoutresult.eq.j_ivresult)   3515
					...
					if(jlp_nrow.eq.0)then
						...
						j_err=.true.
						return
					endif !if(jlp_nrow.eq.0)   3528
 
				endif !if(.not.jlp_fpresent)   3513
				...
				endif !if(itrans.eq.0)   3537
			endif !if(iv2.ne.0)   3512
		enddo !i0=1,nvartot   3502
		...
		if(jlp_fpresent) then   !!!! initilization for factories
			...
			if(allocated(jlp_xkfact))then
				if(warmf.and.size(jlp_xkfact,dim=2).ne. j_o(jlp_ivfact)%i(1))then
					write(6,*)'*number of factories is different, warm-> ignored for factories'
					warmf=.false.
				endif !if(warmf.and.size(jlp_xkfact,dim=2).ne. j_o(jlp_ivfact)%i(   3619
				deallocate(jlp_xkfact)
			else !if(allocated(j_xkfact))then
				warmf=.false.  !factories not allocated
			endif !if(allocated(jlp_xkfact))   3618
			...
			enddo !i_=1,j_o(jlp_ivxkyk)%i(1)   3654
 
		endif !if(jlp_fpresent)   3586
		...
		do i0=1,nvartot       !***********************************
			...
			if(ikeep.le.0.and.iout.le.0)then  !is variable in cdata
				...
				if(ikeep.gt.0.or.iout.gt.0)then
					...
					jlp_cvarl(ncvar)=i
				endif !if(ikeep.gt.0.or.iout.gt.0)   3678
			endif !if(ikeep.le.0.and.iout.le.0)   3675
			...
			if(ikeep.gt.0.or.iout.gt.0)then
				...
				do j=1,irowp  !row definitions
					do k=1,jlp_nvars(j) !nvars number of vars in each row
						...
						endif !if(jlp_irowvars(ival).eq.i)   3697
					enddo !k=1,jlp_nvars(j)   3694
 
				enddo !j=1,irowp   3693
				!factory
					...
					if(itrans.ne.0)then !****!itrans= object trans%util%%sawlog%%sawmill
						...
						do ii=1,j_o(iv2)%i(1)
							...
							toka:	   			do jj=1,j_o(iv3)%i(1)
								...
								enddo !inx=1,jlp_nxkfact(lxk)   3762
								...
								nameloop:  			do ivn=1,j_o(j_ivnames)%i(0)
									...
									if(ivtemp.gt.0)then
										...
										j_err=.true.
										return
									endif !if(ivtemp.gt.0)   3801
									!Jos vastaavutta ei löydy, pitäisi palautua virheilmoitus
								enddo nameloop !eloop:  			do ivn=1,j_o(j_ivnames)%i(0)   3775
								call j_printname('transformation ',itrans,' :')
								call j_getname(iv2el,iv3el)
								write(6,*)'does not contain outputvariable ',j_oname(1:j_loname)//'%%'//j_oname2(1:j_loname2)
								!	call j_printname('not found outputvariable for ',iv2el,' ')
								!	call j_printname(' and ',iv3el,' ')
								j_err=.true.
								return
 
								2578						enddo toka !a:	   			do jj=1,j_o(iv3)%i(1)   3754
						enddo !ii=1,j_o(iv2)%i(1)   3741
							...
							do k=1,jlp_nvars(j) !nvars number of vars in each row
								...
								if(jlp_irowvars(ival).eq.i)then
									...
									if(iiv2_.eq.0) then ! iv2-listaa ei ole vielä käsitelty
										...
										do ii=1,j_o(iv2)%i(1)
											...
											if(iv2elpos.le.0) then
												...
												enddo !ij=1,j_o(jlp_ivkeepx)%i(1)   3848
												j_err=.true.
												return
											endif !if(iv2elpos.le.0)   3846
											ifykeep = ifykeep+1
									...
									if(iiv3_.eq.0) then ! iv3-tehdaslistaa ei ole vielä käsitelty -> etsitään paikat factories-listasta
										...
										enddo !ii=1,j_o(iv3)%i(1)   3869
									else !if(iiv3_.eq.0) then
										jlp_ibafyfact(ival) = jlp_ibafyfact(iiv3_)
									endif !if(iiv3_.eq.0)   3867
 
									if(jlp_coef(ival).ne.1.) then
										write(6,*)'Betas must be 1'
										j_err=.true.
										return
									endif !if(jlp_coef(ival).ne.1.)   3878
								endif !if(jlp_irowvars(ival).eq.i)   3832
							enddo !k=1,jlp_nvars(j)   3829
						enddo !j=1,irowp   3828
						...
						enddo !inx=1,jlp_nxkfact(lxk)   3890
						! tehdasta ei löytynyt taulukosta
						! kasvatetaan ko muuttujan tehtaiden lkm ja lisätään tehdas taulukkoon
						jlp_nxkfact(lxk) = jlp_nxkfact(lxk)+1
						jlp_xkfact(lxk,jlp_nxkfact(lxk))%ifact=lff
						...
						do j=1,irowp  !row definitions
							do k=1,jlp_nvars(j) !nvars number of vars in each row
								...
								endif !if(jlp_irowvars(ival).eq.i)   3905
							enddo !k=1,jlp_nvars(j)   3902
						enddo !j=1,irowp   3901
 
					endif !if(itrans.ne.0)   3729
				endif !if(iv2.ne.0)   3723
 
			endif !if(ikeep.gt.0.or.iout.gt.0)   3689
		enddo !i0=1,nvartot   3662
 
		if(nxvar+nfx+nfy.le.0)then
			write(6,*)'**all variables are z variables even if data->';j_err=.true.;goto 940
		end if !if(nxvar+nfx+nfy.le.0)   3921
		...
		if(jlp_fpresent.and.(nfy == 0)) then
			...
			j_err = .true.
			return
		endif !if(jlp_fpresent.and.(nfy == 0))   3938
	...
	if(sparse)then  !should be tested if it works, not commented yer
		...
		if(allocated(jlp_acol))deallocate(jlp_acol);allocate(jlp_acol(1:jlp_nrow));jlp_acol=j_0
		if(allocated(jlp_acolapu))deallocate(jlp_acolapu);allocate(jlp_acolapu(1:jlp_nrow));jlp_acolapu=j_0;
		if(allocated(jlp_icolapu))deallocate(jlp_icolapu);allocate(jlp_icolapu(1:jlp_nrow));jlp_icolapu=j_0
	...
	endif !if(sparse)   3966
 
	allocate(jlp_a(1:jlp_nrow,0:mxn) ) ;jlp_a=j_0         ! A matrix
	if(allocated(jlp_objr0))deallocate(jlp_objr0)
	if(allocated(jlp_objr2))deallocate(jlp_objr2)
	allocate( jlp_objr0(1:mxnm));allocate( jlp_objr2(1:mxnm));;jlp_objr0=j_0;jlp_objr2=j_0
	...
	if(nz.gt.0)then  !nz=number of z-variables
		...
		if(.not.zmatrix) then
			...
			do i=1,nset  !number of domainset
				...
				do k=1,jlp_nsetr(i) !number of rows in set i
					irow0=irow0+1 	!if(j.eq.1) irow0=irow0+1   !row accoding to intial (nonexpanded) row numbering
					if(irow0.ne.irowobj)then !.or.j.gt.1)then ! k.gt.1)then
						!constraint row
						irow=irow+1
						do ii=1,jlp_nvars(irow0)
							...
							endif !if(.not.jlp_isxval(ival))   4070
						enddo !ii=1,jlp_nvars(irow0)   4068
					else if(irow0.eq.irowobj)then !.and.j.eq.1)then !if(irow0.ne.irowobj.or.j.gt.1)then
						...
						do ii=1,jlp_nvars(irow0)
							...
							if(.not.jlp_isxval(ival))then
								...
								endif !if(jlp_maxo)   4085
							endif !if(.not.jlp_isxval(ival))   4083
						enddo !ii=1,jlp_nvars(irow0)   4081
					endif !if(irow0.ne.irowobj)   4065
				enddo !k=1,jlp_nsetr(i)   4063
			...
			do i=1,jlp_nrow
				...
				enddo !j=1,nz   4102
			enddo !i=1,jlp_nrow   4101
		endif !if(.not.zmatrix)   4034
		...
		if(sparse)then  !sivuutetaan sparse toistaiseksi
			do i=1,nz
				...
				do j=1,jlp_nrow
					...
					endif !if(jlp_a(j,i).ne.jlp_zero)   4120
				enddo !j=1,jlp_nrow   4119
				last(i)=jlp_lavecsp(istart1)+ie
			enddo !i=1,nz   4116
		endif !if(sparse)   4115
	...
	if(jlp_xpresent)then
		...
		if(jlp_fpresent) then  !!!!
			if(.not.jlp_maxo.and.jlp_nfyrow(1)>0) then
				...
				j_err = .true.
				return
			endif !if(.not.jlp_maxo.and.jlp_nfyrow(1)>0)   4155
			...
			allocate(jlp_coeffx(1:nfxval));jlp_coeffx=j_0 !nfxval=count(isfxval)
			...
			enddo !j=2,irowp   4180
 
			jlp_ibafy(1)=0
			do j=2,irowp
				jlp_ibafy(j)=jlp_ibafy(j-1)+jlp_nfyrow(j-1)
			enddo !j=2,irowp   4185
			...
			endif !if(count(jlp_isfyval)>0)   4196
			...
			enddo !i_ = 1, count(jlp_isfyval)   4204
				...
				do inf_=1, jlp_nxkfact(ixkyk_) !xkyk-mjiin liittyvät tehtaat
					...
					do irowj_ = 1,irowp ! tehtävärivit (alkup. ei lavennetut)
						do k=1,jlp_nfxrow(irowj_) ! rivin xk-muuttujat
							...
							endif !if((ixkyk_.eq.jxk).and.(if_.eq.jf))   4230
						enddo !k=1,jlp_nfxrow(irowj_)   4227
							...
							do ixk_=1,j_o(listy)%i(1) !xk-muuttujat
								...
								do iif_=1,j_o(listf)%i(1) !tehtaat
									...
									if ((ixkyk_.eq.jxk).and.(if_.eq.jf)) then
										...
										irowvars_ = irowvars_ + 1
									endif !if ((ixkyk_.eq.jxk).and.(if_.eq.jf))   4248
								enddo !iif_=1,j_o(listf)%i(1)   4245
							enddo !ixk_=1,j_o(listy)%i(1)   4240
						enddo !k=1,jlp_nfyrow(irowj_)   4237
					enddo !irowj_ = 1,irowp   4226
					jlp_xkfact(ixkyk_,inf_)%inxkykrowvar = irowvars_-1
				enddo !inf_=1, jlp_nxkfact(ixkyk_)   4223
			enddo !ixkyk_=1,j_o(jlp_ivxkyk)%i(1)   4222
			...
			do irowj_ = 2,irowp
				...
				endif !if(jlp_nfxrow(irowj_).eq.1.and. jlp_nfyrow(irowj_).eq.0.an   4266
			enddo !irowj_ = 2,irowp   4263
			if(j_err)return
 
		endif !if(jlp_fpresent)   4154
			...
			do i=1,nvartot
				...
				if(.not.jlp_isx(i).and..not.fvari)then
					...
					if(nnz.ge.10.and.nnz.lt.nz)then
						write(6,*)' ...etc...'
						exit
					endif !if(nnz.ge.10.and.nnz.lt.nz)   4308
				endif !if(.not.jlp_isx(i).and..not.fvari)   4305
			enddo !i=1,nvartot   4298
			j_err=.true.
			return
		endif !if(nz.gt.0.and. .not.zopt)   4294
		...
		if(nfx.gt.0.or.nfy>0) then
			...
			if(allocated(jlp_keyfact))then
				if(warmf)then
					if(size(jlp_keyfact,dim=1).ne.jlp_nunits.or. &
						...
						warmf=.false.
					endif !if(size(jlp_keyfact,dim=1).ne.jlp_nunits.o   4331
				endif !if(warmf)   4330
				if(.not.warmf)deallocate(jlp_keyfact)
			endif !if(allocated(jlp_keyfact))   4329
					...
					do j=1,j_o(jlp_ivxkyk)%i(1)
						do keyf_=1,jlp_nxkfact(j)
							if(jlp_xkfact(j,keyf_)%ifact.eq.jlp_keyfact(i,j))then
								...
								goto 171
							endif !if(jlp_xkfact(j,keyf_)%ifact.eq.jlp_keyfact(i,j))   4346
						enddo !keyf_=1,jlp_nxkfact(j)   4345
						warmf=.false.
						write(6,*)'*factories do not agree warm-> ignored for factories'
						goto 172
						171			continue
					enddo !j=1,j_o(jlp_ivxkyk)%i(1)   4344
				enddo !i=1,jlp_nunits   4343
			endif !if(warmf)   4342
			...
			172			if(.not.warmf)then
				...
				enddo !j=1,j_o(jlp_ivxkyk)%i(1)   4370
				...
				do i=2,jlp_nunits
					do j=1,j_o(jlp_ivxkyk)%i(1)
						if(jlp_keyfact(i-1,j).lt.jlp_nxkfact(j)) then
							...
							keyf_ = 1
						endif !if(jlp_keyfact(i-1,j).lt.jlp_nxkfact(j))   4386
						init_=keyf_
					...
					enddo !j=1,j_o(jlp_ivxkyk)%i(1)   4385
				enddo !i=2,jlp_nunits   4384
				...
				! !		j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_),j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_)  !xdatiba
				! !j_valueopt_af(j_rowykfkey(j_)%jcurix) = j_valueopt_af(j_rowykfkey(j_)%jcurix) + &
				! write(6,*)'j_',j_,	j_v(j_rowykfkey(j_)%ivfout)  !*&
				! !	(j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_) -&
				! !	j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_))
				...
				do i=1,jlp_nunits
					...
					enddo !j=1,j_o(jlp_ivxkyk)%i(1)   4473
				enddo !i=1,jlp_nunits   4472
			endif !172			if(.not.warmf)   4369
 
		endif !if(nfx.gt.0.or.nfy>0)   4327
		...
		!		write(6,*)'**********ivxdatmat,j_ivmatx',j_ivmatx,ivxdatmat,j_lopp
		...
		if(.not.isunit)then
			...
			if(iiv.le.0)then
				...
				j_err=.true.
				goto 940
			endif !if(iiv.le.0)   4495
		endif !if(.not.isunit)   4492
		...
		do i=1,nset ! number of sets
			...
			do k=1,jlp_nsetr(i) ! number of rows in set i (before expanding over domains)
				...
				195  continue
					...
					jlp_ixprow(irow0)=0
				endif !if(jlp_nxrow(irow0).gt.0)   4563
				...
				if(irow0.ne.irowobj)then ! .or.k.gt.1)then
					...
					elseif(jlp_isetd(idomba+j).ne.ialldomain)then !if(j_nxrow(irow0).gt.0)then
						...
						j_err=.true.
						return
					endif !if(jlp_nxrow(irow0).gt.0)   4596
				elseif(irow0.eq.irowobj.and.k.eq.1)then !if(irow0.ne.irowobj)then
					...
					elseif(jlp_fpresent) then !if(j_nxrow(irow0).gt.0)then
						if(jlp_nfxrow(irow0)>0.or.jlp_nfyrow(irow0)>0) &
							jlp_irowdomain(0)=jlp_isetd(idomba+j)
					endif !if(jlp_nxrow(irow0).gt.0)   4612
					kobj=k
				endif !if(irow0.ne.irowobj)   4592
			enddo !k=1,jlp_nsetr(i)   4561
		...
		if(jlp_fpresent.and..false.) then
			do irowj_=1,jlp_nrow
				...
				if (jlp_nfxrow(irowrowj_)==1) then
					if(j_o(ivrhs)%d(irowrowj_)==j_ninf) then
						jlp_rhs(irowj_)= j_0
					...
					endif !if(j_o(ivrhs)%d(irowrowj_)==j_ninf)   4647
				endif !if (jlp_nfxrow(irowrowj_)==1)   4646
			enddo !irowj_=1,jlp_nrow   4644
		endif !if(jlp_fpresent.and..false.)   4643
		...
		if(jlp_ndom.gt.0)then  ! ndom is the number of domains
			...
			! Note: ivdomains is not good name because it looks like the index of a J object
			!
			!		write(6,*)'<4774',j_o(jlp_ivprob)%i(15)
			!		ivdomtrans=j_o(jlp_ivprob)%i(15)
			! do i=1,jlp_ndom
			...
			! call j_deftrans(0,'$domtrans$',ivdomtrans,30*jlp_ndom,0,0,iii,iii,iii,0)
			...
			! enddo !do i=1,j_ndom
		endif !if(jlp_ndom.gt.0)   4660
		! generate domains, xvars
		if(jlp_ndom.gt.0)then
			jlp_ndomv=ceiling(real(jlp_ndomainvars)/32.)
			!write(6,*)'<55jlp_ndomv ',jlp_ndomv
			...
			allocate(jlp_domainbits(1:jlp_ndomv*jlp_nunits));jlp_domainbits=0
		...
		endif !if(jlp_ntemp0.gt.j_maxjlpxvars)   4764
		!	write(6,*)'<87777'
		!tree
		if(istree)then
			...
			do i_=0,jlp_nrow
				if(jlp_ix(i_).ne.0)then
					...
					endif !if(i_.eq.0)   4789
				endif !if(jlp_ix(i_).ne.0)   4787
			enddo !i_=0,jlp_nrow   4786
		...
		allocate(jlp_rejects(1:jlp_lopp));jlp_rejects=.false.
		...
		if(allocated(jlp_keys))then
			if(warm)then
				...
				endif !if(size(jlp_keys).ne.jlp_nunits)   4818
			endif !if(warm)   4817
			if(.not.warm)deallocate(jlp_keys)
		endif !if(allocated(jlp_keys))   4816
		if(.not.warm)then
			allocate(jlp_keys(1:jlp_nunits));jlp_keys=0
		endif !if(.not.warm)   4828
		...
		if(ivarea.gt.0)then   ! area-> option present
			...
			endif !if(notareavars.gt.0)   4855
			if(ncvar.gt.0)then
				allocate(jlp_cvar(1:ncvar))
				needc=1 !make logical
			endif !if(ncvar.gt.0)   4877
 
		endif !if(ivarea.gt.0)   4853
		...
		if(isunit)then
			...
			jlp_nunits=1
			old=j_o(ivxdatmat)%d(inde)
			ibas=keepx
			iprev=1
			maxnsch=1
			!	write(6,*)'indet',inde,old
			do i=2,j_o(ivxdatmat)%i(1)
				!	if(i.lt.200)write(6,*)i,j_o(ivxdatmat)%d(ibas+inde),old,ibas
				if(j_o(ivxdatmat)%d(ibas+inde).ne.old)then
					jlp_nsch(jlp_nunits)=i-iprev
					maxnsch=max(maxnch,jlp_nsch(jlp_nunits))
					jlp_nunits=jlp_nunits+1
					old=j_o(ivxdatmat)%d(ibas+inde)
 
					iprev=i
				endif !if(j_o(ivxdatmat)%d(ibas+inde).ne.old)   4903
				ibas=ibas+keepx
			enddo !i=2,j_o(ivxdatmat)%i(1)   4901
			jlp_nsch(jlp_nunits)=j_o(ivxdatmat)%i(1)-iprev
			maxnsch=max(maxnch,jlp_nsch(jlp_nunits))
			!	write(6,*)'shed',jlp_nsch(1:jlp_nunits)
 
		endif !if(isunit)   4892
		ibasc=0
		...
		ibas=0
		!	do iik=1,keepx
		!write(6,*)'477474',iik,j_o(ivxdatmat)%d(iik),j_o(ivxdatmat)%d(keepx+iik)
		!	enddo
		idomba=0
		do iunit=1,jlp_nunits
			...
			if(ivunit.gt.0)then
				...
				j_v(j_o(jlp_ivkeepx)%i2(1:keepx))=j_o(ivxdatmat)%d(ibas+1:ibas+keepx)
				ibas=ibas+keepx
			else
			...
			endif !if(jlp_ivtrans.gt.0)   4941
			if(isdomain)then
				!			write(6,*)'<456doing,needc,jlp_ndom',needc,jlp_ndom
				j_v(jlp_ivunit)=iunit
			...
			endif !if(isdomain)   4949
			if(needc.gt.0)jlp_cvar=j_v(jlp_cvarl(1:ncvar))
 
			if(jlp_ndom.ne.0)then  ! ndom is number of domains
				!domainbits(1:jlp_ndomv,iunit)=0 done at allocation
				! käydään läpi domainmuuttujat
 
				ibit=0
				!	write(6,*)'<45jlp_ndomv,idomb,jlp_ndomainvars',jlp_ndomv,idomba,jlp_ndomainvars
				do j=1,jlp_ndomainvars
 
					if(ibit.gt.31)then
						idomba=idomba+1
						ibit=0
					endif !if(ibit.gt.31)   4969
					call j_getname(domainvars(j))
					!		write(6,*)j_oname(1:j_loname),j_v(domainvars(j)),ibit,idomba
					if(j_v(domainvars(j)).ne.j_0)then
						!	write(6,*)'<45set',j
						jlp_domainbits(idomba)= ibset(jlp_domainbits(idomba),ibit)
						jlp_domainunits(j)=jlp_domainunits(j)+1
					endif !if(j_v(domainvars(j)).ne.j_0)   4975
					ibit=ibit+1
 
				enddo !j=1,jlp_ndomainvars   4967
				...
				! write(6,*)'j,ibit',j,ii,ibit,jlp_ivdomains(j),j_v(jlp_ivdomains(j))
				! iiv=j_o(ivdomainvars)%i2(j)
				! call
				! if(j_v(jlp_ivdomains(j)).ne.j_0 )then
				! write(6,*)'isdom',j,j_v(jlp_ivdomains(j))
				! jlp_domainbits(ii,iunit)= ibset(jlp_domainbits(ii,iunit),ibit)
				! ! domainunits tells the number of units in each domain
				! jlp_domainunits(j)=jlp_domainunits(j)+1
				! endif !if(j_v(j_ivdomains(j)).ne.0. )then
				! enddo !do j=1,j_ndom
			endif !if(jlp_ndom.ne.0)   4961
			! if(iunit.gt.10)then
			...
			do is=1,jlp_nsch(iunit)
				...
				if(.not.isunit.or.is.gt.1)then
					!		write(6,*)jlp_ivkeepx,j_o(jlp_ivkeepx)%i2,keepx,ivxdatmat,ibas
					j_v(j_o(jlp_ivkeepx)%i2(1:keepx))=j_o(ivxdatmat)%d(ibas+1:ibas+keepx)
					ibas=ibas+keepx
 
				endif !if(.not.isunit.or.is.gt.1)   5013
				...
				endif !if(jlp_ivsubtrans.gt.0)   5022
				!	test duplicated schdeulse
				if(inde.eq.1)then
					xvars1=j_v(jlp_xvarl)
					if(is.gt.1)then
						if(all(xvars1.eq.xvars2))then
						...
						endif !if(all(xvars1.eq.xvars2))   5034
					endif !if(is.gt.1)   5033
					inde=2
				else
					xvars2=j_v(jlp_xvarl)
					if(all(xvars1.eq.xvars2))then
						jlp_rejects(nstot)=.true.
						nrej=nrej+1
						nrejtot=nrejtot+1
					...
					endif !if(all(xvars1.eq.xvars2))   5046
					inde=1
				endif !if(inde.eq.1)   5031
				if(subfilter_)then         !subfilter->
					...
					if(testcode.eq.0.)then   ! filter->False, reject
						...
						goto 776
					endif !if(testcode.eq.0.)   5064
				endif !if(subfilter_)   5056
				if(subreject_)then            ! reject->
					...
					if(testcode.ne.0.)then    !reject->True
						...
						nrejtot=nrejtot+1
						goto 776
					endif !if(testcode.ne.0.)   5076
				endif !if(subreject_)   5071
			776     continue
			!		write(6,*)'<8767'
		!	if(j_xmatinmemory)then
				ibxmatx=(nstot-1)*jlp_ntemp0
 
				if(nxvararea.gt.0)then
					! xvarlarea is a vector
					j_v(jlp_xvarlarea)=j_v(ivarea)*j_v(jlp_xvarlarea)   ! multiply area-variables by area
				endif !if(nxvararea.gt.0)   5088
				do i=1,jlp_ntemp0
					iba=jlp_ibatemp(i)
					!	if(iunit.lt.10)write(6,*)iba,jlp_ntemp0,iba,ibxmatx,jlp_nxrowtemp(i),iba+jlp_nxrowtemp(i)
 
					jlp_xmat(i+ibxmatx)= &       ! make temporary variables
						dot_product(jlp_coefx(iba+1:iba+jlp_nxrowtemp(i)), &
						j_v(jlp_irowxvars(iba+1:iba+jlp_nxrowtemp(i))))
					! if(iunit.le.5)then
					! write(6,*)'j_nxrowtemp(i)',jlp_nxrowtemp(i),jlp_xmat(i+ibxmatx),&
					! jlp_coefx(iba+1:iba+jlp_nxrowtemp(i)),j_v(jlp_irowxvars(iba+1:iba+jlp_nxrowtemp(i)))
				...
				enddo !i=1,jlp_ntemp0   5092
				!if(iunit.gt.1)stop 'per'
				!j_ix(0),j_ntemp0
				if(.not.jlp_maxo.and.jlp_ix(0).ne.0) &
					jlp_xmat(jlp_ix(0)+ibxmatx)=-jlp_xmat(jlp_ix(0)+ibxmatx)
 
 
				! if(istree)then
				! if(is.eq.1)then
				! do i=1,jlp_ntemp0
				! previous(i)=jlp_xmat(ibxmatx+iperiodperm(i))!previous(i)=j_xmat(iperiodperm(i),nstot)
				...
				! if(jlp_xmat(iperiodperm(i)+ibxmatx).ne.previous(i))then
				! idiff(nstot)=i
				! do i2_=i,jlp_ntemp0
				! previous(i2_)=jlp_xmat(iperiodperm(i2_)+ibxmatx)
				! !	previous(i2_)=j_xmat(iperiodperm(i2_),nstot)
				! enddo !do i2_=i,j_ntemp0
				! itreesize=itreesize+jlp_ntemp0-i+1
				! exit
				! endif !if(j_xmat(iperiodperm(i)+ibxmatx).ne.previous(i))then
				! enddo !do i=1,j_ntemp0
			...
			enddo !is=1,jlp_nsch(iunit)   5008
			if(subfilre)then
				if(nrej.ge.jlp_nsch(iunit))then
					iwar=iwar+1
					if(iwar.le.10)write(6,*)'*err all schedules were rejected for unit ',iunit
				endif !if(nrej.ge.jlp_nsch(iunit))   5143
			endif !if(subfilre)   5142
			if(j_err)return
			!		write(6,*)'nuni ',jlp_nunits,iunit,ibxmatx
		enddo !iunit=1,jlp_nunits   4927
		!	write(6,*)'<56domainbits',jlp_domainbits
		...
		if(.not.allocated(svalue))then
			allocate(svalue(1:maxnsch),basreject(1:maxnsch))
 
		elseif(size(svalue).lt.maxnsch)then
			deallocate(svalue,basreject)
			allocate(svalue(1:maxnsch),basreject(1:maxnsch))
		endif !if(.not.allocated(svalue))   5174
		...
		if(ivunit.le.0.and.nstot.ne.jlp_lopp)then
			...
			j_err=.true.
			return
		endif !if(ivunit.le.0.and.nstot.ne.jlp_lopp)   5194
		call cpu_time(time00)
		if(jlp_xpresent)then
			write(*,*)'found ',jlp_nunits,' units, ',jlp_lopp,' schedules, filling xmat'
 
			write(6,*)'from which ',nrejtot,' schedules were rejected'
			write(6,*)'from which ',nrejdupl,' were duplicated schedules'
			deallocate (xvars1,xvars2)
			write(6,*)'found ',jlp_ndom,' domains'
		endif !if(jlp_xpresent)   5200
		! empty domains?
		nempt=0
		if(jlp_ndom.gt.0)write(6,*)'domain index,number of units, definition'
		do i=1,min(jlp_ndom,20)
			call j_gettext(jlp_ivdomain,i)
			write(6,*)i,jlp_domainunits(i),j_gottext(1:j_lgottext)
			...
			if(jlp_domainunits(i).le.0) nempt=nempt+1
			! else
 
 
			if(i.eq.20.and.jlp_ndom.gt.20)write(6,*)'etc ..'
		enddo !i=1,min(jlp_ndom,20)   5211
		if(nempt.gt.0)then
			j_err=.true.;write(6,*)'**empty domains:',nempt; return
		end if !if(nempt.gt.0)   5227
		!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 11
		!tehdas xk-mjien ei-negatiivisuus-tarkistus !!!!
		if(jlp_fpresent) then
			ibxdatobs=0
			!	write(6,*)'<6662 ',ivxdatmat,j_o(ivxdatmat)%i(1),j_o(jlp_ivxkyk)%i(1),jlp_ivkeepx
			do iobs=1,j_o(ivxdatmat)%i(1)
				...
				! call j_getname(ivxk)
				! write(6,*)iobs,ivxk,kk,j_oname(1:j_loname),j_o(ivxdatmat)%d(ibxdatobs+k_)
				! enddo
				...
				do ixk_ = 1,j_o(jlp_ivxkyk)%i(1) !tehdas xk-mjat
					...
					k_ = j_inlistobject(ivxk_,jlp_ivkeepx)
					!	iba_=0
					!		do while(iba_ < (j_o(ivxdatmat)%i(1)*j_o(ivxdatmat)%i(2)))
					if (j_o(ivxdatmat)%d(ibxdatobs+k_) < 0) then   !xmatiba
						! datassa negat. tehdas-xk-mja
						call j_getname(ivxk_)
						write(6,*)'**Negative fact xk-variable ',j_oname(1:j_loname), ' in observation ',iobs ,j_o(ivxdatmat)%d(ibxdatobs+k_)
						!in data: ixkyk,ivxk,iba,k, xk',&
						!	ixk_,ivxk_,ibxdatobs,k_,j_o(ivxdatmat)%d(ibxdatobs+k_) !xmatiba
						j_err=.true.
						return
					endif !if (j_o(ivxdatmat)%d(ibxdatobs+k_) < 0)   5254
					!		iba_=iba_+j_o(ivxdatmat)%i(2)
					!		enddo !do while(iba_ < (j_o(ivxdatmat)%i(1)*j_o(ivxdatmat)%i(2)))
				enddo !ixk_ = 1,j_o(jlp_ivxkyk)%i(1)   5249
				ibxdatobs=ibxdatobs+keepx  !jlp_ntemp0
			enddo !iobs=1,j_o(ivxdatmat)%i(1)   5235
			!endif  !xdisk
		endif !if(jlp_fpresent)   5232
 
 
		! calculate sums over key schedules !!!!
		!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 179
		jlp_xps=j_0
			...
			if(subfilre)then
				if(jlp_rejects(jlp_ibaunit(i)+1))then
					...
					do jj=1,jlp_nrowp ;j=jlp_ixcurrows(jj)
						...
						jlp_xmin(j)=j_inf;jlp_xmax(j)=j_ninf
					enddo !jj=1,jlp_nrowp ;j=jlp_ixcurrows(jj)   5297
					goto 745
				endif !if(jlp_rejects(jlp_ibaunit(i)+1))   5292
			endif !if(subfilre)   5291
 
			! get jlp_xmin and jlp_xmax from the first schedule
			ibxmatx=xmatiba(jlp_ibaunit(i)+1) !,1)
			do jj=1,jlp_nrowp
				j=jlp_ixcurrows(jj)
				jlp_nunitsrow(j)=jlp_nunitsrow(j)+1
				jlp_xmin(j)=jlp_xmat(jlp_ix(j)+ibxmatx) ! v(ix(j))
				jlp_xmax(j)=jlp_xmat(jlp_ix(j)+ibxmatx)  ! v(ix(j))
			enddo !jj=1,jlp_nrowp   5307
			...
			do k=2,jlp_nsch(i)
				if(subfilre)then
					if(jlp_rejects(jlp_ibaunit(i)+k))then
						nrej=nrej+1
						cycle  !do k=2,nsch(i)
					endif !if(jlp_rejects(jlp_ibaunit(i)+k))   5317
				endif !if(subfilre)   5316
				!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 149
				! compute for rows containing x-variables the min and max values
				ibxmatx=xmatiba(jlp_ibaunit(i)+k) !,1)
				do jj=1,jlp_nrowp
					j=jlp_ixcurrows(jj)
					curx=jlp_xmat(jlp_ix(j)+ibxmatx)
					if(curx.lt.jlp_xmin(j))then
						jlp_xmin(j)=curx
					endif !if(curx.lt.jlp_xmin(j))   5328
					if(curx.gt.jlp_xmax(j))then
						jlp_xmax(j)=curx
						keyopt=k   ! for the case where there are no constraints
					endif !if(curx.gt.jlp_xmax(j))   5331
				enddo !jj=1,jlp_nrowp   5325
 
			enddo !k=2,jlp_nsch(i)   5315
 
			do jj=1,jlp_nrowp
				j=jlp_ixcurrows(jj)
				jlp_xsmin(j)=jlp_xsmin(j)+jlp_xmin(j)
				jlp_xsmax(j)=jlp_xsmax(j)+jlp_xmax(j)
			enddo !jj=1,jlp_nrowp   5339
...
4689    key=key+1
				...
				endif !if(subfilre)   5350
 
			endif !if(jlp_nrow.eq.0)   5345
			...
			if(warm.and.jlp_nrow.gt.0)then
				...
				if(jlp_keys(i).gt.jlp_nsch(i).or.rejectnow_)then
					...
					key=jlp_keys(i)
				endif !if(jlp_keys(i).gt.jlp_nsch(i).or.rejectnow_)   5359
			endif !if(warm.and.jlp_nrow.gt.0)   5356
			jlp_keys(i)=key
			ibxmatx=xmatiba(jlp_ibaunit(i)+key) !,1)
			do jj=1,jlp_nrowp
				j=jlp_ixcurrows(jj)
				!xps = sums over the keyschedules vector s in Eq. (6.14) in JLP-manual
				jlp_xps(j)=jlp_xps(j)+jlp_xmat(jlp_ix(j)+ibxmatx) !v(ix(j)) !(ix(j)=0 -> no x in row
				!		if(j.eq.0.and.i.ge.6.and.i.le.10)write(n16,*)'<555 ',i,ibxmatx,jlp_ix(j),jlp_xmat(jlp_ix(j)+ibxmatx),jlp_xps(j)
			enddo !jj=1,jlp_nrowp   5369
			...
			if(jlp_fpresent) then
				...
				call jlpfcurix(i)
				call jlpfcurixy(i)
				!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 33
				!write(16,*)j_xdattokaobs,j_ibaunit(i)+key,'j_xdatlast',j_xdatlast
 
				!	ibxdatkey=xmatiba(jlp_ibaunit(i)+key) !,2)
				ibxdatkey=xdatiba(jlp_ibaunit(i)+key) !,2)
 
				!if(i.le.2)write(16,*)'ibxdatkey i',ibxdatkey,i
 
				!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)then
				!write(16,*)ibxdatkey,ibxdatkey/keepx,j_ibaunit(i)+key
				!write(16,*)'j_xdatlast',j_xdatlast,j_xdatfirst2,j_xdatlast2
				!stop 34
				!endif
				!	write(19,*)ivxdatmat,j_ibaunit(i),key,ibxdatkey,j_lopp
				...
				do j=0, jlp_nrow
					if (jlp_ix(j)==0) then
						if (jlp_ixcurfact(j)) then !ehdosta puuttuu rivit, joilla y-tehdasmjia
							jlp_nunitsrow(j) = jlp_nunitsrow(j) + 1
						endif !if (jlp_ixcurfact(j))   5400
					endif !if (jlp_ix(j)==0)   5399
				enddo !j=0, jlp_nrow   5398
				...
				!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 49
				!xk-tehdasmuuttujat
					...
					do k=1,jlp_nfxrow(irowj)
						...
						if(jlp_keyfact(i,jlp_irowfxvars(jlp_ibafx(irowj)+k)).eq. &
							...
							jlp_xps(j)=jlp_xps(j)+ jlp_coeffx(jlp_ibafx(irowj)+k)*&
								j_o(ivxdatmat)%d(ibxdatkey+jlp_irowfkeep(jlp_ibafx(irowj)+k)) !xmatiba
 
							if(j.eq.0.and.i.ge.6.and.i.le.10)write(n16,*)'<5566',irowj,k,jlp_ibafx(irowj), &
								jlp_coeffx(jlp_ibafx(irowj)+k),ibxdatkey,jlp_irowfkeep(jlp_ibafx(irowj)+k),&
								j_o(ivxdatmat)%d(ibxdatkey+jlp_irowfkeep(jlp_ibafx(irowj)+k)),jlp_xps(j)
						endif !if(jlp_keyfact(i,jlp_irowfxvars(jlp_ibafx(irowj)+k)).e   5435
					enddo !k=1,jlp_nfxrow(irowj)   5430
				enddo !jj=1,jlp_nrowpf   5425
 
				if (nfy.gt.0) then
					do j=1,j_o(jlp_ivkeepc)%i(1)
						j_v(j_o(jlp_ivkeepc)%i2(j))=j_o(ivcmat)%d((i-1)*j_o(jlp_ivkeepc)%i(1)+j)
					enddo !j=1,j_o(jlp_ivkeepc)%i(1)   5451
					do j=1,ntrans
					...
					enddo !j=1,ntrans   5454
				endif !if (nfy.gt.0)   5450
				!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 314
				do jj=1,jlp_nrowpfy
						...
						do ivars_=1,j_o(listy)%i(1)
							...
							do ifact_=1,j_o(listf)%i(1)
								...
								if(jlp_keyfact(i,iv2xykypos_).eq.iv3factpos_) then
									!if(p)then
									!endif !if(p)then
									!			write(n16,*)'<666,',i,iv2xykypos_,iv3factpos_
									!				write(n16,*)'<777',jlp_fyfactout(iv2xykypos_,iv3factpos_)
 
									jlp_xps(j)=jlp_xps(j)+ j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
										j_o(ivxdatmat)%d(ibxdatkey+iv2elpos)
									call j_getname(jlp_fyfactout(iv2xykypos_,iv3factpos_))
 
									!			if(j.eq.0.and.i.ge.6.and.i.le.10)write(n16,*)'##& ',i, ibxdatkey,iv2xykypos_,iv3factpos_,&
									!		jlp_fyfactout(iv2xykypos_,iv3factpos_),&
									!			iv2elpos,j_o(ivxdatmat)%d(ibxdatkey+iv2elpos),&
									!			j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_)),jlp_xps(j),j_oname(1:j_loname)
								...
								endif !if(jlp_keyfact(i,iv2xykypos_).eq.iv3factpos_)   5496
 
							enddo !ifact_=1,j_o(listf)%i(1)   5485
						enddo !ivars_=1,j_o(listy)%i(1)   5477
 
					enddo !k=1,jlp_nfyrow(irowj)   5468
				enddo !jj=1,jlp_nrowpfy   5463
 
				if(p) then
					write(n16,*)'**fact** xps <38xx>',i,jlp_xps(0:10)
				endif !if(p)   5529
 
			endif !if(jlp_fpresent)   5378
		enddo !i=1,jlp_nunits   5282
		iprint=2
		!	p=.true.
 
		if(ivprint7.gt.0)then
			if(j_v(ivprint7).eq.6.)call printproblem()
		endif !if(ivprint7.gt.0)   5538
		!	write(n16,*)'xps bef preopt',i,(jlp_xps(jj7),jj7=0,min(jlp_nrow,10)) !!!!
		if(p)write(n16,*)'nrowp',jlp_nrowp
		if(iprint.gt.1)write(6,*)'***row, min,max,initial value, tolerance'
		!	write(6,*)'<loub',jlp_lbou
		!write(6,*)'<u',jlp_ubou
		!	write(6,*)'lower',jlp_lower
		!	write(6,*)'<5454',jlp_x(2)
		! do i=j_lopp-15000,j_lopp
		! iba=xmatiba(i,1)
		! write(17,*)j_o(ivxdatmat)%r(iba+1:iba+7),j_o(ivxdatmat)%r(iba+11:iba+17)
		! iba=xmatiba(i,1)
		...
		if(iprint.gt.0)write(6,*)'min and max sums of x-variable rows, sum in key-schedules'
		...
		enddo !jj=1,jlp_nrowp   5556
 
		!toleranssi tehdasmja-riveille  !!!!
		write(n16,*)'<556',p,jlp_nrow,jlp_fpresent
		if (jlp_fpresent) then
			do j=0, jlp_nrow
				!	write(27,*)'***fact j,nunits,xps,tole', j,jlp_nunitsrow(j),jlp_xps(j),jlp_tole(j)
				! if (jlp_ix(j)==0) then
			...
			enddo !j=0, jlp_nrow   5569
		endif !if (jlp_fpresent)   5568
		...
		if(jlp_nrow.ne.0.and.jlp_xpresent.and..not.jlp_fpresent.and.nz.eq.0)then
			...
			do kief=1,1
				...
				do i=1,jlp_nunits
					...
					do k=1,jlp_nsch(i)
						...
						endif !if(subfilre)   5591
						jlp_value=jlp_zero  !value is the sum of infeasibility
						if(jlp_ixcurrows(1).eq.0)then
							j1=2
						else !if(j_ixcurrows(1).eq.0)then
							j1=1
						endif !if(jlp_ixcurrows(1).eq.0)   5595
						ibxmatx=xmatiba(jlp_ibaunit(i)+k) !,1)
						ibxmatx2=xmatiba(jlp_ibaunit(i)+jlp_keys(i)) !,2)
						do jj=j1,jlp_nrowp
							j=jlp_ixcurrows(jj)
							curx=jlp_xmat(jlp_ix(j)+ibxmatx)
							curk=jlp_xmat(jlp_ix(j)+ibxmatx2)
							...
							endif !if(jlp_xps(j)+curx-curk.gt.jlp_rhs2(j))   5606
 
						enddo !jj=j1,jlp_nrowp   5602
						if(jlp_value.lt.jlp_valueopt)then
							jlp_valueopt=jlp_value
							kopt=k
 
						endif !if(jlp_value.lt.jlp_valueopt)   5613
						if(k.eq.jlp_keys(i))jlp_valuek=jlp_value
					enddo !k=1,jlp_nsch(i)   5590
					if(jlp_valuek.gt.jlp_valueopt)then
						ibxmatx=xmatiba(jlp_ibaunit(i)+kopt) !,1)
						...
						do jj=1,jlp_nrowp
							...
							jlp_xps(j)=jlp_xps(j)+curx-jlp_xmat(jlp_ix(j)+ibxmatx2)
 
							!	if(j.eq.2)write(27,*)'<47747',curx,ibxmatx2,jlp_ix(j)+ibxmatx2,&
							!		jlp_xmat(jlp_ix(j)+ibxmatx2),jlp_xps(j)
 
							! if(i.le.2.and.j.eq.0)write(6,*)'<47747',curx,ibxmatx2,jlp_ix(j)+ibxmatx2,&
							! jlp_xmat(jlp_ix(j)+ibxmatx2),jlp_xps(j)
 
						enddo !jj=1,jlp_nrowp   5624
						jlp_keys(i)=kopt
						apusum=apusum+jlp_valuek-jlp_valueopt
 
					endif !if(jlp_valuek.gt.jlp_valueopt)   5620
				enddo !i=1,jlp_nunits   5586
			...
			if(iprint.ge.0)then
				...
				do jj=1,jlp_nrowp   !nrowp = number of rows with x-variables
					j=jlp_ixcurrows(jj) !j= number of the row
					write(6,*)j,jlp_xps(j)
					!	if(j.eq.2)write(27,*)'<init',jlp_xps(j)
				enddo !jj=1,jlp_nrowp   5649
			endif !if(iprint.ge.0)   5647
			jlp_objf=jlp_small
		endif !if(jlp_nrow.ne.0.and.jlp_xpresent.and..not.jlp_fpresent.an   5582
	...
	if(jlp_xpresent)then
		...
		if(jlp_ixcurrows(1).eq.0)then
			...
			j1=1   ! there is no  "
		endif !if(jlp_ixcurrows(1).eq.0)   5921
		...
		rowloop:			do jj=j1,jlp_nrowp
			...
			if(jlp_xsmin(j).gt.jlp_rhs2(j).or.jlp_xsmax(j).lt.jlp_rhs(j))then
				...
				endif !if(jlp_xsmin(iunf).gt.jlp_rhs2(iunf))   5950
				...
				endif !if(jlp_xsmax(iunf).lt.jlp_rhs(iunf))   5956
			endif !if(jlp_xsmin(j).gt.jlp_rhs2(j).or.jlp_xsmax(j).lt.jlp_rhs(   5940
		enddo rowloop !loop:			do jj=j1,jlp_nrowp   5928
		if(iunf.ne.0)then
			return   ! pitäis tehdä jotain goto 8000
		endif !if(iunf.ne.0)   5964
...
!write(16,*)'newstart'
	...
	if(jlp_xpresent)then
		...
		endif !if(jlp_fpresent)   6015
	endif !if(jlp_xpresent)   6012
	...
	do i=1,jlp_nrow
		...
		if((jlp_ix(i).ne.0).or.jlp_fpresent)then   ! there is a x-variable in the row
			...
			if(jlp_xps(i).gt.jlp_rhs2(i).or..not.jlp_lbou(i))then
				...
				jlp_lower(i)=.true.
			endif !if(jlp_xps(i).gt.jlp_rhs2(i).or..not.jlp_lbou(i))   6029
			!xps is the sum over key schedules
			jlp_rhsw(i)=jlp_rhscur(i)-jlp_xps(i)  ! 6.27 and 6.28 p. 110 , get working rhs
			!	if(i.eq.2)	write(27,*)'<353553rhs ',jlp_rhscur(i),jlp_xps(i)
			...
			if(jlp_lbou(i))then
				...
				jlp_lower(i)=.false.
			endif !if(jlp_lbou(i))   6041
			jlp_rhsw(i)=jlp_rhscur(i)
		endif !if((jlp_ix(i).ne.0).or.jlp_fpresent)   6027
 
	enddo !i=1,jlp_nrow   6025
	...
	if(jlp_nrow.eq.0.and..not.jlp_fpresent)then   ! no constraints, was optimized directly
		...
		return
	endif !if(jlp_nrow.eq.0.and..not.jlp_fpresent)   6085
	j_v(j_ivstartedjlp)=1.  !!!!
	...
	if (jlp_fpresent) then
		...
		enddo !i=jlp_mxd+1,2*jlp_mxd   6167
		jlp_lf0=jlp_mxd
	...
	if(.not.jlp_xpresent2)then
		nkier=1
		nunits=1
	endif !if(.not.jlp_xpresent2)   6282
	...
	call cpu_time(time0)
	write(6,*)'filling xmat took ',time0-time00,' cpu seconds'
	...
	if(jlp_xpresent2)then
		...
		endif !if(isstop)   6326
	endif !if(jlp_xpresent2)   6321
	...
	write(6,'(i5,i8,g22.12,7x,f8.2,5i6,f6.2,f7.2,i5)')&
		0,0,coefmax*jlp_objf,as,lr0,lz0,jlp_ld0,jlp_lf0,nnf,j_0, &
		j_0
...
! !			write(16,*)'eka',iobs,j_memobs(iobs),kier,iunit,i,j_nsch(iunit),iba+j_ntemp0,size(j_xmat)
	...
	! if(j_xdatfromdisk)then  !only in factory
		...
		! do i=1,j_nsch(iunit)
			! iobs=j_ibaunit(iunit)+i
			! read(j_xdatnu,rec=iobs)j_o(ivxdatmat)%r(iba+1:iba+keepx)
			! iba=iba+keepx
		! enddo
				...
				if (jlp_fpresent) then
					if(jlp_nextxkf) then   !!!!
						...
						if ((nfy.gt.0).and.(iunit/=iunitrans)) then
							...
							enddo !j=1,j_o(jlp_ivkeepc)%i(1)   6503
							do j=1,ntrans
								call dotrans(jlp_itransv(j),1)
								if(j_err)then
									write(6,*)'error for transformation ',j
								endif !if(j_err)   6509
							enddo !j=1,ntrans   6507
							iunitrans=iunit
						endif !if ((nfy.gt.0).and.(iunit/=iunitrans))   6502
						...
						ixkykenter = 0  !!!!
					endif !if(jlp_nextxkf)   6499
				endif !if (jlp_fpresent)   6498
...
401 continue
					...
					if(.not.goto700)then
						if(ipivot9.ne.ipivot)then  !vois katsoa muutkin muutokset
							...
							if(jlp_feasible)then
								...
								endif !if(jlp_xpresent2)   6545
								!time0=time
							endif !if(jlp_feasible)   6544
							...
							endif !if(jlp_xpresent2)   6566
						endif !if(ipivot9.ne.ipivot)   6542
						!write(6,*)'<654exitkier'
						if(jlp_xpresent2)write(6,*)'no changes during last round'
						if(jlp_feasible)write(6,*)'SOLUTION FOUND'
 
						exit kierloop
					endif !if(.not.goto700)   6541
				endif !if((iunitprev.eq.iunitv.and.kier.gt.kierv).or.(iunitv==-1.   6536
				...
				endif !if(.not.jlp_xpresent)   6589
...
! if finding feasible voisi suoraan katsoa, että valuoptin ja valuek:n eroa
				...
				if(iopt.eq.jlp_keys(iunit))then  !!!!
					...
					endif !if (.not.jlp_fpresent)   6613
				endif !if(iopt.eq.jlp_keys(iunit))   6612
				if(jlp_valueopt.le.jlp_valuek+jlp_tolecur) then  !!!!
					...
					endif !if (.not.jlp_fpresent)   6621
				...
				endif !if(jlp_valueopt.le.jlp_valuek+jlp_tolecur)   6620
...
!if(pp)write(16,*)'before leaving,ienter',ienter
			...
			enddo inunitloop !nitloop: do while(.true.)   6451
		...
		enddo unitloop !tloop: do iunit=1,nunits   6403
		if(jlp_fpresent.and.jlp_feasible)then
			if(.not.factnow)then
				...
				if(nofact.gt.iterxkf)then
					...
					factnow=.true.
					nofact=0
				endif !if(nofact.gt.iterxkf)   6698
			else !if(.not.factnow)then
				!	write(6,*)'round,factnow put to false',kier
				!		factnow=.false.
			endif !if(.not.factnow)   6696
		endif !if(jlp_fpresent.and.jlp_feasible)   6695
 
		!write(17,*)'aftunitloop',kier,iunit
	enddo kierloop !rloop: do kier=1,nkier   6375
...
940 continue
	if(.not.zmatrix) then
		...
		nullify(jlp_nsetr,jlp_isetd,jlp_nvars,jlp_irowvars,jlp_coef) !jlp_nsetd,
	endif !if(.not.zmatrix)   6783
...
990  continue
	...
	call  cpu_time(time0)
	write(nureport,*)'total cpu-time in jlp() ',time0-time00
	write(nureport,*)' '
	! write(6,*)'hep'
	! close(16)
	...
	subroutine isfeasible()
		...
		!		write(6,*)'<48848 nnf',nnf,lr0,jlp_x(2),'*',jlp_lr(1:lr0)
		jlp_objr2=jlp_zero
		jlp_value=jlp_zero
		...
		if(.true.)then !.not.j_feasible.or.iunit.eq.1)then !
			do i=1,lr0 !lr0 = number of basic residuals
				...
				!	write(6,*)'<355353 nnf,irow',nnf,irow,jlp_x(irow),jlp_tole(irow),jlp_rhscur(irow),&
				!					jlp_rhs2(irow),jlp_ubou(irow),jlp_lbou(irow),jlp_lower(irow),jlp_rhs(irow)
				...
				if(jlp_lbou(irow)) then
					if(jlp_x(irow)-jlp_tole(irow).gt.jlp_rhscur(irow)-jlp_rhs(irow))then
						...
						nnf=nnf+1
						!				write(6,*)'<35535399 nnf,irow',nnf,irow
						!			stop 'here'
						...
						if(.not.jlp_lower(irow))then
							...
							if(irow.eq.2)write(6,*)'44 xuus ',jlp_x(2)
						...
						endif !if(.not.jlp_lower(irow))   6903
						if(p.and.nnf.le.50)write(n16,*)'infer',irow,jlp_x(irow)
						jlp_value=jlp_value-jlp_x(irow)
						jlp_tolecur=jlp_tolecur+jlp_tole(irow)
					endif !if(jlp_x(irow)-jlp_tole(irow).gt.jlp_rhscur(irow)-jlp_rhs(   6896
				endif !if(jlp_lbou(irow))   6895
			...
			if(nnf.gt.0)then
				...
				if(jlp_feasible)then
					...
					endif !if(nnf.gt.1)   6931
					again=.true.
 
					nagain=nagain+1
				endif !if(jlp_feasible)   6930
				jlp_feasible=.false.
				jlp_objr=>jlp_objr2
					...
					if(again)then
						if(nagain.le.1)then
							...
							if(jlp_objf.gt.againsol+jlp_tolecur)then
								...
								endif !if(nagain.gt.7)   6999
 
							endif !if(jlp_objf.gt.againsol+jlp_tolecur)   6995
 
						endif !if(nagain.le.1)   6992
 
					endif !if(again)   6991
				endif !if(.not.jlp_feasible)   6980
 
				jlp_feasible=.true.
				j_v(ivfeasible)=1.
			endif !if(nnf.gt.0)   6928
			...
			if(justkey)then   !only key schedule has been changed
				...
				endif !if(sparse)   7034
			endif !if(justkey)   7029
 
			if(p)write(n16,*)'vc:',jlp_vc(1:min(jlp_nrow,50))
			if(p)write(n16,*)'objf,objfv,tolecur:',jlp_objf,jlp_objfv,jlp_tolecur
			if(jlp_objf.le.jlp_objfv-jlp_tolecur)then !100j_0*j_tolecur)then  ! 100 added Feb. 2011 cnhaged by JL 9.9.2018
				...
				if(nrecover.ge.20)then !10 changed into 20 by J.L 21.2.2019
					...
					j_err=.true.
					goto900=.true.;return
				endif !if(nrecover.ge.20)   7061
				goto1234=.true.;return
			endif !if(jlp_objf.le.jlp_objfv-jlp_tolecur)   7047
	...
	subroutine updatejlp()
		...
		endif !if(iunit.eq.iunit55)   7084
		...
		if(sparse)then
			...
			! write(61,*)'nrow ',jlp_nrow
			! write(61,*)'a'
			! write(61,'(20f8.2/)')jlp_a
			! write(61,*)'lavec '
			! write(61,'(20i8/)')lavec
			! write(61,*)'rhsw'
			! write(61,'(20f8.2/)') jlp_rhsw
			! write(61,*) 'rx '
			! write(61,'(20f8.2/)')jlp_x
			! write(61,*)' ls '
			! write(61,'(20i8/)')jlp_ls
			! write(61,*)' wslu1 '
			! write(61,'(20f8.2/)')wslu1
			! write(61,*)' lws '
			! write(61,'(20i8/)')lwsll1
		...
		if(jlp_fpresent)then
			...
			enddo !ii_=jlp_mxd+1,jlp_lf0   7126
 
 
		endif !if(jlp_fpresent)   7125
		!end new
		...
		if(p)then
			...
			if(jlp_fpresent) then
				...
				if(jlp_lf0>jlp_mxd) then
					...
					enddo !ii_=jlp_mxd+1,jlp_lf0   7181
				endif !if(jlp_lf0>jlp_mxd)   7178
				...
				if(jlp_x(lcur+jlp_nrowz).lt.jlp_wminwrn)then  ! jl 201606033 j_zeroneg)then    !negative weight, zeroneg=-0.000000001d0
					...
					if(jlp_x(lcur+jlp_nrowz).lt.jlp_wminerr)then  !wminerr=-0.02d0 !!!!
						...
						if(nrecover.ge.10)then
							...
							j_err=.true.
							goto900=.true. ;return
						endif !if(nrecover.ge.10)   7235
 
						write(6,*)'***trying to recover'
						kierv=kier
						iunitv=iunit
						goto1234=.true.;return
					endif !if(jlp_x(lcur+jlp_nrowz).lt.jlp_wminerr)   7227
					...
					do ilr=lr0+1,jlp_nrow
						...
						if(abs(r(lcur+jlp_nrowz)).gt.jlp_vcmax)then !cycling
							if(jlp_lr(ilr).ne.icolold)then
								...
								cycling=.true.
							endif !if(jlp_lr(ilr).ne.icolold)   7282
						endif !if(abs(r(lcur+jlp_nrowz)).gt.jlp_vcmax)   7281
					enddo !ilr=lr0+1,jlp_nrow   7256
					...
					endif !if(jlp_vcmax.gt.jlp_tole(jlp_lr(ilrmax)))   7291
				endif !if(jlp_x(lcur+jlp_nrowz).lt.jlp_wminwrn)   7215
		...
		1578	continue ! weak rocoveryn ohitus
				if(jlp_lunit(nex).ne.jlp_lunit(lcur))then
					if(jlp_wsu.gt.jlp_oneps)then
						...
						if(jlp_wsu.gt.jlp_wmaxerr)then
							if(sparse)then
								...
								if(jlp_wsu.gt.jlp_wmaxwrn)then
									...
									jlp_feasible=.false.
									goto1234=.true.;return
								endif !if(jlp_wsu.gt.jlp_wmaxwrn)   7321
							endif !if(sparse)   7317
						endif !if(jlp_wsu.gt.jlp_wmaxerr)   7316
						...
						do ilr=lr0+1,jlp_nrow
							...
							if(abs(r(lcur0+jlp_nrowz)).gt.jlp_vcmax)then
								if(jlp_lr(ilr).ne.icolold)then
									...
									cycling=.true.
								endif !if(jlp_lr(ilr).ne.icolold)   7354
							endif !if(abs(r(lcur0+jlp_nrowz)).gt.jlp_vcmax)   7353
 
						enddo !ilr=lr0+1,jlp_nrow   7341
						if(jlp_vcmax.gt.jlp_tole(jlp_lr(ilrmax)))then
							newc=jlp_lr(ilrmax)
						...
						endif !if(jlp_vcmax.gt.jlp_tole(jlp_lr(ilrmax)))   7364
 
					endif !if(jlp_wsu.gt.jlp_oneps)   7312
 
					jlp_wsu=jlp_zero
 
					lcur0=jlp_next(lcur)
 
				endif !if(jlp_lunit(nex).ne.jlp_lunit(lcur))   7311
 
				lcur=nex
			enddo !j=1,jlp_ld0   7210
 
		endif !if(jlp_xpresent)   7205
	...
	subroutine repo(nureport)
		...
		endif !if(ivoutresult.ne.j_ivresult)   7455
901	continue
 
		jlp_issolution=.true.
		if(jlp_nrow.gt.0)then  !there are constraints
			do i=jlp_nrow+1,ncol+jlp_nrow
				jlp_x(jlp_ls(i))=j_0
				if(jlp_ls(i).eq.2)write(6,*)'<4774putzero'
			end do !i=jlp_nrow+1,ncol+jlp_nrow   7464
		endif !if(jlp_nrow.gt.0)   7463
		if(.not.jlp_maxo)then  ! poistettu bugia metsästettäessä
			jlp_vc=-jlp_vc                        !;objf=-objf done earlier
			jlp_vx=-jlp_vx
		end if !if(.not.jlp_maxo)   7469
...
877 continue !nrow=0
		...
		if(nz.gt.0) then
			...
			do i=lz0+1,nz
				...
				if(jlp_feasible)then
					...
					val_=0.
				endif !if(jlp_feasible)   7481
				do  j=1,jlp_nrow
					val_=val_-jlp_vc(j)*jlp_a(j,newa)
				enddo ! j=1,jlp_nrow   7486
				jlp_redcost(newa)=abs(val_)
			enddo !i=lz0+1,nz   7478
		endif !if(nz.gt.0)   7476
		if(j_err)return
		...
		if(ivoutresult.ne.j_ivresult) then
			...
			endif !if(jlp_nrow>0)   7505
		endif !if(ivoutresult.ne.j_ivresult)   7494
		...
		do i=1,nset
			...
			if(jlp_xpresent.and.jlp_ndom.gt.0)then
				...
				write(nureport,'(a)')jlp_buf(1:79)
			end if !if(jlp_xpresent.and.jlp_ndom.gt.0)   7536
			write(nureport,*)' ',('_',kk=1,78)
			write(jlp_buf,66061)
...
6606 format(t50,'price', t61,'bound',t70, 'bound')
				...
				if(irow0.ne.irowobj)then
					...
					if(j_v(ivfeasible)>0)then
						if(jlp_maxo)then
							...
							jlp_buf(1:5)=' min'
						end if !if(jlp_maxo)   7567
					else !if(j_v(ivfeasible)>0)then
						jlp_buf(1:5)=' '
						jlp_buf(6:33)='Infeasible, temporary object'
						!	le=
					endif !if(j_v(ivfeasible)>0)   7566
 
				endif !if(irow0.ne.irowobj)   7563
	...
	!	j_buf(34:35)=j_dots !chnaged by JL 10.9.2018
					if(le5.gt.iplus0+80)then
						do jj=iplus0+81,iplus0+1,-1
							if(jlp_buf(jj:jj).eq.'+'.or.jlp_buf(jj:jj).eq.'-')then
								iplus=jj-1
								exit
							endif !if(jlp_buf(jj:jj).eq.'+'.or.jlp_buf(jj:jj).eq.'-')   7588
						enddo !jj=iplus0+81,iplus0+1,-1   7587
					else !if(le5.gt.iplus0+80)then
						iplus=le5
					endif !if(le5.gt.iplus0+80)   7586
				...
				endif !7531		if((le5-iplus0)>=34)   7584
				...
				if(irow0.ne.irowobj)then
					...
					end if !if(jlp_vc(irow).ne.0.)   7616
					...
					end if !if(jlp_rhs(irow).eq.jlp_rhs2(irow))   7621
				...
				end if !if(irow0.ne.irowobj)   7606
				write(nureport,'(a)')jlp_buf(1:79)
				if(intapp)then
					jlp_buf=' '
					...
					write(nureport,'(a)')jlp_buf(1:79)
				endif !if(intapp)   7647
			end do !k=1,jlp_nsetr(i)   7556
			...
			if(jlp_xpresent.and.iprint.ge.1)then
				...
				do jx=1,jlp_nxvartot+ncvar+noutsubtrans
					...
					do iro=0,jlp_nrow
						...
						if((jlp_ndom.le.0.or.idom.eq.jlp_irowdomain(iro)).and.jlp_ix(iro).ne.0)then
							...
							do ii=1,jlp_nxrow2(item)
								...
								endif !if(jx.le.jlp_nxvartot)   7678
								if(jlp_yes)then   !häär
									...
									endif ;exit !if(iro.eq.0)   7687
								endif !if(jlp_yes)   7686
							enddo !ii=1,jlp_nxrow2(item)   7676
 
						endif !if((jlp_ndom.le.0.or.idom.eq.jlp_irowdomain(iro)).and.jlp_   7672
 
					enddo !iro=0,jlp_nrow   7670
					if(jx.le.jlp_nxvartot)then
					...
					elseif(jx.le.jlp_nxvartot+ncvar) then !if(jx.le.j_nxvartot)then
						...
						if(jlp_ndom.le.1) j_v(j_o(ivoutsubtrans)%i2(jx-jlp_nxvartot-ncvar))= &
							jlp_sumx(jx)
					endif !if(jx.le.jlp_nxvartot)   7700
				...
				end do !jx=1,jlp_nxvartot+ncvar+noutsubtrans   7665
			end if !if(jlp_xpresent.and.iprint.ge.1)   7655
			!		end do domloop !domloop: do j=1,j_nsetd(i)
		end do !i=1,nset   7532
		...
		if(jlp_xpresent.and.ivoutresult.ne.j_ivresult) then
			...
			endif !if(jlp_nrow.gt.0)   7765
		endif !if(jlp_xpresent.and.ivoutresult.ne.j_ivresult)   7738
		if(ivoutresult.ne.j_ivresult)then
			...
			if(jlp_nrow.gt.0)then
				...
				do i_=1,jlp_nrow
					if(jlp_rhs(i_).eq.j_ninf)j_o(ivout)%d(i_) =j_ninf
					if(jlp_rhs2(i_).eq.j_inf)j_o(ivout2_)%d(i_) =j_inf
				enddo !i_=1,jlp_nrow   7783
				call j_defmatrix(ivoutresult,'%shprice',jlp_nrow,1,j_matreg,ivout2_)
				j_o(ivout2_)%d = jlp_vc(1:jlp_nrow)
			endif !if(jlp_nrow.gt.0)   7776
 
		endif !if(ivoutresult.ne.j_ivresult)   7772
		if(allocated(xvars1))deallocate(xvars1,xvars2)
		allocate(xvars1(1:nxvar),xvars2(1:nxvar))
...
66778 continue !66778 I=1,NZ   7805
		endif !if(nz.gt.0.and.iprint.ge.1)   7797
...
8000 continue  !end printing
		...
		if(j_v(ivunbounded)>0) then
			...
			write(nureport,'(a)')jlp_buf(1:79)
		endif !if(j_v(ivunbounded)>0)   7820
		...
		endif !if(j_v(ivfeasible)>0)   7828
		...
		endif !if(j_v(ivfeasible)>0)   7836
		write(nureport,'(a)')jlp_buf(1:79)
		!write(6,*)'OBJECTIVE&&&&& ',jlp_objf,j_v(ivobjective2)
	...
	subroutine renter()
		...
		if(niter.ge.10)then  !!!!
			...
			if(nnf.eq.nnfiter.and.jlp_objf-jlp_valiter.le.jlp_tolecur)then
				...
				niter=0
				jlp_valiter=jlp_objf
			endif !if(nnf.eq.nnfiter.and.jlp_objf-jlp_valiter.le.jlp_tolecur)   7871
		endif !if(niter.ge.10)   7868
		...
		endif !if(ienterv.eq.1.and.nsame.gt.5.and.jlp_objf.lt.oldsol+jlp_   7903
			...
			if(jlp_rhs2(ico).gt.jlp_rhs(ico))then
				...
				if(jlp_vc(ico).gt.jlp_vcmax.and.jlp_lower(ico).and.ico.ne.icolold)then
					...
					if(jlp_ubou(ico).and.jlp_lbou(ico)) then
						...
						jlp_tmax=j_inf  ;rcur=jlp_tmax
					endif !if(jlp_ubou(ico).and.jlp_lbou(ico))   7927
					...
					if(jlp_ubou(ico).and.jlp_lbou(ico)) then
						...
						jlp_tmax=j_inf ;rcur=jlp_tmax
					endif !if(jlp_ubou(ico).and.jlp_lbou(ico))   7948
					jlp_vcmax=-jlp_vc(ico)
					jlp_post=.true.
					if(p) write(n16,*)'posit resid could enter',ico  !!!!
					if(p.and.jlp_fpresent) write(n16,*)'**fact** KANTAAN >> residual, ienter = 1'
				end if !if(jlp_vc(ico).gt.jlp_vcmax.and.jlp_lower(ico).and.ico.ne.   7919
			end if !if(jlp_rhs2(ico).gt.jlp_rhs(ico))   7914
		end do !ilr=lr0+1,jlp_nrow   7908
	...
	subroutine zenter()
		...
		do ilz=lz0+1,nz
			...
			if(sparse)then
				...
				enddo !jj=jlp_lavecsp(jlp_lavecsp(0)+newa0),last(newa0)   7979
			else !if(sparse)then
				do  j=1,jlp_nrow
					val_=val_-jlp_vc(j)*jlp_a(j,newa0)
				enddo ! j=1,jlp_nrow   7984
			endif !if(sparse)   7977
			...
			if(val_.gt.jlp_vcmax)then
				newa=newa0
				newc=newc0
				if(p)write(n16,*)'z could enter',newa,'tmax',j_inf,'newc',newc
				if(p.and.jlp_fpresent) write(n16,*)'**fact** KANTAAN >> z, ienter = 2'
				ienter=2
				jlp_tmax=j_inf ;rcur=jlp_tmax
				ilzmax=ilz  !	do ilz=lz0+1,nz
			endif !if(val_.gt.jlp_vcmax)   7991
		enddo !ilz=lz0+1,nz   7966
	...
	subroutine senter()
		...
		if(jlp_xpresent)then  !newprice   !B31>
			...
			do i=1,jlp_nrow
				...
				endif !if(jlp_ix(i).ne.0)   8016
			enddo !i=1,jlp_nrow   8015
			if(p)  write(n16,'(a,(1x,5g19.7))')'vx',(jlp_vx(i),i=0,min(jlp_nrow,50))
		endif !if(jlp_xpresent)   8008
 
		jlp_valueopt=jlp_small  !j_ninf
		secondb=jlp_small
		...
		do jj=1,jlp_nrowp
			...
			endif !if(jlp_vx(j).ne.0.)   8045
		enddo !jj=1,jlp_nrowp   8043
		...
		endif !if(nxbas.le.0.and..not.jlp_fpresent)   8052
		if(p)then
		...
		do icin=1,jlp_ntemp0                !remove zero-elements
			...
			endif !if(jlp_ixpack(icin).ne.0)   8062
		enddo !icin=1,jlp_ntemp0   8061
			...
			ulloop:	do jj_=1,jlp_ntemp0
				do jj2_=1,icout
					if(jlp_ixpack2(jj2_).eq.iperiodperm(jj_))then
						...
						idiff3(jj_)=ixtree_
						cycle ulloop
					endif !if(jlp_ixpack2(jj2_).eq.iperiodperm(jj_))   8074
				enddo !jj2_=1,icout   8073
				idiff3(jj_)=ixtree_+1
			enddo ulloop !oop:	do jj_=1,jlp_ntemp0   8072
 
		endif !if(istree)   8069
		...
		if (jlp_fpresent) then
			...
			do jj=1,jlp_nrowpf !!!! rivit, joilla xk-muuttujia
				...
				do k_=1,jlp_nfxrow(irowj_) ! silmukka : rivin xk-muuttujat
					if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)).eq. &
						...
						nvarxk = nvarxk + 1
					endif !if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj_)+k_))   8102
				enddo !k_=1,jlp_nfxrow(irowj_)   8101
			enddo !jj=1,jlp_nrowpf   8098
					...
					do ivars_=1,j_o(listy_)%i(1) ! yk-mjan puutavaralistan muuttujat
						...
						do ifact_=1,j_o(listf_)%i(1) ! yk-mjan tehdaslistan tehtaat
							...
							if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_) then
								nvaryk = nvaryk + 1
							endif !if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_)   8121
						enddo !ifact_=1,j_o(listf_)%i(1)   8118
					enddo !ivars_=1,j_o(listy_)%i(1)   8115
				enddo !k_=1,jlp_nfyrow(irowj_)   8112
			enddo !jj=1,jlp_nrowpfy   8109
 
			if(allocated(jlp_rowxkfkey))then
				if(size(jlp_rowxkfkey).lt.nvarxk)deallocate(jlp_rowxkfkey)
			endif !if(allocated(jlp_rowxkfkey))   8129
			...
			endif !if(allocated(jlp_rowykfkey))   8134
			...
			do jj=1,jlp_nrowpf ! rivit, joilla xk-muuttujia
				...
				do k_=1,jlp_nfxrow(irowj_) ! silmukka : rivin xk-muuttujat
					...
					endif !if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj_)+k_))   8146
				enddo !k_=1,jlp_nfxrow(irowj_)   8145
			enddo !jj=1,jlp_nrowpf   8142
				...
				do k_=1,jlp_nfyrow(irowj_) ! silmukka : rivin yk-muuttujat
					...
					do ivars_=1,j_o(listy_)%i(1) ! yk-mjan puutavaralistan muuttujat
						...
						do ifact_=1,j_o(listf_)%i(1) ! yk-mjan tehdaslistan tehtaat
							...
							endif !if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_)   8166
						enddo !ifact_=1,j_o(listf_)%i(1)   8164
					enddo !ivars_=1,j_o(listy_)%i(1)   8161
				enddo !k_=1,jlp_nfyrow(irowj_)   8158
			enddo !jj=1,jlp_nrowpfy   8155
 
		endif !if (jlp_fpresent)   8091
		...
		if (nfy.gt.0.and.iunit.ne.iunitrans) then
			...
			enddo !j=1,j_o(jlp_ivkeepc)%i(1)   8181
			do j=1,ntrans
				call dotrans(jlp_itransv(j),1)
				if(j_err)then
			...
			enddo !j=1,ntrans   8184
			iunitrans = iunit
		endif !if (nfy.gt.0.and.iunit.ne.iunitrans)   8180
		...
		basreject(1:jlp_nsch(iunit))=.false.
		do k_=1,jlp_ld0
			if(jlp_lunit(jlp_ld(k_)).ne.iunit) cycle
			i=jlp_isch(jlp_ld(k_))
			basreject(i)=.true. !if(jlp_isch(jlp_ld(k_)).ne.i) cycle
			...
			! vxpack3(jj_)*jlp_xmat(ixpack3(jj_)+ibxmatx) !dprod(vxpack3(jj_),jlp_xmat(ixpack3(jj_)+ibxmatx))
			! enddo !do jj_=idiff3(idiff(iobs)),idiff3(idiff(iobs+1))-1,1
			! endif !if(istree)then
			if(fast.and..not.fastnow)fastvalues(i)=j_inf
			!	basreject(cycle nschloop
		enddo !k_=1,jlp_ld0   8215
		ibxmatx=(jlp_ibaunit(iunit)-1)*jlp_ntemp0
		!write(17,*)'nytpå',kier
		!  -fopenmp
		!$OMP PARALLEL DO
		nschloop:	do i=1,jlp_nsch(iunit)
			ibxmatx=ibxmatx+jlp_ntemp0
			if(basreject(i))then
				svalue(i)=j_ninf
				cycle
			endif !if(basreject(i))   8235
			!write(17,*)'nytpot',kier,iunit,i
			iobs=jlp_ibaunit(iunit)+i
			if(fastnow)then
				if(fastreject(iobs).and.i.ne.jlp_keys(iunit))then
					svalue(i)=j_ninf
					cycle  !i.ne.j_keys(iunit) added 20.8.2018 JL
				endif !if(fastreject(iobs).and.i.ne.jlp_keys(iunit))   8242
			endif !if(fastnow)   8241
			if(subfilre)then
				if(jlp_rejects(iobs))then
					! if(istree)then
					! ibxmatx=xmatiba(iobs) !,1)
					! do jj_=idiff3(idiff(iobs)),idiff3(idiff(iobs+1))-1,1
					!valuesums(jj_)=valuesums(jj_-1)+dprod(vxpack3(jj_),jlp_xmat(ixpack3(jj_)+ibxmatx))
					! enddo !do jj_=idiff3(idiff(iobs)),idiff3(idiff(iobs+1))-1,1
					! endif !if(istree)then
					svalue(i)=j_ninf
					cycle
				endif !if(jlp_rejects(iobs))   8248
			endif !if(subfilre)   8247
			...
			if (jlp_xpresent2) then
				...
				! valuesums(jj_)=valuesums(jj_-1)+vxpack3(jj_)*jlp_xmat(ixpack3(jj_)+ibxmatx) !dprod(vxpack3(jj_),jlp_xmat(ixpack3(jj_)+ibxmatx))
 
				! enddo !do jj_=idiff3(idiff(iobs)),nxbas
				! jlp_value=valuesums(nxbas)
 
				! else !if(istree)then
				!			ibxmatx=xmatiba(iobs) !,1)xmatiba(iobs)=(iobs-1)*jlp_ntemp0
				!		jlp_value=dot_product(jlp_vxpack2(1:nxbas),jlp_xmat(jlp_ixpack2(1:nxbas)+ibxmatx) )   !!!!
				svalue(i)=dot_product(jlp_vxpack2(1:nxbas),jlp_xmat(jlp_ixpack2(1:nxbas)+ibxmatx) )   !!!!
				...
				svalue(i)=j_0   !jlp_value = j_0
			endif !if (jlp_xpresent2)   8262
			...
			if (jlp_fpresent) then !!!!
				...
				svalue(i) = svalue(i)+ value_f(iobs)
			endif !if (jlp_fpresent)   8288
			...
			if(fastmake)then
				fastvalues(i)=svalue(i)  !jlp_value
				if(svalue(i).lt.fastvaluemin)fastvaluemin=svalue(i) !jlp_value
			endif !if(fastmake)   8309
 
		enddo nschloop !hloop:	do i=1,jlp_nsch(iunit)   8233
		!$OMP END PARALLEL DO
		jlp_valuek=svalue(jlp_keys(iunit))
		loco=maxloc(svalue(1:jlp_nsch(iunit))) !output must be array
		iopt=loco(1)
		jlp_valueopt=svalue(iopt)
		!if(iunit.lt.15)write(6,*)'iunitetc',iunit,jlp_valuek,iopt,jlp_valueopt
		...
		if(fastmake)then
			...
			! ! ibxmatx=xmatiba(ioob,1)
			! ! !if(pp)write(16,*)ioob,j_memobs(ioob),fastreject(ioob),j_xmat(ibxmatx+1:ibxmatx+j_ntemp0)
			! ! enddo
			...
			! ! ioob=j_ibaunit(iunit)+i
			! ! ibxmatx=xmatiba(ioob,1)
			! ! write(16,*)ioob,j_memobs(ioob),fastreject(ioob),j_xmat(ibxmatx+1:ibxmatx+j_ntemp0)
			! ! enddo
			! ! write(16,*)'j_xmatlast,j_xmatlast2',j_xmatlast,j_xmatlast2
			! ! stop 775
			...
			! j_o(ivxdatmat)%r(iba+1:iba+keepx)=j_o(ivxdatmat)%r(iba2+1:iba2+keepx)
	...
	subroutine tulostele()
		...
		endif !if(jlp_feasible)   8424
 
		call defsolu()
		if(jlp_xpresent)then
 
			call getsolx() !jlp_nunits,ibaunit,keys,lunit,nsch,isch,
		endif !if(jlp_xpresent)   8431
		...
		if(printlf.or.p9)then
			...
			do lfj_=jlp_mxd+1,jlp_lf0
				write(6,*) jlp_lunit(jlp_lf(lfj_)),jlp_ixkf(jlp_lf(lfj_)), &
					jlp_ixkffact(jlp_lf(lfj_)),jlp_keyfact(jlp_lunit(jlp_lf(lfj_)),&
					jlp_ixkf(jlp_lf(lfj_))),jlp_x(jlp_nrowz+jlp_lf(lfj_))
			enddo !lfj_=jlp_mxd+1,jlp_lf0   8440
		endif !if(printlf.or.p9)   8437
	...
	subroutine tulostele2() !prints and checkes if finished
		...
		if(jlp_feasible)then
			if(kier.le.10.or.int(kier/10)*10.eq.kier)then
				...
				if(coefmax*jlp_objfprev.le.j_0)then
					write(6,'(i5,i8,g22.12,7x,f8.2,5i6,f6.2,f7.2,i5)')&
				...
				else !if(coefmax*j_objfprev.le.j_0)then
					if(coefmax*jlp_objfprev.gt.j_0)pros=100.*coefmax*(jlp_objf-jlp_objfprev)/jlp_objfprev
					change=jlp_objf-jlp_objfprev
					nimp2=-1
					if(as.ne.asv.and.asv.ne.100.)nimp2=nimp
					...
					if(isstop.and.kier.gt.10)then
						...
						if(j_codevalue(iob,jlp_stoplink).ne.j_0)then
							...
							goto900=.true.
							return
						endif !if(j_codevalue(iob,jlp_stoplink).ne.j_0)   8495
						...
						if(pros.lt.0.01.and.kier.ge.30)then
							...
							goto900=.true.
							return
						endif !if(pros.lt.0.01.and.kier.ge.30)   8502
 
					endif !if(isstop.and.kier.gt.10)   8489
				endif !if(coefmax*jlp_objfprev.le.j_0)   8466
			...
			endif !if(kier.le.10.or.int(kier/10)*10.eq.kier)   8459
			if(fast)then
				re=count(fastreject)
				...
				endif !if(fastusedsame.gt.fastusesame)   8525
			...
			do i=1,lr0 !lr0 = number of basic residuals
				...
				elseif(.not.jlp_lower(jlp_lr(i)).and.jlp_x(jlp_lr(i)).lt.-jlp_tole(jlp_lr(i))) then !if(j_lower(j_lr(i)).and.j_x(j_lr(i)).gt.j_tole(j_lr(i))) then
					numn=numn+1
					jlp_value=jlp_value+jlp_x(jlp_lr(i))
				endif !if(jlp_lower(jlp_lr(i)).and.jlp_x(jlp_lr(i)).gt.jlp_tole(j   8581
			enddo !i=1,lr0   8579
			if(kier.le.10.or.int(kier/10)*10.eq.kier)then
				as=100.
			...
			endif !if(kier.le.10.or.int(kier/10)*10.eq.kier)   8589
		endif !if(jlp_feasible)   8458
		! if(kier.ge.mxiter)then
		...
		!!!! slow improvement?
		if(((jlp_objf.le.jlp_xirowold2+jlp_tolecur).or.(slow.gt.j_0.and.&
			...
			endif !if((fastusesame.ne.1.or.iterxkf.ne.1).and..not.tried)   8623
			...
			if(jlp_feasible)then
				j_v(ivfeasible)=1.
				j_v(ivoptimal)=1.
			endif !if(jlp_feasible)   8681
 
			goto900=.true.;return   ! <b332>
		endif !if(((jlp_objf.le.jlp_xirowold2+jlp_tolecur).or.(slow.gt.j_   8608
	...
	subroutine fenter0()
		...
		ibxdatobs=xdatiba(iobs)  !,1)
		ibxdatkey_=xdatiba(ikey_)  !,2)
		...
		do jj = 1,nrowxkfkey
			...
			jlp_valueopt_af(jlp_rowxkfkey(jj)%jcurix) = jlp_valueopt_af(jlp_rowxkfkey(jj)%jcurix) +&
				...
				(j_o(ivxdatmat)%d(ibxdatobs+irowfkeep_) -&   !xmatiba
				j_o(ivxdatmat)%d(ibxdatkey_+irowfkeep_))      !xmatiba
			!	if(p.and.j_rowxkfkey(jj)%jcurix.eq.2)write(	16,*)j_valueopt_af(j_rowxkfkey(jj)%jcurix)
		enddo !jj = 1,nrowxkfkey   8701
		...
		do jj=1,nrowykfkey
			...
			!		j_valueopt_af(j_rowykfkey(j_)%jcurix),j_v(j_rowykfkey(j_)%ivfout),&
			!		j_o(ivxdatmat)%r(ibxdatobs+iv2elpos_),j_o(ivxdatmat)%r(ibxdatkey_+iv2elpos_)  !xmatiba
			jlp_valueopt_af(jlp_rowykfkey(jj)%jcurix) = jlp_valueopt_af(jlp_rowykfkey(jj)%jcurix) + &
				j_v(jlp_rowykfkey(jj)%ivfout)*&
				(j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_) -&
				j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_))
		enddo !jj=1,nrowykfkey   8717
	...
	subroutine fenter()
		...
		do ixkyk_=ixkyk1_,j_o(jlp_ivxkyk)%i(1)	!!!!	xkyk-lista
			...
			do inf_=1, jlp_nxkfact(ixkyk_)
				...
				do j=jlp_xkfact(ixkyk_,inf_)%i1xkykrowvar,jlp_xkfact(ixkyk_,inf_)%inxkykrowvar
					...
					if(jlp_irow2curix(0,jlp_xkykrowvars(j)%irow).ne.1)then
						write(6,*)'<fe33>,j',j
						stop 987
					endif !if(jlp_irow2curix(0,jlp_xkykrowvars(j)%irow).ne.1)   8778
					...
					endif !if(jcurix_.ne.jcurix2)   8788
					...
					if (jcurix_ == 0) then
						...
						if (jlp_feasible) then
							...
							endif !if (jlp_xkykrowvars(j)%isxk)   8798
						else !if (j_feasible) then
						...
						endif !if (jlp_xkykrowvars(j)%isxk)   8813
 
						! if(.not.j_feasible)then   !this section was commented by JL 9.9.2018
						! if(j_objr2( jcurix_).ne.j_0)then
					...
					endif !if (jcurix_ == 0)   8795
 
					!enddo !do jc_ = 1,j_irow2curix(0,j_xkykrowvars(j)%irow)
				enddo !j=jlp_xkfact(ixkyk_,inf_)%i1xkykrowvar,jlp_xkfact(ixkyk_,i   8775
 
				if(jlp_value>jlp_valueopt.and.junk_>0) then  !!!!
					jlp_valueopt=jlp_value
					ifopt = if_
					infopt=inf_
				endif !if(jlp_value>jlp_valueopt.and.junk_>0)   8838
 
				if (if_.eq.jlp_keyfact(iunit,ixkyk_)) then
					jlp_valuek = jlp_value
					infkey=inf_
				endif !if (if_.eq.jlp_keyfact(iunit,ixkyk_))   8844
 
			enddo !inf_=1, jlp_nxkfact(ixkyk_)   8762
			...
			if((ifopt/=jlp_keyfact(iunit,ixkyk_)).and.(jlp_valueopt>(jlp_valuek+0.01*jlp_tolecur))) then
				...
				endif !if(isxkzero(ixkykenter))   8860
				...
				do j=jlp_xkfact(ixkyk_,infopt)%i1xkykrowvar,jlp_xkfact(ixkyk_,infopt)%inxkykrowvar
					do jc_ = 1,jlp_irow2curix(0,jlp_xkykrowvars(j)%irow)	! alkup. riviä vastaavat lavennetut rivit
						...
						endif !if (jlp_xkykrowvars(j)%isxk)   8873
					end do !jc_ = 1,jlp_irow2curix(0,jlp_xkykrowvars(j)%irow)   8870
				enddo !j=jlp_xkfact(ixkyk_,infopt)%i1xkykrowvar,jlp_xkfact(ixkyk_   8869
				...
				do j=jlp_xkfact(ixkyk_,infkey)%i1xkykrowvar,jlp_xkfact(ixkyk_,infkey)%inxkykrowvar
					do jc_ = 1,jlp_irow2curix(0,jlp_xkykrowvars(j)%irow)	! alkup. riviä vastaavat lavennetut rivit
						...
						endif !if (jlp_xkykrowvars(j)%isxk)   8890
					end do !jc_ = 1,jlp_irow2curix(0,jlp_xkykrowvars(j)%irow)   8887
				enddo !j=jlp_xkfact(ixkyk_,infkey)%i1xkykrowvar,jlp_xkfact(ixkyk_   8886
			...
			endif !if((ifopt/=jlp_keyfact(iunit,ixkyk_)).and.(jlp_valueopt>(j   8854
 
		enddo !ixkyk_=ixkyk1_,j_o(jlp_ivxkyk)%i(1)   8755
	...
	subroutine entercol()
		...
		if(jlp_ixcur(0).ne.0)then
			jlp_objr0(newc)=jlp_xmat(jlp_ix(0)+ibxmatx2)-jlp_xmat(jlp_ix(0)+ibxmatx) !v(ix(0))
			j1=2
		else !if(j_ixcur(0).ne.0)then
			j1=1
			jlp_objr0(newc)=0.
		endif !if(jlp_ixcur(0).ne.0)   8959
		...
		if(jlp_fpresent) then
			...
			endif !if((jlp_nrowpf.gt.0.and.jlp_ifxcurrows(1).eq.0).or.(jlp_nr   8970
		endif !if(jlp_fpresent)   8968
		...
		if(sparse)then
			...
			do jj=j1,jlp_nrowp
				j=jlp_ixcurrows(jj)
				jlp_apu=jlp_xmat(jlp_ix(j)+ibxmatx2)-jlp_xmat(jlp_ix(j)+ibxmatx)  !a( ,newa ilamn pakkausta
 
				if(abs(jlp_apu).gt.jlp_tiny78)then
					iel=iel+1
					jlp_a(iel,newa)=jlp_apu
					jlp_lavecsp(istart1+iel)=j
				endif !if(abs(jlp_apu).gt.jlp_tiny78)   8986
 
				!robleema, jos pitää vaihtaa
			enddo !jj=j1,jlp_nrowp   8982
			...
			do j=1,jlp_nrow
				if(jlp_ixcur(j).ne.0)then
					jlp_a(j,newa)=jlp_xmat(jlp_ix(j)+ibxmatx2)-jlp_xmat(jlp_ix(j)+ibxmatx)  !v(ix(j))
				else !if(j_ixcur(j).ne.0)then
					jlp_a(j,newa)=0.
				endif !if(jlp_ixcur(j).ne.0)   9000
			enddo !j=1,jlp_nrow   8999
 
			!a:n päivitys
			if(jlp_fpresent) then
				jlp_a(1:jlp_nrow,newa) = jlp_a(1:jlp_nrow,newa)+ jlp_valueopt_af(1:jlp_nrow)
			endif !if(jlp_fpresent)   9008
 
		endif !if(sparse)   8976
	...
	subroutine leaving()
		...
		if(ienter.eq.ienterv.and.	jlp_objf.lt.oldsol+jlp_tolecur)then
			...
			oldsol=jlp_objf
			nsame=0
		endif !if(ienter.eq.ienterv.and.	jlp_objf.lt.oldsol+jlp_tolecur)   9036
		...
		if(sparse)then
			...
			if(ienter.gt.1)then  !!!!
				...
				call fbsub(jlp_nrow,1,jlp_nrow,jlp_a,jlp_lavec,newc,jlp_a(1:,newa),r, & !!!!
					jlp_ls,wslu1,lwsll1,.false.)   !!!!
			endif !if(ienter.gt.1)   9090
		endif !if(sparse)   9073
		...
		if(.not.jlp_post)then
			...
			enddo !jj=1,jlp_nrow   9108
			jlp_post=.true.   !!!!
		endif !if(.not.jlp_post)   9107
		...
		do j=1,lz0  !!!! z leaving
			...
			if(r(leaz).gt.jlp_tiny78)then
				if(jlp_x(leaz).lt.jlp_tmax*r(leaz))then
					...
					leavec=leaz
				endif !if(jlp_x(leaz).lt.jlp_tmax*r(leaz))   9166
			endif !if(r(leaz).gt.jlp_tiny78)   9165
		enddo !j=1,lz0   9162
			...
			do j=1,jlp_ld0  !!!!sched leaving
				...
				if(r(lead).gt.jlp_tiny78)then     !
					if(jlp_x(lead).lt.jlp_tmax*r(lead))then
						...
						leavec=lead  !!!!
					endif !if(jlp_x(lead).lt.jlp_tmax*r(lead))   9178
 
				endif !if(r(lead).gt.jlp_tiny78)   9177
			enddo !j=1,jlp_ld0   9174
			...
			if (jlp_fpresent) then
				...
				do j=jlp_mxd+1,jlp_lf0
					...
					if(r(leaf).gt.jlp_tiny78)then
						...
						endif !if(jlp_x(leaf).lt.jlp_tmax*r(leaf))   9193
 
					endif !if(r(leaf).gt.jlp_tiny78)   9192
				enddo !j=jlp_mxd+1,jlp_lf0   9189
			endif !if (jlp_fpresent)   9187
		endif !if(jlp_xpresent2)   9173
 
		if(p)write(n16,*)'tmaxbefore r',jlp_tmax  !!!!!
		if(p9.and.jlp_tmax.lt.j_0)write(n16,*)'negat tmax,ienter,leavec',jlp_tmax,ienter,leavec
2244	if(jlp_tmax.le.jlp_tmaxmin)then  !!!!! jos mukaan, pitää tsekata keyt ensin
			...
			endif !if(p.or.p9.or.jlp_tmax.eq.-1.d0)   9208
			if(ienter.eq.3.and.secondb.gt.jlp_valuek+jlp_tolecur)then
				...
				secondb=jlp_small
				goto222=.true.;return
			endif !if(ienter.eq.3.and.secondb.gt.jlp_valuek+jlp_tolecur)   9211
			if(ienter.eq.3)call lcursam()
			goto 8883 !!!!! jump88 if j_tmax.le.j_tmaxmin before checking f residual can leave
		endif !2244	if(jlp_tmax.le.jlp_tmaxmin)   9206
			...
			if(r(jlp_lr(j)).gt.jlp_tiny78)then !tole(lr(j)))then
				...
				if(jlp_ubou(jlp_lr(j)))then
					...
					if(jlp_lower(jlp_lr(j)).and.jlp_x(jlp_lr(j))+ &
						...
						if(jlp_lr(j).eq.2)write(6,*)'<777tas ',jlp_x(jlp_lr(j))
						...
						leavec=jlp_lr(j)
					else if( jlp_x(jlp_lr(j)).ge.0.and..not.jlp_lower(jlp_lr(j)))then !if(j_lower(j_lr(j)).and.j_x(j_lr(j))+j_rhs2(j_lr(j))-j_rhscur(j_lr(j)).lt.r(j_lr(j))*j_t
						...
						if(jlp_x(jlp_lr(j)).lt.jlp_tmax*r(jlp_lr(j)) )then
							...
							leavec=jlp_lr(j)
						end if !if(jlp_x(jlp_lr(j)).lt.jlp_tmax*r(jlp_lr(j)) )   9311
					elseif(jlp_x(jlp_lr(j)).lt.j_0.and..not.jlp_lower(jlp_lr(j)).and.jlp_feasible)then !if(j_lower(j_lr(j)).and.j_x(j_lr(j))+j_rhs2(j_lr(j))-j_rhscur(j_lr(j)).
					...
					end if !if(jlp_lower(jlp_lr(j)).and.jlp_x(jlp_lr(j)   9295
				end if !if(jlp_ubou(jlp_lr(j)))   9283
			...
			elseif(r(jlp_lr(j)).lt.jlp_tiny78n) then !if(r(j_lr(j)).gt.j_tiny78)then
				...
				if(jlp_lbou(jlp_lr(j)))then
					...
					if(.not.jlp_lower(jlp_lr(j)).and. &
						...
						if(jlp_lr(j).eq.2)write(6,*)'<497 ', jlp_x(jlp_lr(j))
 
						jlp_tmax=jlp_x(jlp_lr(j))/r(jlp_lr(j)) ;rcur=r(jlp_lr(j))
						jlp_lower(jlp_lr(j))=.true.
						!if(pp)write(n16,*)'post,negr,lbou,NOTlower,lr(j),tmax',j_lr(j),j_tmax
						leavec=jlp_lr(j)
					elseif(jlp_x(jlp_lr(j)).le.j_0.and.jlp_lower(jlp_lr(j)))then !if(.not.j_lower(j_lr(j)).and.j_x(j_lr(j))+j_rhs(j_lr(j))-j_rhscur(j_lr(j)).gt.r(j_lr(j))*j_tm
						...
						if(jlp_x(jlp_lr(j)).gt.jlp_tmax*r(jlp_lr(j)) )then  !-x/-r <tmax-tole -x<-r*(tmax-tole)
							...
							leavec=jlp_lr(j)
						end if !if(jlp_x(jlp_lr(j)).gt.jlp_tmax*r(jlp_lr(j)) )   9385
 
					elseif(jlp_x(jlp_lr(j)).gt.j_0.and.jlp_lower(jlp_lr(j)).and.jlp_feasible)then !if(.not.j_lower(j_lr(j)).and.j_x(j_lr(j))+j_rhs(j_lr(j))-j_rhscur(j_lr(j)).g
						...
						return
						!	goto 2244
					endif !if(.not.jlp_lower(jlp_lr(j)).an   9370
 
				end if !if(jlp_lbou(jlp_lr(j)))   9368
 
			end if !if(r(jlp_lr(j)).gt.jlp_tiny78)   9242
		...
		if(leavec.gt.0)then  !!!!!
			...
			endif !if (p.and.jlp_fpresent.and.(leavec > (jlp_nrowz+jlp_mxd)))   9431
 
		else !if(leavec.gt.0)then
			if(p)write(n16,*)'*no basic variable leaving'  !!!!
		endif !if(leavec.gt.0)   9424
...
! *********************************************'
		...
		do i=1,jlp_ld0
			...
			if(jlp_lunit(nex).ne.jlp_lunit(lcur))then
				...
				endif !if(jlp_lunit(lcur).eq.iunit.and.ienter.eq.3)   9484
				...
				if(jlp_rs.lt.jlp_tiny78n.and.jlp_wsu.gt.jlp_tmax*jlp_rs)then !!!!
					...
					!					end if
				endif !if(jlp_rs.lt.jlp_tiny78n.and.jlp_wsu.gt.jlp_tmax*jlp_rs)   9492
 
				jlp_wsu=jlp_onen   !opposite sign of ws as in old JLP
				jlp_rs=jlp_zero
				lcur0=jlp_next(lcur)   ! lcur0 is again the first col in the next new unit
				! last
			endif !if(jlp_lunit(nex).ne.jlp_lunit(lcur))   9481
			lcur=nex
		enddo !i=1,jlp_ld0   9474
...
!!!!!Avaintehdas vaihtuu, versio 2 (tarkastellaan vaihtuuko avaintehdas)
		if(jlp_fpresent) then
			...
			do ixkyk_=1,j_o(jlp_ivxkyk)%i(1)
				...
				do while (lcur0_unit > jlp_mxd)
					...
					iba1=xdatiba(jlp_ibaunit(lunit_)+ jlp_keys(lunit_)) !,1)
 
					jlp_wsu=j_o(ivxdatmat)%d(iba1+ikeepxkyk_) !xmatiba
					...
					endif !if ((ienter==4).and.(iunit==lunit_).and.(ixkyk_==ixkykente   9541
					...
					enddo !while (jlp_lunit(lcur_) == jlp_lunit(lcur0_unit))   9551
					...
					if ((ienter==3).and.(iunit==lunit_)) then
						...
						ibxdatkey_=xdatiba(ikey_) !,2)
						ibxdatopt_=xdatiba(iopt_) !,1)
						if(p.or.jlp_tmax.eq.-1.d0) write(n16,*)'**fact** <6741> wij tulossa kantaan: iunit,iopt,ikey',&
							iunit,iopt,ikey_
						!
						!		j_o(ivxdatmat)%r(xmatiba(ikey_)+ikeepxkyk_)
 
						jlp_rs=jlp_rs - &
							(j_o(ivxdatmat)%d(ibxdatopt_+ikeepxkyk_) -  & !xmatiba
							j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_))          !!xmatiba
 
					endif !if ((ienter==3).and.(iunit==lunit_))   9559
					...
					do while((jlp_lunit(jlp_lunw(lun_))/=lunit_).and.(lun_<=jlp_lunits0))
						lun_=lun_+1
					enddo !while((jlp_lunit(jlp_lunw(lun_))/=lunit_).and.(lun_<=jlp_l   9579
					...
					if(lun_ <= jlp_lunits0) then
						...
						do while (jlp_lunit(lcur_w) == jlp_lunit(lcur0_w))
							...
							ibxdatobs_=xdatiba(iobs_) !,1)
							if(p.or.jlp_tmax.eq.-1.d0)write(16,*)'tas',ikey_, &
								j_o(ivxdatmat)%d(ibxdatkey_+1:ibxdatkey_+keepx)
							if(p)write(16,*)'ta2',iobs_,j_o(ivxdatmat)%d(ibxdatobs_+1:ibxdatobs_+keepx)
							xkij_=j_o(ivxdatmat)%d(ibxdatobs_+ikeepxkyk_)   !xmatiba
 
 
							jlp_rs=jlp_rs+r(lcur_w+jlp_nrowz)*&
								(xkij_ - j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_))
							jlp_wsu=jlp_wsu+jlp_x(lcur_w+jlp_nrowz)*&
								(xkij_ - j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_))
							if(p)write(16,*)'h3',jlp_wsu
							!write(nu20,*)'hep2',j_o(ivxdatmat)%r(ibxdatkey_+1:ibxdatkey_+10)
							!write(nu20,*)j_o(ivxdatmat)%r(ibxdatkey_+ikeepxkyk_)
							lcur_w = jlp_next(lcur_w)
						enddo !while (jlp_lunit(lcur_w) == jlp_lunit(lcur0_w))   9592
 
					endif !if(lun_ <= jlp_lunits0)   9584
 
					if (p) then
						if(jlp_wsu<j_0)write(n16,*)'**fact** <6738> negat. wsu'
					endif !if (p)   9614
					!wsu-rs kombinaatiot
					! wsu >0, rs >0 -> t = wsu/rs
					...
					endif !if(p.or.jlp_tmax.eq.-1.d0)   9629
					...
					endif !if(jlp_rs>jlp_tiny78.and.jlp_wsu.lt.jlp_tmax*jlp_rs)   9636
 
					lcur0_unit = lcur_
				enddo !while (lcur0_unit > jlp_mxd)   9528
			enddo !ixkyk_=1,j_o(jlp_ivxkyk)%i(1)   9525
 
			if(jlp_tmax.le.jlp_tmaxmin)then ! eq.j_0)then
				! if(ienter.eq.3.and.secondb.gt.jlp_valuek+jlp_tolecur)then
			...
			endif !if(jlp_tmax.le.jlp_tmaxmin)   9656
 
			if ((ienter==4).and.(lcursamef==0)) then
				ikeepxkykenter_ = jlp_ixkykkeep(ixkykenter)
 
				ibas1=xdatiba(jlp_ibaunit(iunit)+ jlp_keys(iunit)) !,1)
				jlp_wsu=j_o(ivxdatmat)%d(ibas1+ikeepxkykenter_)  !xmatiba
				...
				do while((jlp_lunit(jlp_lunw(lun_))/=iunit).and.(lun_<=jlp_lunits0))
					lun_=lun_+1
				enddo !while((jlp_lunit(jlp_lunw(lun_))/=iunit).and.(lun_<=jlp_lu   9678
				...
				if(lun_ <= jlp_lunits0) then
					...
					do while (jlp_lunit(lcur_) == jlp_lunit(lcur0_w))
						...
						ibxdatobs_=xdatiba(iobs_) !,1)
						xkij_=j_o(ivxdatmat)%d(ibxdatobs_+ikeepxkykenter_) !xmatiba
						jlp_rs=jlp_rs+r(lcur_+jlp_nrowz)*&
							(xkij_ - j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkykenter_))  !xmatiba
						jlp_wsu=jlp_wsu+jlp_x(lcur_+jlp_nrowz)*&
							(xkij_ - j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkykenter_))  !xmatiba
						if(p)write(16,*)'h5',jlp_wsu
						lcur_ = jlp_next(lcur_)
					enddo !while (jlp_lunit(lcur_) == jlp_lunit(lcur0_w))   9692
 
				endif !if(lun_ <= jlp_lunits0)   9684
				...
				endif !if((jlp_rs>jlp_tiny78).and.(jlp_wsu.lt.jlp_tmax*jlp_rs))   9710
 
			endif !if ((ienter==4).and.(lcursamef==0))   9667
 
			if ((ienter==4).and.(leavkf==0).and.(leave==0).and.(leavk==0)) then
				lun_= 1
				do while((jlp_lunit(jlp_lunw(lun_))/=iunit).and.(lun_<=jlp_lunits0))
					lun_=lun_+1
				enddo !while((jlp_lunit(jlp_lunw(lun_))/=iunit).and.(lun_<=jlp_lu   9724
				if (lun_<=jlp_lunits0) then
				...
				endif !if (lun_<=jlp_lunits0)   9727
			endif !if ((ienter==4).and.(leavkf==0).and.(leave==0).and.(leavk=   9722
		endif !if(jlp_fpresent)   9513
		...
		if(leavk.gt.0)then
			...
			if(sparse)then
				...
				enddo !i=jlp_lavecsp(jlp_lavecsp(0)+icol),last(icol)   9766
			else !if(sparse)then
				do j=1,jlp_nrow
					if((jlp_ix(j).ne.0).or.jlp_fpresent)jlp_xps(j)=jlp_xps(j)+jlp_a(j,leavk+nz) !leavk col in D
					jlp_rhsw(j)=jlp_rhscur(j)-jlp_xps(j)
				enddo !j=1,jlp_nrow   9773
 
			endif !if(sparse)   9763
			...
			if(iunit.eq.jlp_lunit(leavk).and.ienter.eq.3)then
				...
				if(sparse)then
					...
					enddo !j=1,jlp_nrow   9793
				endif !if(sparse)   9786
			endif !if(iunit.eq.jlp_lunit(leavk).and.ienter.eq.3)   9783
			...
			if(jlp_testl)then
				if(jlp_keys(jlp_lunit(lcur)).gt.jlp_nsch(jlp_lunit(lcur)))then
					...
					write(6,*)'*u* return'
					goto900=.true.;return
				endif !if(jlp_keys(jlp_lunit(lcur)).gt.jlp_nsch(jlp_lunit(lcur)))   9815
			endif !if(jlp_testl)   9814
	...
	! first free column is reserved for newc
			if(jlp_lunit(lcur).eq.jlp_lunit(nex))then
				...
				if(sparse)then
					...
					endif !if(nup.ge.nfreq)   9881
					call pivot(nex+jlp_nrowz,newc2,jlp_nrow, &
						nm,jlp_a,jlp_lavec,e,wslu1,lwsll1,ifail,jlp_info)
				endif !if(sparse)   9870
				ipivot=ipivot+1
				if(p.or.p9.or.p8)write(n16,*)'pivot<9646>',ipivot,1,jlp_objf,jlp_tmax
				...
				endif !if(ipivot.eq.idebug)   9919
				...
				if(jlp_fpresent)then
					...
					do ilu_ = 1,jlp_lunits0
						if (jlp_lunw(ilu_)==nex) then
							jlp_lunw(ilu_)=id
						endif !if (jlp_lunw(ilu_)==nex)   9976
					enddo !ilu_ = 1,jlp_lunits0   9975
				endif !if(jlp_fpresent)   9973
				...
				lcur=id
				goto 17
			endif !if(jlp_lunit(lcur).eq.jlp_lunit(nex))   9830
		...
		if (jlp_fpresent) then
			...
			if(leavkwf>0) then
				...
				if ((nfy.gt.0).and.(jlp_lunit(leavkwf)/=iunitrans)) then
					...
					enddo !j=1,j_o(jlp_ivkeepc)%i(1)  10016
					do j=1,ntrans
						call dotrans(jlp_itransv(j),1)
						if(j_err)then
							write(6,*)'err for trans ',j
							stop 761
						endif !if(j_err)  10021
					enddo !j=1,ntrans  10019
					iunitrans=jlp_lunit(leavkwf)
				endif !if ((nfy.gt.0).and.(jlp_lunit(leavkwf)/=iunitrans))  10015
				...
				do while(jlp_lunit(nex)==jlp_lunit(leavkwf))
					...
					!objr0:n päivitys
					if(jlp_ixcur(0).ne.0)then
						jlp_objr0(newc2)=jlp_xmat(jlp_ix(0)+ibxmatx)-jlp_xmat(jlp_ix(0)+ibxmatkey_)
					else !if(j_ixcur(0).ne.0)then
						jlp_objr0(newc2)=0.
					endif !if(jlp_ixcur(0).ne.0)  10075
 
					!j_ : lavennetut tehtävärivit
					do jj=1,jlp_nrow
						if(jlp_ixcur(jj).ne.0)then
							jlp_a(jj,ia)=jlp_xmat(jlp_ix(jj)+ibxmatx)-jlp_xmat(jlp_ix(jj)+ibxmatkey_)
						else !if(j_ixcur(j_).ne.0)then
							jlp_a(jj,ia)=0.
						endif !if(jlp_ixcur(jj).ne.0)  10083
					enddo !jj=1,jlp_nrow  10082
					...
					do jj=1,jlp_nrowpf ! #tehdasmjarivit
						...
						do k_=1,jlp_nfxrow(irowj_)
							...
							endif !if ((jlp_lunit(leavkwf)==iunit_lkf).and.(ixkyk_==ixkyk_lkf  10103
							...
							if(keyf_==jlp_irowffact(jlp_ibafx(irowj_)+k_)) then
								...
								if (jcurix==0) then
									jlp_objr0(newc2)=jlp_objr0(newc2) + &
										...
										(j_o(ivxdatmat)%d(ibxdatobs_ &
										+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)) - &
										j_o(ivxdatmat)%d(ibxdatkey_+&   !xmatiba
										jlp_irowfkeep(jlp_ibafx(irowj_)+ k_)))
								...
								endif !if (jcurix==0)  10114
							endif !if(keyf_==jlp_irowffact(jlp_ibafx(irowj_)+k_))  10112
 
						enddo !k_=1,jlp_nfxrow(irowj_)  10098
 
					enddo !jj=1,jlp_nrowpf  10093
					...
					do jj=1,jlp_nrowpfy
						...
						do k_=1,jlp_nfyrow(irowj_)
							...
							do ivars_=1,j_o(listy)%i(1) ! yk-mjan puutavaralistan muuttujat
								...
								endif !if ((jlp_lunit(leavkwf)==iunit_lkf).and.(iv2xkykpos_==ixky  10150
									...
									if(keyf_.eq.iv3factpos_) then
										...
										if (jcurix==0) then
											...
											jlp_objr0(newc2)=jlp_objr0(newc2) + j_v(jlp_fyfactout(iv2xkykpos_,iv3factpos_)) *  &
												(j_o(ivxdatmat)%d(ibxdatobs_+iv2elpos_) - &   !xmatiba
												j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_))          !xmatiba
										else !if (jcurix==0) then
											jlp_a(jcurix,ia)=jlp_a(jcurix,ia) + j_v(jlp_fyfactout(iv2xkykpos_,iv3factpos_)) *  &
												(j_o(ivxdatmat)%d(ibxdatobs_+iv2elpos_) - &  !xmatiba
												j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_))  !xmatiba
										endif !if (jcurix==0)  10162
									endif !if(keyf_.eq.iv3factpos_)  10160
 
								enddo !ifact_=1,j_o(listf)%i(1)  10156
							enddo !ivars_=1,j_o(listy)%i(1)  10145
						enddo !k_=1,jlp_nfyrow(irowj_)  10142
					enddo !jj=1,jlp_nrowpfy  10139
					...
					if(abs(jlp_x(nex+jlp_nrowz)).lt.jlp_tiny78)then
						...
						goto55=.true.;return
					endif !if(abs(jlp_x(nex+jlp_nrowz)).lt.jlp_tiny78)  10210
					...
					if(p.or.p9.or.p8)write(n16,*)'<67>',ipivot,jlp_objf,ienter,iunit,jlp_tmax,&
					...
					if(ipivot.eq.idebug)then
						...
						if(p.and.p8)stop 'debug'
						!if(p.and.jlp_nrow.le.50)p2=.true.
						write(n16,*)'*debug*'
						jdebug=jdebug+1
						if(jdebug.le.ndebug)idebug=j_v(j_o(iob)%i(ibasdebug+jdebug))
					endif !if(ipivot.eq.idebug)  10233
					...
					do ilu_ = 1,jlp_lunits0
						if (jlp_lunw(ilu_)==nex) then
							jlp_lunw(ilu_)=id
						endif !if (jlp_lunw(ilu_)==nex)  10342
					enddo !ilu_ = 1,jlp_lunits0  10341
					...
					nex=jlp_next(nex)
				enddo !while(jlp_lunit(nex)==jlp_lunit(leavkwf))  10060
 
			endif !if(leavkwf>0)  10006
			...
			if (leavkf.gt.jlp_mxd) then
				...
				endif !if(p.or.p9)  10381
				ibas1=xdatiba(jlp_ibaunit(iunit_)+jlp_keys(iunit_)) !,1)
				valtas=j_o(ivxdatmat)%d(ibas1+ikeepx_lkf)   !xmatiba
				...
				if(iunit.eq.jlp_lunit(leavkf).and.ienter.eq.4.and.(ixkykenter==ixkyk_lkf)) then
					...
					enddo !j=1,jlp_nrow  10405
 
				endif !if(iunit.eq.jlp_lunit(leavkf).and.ienter.eq.4.and.(ixkyken  10399
				...
				if(iunit.eq.jlp_lunit(leavkf).and.ienter.eq.3)then
					...
					ibxdatopt_=xdatiba(iopt_)! ,1)
					ibxdatkey_=xdatiba(ikey_)! ,2)
					ibxmatxopt_=xmatiba(iopt_)! ,1)
					ibxmatkey_=xmatiba(ikey_) !,2)
					!newc, newa asetettu aiemmin (ienter = 3 paikkeilla)
					if(jlp_ixcur(0).ne.0)then
						jlp_objr0(newc)=jlp_xmat(jlp_ix(0)+ibxmatxopt_)-jlp_xmat(jlp_ix(0)+ibxmatkey_)
					else !if(j_ixcur(0).ne.0)then
						jlp_objr0(newc)=0.
					endif !if(jlp_ixcur(0).ne.0)  10425
 
					do jj=1,jlp_nrow
						if(jlp_ixcur(jj).ne.0) then
							jlp_a(jj,newa) = jlp_xmat(jlp_ix(jj)+ibxmatxopt_)- jlp_xmat(jlp_ix(jj)+ibxmatkey_)
						else !if(j_ixcur(jj).ne.0) then
							jlp_a(jj,newa) = 0
						endif !if(jlp_ixcur(jj).ne.0)  10432
					enddo !jj=1,jlp_nrow  10431
					...
					do jj=1,jlp_nrowpf ! #tehdasmjarivit
						...
						do k_=1,jlp_nfxrow(irowj_)
							...
							endif !if (ixkyk_==ixkyk_lkf)  10447
							...
							if(keyf_==jlp_irowffact(jlp_ibafx(irowj_)+k_)) then
								...
								if (jcurix==0) then
									jlp_objr0(newc)=jlp_objr0(newc) + jlp_coeffx(jlp_ibafx(irowj_)+k_)* &
										(j_o(ivxdatmat)%d(ibxdatopt_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)) - & !xmatiba
										j_o(ivxdatmat)%d(ibxdatkey_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)))   !xmatiba
								else !if (jcurix==0) then
									jlp_a(jcurix,newa)=jlp_a(jcurix,newa) + jlp_coeffx(jlp_ibafx(irowj_)+k_)* &
										(j_o(ivxdatmat)%d(ibxdatopt_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)) - & !xmatiba
										j_o(ivxdatmat)%d(ibxdatkey_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)))    !xmatiba
								endif !if (jcurix==0)  10458
							endif !if(keyf_==jlp_irowffact(jlp_ibafx(irowj_)+k_))  10456
 
						enddo !k_=1,jlp_nfxrow(irowj_)  10444
					enddo !jj=1,jlp_nrowpf  10439
					...
					do jj=1,jlp_nrowpfy	!tehdas-yk-mjia sisältävät tehtävärivit
						...
						do k_=1,jlp_nfyrow(irowj_)
							...
							do ivars_=1,j_o(listy)%i(1) ! yk-mjan puutavaralistan muuttujat
								...
								endif !if (iv2xkykpos_==ixkyk_lkf)  10482
								valuetas=j_o(ivxdatmat)%d(ibxdatopt_+iv2elpos_)-j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_)
								do ifact_=1,j_o(listf)%i(1) ! yk-mjan tehdaslistan tehtaat
									iv3factpos_ = jlp_ifyfactfact(jlp_ibafyfact(jlp_ibafy(irowj_)+k_)+ifact_-1) !tehtaan paikka factories-listassa
									! lisätään  avaintehtaan (gamma*(x_ij_k - x_iJ(i)_k))
									if(keyf_.eq.iv3factpos_) then
										...
										endif !if (jcurix==0)  10494
									endif !if(keyf_.eq.iv3factpos_)  10492
 
								enddo !ifact_=1,j_o(listf)%i(1)  10489
							enddo !ivars_=1,j_o(listy)%i(1)  10478
						enddo !k_=1,jlp_nfyrow(irowj_)  10475
					enddo !jj=1,jlp_nrowpfy  10472
 
					if(p)write(n16,*)'**fact**  objr0(newc) päivitetty :', jlp_objr0(newc)
				endif !if(iunit.eq.jlp_lunit(leavkf).and.ienter.eq.3)  10414
...
! first free column is reserved for newc
				...
				if(jlp_lunit(lcur).eq.jlp_lunit(nex))then
					...
					endif !if(ipivot.eq.idebug)  10606
					...
					lcur=if_
					goto 19
				endif !if(jlp_lunit(lcur).eq.jlp_lunit(nex))  10544
			...
			endif !if (leavkf.gt.jlp_mxd)  10359
 
		endif !if (jlp_fpresent)  10001
...
8888 continue !we come here direrectly (through checking keys) if tmax is sufficiently small
		...
		if(leave.eq.0)then
			if(ienter.eq.3)then
						...
						do k=1,jlp_nfxrow(irowj)
							...
							if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj)+k)).eq.jlp_irowffact(jlp_ibafx(irowj)+k))then
								jlp_xps(j)=jlp_xps(j) &
									...
									j_o(ivxdatmat)%d(ibxdatobs+jlp_irowfkeep(jlp_ibafx(irowj)+k))& !xmatiba
									+ jlp_coeffx(jlp_ibafx(irowj)+k)*&
									j_o(ivxdatmat)%d(ibxdatopt+jlp_irowfkeep(jlp_ibafx(irowj)+k))  !xmatiba
							endif !if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj)+k)).e  10690
						enddo !k=1,jlp_nfxrow(irowj)  10688
					enddo !jj=1,jlp_nrowpf  10684
					...
					do jj=1,jlp_nrowpfy ! yk-muuttujat
						...
						do k=1,jlp_nfyrow(irowj)
							...
							do ivars_=1,j_o(listy)%i(1)
								...
								valuetas=j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_)
								valuetas2=j_o(ivxdatmat)%d(ibxdatopt+iv2elpos_)
								do ifact_=1,j_o(listf)%i(1)
									! iv3factpos_ : tehdaslistan list ifact_:innen tehtaan paikka (järjestysnumero) factories-listassa
									iv3factpos_ = jlp_ifyfactfact(jlp_ibafyfact(jlp_ibafy(irowj)+k)+ifact_-1)
									if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_) then
										jlp_xps(j)=jlp_xps(j) &
											...
											valuetas & !j_o(ivxdatmat)%d(xmatiba(iobs)+iv2elpos_)
											+ j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
											valuetas2
										!	j_o(ivxdatmat)%d(xmatiba(iobsopt)+iv2elpos_)
									endif !if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_)  10717
								enddo !ifact_=1,j_o(listf)%i(1)  10714
							enddo !ivars_=1,j_o(listy)%i(1)  10708
						enddo !k=1,jlp_nfyrow(irowj)  10705
					enddo !jj=1,jlp_nrowpfy  10700
 
				endif !if (jlp_fpresent)  10680
				...
				enddo !jj=1,jlp_nrowp  10734
 
				do jj=1,jlp_nrow
					jlp_rhsw(jj)=jlp_rhscur(jj)-jlp_xps(jj)
				enddo !jj=1,jlp_nrow  10740
				...
				if((jlp_lower(newc).and..not.jlp_ubou(newc)).or.(.not.jlp_lower(newc).and..not.jlp_lbou(newc))) then
					...
					j_v(ivunbounded)=1.
					if(jlp_maxo)then
						j_v(ivobjective)=j_inf
					else !if(j_maxo)then
						j_v(ivobjective)=j_ninf
					endif !if(jlp_maxo)  10758
					j_err=.true.
 
					return
				endif !if((jlp_lower(newc).and..not.jlp_ubou(newc)).or.(.not.jlp_  10751
				...
				endif !if((jlp_ix(newc).ne.0).or.jlp_fpresent)  10774
					...
					do k=1,jlp_nfxrow(irowj)
						if((jlp_irowfxvars(jlp_ibafx(irowj)+k)==ixkykenter).and.&
							...
							jlp_xps(j)=jlp_xps(j) &
								- jlp_coeffx(jlp_ibafx(irowj)+k)*&
								j_o(ivxdatmat)%d(ibxdatobs+jlp_irowfkeep(jlp_ibafx(irowj)+k))  !xmatiba
						endif !if((jlp_irowfxvars(jlp_ibafx(irowj)+k)==ixkykenter).a  10802
						if ((jlp_irowfxvars(jlp_ibafx(irowj)+k)==ixkykenter).and.(ifopt.eq.jlp_irowffact(jlp_ibafx(irowj)+k))) then
							jlp_xps(j)=jlp_xps(j) &
								+ jlp_coeffx(jlp_ibafx(irowj)+k)*&
								j_o(ivxdatmat)%d(ibxdatobs+jlp_irowfkeep(jlp_ibafx(irowj)+k))  !xmatiba
						endif !if ((jlp_irowfxvars(jlp_ibafx(irowj)+k)==ixkykenter).and.(  10808
					enddo !k=1,jlp_nfxrow(irowj)  10801
 
					if(j>0) jlp_rhsw(j)=jlp_rhscur(j)-jlp_xps(j)
				enddo !jj=1,jlp_nrowpf  10795
						...
						do ivars_=1,j_o(listy)%i(1)
							...
							do ifact_=1,j_o(listf)%i(1)
								...
								if((iv2xykypos_==ixkykenter).and.(jlp_keyfact(iunit_lkf,iv2xykypos_).eq.iv3factpos_)) then
									jlp_xps(j)=jlp_xps(j) &
										- j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
										j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_)  !xmatiba
								endif !if((iv2xykypos_==ixkykenter).and.(jlp_keyfact(iunit_lkf,iv  10834
								if((iv2xykypos_==ixkykenter).and.(iv3factpos_==ifopt)) then
									jlp_xps(j)=jlp_xps(j) &
										+ j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
										j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_)  !xmatiba
								endif !if((iv2xykypos_==ixkykenter).and.(iv3factpos_==ifopt))  10839
 
							enddo !ifact_=1,j_o(listf)%i(1)  10830
						enddo !ivars_=1,j_o(listy)%i(1)  10825
					enddo !k=1,jlp_nfyrow(irowj)  10822
					if(j.gt.0) jlp_rhsw(j)=jlp_rhscur(j)-jlp_xps(j)
				enddo !jj=1,jlp_nrowpfy  10817
				...
				if(.not.jlp_feasible)then
					do jj=1,lr0
						if(r(jlp_lr(jj)).gt.jlp_tiny78)then
							...
							endif !if(.not.jlp_lbou(jlp_lr(jj)).or..not.jlp_lower(jlp_lr(jj))  10865
							jlp_tmax=jlp_x(jlp_lr(jj))/r(jlp_lr(jj))  ;rcur=r(jlp_lr(jj))
							leavec=jlp_lr(jj)
							leave=jlp_lsi(leavec)
							goto 8888
						endif !if(r(jlp_lr(jj)).gt.jlp_tiny78)  10864
						if(r(jlp_lr(jj)).lt.jlp_tiny78n)then
							if(.not.jlp_ubou(jlp_lr(jj)).or.jlp_lower(jlp_lr(jj))) then
								!virheilmoitus ja häivytään
							endif !if(.not.jlp_ubou(jlp_lr(jj)).or.jlp_lower(jlp_lr(jj)))  10874
							jlp_tmax=jlp_x(jlp_lr(jj))/r(jlp_lr(jj)) ;rcur=r(jlp_lr(jj))
							leavec=jlp_lr(jj)
							leave=jlp_lsi(leavec)
							goto 8888
						endif !if(r(jlp_lr(jj)).lt.jlp_tiny78n)  10873
					enddo !jj=1,lr0  10863
				endif !if(.not.jlp_feasible)  10862
				...
				if(jlp_maxo)then
					...
					j_v(ivobjective)=j_ninf
				endif !if(jlp_maxo)  10891
				j_err = .true.
				return
			endif !if(ienter.eq.3)  10668
			...
			if(ipivot+1.eq.idebug)then
				...
				if(p.and.p8)stop 'debug'
				write(n16,*)'*debug*'
				if(p.and.j_v(j_ivdollar2).eq.8.d0)then
					p=.false.
					p8=.true.
 
				endif !if(p.and.j_v(j_ivdollar2).eq.8.d0)  10914
 
				jdebug=jdebug+1
				if(jdebug.le.ndebug)idebug=j_v(j_o(iob)%i(ibasdebug+jdebug))
 
			endif !if(ipivot+1.eq.idebug)  10903
			if(sparse)then
				if(newc.gt.jlp_nrowz)mqjjsp=-1  ! mqjj=-1   !new not  something known
			else !if(sparse)then
				if(newc.gt.jlp_nrowz)mqjj=-1  ! mqjj=-1   !new not  something known
			endif !if(sparse)  10924
			...
			if(sparse)then
				...
				endif !if(jlp_xpresent.and.abs(rcur).le.1.d-9)  10959
					...
					if(newc.gt.jlp_nrow)then    !jl 20160608
						...
						if(abs(jlp_x(jlp_ls(leave))).lt.jlp_tiny6)then
							...
							if(p9)then
								...
								write(6,*)'<6788',jlp_x(2)
								write(n16,*)(jlp_ls(jjj),jlp_x(jlp_ls(jjj)),jjj=1,jlp_nrow)
							endif !if(p9)  10978
							goto55=.true.;return
						endif !if(abs(jlp_x(jlp_ls(leave))).lt.jlp_tiny6)  10974
						...
						if(abs(jlp_x(jlp_ls(leave))).lt.jlp_tiny6)then
							...
							goto55=.true.;return
						endif !if(abs(jlp_x(jlp_ls(leave))).lt.jlp_tiny6)  10992
 
					endif !if(newc.gt.jlp_nrow)  10967
				endif !if(route67)  10966
				...
				if(tabu)then
					if(fast)then
						...
						endif !if(leavec.gt.jlp_nrowz.and.leavec.le.jlp_nrowz+jlp_mxd)  11022
					endif !if(fast)  11021
				endif !if(tabu)  11020
 
			endif !if(sparse)  10941
			ipivot=ipivot+1
			if(p7)write(77,*)4,ipivot,jlp_objf
			if(p.or.p9.or.pp.or.p8)write(n16,*)'<75>',ipivot,jlp_objf,ienter,iunit,ienterv,nsame,jlp_tmax,&
			...
			if(ifail.ne.0)then
				...
				if(nrecover.ge.20)then
					...
					j_err=.true.
					goto900=.true.;return
				endif !if(nrecover.ge.20)  11048
				goto1234=.true.;return
 
			endif !if(ifail.ne.0)  11039
			if(sparse)then
				if(jlp_ls(leave).gt.jlp_nrowz) mpjjsp=-1
			else !if(sparse)then
				!	if(ls(leave).gt.nrowz) mpjj=-1
			endif !if(sparse)  11056
		...
		if(ienter.eq.1)then
			...
			if(lcursame.gt.0)then  ! d-eaves
				...
				if(jlp_fpresent)then
					do ilu_ = 1,jlp_lunits0
						...
						if (jlp_lunw(ilu_)==lcursame) then
							jlp_lunw(ilu_)=id
						endif !if (jlp_lunw(ilu_)==lcursame)  11108
					enddo !ilu_ = 1,jlp_lunits0  11106
				endif !if(jlp_fpresent)  11105
				...
				if(jlp_fpresent)then
					...
					enddo !while(jlp_lunit(jlp_lunxkf(ilu_)) /= iunit)  11134
					...
					endif !if (ilu_<=jlp_lunits0)  11138
				endif !if(jlp_fpresent)  11127
 
			endif !if(lcursame.gt.0)  11092
			! edelleed d-enters
			jlp_ld0=jlp_ld0+1
			jlp_lunit(id)=iunit
			jlp_isch(id)=iopt
			...
			if(lcursamef.gt.0)then  ! xk-eaves
				...
				do ilu_ = 1,jlp_lunits0
					...
					endif !if (jlp_lunxkf(ilu_)==lcursamef)  11182
				enddo !ilu_ = 1,jlp_lunits0  11181
				...
				do while(jlp_lunit(jlp_lunw(ilu_))/=iunit)
					ilu_=ilu_+1
					if (ilu_>jlp_lunits0) exit
				enddo !while(jlp_lunit(jlp_lunw(ilu_))/=iunit)  11202
				if (ilu_<=jlp_lunits0) then
					jlp_lunxkf(ilu_)=id
				else !if (ilu_<=j_lunits0) then
				...
				endif !if (ilu_<=jlp_lunits0)  11206
 
			endif !if(lcursamef.gt.0)  11168
			! edelleed d-enters
					...
					if (jlp_lunw(ilu_)==leaved) then
						if (jlp_lunit(leaved) == jlp_lunit(jlp_next(leaved))) then
							...
							if (jlp_lunxkf(ilu_)==jlp_mxd) then
								...
								enddo !i_ = ilu_+1,jlp_lunits0  11265
								jlp_lunw(jlp_lunits0) = 0
								jlp_lunxkf(jlp_lunits0) = jlp_mxd
								jlp_lunits0 = jlp_lunits0-1
							else !if (j_lunxkf(ilu_)==j_mxd) then
								jlp_lunw(ilu_)=0
							endif !if (jlp_lunxkf(ilu_)==jlp_mxd)  11264
						endif !if (jlp_lunit(leaved) == jlp_lunit(jlp_next(leaved)))  11261
					endif !if (jlp_lunw(ilu_)==leaved)  11260
				enddo !ilu_ = 1,jlp_lunits0  11259
			endif !if(jlp_fpresent)  11258
			...
			if(jlp_testl)then
				do i=1,jlp_nrow
					if(jlp_ls(i).gt.jlp_nrowz)then
						...
						enddo !j=1,jlp_ld0  11284
						write(n16,*)'*colnot*',jlp_ls(i),'not in',	  (jlp_ld(j),j=1,jlp_ld0)
						close(16)
						write(6,*)'*colnot* return'
						return
		7188 	    continue
					endif !if(jlp_ls(i).gt.jlp_nrowz)  11283
				enddo !i=1,jlp_nrow  11282
			endif !if(jlp_testl)  11281
			! lista uniteista
					...
					if (jlp_lunit(leaved) == jlp_lunit(jlp_nextf(leaved,jlp_ixkf(leaved)))) then
						...
						if (jlp_lunw(ilu_)==0) then
							do i_ = ilu_+1,jlp_lunits0
								jlp_lunw(i_-1) = jlp_lunw(i_)
								jlp_lunxkf(i_-1) = jlp_lunxkf(i_)
							enddo !i_ = ilu_+1,jlp_lunits0  11313
							jlp_lunw(jlp_lunits0) = 0
							jlp_lunxkf(jlp_lunits0) = jlp_mxd
							jlp_lunits0 = jlp_lunits0-1
						else !if (j_lunw(ilu_)==0) then
							jlp_lunxkf(ilu_)=jlp_mxd
						endif !if (jlp_lunw(ilu_)==0)  11312
					endif !if (jlp_lunit(leaved) == jlp_lunit(jlp_nextf(leaved,jlp_ix  11309
				endif !if (jlp_lunxkf(ilu_)==leaved)  11308
			enddo !ilu_ = 1,jlp_lunits0  11307
			if (pp) write(n16,*)"**fact** lunxkf päivitetty <8088>",(jlp_lunw(jj7),jj7=1,jlp_lunits0)
 
		endif !if(leavec.le.jlp_nrow)  11230
	...
	! read(nuxdisk,rec=iobs)j_o(ivxdatmat)%d(1:keepx)
	! ixdisk1=iobs
	! last=1
	! ibass=0
	! else
	! read(nuxdisk,rec=iobs)j_o(ivxdatmat)%d(keepx+1:2*keepx)
	! ixdisk2=iobs
	! ibass=keepx
	! last=2
	! endif
	...
	! iba2=(iobs-1)*keepx
	! do i=1,keepx
	! if(j_o(ivxdatmat)%d(ibass+i).ne.j_o(ivtestmat)%d(iba2+i))then
	! write(6,*)'iobs',iobs,j_o(ivxdatmat)%d(ibass+1:ibass+keepx)
	...
	! return
	! end function
	! integer function xmatiba(iobs,isaa)  !subroutine
	! ! used to get ivxdatmat in jlp function
	...
	! xmatiba=(j_memobs(iobs)-1)*keepx
	! ! if(j_feasible.or.feas)write(17,*)iobs,xmatiba,3,j_xmat(xmatiba+1:xmatiba+j_ntemp0)
	! ! if(j_feasible.or.feas)xmatiba=100000000
	! return
	! endif
	! if(j_memobs(iobs).le.j_xdatlast2.and..false.)then  !from upper buffer note
	! xmatiba=j_j_xmatibas2+(j_memobs(iobs)-j_xmatlopp1)*keepx !note
	! ! if(j_feasible.or.feas)write(17,*)iobs,xmatiba,3,j_xmat(xmatiba+1:xmatiba+j_ntemp0)
	! ! if(j_feasible.or.feas)xmatiba=100000000
	! return
	! endif
	...
	! if(isaa.eq.1)then
	! xmatiba=j_xdatekabas
	! if(j_xdatekaobs.eq.iobs.and..false.)return
	! j_xdatekaobs=iobs
	! else
	! xmatiba=j_xdattokabas
	! if(j_xdattokaobs.eq.iobs.and..false.)return
	! j_xdattokaobs=iobs
	! endif
	! read(j_xdatnu,rec=iobs)j_o(ivxdatmat)%d(xmatiba+1:xmatiba+keepx)
	! ! if(j_feasible.or.feas)write(17,*)iobs,xmatiba,isaa,j_xmat(xmatiba+1:xmatiba+j_ntemp0)
	! ! if(j_feasible
	...
	! j_xdattokaobs=iobs
	! endif
	! read(j_xdatnu,rec=iobs)j_o(ivxdatmat)%d(xdatiba+1:xdatiba+keepx)
	! ! if(j_feasible.or.feas)write(17,*)iobs,xmatiba,isaa,j_xmat(xmatiba+1:xmatiba+j_ntemp0)
	! ! if(j_feasible
	! return
 
 
	! ! 8541 if(all(rivi.eq.j_o(ivxdatmat)%d(xdatiba+1:xdatiba+keepx)))return
	...
	! ! write(16,*)rivi
	! ! write(16,*)j_o(ivxdatmat)%d(xdatiba+1:xdatiba+keepx)
	! ! stop 1257
	...
	subroutine printproblem()
		...
		do i=1,nset
			...
			end if !if(jlp_xpresent.and.jlp_ndom.gt.0)  11545
			!write(nureport,*)' ',('_',kk=1,78)
			!		write(j_buf,66061)
			!66061 format('row',t38,'value',t50,'shadow',t61,  'lower',t70,'upper')
			...
			do k=1,jlp_nsetr(i)
				...
				if(irow0.ne.irowobj)then
					...
					end if !if(jlp_rhs(irow).eq.jlp_rhs2(irow))  11588
					...
					if(jlp_maxo)then
						...
						j_tempchar(1:5)=' min'
					end if !if(jlp_maxo)  11602
					!else !if(j_v(ivfeasible)>0)then
					!	j_buf(1:5)=' '
					!	j_buf(6:33)='Infeasible, temporary object'
					!endif !if(j_v(ivfeasible)>0)then
				end if !if(irow0.ne.irowobj)  11575
			...
			end do !k=1,jlp_nsetr(i)  11564
			!			enddo domloopp !domloopp: do j=1,j_nsetd(i)
		enddo !i=1,nset  11542
	...
	logical function isxkzero(ixkyk)
		...
		ikey_=jlp_ibaunit(iunit) + jlp_keys(iunit)
		ibxdatkey_=xdatiba(ikey_) !,2)
		if(j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_).ne.0.)then
			isxkzero=.false.
			return
		endif !if(j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_).ne.0.)  11633
		do k_=1,jlp_ld0
			if(jlp_lunit(jlp_ld(k_)).ne.iunit) cycle
			...
			if(j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_).ne.0.)then
				isxkzero=.false.
				return
			endif !if(j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_).ne.0.)  11641
		enddo !k_=1,jlp_ld0  11637
			...
			if (jlp_rowxkfkey(jjj)%jcurix == 0) then
				if(jlp_feasible) then
					value_f = value_f + jlp_coeffx(jlp_rowxkfkey(jjj)%irowfx)*&
						j_o(ivxdatmat)%d(ibxdatobs+irowfkeep_)
					if(p)write(n16,*)'value_f, coeff, xkf ',&
						value_f, jlp_coeffx(jlp_rowxkfkey(jjj)%irowfx),j_o(ivxdatmat)%d(ibxdatobs+irowfkeep_)
				endif !if(jlp_feasible)  11663
				...
				!          j_o(ivxdatmat)%d(xdatiba(iobs)+irowfkeep_)
			endif !if (jlp_rowxkfkey(jjj)%jcurix == 0)  11662
		enddo !jjj = 1,nrowxkfkey  11660
		...
		do jj=1,nrowykfkey
			...
			if (jlp_rowykfkey(jj)%jcurix.eq.0) then ! 0-rivi, ei v-kerrointa
				...
				if(p)write(n16,*)'value_f,vc,gamma,ykf',value_f,jlp_vc(jlp_rowykfkey(jj)%jcurix),&
					j_v(jlp_rowykfkey(jj)%ivfout),j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_)
			endif !if (jlp_rowykfkey(jj)%jcurix.eq.0)  11680
		enddo !jj=1,nrowykfkey  11678
	...
	subroutine lcursam()
		...
		do i=1,jlp_ld0
			...
			if(jlp_lunit(lcur).eq.iunit)then
				lcursame=lcur
				return
			end if !if(jlp_lunit(lcur).eq.iunit)  11705
		end do !i=1,jlp_ld0  11703
		return
...
766		nex=jlp_next(nex)
			...
			if(jj.gt.jlp_ld0.or.nex.eq.0)then
				...
				j_err=.true.
				return
			end if !if(jj.gt.jlp_ld0.or.nex.eq.0)  11721
			jj=jj+1
			goto 766
767		continue
		end do !i=1,jlp_ld0  11715
		! end logical testing
		...
		do i=0,jlp_nrow
			...
			if(i.gt.0)then
				if(jlp_ls(i).gt.jlp_nrowz)then
					...
					!        return
	788		  continue
				endif !if(jlp_ls(i).gt.jlp_nrowz)  11746
			end if !if(i.gt.0)  11745
		end do !i=0,jlp_nrow  11743
		...
		do i=1,jlp_nunits
			...
			do j=0,jlp_nrow
				if(jlp_ixcur(j).ne.0)jlp_testxps(j)=jlp_testxps(j)+jlp_xmat(jlp_ix(j)+ibxmatx) !v(ix(j)) !lasketaan keysumma
			end do !j=0,jlp_nrow  11766
		end do !i=1,jlp_nunits  11761
		...
		do i=1,jlp_ld0
			...
			do j=0,jlp_nrow
				...
				if(jlp_ixcur(j).ne.0) jlp_test(j)=&
					jlp_xmat(jlp_ix(j)+ibxmatx ) !v(ix(j))
			end do !j=0,jlp_nrow  11781
			iobs=jlp_ibaunit(jlp_lunit(jlp_ld(i)))+jlp_isch(jlp_ld(i))
			ibxmatx=xmatiba(iobs) !,1)
			do j=0,jlp_nrow
				if(jlp_ixcur(j).ne.0) jlp_test(j)=jlp_xmat(jlp_ix(j)+ibxmatx) -jlp_test(j) !v(ix(j))-test(j)
				jlp_testxps(j)=jlp_testxps(j)+jlp_x(jlp_ld(i)+jlp_nrowz)*jlp_test(j)
			end do !j=0,jlp_nrow  11788
			...
			if(jlp_x(jlp_ld(i)+jlp_nrowz).LT.0.OR.jlp_x(jlp_ld(i)+jlp_nrowz).GT.1.)then
				...
				j_err=.true.
			end if !if(jlp_x(jlp_ld(i)+jlp_nrowz).LT.0.OR.jlp_x(jlp_ld(i)+jlp_  11793
		end do !i=1,jlp_ld0  11773
...
!      22       20      22
		...
		if(jlp_lunit(nex).ne.lunv)then
			...
			if(wss.gt.1.)then
				...
				write(6,*)'*nex* return'
				j_err=.true. ;return
			end if !if(wss.gt.1.)  11822
		end if !if(jlp_lunit(nex).ne.lunv)  11817
...
! testaus lopui
	...
	subroutine testxpssub(iunit_)
		...
		do i_=1,jlp_nunits  !***********************
			...
			ibxmatx=xmatiba(iobs_) !,1)
			ibxdatobs_=xdatiba(iobs_) !,1)
			do jj_=1,jlp_nrowp
				jj=jlp_ixcurrows(jj_)
				!xps = sums over the keyschedules vector s in Eq. (6.14) in JLP-manual
				jlp_testxps(jj)=jlp_testxps(jj)+jlp_xmat(jlp_ix(jj)+ibxmatx) !(ix(j)=0 -> no x in row
				!                if(ixcur(j).ne.0)xps(j)=xps(j)+xmat(ibxdatkey+ix(j))
			enddo !jj_=1,jlp_nrowp  11846
					...
					do k_=1,jlp_nfxrow(irowj_)
						if(jlp_keyfact(i_,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)).eq.jlp_irowffact(jlp_ibafx(irowj_)+k_))then
							jlp_testxps(jjj)=jlp_testxps(jjj)+ jlp_coeffx(jlp_ibafx(irowj_)+k_)*&
								j_o(ivxdatmat)%d(ibxdatobs_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_))
						endif !if(jlp_keyfact(i_,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)).eq  11864
					enddo !k_=1,jlp_nfxrow(irowj_)  11863
				enddo !jj_=1,jlp_nrowpf  11859
 
				if (nfy.gt.0) then
					do jjj=1,j_o(jlp_ivkeepc)%i(1)
						j_v(j_o(jlp_ivkeepc)%i2(jjj))=j_o(ivcmat)%d((i_-1)*j_o(jlp_ivkeepc)%i(1)+jjj)
					enddo !jjj=1,j_o(jlp_ivkeepc)%i(1)  11872
					do jjj=1,ntrans
						call dotrans(jlp_itransv(jjj),1)
						if(j_err)then
							write(6,*)'err for trans ',j
							stop 741
						endif !if(j_err)  11877
					enddo !jjj=1,ntrans  11875
				endif !if (nfy.gt.0)  11871
							...
							do ifact_=1,j_o(listf_)%i(1)
								...
								if(jlp_keyfact(i_,iv2xykypos_).eq.iv3factpos_) then
									jlp_testxps(jjj)=jlp_testxps(jjj)+ j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
										j_o(ivxdatmat)%d(ibxdatobs_+iv2elpos_) !xdatiba
								endif !if(jlp_keyfact(i_,iv2xykypos_).eq.iv3factpos_)  11903
 
							enddo !ifact_=1,j_o(listf_)%i(1)  11900
						enddo !ivars_=1,j_o(listy_)%i(1)  11894
 
					enddo !k_=1,jlp_nfyrow(irowj_)  11890
				enddo !jj_=1,jlp_nrowpfy  11885
 
			endif !if(jlp_fpresent)  11854
		enddo !i_=1,jlp_nunits  11841
		...
		if(iunit_>0) then
			...
			if(jlp_fpresent) then
				...
				if (nfy.gt.0) then
					...
					enddo !jjj=1,j_o(jlp_ivkeepc)%i(1)  11925
					do jjj=1,ntrans
						call dotrans(jlp_itransv(jjj),1)
						if(j_err)then
							write(6,*)'error for trans ',j
							stop 741
						endif !if(j_err)  11930
					enddo !jjj=1,ntrans  11928
				endif !if (nfy.gt.0)  11924
			endif !if(jlp_fpresent)  11921
		endif !if(iunit_>0)  11918
		...
		do iii=1,jlp_nrow
			...
			endif !if(abs(jlp_testxps(iii)-jlp_xps(iii)).gt.0.001)  11942
		enddo !iii=1,jlp_nrow  11941
		...
		do ldj_=1,jlp_ld0
			...
			do it_=0,jlp_nrow
				if(jlp_ixcur(it_).ne.0) then
					jlp_test(it_) = jlp_xmat(jlp_ix(it_)+ibxmatx)- jlp_xmat(jlp_ix(it_)+ibxmatkey_)
				endif !if(jlp_ixcur(it_).ne.0)  11969
			enddo !it_=0,jlp_nrow  11968
					...
					do k_=1,jlp_nfxrow(irowj_) ! silmukka : rivin xk-muuttujat
						...
						if(jlp_keyfact(iunit_,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)).eq.jlp_irowffact(jlp_ibafx(irowj_)+k_)) then
							jlp_test(jcurix_) = jlp_test(jcurix_) + jlp_coeffx(jlp_ibafx(irowj_)+k_)*&
								(j_o(ivxdatmat)%d(ibxdatobs_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)) -&
								j_o(ivxdatmat)%d(ibxdatkey_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)))
						endif !if(jlp_keyfact(iunit_,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)  11981
					enddo !k_=1,jlp_nfxrow(irowj_)  11979
				enddo !jjj=1,jlp_nrowpf  11976
 
				!y-mjien muunnokset
				if (nfy.gt.0) then
					do jjj=1,j_o(jlp_ivkeepc)%i(1)
						j_v(j_o(jlp_ivkeepc)%i2(jjj))=j_o(ivcmat)%d((iunit_-1)*j_o(jlp_ivkeepc)%i(1)+jjj)
					enddo !jjj=1,j_o(jlp_ivkeepc)%i(1)  11991
					do jjj=1,ntrans
						call dotrans(jlp_itransv(jjj),1)
						if(j_err)then
							write(6,*)' err in tr ',j
							stop 614
						endif !if(j_err)  11996
					enddo !jjj=1,ntrans  11994
				endif !if (nfy.gt.0)  11990
							...
							do ifact_=1,j_o(listf_)%i(1) ! yk-mjan tehdaslistan tehtaat
								...
								if(jlp_keyfact(iunit_,iv2xykypos_).eq.iv3factpos_) then
									jlp_test(jcurix_) = jlp_test(jcurix_) +j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
										(j_o(ivxdatmat)%d(ibxdatobs_+iv2elpos_) -&
										j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_))
								endif !if(jlp_keyfact(iunit_,iv2xykypos_).eq.iv3factpos_)  12015
							enddo !ifact_=1,j_o(listf_)%i(1)  12012
						enddo !ivars_=1,j_o(listy_)%i(1)  12009
					enddo !k_=1,jlp_nfyrow(irowj_)  12006
				enddo !jjj=1,jlp_nrowpfy  12003
 
			endif !if (jlp_fpresent)  11974
 
			write(n16,*)'ld(j)',jlp_ld(ldj_),'test(0:nrow)',(jlp_test(jjj),jjj=0,jlp_nrow)
 
		enddo !ldj_=1,jlp_ld0  11955
			...
			do jjj=1,jlp_nrowpf ! rivit, joilla xk-muuttujia
				...
				do k_=1,jlp_nfxrow(irowj_) ! silmukka : rivin xk-muuttujat
					if((jlp_irowfxvars(jlp_ibafx(irowj_)+k_)==ixkyk_).and.(ifact_.eq.jlp_irowffact(jlp_ibafx(irowj_)+k_))) then
						jlp_test(jcurix_)=jlp_test(jcurix_) + jlp_coeffx(jlp_ibafx(irowj_)+k_)
					endif !if((jlp_irowfxvars(jlp_ibafx(irowj_)+k_)==ixkyk_).and.(ifa  12052
					if((jlp_irowfxvars(jlp_ibafx(irowj_)+k_)==ixkyk_).and.(ifact_.eq.jlp_keyfact(iunit_,ixkyk_))) then
						jlp_test(jcurix_)=jlp_test(jcurix_) - jlp_coeffx(jlp_ibafx(irowj_)+k_)
					endif !if((jlp_irowfxvars(jlp_ibafx(irowj_)+k_)==ixkyk_).and.(ifa  12055
				enddo !k_=1,jlp_nfxrow(irowj_)  12051
			enddo !jjj=1,jlp_nrowpf  12048
 
			if (nfy.gt.0) then
				do jjj=1,j_o(jlp_ivkeepc)%i(1)
					j_v(j_o(jlp_ivkeepc)%i2(jjj))=j_o(ivcmat)%d((iunit_-1)*j_o(jlp_ivkeepc)%i(1)+jjj)
				enddo !jjj=1,j_o(jlp_ivkeepc)%i(1)  12062
				do jjj=1,ntrans
					call dotrans(jlp_itransv(jjj),1)
					if(j_err)then
						write(6,*)'err in tr ',j
					...
					endif !if(j_err)  12067
				enddo !jjj=1,ntrans  12065
			endif !if (nfy.gt.0)  12061
					...
					do ivars_=1,j_o(listy_)%i(1)
						...
						do ifact_=1,j_o(listf_)%i(1)
							...
							if((iv2xykypos_==ixkyk_).and.(ifact_.eq.iv3factpos_)) then
								jlp_test(jcurix_)=jlp_test(jcurix_) + j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))
							endif !if((iv2xykypos_==ixkyk_).and.(ifact_.eq.iv3factpos_))  12086
							if((iv2xykypos_==ixkyk_).and.(ifact_.eq.jlp_keyfact(iunit_,ixkyk_))) then
								jlp_test(jcurix_)=jlp_test(jcurix_) - j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))
							endif !if((iv2xykypos_==ixkyk_).and.(ifact_.eq.jlp_keyfact(iunit_  12089
						enddo !ifact_=1,j_o(listf_)%i(1)  12084
					enddo !ivars_=1,j_o(listy_)%i(1)  12082
 
				enddo !k_=1,jlp_nfyrow(irowj_)  12078
			enddo !jjj=1,jlp_nrowpfy  12075
 
			write(n16,*)'lf(j)',jlp_lf(lfj_),'test(0:nrow)',(jlp_test(jjj),jjj=0,jlp_nrow)
 
		enddo !lfj_=1,jlp_lf0  12037
	...
	subroutine getsolx() !
		...
		if(nz.eq.0)then
			...
			do i=1,jlp_nunits  !***********************
				...
				! ixcurrows= for each row in this domain tells the original (expanded) row
				!  ixcur for each original expanded row tells the index of temporary x-variable
				ibxmatx=xmatiba(jlp_ibaunit(i)+integerschedw(i))  !,1)
				do jj=1,jlp_nrowp
					j=jlp_ixcurrows(jj)
					jlp_solx(j)=jlp_solx(j)+jlp_xmat(jlp_ix(j)+ibxmatx)
				enddo !jj=1,jlp_nrowp  12119
			enddo !i=1,jlp_nunits  12112
		endif !if(nz.eq.0)  12109
		...
		if(jlp_ivsubtrans>0) then
			...
			noutsubtrans = 0
		endif !if(jlp_ivsubtrans>0)  12127
		...
		if(ivarea.gt.0.and.notareavars.gt.0)then
			...
			if(nxvarareatot.lt.jlp_nxvartot.or.notc.gt.0)then
				...
				ipart=1
			endif !if(nxvarareatot.lt.jlp_nxvartot.or.notc.gt.0)  12146
		...
		endif !if(ivarea.gt.0.and.notareavars.gt.0)  12142
		jlp_sumx=j_0
		do iuni=1,jlp_nunits
			...
			endif !if(j_err)  12162
			if(jlp_ivtrans.gt.0)then
				call dotrans(jlp_ivtrans,1)
				if(j_err)stop 770
			endif !if(jlp_ivtrans.gt.0)  12165
			...
			if(ivarea.gt.0)then
				...
				endif !if(ipart.eq.0)  12177
			endif !if(ivarea.gt.0)  12176
			if(jlp_ndom.gt.0)then
				iba=0
				idomba=(iuni-1)*jlp_ndomv
				do id=1,jlp_ndom
					icurint=(id-1)/32+1;icurbit=id-(icurint-1)*32-1
					if(btest(jlp_domainbits(idomba+icurint),icurbit))then
						!         ndomsol(id)=ndomsol(id)+1
 
						do jx=1,jlp_nxvartot
							jlp_sumx(iba+jx)=jlp_sumx(iba+jx)+ j_v(j_o(jlp_ivkeepx)%i2(jx))
						end do !jx=1,jlp_nxvartot  12193
					...
					end if !if(btest(jlp_domainbits(idomba+icurint),icurbit))  12190
					iba=iba+jlp_nxvartot+ncvar+noutsubtrans
				end do !id=1,jlp_ndom  12188
				...
				if(jlp_ivsubtrans>0) jlp_sumx(jlp_nxvartot+ncvar+1:jlp_nxvartot+ncvar+noutsubtrans) = &
					...
					j_v(j_o(ivoutsubtrans)%i2(1:noutsubtrans))
			end if !if(jlp_ndom.gt.0)  12185
		end do !iuni=1,jlp_nunits  12160
		...
		do iuni=1,jlp_nunits
			...
			if(j_err)then
				write(6,*)'error for unit ',iuni
				stop 512
			endif !if(j_err)  12222
			if(jlp_ivtrans.gt.0)then
				call dotrans(jlp_ivtrans,1)
				if(j_err)stop 651
 
			endif !if(jlp_ivtrans.gt.0)  12226
			iobs=jlp_ibaunit(iuni)+integerschedw(iuni)   !+j_ibaunitbas
			call j_getobsiv(iobs,jlp_ivmatx,jlp_ivkeepx,0)! ,jlp_ivtransx,0)  !getsolx
			if(j_err)then
				write(6,*)'error for observation ',iobs
			endif !if(j_err)  12233
			if(jlp_ivsubtrans.gt.0)then
				call dotrans(jlp_ivsubtrans,1)
				if(j_err)stop 719
			endif !if(jlp_ivsubtrans.gt.0)  12236
			if(ivarea.gt.0)then
				if(ipart.eq.0)then
					...
					j_v(jlp_xvarlareatot)=j_v(ivarea)*j_v(jlp_xvarlareatot)
				endif !if(ipart.eq.0)  12241
			endif !if(ivarea.gt.0)  12240
			...
			if(jlp_ndom.gt.0)then
				...
				do id=1,jlp_ndom
					...
					if(btest(jlp_domainbits(idomba+icurint),icurbit))then
						...
						end do !jx=1,jlp_nxvartot  12258
					...
					end if !if(btest(jlp_domainbits(idomba+icurint),icurbit))  12255
 
					iba=iba+jlp_nxvartot+ncvar+noutsubtrans
				end do !id=1,jlp_ndom  12253
			else !if(j_ndom.gt.0)then
			...
			end if !if(jlp_ndom.gt.0)  12250
		end do !iuni=1,jlp_nunits  12220
		...
		do i=1,jlp_ld0
			...
			if(ivarea.gt.0)then
				if(ipart.eq.0)then
					...
					j_v(jlp_xvarlareatot)=j_v(ivarea)*j_v(jlp_xvarlareatot)
				endif !if(ipart.eq.0)  12296
			endif !if(ivarea.gt.0)  12295
			if(jlp_ndom.gt.0)then
				iba=0
				idomba=(iuni-1)*jlp_ndomv
				do id=1,jlp_ndom
			...
			if(ivarea.gt.0)then
				if(ipart.eq.0)then  !if(nxvarareatot.eq.nxvartot)then
					...
					j_v(jlp_xvarlareatot)=j_v(ivarea)*j_v(jlp_xvarlareatot)
				endif !if(ipart.eq.0)  12336
			endif !if(ivarea.gt.0)  12335
			...
			if(jlp_ndom.gt.0)then
				...
				do id=1,jlp_ndom
					...
					if(btest(jlp_domainbits(idomba+icurint),icurbit))then
						do jx=1,jlp_nxvartot
							jlp_sumx(iba+jx)=jlp_sumx(iba+jx)+wei* j_v(j_o(jlp_ivkeepx)%i2(jx))
						end do !jx=1,jlp_nxvartot  12349
 
						do jx=1,noutsubtrans
							jlp_sumx(iba+jlp_nxvartot+ncvar+jx)=jlp_sumx(iba+jlp_nxvartot+ncvar+jx) &
								+ wei*j_v(j_o(ivoutsubtrans)%i2(jx))
						enddo !jx=1,noutsubtrans  12353
					end if !if(btest(jlp_domainbits(idomba+icurint),icurbit))  12348
 
					iba=iba+jlp_nxvartot+noutsubtrans
				end do !id=1,jlp_ndom  12346
			else !if(j_ndom.gt.0)then
				do jx=1,jlp_nxvartot
					jlp_sumx(jx)=jlp_sumx(jx)+wei* j_v(j_o(jlp_ivkeepx)%i2(jx))
				end do !jx=1,jlp_nxvartot  12362
 
				do jx=1,noutsubtrans
					jlp_sumx(jlp_nxvartot+ncvar+jx)=jlp_sumx(jlp_nxvartot+ncvar+jx) &
						+ wei*j_v(j_o(ivoutsubtrans)%i2(jx))
				enddo !jx=1,noutsubtrans  12366
 
			end if !if(jlp_ndom.gt.0)  12343
		end do !i=1,jlp_ld0  12284
	...
	subroutine defsolu()
		...
		do i=1,jlp_ld0
			...
			if(jlp_lunit(id).ne.nunv)then
				...
				sum=0.
				iplace=0
			end if !if(jlp_lunit(id).ne.nunv)  12412
			!
			if(jlp_isch(id).gt.jlp_keys(jlp_lunit(id)).and.iplace.eq.0)then   !reserve place for
				idiv=idiv+1
				iplace=idiv
			end if !if(jlp_isch(id).gt.jlp_keys(jlp_lunit(id)).and.iplace.eq.0  12426
			idiv=idiv+1
			...
			nunv=jlp_lunit(id)
		end do !i=1,jlp_ld0  12408
 
		if(iplace.eq.0)then   ! key is largest
			idiv=idiv+1;iplace=idiv
		end if !if(iplace.eq.0)  12439
		...
		enddo !i=1,jlp_ndiv  12451
 
		do i=1,jlp_ndiv
			jlp_isdiv(i)=isdiv0(iperm(i))
		end do !i=1,jlp_ndiv  12455
		isdiv0=jlp_iunitdiv
		do i=1,jlp_ndiv
			jlp_iunitdiv(i)=isdiv0(iperm(i))
		end do !i=1,jlp_ndiv  12459
	...
	subroutine defsoluf()
		...
		enddo !i_= jlp_i0_xkfsol,jlp_lf0_xkfsol  12483
	...
	subroutine printxkf()
		...
		do iuni=1,jlp_nunits	! laskentayksiköt
			...
			enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  12521
		enddo !iuni=1,jlp_nunits  12512
		...
		do i=1,jlp_ld0	!kannan ve-sarakkeet
			...
			enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  12540
			...
			do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
				keyf_ = jlp_keyfact(iuni,ixk_)
				sumxkf(ixk_,keyf_) = sumxkf(ixk_,keyf_) + wei*j_v(j_o(jlp_ivxkyk)%i2(ixk_))
			enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  12550
		enddo !i=1,jlp_ld0  12528
		...
		enddo !i= jlp_mxd+1,jlp_lf0  12556
		...
		do if_= 1,j_o(jlp_ivfact)%i(1)
			do  ixk_= 1,j_o(jlp_ivxkyk)%i(1)
				do ixkf_=1,jlp_nxkfact(ixk_)
					...
					endif !if (if_ == jlp_xkfact(ixk_,ixkf_)%ifact)  12569
				enddo !ixkf_=1,jlp_nxkfact(ixk_)  12568
			enddo ! ixk_= 1,j_o(jlp_ivxkyk)%i(1)  12567
		enddo !if_= 1,j_o(jlp_ivfact)%i(1)  12566
		...
		if(p.and.any(sumxkf<0.0)) then
			...
			do iuni=1,jlp_nunits	! laskentayksiköt
				...
				do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
					...
					endif !if(sumxkf(ixk_,keyf_)<0.0)  12598
				enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  12594
			enddo !iuni=1,jlp_nunits  12585
			...
			do i=1,jlp_ld0	!kannan ve-sarakkeet
				...
				do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
					...
					if(sumxkf(ixk_,keyf_)<0.0) then
					endif !if(sumxkf(ixk_,keyf_)<0.0)  12624
				enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  12620
				...
				do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
					...
					if(sumxkf(ixk_,keyf_)<0.0) then
						write(n16,*)'Avainve -: unit, ve, paino, ixk,ifact, -, yht : ' ,&
							...
							wei*j_v(j_o(jlp_ivxkyk)%i2(ixk_)),&
							sumxkf2(ixk_,keyf_)
					endif !if(sumxkf(ixk_,keyf_)<0.0)  12638
				enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  12634
 
			enddo !i=1,jlp_ld0  12609
			...
			do i= jlp_mxd+1,jlp_lf0	! kannan tehdasmuuttujat
				...
				endif !if (sumxkf(ixk_,ifact_)<0.0)  12657
				if(sumxkf(ixk_,keyf_)<0.0) then
				...
				endif !if(sumxkf(ixk_,keyf_)<0.0)  12663
			enddo !i= jlp_mxd+1,jlp_lf0  12648
 
			deallocate(sumxkf2)
		endif !if(p.and.any(sumxkf<0.0))  12577
	...
	subroutine printsumxkyk(idom_)
		...
		do ixk_ = 1,j_o(jlp_ivxkyk)%i(1)
			...
			call j_getname(j_o(jlp_ivxkyk)%i2(ixk_))
			write(n16,*) j_oname(1:j_loname),jlp_sumx((idom_-1)*jlp_nxvartot+jx)
		enddo !ixk_ = 1,j_o(jlp_ivxkyk)%i(1)  12684
		return
...
subroutine jlp2(iob,io)   ! %%jlp  !!!!****************************************************
	...
	integer irowobj,irow,irow0,irow0bas,nset
	integer ncvar,noutsubtrans,irowinf
	integer jcurix_,newf,lf01
	integer ivoutsubtrans
	integer ipivot9,iunitprev
 
	integer ::itran,itemp,ivunit,maxnsch,ivcdata,ncvararea,needc
	integer ::ivarea,notareavars,nxvararea,nxvar,ntemp,itemp0,idomba,idombas
	integer ::md
	integer::ivfeasible,ivunbounded,ivobjective,ivoptimal
	integer::ivpivots,ivstartedjlp
	integer ::ilres,nresw,npivotw,nkeyfact
...
7777 format(a16,i5)
		...
		if(jlp_nrow.le.1.and.sparse)then
			write(6,*)'*note: with ', jlp_nrow, 'rows option sparse-> ignored'
			sparse=.false.
		endif !if(jlp_nrow.le.1.and.sparse)  13284
		!nsetr number of rows in each domainset
		!nsetd=nubber of domains in each domainset
		!isetd list of all domain statements (each occurence is counted separately)
		!nvars number of vars in each row
			...
			do k=1,jlp_nsetr(i) ! number of rows in set i
				...
				if(irow0.ne.irowobj)then
					...
					if(j_o(ivrhs2)%d(irow0).eq.0.)then
						...
						jlp_maxo=.false.
						coefmax=-j_1
					end if !if(j_o(ivrhs2)%d(irow0).eq.0.)  13361
				end if !if(irow0.ne.irowobj)  13344
			end do !k=1,jlp_nsetr(i)  13342
			!			end do !do j=1,j_nsetd(i)
		end do !i=1,nset  13338
		jlp_lbou=jlp_rhs.gt.j_ninf   ! is there lower bound
		jlp_ubou=jlp_rhs2.lt.j_inf   ! is there upper bound
	endif !if (.not.zmatrix)  13326
	!write(6,*)'<768'
	!!!! intilization continues
	...
	if(jlp_xpresent)then   !xpresent means that data-> , even if all variables  !!!!
			...
			if(iv2.ne.0)then	! tehdasmuuttuja
				if(.not.jlp_fpresent) then
					...
					if(ivoutresult.eq.j_ivresult) then
						...
						j_err=.true.
						goto 930
					endif !if(ivoutresult.eq.j_ivresult)  13449
					call j_deflistobject(ivoutresult,'%xkyk',jlp_ivxkyk,nres=40)
					call j_deflistobject(ivoutresult,'%factories',jlp_ivfact,nres=40)
					...
					if(jlp_nrow.eq.0)then
						...
						j_err=.true.
						return
					endif !if(jlp_nrow.eq.0)  13462
 
				endif !if(.not.jlp_fpresent)  13447
				...
				if(itrans.eq.0) then ! xk
					...
					do ij=1,j_o(iv2)%i(1)
						...
						if(ijo.le.0)then
							...
							j_err=.true.
							return
						endif !if(ijo.le.0)  13494
						iperk=j_putlistobject(jlp_ivxkyk,single=j_o(iv2)%i2(ij)) ! ,jlp_ivxkyk)
					enddo !ij=1,j_o(iv2)%i(1)  13492
					! do ij=1,j_o(iv3)%i(0)
					! if(j_inlistobject(j_o(iv3)%i(ij),jlp_ivfact).le.0) then
					! iperk= j_putlist2(j_o(iv3)%i(ij),jlp_ivfact)
					! endif !if(j_inlistobject(j_o(iv3)%i(ij),j_ivfact).le.0) then
				...
				endif !if(itrans.eq.0)  13471
			endif !if(iv2.ne.0)  13446
		enddo !i0=1,nvartot  13436
		...
		if(jlp_fpresent) then   !!!! initilization for factories
			...
			if(allocated(jlp_xkfact))then
				if(warmf.and.size(jlp_xkfact,dim=2).ne. j_o(jlp_ivfact)%i(1))then
					write(6,*)'*number of factories is different, warm-> ignored for factories'
					warmf=.false.
				endif !if(warmf.and.size(jlp_xkfact,dim=2).ne. j_o(jlp_ivfact)%i(  13553
				deallocate(jlp_xkfact)
			else !if(allocated(j_xkfact))then
				warmf=.false.  !factories not allocated
			endif !if(allocated(jlp_xkfact))  13552
			...
			do i_=1,j_o(jlp_ivxkyk)%i(1)
				jlp_ixkykkeep(i_)=j_inlistobject(j_o(jlp_ivxkyk)%i2(i_),jlp_ivkeepx)
			enddo !i_=1,j_o(jlp_ivxkyk)%i(1)  13588
 
		endif !if(jlp_fpresent)  13520
		...
		do i0=1,nvartot       !***********************************
			...
			if(ikeep.le.0.and.iout.le.0)then  !is variable in cdata
				...
				if(ikeep.gt.0.or.iout.gt.0)then
					...
					! note that cvarl is not 'standard' list where element 0 tells the number of elements in the list
					jlp_cvarl(ncvar)=i
				endif !if(ikeep.gt.0.or.iout.gt.0)  13612
			endif !if(ikeep.le.0.and.iout.le.0)  13609
			!!!??? what happens when variable is both x and c variable???
			!onko x-muuttuja
			nxval=0   !added by JL 23.1.2021
				...
				do j=1,irowp  !row definitions
					do k=1,jlp_nvars(j) !nvars number of vars in each row
						...
						if(jlp_irowvars(ival).eq.i)then
							...
							nxval=nxval+1
							jlp_nxrow(j)=jlp_nxrow(j)+1      !number of x-variables in row j
						endif !if(jlp_irowvars(ival).eq.i)  13631
					enddo !k=1,jlp_nvars(j)  13628
 
				enddo !j=1,irowp  13627
				!factory
				!tarkistetaan onko vuorossa oleva muuttuja tehdasmuuttuja	(vai z-muuttuja)
			else !if(ikeep.gt.0.or.iout.gt.0)then
					...
					if(itrans.ne.0)then !****!itrans= object trans%util%%sawlog%%sawmill
						...
						do ii=1,j_o(iv2)%i(1)
							...
							toka:	   			do jj=1,j_o(iv3)%i(1)
								...
								do inx=1,jlp_nxkfact(lxk)
									if(jlp_xkfact(lxk,inx)%ifact.eq.lff) goto 728
								enddo !inx=1,jlp_nxkfact(lxk)  13696
								! tehdasta ei löytynyt taulukosta
								! kasvatetaan ko muuttujan tehtaiden lkm ja lisätään tehdas taulukkoon
								jlp_nxkfact(lxk) = jlp_nxkfact(lxk)+1
								jlp_xkfact(lxk,jlp_nxkfact(lxk))%ifact=lff
								...
								nameloop:  			do ivn=1,j_o(j_ivnames)%i(0)
									...
									do iel = 1,lentrim
										if(j_o(j_ivnames)%ch( ibas1+iel).ne. &
											& j_o(j_ivnames)%ch( ibas2+iel))cycle nameloop
									enddo !iel = 1,lentrim  13722
									ibas1=ibas1+lentrim+2
									ibas2 = j_o(j_ivnames)%i(iv3el)-1
									do iel = 1,len3
										if(j_o(j_ivnames)%ch( ibas1+iel).ne. &
											& j_o(j_ivnames)%ch( ibas2+iel))cycle nameloop
									enddo !iel = 1,len3  13728
									!Testataan löytyykö itrans-muunnoksen output-muuttujista
									! ivn otettava talteen (xps:n laskentaa varten)
									ivtemp=j_inlistobject(ivn,j_o(itrans)%i2(2))
									if(ivtemp.gt.0)then
										...
										j_err=.true.
										return
									endif !if(ivtemp.gt.0)  13735
									!Jos vastaavutta ei löydy, pitäisi palautua virheilmoitus
								enddo nameloop !eloop:  			do ivn=1,j_o(j_ivnames)%i(0)  13709
								call j_printname('transformation ',itrans,' :')
								call j_getname(iv2el,iv3el)
								write(6,*)'does not contain outputvariable ',j_oname(1:j_loname)//'%%'//j_oname2(1:j_loname2)
								!	call j_printname('not found outputvariable for ',iv2el,' ')
								...
								2578						enddo toka !a:	   			do jj=1,j_o(iv3)%i(1)  13688
						enddo !ii=1,j_o(iv2)%i(1)  13675
								...
								if(jlp_irowvars(ival).eq.i)then
										...
										do ii=1,j_o(iv2)%i(1)
											...
											if(iv2elpos.le.0) then
												...
												do ij=1,j_o(jlp_ivkeepx)%i(1)
													write(6,*)j_o(iv2)%i2(ii),j_o(jlp_ivkeepx)%i2(ij),&
														j_o(iv2)%i2(ii).eq.j_o(jlp_ivkeepx)%i2(ij)
												enddo !ij=1,j_o(jlp_ivkeepx)%i(1)  13782
												j_err=.true.
												return
											endif !if(iv2elpos.le.0)  13780
											ifykeep = ifykeep+1
											call j_puti(jlp_ifyvarskeep,ifykeep,iv2elpos)
											!ptl-muuttujan paikka xkyk-listassa talteen
											call j_puti(jlp_ifyvarsxkyk,ifykeep,j_inlistobject(j_o(iv2)%i2(ii),jlp_ivxkyk))
										enddo !ii=1,j_o(iv2)%i(1)  13778
									else !if(iiv2_.eq.0) then
										jlp_ibafykeep(ival) = jlp_ibafykeep(iiv2_)
									endif !if(iiv2_.eq.0)  13775
									...
									if(iiv3_.eq.0) then ! iv3-tehdaslistaa ei ole vielä käsitelty -> etsitään paikat factories-listasta
										...
										do ii=1,j_o(iv3)%i(1)
											...
											! Ei tarvinne tarkistaa löytyykö factories-listasta, koska lisäys tehdään heti tehdas-y-mjan käsittelyn aluksi
											call j_puti(jlp_ifyfactfact,ifyfact,j_inlistobject(j_o(iv3)%i2(ii),jlp_ivfact))
										enddo !ii=1,j_o(iv3)%i(1)  13803
									else !if(iiv3_.eq.0) then
										jlp_ibafyfact(ival) = jlp_ibafyfact(iiv3_)
									endif !if(iiv3_.eq.0)  13801
 
									if(jlp_coef(ival).ne.1.) then
										write(6,*)'Betas must be 1'
										j_err=.true.
										return
									endif !if(jlp_coef(ival).ne.1.)  13812
								endif !if(jlp_irowvars(ival).eq.i)  13766
							enddo !k=1,jlp_nvars(j)  13763
						enddo !j=1,irowp  13762
						...
						do inx=1,jlp_nxkfact(lxk)
							if(jlp_xkfact(lxk,inx)%ifact.eq.lff) goto 729
						enddo !inx=1,jlp_nxkfact(lxk)  13824
						! tehdasta ei löytynyt taulukosta
						! kasvatetaan ko muuttujan tehtaiden lkm ja lisätään tehdas taulukkoon
						jlp_nxkfact(lxk) = jlp_nxkfact(lxk)+1
						jlp_xkfact(lxk,jlp_nxkfact(lxk))%ifact=lff
						...
						do j=1,irowp  !row definitions
							do k=1,jlp_nvars(j) !nvars number of vars in each row
								...
								if(jlp_irowvars(ival).eq.i)then
									...
									jlp_irowfkeep(ival)=j_inlistobject(iv2, jlp_ivkeepx)
									jlp_nfxrow(j)=jlp_nfxrow(j)+1      !number of x-variables in row j
								endif !if(jlp_irowvars(ival).eq.i)  13839
							enddo !k=1,jlp_nvars(j)  13836
						enddo !j=1,irowp  13835
 
					endif !if(itrans.ne.0)  13663
				endif !if(iv2.ne.0)  13657
 
			endif !if(ikeep.gt.0.or.iout.gt.0)  13623
		enddo !i0=1,nvartot  13596
 
		if(nxvar+nfx+nfy.le.0)then
			write(6,*)'**all variables are z variables even if data->';j_err=.true.;goto 940
		end if !if(nxvar+nfx+nfy.le.0)  13855
		...
		if(jlp_fpresent.and.(nfy == 0)) then
			...
			j_err = .true.
			return
		endif !if(jlp_fpresent.and.(nfy == 0))  13872
	...
	if(nz.gt.0)then  !nz=number of z-variables
		...
		if(.not.zmatrix) then
					...
					if(irow0.ne.irowobj)then !.or.j.gt.1)then ! k.gt.1)then
						...
						do ii=1,jlp_nvars(irow0)
							...
							if(.not.jlp_isxval(ival))then        !is the variable not x-variable
								...
								! put coefficients of z variables into A matrix
								jlp_a(irow,iz)=jlp_coef(ival)
							endif !if(.not.jlp_isxval(ival))  14004
						enddo !ii=1,jlp_nvars(irow0)  14002
					else if(irow0.eq.irowobj)then !.and.j.eq.1)then !if(irow0.ne.irowobj.or.j.gt.1)then
						! the objective row, pick coefficients into objr0
						! if the objective row is in a domain set and there are several domain
						! then the objective function can refer only to the first domain
						do ii=1,jlp_nvars(irow0)
							...
							if(.not.jlp_isxval(ival))then
								...
								if(jlp_maxo)then  ! objective maximized
									...
									jlp_objr0(jlp_nrow+iz)=-jlp_coef(ival) ! -objective maximized
								endif !if(jlp_maxo)  14019
							endif !if(.not.jlp_isxval(ival))  14017
						enddo !ii=1,jlp_nvars(irow0)  14015
					endif !if(irow0.ne.irowobj)  13999
				enddo !k=1,jlp_nsetr(i)  13997
				!				enddo !do j=1,j_nsetd(i)
				idomba=ival
			enddo !i=1,nset  13994
		else !if(.not.zmatrix) then
			...
			do i=1,jlp_nrow
				do j=1,nz
					...
					jlp_a(i,j)=j_o(ivzmatrix)%d(iel)
				enddo !j=1,nz  14036
			enddo !i=1,jlp_nrow  14035
		endif !if(.not.zmatrix)  13968
		...
		! jlp_a(ie,i)=jlp_a(j,i)
		! jlp_lavecsp(istart1+ie)=j
		! endif !if(jlp_a(j,i).ne.jlp_zero)  14051
		! enddo !j=1,jlp_nrow  14050
		! last(i)=jlp_lavecsp(istart1)+ie
		! enddo !i=1,nz  14047
		! endif !if(sparse)  14046
	...
	if(jlp_xpresent)then
		...
		if(nz.gt.0.and. .not.zopt)then
			...
			call printz()
			goto 990
		endif !if(nz.gt.0.and. .not.zopt)  14091
		...
		if(nfx.gt.0.or.nfy>0) call initkeyfact()
		...
		if(.not.isunit)then
			...
			if(iiv.le.0)then
				...
				j_err=.true.
				goto 940
			endif !if(iiv.le.0)  14124
		endif !if(.not.isunit)  14121
		call initxmat()
		call initjlp2() !further initializations
		if(j_err)goto 990
		!!!!???????????  is it really possible that obsw variable is not in cdata
		!  I think that error ocuurrs in data or linkdta functions if this is not the case
		!JL 23.1 20121  nxval is now computed above, it was used earlier even if it was not define
		...
		do iunit=1,jlp_nunits
			...
			if(jlp_ivtrans.gt.0)then
				...
				if(j_err)then
					write(6,*)'error for unit ',iunit
					stop 815
				endif !if(j_err)  14165
 
			endif !if(jlp_ivtrans.gt.0)  14163
			if(isdomain)then
				!			write(6,*)'<456doing,needc,jlp_ndom',needc,jlp_ndom
				j_v(jlp_ivunit)=iunit
				call dotrans(ivdomtrans,1)  !there are domain transformations
				if(j_err)then
					write(6,*)'error in domain transformations for unit ',iunit
					stop 879
				endif !if(j_err)  14175
 
			endif !if(isdomain)  14171
			if(needc.gt.0)jlp_cvar=j_v(jlp_cvarl(1:ncvar))
			...
			if(jlp_ndom.ne.0)then  ! ndom is number of domains
				...
				do j=1,jlp_ndomainvars
					...
					if(j_v(domainvars(j)).ne.j_0)then
						...
						jlp_domainbits(idomba)= ibset(jlp_domainbits(idomba),ibit)
						jlp_domainunits(j)=jlp_domainunits(j)+1
					endif !if(j_v(domainvars(j)).ne.j_0)  14197
					ibit=ibit+1
 
				enddo !j=1,jlp_ndomainvars  14189
			...
			do is=1,jlp_nsch(iunit)
				...
				endif !if(.not.isunit.or.is.gt.1)  14235
				!			call j_getobsiv(jlp_ibaunitbas+nstot,jlp_ivmatx,jlp_ivkeepx,0) !,jlp_ivtransx,0) !making			xmat
				...
				if(jlp_ivsubtrans.gt.0)then
					...
					if(j_err)then
						write(6,*)'error for schedule ',nstot
						stop 447
					endif !if(j_err)  14247
				endif !if(jlp_ivsubtrans.gt.0)  14244
				!	test duplicated schdeulse
				if(inde.eq.1)then
					xvars1=j_v(jlp_xvarl)
					if(is.gt.1)then
						...
						endif !if(all(xvars1.eq.xvars2))  14256
					endif !if(is.gt.1)  14255
					inde=2
				else
					xvars2=j_v(jlp_xvarl)
					if(all(xvars1.eq.xvars2))then
					...
					endif !if(all(xvars1.eq.xvars2))  14268
					inde=1
				endif !if(inde.eq.1)  14253
				if(subfilter_)then         !subfilter->
					testcode=j_codevalue(iob,j_subfilterlink)
					!call dotrans(iob,j_iosubfilter)
					if(j_err)then
					...
					endif !if(j_err)  14281
					if(testcode.eq.0.)then   ! filter->False, reject
						jlp_rejects(nstot)=.true.
						nrej=nrej+1
						nrejtot=nrejtot+1  !total number of rejected schedules
						goto 776
					endif !if(testcode.eq.0.)  14286
				endif !if(subfilter_)  14278
				if(subreject_)then            ! reject->
					testcode=j_codevalue(iob,j_subrejectlink) !call dotrans(iob,j_iosubreject)
					if(j_err)then
						write(6,*)'error for obs ',iob
					endif !if(j_err)  14295
					if(testcode.ne.0.)then    !reject->True
						jlp_rejects(nstot)=.true.
						nrej=nrej+1
						nrejtot=nrejtot+1
						goto 776
					endif !if(testcode.ne.0.)  14298
				endif !if(subreject_)  14293
			776     continue
			!		write(6,*)'<8767'
		!	if(j_xmatinmemory)then
				ibxmatx=(nstot-1)*jlp_ntemp0
				...
				! endif !if(istree)then
				if(needc.gt.0)j_v(jlp_cvarl(1:ncvar))=jlp_cvar
			enddo !is=1,jlp_nsch(iunit)  14230
			if(subfilre)then
				if(nrej.ge.jlp_nsch(iunit))then
					iwar=iwar+1
					if(iwar.le.10)write(6,*)'*err all schedules were rejected for unit ',iunit
				endif !if(nrej.ge.jlp_nsch(iunit))  14365
			endif !if(subfilre)  14364
			if(j_err)return
			!		write(6,*)'nuni ',jlp_nunits,iunit,ibxmatx
		enddo !iunit=1,jlp_nunits  14149
		!	write(6,*)'<56domainbits',jlp_domainbits
		!	write(6,*)'nunits ',jlp_nunits,jlp_ntemp0
		!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		iba=0
		...
		elseif(size(svalue).lt.maxnsch)then
			deallocate(svalue,basreject)
			allocate(svalue(1:maxnsch),basreject(1:maxnsch))
		endif !if(.not.allocated(svalue))  14396
		...
		if(ivunit.le.0.and.nstot.ne.jlp_lopp)then
			...
			j_err=.true.
			return
		endif !if(ivunit.le.0.and.nstot.ne.jlp_lopp)  14416
		call cpu_time(time00)
		if(jlp_xpresent)then
			write(*,*)'found ',jlp_nunits,' units, ',jlp_lopp,' schedules, filling xmat'
		...
		do i=1,min(jlp_ndom,20)
			...
			if(i.eq.20.and.jlp_ndom.gt.20)write(6,*)'etc ..'
		enddo !i=1,min(jlp_ndom,20)  14433
		if(nempt.gt.0)then
			j_err=.true.;write(6,*)'**empty domains:',nempt; return
		end if !if(nempt.gt.0)  14449
		!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 11
		!tehdas xk-mjien ei-negatiivisuus-tarkistus !!!!
		if(jlp_fpresent) then
			ibxdatobs=0
			...
			do iobs=1,j_o(ivxdatmat)%i(1)
				...
				do ixk_ = 1,j_o(jlp_ivxkyk)%i(1) !tehdas xk-mjat
					...
					if (j_o(ivxdatmat)%d(ibxdatobs+k_) < 0) then   !xmatiba
						...
						j_err=.true.
						return
					endif !if (j_o(ivxdatmat)%d(ibxdatobs+k_) < 0)  14476
					!		iba_=iba_+j_o(ivxdatmat)%i(2)
					!		enddo !do while(iba_ < (j_o(ivxdatmat)%i(1)*j_o(ivxdatmat)%i(2)))
				enddo !ixk_ = 1,j_o(jlp_ivxkyk)%i(1)  14471
				ibxdatobs=ibxdatobs+keepx  !jlp_ntemp0
			enddo !iobs=1,j_o(ivxdatmat)%i(1)  14457
			!endif  !xdisk
		endif !if(jlp_fpresent)  14454
			...
			if(subfilre)then
				if(jlp_rejects(jlp_ibaunit(i)+1))then
					...
					do jj=1,jlp_nrowp ;j=jlp_ixcurrows(jj)
						jlp_nunitsrow(j)=jlp_nunitsrow(j)+1
						jlp_xmin(j)=j_inf;jlp_xmax(j)=j_ninf
					enddo !jj=1,jlp_nrowp ;j=jlp_ixcurrows(jj)  14519
					goto 745
				endif !if(jlp_rejects(jlp_ibaunit(i)+1))  14514
			endif !if(subfilre)  14513
			...
			do k=2,jlp_nsch(i)
				if(subfilre)then
					if(jlp_rejects(jlp_ibaunit(i)+k))then
						nrej=nrej+1
						cycle  !do k=2,nsch(i)
					endif !if(jlp_rejects(jlp_ibaunit(i)+k))  14539
				endif !if(subfilre)  14538
				!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 149
				! compute for rows containing x-variables the min and max values
				ibxmatx=xmatiba(jlp_ibaunit(i)+k) !,1)
				do jj=1,jlp_nrowp
					...
					if(curx.lt.jlp_xmin(j))then
						jlp_xmin(j)=curx
					endif !if(curx.lt.jlp_xmin(j))  14550
					if(curx.gt.jlp_xmax(j))then
						jlp_xmax(j)=curx
						keyopt=k   ! for the case where there are no constraints
					endif !if(curx.gt.jlp_xmax(j))  14553
				enddo !jj=1,jlp_nrowp  14547
 
			enddo !k=2,jlp_nsch(i)  14537
 
			do jj=1,jlp_nrowp
				j=jlp_ixcurrows(jj)
				jlp_xsmin(j)=jlp_xsmin(j)+jlp_xmin(j)
				jlp_xsmax(j)=jlp_xsmax(j)+jlp_xmax(j)
			enddo !jj=1,jlp_nrowp  14561
...
4689    key=key+1
				...
				if(subfilre)then
					if(jlp_rejects(jlp_ibaunit(i)+key))goto 4689
				endif !if(subfilre)  14572
 
			endif !if(jlp_nrow.eq.0)  14567
			...
			if(warm.and.jlp_nrow.gt.0)then
				...
				if(jlp_keys(i).gt.jlp_nsch(i).or.rejectnow_)then
					...
					key=jlp_keys(i)
				endif !if(jlp_keys(i).gt.jlp_nsch(i).or.rejectnow_)  14581
			endif !if(warm.and.jlp_nrow.gt.0)  14578
			jlp_keys(i)=key
			ibxmatx=xmatiba(jlp_ibaunit(i)+key) !,1)
			do jj=1,jlp_nrowp
				j=jlp_ixcurrows(jj)
			...
			if(jlp_fpresent) then
				...
				do j=0, jlp_nrow
					if (jlp_ix(j)==0) then
						if (jlp_ixcurfact(j)) then !ehdosta puuttuu rivit, joilla y-tehdasmjia
							jlp_nunitsrow(j) = jlp_nunitsrow(j) + 1
						endif !if (jlp_ixcurfact(j))  14622
					endif !if (jlp_ix(j)==0)  14621
				enddo !j=0, jlp_nrow  14620
					...
					do k=1,jlp_nfxrow(irowj)
						...
						if(jlp_keyfact(i,jlp_irowfxvars(jlp_ibafx(irowj)+k)).eq. &
							...
							if(j.eq.0.and.i.ge.6.and.i.le.10)write(n16,*)'<5566',irowj,k,jlp_ibafx(irowj), &
								jlp_coeffx(jlp_ibafx(irowj)+k),ibxdatkey,jlp_irowfkeep(jlp_ibafx(irowj)+k),&
								j_o(ivxdatmat)%d(ibxdatkey+jlp_irowfkeep(jlp_ibafx(irowj)+k)),jlp_xps(j)
						endif !if(jlp_keyfact(i,jlp_irowfxvars(jlp_ibafx(irowj)+k)).e  14657
					enddo !k=1,jlp_nfxrow(irowj)  14652
				enddo !jj=1,jlp_nrowpf  14647
 
				if (nfy.gt.0) then
					do j=1,j_o(jlp_ivkeepc)%i(1)
						j_v(j_o(jlp_ivkeepc)%i2(j))=j_o(ivcmat)%d((i-1)*j_o(jlp_ivkeepc)%i(1)+j)
					enddo !j=1,j_o(jlp_ivkeepc)%i(1)  14673
					do j=1,ntrans
						call dotrans(jlp_itransv(j),1)
						if(j_err)then
							write(6,*)'error for unit ',i
							stop 771
						endif !if(j_err)  14678
					enddo !j=1,ntrans  14676
				endif !if (nfy.gt.0)  14672
				!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 314
				do jj=1,jlp_nrowpfy
					!ivn-laskurin käsittely ei toimi, jos mukana on domaineja
					j=jlp_ifycurrows(jj) !domainissa olevat rivit
					...
					do k=1,jlp_nfyrow(irowj)
						...
						do ivars_=1,j_o(listy)%i(1)
							...
							do ifact_=1,j_o(listf)%i(1)
								...
								endif !if(jlp_keyfact(i,iv2xykypos_).eq.iv3factpos_)  14718
 
							enddo !ifact_=1,j_o(listf)%i(1)  14707
						enddo !ivars_=1,j_o(listy)%i(1)  14699
 
					enddo !k=1,jlp_nfyrow(irowj)  14690
				enddo !jj=1,jlp_nrowpfy  14685
 
				if(p) then
					write(n16,*)'**fact** xps <38xx>',i,jlp_xps(0:10)
				endif !if(p)  14751
 
			endif !if(jlp_fpresent)  14600
		enddo !i=1,jlp_nunits  14504
		iprint=2
		!	p=.true.
 
		if(ivprint7.gt.0)then
			if(j_v(ivprint7).eq.6.)call printproblem()
		endif !if(ivprint7.gt.0)  14760
		!	write(n16,*)'xps bef preopt',i,(jlp_xps(jj7),jj7=0,min(jlp_nrow,10)) !!!!
		if(p)write(n16,*)'nrowp',jlp_nrowp
		if(iprint.gt.1)write(6,*)'***row, min,max,initial value, tolerance'
		!	write(6,*)'<loub',jlp_lbou
		...
		if (jlp_fpresent) then
			do j=0, jlp_nrow
				...
				! endif !if ((j_nfxrow(j_irowrow(j))>0).or.(j_nfyrow(j_irowrow(j))>0)) then
				! endif !if (j_ix(j)==0) then
			enddo !j=0, jlp_nrow  14791
		endif !if (jlp_fpresent)  14790
			...
			do kief=1,1
				...
				do i=1,jlp_nunits
					...
					do k=1,jlp_nsch(i)
						if(subfilre)then
							if(jlp_rejects(jlp_ibaunit(i)+k))cycle
						endif !if(subfilre)  14813
						jlp_value=jlp_zero  !value is the sum of infeasibility
						if(jlp_ixcurrows(1).eq.0)then
							j1=2
						else !if(j_ixcurrows(1).eq.0)then
							j1=1
						endif !if(jlp_ixcurrows(1).eq.0)  14817
						ibxmatx=xmatiba(jlp_ibaunit(i)+k) !,1)
						ibxmatx2=xmatiba(jlp_ibaunit(i)+jlp_keys(i)) !,2)
						do jj=j1,jlp_nrowp
							j=jlp_ixcurrows(jj)
							...
							elseif(jlp_xps(j)+curx-curk.lt.jlp_rhs(j))then !if(j_xps(j)+curx-curk.gt.j_rhs2(j))then
								jlp_value=jlp_value+jlp_rhs(j)-jlp_xps(j)-curx+curk
							endif !if(jlp_xps(j)+curx-curk.gt.jlp_rhs2(j))  14828
 
						enddo !jj=j1,jlp_nrowp  14824
						if(jlp_value.lt.jlp_valueopt)then
							jlp_valueopt=jlp_value
							kopt=k
 
						endif !if(jlp_value.lt.jlp_valueopt)  14835
						if(k.eq.jlp_keys(i))jlp_valuek=jlp_value
					enddo !k=1,jlp_nsch(i)  14812
					if(jlp_valuek.gt.jlp_valueopt)then
						ibxmatx=xmatiba(jlp_ibaunit(i)+kopt) !,1)
						! ibxmatx2=j_ibaunit(i)+j_keys(i) put earlier
						...
						enddo !jj=1,jlp_nrowp  14846
						jlp_keys(i)=kopt
						apusum=apusum+jlp_valuek-jlp_valueopt
 
					endif !if(jlp_valuek.gt.jlp_valueopt)  14842
				enddo !i=1,jlp_nunits  14808
				write(6,*)'preoptimization round ',kief,' improved infeasibility sum by ',apusum
				write(6,*)' '
			enddo !kief=1,1  14806
			if(iprint.ge.0)then
				write(6,*)'initial values'
				do jj=1,jlp_nrowp   !nrowp = number of rows with x-variables
					j=jlp_ixcurrows(jj) !j= number of the row
					write(6,*)j,jlp_xps(j)
					!	if(j.eq.2)write(27,*)'<init',jlp_xps(j)
				enddo !jj=1,jlp_nrowp  14871
			endif !if(iprint.ge.0)  14869
			jlp_objf=jlp_small
		endif !if(jlp_nrow.ne.0.and.jlp_xpresent.and..not.jlp_fpresent.an  14804
	...
	if(jlp_xpresent)then
		...
		if(jlp_ixcurrows(1).eq.0)then
			...
			j1=1   ! there is no  "
		endif !if(jlp_ixcurrows(1).eq.0)  15143
		nunf=0 ! number of infeasible rows
		!write(6,*)'<66nz ',nz
		rowloop:			do jj=j1,jlp_nrowp
			j=jlp_ixcurrows(jj)
			...
			if(nz>0) then
				do inz_=1,nz
					if(jlp_a(j,inz_).ne.0.) cycle rowloop
				enddo !inz_=1,nz  15154
			endif !if(nz>0)  15153
			if(jlp_fpresent) then
				if(jlp_nfxrow(jlp_irowrow(j))>0.or.jlp_nfyrow(jlp_irowrow(j))>0) &
					cycle rowloop
			endif !if(jlp_fpresent)  15158
			if(jlp_xsmin(j).gt.jlp_rhs2(j).or.jlp_xsmax(j).lt.jlp_rhs(j))then
				nunf=nunf+1
				iunf=j
				if(nunf.eq.1)then
				...
				if(jlp_xsmin(iunf).gt.jlp_rhs2(iunf))then
					...
					if(nureport.ne.6)write(nureport,*)'row ',iunf,'*smallest possible value ',&
						jlp_xsmin(iunf),' rhs2=',jlp_rhs2(iunf)
				endif !if(jlp_xsmin(iunf).gt.jlp_rhs2(iunf))  15172
				if(jlp_xsmax(iunf).lt.jlp_rhs(iunf))then
					write(6,*)'*greatest possible value ', &
						jlp_xsmax(iunf),' rhs=',jlp_rhs(iunf)
					if(nureport.ne.6)write(nureport,*)'row ',iunf,&
						'*greatest possible value ',jlp_xsmax(iunf),' rhs=',jlp_rhs(iunf)
				endif !if(jlp_xsmax(iunf).lt.jlp_rhs(iunf))  15178
			endif !if(jlp_xsmin(j).gt.jlp_rhs2(j).or.jlp_xsmax(j).lt.jlp_rhs(  15162
		enddo rowloop !loop:			do jj=j1,jlp_nrowp  15150
		if(iunf.ne.0)then
			return   ! pitäis tehdä jotain goto 8000
		endif !if(iunf.ne.0)  15186
...
!write(16,*)'newstart'
	...
	if(jlp_xpresent)then
		...
		if(jlp_fpresent) then
			...
			jlp_nextf(jlp_mxd,1:j_o(jlp_ivxkyk)%i(1))=jlp_mxd
			jlp_iprevf(jlp_mxd,1:j_o(jlp_ivxkyk)%i(1))=jlp_mxd
		endif !if(jlp_fpresent)  15237
	endif !if(jlp_xpresent)  15234
	...
	do i=1,jlp_nrow
		...
		if((jlp_ix(i).ne.0).or.jlp_fpresent)then   ! there is a x-variable in the row
			...
			if(jlp_xps(i).gt.jlp_rhs2(i).or..not.jlp_lbou(i))then
				...
				jlp_rhscur(i)=jlp_rhs(i)
				jlp_lower(i)=.true.
			endif !if(jlp_xps(i).gt.jlp_rhs2(i).or..not.jlp_lbou(i))  15251
			!xps is the sum over key schedules
			jlp_rhsw(i)=jlp_rhscur(i)-jlp_xps(i)  ! 6.27 and 6.28 p. 110 , get working rhs
			!	if(i.eq.2)	write(27,*)'<353553rhs ',jlp_rhscur(i),jlp_xps(i)
		else !if((j_ix(i).ne.0).or.j_fpresent)then
			if(jlp_lbou(i))then
				...
				jlp_rhscur(i)=jlp_rhs2(i)
				jlp_lower(i)=.false.
			endif !if(jlp_lbou(i))  15263
			jlp_rhsw(i)=jlp_rhscur(i)
		endif !if((jlp_ix(i).ne.0).or.jlp_fpresent)  15249
 
	enddo !i=1,jlp_nrow  15247
	if(p2)write(n16,*)'rhscur',jlp_rhscur
	if(p2)write(n16,*)'rhsw',jlp_rhsw
	...
	do j=jlp_nrowz+1,mxnm !!!ncol ol liian vähän
		jlp_xma(j)=1.  ! maximum value for w, later area of the unit
	enddo !j=jlp_nrowz+1,mxnm  15282
	...
	if(jlp_nrow.eq.0.and..not.jlp_fpresent)then   ! no constraints, was optimized directly
		...
		return
	endif !if(jlp_nrow.eq.0.and..not.jlp_fpresent)  15307
	j_v(j_ivstartedjlp)=1.  !!!!
	...
	if (jlp_fpresent) then
		do i=jlp_mxd+1,2*jlp_mxd
			jlp_lf(i)=i          !lists columns of D
			jlp_lfi(i)=i       !inverse list
		enddo !i=jlp_mxd+1,2*jlp_mxd  15389
		jlp_lf0=jlp_mxd
		!jatketaanko seuraavasta tehdasmjasta tarkastelu/ienter, ixkykenter alustukset
		ienter= 0
		ixkykenter = 0
		iunitrans = 0	! yksikkö, jolle viimeksi laskettu tehdasmja muunnokset
	endif !if (jlp_fpresent)  15388
	...
	if(.not.jlp_xpresent2)then
		nkier=1
		nunits=1
	endif !if(.not.jlp_xpresent2)  15504
	!if(j_o(ivxdatmat)%r(10261*keepx).eq.0.)stop 90
	! iba=xmatiba(6841,1)
	! iba2=xmatiba(6842,2)
	! write(18,*)'<129>'
	...
	if(jlp_xpresent2)then
		...
		if(isstop)then
			...
			write(6,*)'stop->(Change%.lt.0.01.and.Round.ge.30)'
		endif !if(isstop)  15548
	endif !if(jlp_xpresent2)  15543
	write(6,*)'printing option, print->',iprint
	!if(memory.eq.0.and.j_xdatfromdisk)write(6,*)'without memory->, xmat is put into memory, but xdata is used from disk'
					...
					if(jlp_nextxkf) then   !!!!
						...
						if ((nfy.gt.0).and.(iunit/=iunitrans)) then
							do j=1,j_o(jlp_ivkeepc)%i(1)
								j_v(j_o(jlp_ivkeepc)%i2(j))= &
									j_o(ivcmat)%d((iunit-1)*j_o(jlp_ivkeepc)%i(1)+j)
							enddo !j=1,j_o(jlp_ivkeepc)%i(1)  15725
							do j=1,ntrans
								call dotrans(jlp_itransv(j),1)
								if(j_err)then
									write(6,*)'error for transformation ',j
								endif !if(j_err)  15731
							enddo !j=1,ntrans  15729
							iunitrans=iunit
						endif !if ((nfy.gt.0).and.(iunit/=iunitrans))  15724
						if(p) write(n16,*)'<5836> nextxkf true'
						goto 5316   !!!! jatketaan seuraavasta tehdasmuuttujasta
						!!!! varmistetaan tehdasmjien läpikäynti alusta tilanteessa,
						!!!! jossa yksikön tehdasmja-listan läpikäyntiä ei jatketa
						...
						ixkykenter = 0  !!!!
					endif !if(jlp_nextxkf)  15721
				endif !if (jlp_fpresent)  15720
...
401 continue
							...
							if(jlp_feasible)then
								if(jlp_xpresent2)then
									...
									write(6,*)'Feasible, objective:', &
										coefmax*jlp_objf,'basic residuals ', lr0, &
										' basic z-vars ',lz0, ' cpu ',(time-time00)/60.
								endif !if(jlp_xpresent2)  15767
								!time0=time
							endif !if(jlp_feasible)  15766
						else !if(ipivot9.ne.ipivot)then
							if(jlp_xpresent2)then
								write(6,'(i5,i8,g22.12,7x,f8.2,5i6,f6.2,f7.2)')&
									kier,ipivot,coefmax*jlp_objf,as,lr0,lz0,jlp_ld0, &
								...
								write(6,*)'Optimal, objective:', &
									coefmax*jlp_objf,'basic residuals ', lr0, ' basic z-vars ',lz0, ' cpu ',(time-time00)/60.
							endif !if(jlp_xpresent2)  15788
						endif !if(ipivot9.ne.ipivot)  15764
						!write(6,*)'<654exitkier'
						if(jlp_xpresent2)write(6,*)'no changes during last round'
						if(jlp_feasible)write(6,*)'SOLUTION FOUND'
 
						exit kierloop
					endif !if(.not.goto700)  15763
				endif !if((iunitprev.eq.iunitv.and.kier.gt.kierv).or.(iunitv==-1.  15758
				...
				if(.not.jlp_xpresent)then  !!!check if optimal with only z-problem   <R1>
					...
					j_v(ivoptimal)=1.
					exit kierloop !goto 900          !   <r1>
				endif !if(.not.jlp_xpresent)  15811
				!if(pp)write(16,*)'beforesenter,iente',ienter,iunit,j_objf,nnf,ipivot,kier,goto400
...
! if finding feasible voisi suoraan katsoa, että valuoptin ja valuek:n eroa
				...
				if(iopt.eq.jlp_keys(iunit))then  !!!!
					if (.not.jlp_fpresent)then   !!!!
						...
						!	write(16,*)'goto5316tas'
						goto 5316 ! fpresent &&  (iopt == key)  !!!!
					endif !if (.not.jlp_fpresent)  15835
				endif !if(iopt.eq.jlp_keys(iunit))  15834
				if(jlp_valueopt.le.jlp_valuek+jlp_tolecur) then  !!!!
					if (.not.jlp_fpresent)then   !!!!
						exit inunitloop !cycle unitloop !goto 400   !goto next unit , 400 is earlier !!!!
					else !if (.not.j_fpresent)then
						!	write(16,*)'goto5316tas'
						goto 5316  ! fpresent &&  (valueopt <= valuek+tolecur) !!!!
					endif !if (.not.jlp_fpresent)  15843
				else !if(j_valueopt.le.j_valuek+j_tolecur) then
					if(jlp_fpresent)call fenter0() !;if (j_fpresent)call fenter0()
					!if(pp)write(16,*)'befentcol'
					call entercol()
					...
					goto 100 !goto 5381 ! (iopt != key) && (valueopt > valuek+tolecur) !!!!
				endif !if(jlp_valueopt.le.jlp_valuek+jlp_tolecur)  15842
...
!if(pp)write(16,*)'before leaving,ienter',ienter
				...
				if(goto112233)exit inunitloop
				!goto 55      !!!!computations after normal pivot are done, goto computing basic vars
			enddo inunitloop !nitloop: do while(.true.)  15673
			...
			!write(17,*)'aftendinunitloop',kier,iunit
		enddo unitloop !tloop: do iunit=1,nunits  15625
		if(jlp_fpresent.and.jlp_feasible)then
			if(.not.factnow)then
				nofact=nofact+1
				if(nofact.gt.iterxkf)then
					...
					factnow=.true.
					nofact=0
				endif !if(nofact.gt.iterxkf)  15920
			else !if(.not.factnow)then
				!	write(6,*)'round,factnow put to false',kier
				!		factnow=.false.
			endif !if(.not.factnow)  15918
		endif !if(jlp_fpresent.and.jlp_feasible)  15917
 
		!write(17,*)'aftunitloop',kier,iunit
	enddo kierloop !rloop: do kier=1,nkier  15597
...
990  continue
	...
	subroutine initjlp()
		...
		elseif(i.eq.0)then !if(i.gt.0)then
			warm=.true.
		endif !if(i.gt.0)  16139
		!keys key schedules for each unit
		if(.not.allocated(jlp_keys).and.warm)then
			write(6,*)'*jlp: old keys not available, warm-> ignored'
			warm=.false.
		endif !if(.not.allocated(jlp_keys).and.warm)  16145
		warmf=warm
		...
		if(jlp_ivtrans.gt.0)then
			if(j_otype(jlp_ivtrans).ne.j_iptrans)then
				call j_printname('**jlp: trans->',jlp_ivtrans,' is not transformation')
				j_err=.true. ;return
			endif !if(j_otype(jlp_ivtrans).ne.j_iptrans)  16232
		endif !if(jlp_ivtrans.gt.0)  16231
		jlp_ivsubtrans=j_igetopt(iob,io,j_msubtrans) !object given in subtrans-option
		!write(6,*)'<5444jlp_ivsubtrans',jlp_ivsubtrans
		if(jlp_ivsubtrans.gt.0)then
			if(j_otype(jlp_ivsubtrans).ne.j_iptrans)then
				call j_printname('**jlp: subtrans->',jlp_ivsubtrans,' is not transformation')
				j_err=.true. ;return
			endif !if(j_otype(jlp_ivsubtrans).ne.j_iptrans)  16240
		endif !if(jlp_ivsubtrans.gt.0)  16239
		...
		if(ivarea.eq.0)then   !option area->,
			...
			if(ivarea.le.0)then
				...
				j_err=.true.
				return
			endif !if(ivarea.le.0)  16253
		endif !if(ivarea.eq.0)  16251
		notareav=j_nargopt(iob,io,j_mnotareavars) !number of notareavars-> -variables
		if(notareav.gt.0)then
			linknotarea=j_linkoption(iob,io,j_mnotareavars)
			if(ivarea.lt.0)then
				...
				j_err=.true.
				return
			endif !if(ivarea.lt.0)  16262
		endif !if(notareav.gt.0)  16260
		! outputobject, not used now
		...
		if(noptarg.eq.1)then
			...
			if(nureport.le.0)then
				...
				if(j_err)return
				nureport=nuu  !j_iounit(ivreport) !j_o(ivreport)%i(4)
			endif !if(nureport.le.0)  16278
 
		else !if(j_nargopt(iob,io,j_mreport).eq.1)then
			nureport=6
		endif !if(noptarg.eq.1)  16273
		...
		if(j_linkoption(iob,io,j_mtest).ge.0)then
			if(j_o(iob)%i(j_linkoption(iob,io,j_mtest)).eq.0)then   ! test->
				...
				jlp_testl=.false.   ! test->0
			end if !if(j_o(iob)%i(j_linkoption(iob,io,j_mtest)).eq.0)  16308
		else !if(j_linkoption(iob,io,j_mtest).gt.0)then
			jlp_testl=.false.
		end if !if(j_linkoption(iob,io,j_mtest).ge.0)  16307
		!debug
		p=.false.
		p2=.false.
		!	ivclass=0
		...
		if(j_linkoption(iob,io,j_mdebug).ge.0)then
			...
			if(ndebug.eq.0)then
				...
				if(idebug.le.0)then
					...
					if(ndebug.ge.2)then           ! pelaakohan tämä niinkuin pitäisi
						idebug=j_v(j_o(iob)%i(ibasdebug+2))
						jdebug=2
					endif !if(ndebug.ge.2)  16348
				endif !if(idebug.le.0)  16345
 
			endif !if(ndebug.eq.0)  16339
			...
			idebug=huge(1)
		endif !if(j_linkoption(iob,io,j_mdebug).ge.0)  16335
		ipivot=0
		...
		if(j_linkoption(iob,io,j_mzmatrix).gt.0)then
			if(j_nargopt(iob,io,j_mzmatrix).le.0)then
				write(6,*)'*jlp: zmatrix-> should have argument'
				j_err=.true.;return
			endif !if(j_nargopt(iob,io,j_mzmatrix).le.0)  16366
			zmatrix=.true.
			if(j_linkoption(iob,io,j_mdata).gt.0.or.j_linkoption(iob,io,j_mproblem).gt.0.or. &
					j_linkoption(iob,io,j_mtrans).gt.0.or.j_linkoption(iob,io,j_msubtrans).gt.0) then
				write(6,*)'*jlp: illegal options with zmatrix->'
				j_err=.true.;return
			endif !if(j_linkoption(iob,io,j_mdata).gt.0.or.j_linkoption(iob,i  16371
			ivzmatrix=j_o(iob)%i(j_linkoption(iob,io,j_mzmatrix)+1)
			if(j_otype(ivzmatrix).ne.j_ipmatrix) then
				call j_printname('*jlp: zmatrix ',ivzmatrix,' is not valid matrix')
				j_err=.true.;return
			endif !if(j_otype(ivzmatrix).ne.j_ipmatrix)  16377
			jlp_nrow=j_o(ivzmatrix)%i(1);nz =j_o(ivzmatrix)%i(2)
		else !if(j_linkoption(iob,io,j_mzmatrix).gt.0)then
			zmatrix=.false.
		endif !if(j_linkoption(iob,io,j_mzmatrix).gt.0)  16365
		...
		if(j_linkoption(iob,io,j_mmax).gt.0)then
			if(.not.zmatrix)then
				write(6,*)'*jlp: zmatrix-> missing'
				j_err=.true.;return
			endif !if(.not.zmatrix)  16388
			if(j_nargopt(iob,io,j_mmax).le.0)then
				write(6,*)'*jlp: max-> should have argument'
				j_err=.true.;return
			endif !if(j_nargopt(iob,io,j_mmax).le.0)  16392
			jlp_maxo =.true.
			zobj=.true.
			ivzobj=j_o(iob)%i(j_linkoption(iob,io,j_mmax)+1)
		endif !if(j_linkoption(iob,io,j_mmax).gt.0)  16387
 
		if(j_linkoption(iob,io,j_mmin).gt.0)then
			if(.not.zmatrix)then
				write(6,*)'*jlp: zmatrix-> missing'
				j_err=.true.;return
			endif !if(.not.zmatrix)  16402
			if(zobj)then
				write(6,*)'*jlp: min'
				j_err=.true.;return
			endif !if(zobj)  16406
			if(j_nargopt(iob,io,j_mmin).le.0)then
				write(6,*)'*jlp: min-> should have argument'
				j_err=.true.;return
			endif !if(j_nargopt(iob,io,j_mmin).le.0)  16410
			jlp_maxo=.false.
			zobj=.true.
			ivzobj=j_o(iob)%i(j_linkoption(iob,io,j_mmin)+1)
		endif !if(j_linkoption(iob,io,j_mmin).gt.0)  16401
 
		if (zobj) then
			if(j_otype(ivzobj).ne.j_ipmatrix) then
				call j_printname('*jlp: max/min ',ivzobj,' is not valid vector')
				j_err=.true.;return
			endif !if(j_otype(ivzobj).ne.j_ipmatrix)  16420
			nzodim1=j_o(ivzobj)%i(1);nzodim2 =j_o(ivzobj)%i(2)
			if((nzodim1*nzodim2).ne.nz) then
				call j_printname('*jlp: max->/min-> ',ivzobj,' should have same number elements as there are columns in zmatrix->')
				write(6,*)'Columns in zmatrix->:',nz
				write(6,*)'Elements in max->/min->:',nzodim1*nzodim2
				j_err=.true.;return
			endif !if((nzodim1*nzodim2).ne.nz)  16425
		endif !if (zobj)  16419
		...
		if(j_linkoption(iob,io,j_mrhs).gt.0)then
			if(.not.zmatrix)then
				write(6,*)'*jlp: zmatrix-option missing'
				j_err=.true.;return
			endif !if(.not.zmatrix)  16434
			if(j_nargopt(iob,io,j_mrhs).le.0)then
				write(6,*)'*jlp: rhs-> should have argument'
				j_err=.true.;return
			endif !if(j_nargopt(iob,io,j_mrhs).le.0)  16438
			rhsopt=.true.
			ivrhs=j_o(iob)%i(j_linkoption(iob,io,j_mrhs)+1)
			if(j_otype(ivrhs).ne.j_ipmatrix) then
				call j_printname('*jlp: rhs ',ivrhs,' is not valid vector')
				j_err=.true.;return
			endif !if(j_otype(ivrhs).ne.j_ipmatrix)  16444
			nrhsoptdim1=j_o(ivrhs)%i(1);nrhsoptdim2 =j_o(ivrhs)%i(2)
			if((nrhsoptdim1*nrhsoptdim2).ne.jlp_nrow) then
				call j_printname('*jlp: rhs ',ivrhs,' should have same number elements as there are rows in zmatrix->')
				write(6,*)'Rows in zmatrix->:',jlp_nrow
				write(6,*)'Elements in rhs->:',nrhsoptdim1*nrhsoptdim2
				j_err=.true.;return
			endif !if((nrhsoptdim1*nrhsoptdim2).ne.jlp_nrow)  16449
		else !if(j_linkoption(iob,io,j_mrhs).gt.0)then
			rhsopt=.false.
		endif !if(j_linkoption(iob,io,j_mrhs).gt.0)  16433
 
		if(j_linkoption(iob,io,j_mrhs2).gt.0)then
			if(.not.zmatrix)then
				write(6,*)'*jlp: zmatrix-option missing'
				j_err=.true.;return
			endif !if(.not.zmatrix)  16460
			if(j_nargopt(iob,io,j_mrhs2).le.0)then
				write(6,*)'*jlp: rhs-> should have argument'
				j_err=.true.;return
			endif !if(j_nargopt(iob,io,j_mrhs2).le.0)  16464
			rhs2opt=.true.
			ivrhs2=j_o(iob)%i(j_linkoption(iob,io,j_mrhs2)+1)
			if(j_otype(ivrhs2).ne.j_ipmatrix) then
				call j_printname('*jlp: rhs2 ',ivrhs2,' is not valid vector')
				j_err=.true.;return
			endif !if(j_otype(ivrhs2).ne.j_ipmatrix)  16470
			nrhs2optdim1=j_o(ivrhs2)%i(1);nrhs2optdim2 =j_o(ivrhs2)%i(2)
			if((nrhs2optdim1*nrhs2optdim2).ne.jlp_nrow) then
				call j_printname('*jlp: rhs2 ',ivrhs2,' should have same number elements as there are rows in zmatrix->')
				write(6,*)'Rows in zmatrix->:',jlp_nrow
				write(6,*)'Elements in rhs->:',nrhsoptdim1*nrhsoptdim2
				j_err=.true.;return
			endif !if((nrhs2optdim1*nrhs2optdim2).ne.jlp_nrow)  16475
		else !if(j_linkoption(iob,io,j_mrhs2).gt.0)then
			rhs2opt=.false.
		endif !if(j_linkoption(iob,io,j_mrhs2).gt.0)  16459
		...
		if (zmatrix) then
			if(ivoutresult.eq.j_ivresult)then
				...
				j_err=.true.
				return
			end if !if(ivoutresult.eq.j_ivresult)  16499
			!joko rhs tai rhs2 oltava
			if(.not.(rhsopt.or.rhs2opt)) then
				write(6,*)'*jlp: rhs-> or rhs2-> missing'
				j_err=.true.;return
			endif !if(.not.(rhsopt.or.rhs2opt))  16505
			if(allocated(jlp_rhs))deallocate(jlp_rhs,jlp_rhs2)
			allocate(jlp_rhs(1:jlp_nrow),jlp_rhs2(1:jlp_nrow))
			if(allocated(jlp_lbou))deallocate(jlp_lbou);if(allocated(jlp_ubou))deallocate(jlp_ubou)
			allocate(jlp_lbou(1:jlp_nrow),jlp_ubou(1:jlp_nrow))
			...
			if(rhsopt) then
				...
				do i_=1,jlp_nrow
					if(jlp_rhs(i_).eq.j_ninf) then
						jlp_rhs(i_)=j_ninf
						jlp_lbou(i_)=.false.
					endif !if(jlp_rhs(i_).eq.j_ninf)  16522
				enddo !i_=1,jlp_nrow  16521
			endif !if(rhsopt)  16518
			if(rhs2opt) then
				jlp_ubou=.true.
				jlp_rhs2(1:jlp_nrow) = j_o(ivrhs2)%d(1:jlp_nrow)
				do i_=1,jlp_nrow
					if(jlp_rhs2(i_).eq.j_inf) then
						jlp_rhs2(i_)=j_inf
						jlp_ubou(i_)=.false.
					endif !if(jlp_rhs2(i_).eq.j_inf)  16532
				enddo !i_=1,jlp_nrow  16531
			endif !if(rhs2opt)  16528
		endif !if (zmatrix)  16498
		...
		if(jlp_ivprob.gt.0)then
			if(j_otype(jlp_ivprob).ne.j_ipproblem)then
				...
				write(*,*)'**problem-> does not refer to a legal problem'
				return
			endif !if(j_otype(jlp_ivprob).ne.j_ipproblem)  16556
			jlp_ndom=j_o(jlp_ivprob)%i(10)
			...
			if(ivdomainvars.gt.0)then
				...
				if(.not.zmatrix) then
					write(6,*)'**no problem given';j_err=.true.;return
				endif !if(.not.zmatrix)  16571
			endif !if(ivdomainvars.gt.0)  16564
		endif !if(jlp_ivprob.gt.0)  16555
		if(j_linkoption(iob,io,j_mrefac).gt.0)then
			nrefac=j_v(j_o(iob)%i(j_linkoption(iob,io,j_mrefac)+1))
		else !if(j_linkoption(iob,io,j_mrefac).gt.0)then
			nrefac=1000  !
		endif !if(j_linkoption(iob,io,j_mrefac).gt.0)  16576
		!	write(6,*)'refac=',nrefac
		...
		if(j_linkoption(iob,io,j_mtole).gt.0)then
			if(j_nargopt(iob,io,j_mtole).le.0)then
				write(6,*)'*jlp: tole-> should have argument'
				j_err=.true.;return
			endif !if(j_nargopt(iob,io,j_mtole).le.0)  16585
			jlp_tolep=j_v(j_o(iob)%i(j_linkoption(iob,io,j_mtole)+1))
		else !if(j_linkoption(iob,io,j_mtole).gt.0)then
			jlp_tolep=jlp_one
		endif !if(j_linkoption(iob,io,j_mtole).gt.0)  16584
		!write(6,*)'tole=',j_tolep
	...
	subroutine initxdata()
		if(isunit)then
			...
			if(ivxdata.le.0.or.j_otype(max(1,ivxdata)).ne.j_ipdata)then
				if(j_o(ivcdata)%i(5).ne.0.)then
					! there is up-data
					if(j_otype(j_o(ivcdata)%i(11)).ne.j_ipdata)then
						call  j_printname('**jlp: up-data  ',j_o(ivcdata)%i(11),' is not a data set');j_err=.true.;return
					endif !if(j_otype(j_o(ivcdata)%i(11)).ne.j_ipdata)  16613
					ivxdata=ivcdata
					ivcdata=j_o(ivcdata)%i(5)
 
				else !if(j_o(ivcdata)%i(5).ne.0.)then
					call j_printname('**no legal data linked to data ',ivcdata,' ');j_err=.true.;return
				endif !if(j_o(ivcdata)%i(5).ne.0.)  16611
			endif !if(ivxdata.le.0.or.j_otype(max(1,ivxdata)).ne.j_ipdata)  16610
			!xdisk testaus
			! ivtestmat=0
			! if(j_linkoption(iob,io,j_msubdata).gt.0)then   !
		...
		if(j_linkoption(iob,io,j_mmaxiter).gt.0)then
			...
			mxiter=2000
		endif !if(j_linkoption(iob,io,j_mmaxiter).gt.0)  16692
	...
	subroutine initfact()
		...
		if(.not.jlp_maxo.and.jlp_nfyrow(1)>0) then
			...
			j_err = .true.
			return
		endif !if(.not.jlp_maxo.and.jlp_nfyrow(1)>0)  16704
		...
		do j=2,irowp
			jlp_ibafx(j)=jlp_ibafx(j-1)+jlp_nfxrow(j-1)
		enddo !j=2,irowp  16729
 
		jlp_ibafy(1)=0
		do j=2,irowp
			jlp_ibafy(j)=jlp_ibafy(j-1)+jlp_nfyrow(j-1)
		enddo !j=2,irowp  16734
		...
		if(count(jlp_isfyval)>0) then
			...
			call pack2(jlp_ibafykeep,jlp_isfyval,nval)
			call pack2(jlp_ibafyfact,jlp_isfyval,nval)
		endif !if(count(jlp_isfyval)>0)  16745
 
		nyxkf_ = 0	! xk/tehdas -yhdistelmien kokonaismäärä yk-esiintymissä
		do i_ = 1, count(jlp_isfyval)
			nyxkf_ = nyxkf_ + j_o(jlp_irowfyvars(i_))%i(1)*j_o(jlp_irowfyfact(i_))%i(1)
		enddo !i_ = 1, count(jlp_isfyval)  16753
		nfxfyexp=nfxval+ nyxkf_
		if(allocated(jlp_xkykrowvars))deallocate(jlp_xkykrowvars)
		allocate(jlp_xkykrowvars(1:nfxfyexp))
				...
				do irowj_ = 1,irowp ! tehtävärivit (alkup. ei lavennetut)
					do k=1,jlp_nfxrow(irowj_) ! rivin xk-muuttujat
						...
						if((ixkyk_.eq.jxk).and.(if_.eq.jf)) then
							...
							jlp_xkykrowvars(irowvars_)%ind = jlp_ibafx(irowj_)+k
							irowvars_ = irowvars_ + 1
						endif !if((ixkyk_.eq.jxk).and.(if_.eq.jf))  16779
					enddo !k=1,jlp_nfxrow(irowj_)  16776
					do k=1,jlp_nfyrow(irowj_) !rivin yk-mjat
						listy=jlp_irowfyvars(jlp_ibafy(irowj_)+k) !yk-mjaa vastaava xk-lista
						listf=jlp_irowfyfact(jlp_ibafy(irowj_)+k) !yk-mjaa vastaava tehdas-lista
						do ixk_=1,j_o(listy)%i(1) !xk-muuttujat
							...
							do iif_=1,j_o(listf)%i(1) !tehtaat
								...
								if ((ixkyk_.eq.jxk).and.(if_.eq.jf)) then
									...
									jlp_xkykrowvars(irowvars_)%ind = jlp_fyfactout(jxk,jf)
									irowvars_ = irowvars_ + 1
								endif !if ((ixkyk_.eq.jxk).and.(if_.eq.jf))  16797
							enddo !iif_=1,j_o(listf)%i(1)  16794
						enddo !ixk_=1,j_o(listy)%i(1)  16789
					enddo !k=1,jlp_nfyrow(irowj_)  16786
				enddo !irowj_ = 1,irowp  16775
				jlp_xkfact(ixkyk_,inf_)%inxkykrowvar = irowvars_-1
			enddo !inf_=1, jlp_nxkfact(ixkyk_)  16772
		enddo !ixkyk_=1,j_o(jlp_ivxkyk)%i(1)  16771
		...
		do irowj_ = 2,irowp
			...
			if(jlp_nfxrow(irowj_).eq.1.and. jlp_nfyrow(irowj_).eq.0.and. &
				...
				endif !if(zeroc)  16829
 
			endif !if(jlp_nfxrow(irowj_).eq.1.and. jlp_nfyrow(irowj_).eq.0.an  16815
		enddo !irowj_ = 2,irowp  16812
		write(6,*)'irowp',irowp
		! write(6,*)'jlp_nfxrow',jlp_nfxrow
		! write(6,*)'jlp_nfyrow',jlp_nfyrow
		! write(6,*)'jlp_irowfxvars',jlp_irowfxvars
		...
		do i=1,nvartot
			...
			if(.not.jlp_isx(i).and..not.fvari)then
				...
				if(nnz.ge.10.and.nnz.lt.nz)then
					write(6,*)' ...etc...'
					exit
				endif !if(nnz.ge.10.and.nnz.lt.nz)  16862
			endif !if(.not.jlp_isx(i).and..not.fvari)  16859
		enddo !i=1,nvartot  16852
		j_err=.true.
		return
			...
			if(warmf)then
				if(size(jlp_keyfact,dim=1).ne.jlp_nunits.or. &
					...
					write(6,*)'*jlp: dimensions of problem do not agree with the previous problem',&
						' warm-> ignored for factories'
					warmf=.false.
				endif !if(size(jlp_keyfact,dim=1).ne.jlp_nunits.o  16878
			endif !if(warmf)  16877
			if(.not.warmf)deallocate(jlp_keyfact)
		endif !if(allocated(jlp_keyfact))  16876
		if(.not.allocated(jlp_keyfact))allocate(jlp_keyfact(1:jlp_nunits,1:j_o(jlp_ivxkyk)%i(1)))
		if(p9)write(16,*)zeroc
		if(warmf)then
			do i=1,jlp_nunits
				do j=1,j_o(jlp_ivxkyk)%i(1)
					do keyf_=1,jlp_nxkfact(j)
						if(jlp_xkfact(j,keyf_)%ifact.eq.jlp_keyfact(i,j))then
							...
							! endif !if(zeroc(j,j_keyfact(i,j)))then
							goto 171
						endif !if(jlp_xkfact(j,keyf_)%ifact.eq.jlp_keyfact(i,j))  16893
					enddo !keyf_=1,jlp_nxkfact(j)  16892
					warmf=.false.
					write(6,*)'*factories do not agree warm-> ignored for factories'
					goto 172
						171			continue
				enddo !j=1,j_o(jlp_ivxkyk)%i(1)  16891
			enddo !i=1,jlp_nunits  16890
		endif !if(warmf)  16889
			...
			do j=1,j_o(jlp_ivxkyk)%i(1)
				...
				! endif
				jlp_keyfact(1,j) =keyf_
			enddo !j=1,j_o(jlp_ivxkyk)%i(1)  16917
			...
			do i=2,jlp_nunits
				do j=1,j_o(jlp_ivxkyk)%i(1)
					if(jlp_keyfact(i-1,j).lt.jlp_nxkfact(j)) then
						...
						keyf_ = 1
					endif !if(jlp_keyfact(i-1,j).lt.jlp_nxkfact(j))  16933
					init_=keyf_
					180					jf_=jlp_xkfact(j,keyf_)%ifact
					! if(zeroc(j,jf_))then
						! keyf_=keyf_+1
					...
					! endif
					jlp_keyfact(i,j) =keyf_
				enddo !j=1,j_o(jlp_ivxkyk)%i(1)  16932
			enddo !i=2,jlp_nunits  16931
 
			!muutetaan keyfactin alkiot suoraan factories-listan indekseiksi
			do i=1,jlp_nunits
				do j=1,j_o(jlp_ivxkyk)%i(1)
					jlp_keyfact(i,j) = jlp_xkfact(j,jlp_keyfact(i,j))%ifact
				enddo !j=1,j_o(jlp_ivxkyk)%i(1)  16956
			enddo !i=1,jlp_nunits  16955
		endif !172			if(.not.warmf)  16916
		...
		do i=1,nset ! number of sets
			...
			do k=1,jlp_nsetr(i) ! number of rows in set i (before expanding over domains)
					...
					do iro=1,irow0-1
						if(jlp_nxrow(iro).eq.jlp_nxrow(irow0))then
							do jj=1,jlp_nxrow(iro)
								...
								if(jlp_irowvars(iba+jj).ne.jlp_irowvars(iba2+jj).or.&
									jlp_coefx(iba+jj).ne.jlp_coefx(iba2+jj))goto 194
							enddo !jj=1,jlp_nxrow(iro)  17029
							!!!!     iro fllows problem statement rows
							itemp0=jlp_ixprow(iro); goto 195 !write(6,*)'samma x',iro,irow0,'ix',itemp0
						endif !if(jlp_nxrow(iro).eq.jlp_nxrow(irow0))  17028
					194    iba2=iba2+jlp_nxrow(iro)
					enddo !iro=1,irow0-1  17027
					itemp=itemp+1
					write(6,*)'<456>itemp',itemp
					itemp0=itemp
					jlp_ibatemp(itemp)=iba
					jlp_nxrowtemp(itemp)=jlp_nxrow(irow0)
					! ibatemp is the basis for accesing irowvars and coefx for temporary variables
				...
				195  continue
					...
					jlp_ixprow(irow0)=0
				endif !if(jlp_nxrow(irow0).gt.0)  17024
				...
				if(irow0.ne.irowobj)then ! .or.k.gt.1)then
					...
					elseif(jlp_isetd(idomba+j).ne.ialldomain)then !if(j_nxrow(irow0).gt.0)then
						...
						j_err=.true.
						return
					endif !if(jlp_nxrow(irow0).gt.0)  17058
				elseif(irow0.eq.irowobj.and.k.eq.1)then !if(irow0.ne.irowobj)then
					jlp_irowrow(0)=irow0
					if(jlp_nxrow(irow0).gt.0)then
						jlp_ix(0)=itemp0
					...
					elseif(jlp_fpresent) then !if(j_nxrow(irow0).gt.0)then
						if(jlp_nfxrow(irow0)>0.or.jlp_nfyrow(irow0)>0) &
							jlp_irowdomain(0)=jlp_isetd(idomba+j)
					endif !if(jlp_nxrow(irow0).gt.0)  17074
					kobj=k
				endif !if(irow0.ne.irowobj)  17054
			enddo !k=1,jlp_nsetr(i)  17022
 
 
			idomba=idomba+1 !jlp_nsetd(i)
		enddo !i=1,nset  17018
 
	end subroutine
 
	subroutine initjlp2()
		write(6,*)'<345irowdomain',jlp_irowdomain
		...
		! if(irowj_+1.ne.irowrowj_) &
		! write(6,*) 'corresponds to row ', irowrowj_, ' in original problem definition'
		! endif !if(j_o(ivrhs)%d(irowrowj_)==j_ninf)  14579
		! endif !if (jlp_nfxrow(irowrowj_)==1)  14578
		! enddo !irowj_=1,jlp_nrow  14576
		! endif !if(jlp_fpresent.and..false.)  14575
		...
		endif !if(jlp_ntemp0.gt.j_maxjlpxvars)  17177
		!	write(6,*)'<87777'
		!tree
		! if(istree)then
		! if(allocated(iperiod))deallocate(iperiod)
		...
		! endif !if(i_.eq.0)  15343
		! endif !if(jlp_ix(i_).ne.0)  15341
		! enddo !i_=0,jlp_nrow  15340
		! call j_quick_sort(rperiod,iperiodperm)
		...
		if(allocated(jlp_keys))then
			if(warm)then
				...
				endif !if(size(jlp_keys).ne.jlp_nunits)  17231
			endif !if(warm)  17230
			if(.not.warm)deallocate(jlp_keys)
		endif !if(allocated(jlp_keys))  17229
		if(.not.warm)then
			allocate(jlp_keys(1:jlp_nunits));jlp_keys=0
		endif !if(.not.warm)  17241
		...
		if(ivarea.gt.0)then   ! area-> option present
			...
			endif !if(notareavars.gt.0)  17268
			if(ncvar.gt.0)then
				allocate(jlp_cvar(1:ncvar))
				needc=1 !make logical
			endif !if(ncvar.gt.0)  17290
 
		endif !if(ivarea.gt.0)  17266
		nstot=0  !total number of schedules
		!ibasclass=0  given earlier
		isc=0  ! have we starter the given class if class option is given
		jlp_ibaunitbas=0  ! basis for schedules, remains 0 if option class-> not given
		...
		if(isunit)then
			...
			do i=2,j_o(ivxdatmat)%i(1)
				...
				if(j_o(ivxdatmat)%d(ibas+inde).ne.old)then
					...
					iprev=i
				endif !if(j_o(ivxdatmat)%d(ibas+inde).ne.old)  17316
				ibas=ibas+keepx
			enddo !i=2,j_o(ivxdatmat)%i(1)  17314
			jlp_nsch(jlp_nunits)=j_o(ivxdatmat)%i(1)-iprev
			maxnsch=max(maxnch,jlp_nsch(jlp_nunits))
			!	write(6,*)'shed',jlp_nsch(1:jlp_nunits)
 
		endif !if(isunit)  17305
		...
		if(.true.)then !.not.j_feasible.or.iunit.eq.1)then !
				...
				if(jlp_ubou(irow)) then
					if(jlp_x(irow)+jlp_tole(irow).lt.jlp_rhscur(irow)-jlp_rhs2(irow))then
						...
						if(jlp_lower(irow))then
							...
							if(irow.eq.2)write(6,*)'irow',irow,'vaihdetaan ylärajaan',jlp_x(irow)
							if((jlp_ix(irow).ne.0).or.jlp_fpresent)jlp_rhsw(irow)=jlp_rhscur(irow)-jlp_xps(irow)
						endif !if(jlp_lower(irow))  17374
						jlp_value=jlp_value+jlp_x(irow)
						if(p.and.nnf.le.50)write(n16,*)'infer',irow,jlp_x(irow)
						jlp_tolecur=jlp_tolecur+jlp_tole(irow)
					endif !if(jlp_x(irow)+jlp_tole(irow).lt.jlp_rhscur(irow)-jlp_rhs2  17367
				endif !if(jlp_ubou(irow))  17366
				if(jlp_lbou(irow)) then
					if(jlp_x(irow)-jlp_tole(irow).gt.jlp_rhscur(irow)-jlp_rhs(irow))then
						! illegal surplus, decrease it
						jlp_objr2(irow)=jlp_onen
						...
						endif !if(.not.jlp_lower(irow))  17395
						if(p.and.nnf.le.50)write(n16,*)'infer',irow,jlp_x(irow)
						jlp_value=jlp_value-jlp_x(irow)
						jlp_tolecur=jlp_tolecur+jlp_tole(irow)
					endif !if(jlp_x(irow)-jlp_tole(irow).gt.jlp_rhscur(irow)-jlp_rhs(  17388
				endif !if(jlp_lbou(irow))  17387
				...
				if(jlp_feasible)then
					if(nnf.gt.1)then
						...
						if(p.or.p9)write(n16,*)'again infeasible row ',irowinf,'infeasibility  ', &
							...
							'ienter=' ,ienter,' leavec ',leavec, 'newc ', &
							newc,'emptmax,j_tmax', jlp_x(max(newc,1)),jlp_tmax
					endif !if(nnf.gt.1)  17423
					again=.true.
 
					nagain=nagain+1
				endif !if(jlp_feasible)  17422
				jlp_feasible=.false.
				jlp_objr=>jlp_objr2
				if(p)write(n16,*)'objr',jlp_objr(1:min(jlp_nrow,40)),'+++0'
				jlp_objf=jlp_value
					...
					if(again)then
						if(nagain.le.1)then
							...
							if(jlp_objf.gt.againsol+jlp_tolecur)then
								...
								endif !if(nagain.gt.7)  17491
 
							endif !if(jlp_objf.gt.againsol+jlp_tolecur)  17487
 
						endif !if(nagain.le.1)  17484
 
					endif !if(again)  17483
				endif !if(.not.jlp_feasible)  17472
 
				jlp_feasible=.true.
				j_v(ivfeasible)=1.
			endif !if(nnf.gt.0)  17420
			...
			if(justkey)then   !only key schedule has been changed
				...
				if(sparse)then
					...
					call tfbsub(jlp_nrow,jlp_a,jlp_lavec,0, &
						jlp_objr,jlp_vc,wslu1,lwsll1,jlp_apu,.false.)
				endif !if(sparse)  17526
			endif !if(justkey)  17521
			...
			if(jlp_objf.le.jlp_objfv-jlp_tolecur)then !100j_0*j_tolecur)then  ! 100 added Feb. 2011 cnhaged by JL 9.9.2018
				...
				if(nrecover.ge.20)then !10 changed into 20 by J.L 21.2.2019
					...
					j_err=.true.
					goto900=.true.;return
				endif !if(nrecover.ge.20)  17553
				goto1234=.true.;return
			endif !if(jlp_objf.le.jlp_objfv-jlp_tolecur)  17539
	...
	subroutine updatejlp()
		...
		endif !if(iunit.eq.iunit55)  17576
		...
		if(jlp_fpresent)then
			do ii_=jlp_mxd+1,jlp_lf0
				...
				!	write(16,*)'**negative amount ,pivot',ipivot,' unit ',iunit, ' ixk ',j_ixkf(j_lf(ii_)),&
				!	' ifact ',j_ixkffact(j_lf(ii_)),' amount ',j_x(j_lf(ii_)+j_nrowz)
			enddo !ii_=jlp_mxd+1,jlp_lf0  17618
 
 
		endif !if(jlp_fpresent)  17617
		!end new
				...
				if(jlp_lf0>jlp_mxd) then
					...
					do ii_=jlp_mxd+1,jlp_lf0
						write(n16,*) ii_,jlp_lf(ii_),jlp_lunit(jlp_lf(ii_)) ,&
							jlp_ixkf(jlp_lf(ii_)),jlp_ixkffact(jlp_lf(ii_)), &
							jlp_x(jlp_lf(ii_)+jlp_nrowz)
					enddo !ii_=jlp_mxd+1,jlp_lf0  17673
				endif !if(jlp_lf0>jlp_mxd)  17670
				...
				if(jlp_x(lcur+jlp_nrowz).lt.jlp_wminwrn)then  ! jl 201606033 j_zeroneg)then    !negative weight, zeroneg=-0.000000001d0
					...
					if(jlp_x(lcur+jlp_nrowz).lt.jlp_wminerr)then  !wminerr=-0.02d0 !!!!
						...
						if(nrecover.ge.10)then
							...
							j_err=.true.
							goto900=.true. ;return
						endif !if(nrecover.ge.10)  17727
 
						write(6,*)'***trying to recover'
						kierv=kier
						iunitv=iunit
						goto1234=.true.;return
					endif !if(jlp_x(lcur+jlp_nrowz).lt.jlp_wminerr)  17719
					...
					do ilr=lr0+1,jlp_nrow
						...
						if(abs(r(lcur+jlp_nrowz)).gt.jlp_vcmax)then !cycling
							if(jlp_lr(ilr).ne.icolold)then
								...
								cycling=.true.
							endif !if(jlp_lr(ilr).ne.icolold)  17774
						endif !if(abs(r(lcur+jlp_nrowz)).gt.jlp_vcmax)  17773
					enddo !ilr=lr0+1,jlp_nrow  17748
					if(jlp_vcmax.gt.jlp_tole(jlp_lr(ilrmax)))then
						...
						write(6,*)'did not remove w, was this due to cycling: ', &
							cycling,' vcmax ',jlp_vcmax,&
							' tole ',jlp_tole(jlp_lr(ilrmax))
					endif !if(jlp_vcmax.gt.jlp_tole(jlp_lr(ilrmax)))  17783
				endif !if(jlp_x(lcur+jlp_nrowz).lt.jlp_wminwrn)  17707
						...
						if(jlp_wsu.gt.jlp_wmaxerr)then
							if(sparse)then
								...
								if(jlp_wsu.gt.jlp_wmaxwrn)then
									...
									jlp_feasible=.false.
									goto1234=.true.;return
								endif !if(jlp_wsu.gt.jlp_wmaxwrn)  17813
							endif !if(sparse)  17809
						endif !if(jlp_wsu.gt.jlp_wmaxerr)  17808
						listapu(1)=lcur0+jlp_nrowz
						jlp_vcmax=jlp_zero
						do ilr=lr0+1,jlp_nrow
							if(sparse)then
							...
							if(abs(r(lcur0+jlp_nrowz)).gt.jlp_vcmax)then
								if(jlp_lr(ilr).ne.icolold)then
									...
									cycling=.true.
								endif !if(jlp_lr(ilr).ne.icolold)  17846
							endif !if(abs(r(lcur0+jlp_nrowz)).gt.jlp_vcmax)  17845
 
						enddo !ilr=lr0+1,jlp_nrow  17833
						if(jlp_vcmax.gt.jlp_tole(jlp_lr(ilrmax)))then
							newc=jlp_lr(ilrmax)
							leavec=lcur0+jlp_nrowz
							leave=jlp_lsi(leavec)
							...
							write(6,*)'did not remove w, was this due to cycling: ',cycling
						endif !if(jlp_vcmax.gt.jlp_tole(jlp_lr(ilrmax)))  17856
 
					endif !if(jlp_wsu.gt.jlp_oneps)  17804
 
					jlp_wsu=jlp_zero
 
					lcur0=jlp_next(lcur)
 
				endif !if(jlp_lunit(nex).ne.jlp_lunit(lcur))  17803
 
				lcur=nex
			enddo !j=1,jlp_ld0  17702
 
		endif !if(jlp_xpresent)  17697
	...
	subroutine repo(nureport)
		...
		if(jlp_maxo) then
			...
			j_v(ivobjective)=-jlp_objf
		endif !if(jlp_maxo)  17942
		if(ivoutresult.ne.j_ivresult) then
			call j_getobject(ivoutresult,'%objective',j_ipreal,ivobjective2)
			j_v(ivobjective2)=j_v(ivobjective)
			write(6,*)'OBJECTIVE ',jlp_objf,j_v(ivobjective2)
		endif !if(ivoutresult.ne.j_ivresult)  17947
901	continue
		...
		if(jlp_nrow.gt.0)then  !there are constraints
			do i=jlp_nrow+1,ncol+jlp_nrow
				jlp_x(jlp_ls(i))=j_0
				if(jlp_ls(i).eq.2)write(6,*)'<4774putzero'
			end do !i=jlp_nrow+1,ncol+jlp_nrow  17956
		endif !if(jlp_nrow.gt.0)  17955
		if(.not.jlp_maxo)then  ! poistettu bugia metsästettäessä
			jlp_vc=-jlp_vc                        !;objf=-objf done earlier
			jlp_vx=-jlp_vx
		end if !if(.not.jlp_maxo)  17961
...
877 continue !nrow=0
		...
		if(nz.gt.0) then
			...
			do i=lz0+1,nz
				...
				if(jlp_feasible)then
					...
					val_=0.
				endif !if(jlp_feasible)  17973
				do  j=1,jlp_nrow
					val_=val_-jlp_vc(j)*jlp_a(j,newa)
				enddo ! j=1,jlp_nrow  17978
				jlp_redcost(newa)=abs(val_)
			enddo !i=lz0+1,nz  17970
		endif !if(nz.gt.0)  17968
		if(j_err)return
		...
		if(ivoutresult.ne.j_ivresult) then
			...
			if(jlp_nrow>0) then
				...
				call j_defmatrix(ivoutresult,'%shprice',jlp_nrow,1,j_matreg,ivout)
				j_o(ivout)%d(1:jlp_nrow)=jlp_vc(1:jlp_nrow)
			endif !if(jlp_nrow>0)  17997
		endif !if(ivoutresult.ne.j_ivresult)  17986
		!write(6,*)'<47zmatrix',zmatrix
		if (zmatrix)then
			write(6,*)' '
			write(6,*)'***because there was zmatrix->, results are given in matrices***'
		...
		do i=1,nset
			...
			if(jlp_xpresent.and.jlp_ndom.gt.0)then
				...
				write(nureport,*)' ',('_',kk=1,78)
				write(nureport,'(a)')jlp_buf(1:79)
			end if !if(jlp_xpresent.and.jlp_ndom.gt.0)  18028
			write(nureport,*)' ',('_',kk=1,78)
			write(jlp_buf,66061)
66061 format('row',t38,'value',t50,'shadow',t61,  'lower',t70,'upper')
			write(nureport,'(a)')jlp_buf(1:79)
...
6606 format(t50,'price', t61,'bound',t70, 'bound')
				...
				if(irow0.ne.irowobj)then
					...
					if(j_v(ivfeasible)>0)then
						if(jlp_maxo)then
							...
							jlp_buf(1:5)=' min'
						end if !if(jlp_maxo)  18059
					else !if(j_v(ivfeasible)>0)then
						jlp_buf(1:5)=' '
						jlp_buf(6:33)='Infeasible, temporary object'
						!	le=
					endif !if(j_v(ivfeasible)>0)  18058
 
				endif !if(irow0.ne.irowobj)  18055
				!lyhennetään ylipitkät rivit etteivät sotkeennu
				le5=le+5
				iplus0=0
				!	istart00_=6
	...
	!	j_buf(34:35)=j_dots !chnaged by JL 10.9.2018
					if(le5.gt.iplus0+80)then
						do jj=iplus0+81,iplus0+1,-1
							if(jlp_buf(jj:jj).eq.'+'.or.jlp_buf(jj:jj).eq.'-')then
								iplus=jj-1
								exit
							endif !if(jlp_buf(jj:jj).eq.'+'.or.jlp_buf(jj:jj).eq.'-')  18080
						enddo !jj=iplus0+81,iplus0+1,-1  18079
					else !if(le5.gt.iplus0+80)then
						iplus=le5
					endif !if(le5.gt.iplus0+80)  18078
					write(nureport,'(a)')jlp_buf(iplus0+1:iplus)
					iplus0=iplus
				...
				elseif(iplus0.gt.0)then !if((le5-iplus0)>=34) then
					jlp_buf(1:le5-iplus0)=jlp_buf(iplus0+1:le5)
					jlp_buf(le5-iplus0+1:34)=jlp_dots
				endif !7531		if((le5-iplus0)>=34)  18076
				...
				if(irow0.ne.irowobj)then
					...
					if(jlp_vc(irow).ne.0.)then
						if(jlp_maxo.eqv.jlp_vc(irow).gt.0.)jlp_buf(78:78)='U'
						if(jlp_maxo.eqv.jlp_vc(irow).lt.0.)jlp_buf(78:78)='L'
					end if !if(jlp_vc(irow).ne.0.)  18108
					...
					end if !if(jlp_rhs(irow).eq.jlp_rhs2(irow))  18113
					...
					! j_buf(6:33)='Infeasible, temporary object'
					! endif !if(j_v(ivfeasible)>0)then
				end if !if(irow0.ne.irowobj)  18098
				write(nureport,'(a)')jlp_buf(1:79)
				if(intapp)then
					jlp_buf=' '
					...
					jlp_buf(36:)=j_chr10(jlp_solx(irow))
					write(nureport,'(a)')jlp_buf(1:79)
				endif !if(intapp)  18139
			end do !k=1,jlp_nsetr(i)  18048
			...
			if(jlp_xpresent.and.iprint.ge.1)then
				...
				do jx=1,jlp_nxvartot+ncvar+noutsubtrans
						...
						if((jlp_ndom.le.0.or.idom.eq.jlp_irowdomain(iro)).and.jlp_ix(iro).ne.0)then
							...
							do ii=1,jlp_nxrow2(item)
								...
								elseif(jx.le.jlp_nxvartot+ncvar) then !if(jx.le.j_nxvartot)then
									...
									jlp_yes=jlp_irowxvars(iba+ii).eq. &
										j_o(ivoutsubtrans)%i2(jx-jlp_nxvartot-ncvar)
								endif !if(jx.le.jlp_nxvartot)  18170
								if(jlp_yes)then   !häär
									if(iro.eq.0)then
										shp=jlp_coefx(iba+ii)
										...
										shp=shp-jlp_coefx(iba+ii)*jlp_vc(iro);indi=1
									endif ;exit !if(iro.eq.0)  18179
								endif !if(jlp_yes)  18178
							enddo !ii=1,jlp_nxrow2(item)  18168
 
						endif !if((jlp_ndom.le.0.or.idom.eq.jlp_irowdomain(iro)).and.jlp_  18164
 
					enddo !iro=0,jlp_nrow  18162
					if(jx.le.jlp_nxvartot)then
					...
					elseif(jx.le.jlp_nxvartot+ncvar) then !if(jx.le.j_nxvartot)then
						...
						if(jlp_ndom.le.1) j_v(j_o(ivoutsubtrans)%i2(jx-jlp_nxvartot-ncvar))= &
							jlp_sumx(jx)
					endif !if(jx.le.jlp_nxvartot)  18192
				...
				end do !jx=1,jlp_nxvartot+ncvar+noutsubtrans  18157
			end if !if(jlp_xpresent.and.iprint.ge.1)  18147
			!		end do domloop !domloop: do j=1,j_nsetd(i)
		end do !i=1,nset  18024
		!	jlp_nxvartot=j_o(jlp_ivkeepx)%i(1)
		!jlp_nsumx=jlp_nxvartot+ncvar+noutsubtrans
		!xvars%, xsum%, xprice%,xvarsproblem%
		if(jlp_xpresent.and.ivoutresult.ne.j_ivresult) then
			...
			endif !if(jlp_nrow.gt.0)  18257
		endif !if(jlp_xpresent.and.ivoutresult.ne.j_ivresult)  18230
		if(ivoutresult.ne.j_ivresult)then
			!call j_defmatrix(ivoutresult,'%problemrows',jlp_nrow,1,j_matreg,ivout)
			...
			if(jlp_nrow.gt.0)then
				...
				do i_=1,jlp_nrow
					if(jlp_rhs(i_).eq.j_ninf)j_o(ivout)%d(i_) =j_ninf
					if(jlp_rhs2(i_).eq.j_inf)j_o(ivout2_)%d(i_) =j_inf
				enddo !i_=1,jlp_nrow  18275
				call j_defmatrix(ivoutresult,'%shprice',jlp_nrow,1,j_matreg,ivout2_)
				j_o(ivout2_)%d = jlp_vc(1:jlp_nrow)
			endif !if(jlp_nrow.gt.0)  18268
 
		endif !if(ivoutresult.ne.j_ivresult)  18264
		if(allocated(xvars1))deallocate(xvars1,xvars2)
		allocate(xvars1(1:nxvar),xvars2(1:nxvar))
		...
		if(nz.gt.0.and.iprint.ge.1)then
			...
			DO 66778 I=1,NZ
				...
				jlp_buf(36:46)=j_chr10(jlp_redcost(i))
				write(nureport,'(a)')'     '//jlp_buf(1:46)
66778 continue !66778 I=1,NZ  18297
		endif !if(nz.gt.0.and.iprint.ge.1)  18289
...
8000 continue  !end printing
		...
		if(j_v(ivunbounded)>0) then
			jlp_buf='Unbounded problem'
			write(nureport,'(a)')jlp_buf(1:79)
		endif !if(j_v(ivunbounded)>0)  18312
		...
		if(j_v(ivfeasible)>0) then
			...
			jlp_buf='Value of the temporary objective: '
		endif !if(j_v(ivfeasible)>0)  18320
		jlp_buf(35:)=j_chr10(dble(jlp_objf))
		write(nureport,'(a)')jlp_buf(1:79)
		...
		if(j_v(ivfeasible)>0) then
			...
			jlp_buf='Solution is infeasible'
		endif !if(j_v(ivfeasible)>0)  18328
		write(nureport,'(a)')jlp_buf(1:79)
		!write(6,*)'OBJECTIVE&&&&& ',jlp_objf,j_v(ivobjective2)
		!	call j_getname(ivobjective2)
		!write(6,*)'%%%',j_oname(1:j_loname)
		if(j_v(ivoptimal)>0) then
			if(kier.ge.mxiter)then
				...
				jlp_buf='Solution is optimal'
			endif !if(kier.ge.mxiter)  18338
		else !if(j_v(ivoptimal)>0) then
			jlp_buf='Solution is not optimal'
		endif !if(j_v(ivoptimal)>0)  18337
		write(nureport,'(a)')jlp_buf(1:79)
		if(nureport.ne.6)write(6,*)'** report-> file remains open'
		return
		!if(nureport.ne.6)call j_closeunit(nureport)
	...
	subroutine renter()
		...
		if(niter.ge.10)then  !!!!
			...
			if(nnf.eq.nnfiter.and.jlp_objf-jlp_valiter.le.jlp_tolecur)then
				...
				niter=0
				jlp_valiter=jlp_objf
			endif !if(nnf.eq.nnfiter.and.jlp_objf-jlp_valiter.le.jlp_tolecur)  18363
		endif !if(niter.ge.10)  18360
		...
		if(ienterv.eq.1.and.nsame.gt.5.and.jlp_objf.lt.oldsol+jlp_tolecur)then
			if(iprint.gt.1)write(6,*)'cycling, Unit',iunit,'Pivot ',ipivot, 'Objective ',jlp_objf
			return    ! we check if z can enter z
		endif !if(ienterv.eq.1.and.nsame.gt.5.and.jlp_objf.lt.oldsol+jlp_  18395
			...
			if(jlp_rhs2(ico).gt.jlp_rhs(ico))then
				...
				if(jlp_vc(ico).gt.jlp_vcmax.and.jlp_lower(ico).and.ico.ne.icolold)then
					...
					if(jlp_ubou(ico).and.jlp_lbou(ico)) then
						...
						jlp_tmax=j_inf  ;rcur=jlp_tmax
					endif !if(jlp_ubou(ico).and.jlp_lbou(ico))  18419
					jlp_post=.false.  !post=positive residual could enter
					! xma ei käy koska voidaan ???
					! tmin=xmi(newc) !min value for the entering
					!					goto 100
					...
					if(jlp_ubou(ico).and.jlp_lbou(ico)) then
						...
						jlp_tmax=j_inf ;rcur=jlp_tmax
					endif !if(jlp_ubou(ico).and.jlp_lbou(ico))  18440
					jlp_vcmax=-jlp_vc(ico)
					jlp_post=.true.
					if(p) write(n16,*)'posit resid could enter',ico  !!!!
					if(p.and.jlp_fpresent) write(n16,*)'**fact** KANTAAN >> residual, ienter = 1'
				end if !if(jlp_vc(ico).gt.jlp_vcmax.and.jlp_lower(ico).and.ico.ne.  18411
			end if !if(jlp_rhs2(ico).gt.jlp_rhs(ico))  18406
		end do !ilr=lr0+1,jlp_nrow  18400
		...
		do ilz=lz0+1,nz
			...
			if(sparse)then
				...
				do jj=jlp_lavecsp(jlp_lavecsp(0)+newa0),last(newa0)
					iel=iel+1
					val_=val_-jlp_vc(jlp_lavecsp(jj))*jlp_a(iel,newa0)
				enddo !jj=jlp_lavecsp(jlp_lavecsp(0)+newa0),last(newa0)  18471
			else !if(sparse)then
				do  j=1,jlp_nrow
					val_=val_-jlp_vc(j)*jlp_a(j,newa0)
				enddo ! j=1,jlp_nrow  18476
			endif !if(sparse)  18469
			! ei ny otettu huomioon mahdollisuutta, että z voisi olla nyt ylärajalla
			! ja jos se tulee kantaan negatiivisena, niin tavoite voiis kasvaa
			!		if(val_.gt.epsj)then
			if(val_.gt.jlp_vcmax)then
				...
				jlp_tmax=j_inf ;rcur=jlp_tmax
				ilzmax=ilz  !	do ilz=lz0+1,nz
			endif !if(val_.gt.jlp_vcmax)  18483
		enddo !ilz=lz0+1,nz  18458
	...
	subroutine senter()
		...
		if(jlp_xpresent)then  !newprice   !B31>
			...
			if(ix0.ne.0)then !ix(0).ne.0.and.feasible
				...
				jlp_vx(0)=0.
			endif !if(ix0.ne.0)  18502
			do i=1,jlp_nrow
				if(jlp_ix(i).ne.0)then
					jlp_vx(i)=-jlp_vc(i)   ! formula 6.41, voitasiko kenties tästä merkinkäännöstä
					! luopua ja pelata suoraan vc:n avulla
				endif !if(jlp_ix(i).ne.0)  18508
			enddo !i=1,jlp_nrow  18507
			if(p)  write(n16,'(a,(1x,5g19.7))')'vx',(jlp_vx(i),i=0,min(jlp_nrow,50))
		endif !if(jlp_xpresent)  18500
		...
		do jj=1,jlp_nrowp
			...
			if(jlp_vx(j).ne.0.)then
				jlp_vxpack(jlp_ix(j))=jlp_vxpack(jlp_ix(j))+jlp_vx(j)
				jlp_ixpack(jlp_ix(j))=jlp_ix(j)   ! result (1*ind1,2*ind2,....)
			endif !if(jlp_vx(j).ne.0.)  18537
		enddo !jj=1,jlp_nrowp  18535
		nxbas=count(jlp_ixpack.ne.0)
		! Jos tehtaita, ei ohiteta
		if(nxbas.le.0.and..not.jlp_fpresent)then
			goto400=.true.;return   !!!! next unit subroutine
		endif !if(nxbas.le.0.and..not.jlp_fpresent)  18544
		if(p)then
			write(n16,*)'nonzero prices:', nxbas, 'ix0,feasible,vx(0)',ix0,jlp_feasible,jlp_vx(0)
			write(n16,*)'ixpack',jlp_ixpack,jlp_ixpack.ne.0
			write(n16,*)'vxpack',jlp_vxpack
		...
		do icin=1,jlp_ntemp0                !remove zero-elements
			if(jlp_ixpack(icin).ne.0)then
				...
				jlp_vxpack2(icout)=jlp_vxpack(icin)
				jlp_ixpack2(icout)=jlp_ixpack(icin)
			endif !if(jlp_ixpack(icin).ne.0)  18554
		enddo !icin=1,jlp_ntemp0  18553
		...
		! idiff3(jj_)=ixtree_
		! cycle ulloop
		! endif !if(jlp_ixpack2(jj2_).eq.iperiodperm(jj_))  18628
		! enddo !jj2_=1,icout  18627
		! idiff3(jj_)=ixtree_+1
		! enddo ulloop !oop:	do jj_=1,jlp_ntemp0  18626
			...
			do jj=1,jlp_nrowpf !!!! rivit, joilla xk-muuttujia
				...
				do k_=1,jlp_nfxrow(irowj_) ! silmukka : rivin xk-muuttujat
					if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)).eq. &
							jlp_irowffact(jlp_ibafx(irowj_)+k_)) then
						nvarxk = nvarxk + 1
					endif !if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj_)+k_))  18594
				enddo !k_=1,jlp_nfxrow(irowj_)  18593
			enddo !jj=1,jlp_nrowpf  18590
				...
				do k_=1,jlp_nfyrow(irowj_) ! silmukka : rivin yk-muuttujat
					...
					do ivars_=1,j_o(listy_)%i(1) ! yk-mjan puutavaralistan muuttujat
						...
						do ifact_=1,j_o(listf_)%i(1) ! yk-mjan tehdaslistan tehtaat
							...
							if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_) then
								nvaryk = nvaryk + 1
							endif !if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_)  18613
						enddo !ifact_=1,j_o(listf_)%i(1)  18610
					enddo !ivars_=1,j_o(listy_)%i(1)  18607
				enddo !k_=1,jlp_nfyrow(irowj_)  18604
			enddo !jj=1,jlp_nrowpfy  18601
 
			if(allocated(jlp_rowxkfkey))then
				if(size(jlp_rowxkfkey).lt.nvarxk)deallocate(jlp_rowxkfkey)
			endif !if(allocated(jlp_rowxkfkey))  18621
			if(.not.allocated(jlp_rowxkfkey))allocate(jlp_rowxkfkey(1:nvarxk))
 
			if(allocated(jlp_rowykfkey))then
				if(size(jlp_rowykfkey).lt.nvaryk)deallocate(jlp_rowykfkey)
			endif !if(allocated(jlp_rowykfkey))  18626
			if(.not.allocated(jlp_rowykfkey))allocate(jlp_rowykfkey(1:nvaryk))
			...
			do jj=1,jlp_nrowpf ! rivit, joilla xk-muuttujia
				...
				do k_=1,jlp_nfxrow(irowj_) ! silmukka : rivin xk-muuttujat
					if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)).eq. &
						...
						jlp_rowxkfkey(nrowxkfkey)%irowfx = jlp_ibafx(irowj_)+k_
						jlp_rowxkfkey(nrowxkfkey)%jcurix = jcurix_
					endif !if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj_)+k_))  18638
				enddo !k_=1,jlp_nfxrow(irowj_)  18637
			enddo !jj=1,jlp_nrowpf  18634
					...
					do ivars_=1,j_o(listy_)%i(1) ! yk-mjan puutavaralistan muuttujat
						...
						do ifact_=1,j_o(listf_)%i(1) ! yk-mjan tehdaslistan tehtaat
							...
							if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_) then
								...
								jlp_rowykfkey(nrowykfkey)%iv2elpos = iv2elpos_
								jlp_rowykfkey(nrowykfkey)%jcurix = jcurix_
							endif !if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_)  18658
						enddo !ifact_=1,j_o(listf_)%i(1)  18656
					enddo !ivars_=1,j_o(listy_)%i(1)  18653
				enddo !k_=1,jlp_nfyrow(irowj_)  18650
			enddo !jj=1,jlp_nrowpfy  18647
 
		endif !if (jlp_fpresent)  18583
 
		!!!!laskentayksikkö vaihtunut -> lasketaan tehdas-y-mjien muunnokset
		if (nfy.gt.0.and.iunit.ne.iunitrans) then
			do j=1,j_o(jlp_ivkeepc)%i(1)
				j_v(j_o(jlp_ivkeepc)%i2(j))=j_o(ivcmat)%d((iunit-1)*j_o(jlp_ivkeepc)%i(1)+j)
			enddo !j=1,j_o(jlp_ivkeepc)%i(1)  18673
			do j=1,ntrans
				call dotrans(jlp_itransv(j),1)
				if(j_err)then
					write(6,*)'err for trans ',j
					stop 975
				endif !if(j_err)  18678
			enddo !j=1,ntrans  18676
			iunitrans = iunit
		endif !if (nfy.gt.0.and.iunit.ne.iunitrans)  18672
		! end preparing for factories
		...
		nschloop:	do i=1,jlp_nsch(iunit)
			...
			if(fastnow)then
				if(fastreject(iobs).and.i.ne.jlp_keys(iunit))then
					svalue(i)=j_ninf
					cycle  !i.ne.j_keys(iunit) added 20.8.2018 JL
				endif !if(fastreject(iobs).and.i.ne.jlp_keys(iunit))  18734
			endif !if(fastnow)  18733
			if(subfilre)then
				if(jlp_rejects(iobs))then
					! if(istree)then
					! ibxmatx=xmatiba(iobs) !,1)
					...
					svalue(i)=j_ninf
					cycle
				endif !if(jlp_rejects(iobs))  18740
			endif !if(subfilre)  18739
			...
			if (jlp_xpresent2) then
				...
				svalue(i)=j_0   !jlp_value = j_0
			endif !if (jlp_xpresent2)  18754
			if(p)write(16,*)'<6712>,nxbas,iunit,i,initval',nxbas,iunit,i,jlp_value
			!!!jos tehtaita, valuen arvoa kasvatetaan
			if (jlp_fpresent) then !!!!
				!		jlp_value = jlp_value + value_f(iobs)
				svalue(i) = svalue(i)+ value_f(iobs)
			endif !if (jlp_fpresent)  18780
			...
			if(fastmake)then
				fastvalues(i)=svalue(i)  !jlp_value
				if(svalue(i).lt.fastvaluemin)fastvaluemin=svalue(i) !jlp_value
			endif !if(fastmake)  18801
 
		enddo nschloop !hloop:	do i=1,jlp_nsch(iunit)  18725
		!$OMP END PARALLEL DO
		jlp_valuek=svalue(jlp_keys(iunit))
		loco=maxloc(svalue(1:jlp_nsch(iunit))) !output must be array
		iopt=loco(1)
		...
		if(fastmake)then
			...
			do i=1,jlp_nsch(iunit)
				if(fastvalues(i).lt.fastcut.and.i.ne.jlp_keys(iunit))fastreject(jlp_ibaunit(iunit)+i)=.true.
				!	if(fastvalues(i).lt.fastcut)write(16,*)'reject',iunit,i
			enddo !i=1,jlp_nsch(iunit)  18818
			! if(.not.j_xmatinmemory)then !pack used observations in memory
			! iba=j_xmatlast*j_ntemp0 !to which place are putted
			! !	write(16,*)'alkuiba',iba,j_xmatlopp,j_xmatlast
			! do i=1,j_nsch(iunit)
	...
	subroutine tulostele()
		...
		endif !if(jlp_feasible)  18916
 
		call defsolu()
		if(jlp_xpresent)then
 
			call getsolx() !jlp_nunits,ibaunit,keys,lunit,nsch,isch,
		endif !if(jlp_xpresent)  18923
		! get the solution ready to be accesed by the inquiry routines
		...
		if(printlf.or.p9)then
			...
			do lfj_=jlp_mxd+1,jlp_lf0
				write(6,*) jlp_lunit(jlp_lf(lfj_)),jlp_ixkf(jlp_lf(lfj_)), &
					jlp_ixkffact(jlp_lf(lfj_)),jlp_keyfact(jlp_lunit(jlp_lf(lfj_)),&
					jlp_ixkf(jlp_lf(lfj_))),jlp_x(jlp_nrowz+jlp_lf(lfj_))
			enddo !lfj_=jlp_mxd+1,jlp_lf0  18932
		endif !if(printlf.or.p9)  18929
	...
	subroutine tulostele2() !prints and checkes if finished
		...
		if(jlp_feasible)then
				...
				if(coefmax*jlp_objfprev.le.j_0)then
					...
					if(isstop.and.kier.gt.10)then
						...
						if(j_codevalue(iob,jlp_stoplink).ne.j_0)then
							...
							goto900=.true.
							return
						endif !if(j_codevalue(iob,jlp_stoplink).ne.j_0)  18987
						...
						if(pros.lt.0.01.and.kier.ge.30)then
							...
							goto900=.true.
							return
						endif !if(pros.lt.0.01.and.kier.ge.30)  18994
 
					endif !if(isstop.and.kier.gt.10)  18981
				endif !if(coefmax*jlp_objfprev.le.j_0)  18958
			...
			endif !if(kier.le.10.or.int(kier/10)*10.eq.kier)  18951
			if(fast)then
				re=count(fastreject)
				!if(pp)write(16,*)'rejpros',re/j_lopp,fastusesame,fastusedsameFround,non
				fastusedsame=fastusedsame+1
				...
				endif !if(fastusedsame.gt.fastusesame)  19017
			...
			do i=1,lr0 !lr0 = number of basic residuals
				...
				elseif(.not.jlp_lower(jlp_lr(i)).and.jlp_x(jlp_lr(i)).lt.-jlp_tole(jlp_lr(i))) then !if(j_lower(j_lr(i)).and.j_x(j_lr(i)).gt.j_tole(j_lr(i))) then
					numn=numn+1
					jlp_value=jlp_value+jlp_x(jlp_lr(i))
				endif !if(jlp_lower(jlp_lr(i)).and.jlp_x(jlp_lr(i)).gt.jlp_tole(j  19073
			enddo !i=1,lr0  19071
			if(kier.le.10.or.int(kier/10)*10.eq.kier)then
				as=100.
				if(fast)as=100.*(1.-1.*count(fastreject)/jlp_lopp) !laske muualla
			...
			endif !if(kier.le.10.or.int(kier/10)*10.eq.kier)  19081
		endif !if(jlp_feasible)  18950
		! if(kier.ge.mxiter)then
		! write(6,*)'**maxiter reached'
		! iunitv=iunitprev
		! goto785=.true.;return
		...
		if(((jlp_objf.le.jlp_xirowold2+jlp_tolecur).or.(slow.gt.j_0.and.&
			...
			endif !if((fastusesame.ne.1.or.iterxkf.ne.1).and..not.tried)  19115
			...
			if(jlp_feasible)then
				j_v(ivfeasible)=1.
				j_v(ivoptimal)=1.
			endif !if(jlp_feasible)  19173
 
			goto900=.true.;return   ! <b332>
		endif !if(((jlp_objf.le.jlp_xirowold2+jlp_tolecur).or.(slow.gt.j_  19100
		...
		do ixkyk_=ixkyk1_,j_o(jlp_ivxkyk)%i(1)	!!!!	xkyk-lista
			...
			do inf_=1, jlp_nxkfact(ixkyk_)
				...
				do j=jlp_xkfact(ixkyk_,inf_)%i1xkykrowvar,jlp_xkfact(ixkyk_,inf_)%inxkykrowvar
					...
					if(jlp_irow2curix(0,jlp_xkykrowvars(j)%irow).ne.1)then
						write(6,*)'<fe33>,j',j
						stop 987
					endif !if(jlp_irow2curix(0,jlp_xkykrowvars(j)%irow).ne.1)  19272
					!do jc_ = 1,j_irow2curix(0,j_xkykrowvars(j)%irow)	! alkup. riviä vastaavat lavennetut rivit
					...
					endif !if(jcurix_.ne.jcurix2)  19282
					!if(j_feasible)write(16,*)'<fenter13>jcurix_,', jcurix_ ,j_xkykrowvars(j)%irow,j_irow2curix(jc_,j_xkykrowvars(j)%irow)
					...
					if (jcurix_ == 0) then
						...
						if (jlp_feasible) then
							if (jlp_xkykrowvars(j)%isxk) then
								...
								!	if(j_feasible)write(n16,*)'v(xkykrowvars(j)%ind)',j_v(j_xkykrowvars(j)%ind)
								jlp_value = jlp_value +  j_v(jlp_xkykrowvars(j)%ind)
							endif !if (jlp_xkykrowvars(j)%isxk)  19292
						else !if (j_feasible) then
							!meidän pitää laskea miten temporary tavoitefunktio riippuu xkf-muuttujan arvosta. On käytävä läpi kaikki rajoiterivit
							! ja katsottava onko rajoite infeasible ja jos on on poimitava xkf-muuttujan kerroint
 
						endif !if (jlp_feasible)  19291
					else !if (jcurix_ == 0) then
						if (jlp_xkykrowvars(j)%isxk) then
							!if(j_feasible)write(16,*)'<fent22>',j_value,j_vc(jcurix_),j_coeffx(j_xkykrowvars(j)%ind)
							jlp_value = jlp_value - jlp_vc(jcurix_)*jlp_coeffx(jlp_xkykrowvars(j)%ind)
							...
							!	if(j_feasible)	write(16,*)'<fent33>',j_value,j_vc(jcurix_),j_v(j_xkykrowvars(j)%ind)
							jlp_value = jlp_value - jlp_vc(jcurix_)*j_v(jlp_xkykrowvars(j)%ind)
						endif !if (jlp_xkykrowvars(j)%isxk)  19307
					...
					endif !if (jcurix_ == 0)  19289
 
					!enddo !do jc_ = 1,j_irow2curix(0,j_xkykrowvars(j)%irow)
				enddo !j=jlp_xkfact(ixkyk_,inf_)%i1xkykrowvar,jlp_xkfact(ixkyk_,i  19269
 
				if(jlp_value>jlp_valueopt.and.junk_>0) then  !!!!
					jlp_valueopt=jlp_value
					ifopt = if_
					infopt=inf_
				endif !if(jlp_value>jlp_valueopt.and.junk_>0)  19332
 
				if (if_.eq.jlp_keyfact(iunit,ixkyk_)) then
					jlp_valuek = jlp_value
					infkey=inf_
				endif !if (if_.eq.jlp_keyfact(iunit,ixkyk_))  19338
 
			enddo !inf_=1, jlp_nxkfact(ixkyk_)  19256
			!	- jos ifopt == avaintehdas => seuraava xkyk-alkio
			! - jos valueopt.le.valuek+tolecur => seuraava xkyk-alkio
			if(p) write(n16,*)'<6682> ifopt,keyfact(iunit,ixkyk_),valueopt,valuek,tolecur',&
				ifopt,jlp_keyfact(iunit,ixkyk_),jlp_valueopt,jlp_valuek,jlp_tolecur
			if((ifopt/=jlp_keyfact(iunit,ixkyk_)).and.(jlp_valueopt>(jlp_valuek+0.01*jlp_tolecur))) then
				...
				if(isxkzero(ixkykenter))then !entering variable is zero in all schedules of the solution
					...
					nkeyf=nkeyf+1
					goto5316=.true.;return ! changed by JL 11.9.2018 was goto 55
				endif !if(isxkzero(ixkykenter))  19354
				...
				do j=jlp_xkfact(ixkyk_,infopt)%i1xkykrowvar,jlp_xkfact(ixkyk_,infopt)%inxkykrowvar
					do jc_ = 1,jlp_irow2curix(0,jlp_xkykrowvars(j)%irow)	! alkup. riviä vastaavat lavennetut rivit
						...
						if (jlp_xkykrowvars(j)%isxk) then
							...
							!	if(jcurix_.eq.3)write(16,*)'hier2',j_valueopt_af(jcurix_), j_v(j_xkykrowvars(j)%ind)
							jlp_valueopt_af(jcurix_) = jlp_valueopt_af(jcurix_) + j_v(jlp_xkykrowvars(j)%ind)
						endif !if (jlp_xkykrowvars(j)%isxk)  19367
					end do !jc_ = 1,jlp_irow2curix(0,jlp_xkykrowvars(j)%irow)  19364
				enddo !j=jlp_xkfact(ixkyk_,infopt)%i1xkykrowvar,jlp_xkfact(ixkyk_  19363
				...
				do j=jlp_xkfact(ixkyk_,infkey)%i1xkykrowvar,jlp_xkfact(ixkyk_,infkey)%inxkykrowvar
					do jc_ = 1,jlp_irow2curix(0,jlp_xkykrowvars(j)%irow)	! alkup. riviä vastaavat lavennetut rivit
						...
						if (jlp_xkykrowvars(j)%isxk) then
							...
							jlp_valuek_af(jcurix_) = jlp_valuek_af(jcurix_) + j_v(jlp_xkykrowvars(j)%ind)
						endif !if (jlp_xkykrowvars(j)%isxk)  19384
					end do !jc_ = 1,jlp_irow2curix(0,jlp_xkykrowvars(j)%irow)  19381
				enddo !j=jlp_xkfact(ixkyk_,infkey)%i1xkykrowvar,jlp_xkfact(ixkyk_  19380
				...
				goto100=.true.;return !!!! poistutaan xkyk-silmukasta & siirrytään tutkimaan leaving variablea
			endif !if((ifopt/=jlp_keyfact(iunit,ixkyk_)).and.(jlp_valueopt>(j  19348
 
		enddo !ixkyk_=ixkyk1_,j_o(jlp_ivxkyk)%i(1)  19249
		goto400=.true. !!!!siirrytään seuraavaan laskentayksikköön
		!endif !if (j_fpresent) then
	...
	subroutine entercol()
		...
		if(jlp_ixcur(0).ne.0)then
			...
			j1=1
			jlp_objr0(newc)=0.
		endif !if(jlp_ixcur(0).ne.0)  19453
		...
		if(jlp_fpresent) then
			...
			if((jlp_nrowpf.gt.0.and.jlp_ifxcurrows(1).eq.0).or.(jlp_nrowpfy.gt.0.and. jlp_ifycurrows(1).eq.0)) then
				!objr0:n päivitys
				jlp_objr0(newc)=jlp_objr0(newc)+jlp_valueopt_af(0)
			endif !if((jlp_nrowpf.gt.0.and.jlp_ifxcurrows(1).eq.0).or.(jlp_nr  19464
		endif !if(jlp_fpresent)  19462
		...
		if(sparse)then
			...
			do jj=j1,jlp_nrowp
				...
				if(abs(jlp_apu).gt.jlp_tiny78)then
					...
					jlp_a(iel,newa)=jlp_apu
					jlp_lavecsp(istart1+iel)=j
				endif !if(abs(jlp_apu).gt.jlp_tiny78)  19480
 
				!robleema, jos pitää vaihtaa
			enddo !jj=j1,jlp_nrowp  19476
			...
			do j=1,jlp_nrow
				if(jlp_ixcur(j).ne.0)then
					...
					jlp_a(j,newa)=0.
				endif !if(jlp_ixcur(j).ne.0)  19494
			enddo !j=1,jlp_nrow  19493
 
			!a:n päivitys
			if(jlp_fpresent) then
				jlp_a(1:jlp_nrow,newa) = jlp_a(1:jlp_nrow,newa)+ jlp_valueopt_af(1:jlp_nrow)
			endif !if(jlp_fpresent)  19502
 
		endif !if(sparse)  19470
		! get optimal schedule, put differences into objr and a
	...
	subroutine leaving()
		...
		if(ienter.eq.ienterv.and.	jlp_objf.lt.oldsol+jlp_tolecur)then
			...
			oldsol=jlp_objf
			nsame=0
		endif !if(ienter.eq.ienterv.and.	jlp_objf.lt.oldsol+jlp_tolecur)  19530
		...
		if(sparse)then
			...
			if(ienter.gt.1)then  !!!!
				...
				call fbsub(jlp_nrow,1,jlp_nrow,jlp_a,jlp_lavec,newc,jlp_a(1:,newa),r, & !!!!
					jlp_ls,wslu1,lwsll1,.false.)   !!!!
			endif !if(ienter.gt.1)  19584
		endif !if(sparse)  19567
		...
		if(.not.jlp_post)then
			do jj=1,jlp_nrow
				r(jlp_ls(jj))=-r(jlp_ls(jj))
			enddo !jj=1,jlp_nrow  19602
			jlp_post=.true.   !!!!
		endif !if(.not.jlp_post)  19601
		...
		do j=1,lz0  !!!! z leaving
			...
			if(r(leaz).gt.jlp_tiny78)then
				if(jlp_x(leaz).lt.jlp_tmax*r(leaz))then
					jlp_tmax=jlp_x(leaz)/r(leaz) ;rcur=r(leaz)
					leavec=leaz
				endif !if(jlp_x(leaz).lt.jlp_tmax*r(leaz))  19660
			endif !if(r(leaz).gt.jlp_tiny78)  19659
		enddo !j=1,lz0  19656
			...
			do j=1,jlp_ld0  !!!!sched leaving
				...
				if(r(lead).gt.jlp_tiny78)then     !
					if(jlp_x(lead).lt.jlp_tmax*r(lead))then
						jlp_tmax=jlp_x(lead)/r(lead)  ;rcur=r(lead)
						leavec=lead  !!!!
					endif !if(jlp_x(lead).lt.jlp_tmax*r(lead))  19672
 
				endif !if(r(lead).gt.jlp_tiny78)  19671
			enddo !j=1,jlp_ld0  19668
				...
				do j=jlp_mxd+1,jlp_lf0
					...
					if(r(leaf).gt.jlp_tiny78)then
						if(jlp_x(leaf).lt.jlp_tmax*r(leaf))then
							...
							leavec=leaf
							if(p) write(n16,*)'**fact** <6151> tmax päivitetty', jlp_tmax
						endif !if(jlp_x(leaf).lt.jlp_tmax*r(leaf))  19687
 
					endif !if(r(leaf).gt.jlp_tiny78)  19686
				enddo !j=jlp_mxd+1,jlp_lf0  19683
			endif !if (jlp_fpresent)  19681
		endif !if(jlp_xpresent2)  19667
...
2244	if(jlp_tmax.le.jlp_tmaxmin)then  !!!!! jos mukaan, pitää tsekata keyt ensin
			...
			if(p.or.p9.or.jlp_tmax.eq.-1.d0)then
				write(n16,*)'jump88,tmax, post,pivot=',jlp_tmax,jlp_post,ipivot,' ienter ',ienter
			endif !if(p.or.p9.or.jlp_tmax.eq.-1.d0)  19702
			if(ienter.eq.3.and.secondb.gt.jlp_valuek+jlp_tolecur)then
				if(p.or.jlp_tmax.eq.-1.d0)write(16,*)'secondb0 ', &
					secondb,jlp_valueopt,jlp_valuek,ipivot,iunit
				iopt=isecond
				...
				secondb=jlp_small
				goto222=.true.;return
			endif !if(ienter.eq.3.and.secondb.gt.jlp_valuek+jlp_tolecur)  19705
			if(ienter.eq.3)call lcursam()
			goto 8883 !!!!! jump88 if j_tmax.le.j_tmaxmin before checking f residual can leave
		endif !2244	if(jlp_tmax.le.jlp_tmaxmin)  19700
				...
				if(jlp_ubou(jlp_lr(j)))then
					...
					if(jlp_lower(jlp_lr(j)).and.jlp_x(jlp_lr(j))+ &
						...
						if(jlp_x(jlp_lr(j)).lt.jlp_tmax*r(jlp_lr(j)) )then
							...
							if(p)write(n16,*)'post,posr,ubou,NOTlower,lr(j),tmax',jlp_lr(j),jlp_tmax
							leavec=jlp_lr(j)
						end if !if(jlp_x(jlp_lr(j)).lt.jlp_tmax*r(jlp_lr(j)) )  19804
					elseif(jlp_x(jlp_lr(j)).lt.j_0.and..not.jlp_lower(jlp_lr(j)).and.jlp_feasible)then !if(j_lower(j_lr(j)).and.j_x(j_lr(j))+j_rhs2(j_lr(j))-j_rhscur(j_lr(j)).
					...
					end if !if(jlp_lower(jlp_lr(j)).and.jlp_x(jlp_lr(j)  19788
				end if !if(jlp_ubou(jlp_lr(j)))  19776
				...
				if(jlp_lbou(jlp_lr(j)))then
					...
					elseif(jlp_x(jlp_lr(j)).le.j_0.and.jlp_lower(jlp_lr(j)))then !if(.not.j_lower(j_lr(j)).and.j_x(j_lr(j))+j_rhs(j_lr(j))-j_rhscur(j_lr(j)).gt.r(j_lr(j))*j_tm
						...
						if(jlp_x(jlp_lr(j)).gt.jlp_tmax*r(jlp_lr(j)) )then  !-x/-r <tmax-tole -x<-r*(tmax-tole)
							...
							!if(pp)write(n16,*)'post,negr,ubou,lower,lr(j),tmax',j_lr(j),j_tmax
							leavec=jlp_lr(j)
						end if !if(jlp_x(jlp_lr(j)).gt.jlp_tmax*r(jlp_lr(j)) )  19878
					...
					elseif(jlp_x(jlp_lr(j)).gt.j_0.and.jlp_lower(jlp_lr(j)).and.jlp_feasible)then !if(.not.j_lower(j_lr(j)).and.j_x(j_lr(j))+j_rhs(j_lr(j))-j_rhscur(j_lr(j)).g
						...
						return
						!	goto 2244
					endif !if(.not.jlp_lower(jlp_lr(j)).an  19863
 
				end if !if(jlp_lbou(jlp_lr(j)))  19861
 
			end if !if(r(jlp_lr(j)).gt.jlp_tiny78)  19735
 
		end do !j=1,lr0  19720
		...
		if(leavec.gt.0)then  !!!!!
			...
			if (p.and.jlp_fpresent.and.(leavec > (jlp_nrowz+jlp_mxd))) then
				...
				'>> xkf: unit,ixkyk,ifact,xkf,fact', &
					...
					j_vname(j_o(jlp_ivxkyk)%i2(jlp_ixkf(leavec-jlp_nrowz))), &
					j_vname(j_o(jlp_ivfact)%i2(jlp_ixkffact(leavec-jlp_nrowz)))
			endif !if (p.and.jlp_fpresent.and.(leavec > (jlp_nrowz+jlp_mxd)))  19924
 
		else !if(leavec.gt.0)then
			if(p)write(n16,*)'*no basic variable leaving'  !!!!
		endif !if(leavec.gt.0)  19917
		!if(pp)write(16,*)'kkkoi'
8883	if(.not.jlp_xpresent)goto 8888  !!!!! check if key schedule is leaving
		...
		do i=1,jlp_ld0
			...
			if(jlp_lunit(nex).ne.jlp_lunit(lcur))then
				...
				if(jlp_lunit(lcur).eq.iunit.and.ienter.eq.3)then
					...
					! store link to be used so that we can put the entering
					! column together with other columns isn the next-sequence
				endif !if(jlp_lunit(lcur).eq.iunit.and.ienter.eq.3)  19977
				...
				if(jlp_rs.lt.jlp_tiny78n.and.jlp_wsu.gt.jlp_tmax*jlp_rs)then !!!!
					...
					! leavk is the first D-column in unit for whcik key is leaving
					!					end if
				endif !if(jlp_rs.lt.jlp_tiny78n.and.jlp_wsu.gt.jlp_tmax*jlp_rs)  19985
 
				jlp_wsu=jlp_onen   !opposite sign of ws as in old JLP
				jlp_rs=jlp_zero
				lcur0=jlp_next(lcur)   ! lcur0 is again the first col in the next new unit
				! last
			endif !if(jlp_lunit(nex).ne.jlp_lunit(lcur))  19974
			lcur=nex
		enddo !i=1,jlp_ld0  19967
		! ! end checking if key-schedule is leaving , if key is leving leavk.gt.
8889 continue   !!!!! we come here if w.gt.1
...
!!!!!Avaintehdas vaihtuu, versio 2 (tarkastellaan vaihtuuko avaintehdas)
		if(jlp_fpresent) then
			...
			do ixkyk_=1,j_o(jlp_ivxkyk)%i(1)
				...
				do while (lcur0_unit > jlp_mxd)
					...
					if ((ienter==4).and.(iunit==lunit_).and.(ixkyk_==ixkykenter)) then
						...
						if(p) write(n16,*)'**fact** <6623> kantasrk:een xkf tulossa kantaan: iunit,ixkyk,lcursamef',&
							lunit_,ixkyk_,lcursamef
					endif !if ((ienter==4).and.(iunit==lunit_).and.(ixkyk_==ixkykente  20034
					...
					do while (jlp_lunit(lcur_) == jlp_lunit(lcur0_unit))
						...
						if(p)write(16,*)'h12',jlp_wsu
						lcur_ = jlp_nextf(lcur_,ixkyk_)
					enddo !while (jlp_lunit(lcur_) == jlp_lunit(lcur0_unit))  20044
					...
					endif !if ((ienter==3).and.(iunit==lunit_))  20052
					...
					do while((jlp_lunit(jlp_lunw(lun_))/=lunit_).and.(lun_<=jlp_lunits0))
						lun_=lun_+1
					enddo !while((jlp_lunit(jlp_lunw(lun_))/=lunit_).and.(lun_<=jlp_l  20072
					!kannassa yksikön ve-srkeita
					lcur0_w = 0
					if(lun_ <= jlp_lunits0) then
						...
						do while (jlp_lunit(lcur_w) == jlp_lunit(lcur0_w))
							...
							!write(nu20,*)j_o(ivxdatmat)%r(ibxdatkey_+ikeepxkyk_)
							lcur_w = jlp_next(lcur_w)
						enddo !while (jlp_lunit(lcur_w) == jlp_lunit(lcur0_w))  20085
 
					endif !if(lun_ <= jlp_lunits0)  20077
					...
					if(p.or.jlp_tmax.eq.-1.d0) then
						write(n16,*)'<7681> wsu,rs,wsu/rs,tmax,lcur0_unit,lcur0_w,lunit(lcur0_unit),ixkyk_',&
							jlp_wsu,jlp_rs,jlp_wsu/jlp_rs,jlp_tmax,lcur0_unit, &
							lcur0_w,jlp_lunit(lcur0_unit),ixkyk_
					endif !if(p.or.jlp_tmax.eq.-1.d0)  20122
					!	if((Abs(j_rs)>j_tiny78).and.(((j_rs*j_wsu)>j_zero).or.(j_wsu.eq.j_zero.and.j_rs>j_zero)).and.&
					!		Abs(j_wsu).lt.j_tmax*Abs(j_rs))then
					if(jlp_rs>jlp_tiny78.and.jlp_wsu.lt.jlp_tmax*jlp_rs)then !changed 27.8.2018 JL
						jlp_tmax=jlp_wsu/jlp_rs ;rcur=jlp_rs
						...
						if(p.or.jlp_tmax.eq.-1.d0) write(n16,*)'**fact** <6678> **tmax/key,wsu,rs',&
							jlp_tmax,jlp_wsu,jlp_rs,'leavkf, unit, ixkyk',leavkf, &
							jlp_lunit(leavkf), ixkyk_
					endif !if(jlp_rs>jlp_tiny78.and.jlp_wsu.lt.jlp_tmax*jlp_rs)  20129
 
					lcur0_unit = lcur_
				enddo !while (lcur0_unit > jlp_mxd)  20021
			enddo !ixkyk_=1,j_o(jlp_ivxkyk)%i(1)  20018
			...
			if(jlp_tmax.le.jlp_tmaxmin)then ! eq.j_0)then
				...
				! goto222=.true.;return
				! endif !if(ienter.eq.3.and.secondb.gt.j_valuek+j_tolecur)then
			endif !if(jlp_tmax.le.jlp_tmaxmin)  20149
			...
			if ((ienter==4).and.(lcursamef==0)) then
				...
				do while((jlp_lunit(jlp_lunw(lun_))/=iunit).and.(lun_<=jlp_lunits0))
					lun_=lun_+1
				enddo !while((jlp_lunit(jlp_lunw(lun_))/=iunit).and.(lun_<=jlp_lu  20171
				...
				if(lun_ <= jlp_lunits0) then
					...
					do while (jlp_lunit(lcur_) == jlp_lunit(lcur0_w))
						...
						if(p)write(16,*)'h5',jlp_wsu
						lcur_ = jlp_next(lcur_)
					enddo !while (jlp_lunit(lcur_) == jlp_lunit(lcur0_w))  20185
 
				endif !if(lun_ <= jlp_lunits0)  20177
				...
				if((jlp_rs>jlp_tiny78).and.(jlp_wsu.lt.jlp_tmax*jlp_rs))then
					...
					if(p.or.jlp_tmax.eq.-1.d0) write(n16,*)'**fact** <6743> **tmax/ei_xkf,wsu,rs',&
						jlp_tmax,jlp_wsu,jlp_rs,'leavkf, unit, ixkyk',leavkf, iunit, ixkykenter
				endif !if((jlp_rs>jlp_tiny78).and.(jlp_wsu.lt.jlp_tmax*jlp_rs))  20203
 
			endif !if ((ienter==4).and.(lcursamef==0))  20160
 
			if ((ienter==4).and.(leavkf==0).and.(leave==0).and.(leavk==0)) then
				lun_= 1
				do while((jlp_lunit(jlp_lunw(lun_))/=iunit).and.(lun_<=jlp_lunits0))
					lun_=lun_+1
				enddo !while((jlp_lunit(jlp_lunw(lun_))/=iunit).and.(lun_<=jlp_lu  20217
				if (lun_<=jlp_lunits0) then
					leavkwf = jlp_lunw(lun_)
					iunit_lkf=iunit
					ixkyk_lkf = ixkykenter
				...
				endif !if (lun_<=jlp_lunits0)  20220
			endif !if ((ienter==4).and.(leavkf==0).and.(leave==0).and.(leavk=  20215
		endif !if(jlp_fpresent)  20006
		...
		if(leavk.gt.0)then
			...
			if(sparse)then
				...
				do i=jlp_lavecsp(jlp_lavecsp(0)+icol),last(icol)
					...
					jlp_xps(j)=jlp_xps(j)+jlp_a(iel,icol)
					jlp_rhsw(j)=jlp_rhscur(j)-jlp_xps(j)
				enddo !i=jlp_lavecsp(jlp_lavecsp(0)+icol),last(icol)  20259
			else !if(sparse)then
				do j=1,jlp_nrow
					if((jlp_ix(j).ne.0).or.jlp_fpresent)jlp_xps(j)=jlp_xps(j)+jlp_a(j,leavk+nz) !leavk col in D
					jlp_rhsw(j)=jlp_rhscur(j)-jlp_xps(j)
				enddo !j=1,jlp_nrow  20266
 
			endif !if(sparse)  20256
			...
			if(iunit.eq.jlp_lunit(leavk).and.ienter.eq.3)then
				...
				if(sparse)then
					...
					do j=1,jlp_nrow
						jlp_a(j,newa)=jlp_a(j,newa)-jlp_a(j,leavk+nz)
					enddo !j=1,jlp_nrow  20286
				endif !if(sparse)  20279
			endif !if(iunit.eq.jlp_lunit(leavk).and.ienter.eq.3)  20276
			...
			if(jlp_testl)then
				if(jlp_keys(jlp_lunit(lcur)).gt.jlp_nsch(jlp_lunit(lcur)))then
					...
					write(6,*)'*u* return'
					goto900=.true.;return
				endif !if(jlp_keys(jlp_lunit(lcur)).gt.jlp_nsch(jlp_lunit(lcur)))  20308
			endif !if(jlp_testl)  20307
	...
	! first free column is reserved for newc
			if(jlp_lunit(lcur).eq.jlp_lunit(nex))then
				...
				if(sparse)then
					...
					do i=1,jlp_nrow
						jlp_a(i,ia)=jlp_a(i,nex+nz)-jlp_a(i,leavk+nz)
					enddo !i=1,jlp_nrow  20347
					if(p)write(n16,*)'iacol',jlp_a(1:min(jlp_nrow,50),ia)
				endif !if(sparse)  20339
				if(p)write(n16,*)'pivot7',nex+jlp_nrowz,newc2, 'in d',nex,id
				jlp_lunit(id)=jlp_lunit(leavk)
				jlp_isch(id)=jlp_isch(nex)
				icolold=nex+jlp_nrowz
				...
				if(sparse)then
					...
					if(nup.ge.nfreq)then
						krefac=krefac+1
						if(p)write(19,*)'*refact'
					endif !if(nup.ge.nfreq)  20374
					call pivot(nex+jlp_nrowz,newc2,jlp_nrow, &
						nm,jlp_a,jlp_lavec,e,wslu1,lwsll1,ifail,jlp_info)
				endif !if(sparse)  20363
				ipivot=ipivot+1
				...
				endif !if(ifail.ne.0)  20384
				if(sparse)then
					mpjjsp=-1
				else !if(sparse)then
					! mpjj=-1   ! old not something to remember
				endif !if(sparse)  20401
 
				if(zmatrix)then
					ipivoti_=ipivot/5000
					if(5000*ipivoti_.eq.ipivot)write(6,*)'**pivots, objf ',ipivot, jlp_objf
 
				endif !if(zmatrix)  20407
				if(ipivot.eq.idebug)then
					p=.not.p
				...
				endif !if(ipivot.eq.idebug)  20412
				...
				if(jlp_fpresent)then
					...
					do ilu_ = 1,jlp_lunits0
						if (jlp_lunw(ilu_)==nex) then
							jlp_lunw(ilu_)=id
						endif !if (jlp_lunw(ilu_)==nex)  20469
					enddo !ilu_ = 1,jlp_lunits0  20468
				endif !if(jlp_fpresent)  20466
				...
				lcur=id
				goto 17
			endif !if(jlp_lunit(lcur).eq.jlp_lunit(nex))  20323
		...
		if (jlp_fpresent) then
			...
			if(leavkwf>0) then
				...
				if ((nfy.gt.0).and.(jlp_lunit(leavkwf)/=iunitrans)) then
					do j=1,j_o(jlp_ivkeepc)%i(1)
						j_v(j_o(jlp_ivkeepc)%i2(j))=j_o(ivcmat)%d((jlp_lunit(leavkwf)-1)*j_o(jlp_ivkeepc)%i(1)+j)
					enddo !j=1,j_o(jlp_ivkeepc)%i(1)  20509
					do j=1,ntrans
						call dotrans(jlp_itransv(j),1)
						if(j_err)then
							write(6,*)'err for trans ',j
							stop 761
						endif !if(j_err)  20514
					enddo !j=1,ntrans  20512
					iunitrans=jlp_lunit(leavkwf)
				endif !if ((nfy.gt.0).and.(jlp_lunit(leavkwf)/=iunitrans))  20508
				...
				do while(jlp_lunit(nex)==jlp_lunit(leavkwf))
					...
					if(jlp_ixcur(0).ne.0)then
						...
						jlp_objr0(newc2)=0.
					endif !if(jlp_ixcur(0).ne.0)  20568
					...
					do jj=1,jlp_nrow
						if(jlp_ixcur(jj).ne.0)then
							...
							jlp_a(jj,ia)=0.
						endif !if(jlp_ixcur(jj).ne.0)  20576
					enddo !jj=1,jlp_nrow  20575
					...
					do jj=1,jlp_nrowpf ! #tehdasmjarivit
						...
						do k_=1,jlp_nfxrow(irowj_)
							...
							if ((jlp_lunit(leavkwf)==iunit_lkf).and.(ixkyk_==ixkyk_lkf)) then
								...
								! avaintehdas ei vaihdu
								keyf_ = jlp_keyfact(iunit_lkf,ixkyk_)
							endif !if ((jlp_lunit(leavkwf)==iunit_lkf).and.(ixkyk_==ixkyk_lkf  20596
							...
							if(keyf_==jlp_irowffact(jlp_ibafx(irowj_)+k_)) then
								...
								endif !if (jcurix==0)  20607
							endif !if(keyf_==jlp_irowffact(jlp_ibafx(irowj_)+k_))  20605
 
						enddo !k_=1,jlp_nfxrow(irowj_)  20591
 
					enddo !jj=1,jlp_nrowpf  20586
						...
						do k_=1,jlp_nfyrow(irowj_)
							...
							do ivars_=1,j_o(listy)%i(1) ! yk-mjan puutavaralistan muuttujat
								...
								if ((jlp_lunit(leavkwf)==iunit_lkf).and.(iv2xkykpos_==ixkyk_lkf)) then
									...
									keyf_ = jlp_keyfact(iunit_lkf,iv2xkykpos_)
								endif !if ((jlp_lunit(leavkwf)==iunit_lkf).and.(iv2xkykpos_==ixky  20643
									...
									if(keyf_.eq.iv3factpos_) then
										...
										if (jcurix==0) then
											...
											jlp_a(jcurix,ia)=jlp_a(jcurix,ia) + j_v(jlp_fyfactout(iv2xkykpos_,iv3factpos_)) *  &
												(j_o(ivxdatmat)%d(ibxdatobs_+iv2elpos_) - &  !xmatiba
												j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_))  !xmatiba
										endif !if (jcurix==0)  20655
									endif !if(keyf_.eq.iv3factpos_)  20653
 
								enddo !ifact_=1,j_o(listf)%i(1)  20649
							enddo !ivars_=1,j_o(listy)%i(1)  20638
						enddo !k_=1,jlp_nfyrow(irowj_)  20635
					enddo !jj=1,jlp_nrowpfy  20632
					...
					if(abs(jlp_x(nex+jlp_nrowz)).lt.jlp_tiny78)then
						...
						goto55=.true.;return
					endif !if(abs(jlp_x(nex+jlp_nrowz)).lt.jlp_tiny78)  20703
					...
					if(ipivot.eq.idebug)then
						...
						jdebug=jdebug+1
						if(jdebug.le.ndebug)idebug=j_v(j_o(iob)%i(ibasdebug+jdebug))
					endif !if(ipivot.eq.idebug)  20726
					...
					if(ifail.ne.0)then
						...
						if(p.or.p9)then
							...
							do jj=jlp_mxd+1,jlp_lf0
								write(n16,*) jj,' : ',(jlp_a(jj7,jj),jj7=1,jlp_nrow)
							enddo !jj=jlp_mxd+1,jlp_lf0  20786
 
							write(n16,*)'**fact** pivot fail: kannan lf() -sarakkeet '
							do jj=jlp_mxd+1,jlp_lf0
								write(n16,*) jlp_lf(jj),' : ',(jlp_a(jj7,jlp_lf(jj)),jj7=1,jlp_nrow)
							enddo !jj=jlp_mxd+1,jlp_lf0  20791
							write(n16,*)'**fact** pivot fail: kannan ld() -sarakkeet '
							do jj=1,jlp_ld0
								write(n16,*) jlp_ld(jj),' : ',(jlp_a(jj7,jlp_ld(jj)),jj7=1,jlp_nrow)
							enddo !jj=1,jlp_ld0  20795
						endif !if(p.or.p9)  20784
						goto1234=.true.;return
 
					endif !if(ifail.ne.0)  20741
					...
					do ilu_ = 1,jlp_lunits0
						if (jlp_lunw(ilu_)==nex) then
							jlp_lunw(ilu_)=id
						endif !if (jlp_lunw(ilu_)==nex)  20834
					enddo !ilu_ = 1,jlp_lunits0  20833
					...
					nex=jlp_next(nex)
				enddo !while(jlp_lunit(nex)==jlp_lunit(leavkwf))  20553
 
			endif !if(leavkwf>0)  20499
			...
			if (leavkf.gt.jlp_mxd) then
				...
				if(iunit.eq.jlp_lunit(leavkf).and.ienter.eq.4.and.(ixkykenter==ixkyk_lkf)) then
					...
					do j=1,jlp_nrow
						! newcol= x-oldkey  leavk:  newkey-oldkey  x-newkey= x-oldkey-(newkey-oldkey)
						jlp_a(j,newa)=jlp_a(j,newa)-jlp_a(j,leavkf+nz)
					enddo !j=1,jlp_nrow  20897
 
				endif !if(iunit.eq.jlp_lunit(leavkf).and.ienter.eq.4.and.(ixkyken  20891
				...
				if(iunit.eq.jlp_lunit(leavkf).and.ienter.eq.3)then
					...
					if(jlp_ixcur(0).ne.0)then
						...
						jlp_objr0(newc)=0.
					endif !if(jlp_ixcur(0).ne.0)  20917
					...
					do jj=1,jlp_nrow
						if(jlp_ixcur(jj).ne.0) then
							...
							jlp_a(jj,newa) = 0
						endif !if(jlp_ixcur(jj).ne.0)  20924
					enddo !jj=1,jlp_nrow  20923
					...
					do jj=1,jlp_nrowpf ! #tehdasmjarivit
						...
						do k_=1,jlp_nfxrow(irowj_)
							...
							if (ixkyk_==ixkyk_lkf) then
								...
								! muut xk-mjat (avaintehdas ei vaihdu)
								keyf_ = jlp_keyfact(iunit,ixkyk_)
							endif !if (ixkyk_==ixkyk_lkf)  20939
							...
							if(keyf_==jlp_irowffact(jlp_ibafx(irowj_)+k_)) then
								...
								if (jcurix==0) then
									...
									jlp_a(jcurix,newa)=jlp_a(jcurix,newa) + jlp_coeffx(jlp_ibafx(irowj_)+k_)* &
										(j_o(ivxdatmat)%d(ibxdatopt_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)) - & !xmatiba
										j_o(ivxdatmat)%d(ibxdatkey_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)))    !xmatiba
								endif !if (jcurix==0)  20950
							endif !if(keyf_==jlp_irowffact(jlp_ibafx(irowj_)+k_))  20948
 
						enddo !k_=1,jlp_nfxrow(irowj_)  20936
					enddo !jj=1,jlp_nrowpf  20931
						...
						do k_=1,jlp_nfyrow(irowj_)
							...
							do ivars_=1,j_o(listy)%i(1) ! yk-mjan puutavaralistan muuttujat
								...
								if (iv2xkykpos_==ixkyk_lkf) then
									...
									keyf_ = jlp_keyfact(iunit,iv2xkykpos_)
								endif !if (iv2xkykpos_==ixkyk_lkf)  20974
								valuetas=j_o(ivxdatmat)%d(ibxdatopt_+iv2elpos_)-j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_)
								do ifact_=1,j_o(listf)%i(1) ! yk-mjan tehdaslistan tehtaat
									iv3factpos_ = jlp_ifyfactfact(jlp_ibafyfact(jlp_ibafy(irowj_)+k_)+ifact_-1) !tehtaan paikka factories-listassa
									! lisätään  avaintehtaan (gamma*(x_ij_k - x_iJ(i)_k))
									if(keyf_.eq.iv3factpos_) then
										...
										endif !if (jcurix==0)  20986
									endif !if(keyf_.eq.iv3factpos_)  20984
 
								enddo !ifact_=1,j_o(listf)%i(1)  20981
							enddo !ivars_=1,j_o(listy)%i(1)  20970
						enddo !k_=1,jlp_nfyrow(irowj_)  20967
					enddo !jj=1,jlp_nrowpfy  20964
 
					if(p)write(n16,*)'**fact**  objr0(newc) päivitetty :', jlp_objr0(newc)
				endif !if(iunit.eq.jlp_lunit(leavkf).and.ienter.eq.3)  20906
...
! first free column is reserved for newc
				...
				if(jlp_lunit(lcur).eq.jlp_lunit(nex))then
					...
					endif !if(ipivot.eq.idebug)  21098
					...
					lcur=if_
					goto 19
				endif !if(jlp_lunit(lcur).eq.jlp_lunit(nex))  21036
			...
			endif !if (leavkf.gt.jlp_mxd)  20851
 
		endif !if (jlp_fpresent)  20494
...
8888 continue !we come here direrectly (through checking keys) if tmax is sufficiently small
		...
		if(leave.eq.0)then
			if(ienter.eq.3)then
						...
						do k=1,jlp_nfxrow(irowj)
							...
							if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj)+k)).eq.jlp_irowffact(jlp_ibafx(irowj)+k))then
								jlp_xps(j)=jlp_xps(j) &
									...
									+ jlp_coeffx(jlp_ibafx(irowj)+k)*&
									j_o(ivxdatmat)%d(ibxdatopt+jlp_irowfkeep(jlp_ibafx(irowj)+k))  !xmatiba
							endif !if(jlp_keyfact(iunit,jlp_irowfxvars(jlp_ibafx(irowj)+k)).e  21182
						enddo !k=1,jlp_nfxrow(irowj)  21180
					enddo !jj=1,jlp_nrowpf  21176
								...
								do ifact_=1,j_o(listf)%i(1)
									...
									if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_) then
										jlp_xps(j)=jlp_xps(j) &
											...
											valuetas2
										!	j_o(ivxdatmat)%d(xmatiba(iobsopt)+iv2elpos_)
									endif !if(jlp_keyfact(iunit,iv2xykypos_).eq.iv3factpos_)  21209
								enddo !ifact_=1,j_o(listf)%i(1)  21206
							enddo !ivars_=1,j_o(listy)%i(1)  21200
						enddo !k=1,jlp_nfyrow(irowj)  21197
					enddo !jj=1,jlp_nrowpfy  21192
 
				endif !if (jlp_fpresent)  21172
				...
				enddo !jj=1,jlp_nrowp  21226
 
				do jj=1,jlp_nrow
					jlp_rhsw(jj)=jlp_rhscur(jj)-jlp_xps(jj)
				enddo !jj=1,jlp_nrow  21232
				...
				if((jlp_lower(newc).and..not.jlp_ubou(newc)).or.(.not.jlp_lower(newc).and..not.jlp_lbou(newc))) then
					...
					if(jlp_maxo)then
						...
						j_v(ivobjective)=j_ninf
					endif !if(jlp_maxo)  21250
					j_err=.true.
 
					return
				endif !if((jlp_lower(newc).and..not.jlp_ubou(newc)).or.(.not.jlp_  21243
				if(jlp_lower(newc))then
					jlp_lower(newc)=.false.
					jlp_rhscur(newc)=jlp_rhs2(newc)
				else !if(j_lower(newc))then
					jlp_lower(newc)=.true.
					jlp_rhscur(newc)=jlp_rhs(newc)
				endif !if(jlp_lower(newc))  21259
				if((jlp_ix(newc).ne.0).or.jlp_fpresent)then
					jlp_rhsw(newc)=jlp_rhscur(newc)-jlp_xps(newc)
				else !if((j_ix(newc).ne.0).or.j_fpresent)then
					jlp_rhsw(newc)=jlp_rhscur(newc)
 
				endif !if((jlp_ix(newc).ne.0).or.jlp_fpresent)  21266
					...
					do k=1,jlp_nfxrow(irowj)
						if((jlp_irowfxvars(jlp_ibafx(irowj)+k)==ixkykenter).and.&
							...
							jlp_xps(j)=jlp_xps(j) &
								- jlp_coeffx(jlp_ibafx(irowj)+k)*&
								j_o(ivxdatmat)%d(ibxdatobs+jlp_irowfkeep(jlp_ibafx(irowj)+k))  !xmatiba
						endif !if((jlp_irowfxvars(jlp_ibafx(irowj)+k)==ixkykenter).a  21294
						if ((jlp_irowfxvars(jlp_ibafx(irowj)+k)==ixkykenter).and.(ifopt.eq.jlp_irowffact(jlp_ibafx(irowj)+k))) then
							jlp_xps(j)=jlp_xps(j) &
								+ jlp_coeffx(jlp_ibafx(irowj)+k)*&
								j_o(ivxdatmat)%d(ibxdatobs+jlp_irowfkeep(jlp_ibafx(irowj)+k))  !xmatiba
						endif !if ((jlp_irowfxvars(jlp_ibafx(irowj)+k)==ixkykenter).and.(  21300
					enddo !k=1,jlp_nfxrow(irowj)  21293
 
					if(j>0) jlp_rhsw(j)=jlp_rhscur(j)-jlp_xps(j)
				enddo !jj=1,jlp_nrowpf  21287
				do jj=1,jlp_nrowpfy ! yk-muuttujarivit
					j=jlp_ifycurrows(jj)
					! silmukka: rivin yk-muuttujat
					!		silmukka: puultavaralajilista
							...
							do ifact_=1,j_o(listf)%i(1)
								...
								if((iv2xykypos_==ixkykenter).and.(jlp_keyfact(iunit_lkf,iv2xykypos_).eq.iv3factpos_)) then
									jlp_xps(j)=jlp_xps(j) &
										- j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
										j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_)  !xmatiba
								endif !if((iv2xykypos_==ixkykenter).and.(jlp_keyfact(iunit_lkf,iv  21326
								if((iv2xykypos_==ixkykenter).and.(iv3factpos_==ifopt)) then
									jlp_xps(j)=jlp_xps(j) &
										+ j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
										j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_)  !xmatiba
								endif !if((iv2xykypos_==ixkykenter).and.(iv3factpos_==ifopt))  21331
 
							enddo !ifact_=1,j_o(listf)%i(1)  21322
						enddo !ivars_=1,j_o(listy)%i(1)  21317
					enddo !k=1,jlp_nfyrow(irowj)  21314
					if(j.gt.0) jlp_rhsw(j)=jlp_rhscur(j)-jlp_xps(j)
				enddo !jj=1,jlp_nrowpfy  21309
				! avaintehdas taulukon  (keyfact) päivitys
				jlp_keyfact(iunit_lkf,ixkykenter) = ifopt
				if(p9)write(16,*)'vain at',iunit_lkf,ixkykenter, ifopt
				if(p) then
				...
				if(.not.jlp_feasible)then
					do jj=1,lr0
						if(r(jlp_lr(jj)).gt.jlp_tiny78)then
							if(.not.jlp_lbou(jlp_lr(jj)).or..not.jlp_lower(jlp_lr(jj))) then
								!virheilmoitus ja häivytään
							endif !if(.not.jlp_lbou(jlp_lr(jj)).or..not.jlp_lower(jlp_lr(jj))  21357
							jlp_tmax=jlp_x(jlp_lr(jj))/r(jlp_lr(jj))  ;rcur=r(jlp_lr(jj))
							leavec=jlp_lr(jj)
							leave=jlp_lsi(leavec)
							goto 8888
						endif !if(r(jlp_lr(jj)).gt.jlp_tiny78)  21356
						if(r(jlp_lr(jj)).lt.jlp_tiny78n)then
							if(.not.jlp_ubou(jlp_lr(jj)).or.jlp_lower(jlp_lr(jj))) then
								!virheilmoitus ja häivytään
							endif !if(.not.jlp_ubou(jlp_lr(jj)).or.jlp_lower(jlp_lr(jj)))  21366
							jlp_tmax=jlp_x(jlp_lr(jj))/r(jlp_lr(jj)) ;rcur=r(jlp_lr(jj))
							leavec=jlp_lr(jj)
							leave=jlp_lsi(leavec)
							goto 8888
						endif !if(r(jlp_lr(jj)).lt.jlp_tiny78n)  21365
					enddo !jj=1,lr0  21355
				endif !if(.not.jlp_feasible)  21354
				...
				if(jlp_maxo)then
					...
					j_v(ivobjective)=j_ninf
				endif !if(jlp_maxo)  21383
				j_err = .true.
				return
			endif !if(ienter.eq.3)  21160
			...
			endif !if(ipivot+1.eq.idebug)  21395
			if(sparse)then
				if(newc.gt.jlp_nrowz)mqjjsp=-1  ! mqjj=-1   !new not  something known
			else !if(sparse)then
				if(newc.gt.jlp_nrowz)mqjj=-1  ! mqjj=-1   !new not  something known
			...
			if(sparse)then
				...
				endif !if(jlp_xpresent.and.abs(rcur).le.1.d-9)  21444
				...
				if(route67)then
					if(newc.gt.jlp_nrow)then    !jl 20160608
						...
						if(abs(jlp_x(jlp_ls(leave))).lt.jlp_tiny6)then
							...
							goto55=.true.;return
						endif !if(abs(jlp_x(jlp_ls(leave))).lt.jlp_tiny6)  21459
					else !if(newc.gt.j_nrow)then
						! residual enters
						call fbsub(jlp_nrow,leave,leave,jlp_a,jlp_lavec,newc,jlp_a(1:,1),jlp_x,jlp_ls,wslu1,lwsll1,.false.)
						!				write(6,*)'<679',jlp_x(2)
						if(abs(jlp_x(jlp_ls(leave))).lt.jlp_tiny6)then
							...
							goto55=.true.;return
						endif !if(abs(jlp_x(jlp_ls(leave))).lt.jlp_tiny6)  21477
 
					endif !if(newc.gt.jlp_nrow)  21452
				endif !if(route67)  21451
				...
				if(tabu)then
					if(fast)then
						if(leavec.gt.jlp_nrowz.and.leavec.le.jlp_nrowz+jlp_mxd)then
							...
							fastreject(jlp_ibaunit(iuni_)+jlp_isch(leavec-jlp_nrowz))=.true.
							if(p)write(16,*)'tabu ',iuni_,jlp_isch(leavec-jlp_nrowz)
						endif !if(leavec.gt.jlp_nrowz.and.leavec.le.jlp_nrowz+jlp_mxd)  21507
					endif !if(fast)  21506
				endif !if(tabu)  21505
 
			endif !if(sparse)  21426
			ipivot=ipivot+1
			if(p7)write(77,*)4,ipivot,jlp_objf
			...
			if(ifail.ne.0)then
				...
				if(nrecover.ge.20)then
					...
					j_err=.true.
					goto900=.true.;return
				endif !if(nrecover.ge.20)  21533
				goto1234=.true.;return
 
			endif !if(ifail.ne.0)  21524
			if(sparse)then
				if(jlp_ls(leave).gt.jlp_nrowz) mpjjsp=-1
			else !if(sparse)then
				!	if(ls(leave).gt.nrowz) mpjj=-1
			endif !if(sparse)  21541
			...
			if(lcursame.gt.0)then  ! d-eaves
				...
				if(jlp_fpresent)then
					do ilu_ = 1,jlp_lunits0
						...
						if (jlp_lunw(ilu_)==lcursame) then
							jlp_lunw(ilu_)=id
						endif !if (jlp_lunw(ilu_)==lcursame)  21593
					enddo !ilu_ = 1,jlp_lunits0  21591
				endif !if(jlp_fpresent)  21590
				...
				if(jlp_fpresent)then
					...
					do while(jlp_lunit(jlp_lunxkf(ilu_)) /= iunit)
						ilu_=ilu_+1
						if (ilu_>jlp_lunits0) exit
					enddo !while(jlp_lunit(jlp_lunxkf(ilu_)) /= iunit)  21619
					if (ilu_<=jlp_lunits0) then
						jlp_lunw(ilu_)=id
					else !if (ilu_<=j_lunits0) then
						!laskentayksikköön ei liittynyt aiemmin kannan kannan vaihtoehto- tai tehdasmuuttujasarakkeita
						...
						jlp_lunw(jlp_lunits0) = id
						jlp_lunxkf(jlp_lunits0) = jlp_mxd
					endif !if (ilu_<=jlp_lunits0)  21623
				endif !if(jlp_fpresent)  21612
 
			endif !if(lcursame.gt.0)  21577
			! edelleed d-enters
			jlp_ld0=jlp_ld0+1
			jlp_lunit(id)=iunit
			jlp_isch(id)=iopt
			...
			if(lcursamef.gt.0)then  ! xk-eaves
				...
				do ilu_ = 1,jlp_lunits0
					if (jlp_lunxkf(ilu_)==lcursamef) then
						jlp_lunxkf(ilu_)=id
						if (p) write(n16,*)"**fact** lunxkf päivitetty <8184>",(jlp_lunxkf(jj7),jj7=1,jlp_lunits0)
					endif !if (jlp_lunxkf(ilu_)==lcursamef)  21667
				enddo !ilu_ = 1,jlp_lunits0  21666
				...
				do while(jlp_lunit(jlp_lunw(ilu_))/=iunit)
					ilu_=ilu_+1
					if (ilu_>jlp_lunits0) exit
				enddo !while(jlp_lunit(jlp_lunw(ilu_))/=iunit)  21687
				if (ilu_<=jlp_lunits0) then
					jlp_lunxkf(ilu_)=id
				else !if (ilu_<=j_lunits0) then
					!laskentayksikköön ei liittynyt aiemmin kannan vaihtoehto- tai tehdasmuuttujasarakkeita
					...
					jlp_lunxkf(jlp_lunits0) = id
					jlp_lunw(jlp_lunits0) = 0
				endif !if (ilu_<=jlp_lunits0)  21691
 
			endif !if(lcursamef.gt.0)  21653
			! edelleed d-enters
			! indeksointi muutettu alkamaan mxd+1:stä (vastaa suoraan kannan saraketta)
			jlp_lf0=jlp_lf0+1
			jlp_lunit(id)=iunit
						...
						if (jlp_lunit(leaved) == jlp_lunit(jlp_next(leaved))) then
							...
							if (jlp_lunxkf(ilu_)==jlp_mxd) then
								do i_ = ilu_+1,jlp_lunits0
									jlp_lunw(i_-1) = jlp_lunw(i_)
									jlp_lunxkf(i_-1) = jlp_lunxkf(i_)
								enddo !i_ = ilu_+1,jlp_lunits0  21750
								jlp_lunw(jlp_lunits0) = 0
								jlp_lunxkf(jlp_lunits0) = jlp_mxd
								jlp_lunits0 = jlp_lunits0-1
							else !if (j_lunxkf(ilu_)==j_mxd) then
								jlp_lunw(ilu_)=0
							endif !if (jlp_lunxkf(ilu_)==jlp_mxd)  21749
						endif !if (jlp_lunit(leaved) == jlp_lunit(jlp_next(leaved)))  21746
					endif !if (jlp_lunw(ilu_)==leaved)  21745
				enddo !ilu_ = 1,jlp_lunits0  21744
			endif !if(jlp_fpresent)  21743
			...
			if(jlp_testl)then
				do i=1,jlp_nrow
					if(jlp_ls(i).gt.jlp_nrowz)then
						do j=1,jlp_ld0
							if(jlp_ls(i)-jlp_nrowz.eq.jlp_ld(j))goto 7188
						enddo !j=1,jlp_ld0  21769
						write(n16,*)'*colnot*',jlp_ls(i),'not in',	  (jlp_ld(j),j=1,jlp_ld0)
						close(16)
						write(6,*)'*colnot* return'
						return
		7188 	    continue
					endif !if(jlp_ls(i).gt.jlp_nrowz)  21768
				enddo !i=1,jlp_nrow  21767
			endif !if(jlp_testl)  21766
			! lista uniteista
					...
					if (jlp_lunit(leaved) == jlp_lunit(jlp_nextf(leaved,jlp_ixkf(leaved)))) then
						...
						if (jlp_lunw(ilu_)==0) then
							do i_ = ilu_+1,jlp_lunits0
								jlp_lunw(i_-1) = jlp_lunw(i_)
								jlp_lunxkf(i_-1) = jlp_lunxkf(i_)
							enddo !i_ = ilu_+1,jlp_lunits0  21798
							jlp_lunw(jlp_lunits0) = 0
							jlp_lunxkf(jlp_lunits0) = jlp_mxd
							jlp_lunits0 = jlp_lunits0-1
						else !if (j_lunw(ilu_)==0) then
							jlp_lunxkf(ilu_)=jlp_mxd
						endif !if (jlp_lunw(ilu_)==0)  21797
					endif !if (jlp_lunit(leaved) == jlp_lunit(jlp_nextf(leaved,jlp_ix  21794
				endif !if (jlp_lunxkf(ilu_)==leaved)  21793
			enddo !ilu_ = 1,jlp_lunits0  21792
			if (pp) write(n16,*)"**fact** lunxkf päivitetty <8088>",(jlp_lunw(jj7),jj7=1,jlp_lunits0)
 
		endif !if(leavec.le.jlp_nrow)  21715
	...
	subroutine printproblem()
		...
		do i=1,nset
			...
			if(jlp_xpresent.and.jlp_ndom.gt.0)then
				...
				!	write(6,*)' ',('_',kk=1,78)
				write(nureport,'(a)')j_tempchar(1:le+12)
			end if !if(jlp_xpresent.and.jlp_ndom.gt.0)  22030
			!write(nureport,*)' ',('_',kk=1,78)
			!		write(j_buf,66061)
			!66061 format('row',t38,'value',t50,'shadow',t61,  'lower',t70,'upper')
			!	write(nureport,'(a)')j_buf
			...
			do k=1,jlp_nsetr(i)
				...
				if(irow0.ne.irowobj)then
					...
					end if !if(jlp_rhs(irow).eq.jlp_rhs2(irow))  22073
					...
					if(jlp_maxo)then
						...
						j_tempchar(1:5)=' min'
					end if !if(jlp_maxo)  22087
					!else !if(j_v(ivfeasible)>0)then
					!	j_buf(1:5)=' '
					!	j_buf(6:33)='Infeasible, temporary object'
					!endif !if(j_v(ivfeasible)>0)then
				end if !if(irow0.ne.irowobj)  22060
				write(nureport,'(a)')j_tempchar(1:le+28)
				!	if(intapp)then
				!		j_buf=' '
				!		j_buf(36:)=j_chr10(j_solx(irow))
				!	write(6,'(a)')j_buf
				!	endif !if(intapp)then
			end do !k=1,jlp_nsetr(i)  22049
			!			enddo domloopp !domloopp: do j=1,j_nsetd(i)
		enddo !i=1,nset  22027
	...
	logical function isxkzero(ixkyk)
		...
		if(j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_).ne.0.)then
			isxkzero=.false.
			return
		endif !if(j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_).ne.0.)  22118
		do k_=1,jlp_ld0
			if(jlp_lunit(jlp_ld(k_)).ne.iunit) cycle
			ikey_=jlp_ibaunit(iunit) + jlp_isch(jlp_ld(k_))
			ibxdatkey_=xdatiba(ikey_) !,2)
			if(j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_).ne.0.)then
				isxkzero=.false.
				return
			endif !if(j_o(ivxdatmat)%d(ibxdatkey_+ikeepxkyk_).ne.0.)  22126
		enddo !k_=1,jlp_ld0  22122
	end function !logical function isxkzero(ixkyk)
	...
	function value_f(iobs)
		...
		do jjj = 1,nrowxkfkey
			...
			if (jlp_rowxkfkey(jjj)%jcurix == 0) then
				...
				!        value_f,j_vc(j_rowxkfkey(j_)%jcurix),j_coeffx(j_rowxkfkey(j_)%irowfx),&
				!          j_o(ivxdatmat)%d(xdatiba(iobs)+irowfkeep_)
			endif !if (jlp_rowxkfkey(jjj)%jcurix == 0)  22147
		enddo !jjj = 1,nrowxkfkey  22145
		...
		do jj=1,nrowykfkey
			...
			if (jlp_rowykfkey(jj)%jcurix.eq.0) then ! 0-rivi, ei v-kerrointa
				if(jlp_feasible) then
					...
					if(p)write(n16,*)'value_f, gamma, ykf',value_f, j_v(jlp_rowykfkey(jj)%ivfout),&
						j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_)
				endif !if(jlp_feasible)  22166
			else !if (j_rowykfkey(j_)%jcurix.eq.0) then
				value_f = value_f - jlp_vc(jlp_rowykfkey(jj)%jcurix)* j_v(jlp_rowykfkey(jj)%ivfout)*&
					j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_)
				if(p)write(n16,*)'value_f,vc,gamma,ykf',value_f,jlp_vc(jlp_rowykfkey(jj)%jcurix),&
					j_v(jlp_rowykfkey(jj)%ivfout),j_o(ivxdatmat)%d(ibxdatobs+iv2elpos_)
			endif !if (jlp_rowykfkey(jj)%jcurix.eq.0)  22165
		enddo !jj=1,nrowykfkey  22163
	...
	subroutine lcursam()
		...
		do i=1,jlp_ld0
			...
			if(jlp_lunit(lcur).eq.iunit)then
				lcursame=lcur
				return
			end if !if(jlp_lunit(lcur).eq.iunit)  22190
		end do !i=1,jlp_ld0  22188
		return
	end subroutine !subroutine lcursam()
...
766		nex=jlp_next(nex)
			...
			if(jj.gt.jlp_ld0.or.nex.eq.0)then
				...
				j_err=.true.
				return
			end if !if(jj.gt.jlp_ld0.or.nex.eq.0)  22206
			jj=jj+1
			goto 766
767		continue
		end do !i=1,jlp_ld0  22200
		! end logical testing
	end subroutine !subroutine testld()
		...
		do i=0,jlp_nrow
			...
			if(i.gt.0)then
				if(jlp_ls(i).gt.jlp_nrowz)then
					...
					!        return
	788		  continue
				endif !if(jlp_ls(i).gt.jlp_nrowz)  22231
			end if !if(i.gt.0)  22230
		end do !i=0,jlp_nrow  22228
		...
		do i=1,jlp_nunits
			...
			do j=0,jlp_nrow
				if(jlp_ixcur(j).ne.0)jlp_testxps(j)=jlp_testxps(j)+jlp_xmat(jlp_ix(j)+ibxmatx) !v(ix(j)) !lasketaan keysumma
			end do !j=0,jlp_nrow  22251
		end do !i=1,jlp_nunits  22246
		write(n16,*)'   ***xps',(jlp_xps(j),j=0,jlp_nrow)
		write(n16,*)'   ***test',(jlp_testxps(j),j=0,jlp_nrow) !pitäs olla sama
		...
		do i=1,jlp_ld0
			...
			do j=0,jlp_nrow
				...
				if(jlp_ixcur(j).ne.0) jlp_test(j)=&
					jlp_xmat(jlp_ix(j)+ibxmatx ) !v(ix(j))
			end do !j=0,jlp_nrow  22266
			iobs=jlp_ibaunit(jlp_lunit(jlp_ld(i)))+jlp_isch(jlp_ld(i))
			ibxmatx=xmatiba(iobs) !,1)
			do j=0,jlp_nrow
				if(jlp_ixcur(j).ne.0) jlp_test(j)=jlp_xmat(jlp_ix(j)+ibxmatx) -jlp_test(j) !v(ix(j))-test(j)
				jlp_testxps(j)=jlp_testxps(j)+jlp_x(jlp_ld(i)+jlp_nrowz)*jlp_test(j)
			end do !j=0,jlp_nrow  22273
			write(n16,*)'  dcoltest',(jlp_test(j),j=0,jlp_nrow)
			if(jlp_x(jlp_ld(i)+jlp_nrowz).LT.0.OR.jlp_x(jlp_ld(i)+jlp_nrowz).GT.1.)then
				write(6,*)'w:',jlp_x(jlp_ld(i)+jlp_nrowz)
				write(6,*)'*dcoltest* return'
				j_err=.true.
			end if !if(jlp_x(jlp_ld(i)+jlp_nrowz).LT.0.OR.jlp_x(jlp_ld(i)+jlp_  22278
		end do !i=1,jlp_ld0  22258
		write(n16,*)'*first nonbasic column*:',icolold,'*in a matrix',icolold.gt.jlp_nrow
		write(n16,*)'  col',(jlp_a(j,icolold-jlp_nrowz),j=1,jlp_nrow)
...
!      22       20      22
		...
		if(jlp_lunit(nex).ne.lunv)then
			...
			if(wss.gt.1.)then
				...
				write(6,*)'*nex* return'
				j_err=.true. ;return
			end if !if(wss.gt.1.)  22307
		end if !if(jlp_lunit(nex).ne.lunv)  22302
		goto 555  !next column in the ordering
556 write(n16,*)'muutosb',muutosb
! testaus lopui
	end subroutine !subroutine testcol()
	...
	subroutine testxpssub(iunit_)
				...
				do jj_=1,jlp_nrowpf
					...
					do k_=1,jlp_nfxrow(irowj_)
						if(jlp_keyfact(i_,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)).eq.jlp_irowffact(jlp_ibafx(irowj_)+k_))then
							jlp_testxps(jjj)=jlp_testxps(jjj)+ jlp_coeffx(jlp_ibafx(irowj_)+k_)*&
								j_o(ivxdatmat)%d(ibxdatobs_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_))
						endif !if(jlp_keyfact(i_,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)).eq  22349
					enddo !k_=1,jlp_nfxrow(irowj_)  22348
				enddo !jj_=1,jlp_nrowpf  22344
 
				if (nfy.gt.0) then
					do jjj=1,j_o(jlp_ivkeepc)%i(1)
						j_v(j_o(jlp_ivkeepc)%i2(jjj))=j_o(ivcmat)%d((i_-1)*j_o(jlp_ivkeepc)%i(1)+jjj)
					enddo !jjj=1,j_o(jlp_ivkeepc)%i(1)  22357
					do jjj=1,ntrans
						call dotrans(jlp_itransv(jjj),1)
						if(j_err)then
							write(6,*)'err for trans ',j
							stop 741
						endif !if(j_err)  22362
					enddo !jjj=1,ntrans  22360
				endif !if (nfy.gt.0)  22356
						...
						do ivars_=1,j_o(listy_)%i(1)
							...
							do ifact_=1,j_o(listf_)%i(1)
								...
								if(jlp_keyfact(i_,iv2xykypos_).eq.iv3factpos_) then
									jlp_testxps(jjj)=jlp_testxps(jjj)+ j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
										j_o(ivxdatmat)%d(ibxdatobs_+iv2elpos_) !xdatiba
								endif !if(jlp_keyfact(i_,iv2xykypos_).eq.iv3factpos_)  22388
 
							enddo !ifact_=1,j_o(listf_)%i(1)  22385
						enddo !ivars_=1,j_o(listy_)%i(1)  22379
 
					enddo !k_=1,jlp_nfyrow(irowj_)  22375
				enddo !jj_=1,jlp_nrowpfy  22370
 
			endif !if(jlp_fpresent)  22339
		enddo !i_=1,jlp_nunits  22326
		...
		if(iunit_>0) then
			...
			if(jlp_fpresent) then
				...
				if (nfy.gt.0) then
					do jjj=1,j_o(jlp_ivkeepc)%i(1)
						j_v(j_o(jlp_ivkeepc)%i2(jjj))=j_o(ivcmat)%d((iunit_-1)*j_o(jlp_ivkeepc)%i(1)+jjj)
					enddo !jjj=1,j_o(jlp_ivkeepc)%i(1)  22410
					do jjj=1,ntrans
						call dotrans(jlp_itransv(jjj),1)
						if(j_err)then
							write(6,*)'error for trans ',j
							stop 741
						endif !if(j_err)  22415
					enddo !jjj=1,ntrans  22413
				endif !if (nfy.gt.0)  22409
			endif !if(jlp_fpresent)  22406
		endif !if(iunit_>0)  22403
		...
		do iii=1,jlp_nrow
			if(abs(jlp_testxps(iii)-jlp_xps(iii)).gt.0.001)then
				nero=nero+1
				if(nero.lt.10)write(n16,*)'row,test,xps', iii,jlp_testxps(iii),jlp_xps(iii)
			endif !if(abs(jlp_testxps(iii)-jlp_xps(iii)).gt.0.001)  22427
		enddo !iii=1,jlp_nrow  22426
		write(n16,*)'testxps', nero
	...
	subroutine testobjr0()
		...
		do ldj_=1,jlp_ld0
			...
			do it_=0,jlp_nrow
				if(jlp_ixcur(it_).ne.0) then
					jlp_test(it_) = jlp_xmat(jlp_ix(it_)+ibxmatx)- jlp_xmat(jlp_ix(it_)+ibxmatkey_)
				endif !if(jlp_ixcur(it_).ne.0)  22454
			enddo !it_=0,jlp_nrow  22453
					...
					do k_=1,jlp_nfxrow(irowj_) ! silmukka : rivin xk-muuttujat
						...
						if(jlp_keyfact(iunit_,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)).eq.jlp_irowffact(jlp_ibafx(irowj_)+k_)) then
							jlp_test(jcurix_) = jlp_test(jcurix_) + jlp_coeffx(jlp_ibafx(irowj_)+k_)*&
								(j_o(ivxdatmat)%d(ibxdatobs_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)) -&
								j_o(ivxdatmat)%d(ibxdatkey_+jlp_irowfkeep(jlp_ibafx(irowj_)+k_)))
						endif !if(jlp_keyfact(iunit_,jlp_irowfxvars(jlp_ibafx(irowj_)+k_)  22466
					enddo !k_=1,jlp_nfxrow(irowj_)  22464
				enddo !jjj=1,jlp_nrowpf  22461
 
				!y-mjien muunnokset
				if (nfy.gt.0) then
					do jjj=1,j_o(jlp_ivkeepc)%i(1)
						j_v(j_o(jlp_ivkeepc)%i2(jjj))=j_o(ivcmat)%d((iunit_-1)*j_o(jlp_ivkeepc)%i(1)+jjj)
					enddo !jjj=1,j_o(jlp_ivkeepc)%i(1)  22476
					do jjj=1,ntrans
						call dotrans(jlp_itransv(jjj),1)
						if(j_err)then
							write(6,*)' err in tr ',j
							stop 614
						endif !if(j_err)  22481
					enddo !jjj=1,ntrans  22479
				endif !if (nfy.gt.0)  22475
							...
							do ifact_=1,j_o(listf_)%i(1) ! yk-mjan tehdaslistan tehtaat
								...
								if(jlp_keyfact(iunit_,iv2xykypos_).eq.iv3factpos_) then
									jlp_test(jcurix_) = jlp_test(jcurix_) +j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))*&
										(j_o(ivxdatmat)%d(ibxdatobs_+iv2elpos_) -&
										j_o(ivxdatmat)%d(ibxdatkey_+iv2elpos_))
								endif !if(jlp_keyfact(iunit_,iv2xykypos_).eq.iv3factpos_)  22500
							enddo !ifact_=1,j_o(listf_)%i(1)  22497
						enddo !ivars_=1,j_o(listy_)%i(1)  22494
					enddo !k_=1,jlp_nfyrow(irowj_)  22491
				enddo !jjj=1,jlp_nrowpfy  22488
 
			endif !if (jlp_fpresent)  22459
 
			write(n16,*)'ld(j)',jlp_ld(ldj_),'test(0:nrow)',(jlp_test(jjj),jjj=0,jlp_nrow)
 
		enddo !ldj_=1,jlp_ld0  22440
	end subroutine !subroutine testobjr0()
		...
		do lfj_=1,jlp_lf0
			...
			do jjj=1,jlp_nrowpf ! rivit, joilla xk-muuttujia
				...
				do k_=1,jlp_nfxrow(irowj_) ! silmukka : rivin xk-muuttujat
					if((jlp_irowfxvars(jlp_ibafx(irowj_)+k_)==ixkyk_).and.(ifact_.eq.jlp_irowffact(jlp_ibafx(irowj_)+k_))) then
						jlp_test(jcurix_)=jlp_test(jcurix_) + jlp_coeffx(jlp_ibafx(irowj_)+k_)
					endif !if((jlp_irowfxvars(jlp_ibafx(irowj_)+k_)==ixkyk_).and.(ifa  22537
					if((jlp_irowfxvars(jlp_ibafx(irowj_)+k_)==ixkyk_).and.(ifact_.eq.jlp_keyfact(iunit_,ixkyk_))) then
						jlp_test(jcurix_)=jlp_test(jcurix_) - jlp_coeffx(jlp_ibafx(irowj_)+k_)
					endif !if((jlp_irowfxvars(jlp_ibafx(irowj_)+k_)==ixkyk_).and.(ifa  22540
				enddo !k_=1,jlp_nfxrow(irowj_)  22536
			enddo !jjj=1,jlp_nrowpf  22533
 
			if (nfy.gt.0) then
				do jjj=1,j_o(jlp_ivkeepc)%i(1)
					j_v(j_o(jlp_ivkeepc)%i2(jjj))=j_o(ivcmat)%d((iunit_-1)*j_o(jlp_ivkeepc)%i(1)+jjj)
				enddo !jjj=1,j_o(jlp_ivkeepc)%i(1)  22547
				do jjj=1,ntrans
					call dotrans(jlp_itransv(jjj),1)
					if(j_err)then
						write(6,*)'err in tr ',j
					...
					endif !if(j_err)  22552
				enddo !jjj=1,ntrans  22550
			endif !if (nfy.gt.0)  22546
				...
				do k_=1,jlp_nfyrow(irowj_)
					...
					do ivars_=1,j_o(listy_)%i(1)
						...
						do ifact_=1,j_o(listf_)%i(1)
							...
							if((iv2xykypos_==ixkyk_).and.(ifact_.eq.iv3factpos_)) then
								jlp_test(jcurix_)=jlp_test(jcurix_) + j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))
							endif !if((iv2xykypos_==ixkyk_).and.(ifact_.eq.iv3factpos_))  22571
							if((iv2xykypos_==ixkyk_).and.(ifact_.eq.jlp_keyfact(iunit_,ixkyk_))) then
								jlp_test(jcurix_)=jlp_test(jcurix_) - j_v(jlp_fyfactout(iv2xykypos_,iv3factpos_))
							endif !if((iv2xykypos_==ixkyk_).and.(ifact_.eq.jlp_keyfact(iunit_  22574
						enddo !ifact_=1,j_o(listf_)%i(1)  22569
					enddo !ivars_=1,j_o(listy_)%i(1)  22567
 
				enddo !k_=1,jlp_nfyrow(irowj_)  22563
			enddo !jjj=1,jlp_nrowpfy  22560
 
			write(n16,*)'lf(j)',jlp_lf(lfj_),'test(0:nrow)',(jlp_test(jjj),jjj=0,jlp_nrow)
 
		enddo !lfj_=1,jlp_lf0  22522
	...
	subroutine getsolx() !
		...
		if(nz.eq.0)then
			...
			do i=1,jlp_nunits  !***********************
				...
				do jj=1,jlp_nrowp
					j=jlp_ixcurrows(jj)
					jlp_solx(j)=jlp_solx(j)+jlp_xmat(jlp_ix(j)+ibxmatx)
				enddo !jj=1,jlp_nrowp  22604
			enddo !i=1,jlp_nunits  22597
		endif !if(nz.eq.0)  22594
		...
		if(jlp_ivsubtrans>0) then
			...
			noutsubtrans = 0
		endif !if(jlp_ivsubtrans>0)  22612
		...
		if(ivarea.gt.0.and.notareavars.gt.0)then
			...
			if(nxvarareatot.lt.jlp_nxvartot.or.notc.gt.0)then
				...
				call j_uniondif(j_o(jlp_ivkeepx)%i2,jlp_nxvartot,jlp_cvarl,ncvar,&
					...
					jlp_xvarlareatot,nxvarareatot)
				ipart=1
			endif !if(nxvarareatot.lt.jlp_nxvartot.or.notc.gt.0)  22631
		elseif(ivarea.gt.0)then !if(ivarea.gt.0.and.notareavars.gt.0)then
			nxvarareatot = jlp_nxvartot + noutsubtrans
			allocate(jlp_xvarlareatot(1:nxvarareatot))
			jlp_xvarlareatot(1:jlp_nxvartot)= j_o(jlp_ivkeepx)%i2(1:jlp_nxvartot)
			if(noutsubtrans>0) jlp_xvarlareatot(jlp_nxvartot+1:nxvarareatot)=j_o(ivoutsubtrans)%i2(1:noutsubtrans)
		endif !if(ivarea.gt.0.and.notareavars.gt.0)  22627
		jlp_sumx=j_0
		do iuni=1,jlp_nunits
			call j_getobsiv(iuni,jlp_ivmatc,jlp_ivkeepc,jlp_ivunit) !,jlp_ivtransc,j_ivunit)
			if(j_err)then
				write(6,*)'error for unit ',iuni
			endif !if(j_err)  22647
			if(jlp_ivtrans.gt.0)then
				call dotrans(jlp_ivtrans,1)
				if(j_err)stop 770
			endif !if(jlp_ivtrans.gt.0)  22650
			iobs=jlp_ibaunit(iuni)+jlp_keys(iuni)+jlp_ibaunitbas
			call j_getobsiv(iobs,jlp_ivmatx,jlp_ivkeepx,0)! ,jlp_ivtransx,0)  !getsolx
			if(j_err)then
				write(6,*)'error for observation ',iobs
				stop 4
			endif !if(j_err)  22656
			if(jlp_ivsubtrans.gt.0)call dotrans(jlp_ivsubtrans,1)
			if(ivarea.gt.0)then
				if(ipart.eq.0)then
					...
					j_v(jlp_xvarlareatot)=j_v(ivarea)*j_v(jlp_xvarlareatot)
				endif !if(ipart.eq.0)  22662
			endif !if(ivarea.gt.0)  22661
			if(jlp_ndom.gt.0)then
				iba=0
				idomba=(iuni-1)*jlp_ndomv
				do id=1,jlp_ndom
					...
					if(btest(jlp_domainbits(idomba+icurint),icurbit))then
						...
						do jx=1,jlp_nxvartot
							jlp_sumx(iba+jx)=jlp_sumx(iba+jx)+ j_v(j_o(jlp_ivkeepx)%i2(jx))
						end do !jx=1,jlp_nxvartot  22678
					...
					end if !if(btest(jlp_domainbits(idomba+icurint),icurbit))  22675
					iba=iba+jlp_nxvartot+ncvar+noutsubtrans
				end do !id=1,jlp_ndom  22673
			else !if(j_ndom.gt.0)then
				...
				if(jlp_ivsubtrans>0) jlp_sumx(jlp_nxvartot+ncvar+1:jlp_nxvartot+ncvar+noutsubtrans) = &
					jlp_sumx(jlp_nxvartot+ncvar+1:jlp_nxvartot+ncvar+noutsubtrans) + &
					j_v(j_o(ivoutsubtrans)%i2(1:noutsubtrans))
			end if !if(jlp_ndom.gt.0)  22670
		end do !iuni=1,jlp_nunits  22645
		...
		do iuni=1,jlp_nunits
			...
			if(j_err)then
				write(6,*)'error for unit ',iuni
				stop 512
			endif !if(j_err)  22707
			if(jlp_ivtrans.gt.0)then
				call dotrans(jlp_ivtrans,1)
				if(j_err)stop 651
 
			endif !if(jlp_ivtrans.gt.0)  22711
			iobs=jlp_ibaunit(iuni)+integerschedw(iuni)   !+j_ibaunitbas
			call j_getobsiv(iobs,jlp_ivmatx,jlp_ivkeepx,0)! ,jlp_ivtransx,0)  !getsolx
			if(j_err)then
				write(6,*)'error for observation ',iobs
			endif !if(j_err)  22718
			if(jlp_ivsubtrans.gt.0)then
				call dotrans(jlp_ivsubtrans,1)
				if(j_err)stop 719
			endif !if(jlp_ivsubtrans.gt.0)  22721
			if(ivarea.gt.0)then
				if(ipart.eq.0)then
					...
					j_v(jlp_xvarlareatot)=j_v(ivarea)*j_v(jlp_xvarlareatot)
				endif !if(ipart.eq.0)  22726
			endif !if(ivarea.gt.0)  22725
			...
			if(jlp_ndom.gt.0)then
				...
				do id=1,jlp_ndom
					...
					if(btest(jlp_domainbits(idomba+icurint),icurbit))then
						...
						do jx=1,jlp_nxvartot
							jlp_sumxi(iba+jx)=jlp_sumxi(iba+jx)+ j_v(j_o(jlp_ivkeepx)%i2(jx))
						end do !jx=1,jlp_nxvartot  22743
					...
					end if !if(btest(jlp_domainbits(idomba+icurint),icurbit))  22740
 
					iba=iba+jlp_nxvartot+ncvar+noutsubtrans
				end do !id=1,jlp_ndom  22738
			else !if(j_ndom.gt.0)then
				...
				if(jlp_ivsubtrans>0) jlp_sumxi(jlp_nxvartot+ncvar+1:jlp_nxvartot+ncvar+noutsubtrans) = &
					jlp_sumxi(jlp_nxvartot+ncvar+1:jlp_nxvartot+ncvar+noutsubtrans) + j_v(j_o(ivoutsubtrans)%i2(1:noutsubtrans))
			end if !if(jlp_ndom.gt.0)  22735
		end do !iuni=1,jlp_nunits  22705
		...
		do i=1,jlp_ld0
			...
			if(ivarea.gt.0)then
				if(ipart.eq.0)then
					...
					j_v(jlp_xvarlareatot)=j_v(ivarea)*j_v(jlp_xvarlareatot)
				endif !if(ipart.eq.0)  22781
			endif !if(ivarea.gt.0)  22780
			if(jlp_ndom.gt.0)then
				iba=0
				idomba=(iuni-1)*jlp_ndomv
				do id=1,jlp_ndom
					...
					if(btest(jlp_domainbits(idomba+icurint),icurbit))then
						do jx=1,jlp_nxvartot
							jlp_sumx(iba+jx)=jlp_sumx(iba+jx)-wei* j_v(j_o(jlp_ivkeepx)%i2(jx))
						end do !jx=1,jlp_nxvartot  22793
 
						do jx=1,noutsubtrans
							jlp_sumx(iba+jlp_nxvartot+ncvar+jx)=jlp_sumx(iba+jlp_nxvartot+ncvar+jx) &
								- wei*j_v(j_o(ivoutsubtrans)%i2(jx))
						enddo !jx=1,noutsubtrans  22797
					end if !if(btest(jlp_domainbits(idomba+icurint),icurbit))  22792
 
					iba=iba+jlp_nxvartot+noutsubtrans
				end do !id=1,jlp_ndom  22790
			else !if(j_ndom.gt.0)then
				do jx=1,jlp_nxvartot
					jlp_sumx(jx)=jlp_sumx(jx)-wei* j_v(j_o(jlp_ivkeepx)%i2(jx))
				end do !jx=1,jlp_nxvartot  22806
 
				do jx=1,noutsubtrans
					jlp_sumx(jlp_nxvartot+ncvar+jx)=jlp_sumx(jlp_nxvartot+ncvar+jx) &
						- wei*j_v(j_o(ivoutsubtrans)%i2(jx))
				enddo !jx=1,noutsubtrans  22810
			end if !if(jlp_ndom.gt.0)  22787
			...
			if(ivarea.gt.0)then
				if(ipart.eq.0)then  !if(nxvarareatot.eq.nxvartot)then
					...
					j_v(jlp_xvarlareatot)=j_v(ivarea)*j_v(jlp_xvarlareatot)
				endif !if(ipart.eq.0)  22821
			endif !if(ivarea.gt.0)  22820
			...
			if(jlp_ndom.gt.0)then
				...
				do id=1,jlp_ndom
					...
					if(btest(jlp_domainbits(idomba+icurint),icurbit))then
						do jx=1,jlp_nxvartot
							jlp_sumx(iba+jx)=jlp_sumx(iba+jx)+wei* j_v(j_o(jlp_ivkeepx)%i2(jx))
						end do !jx=1,jlp_nxvartot  22834
 
						do jx=1,noutsubtrans
							jlp_sumx(iba+jlp_nxvartot+ncvar+jx)=jlp_sumx(iba+jlp_nxvartot+ncvar+jx) &
								+ wei*j_v(j_o(ivoutsubtrans)%i2(jx))
						enddo !jx=1,noutsubtrans  22838
					end if !if(btest(jlp_domainbits(idomba+icurint),icurbit))  22833
 
					iba=iba+jlp_nxvartot+noutsubtrans
				end do !id=1,jlp_ndom  22831
			else !if(j_ndom.gt.0)then
				do jx=1,jlp_nxvartot
					jlp_sumx(jx)=jlp_sumx(jx)+wei* j_v(j_o(jlp_ivkeepx)%i2(jx))
				end do !jx=1,jlp_nxvartot  22847
 
				do jx=1,noutsubtrans
					jlp_sumx(jlp_nxvartot+ncvar+jx)=jlp_sumx(jlp_nxvartot+ncvar+jx) &
						+ wei*j_v(j_o(ivoutsubtrans)%i2(jx))
				enddo !jx=1,noutsubtrans  22851
 
			end if !if(jlp_ndom.gt.0)  22828
		end do !i=1,jlp_ld0  22769
	...
	subroutine defsolu()
		...
		do i=1,jlp_ld0
			...
			if(jlp_lunit(id).ne.nunv)then
				...
				sum=0.
				iplace=0
			end if !if(jlp_lunit(id).ne.nunv)  22897
			!
			if(jlp_isch(id).gt.jlp_keys(jlp_lunit(id)).and.iplace.eq.0)then   !reserve place for
				idiv=idiv+1
				iplace=idiv
			end if !if(jlp_isch(id).gt.jlp_keys(jlp_lunit(id)).and.iplace.eq.0  22911
			idiv=idiv+1
			...
			sum=sum+jlp_x(jlp_nrowz+id)
			nunv=jlp_lunit(id)
		end do !i=1,jlp_ld0  22893
 
		if(iplace.eq.0)then   ! key is largest
			idiv=idiv+1;iplace=idiv
		end if !if(iplace.eq.0)  22924
		jlp_wdiv(iplace)=1.-sum
		jlp_iunitdiv(iplace)=nunv
		isdiv0(iplace)=jlp_keys(nunv)
		smax=maxval(isdiv0)
		...
		do i=1,jlp_ndiv
			jlp_wdiv(i)=sortkey(iperm(i))
		enddo !i=1,jlp_ndiv  22936
 
		do i=1,jlp_ndiv
			jlp_isdiv(i)=isdiv0(iperm(i))
		end do !i=1,jlp_ndiv  22940
		isdiv0=jlp_iunitdiv
		do i=1,jlp_ndiv
			jlp_iunitdiv(i)=isdiv0(iperm(i))
		end do !i=1,jlp_ndiv  22944
	...
	subroutine defsoluf()
		...
		do i_= jlp_i0_xkfsol,jlp_lf0_xkfsol	! kannan tehdasmuuttujat
			...
			if(prevcol_(jlp_xkfsol(i_)%iunit)>jlp_mxd) &
				jlp_xkfsol(prevcol_(jlp_xkfsol(i_)%iunit))%next = i_
			prevcol_(jlp_xkfsol(i_)%iunit) = i_
		enddo !i_= jlp_i0_xkfsol,jlp_lf0_xkfsol  22968
	...
	subroutine printxkf()
		...
		do iuni=1,jlp_nunits	! laskentayksiköt
			...
			do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
				keyf_ = jlp_keyfact(iuni,ixk_)
				sumxkf(ixk_,keyf_) = sumxkf(ixk_,keyf_) + j_v(j_o(jlp_ivxkyk)%i(ixk_))
			enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  23006
		enddo !iuni=1,jlp_nunits  22997
		...
		do i=1,jlp_ld0	!kannan ve-sarakkeet
			...
			do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
				keyf_ = jlp_keyfact(iuni,ixk_)
				sumxkf(ixk_,keyf_) = sumxkf(ixk_,keyf_) - wei*j_v(j_o(jlp_ivxkyk)%i2(ixk_))
			enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  23025
			...
			do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
				keyf_ = jlp_keyfact(iuni,ixk_)
				sumxkf(ixk_,keyf_) = sumxkf(ixk_,keyf_) + wei*j_v(j_o(jlp_ivxkyk)%i2(ixk_))
			enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  23035
		enddo !i=1,jlp_ld0  23013
		...
		do i= jlp_mxd+1,jlp_lf0	! kannan tehdasmuuttujat
			...
			sumxkf(ixk_,keyf_) = sumxkf(ixk_,keyf_) - jlp_x(jlp_lf(i)+jlp_nrowz)
			sumxkf(ixk_,ifact_) = sumxkf(ixk_,ifact_) + jlp_x(jlp_lf(i)+jlp_nrowz)
		enddo !i= jlp_mxd+1,jlp_lf0  23041
		...
		do if_= 1,j_o(jlp_ivfact)%i(1)
			do  ixk_= 1,j_o(jlp_ivxkyk)%i(1)
				do ixkf_=1,jlp_nxkfact(ixk_)
					if (if_ == jlp_xkfact(ixk_,ixkf_)%ifact) then
						write(n16,*) j_vname(j_o(jlp_ivxkyk)%i2(ixk_)),j_vname(j_o(jlp_ivfact)%i2(if_)),sumxkf(ixk_,if_)
					endif !if (if_ == jlp_xkfact(ixk_,ixkf_)%ifact)  23054
				enddo !ixkf_=1,jlp_nxkfact(ixk_)  23053
			enddo ! ixk_= 1,j_o(jlp_ivxkyk)%i(1)  23052
		enddo !if_= 1,j_o(jlp_ivfact)%i(1)  23051
			...
			do iuni=1,jlp_nunits	! laskentayksiköt
				...
				do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
					...
					if(sumxkf(ixk_,keyf_)<0.0) then
						...
						write(n16,*)'  xk, fact : ', &
							j_vname(j_o(jlp_ivxkyk)%i2(ixk_)), j_vname(j_o(jlp_ivfact)%i2(keyf_))
					endif !if(sumxkf(ixk_,keyf_)<0.0)  23083
				enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  23079
			enddo !iuni=1,jlp_nunits  23070
			...
			do i=1,jlp_ld0	!kannan ve-sarakkeet
				...
				do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
					...
					sumxkf2(ixk_,keyf_) = sumxkf2(ixk_,keyf_) +  &
						wei*j_v(j_o(jlp_ivxkyk)%i2(ixk_))
					if(sumxkf(ixk_,keyf_)<0.0) then
					endif !if(sumxkf(ixk_,keyf_)<0.0)  23109
				enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  23105
				...
				do ixk_=1,j_o(jlp_ivxkyk)%i(1)	! puutavaralaji-muuttujat
					...
					if(sumxkf(ixk_,keyf_)<0.0) then
						write(n16,*)'Avainve -: unit, ve, paino, ixk,ifact, -, yht : ' ,&
							...
							wei*j_v(j_o(jlp_ivxkyk)%i2(ixk_)),&
							sumxkf2(ixk_,keyf_)
					endif !if(sumxkf(ixk_,keyf_)<0.0)  23123
				enddo !ixk_=1,j_o(jlp_ivxkyk)%i(1)  23119
 
			enddo !i=1,jlp_ld0  23094
			...
			do i= jlp_mxd+1,jlp_lf0	! kannan tehdasmuuttujat
				...
				if (sumxkf(ixk_,ifact_)<0.0) then
					write(n16,*)'Kantaxkf + : srk, unit,ixk,ifact, +, yht : ', &
						...
						sumxkf2(ixk_,ifact_)
					write(n16,*)'  xk, fact : ',j_vname(j_o(jlp_ivxkyk)%i2(ixk_)), j_vname(j_o(jlp_ivfact)%i2(ifact_))
				endif !if (sumxkf(ixk_,ifact_)<0.0)  23142
				if(sumxkf(ixk_,keyf_)<0.0) then
					write(n16,*)'Avaint. - : srk, unit, ixk,ikeyf, -, yht : ', &
						jlp_lf(i),iuni, ixk_,keyf_, jlp_x(jlp_lf(i)+jlp_nrowz), sumxkf2(ixk_,keyf_)
					write(n16,*)'  xk, fact : ',j_vname(j_o(jlp_ivxkyk)%i(ixk_)), &
						j_vname(j_o(jlp_ivfact)%i2(keyf_))
				endif !if(sumxkf(ixk_,keyf_)<0.0)  23148
			enddo !i= jlp_mxd+1,jlp_lf0  23133
 
			deallocate(sumxkf2)
		endif !if(p.and.any(sumxkf<0.0))  23062
	...
	subroutine printsumxkyk(idom_)
		...
		do ixk_ = 1,j_o(jlp_ivxkyk)%i(1)
			...
			call j_getname(j_o(jlp_ivxkyk)%i2(ixk_))
			write(n16,*) j_oname(1:j_loname),jlp_sumx((idom_-1)*jlp_nxvartot+jx)
		enddo !ixk_ = 1,j_o(jlp_ivxkyk)%i(1)  23169
		return
