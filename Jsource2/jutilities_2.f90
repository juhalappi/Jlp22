...
subroutine j_startfunction(iob,io,iptype,narg,arg,ivout,delout,minarg,maxarg,needsout,argval,nval) ! %%function
	...
	if(present(minarg))then
		if(nargtot.lt.minarg)then
			...
			j_err=.true.
		endif !if(nargtot.lt.minarg)    413
	endif !if(present(minarg))    412
	if(present(maxarg))then
		...
		endif !if(nargtot.gt.maxarg)    422
	endif !if(present(maxarg))    421
...
subroutine j_getoption(iob,io,moption,minarg,maxarg,iptype,needsarg,noptarg,optarg,link) ! %%option
	...
	if(moption<=0.or.moption>j_noptions.or.iptype.lt.-1.or.iptype.gt.j_nobjecttypes) then
		...
		!	j_err = .true.
		return
	endif !if(moption<=0.or.moption>j_noptions.or.iptype.lt.-1.or.ipt    625
			...
			do i=1,noptarg
				if(iptype.eq.-1)then
					if(j_otype(optarg(i)).ne.j_ipreal)then
						...
						call j_del(optarg(i))
					endif !if(j_otype(optarg(i)).ne.j_ipreal)    671
				elseif(j_otype(optarg(i)).ne.iptype)then
					...
					j_err=.true.
				endif !if(iptype.eq.-1)    670
			enddo !i=1,noptarg    669
 
		endif !if(iptype.gt.0.or.iptype.eq.-1)    664
		!write(6,*)'<668',noptarg,optarg
	elseif(ili.eq.0.and.minarg.gt.0)then !if(ili>0) then
		if(needsarg) then
			write(6,*)'option ',j_option_name(moption,j_lenoptions(moption)),' must have arguments'
			j_err = .true.
		endif !if(needsarg)    693
		optarg => j_arglist0
	else !if(ili>0) then
		optarg=>j_arglist1
	endif !if(ili>0)    652
...
recursive subroutine j_command(commandline,passoptions) !execute single %%function from within other function (%%basic)
	...
	endif !if(ivjcommand(j_recursion).eq.0)    844
...
1 inprint=j_v(j_ivprintinput)
	!		write(6,*)'inprint',inprint
			...
			if(j_inp(1:j_linp).eq.'end')then
				j_stop=.true.
				return
			endif !if(j_inp(1:j_linp).eq.'end')    903
		...
		endif !if(commandline(1:6).eq.';incl(')    882
	endif !if(lec.gt.7)    881
...
700	continue
	...
	if(le.gt.j_lenobject)then
		...
		j_err=.true.;return
	endif !if(le.gt.j_lenobject)   1141
	...
	if(ivin.gt.0)then
		...
		endif !if(j_otype(ivin).eq.j_ipchar.and.itype.eq.j_ipchar)   1147
 
 
	else
		name2=name(li1:lena)
		le=lena-li1+1
	endif !if(ivin.gt.0)   1146
	...
	if(ivout.gt.0)then
		...
		if(j_otype(ivout).ne.itype.or.del)then
			call j_del(ivout)
			j_otype(ivout)=itype
		endif !if(j_otype(ivout).ne.itype.or.del)   1184
		j_getobject=ivout
		return
	endif !if(ivout.gt.0)   1182
	...
	if(j_named.ge.j_mxnamedv) then
		...
		j_err=.true.
		return
	endif !if(j_named.ge.j_mxnamedv)   1193
	j_named=j_named+1
	ivout=j_named
	!	if(ivin.le.0)then
	...
	endif !if(itype.eq.j_ipchar.and.name2(1:1).ne."'")   1216
...
subroutine j_getobjectnam(name,itype,ivout) ! %%object get old or new object, UUSI
	...
	if(index(name,':').gt.0.and.name(1:1).ne."'")then
		...
		j_err=.true.
		return
	endif !if(index(name,':').gt.0.and.name(1:1).ne."'")   1267
	if(index(name,"'").gt.0.and.name(1:1).ne."'")then
		write(6,*)"object name cannot contain ' as in ",name
		j_err=.true.
		return
	endif !if(index(name,"'").gt.0.and.name(1:1).ne."'")   1272
	ishipsu=name(1:1).eq."'"
	if(itype.eq.j_ipchar.and.ishipsu)then
		ivout=j_line(j_ivnames,name)
	elseif(itype.eq.j_ipchar)then
		ivout=j_line(j_ivnames,"'"//name//"'")
	endif !if(itype.eq.j_ipchar.and.ishipsu)   1278
	...
	if(j_named.ge.j_mxnamedv) then
		...
		j_err=.true.
		return
	endif !if(j_named.ge.j_mxnamedv)   1288
	j_named=j_named+1
	ivout=j_named
	le=len(name)
	if(itype.eq.j_ipchar)then
		...
		do li1=1,le
			if(.not.(j_isletter(name(li1:li1)).or.name(li1:li1).eq."'"))then
				...
				j_named=j_named-1
				return
			end if !if(.not.(j_isletter(name(li1:li1)).or.name(li1:li1).eq."'"   1316
		enddo !li1=1,le   1315
		call j_puttext(j_ivnames,name)
	endif !if(itype.eq.j_ipchar)   1297
...
subroutine j_getder(ivfunc,args,ders,nder,old)
	...
	if(nder.gt.j_mxder)then
		...
		enddo !iv=j_mxnamedv+j_mxtemporalv0+1,j_mxnamedv+j_mxtemporalv   1346
 
 
	endif !if(nder.gt.j_mxder)   1344
	do ina=1,nder
		!	nteku=nteku+1
...
recursive subroutine j_del(iv) ! %%object delete object , subobects are also deleted,
	...
	if(j_iounit(iv).ne.0)then !if(j_o(iv)%i(3).eq.0)then
		...
		do iiv=1,j_mxnamedv
			...
			endif !if(j_otype(iiv).eq.j_ipchar)   1439
 
		enddo !iiv=1,j_mxnamedv   1438
 
 
 
		j_err=.true.
		return
	endif !if(j_iounit(iv).ne.0)   1432
	...
	elseif(j_otype(iv).gt.j_nobjecttypes)then !if(j_otype(iv).gt.j_nobjecttypes.and. j_otype(iv).le.j_nobjecttypes1)then
		...
		j_err=.true.
		return
	endif !if(j_otype(iv).gt.j_nobjecttypes.and. j_otype(iv).le.j_nob   1482
...
subroutine j_del0(iv) !delete %%object , subobects are not deleted
	...
	if(j_otype(iv).eq.j_ipchar)then
		if(j_o(iv)%i(3).eq.0)then   !character constants cannot be deleted
			...
			return
		endif !if(j_o(iv)%i(3).eq.0)   1514
	endif !if(j_otype(iv).eq.j_ipchar)   1513
	...
	elseif(j_otype(iv).gt.j_nobjecttypes)then !if(j_otype(iv).gt.j_nobjecttypes.and. j_otype(iv).le.j_nobjecttypes1)then
		...
		j_err=.true.
		return
	endif !if(j_otype(iv).gt.j_nobjecttypes.and. j_otype(iv).le.j_nob   1521
	...
	do i=ial,lop
		...
		if(line(i:i).eq.'\')then
			...
			if(line(i+1:i+1).eq.'#')then
				...
				endif !if(line(i+2:i+2).eq.'a')   1570
				ial=i+2
			...
			elseif(line(i+1:i+6).eq.'greek_')then
				...
				goto 10
				!				{/Symbol:Plain=12 l
			endif !if(line(i+1:i+1).eq.'#')   1569
		endif !if(line(i:i).eq.'\')   1567
	end do !i=ial,lop   1565
	le=lop
	return
...
integer function j_defmatrix8(iv,name,ndim1,ndim2,itype,single,nod,rowtot,point,temp) ! defines a matrix object %%matrix
	...
	if(ndim1.le.0.or.ndim2.le.0)then
		...
		j_err=.true.
		return
	endif !if(ndim1.le.0.or.ndim2.le.0)   1751
	...
	endif !if(itype.eq.j_matdiag.and.ndim1.ne.ndim2)   1758
	...
	if(.not.no)then
		...
		end if !if(itype.eq.j_matreg.or.itype.eq.j_matdiag.or.itype.eq.j_m   1829
	endif !if(.not.no)   1826
	j_defmatrix8=ivout
...
integer function j_from(iob,io,ivmat,until)
	...
	if(narg.gt.0)then
		...
		if(j_from.le.0.or.j_from.gt.j_o(ivmat)%i(1))then
			...
			j_err=.true.
		endif !if(j_from.le.0.or.j_from.gt.j_o(ivmat)%i(1))   2051
 
		return
	endif !if(narg.gt.0)   2048
	...
	if(narg.lt.0)then
		...
		if(j_from.le.0.or.j_from.gt.j_o(ivmat)%i(1))then
			...
			j_err=.true.
		endif !if(j_from.le.0.or.j_from.gt.j_o(ivmat)%i(1))   2065
	endif !if(narg.lt.0)   2061
	...
	if(narg.lt.0)then
		...
		if(until.lt.j_from.or.until.gt.j_o(ivmat)%i(1))then
			...
			j_err=.true.
		endif !if(until.lt.j_from.or.until.gt.j_o(ivmat)%i(1))   2076
	endif !if(narg.lt.0)   2072
...
integer*8 function j_nrows(iv,mi,ma)
	...
	if(present(ma))then
		if(j_nrows.gt.ma)then
			...
			j_err=.true.
			return
		endif !if(j_nrows.gt.ma)   2117
	endif !if(present(ma))   2116
...
integer*8 function j_ncols(iv,mi,ma)
	...
	if(present(ma))then
		if(j_ncols.gt.ma)then
			...
			j_err=.true.
			return
		endif !if(j_ncols.gt.ma)   2164
	endif !if(present(ma))   2163
...
integer*8 function j_nelem(iv)
	...
	elseif(j_otype(iv).eq.j_ipreal)then
		...
		write(6,*)j_oname(1:j_loname),' is not MATRIX or REAL'
		j_err=.true.;return
	endif !if(j_otype(iv).eq.j_ipmatrix)   2177
...
subroutine j_cleartrans(ivtrans) !clears %%trans %%object without deleting it
	...
	if(j_otype(ivtrans)==j_iptrans) then
		...
		return
	endif !if(j_otype(ivtrans)==j_iptrans)   2372
	return
	...
	if(ivsource.ne.0)then
		...
		do i=1,j_o(ivsource)%i(0)
			...
			if(j_o(ivsource)%i2(i).ge.io-2)then
				...
				return
			endif !if(j_o(ivsource)%i2(i).ge.io-2)   2433
		enddo !i=1,j_o(ivsource)%i(0)   2431
 
	else !if(ivsource.ne.0)then
		call j_printname('transformation set=',iob,' ')
	endif !if(ivsource.ne.0)   2429
...
subroutine j_debugerr(iob,io)  !writes waht is the origin of an error %%error
	...
	if(ivsource.ne.0)then
		...
		do i=1,io-1
			...
			endif !if(j_o(iob)%i(i).lt.0)   2511
	...
	do loc=1,j_nopt
		if(j_optioniob(loc).eq.iob)then
			...
			endif !if(loc.lt.j_nopt)   2531
			j_nopt=j_nopt-1
		endif !if(j_optioniob(loc).eq.iob)   2530
	enddo !loc=1,j_nopt   2529
...
subroutine j_where(iob,io)
	...
	endif !if(notsame)   2592
	notsame=.false.
	if(n2.eq.n3)then
		notsame=any(j_o(j_ivinput1)%ch(1:n2).ne.j_o(j_ivinput2)%ch(1:n3))
	else !if(n2.eq.n3)then
		notsame=.true.
	endif !if(n2.eq.n3)   2602
	if(notsame)then
		!	write(6,*)'**input after interpreting input programming: '
		write(6,*)'interpreted'
		call j_printtext(j_ivinput2,0)
	endif !if(notsame)   2607
...
subroutine j_puttext(iv,text) ! put %%text into text %%object
	...
	endif !if(j_otype(iv).ne.j_iptext.and.j_otype(iv).ne.j_ipfigure)   2647
	if(j_err)then
	...
	endif !if( j_o(iv)%i(0) .ge.ubound(j_o(iv)%i,dim=1))   2676
...
subroutine j_gettext(iob,line) ! print line of %%text object, if line=0 -> print all  (%%io)
	...
	if(line.gt.j_o(iob)%i(0).or.line.le.0)then
		...
		write(6,*)'*illegal line ',line, ' for text object ',iv,&
			' having ',j_o(iob)%i(0), ' lines'
		j_err=.true.;return
	endif !if(line.gt.j_o(iob)%i(0).or.line.le.0)   2840
	!	integer ::line2_
	...
	do j=line1,line2_
		...
		if(nu.ne.6)then
			...
			if(present(nonum))then
				...
				endif !if(j.le.99)   2909
			endif !if(present(nonum))   2902
			...
			if(present(nonum))then
				...
				endif !if(j.le.99)   2919
			endif !if(present(nonum))   2916
		endif !if(nu.ne.6)   2893
	end do !j=line1,line2_   2889
	return
...
subroutine j_getobjectname(ivin,name,name2,le2)  ! !!object
	...
	if(ivin.gt.0)then
		...
		endif !if(j_otype(ivin).eq.j_ipchar)   2942
	else !if(ivin.gt.0)then
		name2(1:le)=name(1:le)
		le2=le
		goto 90
	endif !if(ivin.gt.0)   2941
...
subroutine j_getline(iv,line,buffer,le) ! get line from %%text object iv into buffer
	...
	if(line.gt.j_o(iv)%i(0).or.line.le.0)then
		...
		j_err=.true.
		return
	endif !if(line.gt.j_o(iv)%i(0).or.line.le.0)   2988
	...
	do k=j_o(iv)%i(line),j_o(iv)%i(line+1)-1
		...
		if(le.gt.lebu)then
			...
			return
		endif !if(le.gt.lebu)   3000
		buffer(le:le)=j_o(iv)%ch(k)
		if(le.ge.lebu)return
	end do !k=j_o(iv)%i(line),j_o(iv)%i(line+1)-1   2998
	return
...
subroutine j_getname(iv,iv2,iv3) ! get line from %%text object iv into buffer
	...
	endif !if(present(iv2))   3025
	if(present(iv3))then
		!	write(6,*)'getnaiv2',iv2,j_named,
		if(iv3.ne.-1)call j_getname0(iv3,j_oname3,j_loname3)
 
 
	endif !if(present(iv3))   3031
...
character*1 function j_getnamech(iv)
	...
	endif !if(iv.le.j_named.and.iv.gt.0)   3047
...
subroutine j_getname0(iv,name,lname)
	...
	endif !if(iv.le.j_named.and.iv.gt.0)   3070
...
subroutine j_getline2(iv,line,buffer,le) !%%text as getline but fills end of buffer with blanks
	...
	if(line.gt.j_o(iv)%i(0).or.line.le.0)then
		...
		j_err=.true.
		return
	endif !if(line.gt.j_o(iv)%i(0).or.line.le.0)   3106
	...
	end do !k=j_o(iv)%i(line),j_o(iv)%i(line+1)-1   3116
	buffer(le+1:)=' '
...
subroutine j_getchar(iv,buffer,le) !%%io get %%char constant or char variable or real var as char*8
	...
	endif !if(j_otype(iv).eq.j_ipchar)   3139
	return
	...
	if(j_otype(iv).eq.j_ipchar)then
		...
		do i=j_o(iv)%i(1),j_o(iv)%i(2)
			...
			if(le.gt.lem)then
				...
				j_err=.true.
				return
			endif !if(le.gt.lem)   3183
			buffer(le:le)=j_o(j_ivnames)%ch(i)
		end do !i=j_o(iv)%i(1),j_o(iv)%i(2)   3181
	elseif(iv.eq.j_ivdollar)then !if(j_otype(iv).eq.j_ipchar)then
		le=1
		buffer(1:1)='*'
	else !if(j_otype(iv).eq.j_ipchar)then
		...
		j_err = .true.
	endif !if(j_otype(iv).eq.j_ipchar)   3178
	return
...
subroutine j_getchar3(iv,buffer,le,ext) !get character
	...
	if(present(iv))then
		...
		endif !if(j_otype(iv).eq.j_ipchar)   3209
	endif !if(present(iv))   3208
	if(present(ext))then
		le2=j_lentrim(ext)
		buffer(le+1:le+le2)=ext
		le=le+le2
	endif !if(present(ext))   3215
	return
	...
	if(present(iv))then
		...
		if(present(needed))then
			if(j_object.le.0)then
				...
				j_err=.true.
			endif !if(j_object.le.0)   3236
		endif !if(present(needed))   3235
	else
		if(len(name).le.0)then
			...
			if(present(needed))then
				if(j_object.le.0)then
					...
					j_err=.true.
				endif !if(j_object.le.0)   3245
			endif !if(present(needed))   3244
		elseif(name(1:1).ne.'@')then !if(len(name).le.0)then
			...
			i1=2
		end if !if(len(name).le.0)   3242
		if(present(needed))then
			if(j_object.le.0)then
				write(6,*)oname(i1:loname)//name,' does not exist'
				j_err=.true.
 
			endif !if(j_object.le.0)   3258
		endif !if(present(needed))   3257
 
	endif !if(present(iv))   3230
...
function j_object2(name,iv)! get index of %%object with name name//name_of_iv if not defined return 0
	...
	names: do j=1,j_o(j_ivnames)%i(0)
		if(j_o(j_ivnames)%i(j+1)-j_o(j_ivnames)%i(j).eq.le+le2)then
			...
			enddo !k=j_o(j_ivnames)%i(j),j_o(j_ivnames)%i(j)+le-1 ;kk=kk+1   3314
			kk=0
			do k=j_o(j_ivnames)%i(j)+le,j_o(j_ivnames)%i(j+1)-1
				...
				kk=kk+1
			enddo !k=j_o(j_ivnames)%i(j)+le,j_o(j_ivnames)%i(j+1)-1   3318
			j_object2=j;return
		endif !if(j_o(j_ivnames)%i(j+1)-j_o(j_ivnames)%i(j).eq.le+le2)   3312
	enddo names !es: do j=1,j_o(j_ivnames)%i(0)   3311
	j_object2=0
	return
...
function j_object3(iv,name,char)
	...
	if(iv.gt.j_named)then
		...
		j_err=.true.
		return
	endif !if(iv.gt.j_named)   3353
	if(iv.gt.0)then
		call j_getname(iv)
		if(le.gt.0)then
			j_oname(j_loname+1:j_loname+le)=name
			j_loname=j_loname+le
		endif !if(le.gt.0)   3360
	else
		j_oname=name
		j_loname=le
	endif !if(iv.gt.0)   3358
	...
	if(ischar)then
		...
		if(ishipsu)then
			...
			j_object3=j_object("'"//j_oname(1:j_loname)//"'")
		endif !if(ishipsu)   3373
	else
 
		j_object3=j_object(j_oname(1:j_loname))
	endif !if(ischar)   3371
	if(j_object3.gt.0)then
		if(ischar)j_o(j_object3)%i(3:4)=0
		return
 
	endif !if(j_object3.gt.0)   3382
	if(j_err)return
...
function j_line(iv,name)   !get line number of %%text object consisting of name, not found =>0
	...
	do j=1,j_o(iv)%i(0)
		...
		if(j_o(iv)%i(j+1)-j_o(iv)%i(j).eq.le)then
			...
			end do !k=j_o(iv)%i(j),j_o(iv)%i(j+1)-1   3419
			j_line=j
			return
		endif !if(j_o(iv)%i(j+1)-j_o(iv)%i(j).eq.le)   3417
5 	continue
	enddo !j=1,j_o(iv)%i(0)   3415
	j_line=0
	return
	...
	do j=1,j_o(ivtext)%i(0)
		if(j_o(ivtext)%i(j+1)-j_o(ivtext)%i(j).eq.le)then
			...
			do k=j_o(ivtext)%i(j),j_o(ivtext)%i(j+1)-1
				...
				kk=kk+1
			enddo !k=j_o(ivtext)%i(j),j_o(ivtext)%i(j+1)-1   3461
			j_line2=j;return
		endif !if(j_o(ivtext)%i(j+1)-j_o(ivtext)%i(j).eq.le)   3458
5 	continue
	enddo !j=1,j_o(ivtext)%i(0)   3457
	j_line2=0
...
subroutine j_putoi2(iv,iel,ival) !putting value ival into element iel the integer fork i2 of %%object iv
	...
	if(iel.gt.ubound(j_o(iv)%i2,dim=1))then
		...
		call j_inci2(iv,iel)
		! iel=iel0
	endif !if(iel.gt.ubound(j_o(iv)%i2,dim=1))   3553
	j_o(iv)%i2(iel)=ival
	return
...
subroutine j_incch(iv,mins) !increase size of %%char fork of an %%object iv i.e. o(iv)%ch
	...
	if(.not.allocated(j_o(iv)%ch))then
		...
		j_err=.true.
		return
	endif !if(.not.allocated(j_o(iv)%ch))   3620
...
subroutine j_inci(iv,mins) !increase size of the integer fork of an %%object iv ,i.e. o(iv)%i
	...
	if(.not.allocated(j_o(iv)%i))then
		...
		j_err=.true.
		return
	endif !if(.not.allocated(j_o(iv)%i))   3653
...
subroutine j_inci2(iv,mins) !increase size of the 2. integer fork of %%object iv, i.e. o(iv)%i2
	...
	if(.not.allocated(j_o(iv)%i2))then
		...
		j_err=.true.
		return
	endif !if(.not.allocated(j_o(iv)%i2))   3754
	...
	do i=1,n
		...
		if(le1.eq.le2) then
			...
			if(text(1:le1).eq.vector(i)(1:le2))then
				j_isin=i
				return
			endif !if(text(1:le1).eq.vector(i)(1:le2))   3783
		endif !if(le1.eq.le2)   3781
	enddo !i=1,n   3779
	j_isin=0
	return
...
function j_nonblank(inp,ial,lop)
	...
	do i=ial,lop
		if(ichar(inp(i:i)).gt.32)then
			j_nonblank=i
			return
		endif !if(ichar(inp(i:i)).gt.32)   4023
	enddo !i=ial,lop   4022
	j_nonblank=lop+1
	return
...
function j_lastblank(inp,ial,lop)
	...
	do i=lop,ial,-1
		if(inp(i:i)<=' ') then
			j_lastblank=i
			return
		endif !if(inp(i:i)<=' ')   4039
	enddo !i=lop,ial,-1   4038
	j_lastblank = 0
	return
...
subroutine j_reps2(inp,i1,i2,lop,aa)
	...
	if(ii2.eq.ii1+1.or.aa.eq.0.)then
		...
		do i=ie-1,2,-1
			...
			nz=nz+1
		enddo !i=ie-1,2,-1   4070
2   if(nz.ne.0.and.ie.lt.8)then
		!        write(6,*)nz,i,':',chv
			do  j=i+1,i+nz
				chv(j:j)=chv(j+nz:j+nz)
				!        write(6,*)j,':',chv
			enddo ! j=i+1,i+nz   4076
		endif !2   if(nz.ne.0.and.ie.lt.8)   4074
		!        write(6,*)i1,i2,lop,chv,nz
		call j_repl(inp,i1,i2,lop,chv,8-nz)
		!        write(6,*)lop,':',inp
	endif !if(ii2.eq.ii1+1.or.aa.eq.0.)   4062
	return
...
subroutine j_repl(jono1,i1,i2,linp,jono2,le2) !replaces the substring jono1(i1:i2) by string jono2(1:le2)
	...
	if(nc.lt.0)then
		...
		if(j.gt.linp2)then
			...
			stop 'per..'
			!         return
		endif !if(j.gt.linp2)   4114
		do  i=linp,i2+1,-1
			jono1(j:j)=jono1(i:i)
			j=j-1 !do 2 i=linp,i2+1,-1
		enddo ! i=linp,i2+1,-1   4119
	endif !if(nc.lt.0)   4102
...
1 format(i8)
	...
	if(jero.lt.0)then
		...
		enddo ! k=lop,i2+1,-1   4157
		!     if(inp(1:1).eq.'$')write(6,*)'häär',inp
	end if !if(jero.lt.0)   4148
...
25    format(g16.3)
		...
		endif !if(buf(j+1:j+1).eq.'0')   4356
		...
		if(buf(13:16).eq.' ')then
			...
			j_chr8b=buf(j:i)//buf(13:last)
		endif !if(buf(13:16).eq.' ')   4360
		!      if(le.gt.8)  write(6,*)'chr8b:',chr8b,' le:',le
		return
	endif !if(a.gt.0.)   4246
...
3 format(g13.6)
	...
	do i=16,1,-1
		if(buf(i:i).eq.'0')then
			...
			exit
		endif !if(buf(i:i).eq.'0')   4382
	enddo !i=16,1,-1   4381
 
	if(buf(i:i).eq.'.')then
		buf(i:i)=' '
		i=i-1
	endif !if(buf(i:i).eq.'.')   4389
	le=i
	j_chr8b=buf(1:8)
	return
...
function j_nextlim(inp,ial,lop,limit,inhipsu)
		...
		do  i=ial,lop
			do  j=1,le
				if(inp(i:i).eq.limit(j:j))then
					j_nextlim=i
					return
				endif !if(inp(i:i).eq.limit(j:j))   4468
			enddo ! j=1,le   4467
 
		enddo ! i=ial,lop   4466
		j_nextlim=lop+1
		return
	endif !if(ishipsu)   4465
	...
	endif !if(ial.le.0.or.lop.gt.linp)   4484
	do  i=ial,lop
		if(inp(i:i).eq."'")then
			hipsu=.not.hipsu
			cycle
		endif !if(inp(i:i).eq."'")   4493
 
		if(inp(i:i).eq.'['.and..not.hipsu)then
			haka=.true.
			cycle
		endif !if(inp(i:i).eq.'['.and..not.hipsu)   4498
		if(haka.and..not.hipsu)then
			if(inp(i:i).eq.']')haka=.false.
			cycle
		endif !if(haka.and..not.hipsu)   4502
		if(haka.or.hipsu)cycle
 
		do  j=1,le
			if(inp(i:i).eq.limit(j:j)) goto 3
		enddo ! j=1,le   4508
	enddo ! i=ial,lop   4492
...
function j_prevlim(inp,ial,limit)
	...
	do  i=ial,1,-1
		if(inp(i:i).eq."'")then
			hipsu=.not.hipsu
			cycle
		endif !if(inp(i:i).eq."'")   4544
		if(nothakalim)then
			if(inp(i:i).eq.']'.and..not.hipsu)then
				haka=.true.
				cycle
			endif !if(inp(i:i).eq.']'.and..not.hipsu)   4549
			if(haka)then
				if(inp(i:i).eq.'[')haka=.false.
				cycle
			endif !if(haka)   4553
			if(haka.or.hipsu)cycle
		endif !if(nothakalim)   4548
		do  j=1,le
			if(inp(i:i).eq.limit(j:j)) goto 3
		enddo ! j=1,le   4559
		!2   continue !do 2 j=1,le
	enddo ! i=ial,1,-1   4543
...
subroutine j_adjul2(inp)
	...
	do  i=1,lop
		...
		if(ichar(inp(i:i)).gt.32)then
			if(i.gt.1)  inp=inp(i:)
			return
		endif !if(ichar(inp(i:i)).gt.32)   4800
	enddo ! i=1,lop   4798
...
subroutine j_gayainit(iob,io)
	...
	endif !if(j_g_maxvarold.lt.j_g_maxvar)   4919
...
subroutine j_f_pca_gaya(x,ix,p,ipp)
	...
	do 3 i=1,5
		...
		if(cc(1:3).eq.cw(i))then
			...
			jz=ichar(cc(4:4))
			goto 4
		endif !if(cc(1:3).eq.cw(i))   5072
3 continue !3 i=1,5   5070
31 continue
	ip=ipp
	ji=1
...
4 continue
	...
	do 1 i=1,ip
		...
		if(jz.ne.0.and.cc(1:3).eq.cw(ji))then
			...
			ix=ix+1
			x(ix)=p(i)
		endif !if(jz.ne.0.and.cc(1:3).eq.cw(ji))   5092
1 continue !1 i=1,ip   5090
	...
	if(jz.gt.0.and.jz.lt.255)then
		if(jz.ne.kz)then
			...
			stop 'F_PCA'
		endif !if(jz.ne.kz)   5108
	endif !if(jz.gt.0.and.jz.lt.255)   5107
	return
	...
	elseif(x.ge.j_o(ivmatrix)%d(npoint)	)then
		...
		do i=1,npoint
			if(x.le.j_o(ivmatrix)%d(i))then
				...
				i1=i
				exit
			endif !if(x.le.j_o(ivmatrix)%d(i))   5171
 
		enddo !i=1,npoint   5170
	endif !if(x.le.j_o(ivmatrix)%d(1))   5163
...
double precision function j_flini(np,x,y,r1,r2)
	...
	if(r1.ge.r2)then
		j_flini=0.
		return
	endif !if(r1.ge.r2)   5224
	if(np.le.1)then
		j_flini=y(1)*(r2-r1)
		return
	end if !if(np.le.1)   5228
	sum=0.d0
...
2 if(i.eq.1)i=2
	...
	do i=ia,np-1
		...
		endif !if(x(i+1).ge.r2.or.i.eq.np-1)   5254
	enddo !i=ia,np-1   5253
...
subroutine j_startfig(iob,io,update,linkxfunc) !defines a figure object
	...
	if(nupdate)then
		...
		if(j_gpiout.gt.j_named)then
			...
			if(j_o(iob)%i(j_gpnewio).eq.3)then !setelem
				...
				j_err=.true.;return
			endif !if(j_o(iob)%i(j_gpnewio).eq.3)   5288
		endif !if(j_gpiout.gt.j_named)   5284
		...
		if(noptarg.le.0)then
			...
			if(isi.le.0.or.isi.ge.10)then
				...
				j_err=.true.
				return
			endif !if(isi.le.0.or.isi.ge.10)   5326
			j_gpps(5:5)=char(48+isi)
		endif !if(noptarg.le.0)   5322
		...
		if(j_otype(j_gpiout).ne.j_ipfigure)then
			...
			j_gpappend=.false.
		end if !if(j_otype(j_gpiout).ne.j_ipfigure)   5344
	...
	if(nupdate)then
		...
		elseif(noptarg.eq.0)then !if(noptarg.lt.0)then
			...
			if(j_gpset.lt.0.or.j_gpset.gt.5)then
				...
				j_err=.true.;return
			endif !if(j_gpset.lt.0.or.j_gpset.gt.5)   5418
		endif !if(noptarg.lt.0)   5412
		...
		if(noptarg.le.0)then
			...
			if(j_otype(j_gpmark).ne.j_ipreal.and.j_otype(j_gpmark).ne.j_ipchar)then
				...
				j_err=.true.;return
			endif !if(j_otype(j_gpmark).ne.j_ipreal.and.j_otype(j_gpmark).ne.   5430
			j_gpmark2=j_gpmark
			if(noptarg.gt.1)then
				...
				if(j_otype(j_gpmark2).ne.j_ipreal.and.j_otype(j_gpmark2).ne.j_ipchar)then
					write(6,*)'mark-> must be REAL or CHAR'
					j_err=.true.;return
				endif !if(j_otype(j_gpmark2).ne.j_ipreal.and.j_otype(j_gpmark2).n   5437
 
 
			endif !if(noptarg.gt.1)   5435
 
		endif !if(noptarg.le.0)   5425
		...
		endif !if(noptarg.le.0)   5489
	endif !if(nupdate)   5409
	...
	if(j_gpletitle.gt.0)then
		...
		endif !if(ifo.gt.0)   5539
	endif !if(j_gpletitle.gt.0)   5534
	!
	...
	endif !if(j_gplexlabel.gt.0)   5546
	if(j_gpleylabel.gt.0)then
		...
		endif !if(ifo.gt.0)   5560
	endif !if(j_gpleylabel.gt.0)   5557
	!	call j_putfig0(3,'set ylabel "'//j_gpylabel(1:j_gpleylabel)//'"')
	...
	if(noptarg.eq.0.and.nupdate)then
		...
		endif !if(ii1.le.0.or.ii2.le.0)   5593
		!xmin=j_v(ii1)
		!xmax=j_v(ii2)
	...
	elseif(noptarg.gt.0)then
		...
		endif !if((noptarg.eq.1.or.noptarg.eq.3).and.lix.lt.0)   5610
		j_o(j_gpiout)%i2(4)=len_trim(j_o(j_gpiout)%txt(4))
 
	endif !if(noptarg.eq.0.and.nupdate)   5585
	if(nox.eq.1)then
		!		write(6,*)'hep',lix
		j_o(j_gpiout)%txt(4)='set xrange['//j_chr10(j_v(j_optarg(1)))//':]'
		j_o(j_gpiout)%i2(4)=len_trim(j_o(j_gpiout)%txt(4))
...
subroutine j_openread(fil,for,nu,irecl) ! open file for reading
	...
	if(j_nused.ge.j_mxunits) then
		...
		j_err = .true.
		return
	endif !if(j_nused.ge.j_mxunits)   5715
	...
	if(for(1:1).eq.'b'.or.for(1:1).eq.'B')then
		...
		endif !if(for(ise:ise).eq.'s')   5729
	elseif(for(1:1).eq.'d')then !if(for(1:1).eq.'b'.or.for(1:1).eq.'B')then
		if(.not.present(irecl))then
			...
			j_err=.true.;return
		endif !if(.not.present(irecl))   5736
		isc=1
	...
	end if !if(for(1:1).eq.'b'.or.for(1:1).eq.'B')   5722
	j_nused=j_nused+1
...
subroutine j_openreadiv(ivi,for,nu,ext,irecl) !get file for reading when file name is stored in character constant iv
	...
	if(iv.eq.j_ivdollar)then
		nu=5
		return
	endif !if(iv.eq.j_ivdollar)   5775
	if(j_otype(iv).eq.j_iptext)then
		nu=-iv
		return
	endif !if(j_otype(iv).eq.j_iptext)   5779
	ifi=iv
	if(j_otype(iv).ne.j_ipchar)then
		if(present(ext))then
			...
			j_err=.true.
			return
		endif !if(present(ext))   5785
	endif !if(j_otype(iv).ne.j_ipchar)   5784
	!file from filemod
	...
	if(j_nused.ge.j_mxunits)then
		write(6,*) '*j* mxunits'
		j_err=.true.
	endif !if(j_nused.ge.j_mxunits)   5804
	nu=j_nunits(j_nused+1)
	!write(6,*)'file:',fil,' ofr',for,nu
	if(for(1:1).eq.'*'.or.for(1:1).eq.'(')then
		if(ivi.gt.0)then
...
904			open(nu,err=904,file=j_filename(1:le),form='unformatted',status='old',&
		...
		goto 91
	end if !if(for(1:1).eq.'*'.or.for(1:1).eq.'(')   5810
...
subroutine j_getfile(nu,rw,ivfile,ivform,forma,ext,replace,irecl,ivout,readit,deleted) !get file for writing, name is stored in character constant ivfile
	...
	if(rw.ne.'r'.and.rw.ne.'w'.and.rw.ne.'a')then
		...
		j_err=.true.;		return
	endif !if(rw.ne.'r'.and.rw.ne.'w'.and.rw.ne.'a')   5901
	nudollar=.true.
	...
	if(present(ivfile))then
		...
		if(present(ext))then
			...
			j_err=.true.
			return
		endif !if(present(ext))   5941
	endif !if(present(ivfile))   5926
	...
	if(j_nused.ge.j_mxunits)then
		...
		j_err=.true.
		return
	endif !if(j_nused.ge.j_mxunits)   5953
	!	write(6,*)'lefi',lefi
	!	write(6,*)j_filename
	goto 67
...
67	if(ivform_.eq.j_ivdi)then
	...
	elseif(ivform_.eq.j_ivdg2)then !if(ivform_.eq.j_ivdi)then
		idac=4;isize=2
		if(nudollar)goto 66
	endif !67	if(ivform_.eq.j_ivdi)   5965
	if(rw.eq.'r'.and.nudollar)then
		nu=5
		return
	endif !if(rw.eq.'r'.and.nudollar)   5980
	...
	if(idac.ne.0.and..not.present(irecl))then
		...
		j_err=.true.
	endif !if(idac.ne.0.and..not.present(irecl))   5995
...
777			if(yes.and..not.replace2.and.rw.eq.'w')then
		...
		if(present(readit))then
			...
			if(ans.eq.'u'.or.ans.eq.'U')then
				...
				if(ope.and.ivform_.ne.j_ivdi.and.ivform_.ne.j_ivdi2)then !j_form(1:1).ne.'d')then
					...
					rewind(nu,err=981)
				endif !if(ope.and.ivform_.ne.j_ivdi.and.ivform_.ne.j_ivdi2)   6027
				if(ope)return
				useit=.true.
			elseif(ans.ne.'d'.and.ans.ne.'D')then !if(ans.eq.'u'.or.ans.eq.'U')then
				...
				if(present(readit))readit=.false.
				replace2=.true.
			endif !if(ans.eq.'u'.or.ans.eq.'U')   6025
			...
			if(ans.eq.'d'.or.ans.eq.'D')then
				...
				ans='y'
			endif !if(ans.eq.'d'.or.ans.eq.'D')   6045
			if(ans.ne.'y'.and.ans.ne.'Y')then
				write(6,*)'**was not allowed replace file:';j_err=.true.
				return
			endif !if(ans.ne.'y'.and.ans.ne.'Y')   6049
			if(present(deleted))deleted=.true.
		endif !if(present(readit))   6021
 
	endif !777			if(yes.and..not.replace2.and.rw.eq.'w')   6019
	...
	if(rw.eq.'r'.or.useit)then
		...
		elseif(idac.ne.0)then !if(iform_.eq.j_ivb.or.iform_.eq.j_ivb2)then
			...
			!	write(6,*)'<19efi'
		end if !if(ivform_.eq.j_ivb.or.ivform_.eq.j_ivb2.or.ivform_.eq.j_i   6075
		...
		elseif(idac.ne.0)then !if(iform_.eq.j_ivb.or.iform_.eq.j_ivb2)then
			...
			open(unit=nu,  err=91,file=j_filename(i1:lefi),&
				...
				iostat=ier)
		endif !if(ivform_.eq.j_ivb.or.ivform_.eq.j_ivb2)   6109
	endif !if(rw.eq.'r'.or.useit)   6062
...
logical function j_exist(ivfile,ext) !check if file exist ivfile can be character or other
	...
	if(j_otype(ivfile).ne.j_ipchar)then
		if(present(ext))then
			...
			if(j_err)return
			!		write(6,*)ifi,j_otype(ifi)
		endif !if(present(ext))   6165
	endif !if(j_otype(ivfile).ne.j_ipchar)   6164
	!	write(6,*)ifi,j_otype(ifi)
	call j_getchar(ifi,j_filename,le)
	i1=1
	if(j_filename(1:1).eq.'$')i1=2
...
subroutine j_clearoption(iob,io) !clear options  %%option
	...
	do i=1,j_nopt
		if(j_optionmoptio(2,i).eq.io)then
			...
			endif !if(nopc.gt.0)   6277
 
		endif !if(j_optionmoptio(2,i).eq.io)   6266
 
	end do !i=1,j_nopt   6265
...
logical function j_isoption(iob,io,mopt,default) ! -1 ==not  0== opt->,  otherwise iv for first argument %%option obsolete
	...
	if(mopt.lt.0.or.mopt.gt.j_noptions)then
		...
		j_err=.true.;return
	endif !if(mopt.lt.0.or.mopt.gt.j_noptions)   6327
...
subroutine j_putoptv(iob,li,j,name,iv,value)
	...
	endif !if(j_otype(ivf).eq.j_ipchar)   6454
...
subroutine j_igetoptv(igeto,name,iv,single,ivout) !get output option varaible e.g. stat(mean->
	...
	if(j_otype(igeto).eq.j_ipchar)then
		...
		ivout=igeto
	endif !if(j_otype(igeto).eq.j_ipchar)   6520
	return
...
subroutine j_getdataobject2(iob,io)  ! initilization for data-> option  %%data
	...
	if(nopt.eq.1)then
		...
		if(j_otype(j_divdata).ne.j_ipdata)then
			...
			do ii=1,j_named
				...
				endif !if(j_otype(ii).eq.j_ipdata)   6566
 
			enddo !ii=1,j_named   6565
			!	write(6,*)j_tempchar2(1:ial)
			return
		endif !if(j_otype(j_divdata).ne.j_ipdata)   6559
	...
	if(nopt.eq.1)then
		...
		if(j_otype(j_divtrans).ne.j_iptrans)then
			...
			j_err=.true. ;return
		endif !if(j_otype(j_divtrans).ne.j_iptrans)   6647
		...
		endif !if(noutv.gt.0)   6654
		j_distrans=.true.
 
	endif !if(nopt.eq.1)   6645
	if(j_disup)then
	...
	if(nopt.gt.0)then
		...
		if(ifirst.lt.1.or.ifirst.gt.j_dnobs)then
			...
			j_err=.true.;return
		endif !if(ifirst.lt.1.or.ifirst.gt.j_dnobs)   6677
		j_dfrom=ifirst
	else !if(nopt.gt.0)then
		j_dfrom=1
	endif !if(nopt.gt.0)   6675
	...
	if(nopt.gt.0)then
		...
		if(ifirst.lt.1.or.ifirst.gt.j_dnobs8)then
			...
			j_err=.true.;return
		endif !if(ifirst.lt.1.or.ifirst.gt.j_dnobs8)   6688
		j_duntil=ifirst
	else !if(nopt.gt.0)then
		j_duntil=j_dnobs8
	endif !if(nopt.gt.0)   6686
	...
	if(nopt.eq.1)then
		...
		if(j_otype(j_optarg0(1)).ne.j_iptrans)then
			...
			j_err=.true. ;return
		endif !if(j_otype(j_optarg0(1)).ne.j_iptrans)   6702
		call dotrans(j_optarg0(1),1)
		!	write(6,*)'{{',3
		if(j_err)return
	endif !if(nopt.eq.1)   6700
	...
	if(nopt.eq.1)then
		...
		if(j_otype(j_depilog).ne.j_iptrans)then
			...
			j_err=.true. ;return
		endif !if(j_otype(j_depilog).ne.j_iptrans)   6716
	endif !if(nopt.eq.1)   6714
	...
	if(iobs.eq.j_dfrom)then
		...
		if(j_disup)then
			...
			do while(nsub.lt.j_dfrom)
				...
				j_dobsup=j_dobsup+1
			enddo !while(nsub.lt.j_dfrom)   6739
			j_v(j_divobsup)=j_dobsup
			j_dnextobs=nsub-j_dfrom+1  !testattava
		endif !if(j_disup)   6735
	endif !if(iobs.eq.j_dfrom)   6732
	if(j_disup)then
		...
		endif !if(iobs.eq.j_dnextobs)   6750
 
 
	endif !if(j_disup)   6748
...
subroutine j_getdataobject(iob,io,ivdata,pointmat,pointkeep,pointmatup,pointkeepup,needsup)  ! initilization for data-> option  %%data
	...
	if(.not.present(ivdata))then
		...
		elseif(j_otype(j_ivdata).eq.j_iplist)then !if(nopt.eq.1)then
			if(j_o(j_ivdata)%i(1).ne.1)then
				write(6,*)'Data=list( ) can give only one data'
				j_err=.true.
			endif !if(j_o(j_ivdata)%i(1).ne.1)   6811
			j_divdata=j_o(j_ivdata)%i2(1)
			...
			j_err=.true.
			return
		endif !if(nopt.eq.1)   6807
	else
		j_divdata=ivdata
 
	endif !if(.not.present(ivdata))   6803
	...
	if(j_disup)then
		...
		endif !if(ivup.le.0)   6889
	endif !if(j_disup)   6887
	...
	if(nopt.eq.1)then
		...
		if(noutv.gt.0)then
			...
			iper=j_putlistobject(j_divvars,ivin=ivoul)
			!function j_putlistobject(ivlist,single,list0,list,ivin,ignored)
		endif !if(noutv.gt.0)   6937
		j_distrans=.true.
 
	endif !if(nopt.eq.1)   6923
	if(j_disup)then
		if(nopt.lt.1)j_divvars=j_deflist(j_divdata,'%vars',ivin=j_divkeep,nres=naddv)
	...
	if(nopt.gt.0)then
		...
		if(ifirst.lt.1.or.ifirst.gt.j_dnobs)then
			write(6,*)'from->  should be between 1 and ',j_dnobs,' it is ',ifirst
			j_err=.true.;return
		endif !if(ifirst.lt.1.or.ifirst.gt.j_dnobs)   6960
		j_dfrom=ifirst
	else !if(nopt.gt.0)then
		j_dfrom=1
	endif !if(nopt.gt.0)   6958
	call j_getoption(iob,io,j_muntil,-1,1,j_ipreal,.true.,nopt,j_optarg0)
	if(nopt.gt.0)then
		ifirst=j_v(j_optarg0(1))
		if(ifirst.lt.1.or.ifirst.gt.j_dnobs)then
			...
			j_err=.true.;return
		endif !if(ifirst.lt.1.or.ifirst.gt.j_dnobs)   6971
		j_duntil=ifirst
	else !if(nopt.gt.0)then
		j_duntil=j_dnobs
	endif !if(nopt.gt.0)   6969
	...
	if(nopt.eq.1)then
		...
		if(j_otype(j_optarg0(1)).ne.j_iptrans)then
			call j_printname('prolog-> does not refer to a transformation: ',j_optarg0(1),' ')
			j_err=.true. ;return
		endif !if(j_otype(j_optarg0(1)).ne.j_iptrans)   6985
		call dotrans(j_optarg0(1),1)
		!	write(6,*)'{{',3
		if(j_err)return
	endif !if(nopt.eq.1)   6983
	...
	if(nopt.eq.1)then
		...
		if(j_otype(j_depilog).ne.j_iptrans)then
			call j_printname('epilog-> does not refer to a transformation: ',j_depilog,' ')
			j_err=.true. ;return
		endif !if(j_otype(j_depilog).ne.j_iptrans)   6999
...
subroutine j_linkdata(ivup,ivsub)
	...
	elseif(j_oname2(1:j_loname2).eq.'%nobsw')then
		...
		j_err=.true.;return
	endif !if(j_oname2(1:j_loname2).eq.j_oname(1:j_loname)//'%nobsw')   7021
	...
	endif !if(nobssub.ne.nobstot)   7110
...
function j_datalevels(ivdata)
	...
	enddo !while (j_o(ivdatac)%i(3).ne.0)   7152
...
function j_bottomdata(ivdata)
	...
	enddo !while (j_o(j_bottomdata)%i(3).ne.0)   7165
...
function j_topdata(ivdata)
	...
	enddo !while (j_o(j_topdata)%i(5).ne.0)   7177
	!	write(6,*)'topdata ',j_topdata,j_o(j_topdata)%i
end function
	...
	if(iobs.eq.j_dfrom)then
		...
		if(j_disup)then
			...
			do while(nsub.lt.j_dfrom)
				...
				j_dibaup=j_dibaup+j_dnkeepup
				j_dobsup=j_dobsup+1
			enddo !while(nsub.lt.j_dfrom)   7361
			j_v(j_divobsup)=j_dobsup
			j_dnextobs=nsub-j_dfrom+1  !testattava
		endif !if(j_disup)   7357
		!	write(6,*)'iobsfro',iobs,j_dfrom,j_dnkeep,j_nkeepup,j_dobsup,j_dnextobs
	endif !if(iobs.eq.j_dfrom)   7354
	if(j_disup)then
		...
		if(iobs.eq.j_dnextobs)then
			...
			j_v( j_divobsw)=j_v( j_divobsw)+1
		endif !if(iobs.eq.j_dnextobs)   7373
 
 
	endif !if(j_disup)   7371
	! write(6,*)'<55',j_diba,j_dnkeep,j_o(j_divkeep)%i2(1:j_dnkeep)
...
function j_nobsdata(ivdata) !number of obs in data
	...
	if(ivdata.le.0.or.ivdata.gt.j_named)then
		...
		j_err=.true.;return
	endif !if(ivdata.le.0.or.ivdata.gt.j_named)   7511
	if(j_otype(ivdata).ne.j_ipdata)then
		call j_printname(' ',ivdata, ' is not data object')
		j_err=.true.
		return
	endif !if(j_otype(ivdata).ne.j_ipdata)   7515
...
17	continue
	...
	if(ial.le.le1)then
		do ilo=ial,le1
			...
			exit
		enddo !ilo=ial,le1   7576
		ilop1=ilo-1
	endif !if(ial.le.le1)   7575
	...
	if(ial.le.le2)then
		do ilo=ial,le2
			...
			exit
		enddo !ilo=ial,le2   7585
		ilop2=ilo-1
	endif !if(ial.le.le2)   7584
	if(ilop1.gt.le1)then
		write(6,*)'illegal ... '
		j_err=.true.
		return
	endif !if(ilop1.gt.le1)   7591
	if(name1(ial:ilop1).eq.name2(ial:ilop2))then
		ial0=ilop1+1
		goto 17
	endif !if(name1(ial:ilop1).eq.name2(ial:ilop2))   7596
	...
	if(ilop1.lt.le1.and.ilop2.lt.le2)then
		if(name1(ilop1+1:le1).ne.name2(ilop2+1:le2))then
			...
			j_err=.true.
			return
		endif !if(name1(ilop1+1:le1).ne.name2(ilop2+1:le2))   7602
	elseif(ilop1.lt.le1.or.ilop2.lt.le2)then
		write(6,*)'illegal ... '
		j_err=.true.
		return
 
	endif !if(ilop1.lt.le1.and.ilop2.lt.le2)   7601
...
79	continue
	...
	enddo !ili=ii1+istep,ii2-istep,istep   7640
	n=n+1
	call testdots(n)
	i2=j_object(name2)
...
99 if(le1.ne.le2)then
		...
		j_err=.true.
		return
	endif !99 if(le1.ne.le2)   7670
 
	if(ial.lt.le1)then
		if(name1(ial+1:le1).ne.name2(ial+1:le1))then
			write(6,*)'end ',name1(ial+1:le1),' differs from ',name2(ial+1:le1)
			j_err=.true. ;return
		endif !if(name1(ial+1:le1).ne.name2(ial+1:le1))   7677
	endif !if(ial.lt.le1)   7676
	if(.not.(name1(ial:ial).ge.'A'.and.name2(ial:ial).le.'Z'.or.&
		...
		j_err=.true.
		return
	endif !if(.not.(name1(ial:ial).ge.'A'.and.name2(ial:ial).le.'Z'.   7682
	istep=1
	...
	enddo !i=ichar(name1(ial:ial))+istep,ichar(name2(ial:ial))-istep,   7694
	n=n+1
	...
	subroutine testdots(nn)
		...
		endif !if(nn.gt.size(j_dotlist))   7718
...
subroutine j_tracecheck(iv)
	...
	if(ii.gt.0)then
		...
		endif !if(j_o(j_ivtracestatus)%d(ii).gt.0)   7771
	endif !if(ii.gt.0)   7770
...
77 if(inp(icur:icur).le.' ')then
		...
		icur=icur+1
		goto 77
	end if !77 if(inp(icur:icur).le.' ')   7808
	i=icur
	!20140618 (,  ,) virheet
...
17 i=j+1   !;goto 100
 ...
 !  write(6,*)'i,last',i,last
		...
		ipos1=j
	endif !if(inp(i:i).eq."'")   7843
	!write(6,*)'icur,ipos1,i,j',icur,ipos1,i,j
	if(inp(i:i).le.' ')then
		i=i+1
		goto 100
	endif !if(inp(i:i).le.' ')   7861
	...
	if(inp(i:i).lt.'0'.or.(inp(i:i).gt.'9'.and.inp(i:i).lt.'A').or. &
		...
		do j=1,noper
			...
			if(inp(i:i+le2-1).eq.oper(j)(1:le2))then
				...
				endif !if(i.ge.3.and.(oper(j)(1:le2).eq.'-'.or.oper(j)(1:le2).eq.   7877
				ipos=i
				ioper=j
				! to treat ==
				if(ioper.eq.20)then
					if(last.ge.i+1.and.inp(i+1:i+1).eq.'=')ioper=25
				endif !if(ioper.eq.20)   7889
				! if(ioper.eq.iops)write(6,*)'iops',i,i+le2-1,input
				return
			endif !if(inp(i:i+le2-1).eq.oper(j)(1:le2))   7875
		enddo !j=1,noper   7870
		if(i.gt.ipos1+1)then
			write(6,*)'**illegal special marks after:',inp(1:ipos1)
			j_err=.true.
			!write(6,*)'ipos1,i,icur',ipos1,i,icur
			return
		endif !if(i.gt.ipos1+1)   7896
	endif !if(inp(i:i).lt.'0'.or.(inp(i:i).gt.'9'.and.inp(i:i).lt.'A'   7867
...
logical function j_istrans(iv) ! is iv a transformation  !ei ehkä ole tar
	...
	if(iv.le.0.or.iv.gt.j_named)then
		...
		j_istrans=.true.
	endif !if(iv.le.0.or.iv.gt.j_named)   8045
	return
...
integer function j_outputlist(ivtrans) !outputlist of transformation set ivtrans
	...
	if(ivtrans.le.0.or.ivtrans.gt.j_named)then
		...
		j_err=.true.
		return
	endif !if(ivtrans.le.0.or.ivtrans.gt.j_named)   8059
	if(j_otype(ivtrans).ne.j_iptrans)then
		call j_printname('*Object ',ivtrans,' is not a transformation set')
		j_err=.true.
		return
	endif !if(j_otype(ivtrans).ne.j_iptrans)   8064
...
function j_lenlist(iob) ! length of list, -1 if not a list
	...
	endif !if(j_otype(iob).ne.j_iplist)   8104
	return
...
function j_inlistobject(i,ivlist) !is i in a list object ivlist
	...
	if(ivlist.le.0.or.ivlist.gt.j_named)then
		...
		if(j_otype(ivlist).ne.j_iplist.and.j_otype(ivlist).ne.j_ipilist)then
			...
			j_err=.true.;return
		endif !if(j_otype(ivlist).ne.j_iplist.and.j_otype(ivlist).ne.j_ip   8184
	endif !if(ivlist.le.0.or.ivlist.gt.j_named)   8178
	j_inlistobject=0
	...
	do j=1,j_o(ivlist)%i(1)
		if(i.eq.j_o(ivlist)%i2(j))then
			j_inlistobject=j
			return
		endif !if(i.eq.j_o(ivlist)%i2(j))   8194
	enddo !j=1,j_o(ivlist)%i(1)   8193
...
function j_putlist(i,list) ! put i into allocated list, bound checking %%list
	...
	if(list(0).ge.iubound_)then
		...
		! deallocate(ivec2)
	end if !if(list(0).ge.iubound_)   8243
	list(0)=list(0)+1
	list(list(0))=i
	j_putlist=list(0)
	return
...
subroutine j_putinput(iv,ivinl,ivoutl,ivarg) !put variable into inputlist if not in the outputlist
	...
	endif !if(ivarg.ne.0)   8413
	if(ivoutl.gt.0)then
		if(j_inlistobject(iv,ivoutl).gt.0)return
	endif !if(ivoutl.gt.0)   8416
	!20140627   JL
...
function j_ipc(iv)  !is iv character, YES=>1 NO=>0
	...
	if(j_otype(iv).eq.j_ipchar)then
		...
		j_ipc=0
	endif !if(j_otype(iv).eq.j_ipchar)   8712
	return
...
subroutine j_andlist(list1,list2,olist)
	...
	do i=1,list1(0)
		if(any(list1(i).eq.list2(1:list2(0)) ))then
			iout=iout+1
			olist(iout)=list1(i)
		endif !if(any(list1(i).eq.list2(1:list2(0)) ))   8727
	enddo !i=1,list1(0)   8726
	olist(0)=iout
	return
...
subroutine j_stopj()
	...
	do i_=j_nused,1,-1
		...
		if(act(1:lenact)=='READ') then
			...
			write(6,*)'write file ', j_tempchar2(1:len_trim(j_tempchar2)),' remains open'
		endif !if(act(1:lenact)=='READ')   8789
	enddo !i_=j_nused,1,-1   8783
	!		write(6,*)j_nused,'+',j_nunits(j_nused+1:6)
	do i_=1,nclosed
		call j_closeunit(closed(i_))
	enddo !i_=1,nclosed   8799
	! j_err=.false.
	...
	do i=1,j_nused
		...
		if(j_nunits(i).eq.nu)then
			...
			if(nunitvar_.ne.0)then
				...
				j_unitchar(nu)=0
			endif !if(nunitvar_.ne.0)   8824
			j_nunits(i)=j_nunits(j_nused)
			j_nunits(j_nused)=nu
			j_nused=j_nused-1
			return
		end if !if(j_nunits(i).eq.nu)   8822
	end do !i=1,j_nused   8820
...
function j_defchar(iv,name) !define a character constant
	...
	if(iv.gt.0)then
		...
		if(name(1:1).eq."'")then
			...
			if(j_defchar.gt.0.or.j_err)return
			call j_getobjectnam("'"//name//"'",j_ipchar,j_defchar)
		endif !if(name(1:1).eq."'")   8882
 
 
 
	endif !if(iv.gt.0)   8855
...
logical function j_ischarconst(iv)   ! %%char
	...
	endif !if(j_otype(iv).eq.j_ipchar)   8945
	return
...
100	continue
	...
	elseif(r.eq.j_0)then !if(r.eq.j_1)then
		j_isconst=j_ivzero
		return
	endif !if(r.eq.j_1)   8971
	!write(6,*)'v(nv+1:nv+nconstantv)',v(nv+1:nv+nconstantv)
	do i=j_nv+1,j_nv+j_nconstantv
		if(j_v(i).eq.r)then
			j_isconst=i
			return
		endif !if(j_v(i).eq.r)   8981
	enddo !i=j_nv+1,j_nv+j_nconstantv   8980
...
100	continue
	...
	elseif(r.eq.j_0)then !if(r.eq.j_1)then
		j_getconst=j_ivzero
		return
	endif !if(r.eq.j_1)   9022
	!write(6,*)'v(nv+1:nv+nconstantv)',v(nv+1:nv+nconstantv)
	do i=j_nv+1,j_nv+j_nconstantv
		if(j_v(i).eq.r)then
			j_getconst=i
			return
		endif !if(j_v(i).eq.r)   9032
	enddo !i=j_nv+1,j_nv+j_nconstantv   9031
...
function j_getkeep(ivdata,point,len)
	...
	if(j_otype(ivdata).ne.j_ipdata)then
		...
		j_err=.true.; return
	endif !if(j_otype(ivdata).ne.j_ipdata)   9058
	...
	if(j_otype(j_getkeep).ne.j_iplist)then
		...
		j_err=.true.; return
	endif !if(j_otype(j_getkeep).ne.j_iplist)   9064
...
function j_gettablecoef(ivtable,point)
	...
	if(j_otype(ivtable).ne.j_iptable)then
		...
		j_err=.true.;return
	endif !if(j_otype(ivtable).ne.j_iptable)   9077
	...
	if(j_otype(ivmatrix).ne.j_ipmatrix)then
		...
		j_err=.true.;return
	endif !if(j_otype(ivmatrix).ne.j_ipmatrix)   9086
	...
	if(j_o(ivmatrix)%i(1).ne.j_o(ivtable)%i(1).or.j_o(ivmatrix)%i(2).ne.j_o(ivtable)%i(2))then
		...
		j_err=.true.;return
	endif !if(j_o(ivmatrix)%i(1).ne.j_o(ivtable)%i(1).or.j_o(ivmatrix   9092
...
subroutine j_checkobject(ivobject,itype,named)
	...
	if(ivobject.le.0.or.ivobject.gt.j_mxv)then
		...
		j_err=.true.
	endif !if(ivobject.le.0.or.ivobject.gt.j_mxv)   9109
	if(itype.le.0.or.itype.gt.j_notypes)then
		write(6,*)'*j_checkobject, illegal itype:',itype
		j_err=.true.
	endif !if(itype.le.0.or.itype.gt.j_notypes)   9113
	if(j_err)return
	nam=.false.
	nonnam=.false.
	...
	elseif(nonnam.and.ivobject.le.j_named)then
		...
		j_err=.true.
	endif !if(nam.and.ivobject.gt.j_named)   9125
		...
		if(j_otype(j_divdata).ne.j_ipdata)then
			...
			do ii=1,j_named
				if(j_otype(ii).eq.itype)then
					...
					!			ial=len_trim(j_tempchar2)+2
				endif !if(j_otype(ii).eq.itype)   9145
 
			enddo !ii=1,j_named   9144
			write(6,*)'there are ',nn, ' objects of the correct type'
			!	write(6,*)j_tempchar2(1:ial)
			return
		endif !if(j_otype(j_divdata).ne.j_ipdata)   9137
		j_err=.true.
	endif !if(j_otype(ivobject).ne. itype)   9133
...
function j_getsubdata(ivdata,needed)
	...
	if(present(needed).and.j_getsubdata.le.0)then
		...
		j_err=.true.
	endif !if(present(needed).and.j_getsubdata.le.0)   9184
...
logical function j_isupdata(iv,ivkeep,keep,nkeep,ivobs,ivnobsw,nobsup,point,nobssub)
	...
	if(j_o(ivkeep0)%i2(1).ne.ivnobsw0)then
		...
		j_err=.true.;return
	endif !if(j_o(ivkeep0)%i2(1).ne.ivnobsw0)   9223
	...
	if(present(nobssub))then
		...
		do i=1,nobsup0
			...
			ibas=ibas+nkeep0
		enddo !i=1,nobsup0   9237
 
	endif !if(present(nobssub))   9234
...
function j_getupdata(ivdata,needed)
	...
	if(present(needed).and.j_getupdata.le.0)then
		...
		j_err=.true.
	endif !if(present(needed).and.j_getupdata.le.0)   9255
...
function j_num2iv(r) !puts const into list of constants, if not there, and output is the indec in j_v
	...
	elseif(r.eq.j_0)then !if(r.eq.j_1)then
		j_num2iv=j_ivzero
		return
	endif !if(r.eq.j_1)   9363
	!write(6,*)'v(nv+1:nv+nconstantv)',v(nv+1:nv+nconstantv)
	do i=j_nv+1,j_nv+j_nconstantv
		if(j_v(i).eq.r)then
			j_num2iv=i
			return
		endif !if(j_v(i).eq.r)   9373
	enddo !i=j_nv+1,j_nv+j_nconstantv   9372
	...
	endif !if(j_nconstantv.ge.j_mxconstantv)   9378
	j_nconstantv=j_nconstantv+1
	j_nvtot=j_nvtot+1
...
1 read(nu,'(a)',end=90,err=99)inp
	...
	if(inp(1:max(1,le)).ne.tag(1:let))then
		...
		goto 1
	endif !if(inp(1:max(1,le)).ne.tag(1:let))   9425
	return
		...
		do i=1,nvector2
			...
			if(le1.eq.le2) then
				...
				if(vector1(it_)(1:le1).eq.vector2(i)(1:le2))then
					...
					j_err = .true.
				endif !if(vector1(it_)(1:le1).eq.vector2(i)(1:le2))   9479
			endif !if(le1.eq.le2)   9477
		enddo !i=1,nvector2   9475
	enddo !it_ = 1, nvector1   9473
	return
...
700   call j_getobs(ial)
	if(j_rejected) then
		...
		if(ial.eq.j_dnobs8)then
			...
			j_err=.true.
			return
		endif !if(ial.eq.j_dnobs8)   9543
		goto 700
	endif !if(j_rejected)   9541
 
	value9(1)=j_v(ix);nval=1;ial=ial+1
	if(iz.gt.0)then
		value2(1)=j_v(iz);nval2=1
	endif !if(iz.gt.0)   9552
...
900     continue
		endif !if(iz.gt.0)   9568
	enddo !i=ial,nobs   9556
...
subroutine j_putmatrix_s(ivmat,irow,icol,val)
	...
	endif !if(j_otype(ivmat).eq.j_iplist)   9608
	...
	endif !if(j_otype(ivmat).eq.j_ipreal.and.irow.eq.1.and.icol.eq.1)   9613
	return
	...
	if(j_otype(irg).eq.j_ipreal)then
		...
		if(present(minv))then
			if(j_v(irg).lt.minv)then
				...
				j_err=.true.
				return
			endif !if(j_v(irg).lt.minv)   9657
		endif !if(present(minv))   9656
		if(present(maxv))then
			if(val.gt.maxv)then
				write(6,*)'*too larg argument ',j_v(irg)
				j_err=.true.
				return
			endif !if(val.gt.maxv)   9664
		endif !if(present(maxv))   9663
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		if(present(minv))then
			...
			if(val.lt.minv)then
				...
				j_err=.true.
				return
			endif !if(val.lt.minv)   9681
		endif !if(present(minv))   9679
		if(present(maxv))then
			...
			if(val.gt.maxv)then
				...
				j_err=.true.
				return
			endif !if(val.gt.maxv)   9694
		endif !if(present(maxv))   9692
		...
		if(present(reci))then
			...
			endif !if(present(coef))   9721
 
		endif !if(present(reci))   9714
		...
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   9654
...
subroutine j_funci(iob,io,ifunc)   !integer function
	...
	endif !if(j_otype(irg).eq.j_ipreal)   9746
	return
...
subroutine j_func2(iob,io,func)   !integer function
	...
	endif !if(j_otype(irg).eq.j_ipreal)   9781
	return
...
subroutine j_putmatrix(ivmat,irow,icol,val)
	...
	endif !if(j_otype(ivmat).eq.j_ipreal.and.irow.eq.1.and.icol.eq.1)   9898
	return
	! 90 write(6,*)'*j* read/write error in j_putmatrix'
	! j_err=.true.
...
subroutine j_putmatrix8(ivmat,irow,icol,val)
	...
	endif !if(j_otype(ivmat).eq.j_ipreal.and.irow.eq.j_18.and.icol.eq   9924
	return
	! 90 write(6,*)'*j* read/write error in j_putmatrix'
	! j_err=.true.
...
double precision function j_getmatel(ivmat,irow,icol) !function
	...
	if(j_otype(ivmat).eq.j_ipmatrix)then !j_o(ivmat)%i(13).ne.0)then
		if(irow.le.0.or.irow.gt.j_o(ivmat)%i(1).or.icol.le.0.or.icol.gt.j_o(ivmat)%i(2))then
			...
			j_err=.true.
			return
		endif !if(irow.le.0.or.irow.gt.j_o(ivmat)%i(1).or.icol.le.0.or.ic   9957
	...
	endif !if(j_otype(ivmat).eq.j_ipmatrix)   9956
...
subroutine j_asschar2(ivin,ivout) !! assign character constant to charvariable ?
	...
	endif !if(j_otype(ivout).ne.j_ipchar)   9993
...
subroutine j_sit() !sit -prompt
	...
	if(j_inp(1:j_linp).eq.'end')then
		...
		write(6,*)'stop'
		return
	endif !if(j_inp(1:j_linp).eq.'end')  10054
...
subroutine j_getinput0(prompt,inprint,nul0t,single)  ! gets next input line
	...
	endif !if(present(inprint))  10110
...
subroutine j_getinput(prompt,inprint,nul0t,single)  ! gets next input line
	...
	if(present(inprint))then
		...
		inprint0=j_v(j_ivprintinput)
	endif !if(present(inprint))  10204
...
3 	continue  !we come here from below if we are getting continuation
	...
	endif !if(j_inpr(1:1).eq.'!'.or.j_inpr(1:1).eq.'*'.and..not.j_byp  10233
 ...
 4	continue
	...
	if(j_inpr(1:1).ne.';')then
		...
		endif !if(iii.gt.0)  10246
	endif !if(j_inpr(1:1).ne.';')  10243
	...
	if(j_ispause.and.j_inpr(1:j_linpr).eq.'e')then
		...
		!	j_err=.true.
		goto 1
	endif !if(j_ispause.and.j_inpr(1:j_linpr).eq.'e')  10262
	...
	if(j_inpr(1:6).eq.';pause')then
		...
		endif !if(j_linpr.eq.6)  10269
		j_ispause=.true.
		goto 1
	endif !if(j_inpr(1:6).eq.';pause')  10268
	...
	if(j_inpr(1:6).eq.';where')then
		...
		enddo !i=j_ninc,2,-1  10281
		goto 1
	endif !if(j_inpr(1:6).eq.';where')  10278
	!
	...
	endif !if(.not.j_bypa(j_niifs) )  10305
	...
	if(.not.j_bypa(j_niifs))then
		...
		if(done)goto 1
	endif !if(.not.j_bypa(j_niifs))  10323
	...
	if(j_inpr(1:j_linpr).eq.'cls'.and..not.(j_inpara.or.j_inpara2))then
		call system('cls')
		goto 1
	endif !if(j_inpr(1:j_linpr).eq.'cls'.and..not.(j_inpara.or.j_inpa  10336
	...
	if(j_inpr(1:1).eq.';')then  !input programming
		...
		endif !if(j_inpr(1:j_linpr).eq.';enddo'.or.j_inpr(1:j_linpr).eq.'  10347
		if(inprint0.gt.0)then    !printline(inprint))then
			!		if(doublesp)write(6,*)' '
			!		write(6,*)'% '//j_inpr(1:j_linpr)
 
			write(6,*)inpup(1:lip)//j_inpr(1:j_linpr)
		endif !if(inprint0.gt.0)  10358
		!if(j_inpr(1:4).eq.';if(')write(6,*)'<3399 ',j_inpr(1:j_linpr)
		...
		elseif(j_inpr(1:4).eq.';do(')then !if(j_inp(1:6).eq.';incl('.or.j_inp(1:6).eq.';goto(')then
			...
			!	write(6,*)'cbbc'
			goto 3
		endif !if(j_inpr(1:6).eq.';incl(')  10368
		...
		if(.not.inpuif.and.j_inpr(1:4 ).ne.';if('.and.j_inpr(1:7).ne.';elseif')then
			...
			j_err=.true. ;return
		endif !if(.not.inpuif.and.j_inpr(1:4 ).ne.';if('.and.j_inpr(1:7).  10407
		if(j_inpr(1:7).eq.';elseif')then !if(j_inp(1:6).eq.';incl('.or.j_inp(1:6).eq.';goto(')then
			!		write(6,*)'elseif'
			call ielseif()
			if(j_err)return
			goto 3
		endif !if(j_inpr(1:7).eq.';elseif')  10411
	endif !if(j_inpr(1:1).eq.';')  10341
	...
	if(j_bypa(j_niifs))then
		...
		endif !if(j_inpr(1:4).eq.';if('.and.j_inpr(max(j_linpr-4,1):j_lin  10422
		goto 1
	endif !if(j_bypa(j_niifs))  10421
	if(j_linpr.ge.2.and.j_linp.gt.2)then
		if(j_inpr(1:2).eq.'<<')then !finish line
			if(j_linpr.eq.2.and.j_inp(j_linp-1:j_linp).eq.'>>')then
				j_linp=j_linp-2
				goto 678
			endif !if(j_linpr.eq.2.and.j_inp(j_linp-1:j_linp).eq.'>>')  10434
			if(j_linpr.eq.2)then
				j_linp=j_linp-1  !remove last
				goto 678
			endif !if(j_linpr.eq.2)  10438
			if(j_inp(j_linp-1:j_linp).eq.'>>')then
				j_linp=j_linp-2
			else
				j_linp=j_linp-1
			endif !if(j_inp(j_linp-1:j_linp).eq.'>>')  10442
			j_inpr(1:j_linpr-2)=j_inpr(3:j_linpr)
			j_linpr=j_linpr-2
		endif !if(j_inpr(1:2).eq.'<<')  10433
	endif !if(j_linpr.ge.2.and.j_linp.gt.2)  10432
...
678	if(j_inp(1:3).eq.'if('.and.(index(j_inp(1:j_linp),';goto').gt.0.or. &
	...
	elseif(j_inp(1:4 ).eq.';if(')then !if(j_inp(1:6).eq.';incl('.or.j_inp(1:6).eq.';goto(')then
		...
		if(j_inp(i+1:j_linp).eq.';then')then
			...
			endif !if(j_yes)  10496
 
		endif !if(j_inp(i+1:j_linp).eq.';then')  10488
 
		!		endif !if(j_inpr(1:7).eq.';elseif')   8866
	endif !678	if(j_inp(1:3).eq.'if('.and.(index(j_inp(1:j_linp),';go  10470
	...
	if(j_ispause)then
		if(j_inp(1:j_linp).eq.'e'.or.j_inp(1:j_linp).eq.'/')then
			j_ispause=.false.
			goto 1
		endif !if(j_inp(1:j_linp).eq.'e'.or.j_inp(1:j_linp).eq.'/')  10522
		call j_command(j_inp(1:j_linp))
		j_err=.false.;j_errprint=.false.
		goto 1
 
	endif !if(j_ispause)  10521
	...
	endif !if(j_ninc.gt.1.and.inprint0.gt.0.and.prompt.ne.' ')  10532
	if(j_inp(1:j_linp).eq.'end')then
		j_stop=.true.;return
 
	endif !if(j_inp(1:j_linp).eq.'end')  10539
	!
...
500			read(5,100)j_inpr
			...
			if(j_inpr(1:j_linpr).eq.'end')then
				...
				!		write(6,*)'dhhd'
				return
			endif !if(j_inpr(1:j_linpr).eq.'end')  10637
			if(j_err)return
			...
			if(iline.gt.j_o(iiv)%i(5))then
				...
				call goto90()
			endif !if(iline.gt.j_o(iiv)%i(5))  10656
		...
		endif !if(jninc.eq.1)  10609
		!**********************************
	end do !while(j_linpr.le.0)  10604
...
subroutine printstar(done)
	...
	endif !if(j_inpr(1:5).eq.';now(')  10696
	...
	elseif(j_inpr(j_linpr-1:j_linpr).eq.');')then
		...
		j_err=.true. ;return
	endif !if(j_inpr(j_linpr:j_linpr).eq.')')  10717
	...
	if(j_inpr(ist-1:ist-1).eq.'(')then
		...
		fromstart=.true.
		ifi1=ie+6
	endif !if(j_inpr(ist-1:ist-1).eq.'(')  10744
	toend=.true.
	if(j_inpr(ila:ila).eq.'?')then
		ila=ila-1
		toend=.false.
	endif !if(j_inpr(ila:ila).eq.'?')  10752
	ila1=ila
	...
	endif !if(ist.gt.ifi1.and.ist.lt.ila)  10759
	lee2=ila2-ifi2+1
	lee=ila1-ifi1+1
	...
	if(listin)nobj=j_o(ivlistin)%i(1)
		...
		do iw=1,nword
			...
			nf=nf+1
		enddo !iw=1,nword  10839
 
	enddo ivloop !oop:		do iv0=1,nobj  10814
	...
	nf=0
		...
		if(fromstart)then
			if(j_inpr(ificur:ilacur).ne.j_oname(1:lee))cycle
		endif !if(fromstart)  10874
		if(toend.and.lee2.gt.j_loname)cycle
		if(toend)then
			if(j_inpr(ifi2:ila2).ne.j_oname(j_loname-lee2+1:j_loname))cycle
		endif !if(toend)  10878
		if(fromstart.and.toend)then
			...
			j_o(ivlist)%i2(nf)=iv
			cycle ivloop2
		endif !if(fromstart.and.toend)  10881
		do iw=1,nword
		...
		enddo !iw=1,nword  10888
	enddo ivloop2 !oop2:		do iv0=1,nobj  10864
	!	else  !list
...
logical function j_startwrite(narg,arg,isbin,isbin8,nu,lenwrite)
	use jmod, only: j_err
	use jmod, only: j_ivb
	use jmod, only: j_ivb2
	use jmod, only: j_ivdollar
	...
	if(narg.le.2)then
		write(6,*)'write need >2 arguments'
		j_err=.true.;return
		return
 
	endif !if(narg.le.2)  10916
	ivfile=arg(1)
	ivform=arg(2)
	...
	if(.not.(ivformat.eq.j_ivdollar.or.isbin.or.isbin8))then
		...
		j_err=.true.
		return
	endif !if(.not.(ivformat.eq.j_ivdollar.or.isbin.or.isbin8))  10926
	if(ivfile.eq.j_ivdollar)then
		...
		if(nu.le.0)then
			...
			endif !if(j_unitchar(nu).lt.0)  10948
		end if !if(nu.le.0)  10936
	endif !if(ivfile.eq.j_ivdollar)  10931
	if(j_err)return
	! do i=3,narg
	...
	do i=3,narg
		if(j_otype(arg(i)).eq.j_ipmatrix)then
			lenwrite=lenwrite+j_o(arg(i))%i(3)
		...
		endif !if(j_otype(arg(i)).eq.j_ipmatrix)  10963
 
	enddo !i=3,narg  10962
...
subroutine j_write(narg,arg,nu,bin,bin8,lenwrite)
	...
	if(lenwrite.eq.narg-2)then
		if(bin)then
			write(nu,err=990)real(j_v(arg(3:narg)))
		elseif(bin8)then
			write(nu,err=990)j_v(arg(3:narg))
		else
	...
	elseif(lenwrite.le.j_lenwrite)then
		...
		do i=3,narg
			if(j_otype(arg(i)).eq.j_ipmatrix)then
				...
				lenwrite0=lenwrite+le
 
			else
				lenwrite0=lenwrite0+1
				j_writetemp(lenwrite0)=j_v(arg(i))
			endif !if(j_otype(arg(i)).eq.j_ipmatrix)  10998
		enddo !i=3,narg  10997
		...
		j_err=.true.
	endif !if(lenwrite.eq.narg-2)  10985
	return
	...
	if(j_otype(iii).eq.j_ipchar)then
		if(j_o(iii)%i(3).ne.iii)then   !character variable
			...
			do while (ihi.le.j_linpr)
				...
				iaa=ihi+1
				ihi=j_nextlim(j_inpr,iaa,j_linpr,'~')
			end do !while (ihi.le.j_linpr)  11079
			! ial=1
			!** key shortcut
			!			edited=.true.
...
subroutine liste(edited)
	...
	do while(ilie.lt.j_linpr)
		...
		if(j_inpr(lef:lef).ne.'(')then
			iald=lef+1
			goto 8800
		endif !if(j_inpr(lef:lef).ne.'(')  11127
		...
		if(.not.islist.and.j_otype(i).ne.j_iptext)then
			...
			goto 900
		endif !if(.not.islist.and.j_otype(i).ne.j_iptext)  11145
		...
		if(islist)then
			if(inde.le.0.or.inde.gt.j_o(i)%i(1))then
				...
				goto 900
			endif !if(inde.le.0.or.inde.gt.j_o(i)%i(1))  11167
				...
				do j=1,ilie
					if(j_inpr(j:j).eq."'")then
						if(nh.eq.1)then
							...
							nh=1
						endif !if(nh.eq.1)  11182
					endif !if(j_inpr(j:j).eq."'")  11181
				enddo !j=1,ilie  11180
			endif !if(j_oname(1:1).eq."'")  11178
			if(nh.eq.0)then
...
8800		ilie=j_nextlim(j_inpr,iald,j_linpr,'@',inhipsu=.true.)
	...
	end do !while(ilie.lt.j_linpr)  11121
	return
...
subroutine expandlist(inp,linp,edited)  !@
	...
	if(itype.eq.0.or.j_otype(iv).ne.j_iplist)then
		...
		j_err=.true.
		return
	endif !if(itype.eq.0.or.j_otype(iv).ne.j_iplist)  11378
	le2=0
	...
	if(ido.lt.linp)then
		...
		if(iva.gt.0)then
			if(j_otype(iva).eq.j_ipchar)then
				...
				iald=ido+les-1
				goto 88
			endif !if(j_otype(iva).eq.j_ipchar)  11465
		endif !if(iva.gt.0)  11464
...
subroutine ienddo()
	...
	endif !if(j_niifs.ne.j_niifsindo(j_ndo))  11554
	if(.not.j_bypa(j_niifs))then
		...
		endif !if(j_iido(1,j_ndo).gt.j_iido(2,j_ndo).and.j_iido(3,j_ndo).  11570
	else
		j_ndo=j_ndo-1
	endif !if(.not.j_bypa(j_niifs))  11565
end subroutine
...
subroutine ielseif()
	...
	if(j_inpr(ir+1:j_linpr).eq.';then')then
		...
		j_err=.true. ;return
	endif !if(j_inpr(ir+1:j_linpr).eq.';then')  11654
...
subroutine getreturn(done,doublesp,inpup)
	...
	if(j_linpr.gt.12)then
		if(j_inpr(1:4).eq.";if(".and.(j_inpr(j_linpr-7:j_linpr)==');return'.or. &
			...
			if(j_inpr(j_linpr-5:j_linpr-5).eq.'R')then
				call goto90()
				return
			endif !if(j_inpr(j_linpr-5:j_linpr-5).eq.'R')  11740
 
		endif !if(j_inpr(1:4).eq.";if(".and.(j_inpr(j_linpr-7:j_linpr)=='  11730
	endif !if(j_linpr.gt.12)  11729
	...
	if(j_inpr(1:j_linpr).eq.';return'.or.done.or.j_inpr(1:j_linpr).eq.';Return'.or.ifret)then
		...
		if(j_ninc2(j_ninc).gt.0)then
			...
			return
		endif !if(j_ninc2(j_ninc).gt.0)  11774
 
		call goto90()
		!		done=.true.
	endif !if(j_inpr(1:j_linpr).eq.';return'.or.done.or.j_inpr(1:j_li  11748
...
subroutine inputinterpret(inpr,linpr,inp,linp,contline) ! interpretin utf
		...
		if(nu.ne.5.and.nu.gt.0)then
			do jj=1,j_nutf8
				if(inpr(i0:i0).eq.j_utf8(jj))then
					...
					!	any
					exit
				endif !if(inpr(i0:i0).eq.j_utf8(jj))  11809
			enddo !jj=1,j_nutf8  11808
		endif !if(nu.ne.5.and.nu.gt.0)  11807
 
		inp(linp:linp)=inpr(i0:i0)
 
	enddo !i0=1,linpr  11796
	...
	endif !if(nu.ne.5.and.nu.gt.0)  11824
...
subroutine debugi(tag)
	...
	end do !j=1,j_o(j_ivinput2)%i(0)  11879
...
100	le=len(line)
	...
	if(ifrom.gt.0)then
		...
		if(line(ifrom+6:ifrom+6).eq."'")then
			...
			if(ifrom2.lt.ifrom1)goto 99
		endif !if(line(ifrom+6:ifrom+6).eq."'")  12021
	endif !if(ifrom.gt.0)  12019
	...
	if(index(line,'form->').gt.0)then
		...
		j_err=.true.
		return
	endif !if(index(line,'form->').gt.0)  12035
	...
	if(ipil.gt.0.or..not.ifrom.gt.0)then  !file name given
		...
		if(ifi.gt.0)then
			...
			if(j_otype(ifi).ne.j_ipchar)then
				...
				j_err=.true.;return
			endif !if(j_otype(ifi).ne.j_ipchar)  12065
		...
		endif !if(j_o(ifi)%txt( j_o(ifi)%i(5))(1:7).ne.';return')  12116
		...
		if(j_inciv(j_ninc+1).eq.0)then
			...
			j_err=.true.;return
		endif !if(j_inciv(j_ninc+1).eq.0)  12122
 
	endif !if(ipil.gt.0.or..not.ifrom.gt.0)  12055
...
!	endif
	...
	if(iiv.eq.ifi)then
		...
		do ic=1,j_ninc2(j_ninc)
			if(j_increturn(j_ninc,ic).eq.j_o(iiv)%i(6))then
				...
				if(j_dapu.eq.j_0)then
					...
					exit
				endif !if(j_dapu.eq.j_0)  12152
 
			endif !if(j_increturn(j_ninc,ic).eq.j_o(iiv)%i(6))  12149
 
		enddo !ic=1,j_ninc2(j_ninc)  12148
		if(j_ninc2(j_ninc).ge.j_mxinc2)then
			...
			j_err=.true. ;return
		endif !if(j_ninc2(j_ninc).ge.j_mxinc2)  12166
		j_ninc2(j_ninc)=j_ninc2(j_ninc)+1
		...
		if(j_ninc.ge.j_mxinc)then
			...
			j_err=.true.
			return
		endif !if(j_ninc.ge.j_mxinc)  12182
		j_ninc=j_ninc+1
	...
	subroutine findad()
		...
		do ili=1,j_o(ifi)%i(5)
			...
			if(line(ifrom1:ifrom2)//':'.eq.j_o(ifi)%txt(ili)(2:ifrom2-ifrom1+3))then
				...
				return
			endif !if(line(ifrom1:ifrom2)//':'.eq.j_o(ifi)%txt(ili)(2:ifrom2-  12229
			cycle
		enddo !ili=1,j_o(ifi)%i(5)  12221
...
subroutine j_inpugoto(doublesp,inpup)  !;goto
	...
	if(j_inpr(7:7).eq."'".or.j_inpr(7:7).eq."~")then
		...
		ifrom1=7
		ifrom2=j_linpr-1
	endif !if(j_inpr(7:7).eq."'".or.j_inpr(7:7).eq."~")  12265
	ihere=0
	...
	liloop:	do ili=1,j_o(iiv)%i(5)
		...
		if(j_inpr(ifrom1:ifrom2)//':'.eq.j_txtp(2:ifrom2-ifrom1+3))then
			...
			return
		endif !if(j_inpr(ifrom1:ifrom2)//':'.eq.j_txtp(2:ifrom2-ifrom1+3)  12324
 
	enddo liloop !oop:	do ili=1,j_o(iiv)%i(5)  12314
...
subroutine j_bypassinc(nu)
	...
	enddo !while(j_o(nu)%txt(il)(1:1).ne.'/')  12345
	!	write(6,*)'bypass>/'
99	j_o( ivinc)%i(6)=il
!	write(6,*)'last',il
...
double precision function j_val(text) !subroutine compute the numeric value of a text string, used in getinput
	...
	if(le.le.0)then
		...
		! !	write(6,*)j_1
		! return
	endif !if(le.le.0)  12396
	iv=j_isconst(text)
	!	write(6,*)'iv ',iv
	if(iv.gt.0)then
		j_val=j_v(iv)
		return
	endif !if(iv.gt.0)  12407
	iv=j_object(text)
	!write(6,*)'iv2 ',iv
	!	i2=min(2,le)
	if(iv.gt.0)then
		if(j_otype(iv).eq.j_ipreal)then
			...
			j_err=.true.
		endif !if(j_otype(iv).eq.j_ipreal)  12415
		return
...
integer function j_tex2iv(text,isplus) !subroutine compute the numeric value of a text string, used in getinput
	...
	if(text(1:1).eq.'-')then
		...
		if(i2.eq.le)then
			...
			j_err=.true.
			return
		endif !if(i2.eq.le)  12462
	elseif(text(1:1).eq.'+')then
		i1=2
		isplus=1
	else
		isplus=1
	endif !if(text(1:1).eq.'-')  12459
	if(text(i1:i1).eq."'")then
		j_tex2iv=j_defchar(0,text(i1:le))
		return
	endif !if(text(i1:i1).eq."'")  12473
...
double precision function j_iv2val(iv)
	...
	elseif(j_otype(iv).eq.j_ipchar)then
		...
		if(le.le.0)then
			...
			j_err=.true.
		endif !if(le.le.0)  12507
		j_iv2val=j_val(j_tempchar3(1:le))
	else
		j_iv2val=j_inf
	endif !if(j_otype(iv).eq.j_ipreal)  12503
	return
...
subroutine j_getdos(inp,linp,iargs,nargs)!  ;do !enddo
	...
	if(il.gt.linp.or.inp(1:4).ne.';do(')then
		...
		j_err=.true.
		return
	endif !if(il.gt.linp.or.inp(1:4).ne.';do(')  12576
	!	ial=il+1
	...
	if(j_ndo.ge.j_mxndo)then
		...
		j_ndo=0
		return
	endif !if(j_ndo.ge.j_mxndo)  12610
		...
		do ili=j_o(ifi)%i(6)+1,j_o(ifi)%i(5)
			...
			elseif(j_o(ifi)%txt(ili)(1:6).eq.';enddo'.or.j_o(ifi)%txt(ili)(1:5).eq.';endo')then !if(j_o(ifi)%txt(ili)(1:4).eq.';do(')then
				if(nde.eq.0)then
					...
					exit
				endif !if(nde.eq.0)  12649
				nde=nde-1
			endif !if(j_o(ifi)%txt(ili)(1:4).eq.';do(')  12646
		enddo !ili=j_o(ifi)%i(6)+1,j_o(ifi)%i(5)  12644
		if(nde.gt.0)then
			write(6,*)';do starting at line ',j_o(ifi)%i(6)+1,' never ends'
			j_err=.true.;return
		endif !if(nde.gt.0)  12656
		j_o(ifi)%i(6)=ili
 
		return
	endif !if(nstep.le.0)  12637
	do jdo=1,j_ndo
		...
		if(j_iido(4,jdo).eq.iargs(1))then
			...
			j_err=.true.
		endif !if(j_iido(4,jdo).eq.iargs(1))  12666
	enddo !jdo=1,j_ndo  12664
	if(j_err)return
	j_ndo=j_ndo+1
...
subroutine j_getsum(inp,linp,plus)   ! ;sum  ;dif
	...
	if(inp(last+1:last+1).ne.'(')then
		...
		j_err=.true.
		return
	endif !if(inp(last+1:last+1).ne.'(')  12890
	...
	if(j_isletter(inp(ial0+1:ial0+1)))then
		...
		j_err=.true.
		return
	endif !if(j_isletter(inp(ial0+1:ial0+1)))  12897
	...
	do j=2,nargs
		...
		if(j_isletter(inp(li(j-1)+1:li(j)-1)))then
			...
			if(iv2.le.0)then
				...
				j_err=.true.
				return
			endif !if(iv2.le.0)  12921
 
		endif !if(j_isletter(inp(li(j-1)+1:li(j)-1)))  12910
		iargs(j)=j_v(iv2)
	enddo !j=2,nargs  12908
	istep=1
	...
	if(istep.eq.0.or. (istep.gt.0.and.iargs(3).lt.iargs(2)).or.&
		...
		j_err=.true.
		return
	endif !if(istep.eq.0.or. (istep.gt.0.and.iargs(3).lt.iargs(2)).  12932
	...
	do i=iargs(2),iargs(3),istep
		...
		if(i.lt.iargs(3))then
			...
			lenf=lenf+1
		endif !if(i.lt.iargs(3))  12956
		j_tempchar2(lu+1:lu+lenf)=j_tempchar(1:lenf)
		lu=lu+lenf
	enddo !i=iargs(2),iargs(3),istep  12944
...
function j_option_name(iopt,le)
	...
	elseif(iopt.le.j_noptions)then !if(iopt.gt.0.and.iopt.le.j_noptions_)then
		...
		j_err=.true.
	endif !if(iopt.gt.0.and.iopt.le.j_noptions_)  12983
	return
...
function j_function_name(ifunc,le)
	...
	elseif(ifunc.le.j_nfunctions)then !if(ifunc.gt.0.and.ifunc.le.j_nfunctions_)then
		...
		j_err=.true.
	endif !if(ifunc.gt.0.and.ifunc.le.j_nfunctions_)  13006
	return
	...
	if(j_objecttype_index==0) then
		...
		if(io_ >0) then
			...
			if(io_ > 0) then
				...
				endif !if(io_ > 0)  13042
			endif !if(io_ > 0)  13038
		endif !if(io_ >0)  13034
	endif !if(j_objecttype_index==0)  13032
...
function j_objecttype_name(ipobjecttype,le)
	...
	endif !if(ipobjecttype.gt.0.and.ipobjecttype.le.j_nobjecttypes)  13060
	return
...
function lenobjects(iv)
	...
	elseif(iv.gt.j_nv.and.iv.le.j_mxv)then !if(iv.gt.0.and.iv.le.j_named)then
		...
		j_err=.true.
	endif !if(iv.gt.0.and.iv.le.j_named)  13159
...
function j_object_name(iv,le)
	...
	endif !if(iv.gt.0.and.iv.le.j_named)  13183
...
real function j_eqf(iv1,iv2)
	...
	elseif((j_otype(iv1)==j_ipchar).and.(j_otype(iv2)==j_ipchar)) then !if(j_otype(iv1)==j_ipreal.and.j_otype(iv2)==j_ipreal) then
		...
		j_err = .true.
	endif !if(j_otype(iv1)==j_ipreal.and.j_otype(iv2)==j_ipreal)  13225
	return
...
subroutine j_printname(text1,iv,text2,iv2) !print variable name with text
	...
	if(iv.le.0.or.iv.gt.j_nv)then
		...
		return
	endif !if(iv.le.0.or.iv.gt.j_nv)  13250
...
subroutine j_allocatei(ivec,len)
	...
	if(allocated(ivec))then
		...
		if(len.le.iubound)then
			ivec=0
			return
		endif !if(len.le.iubound)  13304
		deallocate(ivec)
	endif !if(allocated(ivec))  13302
...
subroutine j_allocates(rvec,len)
	...
	if(allocated(rvec))then
		...
		if(len.le.iubound)then
			rvec=0.
			return
		endif !if(len.le.iubound)  13321
		deallocate(rvec)
	endif !if(allocated(rvec))  13319
...
subroutine j_allocated(dvec,len)
	...
	if(allocated(dvec))then
		...
		if(len.le.iubound)then
			dvec=j_0
			return
		endif !if(len.le.iubound)  13339
		deallocate(dvec)
	endif !if(allocated(dvec))  13337
...
subroutine j_allocated8(dvec,len)
	...
	if(allocated(dvec))then
		...
		if(len.le.iubound)then
			dvec=j_0
			return
		endif !if(len.le.iubound)  13356
		deallocate(dvec)
	endif !if(allocated(dvec))  13354
...
subroutine j_checki(ivec,iel)
	...
	if(iubound.lt.iel)then
		...
		!	deallocate(ivec2)
	end if !if(iubound.lt.iel)  13375
...
subroutine j_checkd(dvec,iel,lenn)
	...
	end if !if(iubound.lt.iel)  13424
...
subroutine j_checkr(dvec,iel,lenn)
	...
	if(iubound.lt.iel)then
		...
		if(present(lenn))lenn=iubound
		!	deallocate(ivec2)
	end if !if(iubound.lt.iel)  13453
...
subroutine j_putim(ivec,iel1,iel2,ival)
	...
	end if !if(ub1.lt.iel1)  13502
	if(ub2.lt.iel2)then
		allo=.true.;new2=2*ub2
	end if !if(ub2.lt.iel2)  13505
	if(allo)then
...
subroutine j_arg2val(arg,narg,val,nval)
	...
	enddo !i=1,narg  13626
	if(j_err)return
	call j_allocated(val,nval)
	nval=0
	do i=1,narg
		select case(j_otype(arg(i)))
		case(j_ipreal)
		...
		do j=1,nel
			...
			endif !if(j_otype(irg).eq.j_ipreal)  13653
...
real function j_valuesspl(ifunc,arg) !compute the value of a smoothing %%spline
	...
	if(argd.lt.j_o(ifunc)%d(1).or.argd.gt.j_o(ifunc)%d(nob2))then
		...
		if(nwarn.gt.mxwarn)then
			j_err=.true. ;nwarn=0
		end if !if(nwarn.gt.mxwarn)  13744
		return
	end if !if(argd.lt.j_o(ifunc)%d(1).or.argd.gt.j_o(ifunc)%d(nob2))  13739
...
integer function j_ibittest(ifunc,irow,icol)
	...
	if(btest(j_o(ifunc)%i2(ii),ibit))then
		...
		j_ibittest=0
	endif !if(btest(j_o(ifunc)%i2(ii),ibit))  13793
	return
...
integer function j_lentrim(inp)   !like len-trim but returns zero aslo when line consists of tabs
	...
	if(le.le.0)then
		j_lentrim=0
		return
	endif !if(le.le.0)  13805
	do j_lentrim=le,1,-1
		if(ichar(inp(j_lentrim:j_lentrim)).gt.32)return
	enddo !j_lentrim=le,1,-1  13809
	j_lentrim=0
	return
...
subroutine j_putiounit(nu,iv)
	...
	if (j_otype(iv).ne.j_ipchar)then
		...
		j_err=.true.
		return
	end if !if (j_otype(iv).ne.j_ipchar)  13907
	if(j_o(iv)%i(3).ne.0)then
		j_o(j_o(iv)%i(3))%i(4)=nu
	else !if(j_o(iv)%i(3).ne.0)then
		j_o(iv)%i(4)=nu
	endif !if(j_o(iv)%i(3).ne.0)  13912
	return
...
subroutine j_copy2(irg,iout)   !makes a copy of an object utility function
	...
	if(irg.ne.iout)then
		...
		if(j_otype(irg).eq.j_ipchar)then
			...
			return
		endif !if(j_otype(irg).eq.j_ipchar)  14069
		!20141219 oli: if(otype(irg).ne.0)then
		if(j_otype(irg).ne.j_ipreal)then
 
			if(allocated(j_o(irg)%r))then
				allocate(j_o(iout)%r( lbound(j_o(irg)%r,1):ubound(j_o(irg)%r,1) ));j_o(iout)%r=j_o(irg)%r
			end if !if(allocated(j_o(irg)%r))  14079
			if(allocated(j_o(irg)%i))then
				allocate(j_o(iout)%i( lbound(j_o(irg)%i,1):ubound(j_o(irg)%i,1)));j_o(iout)%i=j_o(irg)%i
			end if !if(allocated(j_o(irg)%i))  14082
			if(allocated(j_o(irg)%i2))then
				allocate(j_o(iout)%i2( lbound(j_o(irg)%i2,1):ubound(j_o(irg)%i2,1)));j_o(iout)%i2=j_o(irg)%i2
			end if !if(allocated(j_o(irg)%i2))  14085
			if(allocated(j_o(irg)%d))then
				allocate(j_o(iout)%d( lbound(j_o(irg)%d,1):ubound(j_o(irg)%d,1)));j_o(iout)%d=j_o(irg)%d
			end if !if(allocated(j_o(irg)%d))  14088
			if(allocated(j_o(irg)%ch))then
				allocate(j_o(iout)%ch( lbound(j_o(irg)%ch,1):ubound(j_o(irg)%ch,1)));j_o(iout)%ch=j_o(irg)%ch
			end if !if(allocated(j_o(irg)%ch))  14091
			if(allocated(j_o(irg)%txt))then
				allocate(j_o(iout)%txt( lbound(j_o(irg)%txt,1):ubound(j_o(irg)%txt,1)));j_o(iout)%txt=j_o(irg)%txt
			end if !if(allocated(j_o(irg)%txt))  14094
		...
		endif !if(j_otype(irg).ne.j_ipreal)  14077
 
	endif !if(irg.ne.iout)  14065
	!io=io+narg+3
	return
...
subroutine j_getobsiv(iobs,iomat,ivkeep,iviobs) !,iotrans,iviobs)  !getobs used in JLP, faster than getob
	...
	if(j_o(iomat)%i(4).eq.j_matreg)then
		...
		j_err=.true.
	endif !if(j_o(iomat)%i(4).eq.j_matreg)  14178
			...
			if(name(j:j).ge.'0'.and.name(j:j).le.'9')then
				...
				do j2=j+1,loname
					...
					if(name(j2:j2).lt.'0'.or.name(j2:j2).gt.'9')then
						...
						!	write(6,*)'j1',j1
						cycle loop1
					endif !if(name(j2:j2).lt.'0'.or.name(j2:j2).gt.'9')  14237
				enddo !j2=j+1,loname  14235
				write(6,*)'could not find ',num,' numbers in ',name(1:loname)
				j_err=.true.;return
 
				!	endif !if(in.lt.num)  13875
 
			endif !if(name(j:j).ge.'0'.and.name(j:j).le.'9')  14231
		enddo !j=j1,loname  14230
	end do loop1 !p1:	do while(in.lt.num)  14229
...
loop1:	do while(in.lt.num)
		...
		do j=j1,loname
			if(name(j:j).eq.'%')then
				...
				j1=j+1
				cycle loop1
			endif !if(name(j:j).eq.'%')  14286
		enddo !j=j1,loname  14285
 
		write(6,*)'could not find ',num,' % in ',name(1:loname)
		j_err=.true.;return
 
	enddo loop1 !p1:	do while(in.lt.num)  14283
	...
	if(iv_.le.0)then
		if(present(make))then
			...
			j_err=.true.
		endif !if(present(make))  14311
		!	write(6,*)'namew ',name(j+1:j2-1)
		if(j_err)return
	endif !if(iv_.le.0)  14310
	if(present(val))then
		if(j_otype(iv_).ne.j_ipreal)then
			write(6,*)name(j:j2-1),' is not REAL'
			j_err=.true.;return
		endif !if(j_otype(iv_).ne.j_ipreal)  14321
		val=j_v(iv_)
		is=.true.
	endif !if(present(val))  14320
	if(present(iv))then
		iv=iv_
		is=.true.
	endif !if(present(iv))  14328
	if(is)return
...
integer function j_iprintout(iob,idef)
	...
	if(j_linkoption(iob,io,j_mprint).gt.0)then
		if(j_o(iob)%i( j_linkoption(iob,io,j_mprint) ).gt.0)then
			...
			j_iprintout=idef
		endif !if(j_o(iob)%i( j_linkoption(iob,io,j_mprint) ).gt.0)  14367
	else !if(j_linkoption(iob,io,j_mprint).gt.0)then
		j_iprintout=j_v(j_ivprintoutput)
	endif !if(j_linkoption(iob,io,j_mprint).gt.0)  14366
	return
...
subroutine j_objargs(iob,io) !checking if arguments are named objects %%function
	...
	do i=1,j_o(iob)%i(io+1)
		...
		endif !if(j_o(iob)%i(io+i+1).gt.j_named)  14396
	enddo !i=1,j_o(iob)%i(io+1)  14395
	return
		...
		do i_ = il, ir - 1
			do jj = i_+1, ir
				if (array(i_) > array(jj)) then
					...
					order(i_) = order(jj)
					order(jj) = itmp_
				endif !if (array(i_) > array(jj))  14481
			enddo !jj = i_+1, ir  14480
		enddo !i_ = il, ir - 1  14479
...
subroutine j_dbw(mets,y,lkm,n,nmets,bias,sd,sb,sw,pien,suur, &
	...
	if(mets.eq.-2)then !initialization
		...
		do  k=1,lkm
			pien(k)=1.7e37
			suur(k)=-1.7e37
		end do ! k=1,lkm  14562
		return
 
	endif !if(mets.eq.-2)  14547
	if(mets.eq.-1)then  !finalization
		...
		if(illu+lkm+1.gt.napu)then
			...
			deallocate(apu2)
			napu=2*napu
		endif !if(illu+lkm+1.gt.napu)  14576
	...
	if(mets.ne.metsv.and.metsv.ne.-1)then
		...
		if(illu+lkm+1.gt.napu)then
			...
			deallocate(apu2)
			napu=2*napu
		endif !if(illu+lkm+1.gt.napu)  14640
		apu(illu)=nm
		illu=illu+1
		do k=1,lkm
			apu(illu)=sm(k)/nm
	...
	end if !if(mets.ne.metsv.and.metsv.ne.-1)  14638
...
subroutine eigen_(matin,n,ndim2,values)
	...
	ELSE IF( ANRM.GT.RMAX ) THEN !IF( ANRM.GT.ZERO .AND. ANRM.LT.RMIN ) THEN
		ISCALE = 1
		SIGMA = RMAX / ANRM
	END IF !IF( ANRM.GT.ZERO .AND. ANRM.LT.RMIN )  14807
	IF( ISCALE.EQ.1 ) &
	...
	IF( ISCALE.EQ.1 ) THEN
		...
		ELSE !IF( INFO.EQ.0 ) THEN
			IMAX = INFO - 1
		END IF !IF( INFO.EQ.0 )  14842
		CALL DSCAL( IMAX, ONE / SIGMA, W, 1 )
	END IF !IF( ISCALE.EQ.1 )  14841
	...
	! get first everything into source
			...
			if(ikp.gt.0)then
				...
				if(ikp2.le.j_linp.and.ihipsu.eq.0)then
					...
					j_err=.true.
					return
				endif !if(ikp2.le.j_linp.and.ihipsu.eq.0)  15029
			endif !if(ikp.gt.0)  15027
			...
			if(ikp.gt.0.and..not.(ihipsu.gt.0.and.ihipsu.lt.ikp))then
				...
				if(ila.gt.0)then
					if(ilabel(ila).ne.0)then
						...
						j_err=.true.;goto 999
					endif !if(ilabel(ila).ne.0)  15039
				...
				endif !if(ila.gt.0)  15038
 
 
 
			endif !if(ikp.gt.0.and..not.(ihipsu.gt.0.and.ihipsu.lt.ikp))  15036
		...
		if(winput(1:3).eq.'do('.or.winput(1:5).eq.'enddo'.or.winput(1:5).eq.'goto('.or. &
			...
			j_err=.true.;goto 999
		endif !if(winput(1:3).eq.'do('.or.winput(1:5).eq.'enddo'.or.winpu  15111
 
	endif !if(input.eq.'trans')  14976
	...
	mainloop:	do iline=1,nline2
		...
		if(.not.oneline)then
			...
			if(nteku.gt.0)then
				if(j_o(ivteku)%i(nteku).lt.0)then
					...
					nteku=nteku+1
					call teku(nteku,-1)
				endif !if(j_o(ivteku)%i(nteku).lt.0)  15148
			endif !if(nteku.gt.0)  15147
 
			if(isder2)then
				call endder(0)
			endif !if(isder2)  15156
			...
			if(.not.oneline)then
				...
				endif !if(j_ninc.eq.1)  15162
			endif !if(.not.oneline)  15161
			...
			endif !if(j_inp(1:j_linp).eq.'/')  15265
			...
			if(j_inp(1:j_linp).eq.'endif')then
				...
				enddo !j=1,nifgoto(nifthen)  15368
			...
			elseif(j_inp(1:j_linp).eq.'else')then !if(j_inp(1:j_linp).eq.'endif')then
				...
				cycle
			endif !if(j_inp(1:j_linp).eq.'endif')  15355
			...
			endif !if(j_linp.gt.lenwinput)  15429
			...
			endif !if(ikp.gt.0.and..not.(ihipsu.gt.0.and.ihipsu.lt.ikp))  15441
			! isgoto=.false.
		...
		if(j_nextlim(winput(1:lopw),1,lopw,'=').gt.lopw.and.winput(lopw:lopw).eq.';'.and.&
			...
			if(winput(1:3).eq.'if(')then
				...
				endif !if(winput(lopw1:lopw1).eq.';')  15496
				...
				endif !if(winput(lopw-1:lopw-1).eq.';')  15514
				lopw=lopw+12
 
			endif !if(winput(1:3).eq.'if(')  15491
			!		write(6,*)'<515>',winput(1:lopw)
		endif !if(j_nextlim(winput(1:lopw),1,lopw,'=').gt.lopw.and.winput  15488
		...
		if(printout2)then
			lopw=lopw-1
			printout=.false.
		endif !if(printout2)  15533
		if(lopw.lt.1)then
			write(6,*)'*illegal ; -line'
			j_err=.true.
			return
		endif !if(lopw.lt.1)  15537
				...
				if(winput(ira:ipi).eq.'ran'.or.winput(ira:ipi).eq.'rann'.or.winput(ira:ipi).eq.'ranbin'.or.&
					...
					if(ias.gt.0)then
						if(index(winput(1:ias),'-').gt.0.or.index(winput(1:ias),'All').gt.0.or. &
							...
							j_err=.true.
							return
						endif !if(index(winput(1:ias),'-').gt.0.or.index(winput(1:ias),'A  15555
					endif !if(ias.gt.0)  15554
 
 
				endif !if(winput(ira:ipi).eq.'ran'.or.winput(ira:ipi).eq.'rann'.o  15551
			endif !if(ipi.le.lopw)  15549
		endif !if(ira.gt.0.and..not.(j_isletter(winput(ira1:ira1)).or.( w  15546
		...
		if(winput(1:3).eq.'if('.or.winput(1:7).eq.'elseif(')then
			...
			if(winput(lopw-4 :lopw).eq.')then'.or.winput(1:7).eq.'elseif(')then
				...
				endif !if(winput(1:3).eq.'if(')  15604
			...
			endif !if(winput(lopw-4 :lopw).eq.')then'.or.winput(1:7).eq.'else  15595
		...
		endif !if(winput(1:3).eq.'if('.or.winput(1:7).eq.'elseif(')  15583
...
100		if(winput(1:lopw).eq.'return')then
		...
		do in=1,nn
			...
			if(node(in).eq.1.or.node(in).eq.8)then
				...
				do while(isist.ne.0)
					...
					endif !if(node(isist).ne.1.and.node(isist).ne.8)  15741
					isist=sister(isist)
				enddo !while(isist.ne.0)  15740
			endif !if(node(in).eq.1.or.node(in).eq.8)  15737
		...
		do while(ini.gt.0)
			...
			if(ivo.ne.j_ivresult.and.ivo.gt.0)then
				...
				elseif(ivo.gt.j_nv)then
					...
					j_err=.true.;goto 999
				endif !if(ivo.le.j_locked)  15764
				if(isinout)call j_putoutput(ivo,ivinl,ivoutl,ivarg)
			end if !if(ivo.ne.j_ivresult.and.ivo.gt.0)  15761
			ini=sister(ini)
		enddo !while(ini.gt.0)  15759
		...
		do ini=child(sister(2)),nn
			...
			endif !if(ivo.gt.0)  15780
 
		enddo !ini=child(sister(2)),nn  15778
		...
		endif !if(nteku.eq.ntekuv)  15820
		do iop=1,noption
			ilo=optionloc(iop)
		...
		if(nin.eq.nout.and.nin.eq.1.and.nteku.gt.ntekuv)then ! .and..not.isgoto)then
			...
			if(.not.(nin.eq.1.and.nout.gt.1.or.nout.eq.1.or.nin.eq.nout))then
				...
				j_err=.true.;goto 999
			endif !if(.not.(nin.eq.1.and.nout.gt.1.or.nout.eq.1.or.nin.eq.nou  15885
 
 
		endif !if(nin.eq.nout.and.nin.eq.1.and.nteku.gt.ntekuv)  15881
		...
		endif !if(node3.eq.j_fsetelem)  15893
		if(printout)then
			...
			endif !if(node3.eq.j_fsetelem)  15908
...
888	  continue
		...
		!	if(oneline)exit
	enddo mainloop !nloop:	do iline=1,nline2  15140
...
!write(6,*)'line ',iline
		...
		do it=j_mxnamedv+j_mxtemporalv0+1,itemporalv+1  !one extra
			if(allocated(j_o(it)%d))then
				if(size(j_o(it)%d).lt.nder)then
					...
					cycle
				endif !if(size(j_o(it)%d).lt.nder)  16063
			endif !if(allocated(j_o(it)%d))  16062
			!	write(6,*)'<888999it',it,nder
			allocate(j_o(it)%d(1:2*nder)) !make a reserve
		enddo !it=j_mxnamedv+j_mxtemporalv0+1,itemporalv+1  16061
		isder=.false.
		isder2=.false.
	...
	subroutine teku(ipos,ival)
		...
		if(ipos.gt.ntekumax)then
			...
			deallocate(tempteku)
			ntekumax=2*ntekumax
		endif !if(ipos.gt.ntekumax)  16080
	...
	subroutine getconstants()  !+options
		...
		elseif(winput(ial:ial).ge.'0'.and.winput(ial:ial).le.'9')then !if(winput(ial:ial).eq.'.')then
			goto 2
		endif !if(winput(ial:ial).eq.'.')  16252
		ial=2
...
1		continue !find next number
		...
		do while (.not.j_isnumber(winput(ial-1:ial+1)).or.hipsu.or.haka.or.namenum.or. &
				...
				if(.not.hipsu)then
					...
					if(ial.lt.lopw)then
						if(winput(ial:ial).ne.','.and.winput(ial:ial).ne.')')then
							...
							j_err=.true.;return
						endif !if(winput(ial:ial).ne.','.and.winput(ial:ial).ne.')')  16283
					endif !if(ial.lt.lopw)  16282
 
				endif !if(.not.hipsu)  16272
				hipsu=.not.hipsu
 
				if(ial.gt.lopw)return
				cycle
			endif !if(winput(ial:ial).eq."'")  16271
			if(hipsu)then
				ial=ial+1
				if(ial.gt.lopw)return
				cycle
			endif !if(hipsu)  16295
			if(winput(ial:ial+2).eq.'...')then
				...
				if(.not.(j_isletter(winput(ial+3:ial+3))).or.winput(ial-1:ial-1).eq.',')then
					...
					j_err=.true.;return
				endif !if(.not.(j_isletter(winput(ial+3:ial+3))).or.winput(ial-1:  16302
				ial=ial+3
				if(ial.gt.lopw)then
					write(6,*)'input cannot end with ...'
					j_err=.true.;return
				endif !if(ial.gt.lopw)  16307
				cycle
			endif !if(winput(ial:ial+2).eq.'...')  16300
		...
		enddo !while (.not.j_isnumber(winput(ial-1:ial+1)).or.hipsu.or.ha  16269
		...
		if(j_isletter(winput(ial-1:ial-1)).and.winput(ial:ial).ge.'0'.and.winput(ial:ial).le.'9')then
			...
			do while (winput(ial:ial).ge.'0'.and.winput(ial:ial).le.'9')
				ial=ial+1
				if(ial.gt.lopw)return
			enddo !while (winput(ial:ial).ge.'0'.and.winput(ial:ial).le.'9')  16326
			goto 1
		endif !if(j_isletter(winput(ial-1:ial-1)).and.winput(ial:ial).ge.  16324
		!	namenum=.false.
		ial0=ial
		...
		if(winput(ial1:ial1).eq.'+'.or.winput(ial1:ial1).eq.'-')then
			...
			elseif(winput(ial-2:ial-2).eq.'('.or.winput(ial-2:ial-2).eq.'='.or.& !if(ial1.eq.1)then
				...
				ial0=ial1
			endif !if(ial1.eq.1)  16339
		endif !if(winput(ial1:ial1).eq.'+'.or.winput(ial1:ial1).eq.'-')  16338
		! ial1=ial1-1
		...
		enddo !while (winput(ial:ial).ge.'0'.and.winput(ial:ial).le.'9')  16358
		...
		if(winput(ial:ial).eq.'e'.or. &
			...
			do while (winput(ial:ial).ge.'0'.and.winput(ial:ial).le.'9')
				ial=ial+1
				if(ial.gt.lopw)goto 3
			enddo !while (winput(ial:ial).ge.'0'.and.winput(ial:ial).le.'9')  16371
			if(ial.eq.iala)goto 99
		endif !if(winput(ial:ial).eq.'e'.o  16364
		lis=0
			...
			if(iip.gt.0)then
				...
				if(ilo.gt.0)then
					if(ial.eq.1)then
						ial=iip+1
						goto 1
					endif !if(ial.eq.1)  16387
					lis=iip-ial  !-1 -(ial+1) +2=iip-1-ial-1+2
					goto 31
				endif !if(ilo.gt.0)  16386
			endif !if(iip.gt.0)  16382
 
			if(waspiste)goto 99
			!	write(6,*)'continuenumber,ial',ial,winput(ial:ial)
			goto 2 !continue number
		endif !17		if(winput(ial:ial).eq.'.')  16378
...
78	read(winput(ial0+idots+2:ial1),'(i4)',err=88)ii2
			if(abs(ii2-ii1).gt.40)then
				...
				j_err=.true.;return
			endif !if(abs(ii2-ii1).gt.40)  16420
			istep=1
			...
			do iij=ii1,ii2,istep
				...
				endif !if(iij.eq.1)  16430
				if(iij.ne.ii2)then
					iial=iial+1
					winput(iial:iial)=','
					!		write(6,*)winput(1:iial),'/##&&'
				endif !if(iij.ne.ii2)  16449
			enddo !iij=ii1,ii2,istep  16429
			if(lopw.le.ial1)then
				lopw=iial
				!	write(6,*)'tas:',winput(1:lopw)
				return
			endif !if(lopw.le.ial1)  16455
		...
		if(iii.gt.0.and.ial0.gt.1)then
			...
			endif !if(winput(ial0:ial0).eq.'.'.and.(winput(ial0-1:ial0-1).eq.  16475
 
		endif !if(iii.gt.0.and.ial0.gt.1)  16474
		if(iii.le.0)goto 99
...
178				iopt=j_isin(winput(jii+1:ial-1),j_options,j_noptions)
			...
			if(iopt.le.0)then
				...
				endif !if(winput(jii+1:ial-1).eq.'input')  16539
				j_err=.true.
				return
			endif !if(iopt.le.0)  16537
			chii=j_chi5(iopt,1)
			lec=len_trim(chii)
			...
			ial3=min(lopw,ial+3)
			!ial4=min(lopw,ial+4)
			!	write(6,*)'lop ',lop,' lopw',lopw, 'ial ',ial,winput(1:ial+2)
			...
			endif !if(winput(ial2:ial3).eq.'()')  16558
			lopw=len_trim(winput)
			!	write(6,*)'winp',winput(1:lopw)
			ial=j_nextword(winput,jii+10+lec,lopw,'->')
		enddo !while(ial.lt.lopw)  16529
	...
	subroutine jparse(win,isoutput)
		...
		do while(ial.le.le)
			...
			if(win(ir9:ir9).eq.'(')then  !at the end this is not true but we do not refer outsdie win
				...
				if(iv.eq.1)then !setoption
					if(node(mother(nn)).eq.4)then
						...
						j_err=.true.
						return
					endif !if(node(mother(nn)).eq.4)  16695
				endif !if(iv.eq.1)  16694
				...
				endif !if(nchild(mutsi).eq.0)  16711
				...
				if(iv.gt.0)then !functions are stored with positive value
					...
					if(win(ial:ial).ge.'0'.and.win(ial:ial).le.'9')then
						...
						j_err=.true.;return
					endif !if(win(ial:ial).ge.'0'.and.win(ial:ial).le.'9')  16746
					!	write(6,*)'djjdjdj'
					...
					if(ir1.ge.ial)then
						...
						endif !if(isgotonew.and.(win(ial-1:ial-1).eq.'('.and.win(ir1+1:ir  16754
					...
					endif !if(ir1.ge.ial)  16752
					if(j_p.and.iv.eq.0)write(6,*)'ZERO,nn',nn
 
 
				endif !if(iv.gt.0)  16731
				ial=ir+1
				if(ial.gt.le)then
				...
				if(win(ir+1:ir+1).eq.')')then  !no arguments in fun or matrix
					...
					level=level-1
				endif !if(win(ir+1:ir+1).eq.')')  16816
				if(j_p2)write(6,*)'cyclehere,IV',iv
				!if(iv.eq.0)nn=nn-1
				cycle
			endif !if(win(ir9:ir9).eq.'(')  16685
			!! ***************************************end ( and new level
			...
			if(win(ir9:min(le,ir9+1)).eq.',,')then
				...
				j_err=.true.
				return
			endif !if(win(ir9:min(le,ir9+1)).eq.',,')  16831
...
99 			write(6,*)'*j* error when reading from ',win(ial:ir1),' input ',j_inp(1:j_linp)
	...
	80			read(win(ial:ir1),'(i8)',err=99)iv  !numeric constant number
					...
					if(isat)then
						...
						endif !if(.not.allocated(j_dotlist))  16883
						...
						if(isgotonew.and.(j_yes.or. &
							...
							endif !if(ial.gt.iir)  16912
							if(j_p)write(6,*)'getivhere4 ',iv
						endif !if(isgotonew.and.(j_yes.o  16899
				...
				endif !if(win(ial:ial).ge.'0'.and.win(ial:ial).le.'9')  16853
				...
				do while(ilist.le.nlist)
					...
					endif !if(nchild(mutsi).eq.0)  16976
					nchild(mutsi)=nchild(mutsi)+1
					lastchild(mutsi)=nn
					...
					endif !if(ilist.le.nlist)  16988
 
				enddo !while(ilist.le.nlist)  16964
				...
				if(ir.lt.le)then
					do while(win(ir:ir).eq.')')
						...
						ir=ir+1
						if(ir.gt.le)exit
					enddo !while(win(ir:ir).eq.')')  16998
				endif !if(ir.lt.le)  16997
			endif !if(ir1.ge.ial)  16851
			ial=ir+1
			if(ial.gt.le)exit
		enddo !while(ial.le.le)  16658
		...
		do while (inode.le.nn)
			...
			if(node(inode).ne.0)then
				...
				inode=inode+2
			endif !if(node(inode).ne.0)  17018
 
 
		enddo !while (inode.le.nn)  17015
		nn=nn2
		return
	...
	subroutine getiv(inp,iv,isgoto)
		...
		if(oneline.and.inp.eq.input.or..not.oneline.and.inp.eq.j_inp(1:j_linp))then
			...
			j_err=.true.
			return
		endif !if(oneline.and.inp.eq.input.or..not.oneline.and.inp.eq.j_i  17052
		...
		if(isgoto)then
			...
			if(iv.le.0)then
				...
				j_err=.true.
			endif !if(iv.le.0)  17060
			return
		endif !if(isgoto)  17058
		iv=j_object(inp)
		...
		endif !if(iv.eq.0.and.oneline.and.(inp//';'.eq.input.or.inp//';'.  17069
		lenp=len(inp)
		...
		elseif(iv.le.0)then !if(isarg.and.iv.gt.0)then
			...
			if(inp(1:1).eq."'")then
				...
				endif !if(isarg)  17107
			endif !if(inp(1:1).eq."'")  17100
		endif !if(isarg.and.iv.gt.0)  17082
...
100	continue
		...
		enddo !while(lastchild(inode).gt.0)  17172
		...
		endif !if(node(inode).gt.0)  17180
		...
		if(brother(inode).ne.0)then
			...
			if(node(inode).eq.1)call initcodenode(inode,nteku,icode,icodenode)
			goto 100
		endif !if(brother(inode).ne.0)  17198
		...
		if(inode.eq.1.or.inode.lt.0)then
			...
			call teku(nteku+1,0)
			return
		endif !if(inode.eq.1.or.inode.lt.0)  17209
...
200	if(node(inode).eq.1)then
			...
			if(inode.eq.icodenode)then
				...
				if(nargo.gt.1.or.nargo.eq.0)then
					...
					j_err=.true.;return
				endif !if(nargo.gt.1.or.nargo.eq.0)  17224
				...
				endif !if(nteku.eq.icode+4)  17237
				...
				enddo !ii=1,nargo  17262
				nteku=nteku+1 !space for mother
 
 
 
			endif !if(inode.eq.icodenode)  17222
		elseif(node(inode).eq.0)then !if(node(inode).eq.1)then
			...
			endif !if(brother(inode).ne.0)  17275
		...
		elseif(node(inode).gt.0)then !if(node(inode).eq.1)then
			...
			elseif(mother(inode).eq.1)then !if(mother(inode).eq.1.and.brother(inode).ne.0)then
				...
				do ii=1,nargo
					...
					endif !if(outlist(ii).gt.j_nv)  17305
					inode2=sister(inode2)
				enddo !ii=1,nargo  17303
				...
				do ii=1,nargo
					...
					endif !if(j_isnamedfuncarg(node(inode)).and.-node(inode2).gt.j_na  17336
			...
			endif !if(mother(inode).eq.1.and.brother(inode).ne.0)  17291
		else !if(node(inode).eq.1)then
			!	write(6,*)'HUPSISTA'
 
		endif !200	if(node(inode).eq.1)  17218
		if(brother(inode).ne.0)then
			inode=brother(inode)
			...
			if(inode.eq.1.or.inode.lt.0)then
				...
				call teku(nteku+1,0)
				return
			endif !if(inode.eq.1.or.inode.lt.0)  17380
			goto 200
		endif !if(brother(inode).ne.0)  17372
		return
...
992  write(6,*)'function ',node(inode),' ',j_functions(node(inode)),' accepts only ',j_maxarg_(node(inode)),' arguments'
	...
	subroutine initcodenode(inode,nteku,icode,icodenode)
		...
		endif !if(j_codeoption_(-node(ic)))  17412
...
10		ich=jindex(win(1:lopw),ial,ch(1:lec))
		...
		if((iop.eq.4.or.iop.eq.5).and.ich.gt.0.and.win(ich+1:ich+1).eq.'.')then
			...
			ial=ich+3
			goto 10
		endif !if((iop.eq.4.or.iop.eq.5).and.ich.gt.0.and.win(ich+1:ich+1  17479
		...
		do while(ich.gt.0)
			...
			ichloop:do i=ich-1,1,-1
				if(win(i:i).eq."'")then
					hipsu2=.not.hipsu2
					cycle
				endif !if(win(i:i).eq."'")  17496
				if(win(i:i).eq.']'.and..not.hipsu2)then
					haka2=.true.
					cycle
				endif !if(win(i:i).eq.']'.and..not.hipsu2)  17500
				if(win(i:i).eq.'['.and..not.hipsu2)then
					haka2=.false.
					cycle
				endif !if(win(i:i).eq.'['.and..not.hipsu2)  17504
				...
				if(nl.eq.nr)then
					...
					enddo !iop2=iop+1,noper  17517
 
				endif !if(nl.eq.nr)  17515
 
			enddo ichloop !loop:do i=ich-1,1,-1  17495
			i=i+1
			...
			jloop:	do j=ich+lec,lopw  !search right
				...
				if(win(j:j).eq."'")then
					hipsu2=.not.hipsu2
					cycle
				endif !if(win(j:j).eq."'")  17539
				if(win(j:j).eq.'['.and..not.hipsu2)then
					haka2=.true.
					cycle
				endif !if(win(j:j).eq.'['.and..not.hipsu2)  17543
				if(win(i:i).eq.']'.and..not.hipsu2)then
					haka2=.false.
					cycle
				endif !if(win(i:i).eq.']'.and..not.hipsu2)  17547
				if(haka2.or.hipsu2)cycle
				...
				if(nl.eq.nr)then
					...
					enddo !iop2=iop,noper  17567
 
				endif !if(nl.eq.nr)  17563
				!		if(((win(j:j).eq.','.or.win(j:j).eq.'+'.or.win(j:j).eq.'-'.or.&
				!		win(j:j).eq.'*').and.nl.eq.nr).or.nr.gt.nl)exit
			enddo jloop !op:	do j=ich+lec,lopw  17537
			j=j-1
...
20				ich=jindex(win(1:lopw),ial,ch(1:lec))
			...
			if(ich.gt.0)then
				if((iop.eq.4.or.iop.eq.5).and.win(ich+1:ich+1).eq.'.')then
					ial=ich+3
					goto 20
				endif !if((iop.eq.4.or.iop.eq.5).and.win(ich+1:ich+1).eq.'.')  17609
			endif !if(ich.gt.0)  17608
			!	ich=index(win(1:lopw),ch(1:lec))
			!	if(j_p)write(6,*)'<555ich',ich,ch2,'  ',win(1:lopw)
		enddo !while(ich.gt.0)  17486
 
 
	enddo !iop=1,noper  17470
	...
	do i=1,lopw
		...
		elseif(win(i:i).eq.')')then !if(win(i:i).eq.'(')then
			nr=nr+1
			rights(nr)=i
		endif !if(win(i:i).eq.'(')  17625
	enddo !i=1,lopw  17624
	!	ndone=0
	if(nr.le.0)return
	nremove=0
	ir0=1
	if(lefts(1).eq.1.and.rights(nr).eq.lopw)then
		...
		win(lopw:lopw)=' '
		ir0=2
	endif !if(lefts(1).eq.1.and.rights(nr).eq.lopw)  17637
	...
	do i=ir0,nr
		...
		elseif(j_isletter(win(lefts(i)+1:lefts(i)+1)).and.win(rights(i)-1:rights(i)-1).eq.')')then !if(win(lefts(i)-1:lefts(i)-1).eq.'('.and.win(rights(i)+1:rights(i
			...
			win(lefts(i):lefts(i))=' '
			win(rights(i)-1:rights(i)-1)=' '
		endif !if(win(lefts(i)-1:lefts(i)-1).eq.'('.and.win(rights(i)+1:r  17645
	enddo !i=ir0,nr  17644
		...
		do ii=1,lopw
			if(win(ii:ii).ne.' ')then
				...
				if(nn.gt.maxnode)then
					write(6,*)'*j* increase maxnode'
					j_err=.true.;return
				endif !if(nn.gt.maxnode)  17662
				win(nn:nn)=win(ii:ii)
			endif !if(win(ii:ii).ne.' ')  17660
		enddo !ii=1,lopw  17659
		lopw=nn
		if(j_p)write(6,*)'removed ', nremove, ' pairs of unnecessary parenthesis'
	endif !if(nremove.gt.0)  17657
	return
...
integer function jindex(line,ial,word)
	...
	if(lew.gt.le)then
		jindex=0
		return
	endif !if(lew.gt.le)  17688
	do i=ial,le-lew+1
		if(line(i:i).eq."'")then
			in=.not.in
			cycle
		endif !if(line(i:i).eq."'")  17693
		if(line(i:i).eq.'['.and..not.in)then
			inhaka=.true.
			cycle
		endif !if(line(i:i).eq.'['.and..not.in)  17697
		if(line(i:i).eq.']'.and..not.in)then
			inhaka=.false.
			cycle
		endif !if(line(i:i).eq.']'.and..not.in)  17701
		if(inhaka.or.in)cycle
		!	if(.not.in.and..not.inhaka)then
		if(line(i:i+lew-1).eq.word)then
			jindex=i
			return
		endif !if(line(i:i+lew-1).eq.word)  17707
		!	endif !if(.not.in.and..not.inhaka)  16559
	enddo !i=ial,le-lew+1  17692
	jindex=0
...
!	if(loc.gt.0)then
	...
	if(j_optioniob(loc).ne.iob)then
		j_linkoption=-1
		return
	endif !if(j_optioniob(loc).ne.iob)  17748
	...
	if(j_o(iob)%i(j_optionlink(loc)).eq.0)then
		...
		!		j_linkoption=j_optionlink(loc)
	endif !if(j_o(iob)%i(j_optionlink(loc)).eq.0)  17764
	...
	if(present(clear))then
		if(loc.lt.j_nopt)then
			...
			j_optiontot(loc)=j_optiontot(j_nopt)
			j_optionlink(loc)=j_optionlink(j_nopt)
		endif !if(loc.lt.j_nopt)  17772
 
		j_nopt=j_nopt-1
		j_isnopt=j_nopt.gt.0
		!	write(6,*)'option ',mopt,'lcleard'
	endif !if(present(clear))  17771
...
!	loc=findloc( j_optiontot2(1:j_nopt2),optiontot,dim=1)
	if(loc.gt.0)then
		if(j_optioniob2(loc).ne.iob)then
			...
			j_codelink=0
			return
		endif !if(j_optioniob2(loc).ne.iob)  17821
		...
		endif !if(loc.lt.j_nopt2)  17830
		j_nopt2=j_nopt2-1
	else !if(loc.gt.0)then
		j_codelink=0
 
	endif !if(loc.gt.0)  17820
...
double precision function j_codevalue(iob,link)
	...
	if(link.le.0.or.link.gt.j_o(iob)%i(0))then
		...
		j_err=.true.;return
	endif !if(link.le.0.or.link.gt.j_o(iob)%i(0))  17846
...
function j_deflist(iv,name,list0,list,listold,nres,&
	...
	if(present(list0))then
		...
		if(expan)then
			...
			endif !if(j_otype(list(1)).eq.iplist)  17904
		endif !if(expan)  17903
	elseif(present(listold))then !if(present(list0))then
		...
		if(expan.and.j_otype(listold(2)).eq.iplist)then
			...
			expan=.false.
			list00=listold(1)
		endif !if(expan.and.j_otype(listold(2)).eq.iplist)  17912
	elseif(present(ivin))then !if(present(list0))then
		list00=j_o(ivin)%i(1)
	endif !if(present(list0))  17901
	ntot=list00
	...
	if(present(ncol))then
		...
		if(nrow*ncol.ne.list00)then
			...
			j_o(ivout)%i(2)=ncol
		endif !if(nrow*ncol.ne.list00)  17953
	else
		j_o(ivout)%i(2)=1
	endif !if(present(ncol))  17947
	...
	endif !if(expan)  17968
	!		j_o(ivout)%i2=0
	if(iplist.eq.j_iplist)then
		allocate(j_o(ivout)%d(1:ntot))
		j_o(ivout)%d=j_0
	endif !if(iplist.eq.j_iplist)  17984
...
function j_putlistobject(ivlist,single,list0,list,ivin,ignored,append)		 ! put i into %%list object, size increased if needed
	...
	if(ivlist.le.0.or.ivlist.gt.j_mxv)then
		...
		endif !if(present(ivin))  18026
		if(present(single))then
			write(6,*)'single=',single
		endif !if(present(single))  18030
		if(present(list0))then
			write(6,*)'list0=',list0
		endif !if(present(list0))  18033
		if(present(list))then
			write(6,*)'list=',list
		endif !if(present(list))  18036
		if(present(ignored))then
			write(6,*)'ignored=',ignored
		endif !if(present(ignored))  18039
		if(present(append))then
			write(6,*)'append=',append
		endif !if(present(append))  18042
	...
	elseif(j_otype(ivlist).ne.j_iplist.and.j_otype(ivlist).ne.j_ipilist)then
		...
		j_err=.true.;return
	endif !if(ivlist.le.0.or.ivlist.gt.j_mxv)  18024
		...
		do i=1,list00
			...
			if(k.eq.0)then
				if(ignore)then
					...
					endif !if(.not.j_isdollar(listin(i)))  18084
				else !if(ignore)then
					nadd=nadd+1
					netput(nadd)=listin(i)
				endif !if(ignore)  18083
			endif !if(k.eq.0)  18082
		enddo !i=1,list00  18080
	endif !if(islist)  18076
	nold=j_o(ivlist)%i(1)
	if(nadd+nold.gt.j_o(ivlist)%i(4))then
		...
		! deallocate(hh)
		j_o(ivlist)%i(4)=nuus
	endif !if(nadd+nold.gt.j_o(ivlist)%i(4))  18096
	if(islist)then
...
subroutine j_printoptions()
	...
	loop:		do ii=1,j_nopt
		...
		if(j_o(iobi)%i(ioo).gt.j_nfunctions)then
			...
			j_err=.true.
		endif !if(j_o(iobi)%i(ioo).gt.j_nfunctions)  18133
		if(j_o(iobi)%i(ioo).gt.j_nfunctions)then
			write(6,*)ii,j_oname(1:j_loname),'  ',j_options(j_optionmoptio(1,ii)),&
				' option function ',j_o(iobi)%i(ioo), ' is not legal'
			j_err=.true.
		...
		endif !if(j_o(iobi)%i(ioo).gt.j_nfunctions)  18138
		...
		if(ivsource.ne.0)then
			...
			do i=1,j_o(ivsource)%i(0)
				...
				if(j_o(ivsource)%i2(i).ge.ioo-2)then
					...
					!	call j_printtext(ivsource,i-1)
					cycle loop
				endif !if(j_o(ivsource)%i2(i).ge.ioo-2)  18156
			enddo !i=1,j_o(ivsource)%i(0)  18154
			!write(6,*)'* ask J. Lappi where is the error line'
 
		endif !if(ivsource.ne.0)  18150
 
	enddo loop !p:		do ii=1,j_nopt  18129
...
subroutine j_printlist(nu,iv,head)
	...
	endif !if(j_otype(iv).ne.j_iplist)  18177
...
subroutine j_printlist0(nu,list0,list)
	...
	do i_=1,nel
		...
		endif !if(iel.gt.j_mxnamedv.and.iel.le.j_nv)  18216
		if (j_lencline+ipit>80) then
			write(nu,'(1x,a)')j_cline(1:j_lencline)
			j_lencline=0
		endif !if (j_lencline+ipit>80)  18229
		if(temp)then
		...
		endif !if (j_lencline<80)  18240
 
	enddo !i_=1,nel  18213
	write(nu,'(1x,a)')j_cline(1:j_lencline)
...
subroutine j_parent(inp,le)
	...
	do i=1,le
		...
		if(.not.(haka.or.hipsu))then
			...
			if(inp(i1:i1).eq.',')then
				...
				j_err=.true. ;return
			endif !if(inp(i1:i1).eq.',')  18287
		endif !if(.not.(haka.or.hipsu))  18285
		case(',')
		if(.not.(haka.or.hipsu))then
			if(inp(i1:i1).eq.',,')then
			...
			endif !if(inp(i1:i1).eq.',,')  18294
 
		endif !if(.not.(haka.or.hipsu))  18293
		...
		if(.not.(haka.or.hipsu))then
			...
			endif !if(.not.(j_isletter(inp(i0:i0)).or.inp(i0:i0).ge.'0'.or.in  18310
			...
			if(j_isletter(inp(i1:i1)).and.inp(max(le-4,1):le).ne.')then'.and.inp(1:3).ne.'if('.and.&
				...
				write(6,*)inp(1:le)
				j_err=.true.;return
			endif !if(j_isletter(inp(i1:i1)).and.inp(max(le-4,1):le).ne.')the  18317
			...
			endif !if((inp(i1:i1).ge.'0'.and.inp(i1:i1).le.'9'.or.(inp(i1:i1)  18324
			if(nright.gt.nleft)then
				...
				j_err=.true.;return
			endif !if(nright.gt.nleft)  18334
		endif !if(.not.(haka.or.hipsu))  18303
		case('=') !select case(inp(i:i))
		if(.not.(haka.or.hipsu))then
			neq=neq+1
			if(neq.gt.1)then
			...
			if(nleft.ne.nright)then
				...
				j_err=.true. ;return
			endif !if(nleft.ne.nright)  18353
		endif !if(.not.(haka.or.hipsu))  18346
		...
		if(nright2.gt.nleft2)then
			...
			write(6,*)' '
			j_err=.true.
		endif !if(nright2.gt.nleft2)  18386
		lis=i-iv
	...
	if(nleft.ne.nright)then
		...
		!	write(6,*)' '
		j_err=.true. ;return
	endif !if(nleft.ne.nright)  18407
	if(nleft2.ne.0)then
		if(nleft2.ne.nright2)then
			...
			if(iv.lt.le)then
				...
				le2=le2+lis
			endif !if(iv.lt.le)  18422
			le=le2
			inp(1:le)=j_tempchar(1:le)
			!		write(6,*)'pate:',inp(1:le)
		endif !if(nleft2.ne.nright2)  18416
	endif !if(nleft2.ne.0)  18415
...
logical function j_writevar(nu,narg,arg,ivform)
	...
	do iar=1,narg
		if(j_otype(arg(iar)).ne.j_ipchar.and.j_otype(arg(iar)).ne.j_ipreal)then
			...
			endif !if(j_otype(arg(iar)).ne.j_ipmatrix)  18450
			return
 
		endif !if(j_otype(arg(iar)).ne.j_ipchar.and.j_otype(arg(iar)).ne.  18448
		if(j_otype(arg(iar)).eq.j_ipreal)nar=nar+1
	enddo !iar=1,narg  18447
	if(j_err)return
	j_writevar=.true.
	if(nar.eq.narg)then
		...
		elseif(ivform.eq.j_ivdollar2)then
			...
			do iar=1,narg
				...
				endif !if(iar.gt.1)  18472
				j_tempchar(let+1:le)=chvar(1:le)
				let=let+le
			enddo !iar=1,narg  18469
			write(nu,*)j_tempchar(1:let)
		else
			write(nu,*)j_v(arg)
		endif !if(ivform.eq.j_ivdollar)  18465
		return
	endif !if(nar.eq.narg)  18464
	...
	do iar=1,narg
		if(j_otype(arg(iar)).eq.j_ipchar)then
			...
			ial=ial+10
		endif !if(j_otype(arg(iar)).eq.j_ipchar)  18490
	enddo !iar=1,narg  18489
...
logical function j_printvar(nu,narg,arg)
	...
	do iar=1,narg
		...
		endif !if(arg(iar).gt.j_mxv)  18526
		if(j_otype(arg(iar)).eq.j_ipchar)then
			call j_getchar(arg(iar),j_tempchar2(ial:),le)
			ial=ial+le
			ic=iar
			...
			if(iar-1.ne.ic)then
				if(arg(iar).le.j_named)then
					...
					j_tempchar2(ial:ial)='='
					ial=ial+1
				endif !if(arg(iar).le.j_named)  18537
			endif !if(iar-1.ne.ic)  18536
			j_tempchar2(ial:ial+9)=j_chr10(j_v(arg(iar)))
			ial=ial+12
		endif !if(j_otype(arg(iar)).eq.j_ipchar)  18531
	enddo !iar=1,narg  18523
...
subroutine j_readtext(iv)
	...
	if(j_otype(iv).ne.j_ipchar)then
		...
		j_err=.true.;return
	endif !if(j_otype(iv).ne.j_ipchar)  18568
	...
	if(allocated(j_temptxt))then
		...
		endif !if(size(j_temptxt).lt.nlin)  18610
	else !if(allocated(j_temptxt))then
		allocate(j_temptxt(1:nlin))
	endif !if(allocated(j_temptxt))  18609
	if(allocated(lenb))then
		if(size(lenb).lt.nlin)then
			deallocate(lenb)
			allocate(lenb(1:nlin))
		endif !if(size(lenb).lt.nlin)  18618
	else !if(allocated(lenb))then
		allocate(lenb(1:nlin))
	endif !if(allocated(lenb))  18617
	ntrunc=0
	ntrunc2=0
	il=0
	...
	do while(.true.)
		...
		if(leb.gt.160)then
			...
			if(ico.gt.160)then
				...
				ntrunc2=ntrunc2+1
			endif !if(ico.gt.160)  18641
		endif !if(leb.gt.160)  18633
 
		il=il+1
		j_temptxt(il)=j_tempchar(1:max(leb,1))
		lenb(il)=min(leb,160)
		!	if(j_p)WRITE(6,*)'leb,il',leb,il
	enddo !while(.true.)  18629
99		close(j_incnu)
!	write(6,*)'ilines ',il,j_incnu
	...
	endif !if(ntrunc2.gt.0)  18655
	if(il.le.0)then
		write(6,*)'file ',j_filename(1:le),' was empty'
		j_err=.true.
		return
	endif !if(il.le.0)  18662
...
loop:		do iline=j_o(iiv)%i(6)+1,j_o(iiv)%i(5)
		...
		do j=1,linpr
			...
			enddo !j2=j+1,linpr  18703
			j_ispara=.true.
			return
		enddo !j=1,linpr  18700
	enddo loop !p:		do iline=j_o(iiv)%i(6)+1,j_o(iiv)%i(5)  18698
...
!	write(6,*)'jfjfj',j_inp(1:j_linp)
	...
	endif !if(present(isinpu))  18730
	if(j_inp(1:j_linp).ne.'/')then
		j_lenpara=j_lenpara+1
		goto 1
	endif !if(j_inp(1:j_linp).ne.'/')  18733
	j_o(iiv)%i(6)=iline
	j_o(iiv)%i(11)=j_o(iiv)%i(6)+j_lenpara+1
...
subroutine j_exitpara()
	...
	if(j_o(iiv)%i(6).le.0.or.j_o(iiv)%i(6).gt.j_o(iiv)%i(5))then
		...
		j_err=.true.
	endif !if(j_o(iiv)%i(6).le.0.or.j_o(iiv)%i(6).gt.j_o(iiv)%i(5))  18749
...
function j_getin(iob,io,inclines,needed,ivform,lines)  !get in-> file
	...
	if(narg.eq.0)then
		...
		inclines=j_lenpara()
		if(inclines.le.0)then
			write(6,*)'in-> is not followed by input paragraph'
			j_err=.true.;return
 
		endif !if(inclines.le.0)  18794
	...
	endif !if(ivform0.ne.j_ivdollar)  18840
	if(j_err)return
	if(present(lines))then
		...
		if(ivform0.eq.j_ivb)then
			do while(.true.)
				...
				lines=lines+1
			enddo !while(.true.)  18851
		elseif(ivform0.eq.j_ivb2)then
			do while(.true.)
				read(nu,end=700,err=600)aa
				lines=lines+1
			enddo !while(.true.)  18856
		else
			do while(.true.)
				read(nu,*,end=700,err=600)ch
				lines=lines+1
			enddo !while(.true.)  18861
 
		endif !if(ivform0.eq.j_ivb)  18850
 
 
	endif !if(present(lines))  18848
	...
	do j=1,le
		...
		if(text(j:j).eq.';'.or.text(j:j).eq.'/')then
			if(j.gt.1)then
				...
				le=le-j+1
			endif !if(j.gt.1)  18928
			return
		else !if(text(j:j).eq.';'.or.text(j:j).eq.'/')then
			return
		end if !if(text(j:j).eq.';'.or.text(j:j).eq.'/')  18927
	enddo !j=1,le  18925
...
recursive subroutine j_showfig(ivfig,asis)
	...
	if(j_otype(j_ivterminal).eq.j_ipchar)then
		...
		terminal='qt'
		let=2
	endif !if(j_otype(j_ivterminal).eq.j_ipchar)  18971
	if(j_otype(j_ivwindow).eq.j_ipchar)then
		call j_getchar(j_ivwindow,window,lew)
	else
		window='700,700'
		lew=7
	endif !if(j_otype(j_ivwindow).eq.j_ipchar)  18977
	...
	if(j_otype(ivfig).eq.j_ipchar)then
		...
		do ili=j_gpbas+1,j_gpbas+nl
			...
			endif !if(ili.lt.j_gpbas+nl)  19028
 
		enddo !ili=j_gpbas+1,j_gpbas+nl  19027
		...
		do ili=j_gpbas+nres+1,j_o(ivfig)%i(1)
			...
			if(j_o(ivfig)%txt(ili)(1:10).ne.'set label ')then
				write(nu,'(a)')j_o(ivfig)%txt(ili)(1:j_o(ivfig)%i2(ili))
				nc=nc+1
			endif !if(j_o(ivfig)%txt(ili)(1:10).ne.'set label ')  19043
		enddo !ili=j_gpbas+nres+1,j_o(ivfig)%i(1)  19041
		!	write(6,*)'nc,nc2',nc,nc2
		if(nc.ne.nc2)then
			do ili=j_gpbas+nres+1,j_o(ivfig)%i(1)
				if(j_o(ivfig)%txt(ili)(1:10).eq.'set label ')&
					write(nu,'(a)')j_o(ivfig)%txt(ili)(1:j_o(ivfig)%i2(ili))
			enddo !ili=j_gpbas+nres+1,j_o(ivfig)%i(1)  19050
 
		endif !if(nc.ne.nc2)  19049
		write(nu,'(a)')'replot'
	...
	endif !if(j_otype(ivfig).eq.j_ipchar)  18985
	...
	if(label(le:le).eq.'[')then
		do i=le-1,1,-1
			if(label(i:i).eq.']')then
				...
				j_font=i
				return
			endif !if(label(i:i).eq.']')  19084
 
		enddo !i=le-1,1,-1  19083
		write(6,*)'label ',label(1:le), ' does not have matching ]'
		j_err=.true.
	endif !if(label(le:le).eq.'[')  19082
	j_font=0
	return
...
subroutine j_gpplot(text,add)
		...
		if(iplot.ge.2)then
			...
			if(irank.eq.2)then  !plots are on the same line
				if(i2.gt.j_gplenline)then
						...
						do j=j_o(j_gpiout)%i(1),j_gpbas+1,-1
							...
							if(j_o(j_gpiout)%txt(j)(1:ii2).eq.'e')then
								...
								if(iend.eq.2)then
									...
									exit
								endif !if(iend.eq.2)  19185
							endif !if(j_o(j_gpiout)%txt(j)(1:ii2).eq.'e')  19183
 
						enddo !j=j_o(j_gpiout)%i(1),j_gpbas+1,-1  19181
					...
					endif !if(j_o(j_gpiout)%txt(nl)(4:4).eq.'-'.an  19178
				endif !if(i2.gt.j_gplenline)  19175
			else
						...
						do j=j_o(j_gpiout)%i(1),j_gpbas+1,-1
							...
							if(j_o(j_gpiout)%txt(j)(1:ii2).eq.'e')then
								...
								if(iend.eq.2)then
									j_o(j_gpiout)%txt(j)(1:ii2)=' '
									exit
								endif !if(iend.eq.2)  19212
							endif !if(j_o(j_gpiout)%txt(j)(1:ii2).eq.'e')  19210
 
						enddo !j=j_o(j_gpiout)%i(1),j_gpbas+1,-1  19208
 
						!		call j_replacefig(j_o(j_gpiout)%i(1),' ')
						j_o(j_gpiout)%i(3)=j_o(j_gpiout)%i(3)-1
						!	done=.true.
						j_o(j_gpiout)%i2(nl)=0
					endif !if(j_o(j_gpiout)%txt(nl)(4:4).eq.'-'.a  19205
				endif !if(j_o(j_gpiout)%i2(nl-1).gt.j_gplenline)  19201
 
			endif !if(irank.eq.2)  19174
 
		endif !if(iplot.ge.2)  19165
 
		j_o(j_gpiout)%i(3)=j_o(j_gpiout)%i(3)+1
		j_gplenline=le
	else
		j_gplenline=j_gplenline+le
	endif !if(.not.add_)  19163
	iplot=j_o(j_gpiout)%i(3)
	nl=j_gpbas+(iplot+1)/2  !current plot line
	!	write(6,*)'gpp ',text
	...
	if(iplot.gt.j_o(j_gpiout)%i(4))then
		...
		if(nlres+j_o(j_gpiout)%i(1).gt.isize)then
			...
			call j_inctxt(j_gpiout,0)
		endif !if(nlres+j_o(j_gpiout)%i(1).gt.isize)  19244
		...
		enddo !i=j_o(j_gpiout)%i(1),j_gpbas+nlres+1,-1  19248
	...
	endif !if(iplot.gt.j_o(j_gpiout)%i(4))  19240
	ipt=index(text,' pt ')
	i2=j_o(j_gpiout)%i2(nl)
	...
	if(ipt.gt.0)then
		if(j_otype(j_gpmark).eq.j_ipchar)then
			...
			endif !if(ii2.ne.0)  19279
 
		endif !if(j_otype(j_gpmark).eq.j_ipchar)  19266
	endif !if(ipt.gt.0)  19265
	nlres=(j_o(j_gpiout)%i(4)+1)/2  !reserved lines
...
78		if(present(prompt))then
		...
		write(6,1000)'<ret>'
	endif !78		if(present(prompt))  19348
	j_ispause=.true.
	!j_cline=' '
	read(5,'(a)')j_cline
	...
	if(index(j_cline(1:lec),'=;list(').gt.0)then
		...
		goto 78
	endif !if(index(j_cline(1:lec),'=;list(').gt.0)  19363
	...
	if(j_cline(1:lec).eq.'e'.or.j_cline(1:lec).eq.'/')then
		...
		j_err=.true.;goto 90
	endif !if(j_cline(1:lec).eq.'e'.or.j_cline(1:lec).eq.'/')  19371
	if(j_cline(1:lec).eq.'end')then
		!			if(j_ninc.eq.1)goto 90
		write(6,*)'end return from pause'
		j_stop=.true.;goto 90
	endif !if(j_cline(1:lec).eq.'end')  19376
	if(present(do))then
		...
		endif !if(j_err)  19384
		goto 78
	endif !if(present(do))  19381
90	j_ispause=.false.
...
subroutine j_inclinegoto(line)
	...
	if(line.gt.j_o(iiv)%i(5).or.line.le.0)then
		...
		j_err=.true.
		return
	endif !if(line.gt.j_o(iiv)%i(5).or.line.le.0)  19417
...
double precision function j_stemcurveint(ifunc,darg1,darg2,iob,io,cylinder,ivdmin)
	...
	endif !if(darg1.gt.darg2.or.darg1.lt.j_0)  19463
	...
	if(isdmin)then
		...
		endif !if(ivdmin.lt.0)  19477
	endif !if(isdmin)  19476
	...
	endif !if(nocylinder.and.isdmin)  19489
	...
	if(cylinder.and..not.isdmin.and..not.ishmax)then
		...
		return
	endif !if(cylinder.and..not.isdmin.and..not.ishmax)  19495
	...
	if(j_o(ifunc)%i(3).ne.0)then  !harvester
		...
		endif !if(darg2-h0.gt.1.d-5)  19577
		...
		endif !if(cylinder)  19605
		return
	endif !if(j_o(ifunc)%i(3).ne.0)  19521
	...
	do i=1,npo
		if(j_o(ifunc)%d(i).ge.darg1)then
			if(i.eq.1)then
				...
				i1=i
				goto 8
			endif !if(i.eq.1)  19621
		endif !if(j_o(ifunc)%d(i).ge.darg1)  19620
	enddo !i=1,npo  19619
	!	j_stemcurveint=j_0
	return
	8	continue
	if(ishmax)then
		...
		do i=1,npo
			...
			if(dapu2.lt.dapu)then
				...
				return
			endif !if(dapu2.lt.dapu)  19656
			dapu=dapu2
		enddo !i=1,npo  19650
 
 
	endif !if(ishmax)  19635
	...
	do while(j_o(ifunc)%d(i).le.darg2.and.i.le.npo)
		...
		dapu=dapu2
		i=i+1
	enddo !while(j_o(ifunc)%d(i).le.darg2.and.i.le.npo)  19689
	...
	if(darg2-h1.gt.1.d-5)then
		...
		if(isdmin.and.dapu2.lt.dmin)dmin=dapu2
	endif !if(darg2-h1.gt.1.d-5)  19703
	if(isdmin)then
		!cylinder
		...
		if(j_o(ifunc)%i(3).ne.0)then
			...
			if(j_o(ifunc)%d(i1).eq.j_0)then
				do i=i1-1,i1-4
					...
					endif !if(j_o(ifunc)%d(i).ne.j_0)  19758
				enddo !i=i1-1,i1-4  19757
				j_stemcurveval=j_0
				return
			endif !if(j_o(ifunc)%d(i1).eq.j_0)  19756
			...
			return
		endif !if(j_o(ifunc)%i(3).ne.0)  19752
		! h:s are given
		...
		do i=1,npo
			if(j_o(ifunc)%d(i).ge.darg)then
				if(i.eq.1)then
					...
					i1=i
					goto 8
				endif !if(i.eq.1)  19782
			endif !if(j_o(ifunc)%d(i).ge.darg)  19781
		enddo !i=1,npo  19780
		i0=npo-1
		i1=npo
	8	continue
!		write(6,*)i0,i1,j_o(ifunc)%d(ncol+i0),j_o(ifunc)%d(i0),j_o(ifunc)%d(ncol+i1),j_o(ifunc)%d(ncol+i0),&
	!		j_o(ifunc)%d(i1),j_o(ifunc)%d(i0)
		...
		endif !if(j_o(ifunc)%d(ncol+1).lt.dapu.or.npo.lt.2)  19809
		do i=2,npo
			...
			endif !if(j_o(ifunc)%d(ncol+i).lt.dapu)  19816
		enddo !i=2,npo  19814
		i0=npo-1
		i1=npo
		if(i0.le.0)then
			j_stemcurveval=j_0
			return
		endif !if(i0.le.0)  19826
 
		if(j_o(ifunc)%d(ncol+i0)-dapu.gt.2.d0.or.j_o(ifunc)%d(ncol+i1).le.j_o(ifunc)%d(ncol+i0))then
			j_stemcurveval=j_0
			return
		endif !if(j_o(ifunc)%d(ncol+i0)-dapu.gt.2.d0.or.j_o(ifunc)%d(ncol  19831
9	continue
	!	write(6,*)'i0,i1',i0,i1
		...
		if(j_o(ifunc)%i(3).eq.0)then
			...
			! h0+(d-d0)*(h1-h0)/(d1-d0)
		endif !if(j_o(ifunc)%i(3).eq.0)  19838
		if(j_stemcurveval.lt.j_0)j_stemcurveval=j_0
		return
 
 
	endif !if(darg.ge.j_0)  19751
...
double precision function j_stemsplineint(ifunc,darg1,darg2,iob,io)
	...
	endif !if(darg1.gt.darg2)  19867
	if(xl.eq.xu)then
		j_stemsplineint=j_0
		return
	endif !if(xl.eq.xu)  19874
	npo=j_o(ifunc)%i(1)
...
dout:	do j=1,npo
		if(j_o(ifunc)%r(j+npo).ge.xu)then
			...
			if(j.eq.1)then
				...
				enddo !j2=j-1,1,-1  19905
				exit dout
			endif !if(j.eq.1)  19901
		endif !if(j_o(ifunc)%r(j+npo).ge.xu)  19899
 
	enddo dout !t:	do j=1,npo  19898
	...
	endif !if(dmin2.lt.dmin-0.5d0)  19921
...
double precision function j_stemsplineval(ifunc,darg)
	...
	if(darg.gt.j_0)then
		...
		if(arg.le.0.4)then
			...
			endif !if(j_err)  19966
		endif !if(arg.le.0.4)  19962
 
	endif !if(darg.gt.j_0)  19940
...
subroutine j_readfrominp(vect,nvar)
	...
	do while(j.le.j_linp)
		...
		if(j_inp(j:j).eq.','.or.ichar(j_inp(j:j)).lt.35)then
			j=j+1
			cycle
		endif !if(j_inp(j:j).eq.','.or.ichar(j_inp(j:j)).lt.35)  20102
	...
	76		nvar=nvar+1
		...
		if(ivar1.gt.inum1)then
			...
			if(iv.le.0)then
				...
				endif !if(nvar.eq.j_makelistnvar.or.nvar.eq.j_caseread.or.ivar)  20149
			endif !if(iv.le.0)  20147
			if(nvar.eq.j_makelistnvar)then
				iper=j_putlistobject(j_makelist,single=iv)
				j_v(j_o(j_makelist)%i2(iper))=iper
			endif !if(nvar.eq.j_makelistnvar)  20162
			!vect(nvar)=j_v(iv)
			...
			elseif(insparse.and.ivar)then
				...
				if(insparse)then
					...
					vect(nvar)=j_v(iv)
				endif !if(insparse)  20190
			endif !if(nvar.eq.j_caseread)  20168
		else
			if(insparse)then
			...
			endif !if(insparse)  20198
			!write(6,*)j_inp(j:j21),vect(nvar)
		endif !if(ivar1.gt.inum1)  20139
		...
		if(nvar.eq.nsize)then
			...
			inum1=0
			insparse=.true.
		endif !if(nvar.eq.nsize)  20208
		if(j2.gt.j_linp)then
			if(j_needsall.and..not.j_issparse)then
				write(6,*)'line contained only ' ,nvar, ' values from ',nsize ,' needed'
				write(6,*) j_inp(1:j_linp)
				j_err=.true.
			endif !if(j_needsall.and..not.j_issparse)  20216
			return
		endif !if(j2.gt.j_linp)  20215
		j=j2+1
 
	enddo !while(j.le.j_linp)  20100
	return
...
recursive subroutine j_objfunc(get,iob,io,imat,narg,arg,iout)
	...
	if(.not.(get.or.j_otype(imat).eq.j_ipdata))then
		...
		j_err=.true.;return
	endif !if(.not.(get.or.j_otype(imat).eq.j_ipdata))  20252
	...
	if(narg.eq.1)then
		if(arg(1).eq.j_ivninf)then
			...
			endif !if(j_o(imat)%i(3).ne.0)  20277
			return
		elseif(arg(1).eq.j_ivinf)then
			...
			if(j_o(imat)%i(3).ne.0)then
				if(ind.gt.0)then
					...
					j_v(iout)=j_0
				endif !if(ind.gt.0)  20286
			else
				!ncol=j_o(ifunc)%i(2)
				j_v(iout)=j_o(imat)%d(j_o(imat)%i(2)+ind)
			endif !if(j_o(imat)%i(3).ne.0)  20285
			return
 
 
		endif !if(arg(1).eq.j_ivninf)  20275
	...
	elseif(narg.ne.2)then
		...
		if(nmin.lt.0.and.nmax.lt.0)then
			...
			if(nmin.eq.0.or.nmax.eq.0)then
				...
				j_err=.true.;return
			endif !if(nmin.eq.0.or.nmax.eq.0)  20314
			if(nmin.gt.0.and.nmax.gt.0)then
				write(6,*)'there cannot be both min-> max-> '
				j_err=.true.;return
			endif !if(nmin.gt.0.and.nmax.gt.0)  20318
		...
		endif !if(nmin.lt.0.and.nmax.lt.0)  20311
	endif !if(narg.eq.1)  20274
	...
	subroutine data()
		...
		do i=1,narg
			...
			if(j.le.0)then
				...
				j_err=.true.
			endif !if(j.le.0)  20503
			j_itempvector(i)=j
		enddo !i=1,narg  20501
		if(j_err)return
		...
		if(nargc.gt.0)then
			...
			do i=1,nargc
				...
				if(j.le.0)then
					...
					j_err=.true.
				endif !if(j.le.0)  20524
				j_itempvector2(i)=j
			enddo !i=1,nargc  20522
			if(nargc.eq.1.and.narg.eq.1)then
				...
				return
			endif !if(nargc.eq.1.and.narg.eq.1)  20531
			j_i8=narg
			nobs8=nargc
			ioutv=j_defmatrix8(iout,' ',nobs8,j_i8,j_matreg)
			ibas=0
			...
			do j=1,nobs8
				...
				ibas=ibas+narg
			enddo !j=1,nobs8  20545
			return
 
		endif !if(nargc.gt.0)  20513
		...
		if(ifrom.eq.iuntil)then
			if(narg.eq.1)then
				...
				ioutv=j_defmatrix(iout,' ',1,narg,j_matreg)
				j_o(iout)%d(1:narg)=j_o(ivmat)%d(ibasin+j_itempvector(1:narg))
				return
 
			endif !if(narg.eq.1)  20561
		endif !if(ifrom.eq.iuntil)  20560
	...
	subroutine trans()
		...
		if(nres.gt.0)then
			do ir=1,nres
				...
				endif !if(j_otype(j_mxnamedv+ir).eq.j_ipreal)  20620
			enddo !ir=1,nres  20619
			call dotrans(imat,1)
			j_v(j_mxnamedv+1:j_mxnamedv+nres)=j_temporals(1:nres)
		else
			call dotrans(imat,1)
 
		endif !if(nres.gt.0)  20618
