...
subroutine j_jinit0(jparf)
	...
	if(.not.j_inited)then
		...
		j_title='jlp22    29.  6. 2023 (c) Juha Lappi and Natural Resources Institute Finland'
		write(6,*)j_title
...
567		lep=len_trim(jparf)
	...
	if(j_ivprintpause.ne.ivout_)then
		...
		enddo !i=1,j_named    866
		j_stop=.true.
		return
	endif !if(j_ivprintpause.ne.ivout_)    860
...
recursive subroutine dotrans(iob,ioi)
	...
	if(j_recursion.gt.j_mxrecursion)then
		...
		j_err=.true.
		return
	endif !if(j_recursion.gt.j_mxrecursion)    949
	...
	if(j_o(iob)%i(io).lt.0)then
		...
		if(j_v(j_ivdebugtrans).ne.j_0)call j_debug(iob)
		io=io+1
	endif !if(j_o(iob)%i(io).lt.0)   1013
	! if(j_nopt.gt.0)then
	...
	if(j_v(j_ivdollar2).eq.179.d0)then
		...
		if(j_o(iob)%i(io).ne.0)then
			...
			write(6,*)io,'finito'
		endif !if(j_o(iob)%i(io).ne.0)   1039
	endif !if(j_v(j_ivdollar2).eq.179.d0)   1034
...
subroutine setcodeopt(iob,io)
	...
	endif !if(j_nopt2.gt.j_maxopenopt2)   1874
	...
	if(j_o(iob)%i(io+2).eq.0)then    !nothing to be computed
		...
		io=j_o(iob)%i(io+2)
	endif !if(j_o(iob)%i(io+2).eq.0)   1885
...
subroutine setoption(iob,io)
	...
	if(j_nopt.ge.j_maxopenopt.or.(j_v(j_ivdollar).eq.455.D0.and.j_nopt.gt.0))then
		...
		j_err=.true.
		return
	endif !if(j_nopt.ge.j_maxopenopt.or.(j_v(j_ivdollar).eq.455.D0.an   1909
	j_nopt=j_nopt+1
	j_isnopt=.true.
...
recursive subroutine getelem(iob,io)  ! matrix(iel) e.g. out=a(irow,icol)  %%matrix
	...
	elseif(j_otype(imat).eq.j_ipstemcurve)then
		...
		if(narg1.eq.1)then
			if(arg(1).eq.j_ivninf)then
				...
				endif !if(j_o(imat)%i(3).ne.0)   2115
				return
			elseif(arg(1).eq.j_ivinf)then
				...
				if(j_o(imat)%i(3).ne.0)then
					if(ind.gt.0)then
						...
						j_v(iout)=j_0
					endif !if(ind.gt.0)   2124
				else
					!ncol=j_o(ifunc)%i(2)
					j_v(iout)=j_o(imat)%d(j_o(imat)%i(2)+ind)
				endif !if(j_o(imat)%i(3).ne.0)   2123
				return
 
 
			endif !if(arg(1).eq.j_ivninf)   2113
		...
		elseif(narg1.ne.2)then
			...
			if(nmin.lt.0.and.nmax.lt.0)then
				...
				if(nmin.eq.0.or.nmax.eq.0)then
					...
					j_err=.true.;return
				endif !if(nmin.eq.0.or.nmax.eq.0)   2152
				if(nmin.gt.0.and.nmax.gt.0)then
					write(6,*)'there cannot be both min-> max-> '
					j_err=.true.;return
				endif !if(nmin.gt.0.and.nmax.gt.0)   2156
			...
			endif !if(nmin.lt.0.and.nmax.lt.0)   2149
		endif !if(narg1.eq.1)   2112
		!!call j_clearoption(iob,io)
		return
 
 
 
	endif !if(j_otype(imat).eq.j_ipstemspline)   2099
	...
	if(j_otype(imat).eq.j_ipilist.or. &
		...
		if(j_otype(imat).eq.j_iptext)then
			...
			imx=j_o(imat)%i(1)
		endif !if(j_otype(imat).eq.j_iptext)   2188
		...
		endif !if(irg1.le.0.or.irg1.gt.imx)   2194
		...
		endif !if((j_otype(imat).eq.j_iptext.or.j_otype(imat).eq.j_iptxt)   2201
		if(j_otype(imat).eq.j_iptext)then
		...
		endif !if(j_otype(imat).eq.j_iptext)   2207
		if(get)then
		...
		if(irg1.le.0.or.irg1.gt.i)then
			...
			j_err=.true.;return
		endif !if(irg1.le.0.or.irg1.gt.i)   2251
		...
		endif !if(islist)   2257
		return
 
 
 
	endif !if(j_otype(imat).eq.j_ipilist.o   2179
	...
	if(ismatrix.or.isreal)then
		...
		if(j_otype(iout).eq.j_iplist)then
			call getsetlist()
			return
		endif !if(j_otype(iout).eq.j_iplist)   2353
		...
		if(diag.and.set.and.ismatrix)then
			if(narg1.eq.0.and.j_otype(iout).eq.j_ipreal)then
				...
				enddo !i=1,min(ncol,nrow)   2362
				return
			endif !if(narg1.eq.0.and.j_otype(iout).eq.j_ipreal)   2360
 
		endif !if(diag.and.set.and.ismatrix)   2359
		...
		elseif(j_otype(arg(1)).eq.j_ipilist)then
			...
			elseif(narg1.eq.2)then
				...
				if(ncol.gt.1)then
					...
					j_err=.true.
					return
				endif !if(ncol.gt.1)   2430
			endif !if(narg1.gt.2)   2423
...
545		continue
		...
		if(isvector)then
			...
			isrow=.false.
		endif !if(isvector)   2463
		if(norange)then
			irange=1
			call iranges()
			if(j_err)return
		endif !if(norange)   2469
		...
		if(get)then
			...
			if(iout.eq.imat.and..not.isreal)then
				...
				goto 100
			endif !if(iout.eq.imat.and..not.isreal)   2517
			call getmatrix()
			if(j_err)return
			if(p)then
			...
			if(j_otype(iout).eq.j_ipreal)then
				...
				return
			endif !if(j_otype(iout).eq.j_ipreal)   2534
			...
			if(simple)then
				if(j_otype(iout).ne.j_ipreal)then
					...
					j_err=.true.;return
				endif !if(j_otype(iout).ne.j_ipreal)   2545
				if(issum)then
					if(isvector)then
					...
					endif !if(isvector)   2561
 
				endif !if(issum)   2551
				return
			endif !if(simple)   2544
			call setmatrix()
			if(p)then
	...
	endif !if(ismatrix.or.isreal)   2335
	...
	if(narg1.eq.0)then
		...
		elseif(j_otype(imat).eq.j_iptext)then
			...
			j_err=.true.
		endif !if(j_otype(imat).eq.j_ipregr)   2587
		return
	endif !if(narg1.eq.0)   2583
	if(islist)then
		ind=j_v(arg(1))
		...
		if(ind.le.0.or.ind.gt.j_o(imat)%i(1))then
			...
			j_err=.true.;return
		endif !if(ind.le.0.or.ind.gt.j_o(imat)%i(1))   2602
		...
		if(narg1.ne.1)then
			...
			if(ind2.le.0.or.ind2.gt.j_o(imat)%i(1).or.ind2.lt.ind)then
				...
				j_err=.true.;return
			endif !if(ind2.le.0.or.ind2.gt.j_o(imat)%i(1).or.ind2.lt.ind)   2610
			iout=j_deflist(iout,' ',list0=ind2-ind+1,list=j_o(imat)%i2(ind:ind2))
			return
		endif !if(narg1.ne.1)   2608
		! call j_getname(imat,iout)
		...
		if(set)then
			...
			endif !if(j_otype(iel).eq.j_ipreal)   2626
		endif !if(set)   2622
		return
	endif !if(islist)   2599
	...
	elseif(j_otype(imat).eq.j_ipdata)then !if(j_otype(imat).eq.j_iptrans)then
		if(.not.get.and.j_o(imat)%i(10).eq.0)then
			...
			j_err=.true.;return
		endif !if(.not.get.and.j_o(imat)%i(10).eq.0)   2670
		call data()
		return
	...
	endif !if(j_otype(imat).eq.j_iptrans)   2648
	...
	if(ismatrix)then
		...
		end if !if(j_o(iout)%i(1).eq.1.and.j_o(iout)%i(2).eq.1)   2701
	else
		call j_getname(imat)
	...
	subroutine iranges()
		...
		if(j_otype(arg(1)).eq.j_ipilist)then
			...
			isili(1)=.false.
		endif !if(j_otype(arg(1)).eq.j_ipilist)   2745
		if(arg(1).eq.j_ivall)then
			...
			irange(1)=inde(1)
			irange(2)=inde(1)
		endif !if(arg(1).eq.j_ivall)   2757
		...
		if(j_otype(arg(1)).eq.j_ipilist)then
			...
			endif !if(mro.gt.j_o(imat)%i(1))   2784
			...
			endif !if(j_otype(arg(2)).eq.j_ipilist)   2789
		else !if(j_otype(arg(1)).eq.j_ipilist)then
			...
			elseif(arg(2).eq.j_ivall)then
				...
				if(inde(2).le.0)then
					...
					j_err=.true.;return
				endif !if(inde(2).le.0)   2836
				simple=.true.
				!			write(6,*)'irnageshq ',irange
			endif !if(inde(2).lt.0)   2818
			!endif
			! if(.not.isvector)then
			! irange(4)=ncol
			! endif !if(.not.isvector)then
		endif !if(j_otype(arg(1)).eq.j_ipilist)   2777
		...
		if(arg(1).eq.j_ivall)then
			...
			return
		endif !if(arg(1).eq.j_ivall)   2850
		irange(1)=inde(1)
		if(inde(2).lt.0)then
			irange(2)=-inde(2)
			if(arg(2).eq.j_ivtolast)irange(2)=nrow
			...
			if(arg(3).eq.j_ivall)then
				irange(4)=ncol
				return
			endif !if(arg(3).eq.j_ivall)   2864
	...
	subroutine getmatrix()
		if(isreal)then
			if(irange(2).gt.1.or.irange(4).gt.1)then
				...
				j_err=.true.;return
			endif !if(irange(2).gt.1.or.irange(4).gt.1)   2905
			if(j_otype(iout).ne.j_ipreal)call j_del(iout)
			j_v(iout)=j_v(imat)
			return
		endif !if(isreal)   2904
		useold=.false.
		if(j_otype(iout).eq.j_ipmatrix)then
			useold=j_o(iout)%i(3).eq.nel2
		endif !if(j_otype(iout).eq.j_ipmatrix)   2915
		...
		if(isvector)then
			...
			if(irange(2).gt.j_nrows(imat).or.irange(4).gt.j_ncols(imat))then
				...
				j_err=.true.;return
			endif !if(irange(2).gt.j_nrows(imat).or.irange(4).gt.j_ncols(imat   2960
			do iro0=irange(1),irange(2)
				...
				if(isili(2))then
					...
					if(iscoef)then
						...
						enddo !ico=1,ncol2   2973
 
					else !if(iscoef)then
						do ico=1,ncol2
							ie=ie+1
							j_o(iout)%d(ie)=j_o(imat)%d((iro-1)*ncol+ilist2(ico))
						enddo !ico=1,ncol2   2979
					endif !if(iscoef)   2972
					...
					if(iscoef)then
						...
						enddo !ico=irange(3),irange(4)   2986
						...
						enddo !ico=irange(3),irange(4)   2993
					endif !if(iscoef)   2985
				endif !if(isili(2))   2970
			enddo !iro0=irange(1),irange(2)   2966
			...
			if(ist.and.isvectorout)then
				...
				j_o(iout)%i(2)=j_o(iout)%i(1)
				j_o(iout)%i(1)=i2
			endif !if(ist.and.isvectorout)   3010
		endif !if(isvector)   2937
			...
			if(isvector)then
				...
				do iro0=irange(1),irange(2)
					...
					if(isili(2))then
						...
						enddo !ico=1,ncol2   3067
					else !if(isili(2))then
						ibas=(iro-1)*ncol
						j_o(imat)%d(ibas+irange(3):ibas+irange(4))=j_v(iout)
					endif !if(isili(2))   3065
				enddo !iro0=irange(1),irange(2)   3062
			endif !if(isvector)   3052
		endif !if(issum)   3021
	...
	subroutine setmatrix()
		if(nel2.ne.j_o(iout)%i(3))then
			...
			endif !if(j_otype(imat).eq.j_ipmatrix)   3084
			j_err=.true.;return
		endif !if(nel2.ne.j_o(iout)%i(3))   3081
			...
			if(isvector)then
				...
				do iro0=irange(1),irange(2)
					...
					if(isili(2))then
						...
						ibas=ibas+ncol
						ie=ie+ncol2
					endif !if(isili(2))   3105
				enddo !iro0=irange(1),irange(2)   3102
			endif !if(isvector)   3093
				...
				if(diag)then
					...
					do iro0=irange(1),irange(2)
						...
						if(isili(2))then
							...
							ibas=ibas+ncol
							!	ie=ie+ncol2
						endif !if(isili(2))   3140
					enddo !iro0=irange(1),irange(2)   3136
				else
					do iro0=irange(1),irange(2)
						if(isili(1))ibas=(ilist1(iro0)-1)*ncol
						...
						if(isili(2))then
							...
							ibas=ibas+ncol
							ie=ie+ncol2
						endif !if(isili(2))   3158
					enddo !iro0=irange(1),irange(2)   3155
				endif !if(diag)   3134
			endif !if(isvector)   3124
		endif !if(issum)   3091
	...
	subroutine trans()
		...
		if(nres.gt.0)then
			do ir=1,nres
				...
				endif !if(j_otype(j_mxnamedv+ir).eq.j_ipreal)   3200
			enddo !ir=1,nres   3199
			call dotrans(imat,1)
			j_v(j_mxnamedv+1:j_mxnamedv+nres)=j_temporals(1:nres)
		else
			call dotrans(imat,1)
 
		endif !if(nres.gt.0)   3198
		...
		if(ivcases.ne.0)then
			...
			if(ice.le.0)then
				...
				if(ice.le.0.or.ice.gt.j_o(ivmat)%i(1))then
					...
					j_err=.true.;return
				endif !if(ice.le.0.or.ice.gt.j_o(ivmat)%i(1))   3245
 
			endif !if(ice.le.0)   3243
			...
			if(iro.le.0)then
				...
				if(iro.le.0.or.iro.gt.j_o(ivmat)%i(2))then
					...
					j_err=.true.;return
				endif !if(iro.le.0.or.iro.gt.j_o(ivmat)%i(2))   3255
 
			endif !if(iro.le.0)   3253
			...
			if(get)then
				...
				if(j_otype(iout).ne.j_ipreal)then
					...
					j_err=.true.
					return
				endif !if(j_otype(iout).ne.j_ipreal)   3268
...
469		ii=j_inlistobject(arg(1),ivkeep)
		i2=0
		if(ii.le.0)then
			i2=1
			iro=j_v(arg(1))
			if(iro.le.0.or.iro.gt.j_o(ivmat)%i(1))then
				write(6,*)'cannot get observation ',iro,' from ',j_o(ivmat)%i(1),' observations'
				j_err=.true.;return
			endif !if(iro.le.0.or.iro.gt.j_o(ivmat)%i(1))   3300
			if(narg1.eq.2)then
 
 
				ind2=j_inlistobject(arg(2),ivkeep)
				if(ind2.le.0)then
					call j_getname(arg(2),imat)
					write(6,*)'variable ',j_oname(1:j_loname),' not in data ',j_oname2(1:j_loname2)
					j_err=.true.;return
				endif !if(ind2.le.0)   3308
 
				j_v(iout)=j_o(ivmat)%d((iro-1)*j_o(ivmat)%i(2)+ind2)
				return
 
			endif !if(narg1.eq.2)   3304
		endif !if(ii.le.0)   3297
 
		allocate(list(1:narg1-i2))
		!	write(6,*)'keep',j_o(ivkeep)%i2,' arg ',arg
		do j=1,narg1-i2
			list(j)=j_inlistobject(arg(j+i2),ivkeep)
 
			if(list(j).le.0)then
				!	write(6,*)'imat ',imat
				call j_getname(arg(j+i2),imat)
				!call j_printname('variable ',arg(j),' not in data')
				write(6,*)'variable ',j_oname(1:j_loname),' not in data ',j_oname2(1:j_loname2)
				j_err=.true.
			endif !if(list(j).le.0)   3325
		enddo !j=1,narg1-i2   3322
		if(j_err)then
			deallocate(list);return
		endif !if(j_err)   3333
		if(ii.le.0)then
			iout=j_defmatrix(iout,' ',1,narg1-1,j_matreg)
			ibas=(iro-1)*j_o(ivmat)%i(2)
			do j=1,narg1-1
				j_o(iout)%d(j)=j_o(ivmat)%d(ibas+list(j))
 
			enddo !j=1,narg1-1   3339
			return
		endif !if(ii.le.0)   3336
		j_i8=narg1
		...
		if(isvector)then
			...
			do iro0=irange(1),irange(2)
				...
				do ico=irange(3),irange(4)
					...
					endif !if(isili(2))   3420
				enddo !ico=irange(3),irange(4)   3418
 
			enddo !iro0=irange(1),irange(2)   3415
		endif !if(isvector)   3406
		iout=j_defmatrix8(iout,' ',nrow2,ncol2,j_matreg)
		j_o(iout)%d=temp
		deallocate(temp)
		return
	...
	subroutine diags()
			...
			if(get)then
				...
				elseif(j_otype(iout).eq.j_ipmatrix)then !if(j_otype(iout).eq.j_ipreal)then
					if(nrow.ne.j_o(iout)%i(3))then
						...
						j_err=.true.;return
					endif !if(nrow.ne.j_o(iout)%i(3))   3469
					...
					j_err=.true.
				endif !if(j_otype(iout).eq.j_ipreal)   3461
				return
			endif !if(get)   3445
		endif !if(narg1.eq.0)   3443
		irange(1:2)=1
		irange(3:4)=0
		call iranges()
		if(j_err)return
		...
		endif !if(irange(3).ne.0.and.(irange(3).ne.irange(1).or.irange(4)   3491
		...
		if(get)then
			...
			do i=irange(1),irange(2)
				...
				ibas=ibas+nrow
			enddo !i=irange(1),irange(2)   3513
			return
		else !if(get)then
			if(j_otype(iout).eq.j_ipreal)then
				...
				do i=irange(1),irange(2)
					j_o(imat)%d(ibas+i)=j_v(iout)
					ibas=ibas+nrow
				enddo !i=irange(1),irange(2)   3522
				return
			...
			elseif(j_otype(iout).eq.j_ipmatrix)then !if(j_otype(iout).eq.j_ipreal)then
				if(nrow2.ne.j_o(iout)%i(3))then
					...
					j_err=.true.;return
				endif !if(nrow2.ne.j_o(iout)%i(3))   3529
				i2=0
				do i=irange(1),irange(2)
					i2=i2+1
					!				write(6,*)'imat,iout,ibas,ir1,ir2',imat,iout,ibas,ir1,ir2
					...
					ibas=ibas+nrow
				enddo !i=irange(1),irange(2)   3534
				return
			else !if(j_otype(iout).eq.j_ipreal)then
				write(6,*)'input is not MATRIX or REAL'
				j_err=.true.;return
			endif !if(j_otype(iout).eq.j_ipreal)   3520
			...
			if(isvector)then
				...
				if(get)then
					do iro=irange(1),irange(2)
						...
						enddo !ico=irange(3),irange(4)   3567
 
					enddo !iro=irange(1),irange(2)   3566
				else !if(get)then
					do iro=irange(1),irange(2)
						do ico=irange(3),irange(4)
							ie=ie+1
						...
						enddo !ico=irange(3),irange(4)   3577
					enddo !iro=irange(1),irange(2)   3576
 
				endif !if(get)   3565
			endif !if(isvector)   3551
			return
		else !if(nel2.eq.j_o(iout)%i(1))then
			write(6,*)'output list has wrong size ',j_o(iout)%i(1),' should be ',nel2
			j_err=.true.
		endif !if(nel2.eq.j_o(iout)%i(1))   3550
...
subroutine setelem(iob,io)  ! %%matrix put matrix element made from a(i,j)= -statements
	...
	if(j_otype(imat).eq.j_ipilist)then
		if(arg(1).eq.j_ivzero)then
			if(ivin.ne.j_ivzero)then
				...
				j_err=.true.;return
			endif !if(ivin.ne.j_ivzero)   3623
			j_o(imat)%i(1)=0
			...
			if(inde1.gt.j_o(imat)%i(4))then
				...
				j_err=.true.
			endif !if(inde1.gt.j_o(imat)%i(4))   3629
			j_o(imat)%i2(inde1)=j_v(ivin)
			j_o(imat)%i(1)=max(j_o(imat)%i(1),inde1)
		endif !if(arg(1).eq.j_ivzero)   3622
		return
	endif !if(j_otype(imat).eq.j_ipilist)   3621
	...
	if(j_otype(imat).ne.j_iplist)then !if(j_otype(imat).eq.j_iplist)then
		...
		j_err=.true. ;return
	end if !if(j_otype(imat).ne.j_iplist)   3639
	...
	if(narg1.eq.1)then
		...
		if(inde1.le.0.or.inde1.gt.j_o(imat)%i(3))then
			...
			j_err=.true.
			return
		endif !if(inde1.le.0.or.inde1.gt.j_o(imat)%i(3))   3649
		...
		endif !if(j_otype(imat).eq.j_iplist)   3655
		return
 
	endif !if(narg1.eq.1)   3645
	...
	if(j_otype(ivin).eq.j_ipmatrix)then
		...
		endif !if(j_otype(imat).eq.j_ipmatrix)   3690
	else !if(j_otype(ivin).eq.j_ipmatrix)then
		if(j_otype(imat).eq.j_ipmatrix)then
			j_o(imat)%d(inde1:inde2)=j_v(ivin)
		else !if(j_otype(imat).eq.j_ipmatrix)then
			j_v(j_o(imat)%i2(inde1:inde2))=j_v(ivin)
		endif !if(j_otype(imat).eq.j_ipmatrix)   3696
	endif !if(j_otype(ivin).eq.j_ipmatrix)   3689
	return
...
subroutine ilist(iob,io)
	...
	134			j_o(ivout)%i2(i)=iloc
		...
		if(narg.eq.0.and.noptarg.eq.0)then
			...
			j_err=.true.;return
		endif !if(narg.eq.0.and.noptarg.eq.0)   3809
		if(noptarg.gt.0)then
			...
			if(nextra.gt.0)then
				...
				j_err=.true.;return
			endif !if(nextra.gt.0)   3821
			nin=narg+nextra
			iout=j_deflist(ivout,' ',nres=nin)
 
			j_o(iout)%i2(1:narg)=j_v(arg)
			j_o(iout)%i(1)=narg
		endif !if(noptarg.gt.0)   3813
 
	endif !if(noptarg.eq.1)   3792
	!subroutine j_deflist(iv,name,list0,ivout) !allocates list object with size list0,
...
subroutine putlist(iob,io)
	...
	if(j_otype(arg(1)).ne.j_iplist.and.j_otype(arg(1)).ne.j_ipilist)then
		...
		j_err=.true.;return
	endif !if(j_otype(arg(1)).ne.j_iplist.and.j_otype(arg(1)).ne.j_ip   3854
	nle=j_o(arg(1))%i(1)
	narg1=narg-1
	nle2=nle+narg1
...
subroutine type(iob, io)  !
	...
	if(narg.eq.0)then
		...
		enddo !i=1,j_notypes   3959
		return
 
	endif !if(narg.eq.0)   3956
	...
	endif !if(j_linkoption(iob,io,j_mcontent).ge.0.and.j_otype(iarg)=   3968
...
subroutine delete_f(iob,io)
	...
	do i=1,narg
		...
		if(j_otype(iv).ne.j_ipchar)then
			...
			j_err=.true.
			cycle
		endif !if(j_otype(iv).ne.j_ipchar)   4031
...
subroutine exist_f(iob,io)
	...
	if(j_otype(iv).ne.j_ipchar)then
		call j_printname('object ',iv ,' is not a character object which could point to a file')
		j_err=.true.;return
	endif !if(j_otype(iv).ne.j_ipchar)   4065
	nu_ = j_nunits(j_nused+1)	! väliaikainen kanava tiedostolle
	if (j_iounit(iv)>0)then !j_o(iv)%i(4)>0) then! aukioleva tiedosto
		nu_ = j_iounit(iv)  !j_o(iv)%i(4)
		call j_closeunit(nu_)
	endif !if (j_iounit(iv)>0)   4070
	call j_getchar(iv,j_filename,le_)
	inquire(file = j_filename(1:le_) , exist=exis)
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	if(exis) then
	...
	if(narg.ge.2)then
		...
		if (j_iounit(iv)>0)then !j_o(iv)%i(4)>0) then! aukioleva tiedosto
			...
			j_v(iv2)=j_0
		endif !if (j_iounit(iv)>0)   4084
	endif !if(narg.ge.2)   4082
	return
...
subroutine exist_o(iob,io)
	...
	if(j_otype(iv).ne.j_ipchar)then
		...
		j_err=.true.;return
	endif !if(j_otype(iv).ne.j_ipchar)   4100
...
subroutine enddo(iob,io)
	...
	if(j_o(iob)%i(iod+8).gt.0)then
		...
		if(j_o(iob)%i(iod+6).gt.j_o(iob)%i(iod+7))then
			...
			io=io+2
			return
		endif !if(j_o(iob)%i(iod+6).gt.j_o(iob)%i(iod+7))   4174
	else !if(j_o(iob)%i(iod+8).gt.0)then
		if(j_o(iob)%i(iod+6).lt.j_o(iob)%i(iod+7))then
 
			io=io+2
			return
		endif !if(j_o(iob)%i(iod+6).lt.j_o(iob)%i(iod+7))   4180
	endif !if(j_o(iob)%i(iod+8).gt.0)   4172
...
subroutine if(iob,io)
	...
	if(j_otype(irg).ne.j_ipreal)then
		...
		j_err=.true.;return
	endif !if(j_otype(irg).ne.j_ipreal)   4201
...
subroutine which(iob,io)  !which()
	...
	do i=1,narg-imod,2
		...
		if(j_otype(irg).ne.j_ipreal)then
			...
			j_err=.true.;return
		endif !if(j_otype(irg).ne.j_ipreal)   4323
		if(j_v(irg).ne.0.)then
			irg2=j_o(iob)%i(io+2+i)
			goto 77
 
		end if !if(j_v(irg).ne.0.)   4328
	end do !i=1,narg-imod,2   4321
	if(imod.ne.0)then
		...
		j_err=.true.
	endif !if(imod.ne.0)   4334
 77  if(j_otype(irg2).eq.j_ipreal)then
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
		j_v(iout)=j_v(irg2)
	else
		call j_copy2(irg2,iout)
	endif !77  if(j_otype(irg2).eq.j_ipreal)   4341
	return
...
subroutine errexit(iob,io)
	...
	do j=1,narg
		...
		if(j_ipc(iv).ne.0)then
			...
			write(6,*)j_v(iv)
		end if !if(j_ipc(iv).ne.0)   4361
	end do !j=1,narg   4359
	j_err=.true.
	return
...
subroutine goto(iob,io)
	...
	if(igo.lt.0.or.igo.ge.narg)then
		...
		j_err=.true.
		return
	endif !if(igo.lt.0.or.igo.ge.narg)   4416
...
subroutine goto2(iob,io) !select from all labels
	...
	if(igo.lt.0.or.igo.gt.narg)then
		...
		j_err=.true.
		return
	endif !if(igo.lt.0.or.igo.gt.narg)   4448
...
subroutine POWER(iob,io,ioper)
	...
	if(j_otype(irg).eq.j_ipreal)then
		...
		if(ipower)then
			...
			endif !if(j_v(irg).lt.0.d0)   4499
		endif !if(ipower)   4496
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		if(j_o(iout)%i(3).ne.j_o(irg)%i(3))then
			...
			j_o(iout)%i(1)=nrow
			j_o(iout)%i(2)=ncol
		endif !if(j_o(iout)%i(3).ne.j_o(irg)%i(3))   4513
		if(ipower)then
			...
			if(minva.lt.0.d0)then
				...
				j_err=.true.;return
			endif !if(minva.lt.0.d0)   4523
			j_o(iout)%d=	j_o(irg)%d**pow
		endif !if(ipower)   4519
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   4494
...
subroutine HMULT(iob,io)
	...
	if(j_otype(irg1).ne.j_ipmatrix.or.j_otype(irg2).ne.j_ipmatrix)then
		...
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix.or.j_otype(irg2).ne.j_ipmat   4543
	nrow=j_o(irg1)%i(1)
	ncol=j_o(irg1)%i(2)
	nel=j_o(irg1)%i(3)
	if(nrow.ne.j_o(irg2)%i(1).or.ncol.ne.j_o(irg2)%i(2))then
		...
		j_err=.true.;return
	endif !if(nrow.ne.j_o(irg2)%i(1).or.ncol.ne.j_o(irg2)%i(2))   4550
...
subroutine HDIV(iob,io)
	...
	if(j_otype(irg1).ne.j_ipmatrix.or.j_otype(irg2).ne.j_ipmatrix)then
		write(6,*)' *.  requires matrix arguments'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix.or.j_otype(irg2).ne.j_ipmat   4567
	nrow=j_o(irg1)%i(1)
	ncol=j_o(irg1)%i(2)
	nel=j_o(irg1)%i(3)
	if(nrow.ne.j_o(irg2)%i(1).or.ncol.ne.j_o(irg2)%i(2))then
		write(6,*)'arguments of *.  are not compatible'
		j_err=.true.;return
	endif !if(nrow.ne.j_o(irg2)%i(1).or.ncol.ne.j_o(irg2)%i(2))   4574
	if(irg1.ne.iout.and.irg2.ne.iout)ivout=j_defmatrix(iout,' ',nrow,ncol,j_matreg)
	do i=1,nel
		...
		endif !if(j_o(irg2)%d(i).eq.j_0)   4580
		j_o(iout)%d(i)=j_o(irg1)%d(i)/j_o(irg2)%d(i)
	enddo !i=1,nel   4579
	return
...
subroutine MULT(iob,io) ! mata*matb
	...
	if(j_otype(irg(1)).eq.j_ipreal.and.j_otype(irg(2)).eq.j_ipreal)then
		...
		return
	endif !if(j_otype(irg(1)).eq.j_ipreal.and.j_otype(irg(2)).eq.j_ip   4607
	...
	if(nrow(1).eq.1.and.ncol(2).eq.1)then
		...
		return
	endif !if(nrow(1).eq.1.and.ncol(2).eq.1)   4680
	nel=nrow(1)*ncol(2)
	...
	endif !if(nrow(1).eq.ncol(1).and.iout.eq.irg(1).or.iout.eq.irg(2)   4688
	return
...
subroutine DIV(iob,io)
	...
	if(j_otype(irg2).ne.j_ipreal)then
		...
		j_err=.true.
		return
	endif !if(j_otype(irg2).ne.j_ipreal)   4722
	if(j_v(irg2).eq.j_0)then
		write(6,*)'*division by zero'
		j_err=.true.
		return
	endif !if(j_v(irg2).eq.j_0)   4727
...
subroutine BINOP(iob,io,ioper) ! mata+matb !
	...
	if(narg.eq.1)then
		if(j_otype(irg(1)).eq.j_ipmatrix)then
			...
			endif !if(ioper.ne.16.and.ioper.ne.17)   4778
			...
			if(nco.eq.j_18.or.j_linkoption(iob,io,j_many,clear=.true.).ge.0)then
				...
				if(ioper.eq.16)then
					...
					do i=1,nro
						...
						ibas=ibas+nco
					enddo !i=1,nro   4811
				endif !if(ioper.eq.16)   4800
			endif !if(nco.eq.j_18.or.j_linkoption(iob,io,j_many,clear=.true.)   4787
 
 
		endif !if(j_otype(irg(1)).eq.j_ipmatrix)   4776
		return
	endif !if(narg.eq.1)   4775
	!	narg=j_o(iob)%i(io+1)
	...
	if(j_otype(irg(1)).eq.j_ipreal.and.j_otype(irg(2)).eq.j_ipreal)then
		...
		if(j_v(irg(2)).eq.j_0)then
			...
			j_v(iout)=j_v(irg(1))/j_v(irg(2))
		endif !if(j_v(irg(2)).eq.j_0)   4881
		...
		return
	endif !if(j_otype(irg(1)).eq.j_ipreal.and.j_otype(irg(2)).eq.j_ip   4831
	...
	do i=1,2
		...
		if(nrow(i).eq.j_18.or.ncol(i).eq.j_18)then
			...
			matrix(i)=.true.
			imatrix=i
		endif !if(nrow(i).eq.j_18.or.ncol(i).eq.j_18)   4953
	...
	if(iscalar.gt.0.and.(ivector.gt.0.or.imatrix.gt.0))then
		...
		endif !if(j_otype(iout).eq.j_ipmatrix)   4978
		...
		do iel=1,nel
			...
			j_o(iout)%d(iel)=diout
		enddo !iel=1,nel   5021
		case (7) !select case (ioper)
		do iel=1,nel
			diout=j_0
			if(j_o(irgma)%d(iel).ge.j_v(ivs))diout=j_1
			j_o(iout)%d(iel)=diout
		enddo !iel=1,nel   5027
		case (8) !select case (ioper)
		do iel=1,nel
			diout=j_0
		...
		enddo !iel=1,nel   5060
		case (14 ) !select case (ioper)
		do iel=1,nel
			j_o(iout)%d(iel)=j_o(irgma)%d(iel)*j_v(ivs)
		enddo !iel=1,nel   5067
		case (15) !select case (ioper)
		do iel=1,nel
	...
	endif !if(iscalar.gt.0.and.(ivector.gt.0.or.imatrix.gt.0))   4974
	...
	endif !if(vector(1).and.vector(2))   5119
	...
	if(j_otype(iout).eq.j_iplist)then
		...
		listout=.false.
	endif !if(j_otype(iout).eq.j_iplist)   5127
	...
	if(matrix(1).and.matrix(2).or.vector(1).and.vector(2))then
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).eq.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).eq.j_o(irg(2))%d(iel))   5149
		enddo !iel=1,nel   5148
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ne.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ne.j_o(irg(2))%d(iel))   5158
		enddo !iel=1,nel   5157
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).le.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).le.j_o(irg(2))%d(iel))   5167
		enddo !iel=1,nel   5166
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).lt.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).lt.j_o(irg(2))%d(iel))   5176
		enddo !iel=1,nel   5175
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ge.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ge.j_o(irg(2))%d(iel))   5185
		enddo !iel=1,nel   5184
		...
		do iel=1,nel
			...
			if(j_o(irg(1))%d(iel).gt.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).gt.j_o(irg(2))%d(iel))   5195
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0)then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0   5204
		enddo !iel=1,nel   5203
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ne.j_0.or.j_o(irg(2))%d(iel).ne.j_0)then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ne.j_0.or.j_o(irg(2))%d(iel).ne.j_0)   5214
		enddo !iel=1,nel   5213
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0.or.j_o(irg(1))%d(iel).eq.j_0.and.&
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0   5225
		...
		do iel=1,nel
			if(.not.(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0.or.j_o(irg(1))%d(iel).eq.j_0.and.&
				...
				j_o(iout)%d(iel)=j_0
			endif !if(.not.(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).   5236
	...
	endif !if(matrix(1).and.matrix(2).or.vector(1).and.vector(2))   5141
	...
	endif !if(ivector.gt.0.and.imatrix.gt.0)   5289
	...
	do i=1,nrowout
		do j=1,ncolout
			...
			if(j_o(irg(2))%d(iel2).eq.j_0)then
				...
				diout=j_o(irg(1))%d(iel)/j_o(irg(2))%d(iel2)
			endif !if(j_o(irg(2))%d(iel2).eq.j_0)   5350
			case (16) !select case (ioper)
...
subroutine MINUS(iob,io) ! mata+matb !
	...
	if(j_otype(irg(1)).eq.j_ipreal)then
		...
		j_v(iout)=-j_v(irg(1))
		return
	endif !if(j_otype(irg(1)).eq.j_ipreal)   5414
	...
	if(j_otype(iout).eq.j_iplist)then
		...
		j_err=.true.
		return
	endif !if(j_otype(iout).eq.j_iplist)   5432
		...
		if(j_otype(irg1).eq.j_ipmatrix)then
			...
			do j=2,j_o(irg1)%i(3)
				if(j_o(irg1)%d(j).ne.val1)then
					...
					j_v(iout)=j_0
					return
				endif !if(j_o(irg1)%d(j).ne.val1)   5468
			enddo !j=2,j_o(irg1)%i(3)   5467
			j_v(iout)=j_1
			return
		...
		elseif(j_otype(irg1).eq.j_iplist)then !if(j_otype(irg1).eq.j_ipmatrix)then
			...
			do j=2,j_o(irg1)%i(1)
				if(j_v( j_o(irg1)%i2(j)).ne.val1)then
					j_v(iout)=j_0
					return
				endif !if(j_v( j_o(irg1)%i2(j)).ne.val1)   5480
			enddo !j=2,j_o(irg1)%i(1)   5479
			j_v(iout)=j_1
			return
			...
			j_err=.true.
			return
		endif !if(j_otype(irg1).eq.j_ipmatrix)   5465
	endif !if(narg.eq.1)   5464
	irg2=j_o(iob)%i(io+3)
	if(j_v(irg1).eq.j_v(irg2))then
		j_v(iout)=j_1
	else !if(j_v(irg1).eq.j_v(irg2))then
		j_v(iout)=j_0
	endif !if(j_v(irg1).eq.j_v(irg2))   5494
...
subroutine NE(iob,io)
	...
	if(j_v(irg1).ne.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).ne.j_v(irg2))   5509
...
subroutine LE(iob,io)
	...
	if(j_v(irg1).le.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).le.j_v(irg2))   5525
...
subroutine LT(iob,io)
	...
	if(j_v(irg1).lt.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).lt.j_v(irg2))   5539
...
subroutine GE(iob,io)
	...
	if(j_v(irg1).ge.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).ge.j_v(irg2))   5552
...
subroutine GT(iob,io)
	...
	if(j_v(irg1).gt.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).gt.j_v(irg2))   5565
...
subroutine NOT(iob,io)
	...
	if(j_v(irg1).eq.0.d0)then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).eq.0.d0)   5578
...
subroutine AND(iob,io)
	...
	if(j_v(irg1).ne.0.d0.and.j_v(irg2).ne.0)then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).ne.0.d0.and.j_v(irg2).ne.0)   5591
...
subroutine OR(iob,io)
	...
	if(j_v(irg1).ne.0.d0.or.j_v(irg2).ne.0.d0)then
		...
		j_v(iout)=0.d0
	endif !if(j_v(irg1).ne.0.d0.or.j_v(irg2).ne.0.d0)   5604
...
subroutine EQV(iob,io)
	...
	if((j_v(irg1).ne.0.d0.and.j_v(irg2).ne.0.d0).or.(j_v(irg1).eq.0.d0.and.j_v(irg2).eq.0.d0))then
		...
		j_v(iout)=0.d0
	endif !if((j_v(irg1).ne.0.d0.and.j_v(irg2).ne.0.d0).or.(j_v(irg1)   5617
...
subroutine NEQV(iob,io)
	...
	if((j_v(irg1).ne.0.d0.and.j_v(irg2).eq.0.d0).or.(j_v(irg1).eq.0.d0.and.j_v(irg2).ne.0.d0))then
		...
		j_v(iout)=0.d0
	endif !if((j_v(irg1).ne.0.d0.and.j_v(irg2).eq.0.d0).or.(j_v(irg1)   5630
...
subroutine min_(iob,io)
	...
	if(narg.eq.2)then
		if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal)then
			...
			return
		endif !if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal   5644
		if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipmatrix)then
			if(j_o(irg1)%i(1).ne.j_o(irg2)%i(1).or.j_o(irg1)%i(2).ne.j_o(irg2)%i(2))then
				...
				j_err=.true.;return
			endif !if(j_o(irg1)%i(1).ne.j_o(irg2)%i(1).or.j_o(irg1)%i(2).ne.j   5650
			...
			enddo !ii=1,j_o(irg2)%i(3)   5655
			return
		endif !if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipma   5649
		if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipreal)then
			ir=irg1
			irg1=irg2
			irg2=ir
		endif !if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipre   5660
		!now irg2 is matrix
		...
		if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipmatrix)then
			...
			if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)then
				...
				endif !if(j_o(irg2)%i(3).eq.j_o(iout)%i(3))   5670
			...
			endif !if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)   5669
			do ii=1,j_o(irg2)%i(3)
				j_o(iout)%d(ii)=dmin1(j_v(irg1),j_o(irg2)%d(ii))
			enddo !ii=1,j_o(irg2)%i(3)   5679
			return
 
		endif !if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipmatr   5667
 
		write(6,*)'illegal argument types ',j_otypes(j_otype(irg1)),j_otypes(j_otype(irg1))
		j_err=.true.;return
	endif !if(narg.eq.2)   5643
 
 
	if(j_otype(irg1).ne.j_ipmatrix)then
		write(6,*)'one argument must be MATRIX'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix)   5691
	nrows=j_o(irg1)%i(1)
	ncols=j_o(irg1)%i(2)
	if(j_linkoption(iob,io,j_many).ge.0)then
		ncols=1
		!!call j_clearoption(iob,io)
	endif !if(j_linkoption(iob,io,j_many).ge.0)   5697
	if(nrows.eq.1.or.ncols.eq.1)then
		...
		do i=1,nrows
			...
			enddo !j=1,ncols   5711
			ibas=ibas+ncols
		enddo !i=1,nrows   5710
 
	endif !if(nrows.eq.1.or.ncols.eq.1)   5701
	return
...
subroutine max_(iob,io)
	...
	if(narg.eq.2)then
		if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal)then
			...
			return
		endif !if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal   5729
		if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipmatrix)then
			if(j_o(irg1)%i(1).ne.j_o(irg2)%i(1).or.j_o(irg1)%i(2).ne.j_o(irg2)%i(2))then
				write(6,*)'incompatibe dimensions ',j_o(irg1)%i(1:2),j_o(irg2)%i(1:2)
				j_err=.true.;return
			endif !if(j_o(irg1)%i(1).ne.j_o(irg2)%i(1).or.j_o(irg1)%i(2).ne.j   5735
			if(irg1.ne.iout.and.irg2.ne.iout)ivout=j_defmatrix(iout,' ',j_o(irg2)%i(1),j_o(irg2)%i(2),j_matreg)
			do ii=1,j_o(irg2)%i(3)
				j_o(iout)%d(ii)=dmax1(j_o(irg1)%d(ii),j_o(irg2)%d(ii))
			enddo !ii=1,j_o(irg2)%i(3)   5740
			return
		endif !if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipma   5734
		if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipreal)then
			ir=irg1
			irg1=irg2
			irg2=ir
		endif !if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipre   5745
		!now irg2 is matrix
		!	write(6,*)'<66',irg1,irg2,j_otype(irg1),j_otype(irg2),j_ipmatrix
		if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipmatrix)then
			...
			if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)then
				if(j_o(irg2)%i(3).eq.j_o(iout)%i(3))then
					...
					ivout=j_defmatrix(iout,' ',j_o(irg2)%i(1),j_o(irg2)%i(2),j_matreg)
				endif !if(j_o(irg2)%i(3).eq.j_o(iout)%i(3))   5755
			else !if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)then
				ivout=j_defmatrix(iout,' ',j_o(irg2)%i(1),j_o(irg2)%i(2),j_matreg) !could do with one def
			endif !if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)   5754
			do ii=1,j_o(irg2)%i(3)
				j_o(iout)%d(ii)=dmax1(j_v(irg1),j_o(irg2)%d(ii))
			enddo !ii=1,j_o(irg2)%i(3)   5764
			return
 
		endif !if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipmatr   5752
 
		write(6,*)'illegal argument types ',j_otypes(j_otype(irg1)),j_otypes(j_otype(irg1))
		j_err=.true.;return
	endif !if(narg.eq.2)   5728
 
	if(j_otype(irg1).ne.j_ipmatrix)then
		write(6,*)'one arguement must be MATRIX'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix)   5775
	nrows=j_o(irg1)%i(1)
	ncols=j_o(irg1)%i(2)
	if(j_linkoption(iob,io,j_many).ge.0)then
		ncols=j_18
		!!call j_clearoption(iob,io)
	endif !if(j_linkoption(iob,io,j_many).ge.0)   5781
	if(nrows.eq.1.or.ncols.eq.1)then
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
		...
		do i=1,nrows
			...
			enddo !j=1,ncols   5794
			ibas=ibas+ncols
		enddo !i=1,nrows   5793
 
	endif !if(nrows.eq.1.or.ncols.eq.1)   5785
	return
		...
		do i=1,ncols
			...
			do j=1,nrows
				...
				if(j_o(irg1)%d(ibas+i).lt.minva)then
					...
					loc=j
				endif !if(j_o(irg1)%d(ibas+i).lt.minva)   5846
				ibas=ibas+ncols
			enddo !j=1,nrows   5844
			j_o(iout)%d(i)=loc
		enddo !i=1,ncols   5842
 
	endif !if(nrows.eq.1.or.ncols.eq.1)   5832
	return
...
subroutine maxloc_(iob,io)
	...
	if(j_otype(irg1).ne.j_ipmatrix)then
		write(6,*)'argument must be MATRIX'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix)   5879
	nrows=j_nrows(irg1)  !j_o(irg1)%i(1)
	ncols=j_ncols(irg1) !j_o(irg1)%i(2)
	if(j_linkoption(iob,io,j_many).ge.0)then
		ncols=j_18
		!!call j_clearoption(iob,io)
	endif !if(j_linkoption(iob,io,j_many).ge.0)   5885
	if(nrows.eq.1.or.ncols.eq.1)then
		...
		do i=1,ncols
			...
			do j=1,nrows
				if(j_o(irg1)%d(ibas+i).gt.maxva)then
					...
					loc=j
				endif !if(j_o(irg1)%d(ibas+i).gt.maxva)   5903
				ibas=ibas+ncols
			enddo !j=1,nrows   5902
			j_o(iout)%d(i)=loc
		enddo !i=1,ncols   5900
 
	endif !if(nrows.eq.1.or.ncols.eq.1)   5889
	return
...
subroutine cumsum(iob,io)
	...
	if(j_otype(irg1).ne.j_ipmatrix)then
		write(6,*)'argument must be MATRIX'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix)   5941
	nrows=j_o(irg1)%i(1)
	ncols=j_o(irg1)%i(2)
	nel=j_o(irg1)%i(3)
	...
	if(nrows.eq.1.or.ncols.eq.1)then
		...
		enddo !i=2,nrows   5964
	endif !if(nrows.eq.1.or.ncols.eq.1)   5952
	return
...
subroutine corrmatrix(iob,io)
	...
	do i=1,ndim
		...
		endif !if(j_o(irg1)%d(ibas+i).lt.j_0)   6008
	...
	do i=1,ndim
		...
		do j=1,ndim
			...
			if(i.ne.j)then
				...
				endif !if(j_dapu.ne.j_0)   6025
 
			endif !if(i.ne.j)   6023
	...
	if(j_linkoption(iob,io,j_msd,clear=.true.).lt.0)then
		...
		enddo !i=1,ndim   6041
	endif !if(j_linkoption(iob,io,j_msd,clear=.true.).lt.0)   6039
		...
		if(iout.ne.irg)then
			if(j_otype(iout).eq.j_ipmatrix)then
				if(j_o(iout)%i(3).eq.j_o(irg)%i(3))then
					...
					goto 80
				endif !if(j_o(iout)%i(3).eq.j_o(irg)%i(3))   6075
			endif !if(j_otype(iout).eq.j_ipmatrix)   6074
			if(j_otype(iout).ne.j_ipreal)call j_del(iout)
			ivout=j_defmatrix(iout,' ',j_o(irg)%i(1),j_o(irg)%i(2),j_matreg) !matreg?
		endif !if(iout.ne.irg)   6073
80		continue
		j_o(iout)%d=dabs(j_o(irg)%d)
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6068
...
subroutine nint_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6110
...
subroutine int_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6131
...
subroutine ceiling_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6152
...
subroutine floor_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6173
...
6 	if(j_v(irg).lt.j_0)then
		...
		endif !6 	if(j_v(irg).lt.j_0)   6218
		return
...
7 	if(j_v(irg).eq.j_0)then
		...
		elseif(j_v(irg).gt.j_0)Then !if(j_v(irg).eq.j_0)then
			...
			j_v(iout)=-sqrt(-j_v(irg))
		endif !7 	if(j_v(irg).eq.j_0)   6227
		return
		!
8 	if(j_v(irg).lt.0.d0)then
		...
		endif !8 	if(j_v(irg).lt.0.d0)   6236
		return
		!abs
9 	if(j_v(irg).lt.0.d0)then
		...
		endif !9 	if(j_v(irg).lt.0.d0)   6244
		return
10	if(j_v(irg).gt.88.d0)then
		...
		endif !10	if(j_v(irg).gt.88.d0)   6251
		return
		!abs
...
17		if(dabs(j_v(irg)).lt.d-15)then
			...
			j_err=.true.
		endif !17		if(dabs(j_v(irg)).lt.d-15)   6278
...
18	if(dabs(j_v(irg)).lt.d-15)then
			write(6,*)'too close to zero ',j_v(irg)
			j_err=.true.
		endif !18	if(dabs(j_v(irg)).lt.d-15)   6285
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
...
25	if(abs(j_v(irg)).gt.0.d-15)then  !cotan
			...
			j_err=.true.
		endif !25	if(abs(j_v(irg)).gt.0.d-15)   6307
...
26		if(abs(j_v(irg)).gt.0.d-15)then
			...
			write(6,*)'too small argument ',j_v(irg)
			j_err=.true.
		endif !26		if(abs(j_v(irg)).gt.0.d-15)   6316
		return
...
31	j_v(iout)=dabs(j_v(irg))
		...
		return
	endif !if(j_otype(irg).eq.j_ipreal)   6196
	if(j_otype(irg).ne.j_ipmatrix)then
		write(6,*)'argument is not MATRIX or REAL'
		j_err=.true.
 
	endif !if(j_otype(irg).ne.j_ipmatrix)   6343
	...
	endif !if(j_o(iout)%i(3).ne.j_o(irg)%i(3))   6350
...
97 		minva=minval(j_o(irg)%d)
	do i=1,j_o(irg)%i(3)
		...
		elseif(j_o(irg)%d(i).gt.j_0)Then !if(j_o(irg)%d(i).eq.j_0)then
			...
			j_o(iout)%d(i)=-sqrt(-j_o(irg)%d(i))
		endif !if(j_o(irg)%d(i).eq.j_0)   6386
	end do !i=1,j_o(irg)%i(3)   6385
	return
	!
...
subroutine log_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		if(minva.lt.0.d0)then
			...
			allocate( j_o(iout)%d(1:j_o(iout)%i(3)))
			j_o(iout)%d=dlog(j_o(irg)%d)
		endif !if(minva.lt.0.d0)   6597
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6587
...
subroutine log10_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		if(minva.lt.0.d0)then
			...
			allocate( j_o(iout)%d(1:j_o(iout)%i(3)))
			j_o(iout)%d=dlog10(j_o(irg)%d)
		endif !if(minva.lt.0.d0)   6631
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6621
...
subroutine exp_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		if(j_v(irg).gt.88.d0)then
			...
			if(j_otype(iout).ne.j_ipreal)call j_del(iout)
			j_v(iout)=dexp(j_v(irg))
		endif !if(j_v(irg).gt.88.d0)   6656
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
...
subroutine sin_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6689
...
subroutine sind_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6711
...
subroutine cos_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6733
...
subroutine cosd_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6755
...
subroutine tand_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6799
...
subroutine cotan_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		if(dabs(j_v(irg)).lt.d-15)then
			write(6,*)'too close to zero ',j_v(irg)
			j_err=.true.
		endif !if(dabs(j_v(irg)).lt.d-15)   6822
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
		j_v(iout)=dtan(1.d0/j_v(irg))
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		call j_del(iout)
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6821
...
subroutine cotand_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		if(dabs(j_v(irg)).lt.d-15)then
			write(6,*)'too close to zero ',j_v(irg)
			j_err=.true.
		endif !if(dabs(j_v(irg)).lt.d-15)   6848
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
		j_v(iout)=dtan(1.d0/(j_deg*j_v(irg)))
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		call j_del(iout)
...
subroutine asin_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6873
...
subroutine asind_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6895
...
subroutine acos_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6917
...
subroutine acosd_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6939
...
subroutine atan_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6961
...
subroutine atand_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6983
...
subroutine acotan_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		if(abs(j_v(irg)).gt.0.d-15)then
			...
			write(6,*)'too small argument ',j_v(irg)
			j_err=.true.
		endif !if(abs(j_v(irg)).gt.0.d-15)   7007
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		if(minva.lt.1.d-15)then
			...
			j_err=.true.
			return
		endif !if(minva.lt.1.d-15)   7024
		j_o(iout)%d=datan(1.d0/j_o(irg)%d)
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   7006
...
subroutine acotand_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		...
		if(abs(j_v(irg)).gt.0.d-15)then
			...
			write(6,*)'too small argument ',j_v(irg)
			j_err=.true.
		endif !if(abs(j_v(irg)).gt.0.d-15)   7044
		j_v(iout)=dacos(j_v(irg))*j_todeg
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		call j_del(iout)
		j_otype(iout)=j_ipmatrix
		...
		if(minva.lt.1.d-15)then
			...
			j_err=.true.
			return
		endif !if(minva.lt.1.d-15)   7061
		j_o(iout)%d=datan(1.d0/j_o(irg)%d)*j_todeg
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   7042
...
subroutine sinh_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   7079
...
subroutine cosh_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   7101
...
subroutine tanh_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   7123
...
subroutine index_v(iob,io)
	...
	if(j_otype(arg(2)).ne.j_iplist)then
		...
		j_err=.true.
		return
	endif !if(j_otype(arg(2)).ne.j_iplist)   7286
	inde=j_v(arg(1))
	...
	endif !if(inde.lt.1.or.inde.gt.j_o(arg(2))%i(1))   7292
	return
...
subroutine find(iob,io)
	...
	if(noprows.ge.0.or.nopcols.ge.0)then
		...
		j_err=.true.
		return
	endif !if(noprows.ge.0.or.nopcols.ge.0)   7364
	!write(6,*)'infind'
	...
	do i=1,nel
		...
		if(filter.ne.j_0)then
			...
			elseif(isany)then !if(isexpand)then
				...
				j_v(ivout)=i
				return
			endif !if(isexpand)   7414
		endif !if(filter.ne.j_0)   7413
	enddo !i=1,nel   7403
	if(isexpand)return
	if(isany)then
		if(nfound.gt.0)then
...
subroutine matrixstat(iob,io,ifunc)
	...
	endif !if(j_linkoption(iob,io,j_many).ge.0)   7503
	!!call j_clearoption(iob,io)
		...
		if(ncols.eq.1.or.nrows.eq.1)then
			if(j_o(iwe)%i(3).ne.nel)then
				...
				write(6,*)'weight ',j_oname2(1:j_loname2), 'does not have same size as ',&
					j_oname(1:j_loname)
				j_err=.true. ; return
			endif !if(j_o(iwe)%i(3).ne.nel)   7517
			if(j_otype(ivout).ne.j_ipreal)call j_del(ivout)
			!dcoef=j_1/nel
			!dcoef1=j_1/(nel-j_1)
			...
			endif !if(ifunc.eq.1)   7541
			return
 
		endif !if(ncols.eq.1.or.nrows.eq.1)   7516
		if(j_o(iwe)%i(1).ne.1.and.j_o(iwe)%i(2).ne.1)then
			call j_getname(iwe)
			write(6,*)'weight ',j_oname(1:j_loname),' is not vector but ',j_o(iwe)%i(1:2)
			j_err=.true.
			return
		endif !if(j_o(iwe)%i(1).ne.1.and.j_o(iwe)%i(2).ne.1)   7550
	...
	if(ncols.eq.1.or.nrows.eq.1)then
		...
		endif !if(ifunc.eq.1)   7609
		return
 
	endif !if(ncols.eq.1.or.nrows.eq.1)   7588
...
subroutine class(iob,io)
	...
	if(xmax.le.xmin)then
		...
		j_err=.true.
		return
	endif !if(xmax.le.xmin)   7683
	if(j_v(arg(1)).lt.xmin.or.j_v(arg(1)).gt.xmax)then
		j_v(ivout)=0.d0
		return
	endif !if(j_v(arg(1)).lt.xmin.or.j_v(arg(1)).gt.xmax)   7688
	...
	if(ndx.eq.1)then
		...
		if(dx.le.0.d0)then
			...
			j_err=.true.
			return
		endif !if(dx.le.0.d0)   7697
	endif !if(ndx.eq.1)   7695
	...
	if(nc.gt.0.and.ndx.gt.0)then
		...
		nc=0
	endif !if(nc.gt.0.and.ndx.gt.0)   7707
	!!call j_clearoption(iob,io) ! subroutine
...
subroutine newdata(iob,io)
	...
	endif !if(isdelete.and.narg.le.2)   7797
	! enddo !i=1,narg   7175
	...
	if(noptarg.gt.0)then
		...
		if(noutv.le.0)then
			...
			j_err=.true.;return
		endif !if(noutv.le.0)   7832
		!	write(6,*)'<4554',noutv,ivoul,j_o(ivoul)%i2(1:noutv)
	else !if(noptarg.gt.0)then
		maketran=.false.
		noutv=0
	end if !if(noptarg.gt.0)   7827
	...
	if(isappend)then
		...
		do i=1,narg
			if(j_otype(arg(i)).eq.j_ipmatrix)then
				...
				if(ncols.ne.j_o(ivmat)%i(2))then
					...
					j_err=.true.;return
				endif !if(ncols.ne.j_o(ivmat)%i(2))   7915
			elseif(j_otype(arg(i)).eq.j_ipdata)then !if(j_otype(arg(i)).eq.j_ipmatrix)then
				...
				if(i.eq.1)then
					...
					if(j_otype(arg(1)).ne.j_ipdata)then
						...
						j_err=.true.;return
					endif !if(j_otype(arg(1)).ne.j_ipdata)   7928
					if(j_o(ivmat)%i(2).ne.ncols)then
						...
						j_err=.true.;return
					endif !if(j_o(ivmat)%i(2).ne.ncols)   7935
					...
					if(.not.all(j_o(ivkeep1)%i2(1:ncols).eq.j_o(ivkeep)%i2(1:ncols)))then
						...
						j_err=.true.;return
					endif !if(.not.all(j_o(ivkeep1)%i2(1:ncols).eq.j_o(ivkeep)%i2(1:n   7941
					arg2(i)=ivmat
				endif !if(i.eq.1)   7924
				...
				j_err=.true.
				return
			endif !if(j_otype(arg(i)).eq.j_ipmatrix)   7895
		...
		endif !if(j_reject.or.j_filter)   7975
		...
		if(nextra.gt.0)then
			do ij=1,nextra
				...
				endif !if(iper.gt.0)   8016
			enddo !ij=1,nextra   8014
		endif !if(nextra.gt.0)   8013
		if(j_o(ivkeep)%i(1).lt.nvartot)then
			...
			j_err=.true.; return
		endif !if(j_o(ivkeep)%i(1).lt.nvartot)   8023
		...
		if(.not.maketran.and.nextra.le.0)then
			...
			do i=1,narg
				...
				do k=1,j_o(arg2(i))%i(1)
					...
					endif !if(maketran)   8117
 
 
					ibas2=ibas2+ncols
					ibas=ibas+nkeep
				enddo !k=1,j_o(arg2(i))%i(1)   8116
 
			enddo !i=1,narg   8112
 
 
 
		endif !if(.not.maketran.and.nextra.le.0)   8042
		...
		do i=1,narg
			...
			endif !if(j_otype(arg(i)).eq.j_ipmatrix.or.j_otype(arg(i)).eq.j_i   8185
			ncols=j_o(arg2(i))%i(2)
			if(i.eq.1)then
				nob=j_o(arg2(i))%i(1)
				!write(6,*)'nob ',nob
		...
		if(nmatrix.gt.0)then
			...
			if(nread.ne.nvarmatrix)then
				...
				j_err=.true.;return
			endif !if(nread.ne.nvarmatrix)   8230
 
		endif !if(nmatrix.gt.0)   8227
		...
		if(j_o(ivkeep)%i(1).lt.nvar)then
			...
			write(6,*)'There are duplicated variable names'
			j_err=.true.; return
		endif !if(j_o(ivkeep)%i(1).lt.nvar)   8269
		...
		if(nextra.gt.0)then
			do ij=1,nextra
				...
				if(iper.gt.0)then
					...
					iper=j_putlistobject(ivkeep,single=extra(ij))
				endif !if(iper.gt.0)   8282
			enddo !ij=1,nextra   8280
		endif !if(nextra.gt.0)   8279
		...
		do j=1,nob
			...
			do i=1,narg
				...
				endif !if(j_otype(arg2(i)).eq.j_ipmatrix)   8319
	...
	if(nread.gt.0)then
		do i=1,nread
			...
			if(itype.ne.j_ipreal)then
				...
				call j_del(readv(i))
			endif !if(itype.ne.j_ipreal)   8435
		enddo !i=1,nread   8433
	endif !if(nread.gt.0)   8432
...
subroutine exceldata(iob,io)
	...
	if(isnobs)then
		if(noptarg.ne.nin_)then
			...
			j_err=.true.;return
		endif !if(noptarg.ne.nin_)   8535
 
	endif !if(isnobs)   8534
 
	if(allocated(nobs))then
		if(size(nobs).lt.nin_)deallocate(nobs,nu)
	endif !if(allocated(nobs))   8542
	...
	do ifi=1,nin_
		...
		if(ifi.eq.1)then
			...
			if(j_inpr(1:linfi).ne.j_cline(1:linfi))then
				...
				j_err=.true.
				return
			endif !if(j_inpr(1:linfi).ne.j_cline(1:linfi))   8571
		endif !if(ifi.eq.1)   8564
		nhead(ifi)=1
		nlin=1
			...
			do ic=3,lin+1
				...
				if(j_inpr(ic:ic).eq.';'.or.ic.eq.lin+1)then
					...
					if(j_inpr(ic0+1:ic0+1).ne.'!')then
						...
						endif !if(j_inpr(ic-1:ic-1).eq.'?')   8637
						...
						keepv(nkeep)=iread
					endif !if(j_inpr(ic0+1:ic0+1).ne.'!')   8635
					ic0=ic
					!	if(ic.gt.lin-30)write(6,*)ic,ic0
				endif !if(j_inpr(ic:ic).eq.';'.or.ic.eq.lin+1)   8632
			enddo !ic=3,lin+1   8626
			nkeep=nkeep+1
	...
	do ifi=1,nin_
		...
		do i=1,100000
			...
			if(j_inpr(1:linfi).eq.j_cline(1:linfi))then
				...
				!			nlinv=nlin
				cycle
			endif !if(j_inpr(1:linfi).eq.j_cline(1:linfi))   8751
			iobsw=iobsw+1
			npp0=0
			nrea=0
			ic0=0
...
subroutine classvector(iob,io)
	...
	if(narg.le.0.and.ilast.lt.0.and.ifirst.lt.0)then
		...
		j_err=.true.
		return
	endif !if(narg.le.0.and.ilast.lt.0.and.ifirst.lt.0)   9087
	if(iw.ge.0.and.nexpand.lt.0)then
		write(6,*)'option ->w requires expand->'
		j_err=.true.;return
 
	endif !if(iw.ge.0.and.nexpand.lt.0)   9092
	...
	nlo=max(narg,1)
		...
		if(narg.gt.0)then
			...
			endif !if(iw.ge.0)   9114
		endif !if(narg.gt.0)   9104
		classv=-1.79d37
		ifi=1
...
!	write(6,*)'i',i
			...
			if(j_iobs.le.j_dnobs)then
				...
				!			if(j_rejected)cycle
			endif !if(j_iobs.le.j_dnobs)   9133
			...
			if((j_v(ivclass).ne.classv.and.j_iobs.gt.1).or.j_iobs.eq.j_dnobs+1)then
				...
				cmin=1.d60
				cmax=-1.d60
			endif !if((j_v(ivclass).ne.classv.and.j_iobs.gt.1).or.j_iobs.eq.j   9145
			classv=j_v(ivclass)
			if(j_rejected)then
				nrej=nrej+1
				cycle ob
 
			endif !if(j_rejected)   9181
			n=n+1
			if(narg.gt.0)then
			...
			endif !if(narg.gt.0)   9187
		enddo ob !	do j_iobs=j_dfrom,j_duntil+1   9129
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					do j=ifi,last0(ii)
						...
						j_o(ive)%d(ij)=ifi
						!		call j_putmatrix(ive,i,1,last0(i-1)+1.d0)
					enddo !j=ifi,last0(ii)   9225
					ifi=last0(i)+1
				enddo !i=1,nclass   9224
				call j_printname('   done ',ive,' ')
			endif !if(nexpand.lt.0)   9200
		endif !if(ifirst.ge.0.and.ia.eq.1)   9199
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					do j=ifi,last0(ii)
						...
						!		call j_putmatrix(ive,i,1,dble(last0(i)) )
					enddo !j=ifi,last0(ii)   9261
					ifi=last0(i)+1
				enddo !i=1,nclass   9260
				!	write(6,*)'nclass',nclass,last0(1:10)
				call j_printname('   done ',ive,' ')
			endif !if(nexpand.lt.0)   9241
		endif !if(inobsw0.ge.0.and.ia.eq.1)   9240
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					do j=ifi,last0(ii)
						...
						!		call j_putmatrix(ive,i,1,dble(last0(i)) )
					enddo !j=ifi,last0(ii)   9287
					ifi=last0(i)+1
				enddo !i=1,nclass   9286
 
				call j_printname('   done ',ive,' ')
			endif !if(nexpand.lt.0)   9274
		endif !if(ilast.ge.0.and.ia.eq.1)   9273
		...
		if(iobsw.ge.0.and.ia.eq.1)then
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   9309
					ifi=last0(i)+1
				enddo !i=1,nclass   9308
 
				call j_printname('   done ',ive,' ')
			endif !if(nexpand.lt.0)   9300
 
 
		endif !if(iobsw.ge.0.and.ia.eq.1)   9299
		...
		if(imean0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   9348
					ifi=last0(i)+1
				enddo !i=1,nclass   9347
			endif !if(nexpand.lt.0)   9328
			call j_printname('   done ',ive,' ')
		endif !if(imean0.ge.0)   9326
		...
		if(iw.ge.0)then
			...
			do i=1,nclass
				...
				enddo !j=ifi,last0(i)   9364
				ifi=last0(i)+1
			enddo !i=1,nclass   9363
			call j_printname('   done ',ivew,' ')
		endif !if(iw.ge.0)   9360
		...
		if(ivar0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   9398
					ifi=last0(i)+1
				enddo !i=1,nclass   9397
			endif !if(nexpand.lt.0)   9379
			call j_printname('   done ',ive,' ')
		endif !if(ivar0.ge.0)   9377
		...
		if(isd0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   9430
					ifi=last0(i)+1
				enddo !i=1,nclass   9429
			endif !if(nexpand.lt.0)   9411
			call j_printname('   done ',ive,' ')
		endif !if(isd0.ge.0)   9409
		...
		if(isum0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   9462
					ifi=last0(i)+1
				enddo !i=1,nclass   9461
			endif !if(nexpand.lt.0)   9443
			call j_printname('   done ',ive,' ')
		endif !if(isum0.ge.0)   9441
		...
		if(imin0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   9494
					ifi=last0(i)+1
				enddo !i=1,nclass   9493
			endif !if(nexpand.lt.0)   9475
			call j_printname('   done ',ive,' ')
		endif !if(imin0.ge.0)   9473
		...
		if(imax0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   9526
					ifi=last0(i)+1
				enddo !i=1,nclass   9525
			endif !if(nexpand.lt.0)   9507
			call j_printname('   done ',ive,' ')
		endif !if(imax0.ge.0)   9505
 
 
 
 
	enddo vloop !op: do ia=1,nlo   9102
	!	write(6,*)'tas'
...
subroutine varcomp(iob,io)
	...
	if(narg.le.0.and..not.isfunc)then
		...
		j_err=.true.
	endif !if(narg.le.0.and..not.isfunc)   9582
	if(isfunc)narg=1
	...
	if(ivout.eq.j_ivresult.and.iscov)then
		...
		j_err=.true.
	endif !if(ivout.eq.j_ivresult.and.iscov)   9595
	...
	if(iscov.and.isfunc)then
		...
		j_err=.true.;return
	endif !if(iscov.and.isfunc)   9607
	...
	! write(6,*)'from',j_from,j_duntil
		...
		if(isfunc)then
			...
			endif !if(abs(j_dapu).ge.1.7d19)   9632
		else
			do j=1,narg
				...
				endif !if(abs(j_v(arg(j))).ge.1.7d19)   9639
			enddo !j=1,narg   9638
		endif !if(isfunc)   9630
		ntot=ntot+1
		...
		do j=1,narg
			if(iscov)then
				do j2=1,j-1
					...
					jc=jc+1
				end do !j2=1,j-1   9652
			end if !if(iscov)   9651
			if(isfunc)then
				s(jc)=j_dapu
			else
				s(jc)=j_v(arg(j))
			endif !if(isfunc)   9657
			jc=jc+1
		end do !j=1,narg   9650
 
		call j_dbw(mets,s,lkm,iobsw,nmets,xs,xss,sb,sumwt,xmi,xma,xs2,s2)
 
	enddo obloop !oop:	do j_iobs=j_dfrom,j_duntil   9621
	...
	do j=1,narg
		if(iscov)then
			do j2=1,j-1
				...
				endif !if(icorr.ge.0)   9753
				jc=jc+1
			end do !j2=1,j-1   9744
		end if !if(iscov)   9743
		if(icov.ge.0)then
...
subroutine dos(iob,io)  !do loops in input programming  ;do()
	...
	if(j_ndo.ge.j_mxndo)then
		...
		j_ndo=0
		return
	endif !if(j_ndo.ge.j_mxndo)   9942
	if(j_ndo.eq.j_ndoinc(j_ninc))then
		if(j_reacom(j_ninc))then
			...
			endif !if(.not.j_savcom(j_ninc))   9952
			icu=istart
			...
			endif !if(j_inp(1:j_linp).eq.'endsimu')   9977
			call j_puttext(j_ivbuf(j_ninc), j_inp(1:j_linp))
			...
			if(j_inp(j_ialb:j_ialb+2).eq.';do'.and.j_inp(ialb2:ialb2).eq.'(')then
				ndotemp=ndotemp+1
			endif !if(j_inp(j_ialb:j_ialb+2).eq.';do'.and.j_inp(ialb2:ialb2).   9984
			if(j_inp(j_ialb:j_ialb+5).eq.';enddo'.or.j_inp(j_ialb:j_ialb+6).eq.';end do')then
				ndotemp=ndotemp-1
			endif !if(j_inp(j_ialb:j_ialb+5).eq.';enddo'.or.j_inp(j_ialb:j_ia   9987
...
subroutine gotos_(iob,io) ! ;goto  in input programming
	...
	if(j_ninc.lt.2.or.j_ipc(iva).eq.0)then
		...
		return
	end if !if(j_ninc.lt.2.or.j_ipc(iva).eq.0)  10066
 
	if(j_ndo.gt.0)then
		write(6,*)'**open ;do loops closed'
		j_ndo=0
	endif !if(j_ndo.gt.0)  10071
	...
	if(ivb.ne.0.and.j_otype(max(ivb,1)).eq.j_iptext)then !search for labels
			...
			end do !j=0,iva2-iva1  10083
			j_icurl(j_ninc)=iil
			j_reacom(j_ninc)=.false.
			return !adrloop: do i=1,j_o(ivb)%i2(0)
 
		enddo adrloop !loop: do i=1,j_o(ivb)%i2(0)  10079
		sav=.true.
	else !if(ivb.ne.0.and.j_otype(max(ivb,1)).eq.j_iptext)then
		!not saving
		sav=.false.
	end if !if(ivb.ne.0.and.j_otype(max(ivb,1)).eq.j_iptext)  10078
	!like in incl
...
subroutine rannegbin(iob,io)
	...
	elseif(pp_.eq.0.)then
		...
		enddo !i=1,j_o(iout)%i(3)  10206
	endif !if(j_otype(iout).eq.j_ipreal)  10191
...
subroutine elementsum(iob,io)   ! %%matrix
	...
	if(j_otype(iarg).ne.j_ipmatrix)then
		...
		j_err=.true. ;return
	endif !if(j_otype(iarg).ne.j_ipmatrix)  10229
	...
	if(ivrow_.gt.0)then
		...
		if(irow.le.0.or.irow.gt.j_o(iarg)%i(1))then
			...
			j_err=.true.  ;return
		endif !if(irow.le.0.or.irow.gt.j_o(iarg)%i(1))  10236
	endif !if(ivrow_.gt.0)  10234
	...
	if(ivcolumn.gt.0)then
		...
		if(icolumn.le.0.or.icolumn.gt.j_o(iarg)%i(2))then
			...
			j_err=.true.  ;return
		endif !if(icolumn.le.0.or.icolumn.gt.j_o(iarg)%i(2))  10244
	endif !if(ivcolumn.gt.0)  10242
	...
	if(ivrow_.gt.0)then
		...
		endif !if(las.gt.j_o(iarg)%i(2).or.las.le.0)  10272
	...
	elseif(ivcolumn.gt.0)then !if(ivrow_.gt.0)then
		...
		end if !if(las.le.0.or.las.gt.j_o(iarg)%i(1))  10284
		su=0
		ii=icolumn
		do i=1,las
			su=su+j_o(iarg)%d(ii)
		...
		if(las.le.0.or.las.gt.j_o(iarg)%i(3))then
			...
			j_err=.true. ;return
		end if !if(las.le.0.or.las.gt.j_o(iarg)%i(3))  10298
		j_v(iout)=sum(j_o(iarg)%d(1:las))
	endif !if(ivrow_.gt.0)  10271
	...
	if(ivrow_.gt.0)then
		if(las.gt.j_o(iarg)%i(2).or.ifi.gt.las.or.ifi.le.0)then
			...
			j_err=.true.
			return
		endif !if(las.gt.j_o(iarg)%i(2).or.ifi.gt.las.or.ifi.le.0)  10309
		ifi=(irow-1)*j_o(iarg)%i(2)
		j_v(iout)=sum(j_o(iarg)%d(ifi+1:ifi+las))
	...
	elseif(ivcolumn.gt.0)then !if(ivrow_.gt.0)then
		if(las.le.0.or.las.gt.j_o(iarg)%i(2))then
			...
			j_err=.true. ;return
		end if !if(las.le.0.or.las.gt.j_o(iarg)%i(2))  10320
		su=0
		ii=icolumn
		do i=1,las
			su=su+j_o(iarg)%d(ii)
		...
		if(ifi.le.0.or.ifi.gt.j_o(iarg)%i(3).or.las.le.0.or.las.gt.j_o(iarg)%i(3))then
			...
			j_err=.true. ;return
		end if !if(ifi.le.0.or.ifi.gt.j_o(iarg)%i(3).or.las.le.0.or.las.gt  10333
		j_v(iout)=sum(j_o(iarg)%d(ifi:las))
	endif !if(ivrow_.gt.0)  10308
...
subroutine dotproduct(iob,io)  !%%matrix
	...
	if(las.le.0.or.las.gt.nele)then
		...
		j_err=.true. ;return
	end if !if(las.le.0.or.las.gt.nele)  10371
	...
	if(ifi.le.0.or.ifi.gt.nele.or.las.le.0.or.las.gt.nele)then
		...
		j_err=.true. ;return
	end if !if(ifi.le.0.or.ifi.gt.nele.or.las.le.0.or.las.gt.nele)  10378
	end select !select case (narg)
...
subroutine loadtrees(iob,io)
	...
	do itre=j_itree1,j_itree2
		...
		enddo !i=1,j_gettreevars(0)  10394
	enddo !itre=j_itree1,j_itree2  10392
	!	io=io+narg+3
	return
...
subroutine assone(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		...
		if(j_otype(iout).eq.j_ipmatrix)then
			...
			return
		endif !if(j_otype(iout).eq.j_ipmatrix)  10409
		...
		if(j_otype(argout(1)).eq.j_ipmatrix.and.j_otype(argin(1)).eq.j_ipreal)then
			...
			if(j_o(argout(1))%i(4).eq.j_matdiag)then
				...
				do i=1,nrow
					...
					enddo !j=1,ncol  10491
				enddo !i=1,nrow  10490
 
			endif !if(j_o(argout(1))%i(4).eq.j_matdiag)  10477
			return
		endif !if(j_otype(argout(1)).eq.j_ipmatrix.and.j_otype(argin(1)).  10470
	endif !if(narin.eq.1.and.narout.gt.1)  10455
...
subroutine properties(iob,io)
	...
	if(inprint.gt.0)then
		...
		enddo !j=1,narg  10572
		write(6,*)' '
		!	write(6,*)j_inp(1:iip),(j_v(linevar(j)),j=1,narg)
	endif !if(inprint.gt.0)  10571
	goto 1
...
subroutine classify(iob,io)
	...
	if(j_linkoption(iob,io,j_mxrange).lt.0.and.j_linkoption(iob,io,j_many).lt.0)then
		...
		j_err=.true.;return
	endif !if(j_linkoption(iob,io,j_mxrange).lt.0.and.j_linkoption(io  10898
	...
	if(nr.lt.0)then
		...
		endif !if(j_linkoption(iob,io,j_mdx).gt.0)  10909
		if(j_linkoption(iob,io,j_mclasses).ge.0)then
			write(6,*)'classes-> requires xrange->'
			j_err=.true.;return
 
		endif !if(j_linkoption(iob,io,j_mclasses).ge.0)  10915
		xrange=.false.
	...
	elseif(nr.eq.0.and..not.isxfunc)then !if(nr.lt.0)then
		...
		endif !if(ii1.le.0.or.ii2.le.0)  10929
		xmin=j_v(ii1)
		xmax=j_v(ii2)
	...
	if(xrange)then
		...
		if(nargoc.gt.0)then
			...
			clasn=7.
		endif !if(nargoc.gt.0)  10960
		if(nargoc.ge.2)then
			clasz=j_v(j_o(iob)%i( li+2))
		else !if(nargoc.ge.2)then
			clasz=7.
		endif !if(nargoc.ge.2)  10965
		...
		if(nargopt.gt.0)then
			...
			nval=clasn
		end if !if(nargopt.gt.0)  10976
	end if !if(xrange)  10951
	nval2=0
		...
		if(nargopt.lt.2)then
			...
			if(allocated(iperm))then
				if(size(iperm).lt.jnobstot)then
					deallocate(iperm)
					allocate(iperm(1:jnobstot))
				endif !if(size(iperm).lt.jnobstot)  10998
			else !if(allocated(iperm))then
				allocate(iperm(1:jnobstot))
			endif !if(allocated(iperm))  10997
			...
			if(nargopt.ge.1)then
				...
				nval2=clasz
			end if !if(nargopt.ge.1)  11010
 
		end if !if(nargopt.lt.2)  10994
	endif !if(iz.gt.0)  10993
	...
	if(minobs.gt.0)then
		...
		endif !if(iz.gt.0)  11028
	endif !if(minobs.gt.0)  11025
	...
	endif !if(.not.xrange.and.iz.gt.0.and..not.zrange)  11041
	...
	do j_iobs=j_dfrom,j_duntil
		...
		if(isxfunc)then
			...
			xvalue=j_v(ix_)
		endif !if(isxfunc)  11074
		if(abs(xvalue).ge.1.7d19)then
			missing=missing+1
			cycle
		endif !if(abs(xvalue).ge.1.7d19)  11079
		if(iz.gt.0)then
			if(abs(j_v(iz)).ge.1.7d19)then
				missing=missing+1
				cycle
			endif !if(abs(j_v(iz)).ge.1.7d19)  11084
 
		endif !if(iz.gt.0)  11083
		if(iy.gt.0)then
			if(abs(j_v(iy)).ge.1.7d19)then
				missing=missing+1
				cycle
			endif !if(abs(j_v(iy)).ge.1.7d19)  11091
		endif !if(iy.gt.0)  11090
		...
		if(xrange)then
			...
			endif !if(.not.istailtofirst.and.xvalue.lt.xmin-0.000000001d0)  11100
			if(.not.istailtolast.and.xvalue.gt.xmax+0.000000001d0)then
				nup=nup+1
				cycle
			endif !if(.not.istailtolast.and.xvalue.gt.xmax+0.000000001d0)  11105
			j=(xvalue-xmin)/dx+1
			j=min(j,nval)
			j=max(j,1)
			!if(j.lt.0)cycle
			...
			enddo !j=1,nval  11116
 
		endif !if(xrange)  11098
		if(iz.gt.0)then
			if(zrange)then
				...
				enddo !j2=1,nval2  11130
 
			end if !if(zrange)  11122
		end if !if(iz.gt.0)  11121
		...
		endif !if(j2.gt.0)  11150
	enddo !j_iobs=j_dfrom,j_duntil  11069
	...
	if(minobs.gt.0)then
		...
		do i2=2,nval
			...
			elseif(sumwtx(i2).gt.0)then
				...
				endif !if(i0.lt.i2)  11214
			endif !if(sumwtx(i0).lt.dinobs.and.sumwtx(i2).gt.0)  11196
 
			!		endif
		enddo !i2=2,nval  11194
	...
	if(iout.ne.j_ivresult)then
		if(iz.gt.0)then
			...
			if(xrange)then
				...
				end do !i=1,nval  11265
			else !if(xrange)then
				j_o(ivmat)%d(2:nval+1)=value9(1:nval)
			end if !if(xrange)  11262
			if(zrange)then
			...
			if(isy)then
				...
				do j=0,nval
					if(sumwt(0,j).gt.1.d0.and.xss(0,j).gt.0.d0)then
						...
						haj=0.d0
					end if !if(sumwt(0,j).gt.1.d0.and.xss(0,j).gt.0.d0)  11352
					j2=j
					if(j.eq.0)j2=nval1
					j_o(ivmat)%d(3*(nval1)+j2)=haj
				end do !j=0,nval  11351
				j_dapu2=j_0
				...
				do j_iobs=j_dfrom,j_duntil
					...
					if(abs(j_dapu).ge.1.7d19.or.j_rejected)then
						...
						cycle
					endif !if(abs(j_dapu).ge.1.7d19.or.j_rejected)  11376
					ico1=2
					do while(j_dapu.lt.j_o(ivmat)%d(ico1).and.ico1.lt.nval)
						ico1=ico1+1
					enddo !while(j_dapu.lt.j_o(ivmat)%d(ico1).and.ico1.lt.nval)  11382
					ico0=ico1-1
				...
				enddo !j_iobs=j_dfrom,j_duntil  11370
		...
		end if !if(iz.gt.0)  11260
		if(isfunc)iy=j_ivresult
		j_o(ivmat)%i2=(/ix_,iz,iy/)
	end if !if(iout.ne.j_ivresult)  11259
	...
	if(iz.gt.0)then
		...
		if(isy)then
			...
			do j=1,nval
				...
				endif !if(xrange)  11459
				write(6,*)' '
			enddo !j=1,nval  11458
		endif !if(isy)  11441
...
4561	format(f12.5,i8,3f12.5)
			...
			if(j_dprint.ge.2)then
				...
				do j=1,nval
					...
					if(freqtot(0,j).gt.0)then
						...
						endif !if(j.lt.ifim)  11518
						!	write(6,456)xmin+(j+0.5)*dx,freqtot(0,j),xs(0,j),j_o(ivmat)%d(3*(nval+1)+j)
					endif !if(freqtot(0,j).gt.0)  11515
				enddo !j=1,nval  11513
				write(6,*)'---------------'
				...
				enddo !j=1,nval  11532
				write(6,*)'---------------'
				write(6,4561)xsx(0),nobst,xs(0,0)
			endif !if(j_dprint.ge.2)  11510
		...
		elseif(isy)then !if(xrange.and.isy)then
			...
			enddo !j=1,nval  11572
			write(6,*)'-----------'
			write(6,*)'       ',nobst
 
		endif !if(xrange.and.isy)  11509
		!	endif
	end if !if(iz.gt.0)  11421
...
recursive subroutine show(iob,io)
	...
	elseif(j_otype(ivfig).eq.j_ipfigure)then !if(j_otype(ivfig).eq.j_ipchar)then
		...
		j_err=.true.
	endif !if(j_otype(ivfig).eq.j_ipchar)  11619
	!!call j_clearoption(iob,io)
	return
...
subroutine plot3d(iob,io)
	...
	if(j_otype(imat).ne.j_ipmatrix)then
		...
		j_err=.true.;return
	endif !if(j_otype(imat).ne.j_ipmatrix)  11697
	nrow=j_o(imat)%i(1)
	...
	endif !if(j_o(imat)%i(2).ne.3)  11704
	...
	if(sorted)then
		...
		do i=1,nrow
			...
			endif !if(i.lt.nrow)  11746
			ibas=ibas+3
		enddo !i=1,nrow  11744
	else
		j=(p(1)-1)*3
		do i=1,nrow
			...
			if(i.lt.nrow)then
				...
				j=j2
			endif !if(i.lt.nrow)  11755
		end do !i=1,nrow  11753
	endif !if(sorted)  11742
...
subroutine clearbits(iob,io)
	...
	do i=2,narg
		...
		if(j.gt.32.or.j.le.0)then
			...
			j_err=.true.
			return
		end if !if(j.gt.32.or.j.le.0)  11796
		!   ii=(j-1)/32+1
		ibit=j-1  ! bit numbering starts from zero
		rw=j_v(ivb)
		iw= ibclr(iw,ibit)
		j_v(ivb)=rw
	end do !i=2,narg  11794
	...
	do i=2,narg
		...
		if(ii.gt.j_o(ivb)%i(0).or.j.le.0)then
			...
			j_err=.true.
			return
		end if !if(ii.gt.j_o(ivb)%i(0).or.j.le.0)  11813
		ibit=j-(ii-1)*64-1   !(ii-1)*32-1  ! bit numbering starts from zero
		rw=j_v(j_o(ivb)%i(ii))
		iw= ibclr(iw,ibit)
		j_v(j_o(ivb)%i(ii))=rw
	end do !i=2,narg  11810
...
subroutine setbits(iob,io)
	...
	do i=2,narg
		...
		if(j.gt.64.or.j.le.0)then
			...
			j_err=.true.
			return
		end if !if(j.gt.64.or.j.le.0)  11848
		!   ii=(j-1)/32+1
		ibit=j-1  ! bit numbering starts from zero
		rw=j_v(ivb)
		iw= ibset(iw,ibit)
		j_v(ivb)=rw
	end do !i=2,narg  11846
	case(j_iplist) !select case(j_otype(ivb))
	do i=2,narg
		j=j_v(j_o(iob)%i2(io+1+i))
		if(j.gt.j_o(ivb)%i(1).or.j.le.0)then
			...
			j_err=.true.
			return
		end if !if(j.gt.j_o(ivb)%i(1).or.j.le.0)  11863
		ii=(j-1)/64+1  !32+1
		if(ii.gt.j_o(ivb)%i(0).or.j.le.0)then
			call j_printname('**setbits: list', ivb,' ')
			...
			j_err=.true.
			return
		end if !if(ii.gt.j_o(ivb)%i(0).or.j.le.0)  11870
		ibit=j-(ii-1)*64-1  !32-1  ! bit numbering starts from zero
		rw=j_v(j_o(ivb)%i(ii))
		iw= ibset(iw,ibit)
		j_v(j_o(ivb)%i(ii))=rw
	end do !i=2,narg  11861
	end select !select case(j_otype(ivb))
	return
...
subroutine getbit(iob,io)
	...
	if(j.gt.64.or.j.le.0)then
		...
		j_err=.true.
		return
	end if !if(j.gt.64.or.j.le.0)  11908
	icurbit=j-1
	rw=j_v(ivb)
	if(btest(iw,icurbit))then
		...
		j_v(iout)=0.
	end if !if(btest(iw,icurbit))  11916
	case(j_iplist) !select case(j_otype(ivb))
	ii=(j-1)/64+1
	if(ii.gt.j_o(ivb)%i(0).or.j.le.0)then
		...
		j_err=.true.
		return
	end if !if(ii.gt.j_o(ivb)%i(0).or.j.le.0)  11923
	icurbit=j-(ii-1)*64-1
	rw=j_v(j_o(ivb)%i(ii))
	if(btest(iw,icurbit))then
		j_v(iout)=j_1
		...
		j_v(iout)=j_0
	end if !if(btest(iw,icurbit))  11931
	end select !select case(j_otype(ivb))
	return
...
subroutine getbitch(iob,io)
	...
	do i=i1,i2
		...
		if(btest(iw,i-1))then
			...
			j_cline(ie:ie)='0'
		end if !if(btest(iw,i-1))  11975
	end do !i=i1,i2  11973
	case(j_iplist) !select case(j_otype(ivb))
	...
	do i=i1,i2
		...
		if(btest(iw,icurbit))then
			...
			j_cline(ie:ie)='0'
		end if !if(btest(iw,icurbit))  11991
	end do !i=i1,i2  11986
	end select !select case(j_otype(ivb))
...
subroutine sort(iob,io)
	...
	if(j_otype(imat).ne.j_ipmatrix)then
		...
		j_err=.true.
		return
	end if !if(j_otype(imat).ne.j_ipmatrix)  12038
	nrow_=j_o(imat)%i(1)
	ncol=j_o(imat)%i(2)
	...
	if(key.le.0.or.key.gt.ncol)then
		...
		j_err=.true.
		!	return
	end if !if(key.le.0.or.key.gt.ncol)  12051
	...
	if(noptarg.eq.2)then
		...
		if(key2.le.0.or.key2.gt.ncol)then
			...
			j_err=.true.
			return
		end if !if(key2.le.0.or.key2.gt.ncol)  12067
...
subroutine thisfile(iob,io) ! returns the character object referring to current include file
	...
	if(j_ninc.le.1)then
		...
		if(ivchar.le.0)then
			...
			j_err=.true.
		endif !if(ivchar.le.0)  12178
		if(j_err) return
 
		call j_asschar2(ivchar,ivout)
	endif !if(j_ninc.le.1)  12170
	return
...
subroutine filestat(iob,io)
	...
	if(j_otype(ifile).ne.j_ipchar)then
		write(6,*)'illegal argument'
		j_err=.true. ;return
	endif !if(j_otype(ifile).ne.j_ipchar)  12197
	...
	do i=1,narg
		...
		if(nuu.ne.0)then
			...
			if(j_yes)then
				...
				j_err=.true.
				return
			endif !if(j_yes)  12271
		endif !if(nuu.ne.0)  12264
		nlin=0
...
SUBROUTINE envelope(iob,io)
	...
	DO i = 2, n-1
		...
		ELSE IF (temp > xmax) THEN !IF (temp < xmin) THEN
			vertex(2) = i
			xmax = temp
		END IF !IF (temp < xmin)  12503
	END DO !i = 2, n-1  12501
	...
	IF (xmax == xmin) THEN
		...
		DO i = 2, n-1
			...
			ELSE IF (temp > ymax) THEN !IF (temp < ymin) THEN
				vertex(2) = i
				ymax = temp
			END IF !IF (temp < ymin)  12527
		END DO !i = 2, n-1  12525
	...
	DO i = 1, n
		...
		ELSE IF (dist < zero) THEN !IF (dist > zero) THEN
			...
			IF (dist < dmin) THEN
				next(2) = i
				dmin = dist
			END IF !IF (dist < dmin)  12573
		END IF !IF (dist > zero)  12563
	END DO !i = 1, n  12560
	...
	40 IF (next(j) < 0) THEN
		...
		j = j + 1
		GO TO 40
	END IF !40 IF (next(j) < 0)  12593
 
	jp1 = j + 1
	DO i = nvert, jp1, -1
		vertex(i+1) = vertex(i)
		next(i+1) = next(i)
	END DO !i = nvert, jp1, -1  12600
	jp2 = jp1 + 1
	nvert = nvert + 1
	IF (jp2 > nvert) jp2 = 1
	i1 = vertex(j)
		...
		ELSE !IF (dist > zero) THEN
			...
			IF (dist > zero) THEN
				...
				IF (dist > dmax2) THEN
					next(jp1) = i
					dmax2 = dist
				END IF !IF (dist > dmax2)  12649
			END IF !IF (dist > zero)  12646
		END IF !IF (dist > zero)  12637
		i = iwk(i)
	ELSE !IF (i /= i2save) THEN
		i = i2next
	END IF !60 IF (i /= i2save)  12635
...
100	continue  !call j_nextobs();if(j_err)return
	...
	if(j_rejected)then
		...
		if(ial.gt.j_duntil)then
			...
			j_err=.true.
			return
		endif !if(ial.gt.j_duntil)  12738
		goto 100
	endif !if(j_rejected)  12736
	...
	do i=ial,j_duntil
		...
		if(abs(j_v(iv)).ge.1.7d19)then
			missining=missing+1
			cycle
		endif !if(abs(j_v(iv)).ge.1.7d19)  12753
		nacc=nacc+1
		if(.not.any(value9(1:nval).eq.j_v(iv)))then
			nval=nval+1;value9(nval)=j_v(iv)
		end if !if(.not.any(value9(1:nval).eq.j_v(iv)))  12758
	end do !i=ial,j_duntil  12748
	!	end do !do k=1,jndatasetss
...
subroutine solve(iob,io)
	...
	if(j_otype(arg(1)).eq.j_ipreal)then
		if(j_v(arg(1)).eq.j_0)then
			...
			j_err=.true.;return
		endif !if(j_v(arg(1)).eq.j_0)  12836
		...
		elseif(j_otype(arg(2)).eq.j_ipmatrix)then
			...
			return
		end if !if(j_otype(arg(2)).eq.j_ipreal)  12840
	endif !if(j_otype(arg(1)).eq.j_ipreal)  12835
	...
	if(itarg1.ne.j_ipmatrix.or.itarg2.ne.j_ipmatrix)then
		...
		j_err=.true.;return
	endif !if(itarg1.ne.j_ipmatrix.or.itarg2.ne.j_ipmatrix)  12853
	...
	if(ndim.ne.j_ncols(arg(1)).or.j_nrows(arg(2)).ne.ndim)then   !j_o(arg(2))%i8(1).ne.ndim)then
		...
		j_err=.true.
		return
	endif !if(ndim.ne.j_ncols(arg(1)).or.j_nrows(arg(2)).ne.ndim)  12860
	...
	do i=1,ndim
		...
		enddo !j=1,ndim  12870
 
		do j=1,ndim2
			iel2=iel2+1
			rhs_(i,j)=j_o(arg(2))%d(iel2)
		enddo !j=1,ndim2  12875
	enddo !i=1,ndim  12869
	...
	if(info_.ne.0)then
		...
		do i=1,ndim
			...
			enddo !j=1,ndim2  12891
		enddo !i=1,ndim  12890
	endif !if(info_.ne.0)  12882
...
subroutine regr(iob,io)   !compute a linear regression model using linpack (%%stat)
	...
	intcep2=intcep+1
		...
		if(isvariance)then
			...
			sumw=j_0
				...
				if(abs(j_v(iy)).ge.1.7d19)then
					missing=missing+1
					cycle
				endif !if(abs(j_v(iy)).ge.1.7d19)  13084
 
				do j=intcep2,ncoef
					if(abs(j_v(regl(j))).ge.1.7d19)then
						missing=missing+1
						cycle obloopw
					endif !if(abs(j_v(regl(j))).ge.1.7d19)  13090
				...
				do j=1,ncoef
					...
					enddo !j2=j,ncoef  13118
				enddo !j=1,ncoef  13115
				sumw=sumw+weight
				sumy=sumy+weight*j_v(iy)
				sumynw=sumynw+j_v(iy)
			enddo obloopw !oopw:do i=j_dfrom,j_duntil  13078
				...
				if(abs(j_v(iy)).ge.1.7d19)then
					missing=missing+1
					cycle
				endif !if(abs(j_v(iy)).ge.1.7d19)  13136
 
				do j=intcep2,ncoef
					if(abs(j_v(regl(j))).ge.1.7d19)then
						missing=missing+1
						cycle obloop1
					endif !if(abs(j_v(regl(j))).ge.1.7d19)  13142
				enddo !j=intcep2,ncoef  13141
				...
				do j=1,ncoef
					...
					enddo !j2=j,ncoef  13162
				enddo !j=1,ncoef  13159
 
				sumy=sumy+j_v(iy)
			enddo obloop1 !oop1:do i=j_dfrom,j_duntil  13130
		endif !if(isvariance)  13075
		if(ntot.lt.ncoef)then
			write(6,*)'*only ',ntot,' observations accepted, estimation not possible'
			j_err=.true. ;return
		endif !if(ntot.lt.ncoef)  13171
		...
		do j=1,ncoef
			if(mat(j,j).lt.1.d-18)then
				...
				goto 445
			endif !if(mat(j,j).lt.1.d-18)  13179
			do j2=1,j-1
				mat(j,j2)=mat(j2,j)
				if(abs(mat(j,j2)/sqrt(mat(j,j)*mat(j2,j2))).gt.0.99999d0)then
					...
					endif !if(j2.eq.1.and.intcep2.gt.1)  13189
					iimin=j
					singutot=.true.
					singucur=.true.
					goto 445
				endif !if(abs(mat(j,j2)/sqrt(mat(j,j)*mat(j2,j2))).gt.0.99999d0)  13188
			enddo !j2=1,j-1  13186
		enddo !j=1,ncoef  13178
		...
		ibas=-j_dnkeep+(j_dfrom-1)*j_dnkeep    !dtas
			...
			if(j_rejected.or.abs(j_v(iy)).ge.1.7d19)then
				...
				if(iresid.gt.0)j_o(j_divmat)%d(ibas+iresid)=j_0
				cycle
			endif !if(j_rejected.or.abs(j_v(iy)).ge.1.7d19)  13228
 
			do j=intcep2,ncoef
				if(abs(j_v(regl(j))).ge.1.7d19)cycle obloop2
			enddo !j=intcep2,ncoef  13234
		...
		if(step)then
			...
			do ii=intcep2,ncoef
				if(abs(t(ii)).lt.tmin0)then
					iimin=ii !REGL
					tmin0=abs(t(ii))
				endif !if(abs(t(ii)).lt.tmin0)  13281
			enddo !ii=intcep2,ncoef  13280
		...
		if(nrow.gt.0)then
			...
			do j=intcep2,ncoef
				...
				if(rhs_(j,ncoef1).lt.j_0)then
					...
					j_tempchar(lout:lout)='+'
				endif !if(rhs_(j,ncoef1).lt.j_0)  13319
	...
	718 format(1x,a15,2g16.8e1,f11.2)
			...
			end do !j=intcep2,ncoef  13351
 
			write(6,*)' '
			write(6,'(a,g15.7,a,f8.5,a,i8)')'RMSE= ',rmse,' R2=',r2, ' df ',idfe
		endif !if(nrow.gt.0)  13301
		if(isvariance)then
		...
		endif !if(isvariance)  13361
		if(intcep.gt.0)then
			write(6,*)'F reg (',ncoef-1,idfe,')=',(sst-sse)/(mse_*(ncoef-1.))
		end if !if(intcep.gt.0)  13367
445	if(step.and.tmin0.lt.tmin.or.singucur)then
			if(iimin.lt.ncoef)then
				do ii=iimin,ncoef-1
					regl(ii)=regl(ii+1)
				enddo !ii=iimin,ncoef-1  13372
			endif !if(iimin.lt.ncoef)  13371
 
 
			ncoef=ncoef-1
			ncoef1=ncoef1-1
			cycle steploop
		endif !445	if(step.and.tmin0.lt.tmin.or.singucur)  13370
		exit steploop
 
	enddo steploop !ploop:	do iste=1,ncoef  13062
	...
	if(iout.ne.j_ivresult)then
		...
		if(var)then
			...
			do i=1,ncoef
				...
				enddo !j=1,ncoef  13398
			enddo !i=1,ncoef  13397
		endif !if(var)  13394
		ivcor=0
		if(corr)then
			...
			do i=1,ncoef
				do j=1,ncoef
					...
					endif !if(i.eq.j)  13415
					!		endif
				enddo !j=1,ncoef  13412
			enddo !i=1,ncoef  13411
		endif !if(corr)  13408
		...
		endif !if(getpar)  13445
 
	endif !if(iout.ne.j_ivresult)  13387
...
subroutine coefse(iob,io,ity)  !ity=0 coef, ity=1 se coef(iob,io)
	...
	if(inde.eq.j_ivall)then
		...
		return
	endif !if(inde.eq.j_ivall)  13498
	...
	do i=1,nvar
		...
		endif !if(j_o(irg)%i(i).eq.inde)  13506
	end do !i=1,nvar  13505
	i=j_v(inde)
	if(i.gt.0.and.i.le.nvar)then
		j_v(iout)=j_o(irg)%d(i+ity*j_o(irg)%i(0))
		return
	endif !if(i.gt.0.and.i.le.nvar)  13514
	call j_getname(inde)
...
subroutine nonlin(iob,io)   !compute a linear regression model using linpack (%%stat)
	...
	if(ncoef1.gt.0)then
		if(ncoef.ne.ncoef1)then
			...
			j_err=.true. ;return
		endif !if(ncoef.ne.ncoef1)  13613
 
		j_v(par(1:ncoef1))=j_v(initial(1:ncoef1))
	endif !if(ncoef1.gt.0)  13612
	...
	enddo !ii=1,ncoef  13626
	if(nnz.eq.0)then
		write(6,*)'some initial values should be nonzero'
		j_err=.true.
		return
	endif !if(nnz.eq.0)  13629
	...
	if(narg.gt.0.)then
		if(narg.ne.ncoef)then
			...
			isparmin=.true.
		endif !if(narg.ne.ncoef)  13646
	endif !if(narg.gt.0.)  13645
	...
	if(narg.gt.0)then
		if(narg.ne.ncoef)then
			...
			isparmax=.true.
		endif !if(narg.ne.ncoef)  13660
	endif !if(narg.gt.0)  13659
	...
	if(narg.gt.0)then
		if(narg.ne.ncoef)then
			...
			isdpar=.true.
		endif !if(narg.ne.ncoef)  13674
	endif !if(narg.gt.0)  13673
	step=1
	...
	do i=j_dfrom,j_duntil
		...
		if(abs(j_v(iy)).ge.1.7d19)then
			missing=missing+1
			cycle
		endif !if(abs(j_v(iy)).ge.1.7d19)  13743
		ntot=ntot+1
		...
		if(isvariance)then
			...
			if(variance(i).le.j_0)then
				...
				j_err=.true. ;return
			endif !if(variance(i).le.j_0)  13755
			...
			do j=1,ncoef
				...
				enddo !j2=j,ncoef  13772
			enddo !j=1,ncoef  13769
	...
	if(eka)then
		if(j_dprint.gt.1)then
			...
			endif !if(isvariance)  13818
			if(isloglike)write(6,*)'log likelihood ',loglike
		endif !if(j_dprint.gt.1)  13814
		eka=.false.
	endif !if(eka)  13813
	...
	if(iter.gt.maxiter)then
		...
		j_v(par(1:ncoef))=current
		goto 200
	endif !if(iter.gt.maxiter)  13834
	if(iter2.gt.8)then
		j_v(par(1:ncoef))=current
	...
	if(isparmin)then
		do i=1,ncoef
			...
			endif !if(j_v(par(i)).lt.parmin(i))  13851
		enddo !i=1,ncoef  13850
 
	endif !if(isparmin)  13849
	scalemin2=scale
	if(isparmax)then
		do i=1,ncoef
	...
	do i=j_dfrom,j_duntil
		...
		if(isvariance)then
			...
			sumf=sumf+j_v(ivfunc)
		endif !if(isvariance)  13915
	end do !i=j_dfrom,j_duntil  13906
	!	end do !do k=1,jndatasetss
	...
	if(j_dprint.gt.0)then
		...
		end do !j=1,ncoef  13981
	endif !if(j_dprint.gt.0)  13973
...
589	format(a,g14.6,a,f7.4,a,i7)
		...
		endif !if(isvariance)  13992
	endif !if(j_dprint.gt.0)  13988
	if(isloglike)then
		write(6,*)'loglikehilihood ',loglike
		write(6,*)' '
	endif !if(isloglike)  14000
	if(iout.ne.j_ivresult)then
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
		...
		if(var)then
			...
			do i=1,ncoef
				do j=1,ncoef
					it=it+1
					j_o(ivvar)%d(it)=rhs_(i,j)*mse_  !laske
				enddo !j=1,ncoef  14021
			enddo !i=1,ncoef  14020
		endif !if(var)  14017
		ivcor=0
		if(corr)then
			ivcor=j_defmatrix(iout,'%corr',ncoef,ncoef,j_matreg)
			...
			do i=1,ncoef
				do j=1,ncoef
					...
					if(i.eq.j)then
						...
						j_o(ivcor)%d(it)=rhs_(i,j)/sqrt(rhs_(i,i)*rhs_(j,j))
					endif !if(i.eq.j)  14034
				enddo !j=1,ncoef  14032
			enddo !i=1,ncoef  14031
		endif !if(corr)  14028
	...
	endif !if(iout.ne.j_ivresult)  14004
	if(j_depilog.gt.0)call dotrans(j_depilog,1)
	return
	900   write(6,*)'*regr: error in transformations'
	return
...
recursive subroutine value_(iob,io)  ! Jlp22-function value(
	...
	if(isz)then  !bilinear interpolation
		...
		if(nrows.lt.3.or.ncols.lt.3.or.narg.lt.3)then
			...
			j_err=.true.
			return
		endif !if(nrows.lt.3.or.ncols.lt.3.or.narg.lt.3)  14116
		arg2=j_v(j_o(iob)%i(io+4))
 
		do icol=3,ncols
			if(arg.le.j_getmatel(ifunc,1,icol))goto 17
 
		enddo !icol=3,ncols  14123
		icol=ncols
						17  continue
	...
	if(irow.le.0.and.j_o(ifunc)%i(4).eq.j_matreg)then
		...
		if(iout.gt.j_named)then
			...
			j_err=.true.
			return
		endif !if(iout.gt.j_named)  14146
		...
		do i=2,j_o(ifunc)%i(2)
			if(j_o(ifunc)%d(i).ge.arg.or.i.eq.j_o(ifunc)%i(2))then
				...
				end do !irow=2,j_o(ifunc)%i(1)  14155
				return
			end if !if(j_o(ifunc)%d(i).ge.arg.or.i.eq.j_o(ifunc)%i(2))  14153
 
		end do !i=2,j_o(ifunc)%i(2)  14152
		...
		if(irow.le.0.or.irow.gt.j_o(ifunc)%i(1))then
			...
			j_err=.true.
			return
		end if !if(irow.le.0.or.irow.gt.j_o(ifunc)%i(1))  14169
	end if !if(irow.le.0.and.j_o(ifunc)%i(4).eq.j_matreg)  14143
	...
	if(iout.gt.j_mxnamedv+1)then
		...
		nsav=0
	endif !if(iout.gt.j_mxnamedv+1)  14199
	...
	if(inde.le.0.or.inde.gt.j_o(ifunc)%i(1))then
		...
		j_err=.true.
		return
	endif !if(inde.le.0.or.inde.gt.j_o(ifunc)%i(1))  14238
	...
	if(j.gt.j_o(ifunc)%i(1).or.j.le.0)then
		if(j_linkoption(iob,io,j_many).ge.0)then
			...
			!	!!call j_clearoption(iob,io)  ! subroutine
			return
		endif !if(j_linkoption(iob,io,j_many).ge.0)  14254
		call j_printname('**value: first index has illegal value for bitmatrix ',ifunc, ' ')
		j_err=.true.;return
	endif !if(j.gt.j_o(ifunc)%i(1).or.j.le.0)  14253
	if(narg.gt.2)then
		...
		if(inde2.gt.j_o(ifunc)%i(2).or.inde2.lt.j_o(ifunc)%i(3))then
			if(j_linkoption(iob,io,j_many).ge.0)then
				...
				!	!!call j_clearoption(iob,io)  ! subroutine
				return
			endif !if(j_linkoption(iob,io,j_many).ge.0)  14265
			call j_printname('**value: second argument has illegal value for bitmatrix ',ifunc, ' ')
			j_err=.true.;return
		endif !if(inde2.gt.j_o(ifunc)%i(2).or.inde2.lt.j_o(ifunc)%i(3))  14264
	endif !if(narg.gt.2)  14262
...
subroutine ran(iob,io) !rando_loopm uniform number netlib ranlib
	...
	if(j_otype(iout).ne.j_ipreal)then
		if(j_otype(iout).ne.j_ipmatrix.or.iout.eq.j_ivresult.or.iout.gt.j_named)call j_del(iout)
	endif !if(j_otype(iout).ne.j_ipreal)  14320
	if(narg.gt.0)then
	...
	end if !if(narg.gt.0)  14323
	if(j_otype(iout).eq.j_ipmatrix)then
		do j=1,j_o(iout)%i(3)
			j_o(iout)%d(j)=ranf()
		enddo !j=1,j_o(iout)%i(3)  14331
	else !if(j_otype(iout).eq.j_ipmatrix)then
 
		j_v(iout)=ranf()
	endif !if(j_otype(iout).eq.j_ipmatrix)  14330
...
subroutine select(iob,io)
	...
	if(iout.eq.j_ivresult)then
		...
		j_err=.true.
	endif !if(iout.eq.j_ivresult)  14366
	...
	if(n.lt.0.or.n.gt.ntot.or.ntot.le.0)then
		...
		j_err=.true.
	endif !if(n.lt.0.or.n.gt.ntot.or.ntot.le.0)  14373
	...
	do i=1,n
		...
		do j=1,ntot
			...
			endif !if(ir.eq.icount)  14394
 
		enddo !j=1,ntot  14392
	enddo !i=1,n  14387
		...
		jloop:		do j=1,j_o(iout)%i(3)
				...
				do i=1,ncol-1
					...
					if(j_o(iarg)%d(ncol+i).ge.ra)then
						...
						cycle jloop
					endif !if(j_o(iarg)%d(ncol+i).ge.ra)  14442
				enddo !i=1,ncol-1  14440
				j_o(iout)%d(j)=j_o(iarg)%d(ncol)
 
				!		j_o(iout)%d(j)=ranf()
			enddo jloop !op:		do j=1,j_o(iout)%i(3)  14437
			...
			do j=1,j_o(iout)%i(3)
				...
				do i=2,ncol
					...
					endif !if(j_o(iarg)%d(ncol+i).ge.ra)  14461
 
 
				enddo !i=2,ncol  14459
				!		j_o(iout)%d(j)=ranf()
			enddo !j=1,j_o(iout)%i(3)  14456
		endif !if(diskr)  14436
	elseif(diskr)then
		do i=1,ncol-1
			!y=y0+(y1-y0)*(x-x0)/(x1-x0)  here x is value of rnadom uniform and y is the xavalue
			if(j_o(iarg)%d(ncol+i).ge.ra)then
				...
				return
			endif !if(j_o(iarg)%d(ncol+i).ge.ra)  14480
		enddo !i=1,ncol-1  14478
		...
		do i=2,ncol
			...
			endif !if(j_o(iarg)%d(ncol+i).ge.ra)  14497
		enddo !i=2,ncol  14495
		!	j_v(iout)=ranf()
	endif !if(j_otype(iout).eq.j_ipmatrix)  14435
...
subroutine rann(iob,io)  !calls function gennor from netlib ranlib
	...
	if(narg.ge.1)then
		...
		if(narg.ge.2)then
			...
			sd=1.
		endif !if(narg.ge.2)  14570
	else !if(narg.ge.1)then
		av=0.
		sd=1.
	endif !if(narg.ge.1)  14568
	if(j_otype(iout).eq.j_ipmatrix)then
		do j=1,j_o(iout)%i(3)
	...
	endif !if(j_otype(iout).eq.j_ipmatrix)  14579
...
subroutine ranpoi(iob,io)  !calls function ignpoi from netlib ranlib
	...
	if(j_otype(iout).ne.j_ipreal)then
		if(j_otype(iout).ne.j_ipmatrix.or.iout.eq.j_ivresult.or.iout.gt.j_named)call j_del(iout)
	endif !if(j_otype(iout).ne.j_ipreal)  14601
	...
	endif !if(rmu.le.0)  14605
	if(j_otype(iout).eq.j_ipmatrix)then
		do j=1,j_o(iout)%i(3)
			j_o(iout)%d(j)=ignpoi(rmu)
		enddo !j=1,j_o(iout)%i(3)  14612
	else !if(j_otype(iout).eq.j_ipmatrix)then
 
		j_v(iout)=ignpoi(rmu)
	endif !if(j_otype(iout).eq.j_ipmatrix)  14611
...
subroutine ranbin(iob,io)
	...
	if(j_otype(iout).eq.j_ipmatrix)then
		...
		elseif(pp_.eq.1.)then !if(pp_.eq.0.)then
			...
			enddo !j=1,j_o(iout)%i(3)  14687
		endif !if(pp_.eq.0.)  14682
		...
		endif !if(pp_.eq.0.)  14693
	endif !if(j_otype(iout).eq.j_ipmatrix)  14681
	...
	if(ilevel.eq.level.or.ilevel.eq.3)then
		...
		if(j_o(iob)%i(io+1).eq.1)then
			...
			elseif(j_otype(irg).eq.j_iptrans)then
				...
				return
			endif !if(j_otype(irg).eq.j_ipchar)  15045
		endif !if(j_o(iob)%i(io+1).eq.1)  15041
		call print(iob,io)
 
		if(ilevelpause.eq.level.or.ilevelpause.eq.3)call j_pause('<Printpause>',do=.true.)
	endif !if(ilevel.eq.level.or.ilevel.eq.3)  15039
		...
		if(j_keepper.lt.j_nper)then
			...
			if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))then !aa
				...
				if(j_iper.le.j_keepper)then
					io=j_ioret
					return
				end if !if(j_iper.le.j_keepper)  15115
				goto 500
				...
				iob=j_ivsimu    !!!!!!!!!!!!1
				return
			end if !if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))  15113
		else !if(j_keepper.lt.j_nper)then
			io=j_ioret
			return
		end if !if(j_keepper.lt.j_nper)  15111
	end if !if(j_iper.ge.j_nper)  15110
	if(j_nextnodes(j_iper).le.0)then   !nextnodes for period whee next is give
		if(j_iper.eq.j_keepper)then
			io=j_ioret;return
		end if !if(j_iper.eq.j_keepper)  15134
		5000  	continue
		iper1=j_iper-1
		if(iper1.le.0)then
		...
		if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))then
			...
			if(j_iper.le.1)then
				...
				goto 5000
			end if !if(j_iper.le.1)  15146
		end if !if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))  15144
	end if !if(j_nextnodes(j_iper).le.0)  15133
	j_iper=j_iper+1
...
subroutine draw(iob,io) ! draw()
	...
	if(j_o(j_gpiout)%i(5).eq.0)then
		...
		write(nu,*)' '
		write(nu,*)' '
	endif !if(j_o(j_gpiout)%i(5).eq.0)  15272
	if(j_err)return
	...
	endif !if(j_filename(2:2).eq.'$')  15323
	...
	end if !if(j_o(j_gpiout)%d(1).eq.j_inf.and.j_o(j_gpiout)%d(2).eq.j  15337
 
	if(j_gpix.le.0)then
		write(6,*)'x-> not given in draw';goto 99
	end if !if(j_gpix.le.0)  15341
	...
	end if !if(j_o(j_gpiout)%d(1).ge.j_o(j_gpiout)%d(2))  15345
	if(j_err)goto 99
	...
	do i=1,j_gppoints
		...
		if(linkxfunc.gt.0)then
			...
			write(nu,*)xx,yyy
		endif !if(linkxfunc.gt.0)  15361
		xx=xx+dxx
	end do !i=1,j_gppoints  15354
...
subroutine drawline(iob,io)
	...
	if(j_gpset.gt.0)then
		if(j_o(j_gpiout)%i(4+j_gpset).eq.0)then
			...
			write(nu,*)' '
		endif !if(j_o(j_gpiout)%i(4+j_gpset).eq.0)  15494
		if(j_err)return
	...
	if(j_gpnarg.eq.2)then
		if(j_otype(j_gparg(1)).eq.j_ipreal.and.j_otype(j_gparg(2)).eq.j_ipreal)then
			if(j_gplelabel.eq.0)then
				...
				j_err=.true.;return
			endif !if(j_gplelabel.eq.0)  15533
			onlylabel=.true.
		endif !if(j_otype(j_gparg(1)).eq.j_ipreal.and.j_otype(j_gparg(2))  15532
	endif !if(j_gpnarg.eq.2)  15531
	...
	if(makeplot)then
		...
		if(j_filename(2:2).eq.'$')then
			...
			elseif(j_gpstyle.gt.0.and..not.onlylabel)then !if(j_gpstyle.gt.0.and.j_gpmark.ne.j_ivzero)then
				...
				endif !if(onlylabel)  15576
			endif !if(j_gpstyle.gt.0.and.j_gpmark.ne.j_ivzero.and..not.onlyla  15570
			...
			elseif(j_gpstyle.gt.0.and..not.onlylabel)then !if(j_gpstyle.gt.0.and.j_gpmark.ne.j_ivzero)then
				...
				endif !if(onlylabel)  15593
			endif !if(j_gpstyle.gt.0.and.j_gpmark.ne.j_ivzero.and..not.onlyla  15586
		endif !if(j_filename(2:2).eq.'$')  15568
	...
	if(j_gpnarg.eq.1)then
		if(j_otype(iarg).ne.j_ipmatrix)then
			...
			j_err=.true.;return
		endif !if(j_otype(iarg).ne.j_ipmatrix)  15614
	...
	elseif(j_gpnarg.eq.2.and.j_otype(iarg).eq.j_ipmatrix)then !if(j_gpnarg.eq.1)then
		...
		endif !if(npoints*2.ne.j_gpnarg)  15642
		allocate(j_gpval(1:2*npoints))
		j_gpval=j_v(j_gparg(1:2*npoints))
	endif !if(j_gpnarg.eq.1)  15613
	...
	if(j_gpset.eq.0.and..not.onlylabel)then
		...
		do ip=1,npoints
			...
			endif !if(isbreak.and.ip.gt.1)  15658
		...
		if(.not.onlylabel)then
			do ip=1,npoints
				...
				endif !if(isbreak.and.ip.gt.1)  15672
			...
			enddo !ip=1,npoints  15670
			call j_closeunit(nu)  ! nu is unit nuf is the char constant for the name
		endif !if(.not.onlylabel)  15669
 
 
	endif !if(j_gpset.eq.0.and..not.onlylabel)  15654
	...
	if(j_gplelabel.gt.0)then
		...
		if(ifo.gt.0)then
			...
			call j_putfig('set label "'//j_gplabel(1:j_gplelabel)//'" at '// &
				j_chr10(j_gpval(npoints))//','//j_chr10(j_gpval(2*npoints)))
		endif !if(ifo.gt.0)  15692
	endif !if(j_gplelabel.gt.0)  15690
	deallocate(j_gpval)
	!if(j_gpshow)call j_showfig(j_gpiout)
	!	io=j_gpnewio
	return
...
subroutine drawline3(iob,io)
	...
	endif !if(j_otype(iout).ne.j_ipfigure)  15737
	...
	if(append)then
		...
		if(.not.islabel)then
			...
			ili=3
		endif !if(.not.islabel)  15772
	endif !if(append)  15752
	if(narg.eq.1)then
		if(j_otype(arg(1)).ne.j_ipmatrix)then
			write(6,*)'only argument must be matrix'
			j_err=.true.;return
		endif !if(j_otype(arg(1)).ne.j_ipmatrix)  15783
		if(j_o(arg(1))%i(1).ne.2)then
			write(6,*)'argument matrix must have two rows'
			j_err=.true.;return
		endif !if(j_o(arg(1))%i(1).ne.2)  15787
	...
	elseif(narg.eq.2)then !if(narg.eq.1)then
		...
		endif !if(j_otype(arg(1)).eq.j_ipmatrix.and.j_otype(arg(2)).eq.j_  15809
	endif !if(narg.eq.1)  15782
	if(show)then
		...
		enddo !ili=1,j_o(iout)%i(1)  15843
		!	write(nu,'(a)')'e'
		write(nu,'(a)')'replot'
		write(nu,'(a)')'unset title'
...
78		write(6,1000)'<ret>'
		...
		if(ch.ne.' ')then
			...
			if(ch(1:lec).eq.'e')then
				...
				j_err=.true.;return
			endif !if(ch(1:lec).eq.'e')  15861
 
			call j_command(ch(1:lec))
			if(j_stop)then
				call j_stopj()
				return
			endif !if(j_stop)  15867
			if(j_err)then
				j_err=.false.
				write(6,*)'try again, <ret> to continue e => ;return'
 
			endif !if(j_err)  15871
			goto 78
		endif !if(ch.ne.' ')  15858
	endif !if(show)  15838
...
subroutine drawclass(iob,io) ! draw()
	...
	if(ix.gt.0.and.j_gplexlabel.eq.0)then
		...
		!		call j_replacefig(2,'set xlabel "'//j_gpxlabel(1:j_gplexlabel)//'"')
	endif !if(ix.gt.0.and.j_gplexlabel.eq.0)  15987
	...
	endif !if(j_gpleylabel.eq.0.and.iy.gt.0)  15998
	...
	if(histo)then
		...
		do ip=1,ncol-1
			...
			elseif(ismean.and.issd)then
				...
				yy=j_getmatel(iarg,3,ip)
			endif !if(issd.and..not.ismean)  16134
		...
		end do !ip=1,ncol-1  16128
	end if !if(histo)  16032
	!write(6,*)'377373'
	call j_putfig('e')
...
subroutine plotyx(iob,io)   !plotyx()
	...
	elseif(.not.(isfunc.and.isxfunc))then !if(isfunc)then
		if(j_gpnarg.ne.2)then
			...
			j_err=.true.;return
		endif !if(j_gpnarg.ne.2)  16272
		!		j_fig_ixx=j_o(iob)%i(io+3)
		!		ixy=j_o(iob)%i(io+2)
		ix=j_gparg(2)
		iy=j_gparg(1)
	endif !if(isfunc.and..not.isxfunc)  16253
	...
	if(j_gplexlabel.eq.0)then
		if(ix.gt.0)then
			...
			call j_replacefig(2,'set xlabel "xfunc->"')
		endif !if(ix.gt.0)  16290
		!else
		!	call j_replacefig(2,'set xlabel "'//j_gpxlabel(1:j_gplexlabel)//'"')
	endif !if(j_gplexlabel.eq.0)  16289
	...
	if(j_gpleylabel.eq.0)then
		if(iy.gt.0)then
			...
			call j_replacefig(3,'set ylabel "func->"')
		endif !if(iy.gt.0)  16301
		!	write(6,*)'<888',j_o(j_gpiout)%i(1)
	endif !if(j_gpleylabel.eq.0)  16300
	if(j_gpappend)then
		ch0=', '
		! j_o(j_gpiout)%txt(6)(i2+1:i2+1)=','
		! i2=i2+1
	...
	if(j_o(j_gpiout)%i(5).eq.0)then
		...
		write(nu,*)' '
		write(nu,*)' '
	endif !if(j_o(j_gpiout)%i(5).eq.0)  16322
	if(j_err)return
	j_o(j_gpiout)%i(5)=j_o(j_gpiout)%i(5)+1
	...
	end do !i=j_dfrom,j_duntil  16349
	call j_closeunit(nu)
	!	end do !do k=1,jndatasetss
	!	j_fig_np=ntot
	j_v(j_ivaccepted)=ntot
...
subroutine nobs(iob,io) !Number of observations in DATA
	...
	end if !if(j_otype(iv).eq.j_ipregr)  16407
	return
...
subroutine joindata(iob,io) !
	...
	endif !if(isnrowtot.and.nrowtot.le.nobstot)  16600
	!	write(6,*)'ivout ',ivout
	...
	do i=1,ndata
		...
		if(ipe.ne.1.and..not.istwo)then
			...
			j_err=.true. ;return
		endif !if(ipe.ne.1.and..not.istwo)  16617
		...
		if(i.gt.1)then
			...
			endif !if(istwo)  16640
		endif !if(i.gt.1)  16639
		!	write(6,*)'i,nkeep,nkeep2,nkeeptot ',i,nkeep(i),nkeep2(i),nkeeptot
	enddo !i=1,ndata  16611
	!	write(6,*)'nkeep',nkeep
	...
	if(istwo)then
		...
		enddo !i=2,ndata  16669
	endif !if(istwo)  16661
	...
	if(isappend)then
		...
		endif !if(isnrowtot)  16693
		ibas2=0
	endif !if(isappend)  16681
...
700		if(obsw(level0).eq.nobsw(level0))then
		!		if(p)write(6,*)'level0',level0,obsw(level0),nobsw(level0)
			...
			endif !700		if(obsw(level0).eq.nobsw(level0))  16721
			...
			do ile=level0,ndata
				...
				endif !if(ile.eq.2)  16750
				iba(ile)=iba(ile)+nkeep(ile)
			enddo !ile=level0,ndata  16749
		...
		do i=1,nobsup
			...
			do j=1,nobo
				...
				ibas2=ibas2+nkeep(2)
			enddo !j=1,nobo  16782
			ibas=ibas+nkeep(1)
		enddo !i=1,nobsup  16779
 
 
 
 
	endif !if(.not.istwo)  16713
	...
	if(.not.istwo)then
		...
		enddo !i=1,nobstop  16837
 
		call j_linkdata(datat(1),ivout)
	endif !if(.not.istwo)  16836
	...
	do i=1,nperiod
		...
		do j=1,nkeepper(i)
			...
			if(ikeep(i,j).le.0)then
				...
				j_err=.true.;return
			endif !if(ikeep(i,j).le.0)  16957
		enddo !j=1,nkeepper(i)  16954
		!write(6,*)'period,keep',i,ikeep(i,1:nkeepper(i))
		nvar=nvar+nkeepper(i)
	enddo !i=1,nperiod  16951
	...
	if(nvar.ne.nkeep)then
		...
		do k=1,nkeep
			...
			do i=1,nperiod
				...
				enddo !j=1,nkeepper(i)  16976
 
			enddo !i=1,nperiod  16975
			if(lkm.ne.1)then
				call j_getname(iv)
				write(6,*)j_oname(1:j_loname),' appears ',lkm, ' times in periodvars'
 
			endif !if(lkm.ne.1)  16982
		enddo !k=1,nkeep  16972
		j_err=.true.
	endif !if(nvar.ne.nkeep)  16970
 
 
	if(j_err)then
		deallocate(ikeep)
		return
	endif !if(j_err)  16992
...
ploop:	do i=1,nperiod
				...
				do j=1,nkeepper(i)
					...
					if(j_o(ivmatrix)%d(ibas+ikeep(i,j)).ne.j_o(ivmatrix)%d(ibas0+ikeep(i,j))) then
						...
						goto 567
					endif !if(j_o(ivmatrix)%d(ibas+ikeep(i,j)).ne.j_o(ivmatrix)%d(iba  17050
				enddo !j=1,nkeepper(i)  17045
 
			enddo ploop !op:	do i=1,nperiod  17042
...
567			ibas0=ibas
		...
		enddo iobloop !loop:		do iobsw=2,nobsw  17033
		!	endif !if(nobsw.eq.1)  15415
	enddo !iunit=1,nobsup  17015
	...
	do iunit=1,nobsup
		...
		do j=1,nobsw
			...
			end do !ip=iperiod(j_iobs),nperiod  17145
			ibas=ibas+nkeep
		enddo !j=1,nobsw  17139
		ibasup=ibasup+nkeepup
	enddo !iunit=1,nobsup  17130
...
subroutine partdata(iob,io)
	...
	if(ivout.eq.j_divdata)then
		...
		call j_del(ivout)
		same=.true.
	endif !if(ivout.eq.j_divdata)  17238
	...
	if(nkeep.gt.0.and.ndrop.gt.0)then
		...
		j_err=.true.;return
	endif !if(nkeep.gt.0.and.ndrop.gt.0)  17249
...
loop:		do i=1,j_dnkeep
			...
			enddo !j=1,ndrop  17264
			nk=nk+1
			j_o(ivkeep)%i2(nk)=j_dkeep(i)
 
		enddo loop !p:		do i=1,j_dnkeep  17263
	else
		nkeep=j_dnkeep
		ivkeep=j_deflist(ivout,'%keep',ivin=j_divkeep)
 
 
	endif !if(nkeep.gt.0)  17254
	...
	do j_iobs=j_dfrom,j_duntil
		...
		if(j_rejected)cycle
		no8=no8+1_8
	end do !j_iobs=j_dfrom,j_duntil  17282
	nkeep8=nkeep
	...
	do j_iobs=j_dfrom,j_duntil
		...
		ibas8=ibas8+nkeep8
	end do !j_iobs=j_dfrom,j_duntil  17294
	...
	if(j_linkoption(iob,io,j_many).gt.0)then
		...
		do j=1,j_o(ivlist)%i(3)
			if(j_o(ivlist)%d(j).ge.valu)then
				j_v(iout)=j
				return
			endif !if(j_o(ivlist)%d(j).ge.valu)  17477
		enddo !j=1,j_o(ivlist)%i(3)  17476
		j_v(iout)=j_0
		...
		do j=1,j_o(ivlist)%i(3)
			if(valu.eq.j_o(ivlist)%d(j))then
				j_v(iout)=j
				return
			endif !if(valu.eq.j_o(ivlist)%d(j))  17485
		enddo !j=1,j_o(ivlist)%i(3)  17484
		j_v(iout)=j_0
	endif !if(j_linkoption(iob,io,j_many).gt.0)  17474
...
subroutine askc(iob,io) ! %%io
	...
	if(ivexit.gt.0)then
		if(j_otype(ivexit).ne.j_ipchar)then
			...
			j_err=.true.
			return
		endif !if(j_otype(ivexit).ne.j_ipchar)  17544
		call j_getchar(ivexit,exitc,lex)
	endif !if(ivexit.gt.0)  17543
	...
	do i=1,max(narg,1)
		if(narg.le.0)then
			...
			if(j_otype(iv).eq.j_ipchar)then
				...
				endif !if(j_o(iv)%i(3).eq.0)  17558
 
			endif !if(j_otype(iv).eq.j_ipchar)  17557
		end if !if(narg.le.0)  17553
		...
		end if !if(j_linkoption(iob,io,j_mq).le.0.or.j_ipc(j_o(iob)%i(j_li  17567
		lef=0
		if(lindef.gt.0)then
			...
			end if !if(j_otype(ide).eq.j_ipchar)  17577
		end if !if(lindef.gt.0)  17575
		if(lef.gt.0)then
			if(ivexit.gt.0)then
		...
		if(line_(2:).eq.' '.or.nc.le.0)then
			...
			end if !if(line_(2:2).eq."'")  17604
			if(ivexit.gt.0)then
				if(line_(nc1+1:nc-1).eq.exitc(1:lex))then
					write(6,*)'**exit from askc due to exit->'
					j_err=.true.
					return
				endif !if(line_(nc1+1:nc-1).eq.exitc(1:lex))  17611
			endif !if(ivexit.gt.0)  17610
			ivchar=j_defchar(0,line_(nc1:nc))
			if(j_err) return
		end if !if(line_(2:).eq.' '.or.nc.le.0)  17597
		call j_asschar2(ivchar,iv)
	end do !i=1,max(narg,1)  17552
	900 if(j_err)return
	!!call j_clearoption(iob,io)  ! subroutine
	return
...
subroutine ask(iob,io)  !ask()
	...
	endif !if(noptarg.gt.0.and.noptarg.ne.narg)  17670
	...
	if(nchi2.ge.0)then
		...
		if(narg.lt.2)then
			...
			if(idf.le.0)then
				...
				j_err=.true.
				return
			endif !if(idf.le.0)  17798
			if(arg.lt.0)then
				write(6,*)'*cdf(arg,df,chi2->): arg is negative',arg
				j_err=.true.
				return
 
			endif !if(arg.lt.0)  17803
			arg2=idf
		endif !if(narg.lt.2)  17792
...
subroutine close_(iob,io)  ! close()
	...
	do i=1,narg
		...
		if(nu.le.0)then
			...
			! j_err=.true.
			return
		endif !if(nu.le.0)  17850
		!		write(6,*)'<9'
		call j_closeunit(nu)
		if(j_err)return
	end do !i=1,narg  17845
	return
...
subroutine read(iob,io)   !read()   %%io
	...
	if(nu.le.0.and.nu.ne.5)then
		...
		if(j_unitchar(nu).gt.0)then
			...
			j_err=.true.
			return
		endif !if(j_unitchar(nu).gt.0)  17988
		if(ivform.ne.j_ivdollar)call j_getchar2(ivform,j_form,lef)
		if(j_err)return
	end if !if(nu.le.0.and.nu.ne.5)  17975
	if(j_form(1:1).eq.'b')then
		if(j_form(lef:lef).eq.'n')then
			...
			endif !if(narg.eq.2)  18001
		endif !if(j_form(lef:lef).eq.'n')  17997
	elseif(j_form(1:1).eq.'B')then !if(j_form(1:1).eq.'b')then
		...
		endif !if(j_form(lef:lef).eq.'n')  18043
		...
		elseif(ivform.eq.j_ivdollar)then !if(j_form(lef:lef).eq.'n')then
			...
			endif !if(j_otype(ivarg3).eq.j_ipmatrix)  18057
		else !if(j_form(lef:lef).eq.'n')then
			write(6,*)'*read: illegal format ',j_form(1:lef)
			if(j_ninc.eq.1)write(6,*)' file remains open'
			j_err=.true.
			return
		endif !if(j_form(lef:lef).eq.'n')  18051
	end if !if(j_form(1:1).eq.'b')  17996
	900	continue
	return
...
subroutine write(iob,io)   !write( ) %%io
	...
	elseif(nargval.eq.2)then !if(nargval.eq.1)then
		...
		if(nrow1.lt.1.or.nrow2.lt.nrow1)then
			...
			j_err=.true.
			return
		endif !if(nrow1.lt.1.or.nrow2.lt.nrow1)  18264
	endif !if(nargval.eq.1)  18259
	...
	if(ivfile.eq.j_ivdollar)then
		...
		if(nu.le.0)then
			...
			endif !if(j_otype(ivform).eq.j_iptext)  18292
			if(j_err)return
			!nu=j_iounit(ivfile) !j_o(ivfile)%i(4)
			...
			if(j_unitchar(nu).lt.0)then
				...
				j_err=.true.
				return
			endif !if(j_unitchar(nu).lt.0)  18307
		end if !if(nu.le.0)  18290
	...
	end if !if(ivfile.eq.j_ivdollar)  18285
	...
	if(ivform.eq.j_ivdollar.or.ivform.eq.j_ivdollar2)then
		...
		if(narg.eq.1.and.j_otype(arg(1)).eq.j_iptext)then
			...
			endif !if(nu.eq.6)  18328
			return
		endif !if(narg.eq.1.and.j_otype(arg(1)).eq.j_iptext)  18327
		!		write(6,*)'hep'
		if(j_writevar(nu,narg,arg,ivform))return
 
	endif !if(ivform.eq.j_ivdollar.or.ivform.eq.j_ivdollar2)  18322
		...
		if(single)then
			do i=1,narg
				...
				if(j_otype(arg(i)).eq.j_ipreal)then
					...
					nle=nle+nel
				endif !if(j_otype(arg(i)).eq.j_ipreal)  18349
 
			enddo !i=1,narg  18346
			if(bin)then
			...
			do i=1,narg
				...
				if(j_otype(arg(i)).eq.j_ipreal)then
					...
					nle=nle+nel
				endif !if(j_otype(arg(i)).eq.j_ipreal)  18371
 
			enddo !i=1,narg  18368
			if(bin8)then
			...
			elseif(ivform.eq.j_ivdollar2)then
				...
				if(j_otype(ivform).eq.j_ipchar)then
					...
					j_err=.true. ;return
				endif !if(j_otype(ivform).eq.j_ipchar)  18387
 
 
				write(nu,j_form(1:lef),err=990)j_dvector(1:nle)
			endif !if(bin8)  18382
 
		endif !if(single)  18345
		return
	endif !if(narg.gt.1)  18343
...
800	if(ivform.eq.j_ivbuffer)then
		if(j_v(j_ivbuffer).eq.0.)then
				...
				do i=1,lenbu
					if(j_buffer(i:i).ne.' ')then
						...
						if(j0.gt.0.and.j_buffer(i1:i1).eq.' ')then
							...
							line_(j0:j0)=char(9)
						endif !if(j0.gt.0.and.j_buffer(i1:i1).eq.' ')  18415
 
						j0=j0+1
						line_(j0:j0)=j_buffer(i:i)
					endif !if(j_buffer(i:i).ne.' ')  18413
				enddo !i=1,lenbu  18412
				if(j0.gt.0)then
					if(line_(j0:j0).eq.'.')j0=j0-1
				endif !if(j0.gt.0)  18424
			...
			endif !if(tabform)  18409
		end if !if(j_v(j_ivbuffer).eq.0.)  18405
		return
	elseif(j_otype(ivform).eq.j_iptext.and.narg.eq.0)then !if(ivform.eq.j_ivbuffer)then
		...
		endif !if(nu.eq.6)  18439
		return
 
		! !		WRITE(6,*)'<J331NU,ITE,0,.TRUE',nu,ite
		! call j_writetext(nu,0,0,.true.)
		! return
	end if !800	if(ivform.eq.j_ivbuffer)  18404
	if(narg.le.0)then
		if(j_otype(ivform).eq.j_ipchar)then
			...
			goto 990
		endif !if(j_otype(ivform).eq.j_ipchar)  18452
	endif !if(narg.le.0)  18451
	...
	if(narg.eq.1.and.j_otype(irg).eq.j_ipmatrix)then
		...
		if(ivform.eq.j_ivdollar2)then
			if(ncols.eq.1)then
				...
				enddo !ir=nrow1c,nrow2c  18512
				!endif !if(single)then
			endif !if(ncols.eq.1)  18504
			return
		elseif(ivform.eq.j_ivdollar)then
			...
			if(ncols.eq.1)then
				...
				enddo !ir=nrow1c,nrow2c  18528
				!endif !if(single)then
			endif !if(ncols.eq.1)  18520
			return
		elseif(bin)then !if(ivform.eq.j_ivdollar)then
			...
			if(ncols.eq.1)then
				...
				do ir=nrow1c,nrow2c
					write(nu,err=990)real(j_o(irg)%d((ir-1)*ncols+1:ir*ncols))
				enddo !ir=nrow1c,nrow2c  18539
			endif !if(ncols.eq.1)  18536
			!endif !if(single)then
			return
		elseif(bin8)then !if(ivform.eq.j_ivdollar)then
			if(ncols.eq.1)then
				...
				do ir=nrow1c,nrow2c
					write(nu,err=990)j_o(irg)%d((ir-1)*ncols+1:ir*ncols)
				enddo !ir=nrow1c,nrow2c  18549
				return
			endif !if(ncols.eq.1)  18546
		...
		elseif(j_otype(ivform).eq.j_ipchar)then !if(ivform.eq.j_ivdollar)then
			...
			if(ncols.eq.1)then
				...
				enddo !ir=nrow1c,nrow2c  18561
			endif !if(ncols.eq.1)  18557
			return
			!endif !if(single)then
		endif !if(ivform.eq.j_ivdollar2)  18503
		ismat=.true.
	else !if(narg.eq.1.and.j_otype(irg).eq.j_ipmatrix)then
		ismat=.false.
		!	nval=narg-2
	endif !if(narg.eq.1.and.j_otype(irg).eq.j_ipmatrix)  18489
	...
	if(bin8.and.narg.gt.0)then
		...
		return
	endif !if(bin8.and.narg.gt.0)  18605
	...
	elseif(ivform.eq.j_ivdollar2)then
		...
		end if !if(ivfile.ne.j_ivbuffer)  18618
		...
		if(j_form(1:1).eq.'(')then
					...
					do i=1,lenbu
						if(line_(i:i).ne.' ')then
							...
							if(j0.gt.0.and.line_(i1:i1).eq.' ')then
								if(line_(j0:j0).ne.'.')j0=j0+1
								line_(j0:j0)=char(9)
							endif !if(j0.gt.0.and.line_(i1:i1).eq.' ')  18638
							j0=j0+1
							line_(j0:j0)=line_(i:i)
						endif !if(line_(i:i).ne.' ')  18636
					enddo !i=1,lenbu  18635
					if(j0.gt.0)then
						if(line_(j0:j0).eq.'.')j0=j0-1
					endif !if(j0.gt.0)  18646
					!	if(nu.ne.6)call j_toutf8(line_(1:max(j0,1)))
					!	write(6,*)'>12' ,line_(1:max(j0,1))
					write(nu,'(a)',err=990)line_(1:max(j0,1))
				else !if(tabform)then
			...
			do j=1,narg,2
				...
				end if !if(abs(itab).gt.80)  18674
				iv=arg(j)
				if(j_otype(iv).eq.j_ipreal)then
					...
					end if !if(wform.and.nint(100.*tab).ne.100*itab)  18680
					...
					j_err=.true.;return
				end if !if(j_otype(iv).eq.j_ipreal)  18678
			...
			if(ivfile.ne.j_ivbuffer)then
					...
					do i=1,lel
						if(line_(i:i).ne.' ')then
							...
							if(j0.gt.0.and.line_(i1:i1).eq.' ')then
								if(line_(j0:j0).ne.'.')j0=j0+1
								line_(j0:j0)=char(9)
							endif !if(j0.gt.0.and.line_(i1:i1).eq.' ')  18721
							j0=j0+1
							line_(j0:j0)=line_(i:i)
						endif !if(line_(i:i).ne.' ')  18719
					enddo !i=1,lel  18718
					if(j0.gt.0)then
						if(line_(j0:j0).eq.'.')j0=j0-1
					endif !if(j0.gt.0)  18729
					write(nu,'(a)',err=990)line_(1:max(j0,1))
				else !if(tabform)then
					write(nu,'(a)',err=990)line_(1:lel)
				endif !if(tabform)  18715
			else !if(ivfile.ne.j_ivbuffer)then
				if(wform)then
					leb=j_v(j_ivbuffer)
				...
				endif !if(wform)  18737
			end if !if(ivfile.ne.j_ivbuffer)  18714
		else !if(j_form(1:1).eq.'(')then
			write(6,*)'**illegal format in write';j_err=.true.
		end if !if(j_form(1:1).eq.'(')  18629
	end if !if(ivform.eq.j_ivdollar)  18611
	900 continue !   if(j_err)return
...
subroutine elementprod(iob,io)
	...
	do i=1,2
		...
		if(j_otype( irg(i)).eq.j_ipmatrix)then
			...
			if(j_otype( irg(i)).eq.j_ipreal)then
				...
				j_err=.true.
				return
			endif !if(j_otype( irg(i)).eq.j_ipreal)  18779
		end if !if(j_otype( irg(i)).eq.j_ipmatrix)  18775
	end do !i=1,2  18773
	...
	if(ndim1(1).eq.0.and.ndim1(2).eq.0)then
		...
		j_err=.true.
	end if !if(ndim1(1).eq.0.and.ndim1(2).eq.0)  18791
...
subroutine t(iob,io) ! t(mat)  transpose  %%matrix
	...
	if(j_otype(irg).ne.j_ipmatrix)then
		...
		endif !if(j_otype(irg).eq.j_ipreal)  18832
		return
	end if !if(j_otype(irg).ne.j_ipmatrix)  18831
	...
	if(irg.eq.iout)then
		...
		do i=1,ndim1
			...
			enddo !j=1,ndim2  18853
		enddo !i=1,ndim1  18852
...
subroutine inverse(iob,io)  !inverse(mat)
	...
	if(j_otype(irg).ne.j_ipmatrix)then
		if(j_otype(irg).eq.j_ipreal)then
			...
			write(6,*)'#type is:',j_otypes( j_otype(irg))
			j_err=.true. ;return
		end if !if(j_otype(irg).eq.j_ipreal)  18920
	end if !if(j_otype(irg).ne.j_ipmatrix)  18919
	...
	if(ndim1.ne.ndim2)then
		...
		j_err=.true. ; return
	end if !if(ndim1.ne.ndim2)  18932
	if(j_o(irg)%i(4).eq.j_matdiag)then
		...
		do i=1,ndim1
			if(j_o(irg)%d(ibas+i).eq.j_0)then
				...
				j_err=.true.;return
			endif !if(j_o(irg)%d(ibas+i).eq.j_0)  18940
			j_o(iout)%d(ibas+i)=j_1/j_o(irg)%d(ibas+i)
			ibas=ibas+ndim1
		enddo !i=1,ndim1  18939
		return
	endif !if(j_o(irg)%i(4).eq.j_matdiag)  18936
	...
	do i=1,ndim1
		...
		enddo !j=1,ndim1  18959
		rhs_(i,i)=1.d0
	enddo !i=1,ndim1  18958
	...
	do i=1,ndim1
		...
		enddo !j=1,ndim1  18975
	enddo !i=1,ndim1  18974
	...
	if(ivout.le.j_named)then
		...
		enddo !i=1,ndim1*ndim1  18985
		j_v(ivnorm)=sqrt(j_dapu)*sqrt(j_dapu2)
	endif !if(ivout.le.j_named)  18981
...
subroutine qr(iob,io)
	...
	if(j_otype(irg).ne.j_ipmatrix)then
		...
		write(6,*)'#type is:',j_otypes( j_otype(irg))
		j_err=.true. ;return
	end if !if(j_otype(irg).ne.j_ipmatrix)  19028
	...
	do i=1,ndim1
		...
		enddo !j=1,ndim2  19038
	enddo !i=1,ndim1  19037
	...
	do i=1,narg
		...
		do j=1,i-1
			if(j_arg(i).eq.j_arg(j))then
				j_yes=.true.
				exit
			endif !if(j_arg(i).eq.j_arg(j))  19104
 
		enddo !j=1,i-1  19103
		if(.not.j_yes)nn=nn+1
 
	end do !i=1,narg  19101
	...
	do i=1,narg
		...
		do j=1,i-1
			if(j_arg(i).eq.j_arg(j))then
				j_yes=.true.
				exit
			endif !if(j_arg(i).eq.j_arg(j))  19119
		...
		endif !if(.not.j_yes)  19125
	end do !i=1,narg  19116
	j_o(iout)%i(1)=nn
...
subroutine print(iob,io)  !print-function  %%io  print()
	...
	if(narg.eq.1)then
		if(j_otype(arg(1)).eq.j_iplist)then
			...
			endif !if(j_linkoption(iob,io,j_mexpand,clear=.true.).ge.0)  19314
 
		endif !if(j_otype(arg(1)).eq.j_iplist)  19311
	endif !if(narg.eq.1)  19310
	...
	endif !if(j_linkoption(iob,io,j_moptions,clear=.true.).ge.0)  19324
	...
	endif !if(noptargf>0.and.j_optarg0(1).ne.j_ivdollar)  19335
	...
	if(j_linkoption(iob,io,j_mfunctions).ge.0)then   !print available functions
		...
		do i=1,j_nfunctions
			...
			if(icur.eq.3.or.i.eq.j_nfunctions_.or.i.eq.j_nfuncs1.or. &
				...
				j_cline=' '
				icur=0
			endif !if(icur.eq.3.or.i.eq.j_nfunctions_.or.i.eq.j_nfuncs1.o  19358
			select case(i)
		...
		enddo !i=1,j_nfunctions  19352
		!!call j_clearoption(iob,io)  ! subroutine
		!goto 900
	endif !if(j_linkoption(iob,io,j_mfunctions).ge.0)  19344
	...
	if(j_linkoption(iob,io,j_mmaxlines).gt.0)then
		...
		maxlines=j_v(j_ivmaxlines)
	end if !if(j_linkoption(iob,io,j_mmaxlines).gt.0)  19415
	...
	do j=1,narg  ! o(iob)%i(io+1)
		...
		if(j_otype(iv).gt.j_ipreal)then  !vois muuttaa otypet
			...
			elseif(j_otype(iv).eq.j_ipmatrix)then
				...
				do i=1,min(maxlines,j_o(iv)%i(1))
					if(noptargf.gt.0)then
						...
						endif !if(isrow)  19489
					endif !if(noptargf.gt.0)  19481
					iba=iba+j_o(iv)%i(2)
 
				end do !i=1,min(maxlines,j_o(iv)%i(1))  19480
				...
				do jj=1,min(j_o(iv)%i(1),maxlines)
					...
					do kk=j_o(iv)%i(3),min(j_o(iv)%i(2),79+j_o(iv)%i(3))
						...
						if(j_ibittest(iv,jj,kk).ne.0)then
							...
							j_cline(ic:ic)='0'
						end if !if(j_ibittest(iv,jj,kk).ne.0)  19609
					end do !kk=j_o(iv)%i(3),min(j_o(iv)%i(2),79+j_o(iv)%i(3))  19607
					write(nu_,'(a)')j_cline(1:ic)
				enddo !jj=1,min(j_o(iv)%i(1),maxlines)  19605
			...
			elseif(j_otype(iv).eq.j_ipilist)then !if(iotype.eq.j_ipmatrix)then
				...
				endif !if(j_o(iv)%i(2).ne.1)  19630
			...
			end if !if(j_otype(iv).eq.j_iptxt)  19462
		else !if(iotype.gt.j_ipreal)then
			if(j_otype(iv).eq.j_ipreal)then
				call j_getname(iv)
				write(nu_,*)' '
				write(nu_,*)j_oname(1:j_loname),'=',j_v(arg(j))
			end if !if(j_otype(iv).eq.j_ipreal)  19692
		end if !if(j_otype(iv).gt.j_ipreal)  19461
	end do !j=1,narg  19433
...
subroutine asschar(iob,io) ! %%char assign character variable
	...
	endif !if(j_otype(iout).ne.j_ipchar)  19756
...
subroutine data(iob,io)  !data(
	...
	use jmod, only: j_mcontinue
	use jmod, only: j_makelistvar
	use jmod, only: j_iscode
	use jmod, only: j_mcode
	use jmod, only: j_mtime
	use jmod, only: j_iobs
	...
	use jmod, only: j_mreject
	use jmod, only: j_eof
	use jmod, only: j_mcase
	use jmod, only: j_caseread
	use jmod, only: j_ivcase
	use jmod, only: j_nlinesnu
	...
	! code-> option to indicate that interpreation is need for getting values. If there is sparse-> or case->
	...
	if(.not.continue)then
		...
		if(p)write(6,*)'nu,ivform,nobs',nu,ivform,nobs
		if(j_err)return
		!	write(6,*)'nobs',nobs,nu,ivform
		isnobs=nobs.gt.0
	endif !if(.not.continue)  20275
	...
	if(ivform.gt.j_predefined)then
		...
		if(form(1:1).ne.'('.or.form(il:il).ne.')')then
			...
			j_err=.true.;return
		endif !if(form(1:1).ne.'('.or.form(il:il).ne.')')  20294
		!		ivform=ivformfortran
	endif !if(ivform.gt.j_predefined)  20290
	...
	if(.not.isnobs)then !can come from getin(
		...
		if(isnobs)then
			...
			if(nobs.le.0)then
				...
				j_err=.true. ;return
			endif !if(nobs.le.0)  20354
		else !if(isnobs)then
			nobs=10**9  !integer*8
		endif !if(isnobs)  20351
	endif !if(.not.isnobs)  20346
	...
	endif !if(.not.up.and.isgaya)  20369
	!*********************
	isdupl=.false.
	if(up)then
		...
		if(narg.gt.0)then
			...
			if(ivduplicate.le.0)then
				...
				j_err=.true. ;return
			end if !if(ivduplicate.le.0)  20426
	...
	if(nkeep.gt.0)then
		...
		if(ndol.gt.0)then
			...
			iskeep=.false.
		endif !if(ndol.gt.0)  20520
	endif !if(nkeep.gt.0)  20480
	...
	if(iskeep)then
		...
		do i=1,j_nread
			...
			if(ik.gt.0)then
				...
				nokeep=nokeep+1
			endif !if(ik.gt.0)  20548
		enddo !i=1,j_nread  20546
		if(isreadindex)then
			nki2=0
			do i=1,nkeep
				ik=j_inlist(j_keep(i),j_nread,j_readv)
				...
				endif !if(ik.gt.0)  20563
 
			enddo !i=1,nkeep  20561
 
 
 
		endif !if(isreadindex)  20559
	...
	elseif(noptarg.gt.0)then
		...
		if(j_caseread.le.0)then
			...
			j_err=.true.;return
		endif !if(j_caseread.le.0)  20622
	endif !if(noptarg.eq.0)  20612
	...
	endif !if(nu.gt.0.and.(j_caseread.gt.0.or.j_iscode))  20634
	!	write(6,*)'isno',isnobs,iout,'%'//j_oname(1:j_loname),'nobbs',nobs
	if(j_caseread.gt.0)then
		nobs4=nobs
		j_ivcaselist=j_deflist(iout,'%'//j_oname(1:j_loname),nres=nobs4)
 
 
	endif !if(j_caseread.gt.0)  20641
	...
	if(.not.isnobs)then
		...
		if(ivdefn.gt.0)then
			...
			nobsb=10000
		end if !if(ivdefn.gt.0)  20656
		allocate(tail%rbuf(1:nobsb*nkeep))
		iba=0
 
	endif !if(.not.isnobs)  20651
	...
	endif !if(ivform.eq.j_ivb.or.ivform.eq.j_ivdi.or.ivform.eq.j_ivdg  20666
	...
	if(nhead.eq.1)then
		...
		if(nhead.lt.1)then
			...
			j_err=.true.;return
		endif !if(nhead.lt.1)  20693
	elseif(nhead.eq.0)then
		nhead=1
 
 
	endif !if(nhead.eq.1)  20690
	!	else
	!		rfheadlink=999999
	...
	if(ncode.eq.1)then
		...
		if(ncode.lt.1)then
			...
			j_err=.true.;return
		endif !if(ncode.lt.1)  20716
	elseif(ncode.eq.0)then
		ncode=1
	endif !if(ncode.eq.1)  20713
...
456		do ilin=1,ncode+nhead
		if(ivform.eq.ivformd.or.ivform.eq.ivformb)then
			...
			j_err=.true.;return
		endif !if(ivform.eq.ivformd.or.ivform.eq.ivformb)  20731
		...
		if(rfheadlink.gt.rfcodelink.and.nhead.gt.0)then
			...
			if(j_err)then
				...
				return
			endif !if(j_err)  20746
			ncode=ncode-1
			if(ncode.eq.0)rfcodelink=0
		endif !if(rfheadlink.gt.rfcodelink.and.nhead.gt.0)  20739
 
	enddo !		do ilin=1,ncode+nhead  20730
	...
	endif !if(ivform.eq.j_ivb)  20795
	...
	if(j_ismakelist)then
		...
		endif !if(j_isnobs)  20834
 
	endif !if(j_ismakelist)  20832
	j_needsall=.true.
	...
	endif !if(.not.istrans.and..not.isgaya.and..not.up)  20866
	if(nu.le.0)then
...
543 deallocate(j_sparsen,j_sparsekeep,j_sparseval)
	...
	if(.not.isnobs)then
		...
		do i=1,nbuf
			...
			nullify(tail%pnext)
			tail=>ptr
		end do !i=1,nbuf  20982
	endif !if(.not.isnobs)  20954
	...
	if(isnobs.and.nobs>j_iobs)then
		...
		endif !if(j_iobs.eq.0)  20993
 
	endif !if(isnobs.and.nobs>j_iobs)  20992
	...
	if(.not.j_err)then
		...
		if(j_dlastdata.eq.iout)then
			j_dlastdata=0
			j_dlastdata=0
		endif !if(j_dlastdata.eq.iout)  21030
	endif !if(.not.j_err)  21011
	!	j_o(j_ivlastdata)%i(1)=1;j_o(j_ivlastdata)%i2(1)=iout
	...
	subroutine putlinked()
		...
		if(iobc.gt.nobsb)then !store in chained list
			...
			endif !if(associated(tail%pnext))  21063
...
!		write(6,*)'putli',iba+1,iba+nkeep,nkeep,j_keepvec
	...
	subroutine fast()
				...
				if(isreadindex)then
					if(isnobs)then
						...
						do j_iobs=1,nobs
							...
							call putlinked()
						enddo !j_iobs=1,nobs  21144
 
					endif !if(isnobs)  21134
					...
					if(isnobs)then
						...
						do j_iobs=1,nobs
							...
							call putlinked()
						enddo !j_iobs=1,nobs  21162
 
					endif !if(isnobs)  21153
				endif !if(isreadindex)  21133
			else !iskeep
				if(isnobs)then
					do j_iobs=1,nobs
						read(nu,err=957,end=958)j_readvecsing  !(1:nkeep(1))
					...
					do j_iobs=1,nobs
						...
						call putlinked()
					enddo !j_iobs=1,nobs  21180
 
				endif !if(isnobs)  21171
 
			endif !if(iskeep)  21132
				...
				if(isreadindex)then
					if(isnobs)then
						...
						do j_iobs=1,nobs
							...
							call putlinked()
						enddo !j_iobs=1,nobs  21206
 
					endif !if(isnobs)  21196
					...
					if(isnobs)then
						...
						do j_iobs=1,nobs
							...
							call putlinked()
						enddo !j_iobs=1,nobs  21224
 
					endif !if(isnobs)  21215
				endif !if(isreadindex)  21195
			else !iskeep
				if(isnobs)then
					do j_iobs=1,nobs
						read(nu,err=957,end=958)j_o(ivmat)%d(ibas+1:ibas+nkeep0)
						ibas=ibas+nkeep
					enddo !j_iobs=1,nobs  21234
					j_iobs=nobs;return
				else !j_iobss
					...
					do j_iobs=1,nobs
						...
						call putlinked()
					enddo !j_iobs=1,nobs  21241
 
				endif !if(isnobs)  21233
 
			endif !if(iskeep)  21194
		else if(ivform.eq.j_ivdollar)then
			...
			if(iskeep)then
				if(isreadindex)then
					if(isnobs)then
						...
						do j_iobs=1,nobs  !nobs i huge
							...
							call putlinked()
						enddo !j_iobs=1,nobs  21268
						!	if(j_isgetinp)call j_exitpara()
					endif !if(isnobs)  21253
					...
					if(isnobs)then
						...
						do j_iobs=1,nobs
							...
							!	if(j_iobs.eq.1)write(6,*)'k_vec',j_keepvec(1:nkeep0)
							call putlinked()
						enddo !j_iobs=1,nobs  21298
						!	if(j_isgetinp)call j_exitpara()
					endif !if(isnobs)  21284
				endif !if(isreadindex)  21252
				...
				if(isnobs)then
					...
					do j_iobs=1,nobs
						...
						call putlinked()
					enddo !j_iobs=1,nobs  21326
					!	if(j_isgetinp)call j_exitpara()
				endif !if(isnobs)  21314
				!
			endif !if(iskeep)  21251
		else if(isff)then
				...
				if(isreadindex)then
					if(isnobs)then
						...
						do j_iobs=1,nobs
							...
							j_keepvec(j_readindex)=j_readvec(j_keepindex) !(1:nkeep(1))
							call putlinked()
						enddo !j_iobs=1,nobs  21352
 
					endif !if(isnobs)  21342
					...
					if(isnobs)then
						...
						do j_iobs=1,nobs
							...
							j_keepvec(1:nkeep0)=j_readvec(j_keepindex) !(1:nkeep(1))
							call putlinked()
						enddo !j_iobs=1,nobs  21372
 
					endif !if(isnobs)  21363
				endif !if(isreadindex)  21341
			else !iskeep
				if(isnobs)then
					do j_iobs=1,nobs
						read(nu,form(1:lenform),err=957,end=958)j_o(ivmat)%d(ibas+1:ibas+nkeep0)
						ibas=ibas+nkeep
					enddo !j_iobs=1,nobs  21384
					j_iobs=nobs;return
				else !j_iobss
					!write(6,*)'ncncnc',nobs
					do j_iobs=1,nobs
						...
						!write(6,*)'j_iobs',j_iobs,j_keepvec
						call putlinked()
					enddo !j_iobs=1,nobs  21391
 
				endif !if(isnobs)  21383
 
			endif !if(iskeep)  21340
		...
		endif !if(ivform.eq.j_ivb)  21131
		!	j_iobs=iob
...
958 		j_iobs=j_iobs-1
	...
	5 write(6,*)'error in line ',j_iobs,' ',j_inp(1:j_linp)
		...
		endif !if(nu.eq.0.and..false.)  21429
...
958		return  !	if(j_isgetinp)call j_exitpara()
	...
	5 write(6,*)'error in line ',j_iobs,' ',j_inp(1:j_linp)
			...
			do iu=1,nobsup
				...
				do ibs=1,nobsw
					...
					if(isnobs)then
						...
						call putlinked()
					endif !if(isnobs)  21599
				enddo !ibs=1,nobsw  21589
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21584
			!		return
		elseif(ivform.eq.j_ivb2)then
			...
			do iu=1,nobsup
				...
				do ibs=1,nobsw
					...
					if(isnobs)then
						...
						call putlinked()
					endif !if(isnobs)  21626
				enddo !ibs=1,nobsw  21616
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21611
			!		return
		else if(ivform.eq.j_ivdollar)then
			...
			do iu=1,nobsup
				...
				do ibs=1,nobsw
					...
					if(isnobs)then
						...
						call putlinked()
					endif !if(isnobs)  21659
				enddo !ibs=1,nobsw  21643
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21638
			!	if(j_isgetinp)call j_exitpara()
		else if(isff)then
			...
			do iu=1,nobsup
				...
				do ibs=1,nobsw
					...
					if(isnobs)then
						...
						call putlinked()
					endif !if(isnobs)  21689
				enddo !ibs=1,nobsw  21676
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21671
			...
			do iu=1,nobsup
				...
				do ibs=1,nobsw
					...
					if(isnobs)then
						...
						call putlinked()  !gaya
					endif !if(isnobs)  21721
				enddo !ibs=1,nobsw  21707
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21702
		...
		endif !if(nu.eq.0.and..false.)  21536
		...
		if(nu.eq.0.and..false.)then
			...
			do iu=1,nobsup
				...
				do ibs=1,nobsw
					...
					!write(6,*)'ibas,nkeep,j_iobs,',ibas,nkeep,j_iobs,j_keepvec
				enddo !ibs=1,nobsw  21765
				!write(6,*)'ibasup ibasup+nkeepup,nkeepup',ibasup,ibasup+nkeepup,nkeepup
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21761
			j_iobs=j_iobs+nobsw
			!write(6,*)'inpfin',ivup,iout,ibs, j_inp(1:j_linp),'j_iobs ',j_iobs
			call j_getinput(' ')  !/
			!call j_linkdata(ivup,ivsub)
	...
	!		call j_exitpara()
		...
		elseif(ivform.eq.j_ivb)then
			...
			do iu=1,nobsup
				...
				enddo !ibs=1,nobsw  21795
				j_iobs=j_iobs+nobsw
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21792
			!		return
		elseif(ivform.eq.j_ivb2)then
			...
			do iu=1,nobsup
				...
				enddo !ibs=1,nobsw  21814
				j_iobs=j_iobs+nobsw
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21811
			!		return
		else if(ivform.eq.j_ivdollar)then
			...
			do iu=1,nobsup
				...
				enddo !ibs=1,nobsw  21832
				j_iobs=j_iobs+nobsw
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21828
		else if(isff)then
			...
			do iu=1,nobsup
				...
				enddo !ibs=1,nobsw  21854
				j_iobs=j_iobs+nobsw
				ibasup=ibasup+nkeepup
			enddo !iu=1,nobsup  21850
		...
		endif !if(nu.eq.0.and..false.)  21759
...
958		return ! if(j_isgetinp)call j_exitpara()
	...
	subroutine v()
		...
		if(ivform.eq.j_ivb)then
			if(isnobs)then
				...
				do ibs=1,nobs
					...
					call putlinked()
				enddo !ibs=1,nobs  21919
 
 
 
			endif !if(isnobs)  21905
		...
		elseif(ivform.eq.j_ivb2)then
			if(isnobs)then
				...
				do ibs=1,nobs
					...
					j_keepvec=j_v(j_keep)
					call putlinked()
				enddo !ibs=1,nobs  21948
 
 
 
 
			endif !if(isnobs)  21935
			...
			if(isnobs)then
				...
				do ibs=1,nobs
					...
					call putlinked()
				enddo !ibs=1,nobs  21983
 
 
			endif !if(isnobs)  21965
		else if(isff)then
			if(isnobs)then
				do ibs=1,nobs
				...
				do ibs=1,nobs
					...
					j_keepvec=j_v(j_keep)
					call putlinked()
				enddo !ibs=1,nobs  22017
 
 
			endif !if(isnobs)  22005
		...
		endif !if(ivform.eq.j_ivb)  21904
		!	j_iobs=ib
	...
	subroutine transu()
		...
		if(ismaketrans)then
			...
			if(j_err)then
				...
				return
			endif !if(j_err)  22065
			j_keepvec=j_v(j_keep)
		end if !if(ismaketrans)  22061
		...
		if(isfilter)then
			...
			if(j_err)then
				...
				if(irecord.ne.j_iobs)write(6,*)'record ',irecord
				return
			endif !if(j_err)  22078
			!end if !if(j_v(j_ivfilter).eq.0.)then
		end if !if(isfilter)  22074
		if(isreject)then
		...
		if(isdupl)then
			...
			do id=0,ndupl
				...
				endif !if(id.lt.ndupl)  22130
			enddo !id=0,ndupl  22118
 
			j_keepvec=j_v(j_keep)  !(1:nkeep(1))
			!write(6,*)'j_iobs',j_iobs,j_keepvec
		endif !if(isdupl)  22102
...
subroutine stat_(iob,io)  ! stat(iob,io)
	...
	endif !if(j_v(j_ivprintinput).gt.0)  22250
	...
	endif !if(narg.eq.0.and..not.isfunc)  22300
	if(isfunc)narg=1
	xw=1.d0  !weight
	xws=0.d0
	j_iobs=0
	...
	if(missing)then
		...
		do j_iobs=j_dfrom,j_duntil
			...
			!				end do !do j=1,narg
		end do !j_iobs=j_dfrom,j_duntil  22361
	endif !if(missing)  22328
	...
	if(j_dprint.ne.0)then
		...
		endif !if(lirmse.lt.0)  22434
 
		write(j_form(6:7),'(i2)')maxlen+1
	end if !if(j_dprint.ne.0)  22425
	!	write(6,*)j_form(1:20)
	do j=1,narg
		...
		if(j_dprint.ne.0)then
			if(lirmse.lt.0)then
				...
				endif !if(missing.and.weisu.lt.xws)  22478
			else
			...
			endif !if(lirmse.lt.0)  22477
			if(j_err)then
				write(6,*)'*j* problem with variable ',iv
				return
			endif !if(j_err)  22496
 
		endif !if(j_dprint.ne.0)  22476
		if(iout.eq.j_ivresult)then
		...
		endif !if(iout.eq.j_ivresult)  22502
	end do !j=1,narg  22452
...
subroutine transdata(iob,io)
	...
	do iobs=j_dfrom,j_duntil
		...
		call j_getobs(iobs)
		if(j_err)return
	end do !iobs=j_dfrom,j_duntil  22588
	! else
	! call j_getdataobject(iob,io)
	! if(j_err)return
	! !!call j_clearoption(iob,io)  ! subroutine
...
subroutine find_d(iob,io)
	...
	endif !if(.not.(j_disfilter.or.j_disreject))  22634
	...
	do iobs=j_dfrom,j_duntil
		...
		endif !if(.not.j_rejected)  22649
	end do !iobs=j_dfrom,j_duntil  22645
...
subroutine corr(iob,io,itype)  !stat()
	...
	endif !if(j_otype(ivi).eq.j_iplist)  22738
	iout=j_o(iob)%i(io+2+narg)
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	...
	!call j_getdataset(j_datasets(kk),nobs)
		...
		do i=1,narg
			...
			do 4 j=1,i
				...
				4	   				continue !4 j=1,i  22796
		enddo !i=1,narg  22793
	enddo obloop !oop:		do ii=j_dfrom,j_duntil  22774
		...
		do i=1,narg
			...
			do j=1,i-1
				if(xc(i).gt.0..and.xc(j).gt.0.)then
					...
					cor=0.
				endif !if(xc(i).gt.0..and.xc(j).gt.0.)  22832
				j_o(iout)%d((i-1)*narg+j)=cor
				j_o(iout)%d((j-1)*narg+i)= cor
			enddo !j=1,i-1  22831
		enddo !i=1,narg  22827
	endif !if(.not.iscorr)  22818
	if(j_dprint.ge.1)then
		...
		if(j_o(iob)%i(io) .eq.212)then  !not correlation
			...
			enddo !i=1,narg  22852
 
 
 
		endif !if(j_o(iob)%i(io) .eq.212)  22844
 
 
	endif !if(j_dprint.ge.1)  22842
...
subroutine matrix(iob,io) ! matrix()
	...
	if(nva.gt.0)then
		...
		do iva=1,nva
			...
			endif !if(j_otype(j_optarg0(iva)).eq.j_ipmatrix)  23005
		enddo !iva=1,nva  23003
	endif !if(nva.gt.0)  22996
	...
	if(isinf)then
		if(nu.lt.0)then
			...
			ndim1=lines
		endif !if(nu.lt.0)  23040
	endif !if(isinf)  23039
	if(diag.and.matmat.and.narg.eq.0.and.nva.eq.1)then
		...
		do i=1,ndim
			...
			ibas=ibas+ndim
		enddo !i=1,ndim  23055
		return
 
	endif !if(diag.and.matmat.and.narg.eq.0.and.nva.eq.1)  23048
	...
	if(narg.eq.1.and.j_otype(arg(1)).eq.j_iplist)then
		...
		goto 70
	endif !if(narg.eq.1.and.j_otype(arg(1)).eq.j_iplist)  23066
	!******
	...
	elseif(narg.eq.2)then
		...
		if(diag.and.ndim1.ne.ndim2)then
			...
			j_err=.true.;return
		endif !if(diag.and.ndim1.ne.ndim2)  23101
	endif !if(matmat.and.narg.eq.0)  23090
	...
	if(narg.eq.0.and..not.matmat)then
		...
		elseif(ndo.ge.0)then !if(nva.le.0.and.ndo.eq.0)then
			if(ndo.lt.2)then
				...
				j_err=.true. ;goto 70
			endif !if(ndo.lt.2)  23167
			dostep=1.d0
			if(ndo.eq.3)dostep=j_v(doval(3))
			if(dostep.eq.0)then
				write(6,*)'do-step is zero'
				j_err=.true. ; goto 70
			endif !if(dostep.eq.0)  23173
			...
			if(diag)then
				...
				enddo !i=1,nval  23201
			endif !if(diag)  23186
			goto 70
		endif !if(nva.le.0.and.ndo.eq.0)  23143
 
	endif !if(narg.eq.0.and..not.matmat)  23142
	...
	if(narg.eq.2)then
		if(j_otype(arg(1)).eq.j_iplist)then
			...
			if(inde.lt.1.or.inde.gt.j_nrows(arg(1)))then
				...
				j_err=.true.;goto 70
			endif !if(inde.lt.1.or.inde.gt.j_nrows(arg(1)))  23217
			irg=j_o(arg(1))%i2(inde)
			if(j_otype(irg).eq.j_ipreal)then
				if(j_otype(iout).ne.j_ipreal)call j_del(iout)
				j_v(iout)=j_v(irg)
				goto 70
			endif !if(j_otype(irg).eq.j_ipreal)  23222
			if(j_otype(irg).ne.j_ipmatrix)then
				call j_printname('*not a matrix:',irg,' ')
				j_err=.true. ;goto 70
			endif !if(j_otype(irg).ne.j_ipmatrix)  23227
			nel=ndim1*ndim2
			!	if(p)write(6,*)'ioutbe ',iout,j_otype(iout)
			...
			!write(6,*)' '
			goto 70
		end if !if(j_otype(arg(1)).eq.j_iplist)  23214
	endif !if(narg.eq.2)  23213
	...
	if(.not.matmat)then
		...
		elseif(.not.diag.and.nva.gt.0.and.ndim1*ndim2.ne.nva.and..not.isany.and.nva.ne.1)then !if(diag.and.nva.gt.0.and.nva.ne.ndim1.and.nva.ne.1)then
			write(6,*)'nrows=',ndim1,' and ncols=',ndim2, 'and there are ',nva, &
				'values and no any->'
			j_err=.true.
		endif !if(diag.and.nva.gt.0.and.nva.ne.ndim1.and.nva.ne.1)  23243
		if(j_err)return
 
		iout=j_defmatrix8(iout,' ',ndim1,ndim2,itype)
		if(p)write(6,*)'defm ',ndim1,ndim2
	endif !if(.not.matmat)  23242
	...
	if(matmat)then
		...
		if(nva.ne.ndim1*ndim2.and..not.diag)then
			...
			j_err=.true. ;goto 70
		endif !if(nva.ne.ndim1*ndim2.and..not.diag)  23263
		!***********************
		if(diag)then
			nrows=0
			ivastep=1
			...
			do iro=1,ndim1
				...
				if(j_otype(ii).eq.j_ipreal)then
					...
					if(j_otype(ii).ne.j_ipmatrix)then
						...
						call printvalues()
						j_err=.true. ;goto 70
					endif !if(j_otype(ii).ne.j_ipmatrix)  23287
					niro=j_o(ii)%i(1)
					ncols=j_o(ii)%i(2)
					...
					endif !if(ncols.ne.niro)  23295
				endif !if(j_otype(ii).eq.j_ipreal)  23283
				nrows=nrows+niro
			enddo !iro=1,ndim1  23275
				...
				if(j_otype(ii).eq.j_ipreal)then
					...
					do ir0=1,niro
						do ir=1,niro
							...
							ie=ie+1
						enddo !ir=1,niro  23322
						!		write(6,*)'iro, ndim1,nrows,ibas',iro,ndim1,nrows,ibas
						ibas=ibas+nrows
					enddo !ir0=1,niro  23321
					ibas=ibas+niro
				endif !if(j_otype(ii).eq.j_ipreal)  23314
 
			enddo !iro=1,ndim1  23310
		...
		do iro=1,ndim1
			...
			if(j_otype(ii).eq.j_ipreal)then
				...
				if(j_otype(ii).ne.j_ipmatrix)then
					...
					call printvalues()
					j_err=.true. ;goto 70
				endif !if(j_otype(ii).ne.j_ipmatrix)  23347
				niro=j_o(ii)%i(1)
				ncols=j_o(ii)%i(2)
			endif !if(j_otype(ii).eq.j_ipreal)  23343
			nrows=nrows+niro
			do j2=2,ndim2
				...
				if(j_otype(ii).eq.j_ipreal)then
					...
					if(j_otype(ii).ne.j_ipmatrix)then
						...
						call printvalues()
						j_err=.true.;goto 70
					endif !if(j_otype(ii).ne.j_ipmatrix)  23364
					niro2=j_o(ii)%i(1)
					ncol=j_o(ii)%i(2)
				endif !if(j_otype(ii).eq.j_ipreal)  23360
				if(niro2.ne.niro)then
					...
					call printvalues()
					j_err=.true.;goto 70
				endif !if(niro2.ne.niro)  23373
				ncols=ncols+ncol
 
			enddo !j2=2,ndim2  23357
			if(iro.gt.1.and.ncols.ne.ncolsv)then
				write(6,*)'row ',iro,' does not have the same number of columns as previous '
				call printvalues()
				j_err=.true.;goto 70
			endif !if(iro.gt.1.and.ncols.ne.ncolsv)  23384
			ncolsv=ncols
 
		enddo !iro=1,ndim1  23340
		if(inout)then
			...
			do j2=1,ndim2
				...
				if(j_otype(ii).eq.j_ipreal)then
					...
					do iri=1,nrow0
						...
						enddo !jic=1,ncol0  23415
					enddo !iri=1,nrow0  23414
					ncols0=ncols0+ncol0
				endif !if(j_otype(ii).eq.j_ipreal)  23407
			enddo !j2=1,ndim2  23404
			nrows0=nrows0+nrow0
 
 
		enddo !iro=1,ndim1  23402
		if(inout)then
		...
		if(nva.eq.1)then  !constant value
			...
			if(diag)then
				...
				do i=1,min(ndim1,ndim2)
					j_o(iout)%d(iba)=val
					iba=iba+ndim2+1
				enddo !i=1,min(ndim1,ndim2)  23454
			else !if(diag)then
			...
			if(diag)then
				...
				do j=1,min(ndim1,ndim2,nva)
					...
					iba=iba+ndim2+1
				enddo !j=1,min(ndim1,ndim2,nva)  23466
			else !if(diag)then
	...
	elseif(nu.ge.0)then !if(nva.ge.1)then
		...
		endif !if(nu.eq.0.and.arg(1).eq.j_ivinf)  23480
		! ndim1=lin-lin0
			...
			if(diag)then
				...
				do lin=1,lines
					...
					if(j_err)then
						...
						goto 767
					endif !if(j_err)  23512
					leva=leva+nvar
					if(leva.ge.ndim1)exit
				enddo !lin=1,lines  23510
				...
				if(ndim2.eq.1.or.ndim1.eq.1)then
					...
					do lin=1,lines
						...
						if(j_err)then
							...
							return
						endif !if(j_err)  23539
						leva=leva+nvar
						!	write(6,*)ndi,lin,leva
						if(leva.ge.ndi)exit
					enddo !lin=1,lines  23537
					...
					do ii=1,min(ndim1b,ndim1)
						...
						leva=leva+ndim2
					enddo !ii=1,min(ndim1b,ndim1)  23551
					...
					endif !if(ndim1.lt.ndim1b)  23568
				endif !if(ndim2.eq.1.or.ndim1.eq.1)  23535
			endif !if(diag)  23502
			!	leva=iva2
...
767			if(nu.eq.0)call j_exitpara()
	...
	! 80		if(diag)then
			...
			return
		endif !if(nu.ge.0)  23500
		if(diag)then
			do i=1,ndim1
				j_o(iout)%d((i-1)*ndim1+i)=j_tempvector(i)
 
			enddo !i=1,ndim1  23606
	...
	if(ndo.eq.0)then !if(ndo.lt.0)then
		if(diag)then
			do i=1,ndim1
				j_o(iout)%d(ibas+i)=i
				ibas=ibas+ndim1
			enddo !i=1,ndim1  23654
 
		else !if(diag)then
			do i=1,ndim1*ndim2
				j_o(iout)%d(i)=i
			enddo !i=1,ndim1*ndim2  23660
	...
	elseif(ndo.eq.1)then !if(ndo.lt.0)then
		if(diag)then
			do i=1,ndim1
				...
				ibas=ibas+ndim1
			enddo !i=1,ndim1  23667
		else !if(diag)then
			do i=1,ndim1*ndim2
				j_o(iout)%d(i)=i+j_v(doval(1))-1
			enddo !i=1,ndim1*ndim2  23672
		endif !if(diag)  23666
	elseif(ndo.eq.2)then !if(ndo.lt.0)then
		...
		if(diag)then
			...
			do i=1,min(ndim1,iup)
				j_o(iout)%d(ibas+i)=i+j_v(doval(1))-1
				ibas=ibas+ndim1
			enddo !i=1,min(ndim1,iup)  23680
 
		else !if(diag)then
			do i=1,min(ndim1*ndim2,iup)
				j_o(iout)%d(i)=i+j_v(doval(1))-1
			enddo !i=1,min(ndim1*ndim2,iup)  23686
		endif !if(diag)  23678
	elseif(ndo.eq.3)then !if(ndo.lt.0)then
		...
		if(diag)then
			do i=1,ndim1
				...
				endif !if(j_v(doval(3)).lt.j_0)  23696
				ibas=ibas+ndim1
			enddo !i=1,ndim1  23693
			...
			do i=1,ndim1*ndim2
				...
				if(j_v(doval(3)).lt.j_0)then
					...
					if(docur.gt.j_v(doval(2))+1.0d-17)exit
				endif !if(j_v(doval(3)).lt.j_0)  23708
 
			enddo !i=1,ndim1*ndim2  23705
 
		endif !if(diag)  23692
 
	endif !if(ndo.eq.0)  23652
	...
	if(j_o(iout)%i(3).eq.1)then
		...
		call j_del(iout)
		j_v(iout)=docur
	endif !if(j_o(iout)%i(3).eq.1)  23725
	if(isreal)then
		docur=j_o(iout)%d(1)
		call j_del(iout)
		j_v(iout)=docur
	endif !if(isreal)  23730
	...
	subroutine printvalues()
		...
		do j=1,nva
			...
			endif !if(j_otype(ii).eq.j_ipmatrix)  23755
		enddo !j=1,nva  23752
		return
...
subroutine trans(iob,io)  ! define transformation set   output=trans()
	...
	if(nin.gt.0)then
		...
		if(j_otype(ivtext).ne.j_iptext)then
			...
			j_err=.true.;return
		endif !if(j_otype(ivtext).ne.j_iptext)  23957
		...
		endif !if(j_gottext(1:j_lgottext).ne.'/')  23963
	...
	if(nmask.gt.0)then
		...
		do i=1,nmask
			...
			if(nval.gt.0)then
				...
				end do !j=1,nval  24226
			else if(nval.eq.0)then !if(nval.gt.0)then
				nel2=nel2+1
			else !if(nval.gt.0)then
				nel2=nel2-nval
			end if !if(nval.gt.0)  24225
		end do !i=1,nmask  24223
		j_o(iv2)%i(1)=nel
...
loop:		do i=2,narg
	...
	if(ndo.ge.0)then !if(nva.le.0.and.ndo.eq.0)then
		...
		if(ndo.eq.0)then
			...
			enddo !i=1,nel  24269
			return
		endif !if(ndo.eq.0)  24268
		if(ndo.eq.1)then
			do i=1,nel
				j_v(j_o(iout)%i2(i))=i-1+j_v(j_optarg(1))
			enddo !i=1,nel  24275
			return
		endif !if(ndo.eq.1)  24274
		if(ndo.eq.2)then
			...
			enddo !i=1,nel  24283
			return
		endif !if(ndo.eq.2)  24280
 
 
		write(6,*)'do-> needs 0 ,1 , or 2 arguments'
		j_err=.true. ;return
	endif !if(ndo.ge.0)  24266
	!		write(6,*)'ndo',ndo,doval
...
subroutine stempolar(iob,io)  !
	...
	if(j_otype(ispl).ne.j_ipstemspline)then
		...
		j_err=.true.
		return
	end if !if(j_otype(ispl).ne.j_ipstemspline)  24343
	if(j_linkoption(iob,io,j_morigo).gt.0)then
		...
		endif !if(j_o(iob)%i(j_linkoption(iob,io,j_morigo)).ne.1)  24349
		origo=100.*j_v(j_o(iob)%i(j_linkoption(iob,io,j_morigo)+1) )
 
	else !if(j_linkoption(iob,io,j_morigo).gt.0)then
		origo=0.
	endif !if(j_linkoption(iob,io,j_morigo).gt.0)  24348
	...
	do 441 L=1,npo2-1
		...
		if(abs(j_o(ispl)%d(lir+L)-dref).lt.0.001)then
			j_v(iout)=dref
			goto 90
		endif !if(abs(j_o(ispl)%d(lir+L)-dref).lt.0.001)  24376
 
		if(j_o(ispl)%d(lir+L).le.dref)goto 221
 
		441       continue !441 L=1,npo2-1  24371
	...
	if(L.eq.1.and.j_o(ispl)%d(1).le.cotang*(j_o(ispl)%d(lirh)-origo))then
		...
		end if !if(r.gt.0.01.or.-r.gt.j_o(ispl)%d(lirh)-origo)  24441
		...
		end if !IF (R.LT.-0.01 .OR.r.gt.sero)  24453
	end if !if(L.eq.1.and.j_o(ispl)%d(1).le.cotang*(j_o(ispl)%d(lirh)-  24436
	...
	return
		...
		endif !if(j_otype(iverr).ne.j_iptrans)  24477
 
	endif !99 	   if(iverr.gt.0)  24476
	j_err=.true.
	return
...
subroutine stemspline(iob,io)     !
	...
	if(narg.eq.2)then
		...
		if(j_otype(ivh_).ne.j_ipmatrix.or.j_otype(ivd_).ne.j_ipmatrix)then
			...
			goto 90
		endif !if(j_otype(ivh_).ne.j_ipmatrix.or.j_otype(ivd_).ne.j_ipmat  24587
		npo=j_o(ivh_)%i(1)
		if(j_o(ivd_)%i(1).ne.npo)then
			write(6,*)'*stemspline:arguments have different dimensions'
			goto 90
		endif !if(j_o(ivd_)%i(1).ne.npo)  24592
		vec=.true.
	else !if(narg.eq.2)then
		if(test)then
			npo=0
			...
			do i=1,npobas
				if(j_v(j_o(iob)%i(io+1+i)).le.0.and.j_v(j_o(iob)%i(io+1+i+npobas)).le.0.)then
					npo=i-1
					goto 756
				endif !if(j_v(j_o(iob)%i(io+1+i)).le.0.and.j_v(j_o(iob)%i(io+1+i+  24602
			enddo !i=1,npobas  24601
			npo=narg/2
			756		continue
	...
	if(sort)then
		...
		if(allocated(iperm))then
			...
			endif !if(size(iperm).lt.npo)  24634
		else !if(allocated(iperm))then
			allocate(iperm(1:npo+1))
		endif !if(allocated(iperm))  24633
		ii=1
		do i=1,npo
			if(vec)then
		...
		do i=2,npo2
			...
			endif !if(j_o(iout)%d(npo+i)-j_o(iout)%d(npo+ii-1).lt.delh)  24667
			ii=ii+1
 
		enddo !i=2,npo2  24666
		...
		do 31 i=1,npo
			...
			if( j_o(iout)%r(ii+npo).lt.0..or.j_o(iout)%r(ii).lt.0.)then
				goto 311
			end if !if( j_o(iout)%r(ii+npo).lt.0..or.j_o(iout)%r(ii).lt.0.)  24701
			if(ii.gt.1)then
				if(j_o(iout)%r(npo+ii).lt.j_o(iout)%r(npo+ii-1))then
					...
					goto 90
				endif !if(j_o(iout)%r(npo+ii).lt.j_o(iout)%r(npo+ii-1))  24705
				if(j_o(iout)%r(npo+ii)-j_o(iout)%r(npo+ii-1).lt.delh)then
					...
					endif !if(i.lt.npo)  24716
					ii=ii-1
				end if !if(j_o(iout)%r(npo+ii)-j_o(iout)%r(npo+ii-1).lt.delh)  24715
			end if !if(ii.gt.1)  24704
			ii=ii+1
 
			31     continue !31 i=1,npo  24691
	...
	endif !if(iprint.gt.2.or.p)  24737
	...
	700    do i=1,npo2-1
		...
		if(testma-dma.gt.dif0)then
			...
			f=0.7*dmi+0.3*dma
			isok=0
		endif !if(testma-dma.gt.dif0)  24774
		if(p)then
		...
		if(isok.le.0.and..false.)then
			...
			goto 90
		endif !if(isok.le.0.and..false.)  24787
 
	enddo !    do i=1,npo2-1  24749
...
subroutine stemcurve(iob,io)     !
	...
	elseif(nharvest.eq.2)then
		...
		if(ind1.le.0.or.ind1.gt.j_dnkeep.or. &
			...
			j_err=.true.;return
		endif !if(ind1.le.0.or.ind1.gt.j_dnkeep.o  24875
	...
	elseif(narg.eq.1)then
		...
		if(j_otype(ivh_).ne.j_ipmatrix)then
			...
			j_err=.true.;return
		endif !if(j_otype(ivh_).ne.j_ipmatrix)  24887
	...
	elseif(narg.eq.2)then
		...
		if(j_otype(ivh_).ne.j_ipmatrix.or.j_otype(ivd_).ne.j_ipmatrix)then
			...
			j_err=.true.;return
		endif !if(j_otype(ivh_).ne.j_ipmatrix.or.j_otype(ivd_).ne.j_ipmat  24896
		npo=j_o(ivh_)%i(3)
		if(j_o(ivd_)%i(3).ne.npo)then
			write(6,*)'*stemcurve:arguments have different dimensions'
			j_err=.true.;return
		endif !if(j_o(ivd_)%i(3).ne.npo)  24901
		vec=.true.
	else !if(narg.eq.2)then
		if(test)then
			npo=0
			...
			do i=1,npobas
				if(j_v(j_o(iob)%i(io+1+i)).le.0.and.j_v(j_o(iob)%i(io+1+i+npobas)).le.0.)then
					npo=i-1
					goto 756
				endif !if(j_v(j_o(iob)%i(io+1+i)).le.0.and.j_v(j_o(iob)%i(io+1+i+  24911
			enddo !i=1,npobas  24910
			npo=narg/2
			756		continue
			!!write(6,*)'npo',npo,j_v(j_o(iob)%i(io+1+i:io+1+npo))
		else !if(test)then
	...
	if(j_otype(iout).eq.j_ipstemcurve)then
		if(j_o(iout)%i(2).ne.npo.or.j_o(iout)%i(3).ne.ih)then
			call j_del(iout)
			new=.true.
		endif !if(j_o(iout)%i(2).ne.npo.or.j_o(iout)%i(3).ne.ih)  24929
	else
		new=.true.
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	endif !if(j_otype(iout).eq.j_ipstemcurve)  24928
...
subroutine stemopt(iob,io)     !
	...
	double precision ::hs,stumpd,h0,h1,dapu,dapu2,dapu3,suhde,suhde0,saw0,sawcur,sawprev,sawopt
	double precision ::hcur,sawpot
	parameter(pi4=0.007853981852531433d0)
	integer,dimension(itot)::loglen,loglenopt
	integer,dimension(itot)::loglenc=0
	...
	if(j_otype(icalle).ne.j_ipstemcurve)then
		...
		j_err=.true.;return
	endif !if(j_otype(icalle).ne.j_ipstemcurve)  25019
	!ismax=j_isoption(iob,io,j_mmax)
	do i=2,narg
		if(j_otype(j_arg(i)).ne.j_ipreal)then
			call j_getname(j_arg(i))
			write(6,*)j_oname(1:j_loname),' is not REAL'
			j_err=.true.
		endif !if(j_otype(j_arg(i)).ne.j_ipreal)  25026
	enddo !i=2,narg  25025
	...
	if(j_otype(iout).ne.j_ipstemopt)then
		...
		!maksimoidaan sahatavraa maksimitukkipituuteen
		...
		iivnlump=j_getobject(iout,'%Nlump',j_ipreal)
		iivslump=j_defmatrix(iout,'%Slump',1,20,j_matreg)
		iivvlump=j_defmatrix(iout,'%Vlump',1,20,j_matreg)
		!maksimoidaan sahatavran ja tilavuuden suhdetta
		ivnlog2=j_getobject(iout,'%nlog2',j_ipreal)
		iivlenc2=j_defmatrix(iout,'%Lenc2',1,neltot,j_matreg)
		write(6,*)'iivlenc2ny',iivlenc2
		iivh2=j_defmatrix(iout,'%H2',1,neltot,j_matreg)
		iivd2=j_defmatrix(iout,'%D2',1,neltot,j_matreg)
		iiva2=j_defmatrix(iout,'%A2',1,neltot,j_matreg)
		iivv2=j_defmatrix(iout,'%V2',1,neltot,j_matreg)
		iivvc2=j_defmatrix(iout,'%Vc2',1,neltot,j_matreg)
		iivsaw2=j_defmatrix(iout,'%Saw2',1,neltot,j_matreg)
		iivsawc2=j_defmatrix(iout,'%Sawc2',1,neltot,j_matreg)
		iivinchip2=j_defmatrix(iout,'%Inchip2',1,neltot,j_matreg)
		iivinchipc2=j_defmatrix(iout,'%Inchipc2',1,neltot,j_matreg)
		iivoutchip2=j_defmatrix(iout,'%Outchip2',1,neltot,j_matreg)
		iivoutchipc2=j_defmatrix(iout,'%Outchipc2',1,neltot,j_matreg)
		iivf2=j_defmatrix(iout,'%F2',1,neltot,j_matreg)
		iivsyl2=j_defmatrix(iout,'%Syl2',1,neltot,j_matreg)
		iivsylc2=j_defmatrix(iout,'%Sylc2',1,neltot,j_matreg)
		iivnlump2=j_getobject(iout,'%Nlump2',j_ipreal)
		iivslump2=j_defmatrix(iout,'%Slump2',1,25,j_matreg)
		iivvlump2=j_defmatrix(iout,'%Vlump2',1,25,j_matreg)
		iivlen2=j_defmatrix(iout,'%Len2',1,neltot,j_matreg)
 
		!	iivvlump=j_defmatrix(iout,'%Vlump',1,20,j_matreg)
 
		! jvn=j_getobject(iout,'%jn',j_ipreal)
		! jvlen=j_defmatrix(iout,'%jlen',1,20,j_matreg)
		! jvv=j_defmatrix(iout,'%jv',1,20,j_matreg)
		! jvs=j_defmatrix(iout,'%js',1,20,j_matreg)
		! jvc=j_defmatrix(iout,'%jc',1,20,j_matreg)
		! !jvout=j_defmatrix(iout,'%jout',1,20,j_matreg)
		! jvpulp=j_defmatrix(iout,'%jpulp',1,20,j_matreg)
		! jvd=j_defmatrix(iout,'%jd',1,20,j_matreg) !top diam
		! jvbetter=j_getobject(iout,'%jbetter',j_ipreal)
 
 
 
		! ijvn=j_getobject(iout,'%ijn',j_ipreal)
		! ijvlen=j_defmatrix(iout,'%ijlen',1,20,j_matreg)
		! ijvv=j_defmatrix(iout,'%ijv',1,20,j_matreg)
		! ijvs=j_defmatrix(iout,'%ijs',1,20,j_matreg)
		! ijvc=j_defmatrix(iout,'%ijc',1,20,j_matreg)
		! !jvout=j_defmatrix(iout,'%jout',1,20,j_matreg)
		! ijvpulp=j_defmatrix(iout,'%ijpulp',1,20,j_matreg)
		! ijvd=j_defmatrix(iout,'%ijd',1,20,j_matreg) !top diam
		! ijvbetter=j_getobject(iout,'%ijbetter',j_ipreal)
		! !	jvh=j_defmatrix(iout,'%jh',1,20,j_matreg)
		! iivlen2=j_defmatrix(iout,'%Len2',1,neltot,j_matreg)
		! iivlenc2=j_defmatrix(iout,'%Lenc2',1,neltot,j_matreg)
		! iivh2=j_defmatrix(iout,'%H2',1,neltot,j_matreg)
		! iivd2=j_defmatrix(iout,'%D2',1,neltot,j_matreg)
		...
		allocate(j_o(iout)%i(58))
		j_o(iout)%i= (/ivnlog,ivhs,ivnlump,ivlen,ivlenc,ivh,ivd,iva,ivv,ivvc,&
			ivsaw,ivsawc,ivinchip,ivinchipc,ivoutchip,ivoutchipc,ivf,ivsyl,ivsylc,iivlen,&
			iivlenc,iivh,iivd,iiva,iivv,iivvc,iivsaw,iivsawc,iivinchip,iivinchipc,&
			iivoutchip,iivoutchipc,iivf,iivsyl,iivsylc,iivnlump,iivslump,iivvlump,iivlen2,iivlenc2,&
			iivh2,iivd2,iiva2,iivv2,iivvc2,iivsaw2,iivsawc2,iivinchip2,iivinchipc2,iivoutchip2,&
			iivoutchipc2,iivf2,iivsyl2,iivsylc2,iivnlump2,iivslump2,iivvlump2,ivnlog2/)
		!	write(62,*)'*',j_o(iout)%i
		...
		ivnlog=j_o(iout)%i(1)
		ivhs=j_o(iout)%i(2)
		ivnlump=j_o(iout)%i(3)
		ivlen=j_o(iout)%i(4) !eka kanto
		ivlenc=j_o(iout)%i(5)
		ivh=j_o(iout)%i(6)
		ivd=j_o(iout)%i(7)
		iva=j_o(iout)%i(8)
		...
		ivvc=j_o(iout)%i(10)
		ivsaw=j_o(iout)%i(11)
		ivsawc=j_o(iout)%i(12)
		ivinchip=j_o(iout)%i(13)
		ivinchipc=j_o(iout)%i(14)
		ivoutchip=j_o(iout)%i(15)
		ivoutchipc=j_o(iout)%i(16)
		ivf=j_o(iout)%i(17)
		ivsyl=j_o(iout)%i(18)
		ivsylc=j_o(iout)%i(19)
 
		iivlen=j_o(iout)%i(20) !eka kanto
		if(p)write(6,*)'iivlenhere',iivlen
		iivlenc=j_o(iout)%i(21) !eka kanto
		iivh=j_o(iout)%i(22)
		iivd=j_o(iout)%i(23)
		iiva=j_o(iout)%i(24)
		iivv=j_o(iout)%i(25)
		iivvc=j_o(iout)%i(26)
		iivsaw=j_o(iout)%i(27)
		iivsawc=j_o(iout)%i(28)
		iivinchip=j_o(iout)%i(29)
		iivinchipc=j_o(iout)%i(30)
		iivoutchip=j_o(iout)%i(31)
		iivoutchipc=j_o(iout)%i(32)
		iivf=j_o(iout)%i(33)
		iivsyl=j_o(iout)%i(34)
		iivsylc=j_o(iout)%i(35)
		iivnlump=j_o(iout)%i(36)
		iivslump=j_o(iout)%i(37)
		iivvlump=j_o(iout)%i(38)
 
		iivlen2=j_o(iout)%i(39) !eka kanto
		iivlenc2=j_o(iout)%i(40) !eka kanto
		!	write(6,*)'iivlenc2 ',iivlenc2
		iivh2=j_o(iout)%i(41)
		iivd2=j_o(iout)%i(42)
		iiva2=j_o(iout)%i(43)
		iivv2=j_o(iout)%i(44)
		iivvc2=j_o(iout)%i(45)
		iivsaw2=j_o(iout)%i(46)
		iivsawc2=j_o(iout)%i(47)
		iivinchip2=j_o(iout)%i(48)
		iivinchipc2=j_o(iout)%i(49)
		iivoutchip2=j_o(iout)%i(50)
		iivoutchipc2=j_o(iout)%i(51)
		iivf2=j_o(iout)%i(52)
		iivsyl2=j_o(iout)%i(53)
		iivsylc2=j_o(iout)%i(54)
		iivnlump2=j_o(iout)%i(55)
		iivslump2=j_o(iout)%i(56)
		iivvlump2=j_o(iout)%i(57)
		ivnlog2=j_o(iout)%i(58)
		!		iivlen22=j_o(iout)%i(59)
		! iivsawc2=j_o(iout)%i(47)
		! iivinchip2=j_o(iout)%i(48)
		! iivinchipc2=j_o(iout)%i(49)
		! iivoutchip2=j_o(iout)%i(50)
		...
		! iivsyl2=j_o(iout)%i(53)
		! iivsylc2=j_o(iout)%i(54)
		! iivnlog2=j_o(iout)%i(55)
 
	endif !if(j_otype(iout).ne.j_ipstemopt)  25042
	do i=3,34
		ii=j_o(iout)%i(i)
		if(allocated(j_o(ii)%d))j_o(ii)%d=1.d20
	enddo !i=3,34  25242
	!j_v(jvbetter)=j_0
	...
	j_o(iivh)%d(ipulp)=j_o(ivh)%d(ipulp)
	!	write(6,*)'dminp',dminp,j_o(ivh)%d(ipulp)
	!	read(5,*)ururu
	...
	if(nlog.le.0)then
		j_o(ivv)%d(ipulp)=j_stemcurveint(icalle,stump,j_o(ivh)%d(ipulp),iob,io,.false.)
		j_o(ivvc)%d(ipulp)=j_o(ivv)%d(ipulp)
 
		j_o(ivvc)%d(itot)=j_0
		j_o(iivvc)%d(itot)=j_0
		j_o(ivsylc)%d(itot)=j_0
		j_o(ivoutchipc)%d(itot)=j_0
		j_o(ivinchipc)%d(itot)=j_0
		j_o(ivsawc)%d(itot)=j_0
		j_v(ivnlog2)=j_0
		!j_v(jvn)=j_0
 
		return
	endif !if(nlog.le.0)  25283
	!if(nlog.gt.0)then
	...
	if(nlog.gt.0)then
		...
		endif !if(loglentot.gt.nlog*lmax)  25302
		npatk=(loglentot-nlog*lmin)/3
		...
		!logtot=0
		!	if(nlog.gt.5)write(6,*)'nlog',nlog,loglentot,nplus,log0,log1,nyks,nkaks
		do i=1,nlog
			i1=i+1
			!	write(6,*)'a',i
			if(i.le.nyks)then
		...
		enddo !i=1,nlog  25336
		!if(nlog.gt.0)then
		j_o(iva)%d(itot)=j_tempv2(loglenc(nlog1))
 
 
 
	endif !if(nlog.gt.0)  25301
	...
	j_o(ivoutchipc)%d(itot)=j_o(ivoutchipc)%d(nlog1)
	...
	vtot=j_o(ivv)%d(ipulp)+j_o(ivvc)%d(itot)
	...
	j_o(iivh)%d(1)=stump
	j_o(iiva)%d(1)=j_o(iva)%d(1)
	j_o(iivd)%d(1)=j_o(ivd)%d(1)
	nlog2=nlog-1
	j_v(ivnlog2)=nlog2
	j_v(ivnlog2)=max(j_v(ivnlog2),j_0)
	...
	loglen(1:nlog)=lmin
	loglenopt(1:nlog)=loglen(1:nlog)
	if(nlog.lt.nlogmax)loglen(nlog1:nlogmax)=0
	sawcur=j_0
	do i=1,nlog
		i1=i+1
		sawcur=sawcur+j_tempv3(i*lmin)*lmin
 
	enddo !i=1,nlog  25444
	!saw0=j_dapu   !used later when maximizing
	j_o(iivslump)%d(1)=sawcur
	hcur=nlog*lmin+stump   !volume with minimum size logs
	j_o(iivvlump)%d(1)=j_stemcurveint(icalle,stump,hcur,iob,io,.false.)
	!if(j_dapu.gt.1000)	write(6,*)'saw2  ',j_v(j_ivobs),j_dapu,nlog
	!	write(6,*)'j_vlump0',j_o(ivvlump)%d(1)
	!	j_o(ijvv)%d(1)=j_o(ivvlump)%d(1)
	!	j_o(ijvs)%d(1)=j_dapu
	sawopt=sawcur  !global maximum
	! here all pieces are added to logs so that maximum sawtimber is produced
	if(p)write(6,*)'vlumpinit',j_o(iivvlump)%d(1),hcur
	!	j_yes=.true.
	do ipat=1,npatk
		ipat1=ipat+1
		! j_dapu
		if(p)write(6,*)'stem',hcur,j_stemcurveint(icalle,hcur,hcur+3.d0,iob,io,.false.)
		!		j_o(ijvv)%d(ipat+1)=j_o(ivvlump)%d(ipat+1)
		j_o(iivvlump)%d(ipat1)=j_o(iivvlump)%d(ipat)+j_stemcurveint(icalle,hcur,hcur+3.d0,iob,io,.false.)
		!		j_o(ijvv)%d(ipat+1)=j_o(ivvlump)%d(ipat+1)
		!		write(6,*)'ipat',ipat,j_o(ivvlump)%d(ipat+1),j_dapu4
		hcur=hcur+3.d0
		if(p)write(6,*)'ipat,hcur,j_o(iivvlump)%d(ipat1)',&
			ipat,hcur,j_o(iivvlump)%d(ipat1)
		logtot=0 ! iad loop is done to this point
		!write(17,*)'ipat',ipat,npatk,logtot
		! add to logs 3 dm pieces as long as saw increase
 
		sawprev=j_0  ! amount of saw timber up to iad
		imax=0
		sawcur=j_0  !local optimimum for current lump
		do iad=1,nlog
			logtot=logtot+loglen(iad)
 
			!	write(6,*)'ipat,iad,logtot,loglen(iad)',ipat,iad,logtot,loglen(iad)
			if(loglen(iad).eq.lmax)then
				sawprev=sawprev+loglen(iad)*j_tempv3(logtot) !sawtimber up
 
				cycle
			endif !if(loglen(iad).eq.lmax)  25483
			logtot2=logtot+3
			sawpot=sawprev+(loglen(iad)+3)*j_tempv3(logtot2)
			...
			endif !if(sawpot.gt.sawcur)  25502
 
			sawprev=sawcur
		enddo !iad=1,nlog  25479
		!	if(imax.gt.0)then
		!		write(6,*)'imax,loglen',imax,loglen(imax)
		j_o(iivslump)%d(ipat1)=sawcur
 
 
		if(sawcur.gt.sawopt)then
			sawopt=sawcur
			loglenopt(1:nlog)=loglen(1:nlog)
			loglenopt(imax)=loglen(imax)+3
			!	j_dapu2=j_dapu2+loglen(imax)*j_tempv3(logtot)
		endif !if(sawcur.gt.sawopt)  25518
		loglen(imax)=loglen(imax)+3
 
 
	enddo !ipat=1,npatk  25461
 
	!	if(j_yes.and.j_dapu.le.dapu.or.ipat.eq.npatk)then
	j_v(iivnlump)=npatk+1
	!		j_yes=.false.
 
	j_o(iivlen)%d(2:nlog1)=loglenopt(1:nlog)
	lento=0
	do iip=1,nlog
		iip1=iip+1
		lento=lento+loglenopt(iip)
		j_o(iivlenc)%d(iip1)=lento
		j_o(iivh)%d(iip1)=j_o(iivh)%d(iip)+j_o(iivlen)%d(iip1)
		j_o(iivd)%d(iip1)=j_tempv(lento)
		j_o(iiva)%d(iip1)=j_tempv2(lento)
		j_o(iivsaw)%d(iip1)=j_tempv3(lento)*j_o(iivlen)%d(iip1)
		j_o(iivv)%d(iip1)=j_stemcurveint(icalle,j_o(iivh)%d(iip),j_o(iivh)%d(iip1),iob,io,.false.)
		j_o(iivvc)%d(iip1)=j_o(iivvc)%d(iip)+j_o(iivv)%d(iip1)
		j_o(iivsawc)%d(iip1)=j_o(iivsawc)%d(iip)+j_o(iivsaw)%d(iip1)
		j_o(iivsyl)%d(iip1)=j_tempv2(lento)*j_o(iivlen)%d(iip1)
		j_o(iivsylc)%d(iip1)=j_o(iivsylc)%d(iip)+j_o(iivsyl)%d(iip1)
		j_o(iivf)%d(iip1)=1-b*gam**j_o(iivd)%d(iip1)
		j_o(iivoutchip)%d(iip1)=j_o(iivv)%d(iip1)-j_o(iivsyl)%d(iip1)
		j_o(iivoutchipc)%d(iip1)=j_o(iivoutchipc)%d(iip)+j_o(iivoutchip)%d(iip1)
		j_o(iivinchip)%d(iip1)=j_o(iivsyl)%d(iip1)-j_o(iivsaw)%d(iip1)
		j_o(iivinchipc)%d(iip1)=j_o(iivinchipc)%d(iip)+j_o(iivinchip)%d(iip1)
	enddo !iip=1,nlog  25535
 
 
	!		endif !if(j_yes.and.j_dapu.le.dapu.or.ipat.eq.npatk)  24933
	j_o(iivlenc)%d(itot)=lento
	j_o(iivoutchipc)%d(itot)=j_o(iivoutchipc)%d(nlog1)
	j_o(iivsylc)%d(itot)=j_o(iivsylc)%d(nlog1)
	j_o(iivinchipc)%d(itot)=j_o(iivinchipc)%d(nlog1)
	...
	!	j_o(iivsylc)%d(itot)=j_o(iivsylc)%d(nlog1)
	!	j_o(ivinchipc)%d(itot)=j_o(ivinchipc)%d(nlog1)
	...
	!	j_o(iivsawc)%d(itot)=j_o(iivsawc)%d(nlog1)
	j_v(iivnlump)=j_1+(j_o(iivlenc)%d(itot)-nlog*lmin)/3
	!	write(6,*)j_o(iivlenc)%d(itot),nlog,lmin,j_o(iivlenc)%d(itot)-nlog*lmin,j_v(iivnlump)
 
 
	j_v(ivnlump)=npatk+1
	j_o(iivslump)%d(npatk+2:20)=j_0
	j_o(iivvlump)%d(npatk+2:20)=j_0
	...
	! if(p.and.nlog.gt.3.and.i.eq.nlog-1)write(6,*)'nlog ',j_o(iivlenc)%d(i1-1:i1)
	...
	!*****************************	suhteen maksimointi maksimimäärällä tukkeja
	p=.true.
 
	nlog1=nlog
	nlog=nlog2
	loglenmax=min(nlog2*lmax,loglen0)
	!	npatk=(loglenmax-lenc)/3
	loglenmin=loglen0-lmin+1
	logtot=nlog*lmin
	if(p)write(6,*)'loglenmax,loglenmin',loglenmax,loglenmin,nlog,nlog1
	if(loglenmin.gt.loglenmax)then
		j_v(iivnlump2)=0
		return
 
	endif !if(loglenmin.gt.loglenmax)  25641
	...
	enddo !i=1,nlog  25651
	!saw0=j_dapu   !used later when maximizing
	j_o(iivslump2)%d(1)=sawcur
	hcur=nlog*lmin+stump   !volume with minimum size logs
	j_o(iivvlump2)%d(1)=j_stemcurveint(icalle,stump,hcur,iob,io,.false.)
	!if(j_dapu.gt.1000)	write(6,*)'saw2  ',j_v(j_ivobs),j_dapu,nlog
	!	write(6,*)'j_vlump0',j_o(ivvlump2)%d(1)
	!	j_o(ijvv2)%d(1)=j_o(ivvlump2)%d(1)
	!	j_o(ijvs2)%d(1)=j_dapu
	sawopt=sawcur/j_o(iivvlump2)%d(1)  !global maximum
	! here all pieces are added to logs so that maximum sawtimber is produced
	if(p)write(6,*)'vlumpinit',j_o(iivvlump2)%d(1),hcur
	!	j_yes=.true.
	npatk0=0
	if(logtot.ge.loglenmin)npatk0=1
	do ipat=1,100
		if(logtot.ge.loglenmax)exit
		ipat1=ipat+1
		! j_dapu
		if(p)write(6,*)'stem',hcur,j_stemcurveint(icalle,hcur,hcur+3.d0,iob,io,.false.)
		if(ipat1.gt.20)write(6,*)'logtot,loglenmax,nlog ',logtot,loglenmax,nlog
		!		j_o(ijvv2)%d(ipat+1)=j_o(ivvlump2)%d(ipat+1)
		j_o(iivvlump2)%d(ipat1)=j_o(iivvlump2)%d(ipat)+j_stemcurveint(icalle,hcur,hcur+3.d0,iob,io,.false.)
		!		j_o(ijvv2)%d(ipat+1)=j_o(ivvlump2)%d(ipat+1)
		!		write(6,*)'ipat',ipat,j_o(ivvlump2)%d(ipat+1),j_dapu4
		hcur=hcur+3.d0
		if(p)write(6,*)'ipat,hcur,j_o(iivvlump2)%d(ipat1)',&
			ipat,hcur,j_o(iivvlump2)%d(ipat1)
		logtot=0 ! iad loop is done to this point
		!write(17,*)'ipat',ipat,npatk,logtot
		! add to logs 3 dm pieces as long as saw increase
		...
		do iad=1,nlog
			...
			if(loglen(iad).eq.lmax)then
				...
				cycle
			endif !if(loglen(iad).eq.lmax)  25694
			logtot2=logtot+3
			sawpot=sawprev+(loglen(iad)+3)*j_tempv3(logtot2)
			!		write(6,*)'logtot2,loglen(iad),j_dapu2,j_dapu3',logtot2,loglen(iad),j_dapu2,j_dapu3
			!j_dapu3 is the amount of sawtimber if extra piece is added to iad
			...
			if(sawpot.gt.sawcur)then
				...
				imax=iad
				! j-dapu global maxmimum of sawtimber
			endif !if(sawpot.gt.sawcur)  25713
 
			sawprev=sawcur
		enddo !iad=1,nlog  25690
		!	if(imax.gt.0)then
		!		write(6,*)'imax,loglen',imax,loglen(imax)
		j_o(iivslump2)%d(ipat1)=sawcur
 
 
		if(sawcur.gt.sawopt*j_o(iivvlump2)%d(ipat1))then
			sawopt=sawcur/j_o(iivvlump2)%d(ipat1)
			loglenopt(1:nlog)=loglen(1:nlog)
			loglenopt(imax)=loglen(imax)+3
			!	j_dapu2=j_dapu2+loglen(imax)*j_tempv3(logtot)
		endif !if(sawcur.gt.sawopt*j_o(iivvlump2)%d(ipat1))  25729
		loglen(imax)=loglen(imax)+3
		logtot=logtot+3
		if(logtot.ge.loglenmin)npatk0=npatk0+1
 
 
	enddo !ipat=1,100  25670
 
	!	if(j_yes.and.j_dapu.le.dapu.or.ipat.eq.npatk)then
	npatk=ipat-npatk0
	npatk1=npak+1
	j_v(iivnlump2)=npatk+1
	write(6,*)'*npatk0,npatk',npatk0,npatk
	j_o(iivvlump2)%d(1:npatk)=j_o(iivvlump2)%d(npatk0+1:ipat)
	j_o(iivvlump2)%d(1:npatk)=j_o(iivslump2)%d(npatk0+1:ipat)
	!		j_yes=.false.
 
	j_o(iivlen2)%d(2:nlog1)=loglenopt(1:nlog)
	lento=0
	do iip=1,nlog
		iip1=iip+1
		lento=lento+loglenopt(iip)
		j_o(iivlenc2)%d(iip1)=lento
		j_o(iivh2)%d(iip1)=j_o(iivh2)%d(iip)+j_o(iivlen2)%d(iip1)
		j_o(iivd2)%d(iip1)=j_tempv(lento)
		j_o(iiva2)%d(iip1)=j_tempv2(lento)
		j_o(iivsaw2)%d(iip1)=j_tempv3(lento)*j_o(iivlen2)%d(iip1)
		j_o(iivv2)%d(iip1)=j_stemcurveint(icalle,j_o(iivh2)%d(iip),j_o(iivh2)%d(iip1),iob,io,.false.)
		j_o(iivvc2)%d(iip1)=j_o(iivvc2)%d(iip)+j_o(iivv2)%d(iip1)
		j_o(iivsawc2)%d(iip1)=j_o(iivsawc2)%d(iip)+j_o(iivsaw2)%d(iip1)
		j_o(iivsyl2)%d(iip1)=j_tempv2(lento)*j_o(iivlen2)%d(iip1)
		j_o(iivsylc2)%d(iip1)=j_o(iivsylc2)%d(iip)+j_o(iivsyl2)%d(iip1)
		j_o(iivf2)%d(iip1)=1-b*gam**j_o(iivd2)%d(iip1)
		j_o(iivoutchip2)%d(iip1)=j_o(iivv2)%d(iip1)-j_o(iivsyl2)%d(iip1)
		j_o(iivoutchipc2)%d(iip1)=j_o(iivoutchipc2)%d(iip)+j_o(iivoutchip2)%d(iip1)
		j_o(iivinchip2)%d(iip1)=j_o(iivsyl2)%d(iip1)-j_o(iivsaw2)%d(iip1)
		j_o(iivinchipc2)%d(iip1)=j_o(iivinchipc2)%d(iip)+j_o(iivinchip2)%d(iip1)
	enddo !iip=1,nlog  25753
 
 
	!		endif !if(j_yes.and.j_dapu.le.dapu.or.ipat.eq.npatk)  24933
	j_o(iivlenc2)%d(itot)=lento
	j_o(iivoutchipc2)%d(itot)=j_o(iivoutchipc2)%d(nlog1)
	j_o(iivsylc2)%d(itot)=j_o(iivsylc2)%d(nlog1)
	j_o(iivinchipc2)%d(itot)=j_o(iivinchipc2)%d(nlog1)
	j_o(iivvc2)%d(itot)=j_o(iivvc2)%d(nlog1)
	j_o(iivsawc2)%d(itot)=j_o(iivsawc2)%d(nlog1)
	j_o(iivd2)%d(itot)=j_o(iivd2)%d(nlog1)
	j_o(iivd2)%d(ipulp)=dminp
	j_o(iiva2)%d(itot)=j_o(iiva2)%d(nlog1)
	!	j_o(iivsylc2)%d(itot)=j_o(iivsylc2)%d(nlog1)
	!	j_o(ivinchipc2)%d(itot)=j_o(ivinchipc2)%d(nlog1)
	j_o(iivh2)%d(itot)=j_o(iivh2)%d(nlog1)
	!	j_o(iivlenc2)%d(itot)=j_o(iivlenc2)%d(nlog1)
	j_o(iivlenc2)%d(ipulp)=j_o(iivh2)%d(ipulp)-stump
	j_o(iivh2)%d(ipulp)=j_o(ivh)%d(ipulp)
	!	j_o(iivvc2)%d(itot)=j_o(iivvc2)%d(nlog1)
 
	j_o(iivv2)%d(ipulp)=j_stemcurveint(icalle,j_o(iivh2)%d(itot),j_o(iivh2)%d(ipulp),iob,io,.false.)
	j_o(iivlen2)%d(ipulp)=j_o(iivh2)%d(ipulp)-j_o(iivlenc2)%d(itot)
	!	j_o(iivsawc2)%d(itot)=j_o(iivsawc2)%d(nlog1)
	j_v(iivnlump)=j_1+(j_o(iivlenc2)%d(itot)-nlog*lmin)/3
	!	write(6,*)j_o(iivlenc2)%d(itot),nlog,lmin,j_o(iivlenc2)%d(itot)-nlog*lmin,j_v(iivnlump)
 
 
	j_v(ivnlump)=npatk+1
	j_o(iivslump2)%d(npatk+2:25)=j_0
	j_o(iivvlump2)%d(npatk+2:25)=j_0
	...
	!*********************
	j_o(iivlenc2)%d(1)=j_0
	j_o(iivsylc2)%d(1)=j_0
	j_o(iivvc2)%d(1)=j_0
	j_o(iivsawc2)%d(1)=j_0
	j_o(iivoutchipc2)%d(1)=j_0
	j_o(iivinchipc2)%d(1)=j_0
	j_o(iivh2)%d(1)=stump
	j_o(iiva2)%d(1)=j_o(iva2)%d(1)
	j_o(iivd2)%d(1)=j_o(ivd2)%d(1)
	nlog2=nlog-1
	j_v(ivnlog2)=nlog2
	j_v(ivnlog2)=max(j_v(ivnlog2),j_0)
	...
	enddo !i=1,nlog  25832
	!saw0=j_dapu   !used later when maximizing
	j_o(iivslump2)%d(1)=j_dapu
	j_dapu4=nlog*lmin+stump   !volume with minimum size logs
	j_o(iivvlump2)%d(1)=j_stemcurveint(icalle,stump,j_dapu4,iob,io,.false.)
	!if(j_dapu.gt.1000)	write(6,*)'saw2  ',j_v(j_ivobs),j_dapu,nlog
	!	write(6,*)'j_vlump0',j_o(ivvlump2)%d(1)
	!	j_o(ijvv2)%d(1)=j_o(ivvlump2)%d(1)
	!	j_o(ijvs2)%d(1)=j_dapu
	dapu=j_dapu  !maximum
	! here all pieces are added to logs so that maximum sawtimber is produced
	if(p)write(6,*)'vlumpinit',j_o(iivvlump2)%d(1),j_dapu4
	j_yes=.true.
	do ipat=1,npatk
		ipat1=ipat+1
		! j_dapu
		if(p)write(6,*)'stem',j_dapu4,j_stemcurveint(icalle,j_dapu4,j_dapu4+3.d0,iob,io,.false.)
		!		j_o(ijvv2)%d(ipat+1)=j_o(ivvlump2)%d(ipat+1)
		j_o(iivvlump2)%d(ipat1)=j_o(iivvlump2)%d(ipat)+j_stemcurveint(icalle,j_dapu4,j_dapu4+3.d0,iob,io,.false.)
		!		j_o(ijvv2)%d(ipat+1)=j_o(ivvlump2)%d(ipat+1)
		!		write(6,*)'ipat',ipat,j_o(ivvlump2)%d(ipat+1),j_dapu4
		j_dapu4=j_dapu4+3.d0
		if(p)write(6,*)'ipat,j_dapu4,j_o(iivvlump2)%d(ipat1)',&
			ipat,j_dapu4,j_o(iivvlump2)%d(ipat1)
		logtot=0 ! iad loop is done to this point
		!write(17,*)'ipat',ipat,npatk,logtot
		! add to logs 3 dm pieces as long as saw increase
		...
		do iad=1,nlog
			...
			!	write(6,*)'ipat,iad,logtot,loglen(iad)',ipat,iad,logtot,loglen(iad)
			if(loglen(iad).eq.lmax)then
				j_dapu2=j_dapu2+loglen(iad)*j_tempv3(logtot) !sawtimber up
 
				cycle
			endif !if(loglen(iad).eq.lmax)  25871
			logtot2=logtot+3
			...
			do i=iad+1,nlog !shift other
				...
				j_dapu3=j_dapu3+loglen(i)*j_tempv3(logtot2)  !sawlog
				!			write(6,*)'i,logtot2,loglen(i),j_dapu3,ad',i,logtot2,loglen(i),j_dapu3,loglen(i)*j_tempv3(logtot2)
			enddo !i=iad+1,nlog  25880
			!		write(6,*)'ipat,iad,',imax,j_dapu2,j_dapu3,j_dapu
			if(j_dapu3.gt.j_dapu)then
				...
				imax=iad
				! j-dapu global maxmimum of sawtimber
			endif !if(j_dapu3.gt.j_dapu)  25890
			! if(iad.eq.1)then
			! j_dapu5=j_dapu3
			! imax0=1
			...
			! endif !if(j_dapu3.gt.j_dapu5)  24861
 
 
			! endif !if(iad.eq.1)  24856
			!	write(6,*)'logtot ',logtot,'imax',imax
 
 
		enddo !iad=1,nlog  25867
		!	if(imax.gt.0)then
		!		write(6,*)'imax,loglen',imax,loglen(imax)
		j_o(iivslump2)%d(ipat1)=j_dapu
		! J_DAPU CURRENT  sawlogmax for this lump , dapu globaö
 
		if(j_dapu.gt.dapu.and.logtot.ge.loglenmin)then
			dapu=j_dapu
			loglen(imax)=loglen(imax)+3
			j_dapu2=j_dapu2+loglen(imax)*j_tempv3(logtot)
		endif !if(j_dapu.gt.dapu.and.logtot.ge.loglenmin)  25920
		if(j_yes.and.j_dapu.le.dapu.or.ipat.eq.npatk)then
			j_v(iivnlump)=ipat+1
			j_yes=.false.
 
			j_o(iivlen2)%d(2:nlog1)=loglen(1:nlog)
			lento=0
			do iip=1,nlog
				iip1=iip+1
				lento=lento+loglen(iip)
				j_o(iivlenc2)%d(iip1)=lento
				j_o(iivh2)%d(iip1)=j_o(iivh2)%d(iip)+j_o(iivlen2)%d(iip1)
				j_o(iivd2)%d(iip1)=j_tempv(lento)
				j_o(iiva2)%d(iip1)=j_tempv2(lento)
				...
				j_o(iivinchip2)%d(iip1)=j_o(iivsyl2)%d(iip1)-j_o(iivsaw2)%d(iip1)
				j_o(iivinchipc2)%d(iip1)=j_o(iivinchipc2)%d(iip)+j_o(iivinchip2)%d(iip1)
			enddo !iip=1,nlog  25931
 
 
		endif !if(j_yes.and.j_dapu.le.dapu.or.ipat.eq.npatk)  25925
		...
		!	endif !if(imax.gt.0)  24873
 
 
 
 
 
	enddo !ipat=1,npatk  25849
	...
	j_o(iivh2)%d(itot)=j_o(iivh2)%d(nlog1)
	!	j_o(iivlenc2)%d(itot)=j_o(iivlenc2)%d(nlog1)
	j_o(iivlenc2)%d(ipulp)=j_o(ivh2)%d(ipulp)-stump
	!	j_o(iivvc2)%d(itot)=j_o(iivvc2)%d(nlog1)
	j_o(iivv2)%d(ipulp)=j_stemcurveint(icalle,j_o(iivh2)%d(itot),j_o(iivh2)%d(ipulp),iob,io,.false.)
	j_o(iivlen2)%d(ipulp)=j_o(iivh2)%d(ipulp)-j_o(iivlenc2)%d(itot)
	!	j_o(iivsawc2)%d(itot)=j_o(iivsawc2)%d(nlog1)
	...
	enddo !i=1,nlog  26005
	!saw0=j_dapu   !used later when maximizing
	logtot=nlog*lmin
	!	j_o(ivslump2)%d(1)=j_dapu
	j_dapu4=nlog*lmin+stump   !volume with minimum size logs
	!	j_o(iivlump2)%d(1)=j_stemcurveint(icalle,stump,j_dapu4,iob,io,.false.)
	!if(j_dapu.gt.1000)	write(6,*)'saw2  ',j_v(j_ivobs),j_dapu,nlog
	!	write(6,*)'j_vlump0',j_o(ivvlump2)%d(1)
	!	j_o(ijvv2)%d(1)=j_o(ivvlump2)%d(1)
	!	j_o(ijvs2)%d(1)=j_dapu
	dapu=j_dapu  !maximum
	! here all pieces are added to logs so that maximum sawtimber is produced
	if(p)write(6,*)'vlumpinit',j_o(iivvlump2)%d(1),j_dapu4,'nlog',nlog,j_dapu
	j_yes=.true.
	ipat=0
	logtot=0
	do ipat0=1,100
		if(p)write(6,*)'logtot',logtot,'ipat0',ipat0
		if(logtot+3.gt.loglenmax)exit
		!volume with minimum size logs
 
		if(logtot.ge.loglenmin)ipat=ipat+1
		ipat1=ipat+1
		if(p)write(6,*)'ipat0,ipat,ipat1',ipat0,ipat,ipat1,logtot
		if(ipat.eq.1)then
			j_o(iivlump2)%d(1)=j_stemcurveint(icalle,stump,j_dapu4+3.d0,iob,io,.false.)
		elseif(ipat.gt.1)then
			j_o(iivlump2)%d(ipat1)=j_o(iivlump2)%d(ipat1)+ &
				j_stemcurveint(icalle,j_dapu4,j_dapu4+3.d0,iob,io,.false.)
 
 
 
		endif !if(ipat.eq.1)  26033
		j_dapu4=j_dapu4+3.d0
		! j_dapu
		if(p)write(6,*)'stem',ipat,j_dapu4,j_o(iivvlump2)%d(ipat1)
		...
		do iad=1,nlog
			...
			if(p)write(6,*)'ipat,iad,logtot,loglen(iad)',ipat,iad,logtot,loglen(iad)
			if(loglen(iad).eq.lmax)then
				j_dapu2=j_dapu2+loglen(iad)*j_tempv3(logtot) !sawtimber up
 
				cycle
			endif !if(loglen(iad).eq.lmax)  26054
			logtot2=logtot+3
			j_dapu3=j_dapu2+(loglen(iad)+3)*j_tempv3(logtot2)
			!		write(6,*)'logtot2,loglen(iad),j_dapu2,j_dapu3',logtot2,loglen(iad),j_dapu2,j_dapu3
			!j_dapu3 is the amount of sawtimber if extra piece is added to iad
			do i=iad+1,nlog !shift other
				...
				j_dapu3=j_dapu3+loglen(i)*j_tempv3(logtot2)  !sawlog
				!			write(6,*)'i,logtot2,loglen(i),j_dapu3,ad',i,logtot2,loglen(i),j_dapu3,loglen(i)*j_tempv3(logtot2)
			enddo !i=iad+1,nlog  26063
			if(p)write(6,*)'vert ipat,iad,',imax,j_dapu3,dapu,j_o(iivvlump2)%d(ipat1),&
				dapu*j_o(iivvlump2)%d(ipat1)
			if(j_dapu3.gt.dapu*j_o(iivvlump2)%d(ipat1))then
				...
				imax=iad
				! j-dapu global maxmimum of sawtimber
			endif !if(j_dapu3.gt.dapu*j_o(iivvlump2)%d(ipat1))  26074
			! if(iad.eq.1)then
			! j_dapu5=j_dapu3
			! imax0=1
		...
		enddo !iad=1,nlog  26050
		!	if(imax.gt.0)then
		if(p)write(6,*)'imax,loglen',imax,loglen(imax),'slump',j_dapu,dapu
		j_o(iivslump2)%d(ipat1)=j_dapu
 
 
		if(j_dapu.gt.dapu)then
			dapu=j_dapu
			loglen(imax)=loglen(imax)+3
			j_dapu2=j_dapu2+loglen(imax)*j_tempv3(logtot)
		endif !if(j_dapu.gt.dapu)  26104
		if(p)write(6,*)'**ipat,logtot,j_yes,dapu,j_dapu',ipat,logtot,j_yes,dapu,j_dapu
		if(j_yes.and.j_dapu.le.dapu.or.ipat.eq.npatk)then
			j_v(iivnlump2)=ipat1
			j_yes=.false.
			...
			do iip=1,nlog
				...
				j_o(iivinchip2)%d(iip1)=j_o(iivsyl2)%d(iip1)-j_o(iivsaw2)%d(iip1)
				j_o(iivinchipc2)%d(iip1)=j_o(iivinchipc2)%d(iip)+j_o(iivinchip2)%d(iip1)
			enddo !iip=1,nlog  26116
 
 
 
 
		endif !if(j_yes.and.j_dapu.le.dapu.or.ipat.eq.npatk)  26110
	...
	enddo !ipat0=1,100  26025
	if(p)write(6,*)'ipatf',ipat
 
	!j_v(ivnlump)=npatk+1
	!j_o(ivslump2)%d(npatk+2:20)=j_0
	!j_o(ivvlump2)%d(npatk+2:20)=j_0
	j_o(iivlenc2)%d(itot)=lento
	j_o(iivoutchipc2)%d(itot)=j_o(iivoutchipc2)%d(nlog1)
	j_o(iivsylc2)%d(itot)=j_o(iivsylc2)%d(nlog1)
	j_o(iivinchipc2)%d(itot)=j_o(iivinchipc2)%d(nlog1)
	...
	!	j_o(iivsawc2)%d(itot)=j_o(iivsawc2)%d(nlog1)
	!		j_v(iivnlump)=j_1+(j_o(iivlenc2)%d(itot)-nlog*lmin)/3
	!	write(6,*)j_o(iivlenc2)%d(itot),nlog,lmin,j_o(iivlenc2)%d(itot)
 
	return
 
	! ! j_o(ijvlen)%d(1)=loglenmin
	! ! volbas=j_stemcurveint(icalle,stump,stump+loglenmin,iob,io,.false.)
	! ! if(p)write(6,*)'suhde0 npatk',npatk
	! ! j_o(ijvv)%d(1)=volbas
	! ! j_o(ijvs)%d(1)=saw0
	...
	! ! do ipat=1,npatk
	! ! ipat1=ipat+1
	! kullekin lisättävälle pätkälle pätkä lisätään tukkiin josta saw/vol on suurin
 
 
	! j_dapu=j_0  !volume !dapu tempirary j_dapu stored
	! j_dapu2=j_0 !cylinder
	! j_dapu3=j_0 !saw
	! !
	! ! suhde=j_0
	! dapu3=j_0
	! ! j_dapu3=j_0  !keeps track without addition
	! suhde=j_0
	! ! logtotv=0 !keeps track without addition
	! ! if(p)write(6,*)'ipat suhde0 ',ipat
	! j_dapu4=stump
	! ! iad2=0
	! ! do iad=1,nlog
	! iad mihin tukkiin lisataan
	! ! dapu3=j_dapu3
	! ! logtot=logtotv
	! ! if(p)write(6,*)'iad',iad,loglen(iad),loglen(iad).eq.lmax
	! ! if(loglen(iad).eq.lmax)then
	! ! logtotv=logtot+lmax
	! ! j_dapu3=j_dapu3+lmax*j_tempv3(logtotv)
	! ! if(p)write(6,*)
	! suhde=j_dapu3
	! ! cycle
	! ! endif !if(loglen(iad).eq.lmax)  25059
	! ! logtotv=logtotv+loglen(iad)
	! ! j_dapu3=j_dapu3+j_tempv3(logtotv)*loglen(iad)
	! suhde=j_dapu3
	! ! logtot=logtot+loglen(iad)+3
	! ! dapu3=dapu3+j_tempv3(logtot)*(loglen(iad)+3.d0) !saw
 
 
	! ! do i=iad+1,nlog !shift other
 
	! ! logtot=logtot+loglen(i)
	! ! if(p)write(6,*)'i,loglen(i),logtot',i,loglen(i),logtot
	! ! dapu3=dapu3+loglen(i)*j_tempv3(logtot) !saw
	! ! if(p)write(6,*)'loppu i',i,logtot,real(dapu3)
	! suhde=suhde+loglen(i)*j_tempv3(logtot-3)
	! write(6,*)'i,logtot2,loglen(i),j_dapu3,ad',i,logtot2,loglen(i),j_dapu3,loglen(i)*j_tempv3(logtot2)
	! ! enddo !i=iad+1,nlog  25073
	! ! if(p)write(6,*)'dapu3,suhde ',real(dapu3),real(suhde)
	! ! if(dapu3.gt.suhde)then
	! ! suhde=dapu3
	! ! imax=iad
	! ! if(p)write(6,*)'imax',imax,logtot,real(dapu3)
 
	! ! endif !if(dapu3.gt.suhde)  25083
	! read(5,*)uuuu
	! ! enddo !iad=1,nlog  25054
 
	! ! loglen(imax)=loglen(imax)+3
	! ! if(p)write(6,*)'fin,ipat,',ipat,real(dapu3),'imax ',imax,real(suhde)
	! if(ipat.gt.npatk0)then
	! ipatc=ipat-npatk0
	! if(ipatc.eq.1)then
	! ! j_o(ijvlen)%d(ipat1)=loglenmin+(ipat)*3.d0
	! ! j_o(ijvv)%d(ipat1)=volbas
 
 
	! else
	! j_o(ijvlen)%d(ipatc)=j_o(ijvlen)%d(ipatc-1)+3
	! ! j_o(ijvv)%d(ipat1)=j_o(ijvv)%d(ipat)+j_stemcurveint(icalle,stump+j_o(ijvlen)%d(ipat),&
	! ! stump+j_o(ijvlen)%d(ipat1),iob,io,.false.)
	! endif !if(ipatc.eq.1)  25017
	! ! ito=j_o(ijvlen)%d(ipat1)
	! ! j_o(ijvd)%d(ipat1)=j_tempv(ito)
	! ! j_o(ijvs)%d(ipat1)=suhde  !sawtimber
	! ! j_o(ijvc)%d(ipat1)=j_o(ijvv)%d(ipat1)-suhde  !in chip
	! j_o(ijvout)%d(ipatc)=dapu-dapu3
	! ! j_o(ijvpulp)%d(ipat1)=vtot-j_o(ijvv)%d(ipat1)
 
	! endif !if(ipat.gt.npatk0)  25015
	! ! enddo !ipat=1,npatk  25037
 
	! ! suhde=j_0
	! ! do ipat=1,npatk+1
	! ! suhde0=j_o(ijvs)%d(ipat)/j_o(ijvv)%d(ipat)
	! ! if(suhde0.gt.suhde)then
	! ! suhde=suhde0
	! ! imax=ipat
	! ! endif !if(suhde0.gt.suhde)  25119
 
	! ! enddo !ipat=1,npatk+1  25117
	! write(6,*)'imax,npatk,j_v(ijvn),',imax,npatk,j_o(ijvs)%d(npatk),j_o(ijvv)%d(npatk)
	! ! j_v(ijvn)=npatk+1
	! ! j_o(ijvv)%d(20)=j_o(ijvv)%d(imax)
	! ! j_o(ijvs)%d(20)=j_o(ijvs)%d(imax)
	! write(6,*)'imax',j_o(ijvv)%d(20),j_o(ijvs)%d(20),j_o(ijvs)%d(imax),j_o(ijvs)%d(20)
	! write(6,*)j_o(ijvv)%d(20),j_o(iivsawc)%d(itot),j_o(ijvs)%d(20),j_o(iivvc)%d(itot),&
	! j_o(ijvv)%d(20)*j_o(iivsawc)%d(itot),j_o(ijvs)%d(20)*j_o(iivv)%d(itot)
	! ! if(j_o(ijvs)%d(20)*j_o(iivvc)%d(itot).gt.j_o(ijvv)%d(20)*j_o(iivsawc)%d(itot)) &
	! ! j_v(ijvbetter)=j_1                ! S/V>s/v  S*v>V*s
	! call j_getname(ijvbetter)
	! write(6,*)'bet ',j_oname(1:j_loname),j_v(ijvbetter)
	! ! j_o(ijvc)%d(20)=j_o(ijvc)%d(imax)
	! ! j_o(ijvpulp)%d(20)=j_o(ijvpulp)%d(imax)
	! ! j_o(ijvlen)%d(20)=j_o(ijvlen)%d(imax)
	! ! j_o(ijvd)%d(20)=j_o(ijvd)%d(imax)
 
	! ! j_o(ijvlen)%d(npatk+2:19)=j_0
	! ! j_o(ijvv)%d(npatk+2:19)=j_0  !volume
	! ! j_o(ijvs)%d(npatk+2:19)=j_0  !sawtimber
	! ! j_o(ijvc)%d(npatk+2:19)=j_0 !in chip
	! ! j_o(ijvd)%d(npatk+2:19)=j_0
	! j_o(ijvout)%d(npatk+1:20)=j_0
	! ! j_o(ijvpulp)%d(npatk+2:19)=j_0
	...
	! ! loglenmin=loglen0-lmin+1
	! ! volbas=j_stemcurveint(icalle,stump,stump+loglenmin,iob,io,.false.)
	! ! loglenmax=min(nlog2*lmax,loglen0) !is min needed
	! ! if(p)write(6,*)'loglenmin,loglenmax,volbas,nlog2',loglenmin,loglenmax,volbas,nlog2
	! ! if(nlog2.le.0.or.loglenmin.gt.loglenmax)then
	! ! j_v(jvnlog2)=j_0
	! ! j_v(jvn)=0
	! ! return
	! ! endif !if(nlog2.le.0.or.loglenmin.gt.loglenmax)  25157
	! ! lenc=j_o(iivlenc)%d(nlog)
	! ! if(p.and.nlog.gt.2)write(6,*)'nlog**',j_o(iivlenc)%d(nlog-1:nlog+1)
	! ! npatk=(loglenmax-lenc)/3
	! j_v(jvn)=npatk
	! ! j_v(jvnlog2)=nlog2
 
	! lenc+npatk0*3>=loglenmin   npatk0*3>=loglenmin  npatk0>= (loglenmin-lenc)/3
	! ! npatk0=ceiling((loglenmin-lenc )/3. )-1 !number of pieces before loglenmin
	! ! loglenmin=lenc+(npatk0+1)*3
	! ! if(p)write(6,*)'uusloglenmin ',loglenmin,npatk0
	! ! npatkd=npatk-npatk0
	! ! if(npatkd.lt.0)then
	! ! j_v(jvnlog2)=j_0
	! ! j_v(jvn)=0
	! ! return
	! ! endif !if(npatkd.lt.0)  25173
	! ! if(loglenmax.eq.nlog2*lmax.and.npatkd.eq.1)then
	! ! logtotv=0
	! ! do iad=1,nlog2
	! ! logtotv=logtotv+loglenold(iad)
	! ! suhde=suhde+loglenold(iad)*j_tempv3(logtotv)
 
	! ! enddo !iad=1,nlog2  25180
	! ! imax=1
	! ! j_o(jvv)%d(1)=j_stemcurveint(icalle,stump,&
	! ! stump+logtotv,iob,io,.false.)
	! ! j_o(jvlen)%d(1)=loglenmax
	! ! imax=1
	! ! j_o(jvpulp)%d(1)=vtot-j_o(jvv)%d(1)
	! ! npatkd=1
	! ! goto 700  !sorry
	! ! endif !if(loglenmax.eq.nlog2*lmax.and.npatkd.eq.1)  25178
 
	! if(lenc+(npatk+npatk0)*3.gt.nlog2*lmax)write(6,*)'npatk-1'
	! if(lenc+(npatk+npatk0)*3.gt.nlog2*lmax)npatk=npatk-1
	! ! if(p)write(6,*)'nlog,npatk,npatk0,loglen0 ,loglenmin,logenmax',nlog,npatk,npatk0,loglen0,loglenmin,&
	! ! loglenmax,j_o(iivh)%d(itot),'lenc',lenc,lenc+npatk*3
	! if(npatk.le.0)then
	! if(p)write(6,*)'kukuluuruu'
	! j_v(jvnlog2)=j_0
	! j_v(jvn)=0
	! return
	! endif !if(npatk.le.0)  24935
	! ilog=0
	! h1=stump
	! logtotv=0
	! n=0
	...
	! j_dapu2=j_0 !cylinder
	! j_dapu3=j_0 !saw
	! ilog=0 ! tukki johon lisätään pätkä
	! j_dapu4=lenc+stump !mista aloitetaan
	! ! if(p)write(6,*)'npatk,npatk0,npatkd',npatk,npatk0,npatkd
	! ! if(npatk.eq.0)then
	! ! suhde=j_0
	! ! logtotv=0
	! ! do iad=1,nlog2
	...
	! ! dapu3=j_dapu3
	! ! logtot=logtotv
	! ! if(p)write(6,*)'iad',iad,loglenold(iad),loglenold(iad).eq.lmax
	! ! if(loglenold(iad).eq.lmax)then
	! ! logtotv=logtot+lmax
	! ! j_dapu3=j_dapu3+lmax*j_tempv3(logtotv)
	! ! if(p)write(6,*)
	! suhde=j_dapu3
	! ! cycle
	! ! endif !if(loglenold(iad).eq.lmax)  25249
	! ! logtotv=logtotv+loglenold(iad)
	! ! j_dapu3=j_dapu3+j_tempv3(logtotv)*loglenold(iad)
	! suhde=j_dapu3
	! ! logtot=logtot+loglenold(iad)+3
	! ! dapu3=dapu3+j_tempv3(logtot)*(loglenold(iad)+3.d0) !saw
 
 
	! ! do i=iad+1,nlog2 !shift other
 
	! ! logtot=logtot+loglenold(i)
	! ! if(p)write(6,*)'i,loglenold(i),logtot',i,loglenold(i),logtot
	! ! dapu3=dapu3+loglenold(i)*j_tempv3(logtot) !saw
	! ! if(p)write(6,*)'loppu i',i,logtot,real(dapu3)
	! suhde=suhde+loglenold(i)*j_tempv3(logtot-3)
	! write(6,*)'i,logtot2,loglenold(i),j_dapu3,ad',i,logtot2,loglenold(i),j_dapu3,loglenold(i)*j_tempv3(logtot2)
	! ! enddo !i=iad+1,nlog2  25263
	! ! if(p)write(6,*)'dapu3,suhde ',real(dapu3),real(suhde)
	! ! if(dapu3.gt.suhde)then
	! ! suhde=dapu3
	! ! imax=iad+1
	...
	! ! endif !if(dapu3.gt.suhde)  25273
	! read(5,*)uuuu
	! ! enddo !iad=1,nlog2  25244
 
	! ! loglenold(imax)=loglenold(imax)+3
	! ! if(p)write(6,*)'fin2,ipat,',ipat,ipat-npatk0,real(dapu3),'imax ',imax,real(suhde)
	! ! if(ipat.gt.npatk0)then
	! ! ipatc=ipat-npatk0
	! ! if(ipatc.eq.1)then
	! ! j_o(jvlen)%d(ipatc)=loglenmin+(ipat-1)*3.d0
	! ! j_o(jvv)%d(ipatc)=volbas
 
 
	! ! else
	! ! j_o(jvlen)%d(ipatc)=j_o(jvlen)%d(ipatc-1)+3
	! ! j_o(jvv)%d(ipatc)=j_o(jvv)%d(ipatc-1)+j_stemcurveint(icalle,stump+j_o(jvlen)%d(ipatc)-3.d0,&
	! ! stump+j_o(jvlen)%d(ipatc),iob,io,.false.)
	! ! endif !if(ipatc.eq.1)  25286
	! ! if(p)write(6,*)'ipatc ',ipatc
	! ! ito=j_o(jvlen)%d(ipatc)
	! ! j_o(jvd)%d(ipatc)=j_tempv(ito)
	! ! j_o(jvs)%d(ipatc)=suhde  !sawtimber
	! ! j_o(jvc)%d(ipatc)=j_o(jvv)%d(ipatc)-suhde  !in chip
	! j_o(jvout)%d(ipatc)=dapu-dapu3
	! ! j_o(jvpulp)%d(ipatc)=vtot-j_o(jvv)%d(ipatc)
 
	! ! endif !if(ipat.gt.npatk0)  25284
	! ! enddo !ipat=1,npatk  25227
 
	! ! suhde=j_0
	! ! do ipatc=1,npatkd
	! ! suhde0=j_o(jvs)%d(ipatc)/j_o(jvv)%d(ipatc)
	! ! if(suhde0.gt.suhde)then
	! ! suhde=suhde0
	! ! imax=ipatc
	! ! endif !if(suhde0.gt.suhde)  25310
 
	! ! enddo !ipatc=1,npatkd  25308
	! ! endif !if(npatk.eq.0)  25215
	! write(6,*)'imax,npatk,j_v(jvn),',imax,npatk,j_o(jvs)%d(npatk),j_o(jvv)%d(npatk)
	! ! if(p)write(6,*)'imax,npatkd,',imax,npatkd,'jvv',j_o(jvv)%d
	! ! 700	j_v(jvn)=npatkd
	! ! j_o(jvv)%d(20)=j_o(jvv)%d(imax)
	! ! j_o(jvs)%d(20)=j_o(jvs)%d(imax)
	! write(6,*)'imax',j_o(jvv)%d(20),j_o(jvs)%d(20),j_o(jvs)%d(imax),j_o(jvs)%d(20)
	! write(6,*)j_o(jvv)%d(20),j_o(iivsawc)%d(itot),j_o(jvs)%d(20),j_o(iivvc)%d(itot),&
	! j_o(jvv)%d(20)*j_o(iivsawc)%d(itot),j_o(jvs)%d(20)*j_o(iivv)%d(itot)
	! ! if(j_o(jvs)%d(20)*j_o(iivvc)%d(itot).gt.j_o(jvv)%d(20)*j_o(iivsawc)%d(itot)) &
	! ! j_v(jvbetter)=j_1                ! S/V>s/v  S*v>V*s
	! call j_getname(jvbetter)
	! write(6,*)'bet ',j_oname(1:j_loname),j_v(jvbetter)
	! ! j_o(jvc)%d(20)=j_o(jvc)%d(imax)
	! ! j_o(jvpulp)%d(20)=j_o(jvpulp)%d(imax)
	! ! j_o(jvlen)%d(20)=j_o(jvlen)%d(imax)
	! ! j_o(jvd)%d(20)=j_o(jvd)%d(imax)
 
	! ! j_o(jvlen)%d(npatkd+1:19)=j_0
	! ! j_o(jvv)%d(npatkd+1:19)=j_0  !volume
	! ! j_o(jvs)%d(npatkd+1:19)=j_0  !sawtimber
	! ! j_o(jvc)%d(npatkd+1:19)=j_0 !in chip
	! ! j_o(jvd)%d(npatkd+1:19)=j_0
	! j_o(jvout)%d(npatk+1:20)=j_0
	! ! j_o(jvpulp)%d(npatkd+1:19)=j_0
	! if(npatk.ge.6)read(5,*)iii
	...
	if(sort)then
		if(allocated(iperm))then
			if(size(iperm).lt.npo)then
				deallocate(iperm)
				allocate(iperm(1:npo+1)) !some reserve
			endif !if(size(iperm).lt.npo)  26578
		else !if(allocated(iperm))then
			allocate(iperm(1:npo+1))
		endif !if(allocated(iperm))  26577
		...
		do i=1,npo-1
			if(tau(i).gt.tau(i+1))then
				...
				j_err=.true.
				return
			endif !if(tau(i).gt.tau(i+1))  26594
		enddo !i=1,npo-1  26593
	endif !if(sort)  26576
...
subroutine eigen(iob,io)
	...
	ELSE IF( ANRM.GT.RMAX ) THEN !IF( ANRM.GT.ZERO .AND. ANRM.LT.RMIN ) THEN
		ISCALE = 1
		SIGMA = RMAX / ANRM
	END IF !IF( ANRM.GT.ZERO .AND. ANRM.LT.RMIN )  26710
	IF( ISCALE.EQ.1 ) &
	...
	IF( ISCALE.EQ.1 ) THEN
		...
		ELSE !IF( INFO.EQ.0 ) THEN
			IMAX = INFO - 1
		END IF !IF( INFO.EQ.0 )  26740
		CALL DSCAL( IMAX, ONE / SIGMA, W, 1 )
	END IF !IF( ISCALE.EQ.1 )  26739
...
recursive subroutine call(iob,io)   !
	...
	endif !if(j_otype(ifunc).ne.j_iptrans)  26824
...
subroutine pause(iob,io)
	...
	endif !if(j_o(iob)%i(io+1).gt.0)  26861
...
subroutine text(iob,io)
	...
	end if !if(iout.eq.j_ivresult)  26949
	...
	1  write(6,1000)'text>'
		...
		if(j_inp(1:2).ne.'//')then
			...
			endif !if(clean)  26969
			goto 1
		end if !if(j_inp(1:2).ne.'//')  26968
...
195		if(j_o(iiv)%txt(irow+1)(1:2).ne.'//')then
	!		write(6,*)'<5454>',j_o(iiv)%txt(irow+1)(1:j_o(iiv)%i2(irow+1))
			...
			endif !if(clean)  26981
			irow=irow+1
			goto 195
		end if !195		if(j_o(iiv)%txt(irow+1)(1:2).ne.'//')  26979
		j_o(iiv)%i(6)=irow+1
 
	endif !if(j_ninc.eq.1)  26961
...
subroutine txt(iob,io)   !
	...
	end if !if(iout.eq.j_ivresult)  27016
	...
	enddo !while(j_inpr(1:2).ne.'//')  27034
...
subroutine crossed(iob,io)  !
	...
	do i=2,j_dnobs
		...
		if(class.ne.classv)then
			...
			endif !if(nclass.gt.mxclass)  27182
			classv=class
			inclass(nclass)=0
 
		endif !if(class.ne.classv)  27175
		inclass(nclass)=inclass(nclass)+1
 
	enddo !i=2,j_dnobs  27173
	if(sym)then
		nobc=nobc+inclass(nclass)*(inclass(nclass)+1)/2
	else !if(sym)then
		nobc=nobc+inclass(nclass)*inclass(nclass)
	...
	do j_iobs=j_dfrom,j_duntil
		...
		if(iclass.eq.inclass(ncl))then
			...
			iclass=0
			ncl=ncl+1
		endif !if(iclass.eq.inclass(ncl))  27242
 
	enddo !j_iobs=j_dfrom,j_duntil  27213
	!enddo !do k=1,jndatasetss
...
subroutine abs(iob,io) ! abs(   absolute value
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		j_err=.true.
		return
	endif !if(j_otype(irg).eq.j_ipreal)  27262
	...
	10		continue
			...
			if(ndim2.lt.0.and.n.gt.0)then  !need to c
				...
				if((ivcolmin.gt.0.or.ndim2.gt.0).and.ivalmin2.lt.icolmin)then
					...
					goto 90
				endif !if((ivcolmin.gt.0.or.ndim2.gt.0).and.ivalmin2.lt.icolmin)  27368
 
				ivalmin=min(ivalmin,ivalmin2)
				ivalmax=max(ivalmax,maxval(ivals(1:n)))
			endif !if(ndim2.lt.0.and.n.gt.0)  27355
		...
		do iobs=1,ndim1
			...
			do k=1,n
				...
				if(ivals(k).lt.icolmin.or.ivals(k).gt.ndim2)then
					...
					return
				endif !if(ivals(k).lt.icolmin.or.ivals(k).gt.ndim2)  27424
				call j_bitset(iout,iobs,ivals(k),1.)
 
			enddo !k=1,n  27417
 
		enddo !iobs=1,ndim1  27408
		deallocate(ivals)
	endif !if(ivin.ge.0)  27407
	if(ivin.eq.0)then
		...
		if(loppu(1:1).ne.'/')then
			...
			j_err=.true.
		end if !if(loppu(1:1).ne.'/')  27441
 
	end if !if(ivin.eq.0)  27438
...
subroutine setvalue(iob,io) !set value for a bitmatrix  %%bitmatrix
	...
	if(j.gt.j_o(irg)%i(1).or.j.le.0)then
		...
		j_err=.true. ;return
	endif !if(j.gt.j_o(irg)%i(1).or.j.le.0)  27480
	...
	if(j_o(irg)%i(2).ne.j_o(irg)%i(3))then
		...
		j_err=.true.;return
	endif !if(j_o(irg)%i(2).ne.j_o(irg)%i(3))  27488
	icol=j_o(irg)%i(2)
	...
	if(icol.lt.j_o(irg)%i(3).or.icol.gt.j_o(irg)%i(2))then
		...
		j_err=.true. ;return
	endif !if(icol.lt.j_o(irg)%i(3).or.icol.gt.j_o(irg)%i(2))  27496
...
subroutine closures(iob,io)             !
	...
	if(j_o(irg)%i(3).ne.1.or.j_o(irg)%i(2).ne.j_o(irg)%i(1))then
		...
		j_err=.true.
		return
	endif !if(j_o(irg)%i(3).ne.1.or.j_o(irg)%i(2).ne.j_o(irg)%i(1))  27536
	ndim=j_o(irg)%i(1)
	...
	do i=1,ndim   !row
		do j=1,ndim
			...
			endif !if(i.eq.j)  27548
		enddo !j=1,ndim  27547
	enddo !i=1,ndim  27546
	lkm=1
		...
		do j=1,ndim
			if(i.eq.j)then
				...
				if(itemp(i,j))then
					...
					endif !if(all(itemp(j,nei(lkm,1:nn))))  27573
				endif !if(itemp(i,j))  27570
			endif !if(i.eq.j)  27562
 
		enddo !j=1,ndim  27561
			...
			do jj=1,nn2
				...
				if(i.eq.j)then
					...
					if(itemp(key,j))then
						...
						endif !if(all(itemp(j,nei(lkm,1:nn))))  27609
 
					endif !if(itemp(key,j))  27606
				endif !if(i.eq.j)  27601
			enddo !jj=1,nn2  27599
			goto 15
 
		endif !if(key2.gt.0)  27594
 
	enddo !i=1,ndim  27557
...
subroutine difference(iob,io)
	...
	if(j_otype(irg1).ne.j_iplist)then
		...
		j_err=.true.
		return
	endif !if(j_otype(irg1).ne.j_iplist)  27687
	if(j_otype(irg2).ne.j_iplist)then
		...
		if(ii2.lt.j_o(irg1)%i(1))then
			...
			enddo !ij=ii2,j_o(irg1)%i(1)-1  27701
		endif !if(ii2.lt.j_o(irg1)%i(1))  27700
		j_o(irg1)%i(1)=j_o(irg1)%i(1)-1   ! there remains unused elements
		return
	endif !if(j_otype(irg2).ne.j_iplist)  27692
...
subroutine interpolate(iob,io)
	...
	if(narg.gt.2.and.2*np.eq.narg)then
		...
		j_err=.true.
		return
	endif !if(narg.gt.2.and.2*np.eq.narg)  27770
	iout=j_o(iob)%i(io+2+narg)
	if(narg.gt.3)then
	...
	elseif(narg.eq.3)then !if(narg.eq.5)then
		if(j_otype(argx(1)).ne.j_ipmatrix.or.j_otype(argx(2)).ne.j_ipmatrix)then
			...
			j_err=.true.
			return
		endif !if(j_otype(argx(1)).ne.j_ipmatrix.or.j_otype(argx(2)).ne.j  27850
		iy=argx(2)
		ibasy=0
		...
		endif !if(j_o(argx(1))%i(1).gt.1.and.j_o(argx(1))%i(2).gt.1.o  27858
 
 
 
 
	endif !if(narg.eq.5)  27785
	found=.false.
	if(ismatrix)then
		j_v(iout)=j_rlinterm(argx(1),arg)
		do i=1,nval-1
			if(arg.ge.j_o(argx(1))%d(i).and.arg.le.j_o(argx(1))%d(i+1).or. &
				...
				found=.true.
				exit
			endif !if(arg.ge.j_o(argx(1))%d(i).and.arg.le.j_o(argx(1))%d(i+1)  27874
 
		enddo !i=1,nval-1  27873
		...
		do i=1,np-1
			if(arg.ge.j_v(argx(i)).and.arg.le.j_v(argx(i+1)).or. &
				...
				exit
			endif !if(arg.ge.j_v(argx(i)).and.arg.le.j_v(argx(i+1)).o  27891
		enddo !i=1,np-1  27890
 
		if(found)j_v(iout)=j_rlinter(j_v(argx(i)),j_v(argx(i+1)),&
			j_v(argy(i)),j_v(argy(i+1)),arg)
 
	endif !if(ismatrix)  27871
	if(.not.found)then
		write(6,*)'no part found for ',arg
		j_err=.true.
 
	endif !if(.not.found)  27903
...
subroutine sign(iob,io)  !
	...
	if(j_otype(irg).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal)then
		...
		if(j_v(irg2).ge.j_0)then
			...
			j_v(iout)=-abs(j_v(irg))
		endif !if(j_v(irg2).ge.j_0)  27924
	elseif(j_otype(irg).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipreal)then
		...
		if(j_v(irg2).ge.j_0)then
			...
			j_o(iout)%d(1:nd)=-abs(j_o(irg)%d(1:nd))
		endif !if(j_v(irg2).ge.j_0)  27932
	else
		call j_getname(irg,irg2)
		write(6,*)'sign(), illegal arguments ', j_oname(1:j_loname),' ',j_oname2(1:j_loname2)
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal)  27922
	!io=io+narg+3
	return
...
subroutine getobs(iob,io)   !this is j_function
	...
	if(j_otype(ivdata).ne.j_ipdata)then
		...
		j_err=.true.
		return
	endif !if(j_otype(ivdata).ne.j_ipdata)  27971
	...
	if(j_iobs.le.0.or.j_iobs.gt.nobs )then
		...
		j_err=.true.
		return
	endif !if(j_iobs.le.0.or.j_iobs.gt.nobs )  27977
	...
	if(ivtrans.gt.0)then
		if(j_otype(ivtrans).ne.j_iptrans)then
			...
			j_err=.true.
			return
		endif !if(j_otype(ivtrans).ne.j_iptrans)  27987
		call dotrans(ivtrans,1)
		if(j_err)return
		!!call j_clearoption(iob,io)  ! subroutine
	endif !if(ivtrans.gt.0)  27986
...
subroutine der(iob,io)
	...
	do while(.true.)
		...
		endif !if(io2.eq.iexitloc)  28096
...
136					isilo2=il2.gt.0  !is second argument in der () ,il2 is then the position
			...
			endif !if(isvar2)  28132
		endif !if(isarg2)  28128
		...
		elseif(isilo2)then !if(.not.isvar2)then
			...
			if(u.le.j_0)then
				write(6,*)'in derivates, nonpositive number ',u,' raised to power'
				j_err=.true.;return
			endif !if(u.le.j_0)  28293
			j_o(iout)%d(il2)=j_o(iout)%d(il2)+log(u)*j_v(iout)
		endif !if(.not.isvar2)  28282
...
subroutine itraceoff(iob,io)
	...
	do i=1,narg
		...
		endif !if(il.le.0)  28434
	enddo !i=1,narg  28432
...
subroutine trace(iob,io)
	...
	if(imi.gt.0)then
		...
		elseif(narg.ne.nmin.and.nmin.ne.0)then !if(nmin.eq.1)then
			...
			j_err=.true.
			return
		endif !if(nmin.eq.1)  28464
		ismin=iexit
	endif !if(imi.gt.0)  28461
	...
	if(imi.gt.0)then
		...
		elseif(narg.ne.nmax.and.nmax.ne.0)then !if(nmax.eq.1)then
			...
			j_err=.true.
			return
		endif !if(nmax.eq.1)  28480
		ismax=iexit
	endif !if(imi.gt.0)  28478
	if(nmax.lt.0.and.nmin.lt.0.and.iexit.gt.1)then
		write(6,*)'trace:, errexit must be with min-> or max->'
 
	endif !if(nmax.lt.0.and.nmin.lt.0.and.iexit.gt.1)  28490
	...
	if(ivlevel.gt.0)then
		...
		endif !if(tracelevel.lt.0.or.tracelevel.gt.2)  28502
 
	endif !if(ivlevel.gt.0)  28500
	...
	do i=1,narg
		...
		endif !if(nmax.ge.0)  28523
	enddo !i=1,narg  28508
...
subroutine existobject(iob,io)  !  vapaa
	...
	if(j_otype(irg).ne.j_ipchar)then
		...
		j_err=.true.
		return !goto 90 !return
	endif !if(j_otype(irg).ne.j_ipchar)  28642
	...
	endif !if(j_linkoption(iob,io,j_mobject).ge.0)  28648
	!!call j_clearoption(iob,io)  ! subroutine
...
subroutine exist(iob,io)  !  exist(
	...
	if(j_otype(irg).ne.j_ipchar)then
		...
		j_err=.true.
		return !goto 90 !return
	endif !if(j_otype(irg).ne.j_ipchar)  28670
	call j_getchar(irg,j_filename,le)
	if(j_linkoption(iob,io,j_mobject).ge.0)then
		...
		inquire(file=j_filename(1:le),exist=ex)
		if(ex)j_v(iout)=1.d0
	endif !if(j_linkoption(iob,io,j_mobject).ge.0)  28676
...
subroutine laaspoly(iob,io)   !
	...
	endif !if(j_otype(iout).le.j_ipreal)  28732
...
subroutine negbin(iob,io)  !
	...
	endif !if(arg.lt.j_0.or.amyy.lt.j_0.or.ak.lt.j_0)  28822
	if(amyy.eq.j_0)then
		if(arg.eq.j_0)then
			j_v(iout)=j_1
		else
			j_v(iout)=j_0
		endif !if(arg.eq.j_0)  28830
	endif !if(amyy.eq.j_0)  28829
	if(ak.eq.j_0)then
		iarg=arg
		if(dble(iarg).ne.iarg)then
			...
			j_err=.true.
			return
		endif !if(dble(iarg).ne.iarg)  28838
		! kert=j_1
		! do i=1,iarg
		! kert=kert+kert*i
		! enddo !do i=1,iarg
...
subroutine density(iob,io)
	...
	if(linkfunc.gt.0)then
		...
		if(isdisc)then
			...
			enddo !i=1,ncol  28940
			do i=1,ncol
				j_o(iout)%d(ncol+i)=j_o(iout)%d(ncol+i)/sum
			enddo !i=1,ncol  28947
			...
			enddo !i=2,ncol  28959
			do i=1,ncol
				j_o(iout)%d(ncol+i)=j_o(iout)%d(ncol+i)/sum
			enddo !i=1,ncol  28966
		endif !if(isdisc)  28936
		return
	...
	elseif(narg.eq.1)then
		if(j_o(arg(1))%i(4).ne.j_matclass)then
			...
			j_err=.true. ;return
			!goto 900
		endif !if(j_o(arg(1))%i(4).ne.j_matclass)  28973
		nrow_=j_o(arg(1))%i(1)
		ncol=j_o(arg(1))%i(2)
...
subroutine bin(iob,io)  !binomial distribution
	...
	endif !if(k.lt.0.d0)  29029
	ios=1
	do i=k2+1,n
		ios=ios*i
	enddo !i=k2+1,n  29036
	in=1
	do i=1,nk
		in=in*i
	enddo !i=1,nk  29040
...
subroutine mod_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)  29074
...
subroutine fraction_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)  29094
