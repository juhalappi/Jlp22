...
subroutine j_jinit(jparf)
	...
	do i=1,j_nnamedfuncarg
		...
		endif !if(ifun.le.0)    304
	enddo !i=1,j_nnamedfuncarg    301
	do i=1,o1_nnamedfuncarg
		...
		endif !if(ifun.le.0)    314
	enddo !i=1,o1_nnamedfuncarg    311
	...
	do i=1,o2_nnamedfuncarg
		...
		endif !if(ifun.le.0)    325
	enddo !i=1,o2_nnamedfuncarg    322
	!write(6,*)'olo2'
	do i=1,o3_nnamedfuncarg
		...
		endif !if(ifun.le.0)    336
	enddo !i=1,o3_nnamedfuncarg    333
	!write(6,*)'olo3'
	do i=1,j_nnamedoptarg
		...
		endif !if(ifun.le.0)    347
	enddo !i=1,j_nnamedoptarg    344
	!write(6,*)'olnam'
	do i=1,o1_nnamedoptarg
		...
		endif !if(ifun.le.0)    358
	enddo !i=1,o1_nnamedoptarg    355
	!write(6,*)'olnamo1'
	do i=1,o2_nnamedoptarg
		...
		endif !if(ifun.le.0)    369
	enddo !i=1,o2_nnamedoptarg    366
	...
	do i=1,o3_nnamedoptarg
		...
		endif !if(ifun.le.0)    380
	enddo !i=1,o3_nnamedoptarg    377
	!write(6,*)'olnamo3'
	!newvar
	do i=1,j_nnewvar
		...
		endif !if(ifun.le.0)    392
	enddo !i=1,j_nnewvar    389
	...
	do i=1,o1_nnewvar
		...
		endif !if(ifun.le.0)    403
	enddo !i=1,o1_nnewvar    400
	...
	do i=1,o2_nnewvar
		...
		endif !if(ifun.le.0)    414
	enddo !i=1,o2_nnewvar    411
	...
	do i=1,o3_nnewvar
		...
		endif !if(ifun.le.0)    425
	enddo !i=1,o3_nnewvar    422
	!write(6,*)'olnew'
	!codeoptions
	...
	do i=1,o1_ncodeoptions,1
		...
		endif !if(ifun.le.0)    445
	enddo !i=1,o1_ncodeoptions,1    442
	...
	do i=1,o2_ncodeoptions,1
		...
		endif !if(ifun.le.0)    457
	enddo !i=1,o2_ncodeoptions,1    454
	...
	do i=1,o3_ncodeoptions,1
		...
		endif !if(ifun.le.0)    470
	enddo !i=1,o3_ncodeoptions,1    467
	!write(6,*)'olcode3'
...
567		lep=len_trim(jparf)
	...
	if(jpar)then
		...
		if(le.gt.1.and.j_cline(1:1).eq.'*')then
			...
			if(j_mxnamedv.lt.200)then
				...
				j_err=.true.
				return
			endif !if(j_mxnamedv.lt.200)    524
		endif !if(le.gt.1.and.j_cline(1:1).eq.'*')    522
	...
	if(j_ivcursori.ne.ivout_)then
		...
		j_stop=.true.
		return
	endif !if(j_ivcursori.ne.ivout_)    833
...
recursive subroutine dotrans(iob,ioi)
	...
	data gotos/&
		...
		251,252,253,254,255,256,257,258,259,260,261,262,263,264,265,266,267,268,269,270, & ! linear programming
	...
	if(j_recursion.gt.j_mxrecursion)then
		...
		j_err=.true.
		return
	endif !if(j_recursion.gt.j_mxrecursion)    906
	...
	if(j_o(iob)%i(io).lt.0)then
		...
		if(j_v(j_ivdebugtrans).ne.j_0)call j_debug(iob)
		io=io+1
	endif !if(j_o(iob)%i(io).lt.0)    966
	! if(j_nopt.gt.0)then
	...
	if(j_v(j_ivdollar2).eq.179.d0)then
		...
		if(j_o(iob)%i(io).ne.0)then
			...
			write(6,*)io,'finito'
		endif !if(j_o(iob)%i(io).ne.0)    992
	endif !if(j_v(j_ivdollar2).eq.179.d0)    987
...
subroutine setcodeopt(iob,io)
	...
	endif !if(j_nopt2.gt.j_maxopenopt2)   1791
	...
	if(j_o(iob)%i(io+2).eq.0)then    !nothing to be computed
		...
		io=j_o(iob)%i(io+2)
	endif !if(j_o(iob)%i(io+2).eq.0)   1802
...
subroutine setoption(iob,io)
	...
	if(j_nopt.ge.j_maxopenopt.or.(j_v(j_ivdollar).eq.455.D0.and.j_nopt.gt.0))then
		...
		j_err=.true.
		return
	endif !if(j_nopt.ge.j_maxopenopt.or.(j_v(j_ivdollar).eq.455.D0.an   1826
	j_nopt=j_nopt+1
...
recursive subroutine getelem(iob,io)  ! matrix(iel) e.g. out=a(irow,icol)  %%matrix
	...
	if(ismatrix.or.isreal)then
		...
		if(j_otype(iout).eq.j_iplist)then
			call getsetlist()
			return
		endif !if(j_otype(iout).eq.j_iplist)   2068
		...
		if(isrealfirst)then
			...
			enddo !i=2,narg1   2115
			if(ismatrix.or.isreal)then
				if(narg1.gt.4)then
					write(6,*)'illegal indices for matrix ',j_oname(1:j_loname)
					j_err=.true.;return
				endif !if(narg1.gt.4)   2120
				inde(1:narg1)=j_v(arg)
			endif !if(ismatrix.or.isreal)   2119
		elseif(j_otype(arg(1)).eq.j_ipilist)then
			...
			elseif(narg1.eq.2)then
				...
				if(ncol.gt.1)then
					...
					j_err=.true.
					return
				endif !if(ncol.gt.1)   2134
			endif !if(narg1.gt.2)   2127
...
545		continue
		...
		if(isvector)then
			...
			isrow=.false.
		endif !if(isvector)   2167
		if(norange)then
			irange=1
			call iranges()
		endif !if(norange)   2173
		...
		if(get)then
			...
			if(iout.eq.imat.and..not.isreal)then
				...
				goto 100
			endif !if(iout.eq.imat.and..not.isreal)   2215
			call getmatrix()
			if(p)then
			...
			if(j_otype(iout).eq.j_ipreal)then
				...
				return
			endif !if(j_otype(iout).eq.j_ipreal)   2230
			...
			if(simple)then
				if(j_otype(iout).ne.j_ipreal)then
					...
					j_err=.true.;return
				endif !if(j_otype(iout).ne.j_ipreal)   2241
				if(issum)then
					if(isvector)then
					...
					endif !if(isvector)   2257
 
				endif !if(issum)   2247
				return
			endif !if(simple)   2240
			call setmatrix()
			if(p)then
	...
	endif !if(ismatrix.or.isreal)   2050
	...
	if(narg1.eq.0)then
		...
		if(j_otype(imat).eq.j_ipregr)then
			...
			j_err=.true.
		endif !if(j_otype(imat).eq.j_ipregr)   2283
		return
	endif !if(narg1.eq.0)   2279
	if(islist)then
		if(narg1.ne.1)then
			write(6,*)'list() requires one index argument'
			j_err=.true.;return
		endif !if(narg1.ne.1)   2293
		ind=j_v(arg(1))
		...
		if(set)then
			...
			endif !if(j_otype(iel).eq.j_ipreal)   2304
		endif !if(set)   2301
		return
	endif !if(islist)   2292
	...
	elseif(j_otype(imat).eq.j_ipdata)then !if(j_otype(imat).eq.j_iptrans)then
		if(.not.get.and.j_o(imat)%i(10).eq.0)then
			...
			j_err=.true.;return
		endif !if(.not.get.and.j_o(imat)%i(10).eq.0)   2348
		call data()
		return
	...
	endif !if(j_otype(imat).eq.j_iptrans)   2326
	...
	end if !100 if(j_o(iout)%i(1).eq.1.and.j_o(iout)%i(2).eq.1)   2377
	...
	subroutine iranges()
		...
		if(j_otype(arg(1)).eq.j_ipilist)then
			...
			isili(1)=.false.
		endif !if(j_otype(arg(1)).eq.j_ipilist)   2409
		if(arg(1).eq.j_ivall)then
			...
			irange(1)=inde(1)
			irange(2)=inde(1)
		endif !if(arg(1).eq.j_ivall)   2421
		...
		if(j_otype(arg(1)).eq.j_ipilist)then
			...
			endif !if(mro.gt.j_o(imat)%i(1))   2446
			...
			endif !if(j_otype(arg(2)).eq.j_ipilist)   2451
		else !if(j_otype(arg(1)).eq.j_ipilist)then
			...
			elseif(arg(2).eq.j_ivall)then
				...
				irange(4)=inde(2)
				simple=.true.
			endif !if(inde(2).lt.0)   2479
			!endif
			! if(.not.isvector)then
			! irange(4)=ncol
			! endif !if(.not.isvector)then
		endif !if(j_otype(arg(1)).eq.j_ipilist)   2439
		...
		if(arg(1).eq.j_ivall)then
			...
			return
		endif !if(arg(1).eq.j_ivall)   2506
		irange(1)=inde(1)
		if(inde(2).lt.0)then
			irange(2)=-inde(2)
			if(arg(2).eq.j_ivtolast)irange(2)=nrow
			...
			if(arg(3).eq.j_ivall)then
				irange(4)=ncol
				return
			endif !if(arg(3).eq.j_ivall)   2520
	...
	subroutine getmatrix()
		if(isreal)then
			if(irange(2).gt.1.or.irange(4).gt.1)then
				...
				write(6,*)'illegal ranges ',inde(1:narg1),' for REAL ',&
					j_oname(1:j_loname)
				j_err=.true.;return
			endif !if(irange(2).gt.1.or.irange(4).gt.1)   2560
			if(j_otype(iout).ne.j_ipreal)call j_del(iout)
			j_v(iout)=j_v(imat)
			return
		endif !if(isreal)   2559
		useold=.false.
		if(j_otype(iout).eq.j_ipmatrix)then
			useold=j_o(iout)%i(3).eq.nel2
		endif !if(j_otype(iout).eq.j_ipmatrix)   2571
		...
		if(isvector)then
			...
			if(irange(2).gt.j_o(imat)%i(1).or.irange(4).gt.j_o(imat)%i(2))then
				...
				j_err=.true.;return
			endif !if(irange(2).gt.j_o(imat)%i(1).or.irange(4).gt.j_o(imat)%i   2615
			do iro0=irange(1),irange(2)
				...
				if(isili(2))then
					...
					if(iscoef)then
						...
						enddo !ico=1,ncol2   2628
 
					else !if(iscoef)then
						do ico=1,ncol2
							ie=ie+1
							j_o(iout)%d(ie)=j_o(imat)%d((iro-1)*ncol+ilist2(ico))
						enddo !ico=1,ncol2   2634
					endif !if(iscoef)   2627
					...
					if(iscoef)then
						...
						enddo !ico=irange(3),irange(4)   2641
						...
						enddo !ico=irange(3),irange(4)   2648
					endif !if(iscoef)   2640
				endif !if(isili(2))   2625
			enddo !iro0=irange(1),irange(2)   2621
			...
			if(ist.and.isvectorout)then
				...
				j_o(iout)%i(2)=j_o(iout)%i(1)
				j_o(iout)%i(1)=i2
			endif !if(ist.and.isvectorout)   2665
		endif !if(isvector)   2592
			...
			if(isvector)then
				...
				do iro0=irange(1),irange(2)
					...
					if(isili(2))then
						...
						enddo !ico=1,ncol2   2694
					else !if(isili(2))then
						ibas=(iro-1)*ncol
						j_o(imat)%d(ibas+irange(3):ibas+irange(4))=&
							j_o(imat)%d(ibas+irange(3):ibas+irange(4))+sumcoef*j_v(iout)
					endif !if(isili(2))   2692
				enddo !iro0=irange(1),irange(2)   2689
			endif !if(isvector)   2677
		else !if(issum)then
			if(isvector)then
				if(isili(1))then
				...
				do iro0=irange(1),irange(2)
					...
					if(isili(2))then
						...
						enddo !ico=1,ncol2   2722
					else !if(isili(2))then
						ibas=(iro-1)*ncol
						j_o(imat)%d(ibas+irange(3):ibas+irange(4))=j_v(iout)
					endif !if(isili(2))   2720
				enddo !iro0=irange(1),irange(2)   2717
			endif !if(isvector)   2707
		endif !if(issum)   2676
	...
	subroutine setmatrix()
		if(nel2.ne.j_o(iout)%i(3))then
			...
			j_err=.true.;return
		endif !if(nel2.ne.j_o(iout)%i(3))   2736
			...
			if(isvector)then
				...
				do iro0=irange(1),irange(2)
					...
					if(isili(2))then
						...
						ibas=ibas+ncol
						ie=ie+ncol2
					endif !if(isili(2))   2758
				enddo !iro0=irange(1),irange(2)   2755
			endif !if(isvector)   2746
			...
			if(isvector)then
				...
				do iro0=irange(1),irange(2)
					...
					if(isili(2))then
						...
						ibas=ibas+ncol
						ie=ie+ncol2
					endif !if(isili(2))   2788
				enddo !iro0=irange(1),irange(2)   2785
			endif !if(isvector)   2776
		endif !if(issum)   2744
	...
	subroutine trans()
		...
		if(nres.gt.0)then
			do ir=1,nres
				...
				endif !if(j_otype(j_mxnamedv+ir).eq.j_ipreal)   2829
			enddo !ir=1,nres   2828
			call dotrans(imat,1)
			j_v(j_mxnamedv+1:j_mxnamedv+nres)=j_temporals(1:nres)
		else
			call dotrans(imat,1)
 
		endif !if(nres.gt.0)   2827
		...
		if(ivcases.ne.0)then
			...
			if(ice.le.0)then
				...
				if(ice.le.0.or.ice.gt.j_o(ivmat)%i(1))then
					...
					j_err=.true.;return
				endif !if(ice.le.0.or.ice.gt.j_o(ivmat)%i(1))   2871
 
			endif !if(ice.le.0)   2869
			...
			if(iro.le.0)then
				...
				if(iro.le.0.or.iro.gt.j_o(ivmat)%i(2))then
					...
					j_err=.true.;return
				endif !if(iro.le.0.or.iro.gt.j_o(ivmat)%i(2))   2881
 
			endif !if(iro.le.0)   2879
			...
			if(get)then
				...
				if(j_otype(iout).ne.j_ipreal)then
					...
					j_err=.true.
					return
				endif !if(j_otype(iout).ne.j_ipreal)   2894
...
469			allocate(list(1:narg1))
		...
		do j=1,narg1
			...
			if(list(j).le.0)then
				...
				j_err=.true.
			endif !if(list(j).le.0)   2926
		enddo !j=1,narg1   2923
		if(j_err)then
		...
		if(isvector)then
			...
			do iro0=irange(1),irange(2)
				...
				do ico=irange(3),irange(4)
					...
					endif !if(isili(2))   3011
				enddo !ico=irange(3),irange(4)   3009
 
			enddo !iro0=irange(1),irange(2)   3006
		endif !if(isvector)   2997
		call j_defmatrix(iout,' ',nrow2,ncol2,j_matreg,iout)
		j_o(iout)%d=temp
		deallocate(temp)
		return
	...
	subroutine diags()
			...
			if(get)then
				...
				elseif(j_otype(iout).eq.j_ipmatrix)then !if(j_otype(iout).eq.j_ipreal)then
					if(nrow.ne.j_o(iout)%i(3))then
						...
						j_err=.true.;return
					endif !if(nrow.ne.j_o(iout)%i(3))   3060
					...
					j_err=.true.
				endif !if(j_otype(iout).eq.j_ipreal)   3052
				return
			endif !if(get)   3036
		endif !if(narg1.eq.0)   3034
		irange(1:2)=1
		irange(3:4)=0
		call iranges()
		write(6,*)'ira',irange
		...
		endif !if(irange(3).ne.0.and.(irange(3).ne.irange(1).or.irange(4)   3081
		...
		if(get)then
			...
			do i=irange(1),irange(2)
				...
				ibas=ibas+nrow
			enddo !i=irange(1),irange(2)   3103
			return
		else !if(get)then
			if(j_otype(iout).eq.j_ipreal)then
				...
				do i=irange(1),irange(2)
					j_o(imat)%d(ibas+i)=j_v(iout)
					ibas=ibas+nrow
				enddo !i=irange(1),irange(2)   3112
				return
			...
			elseif(j_otype(iout).eq.j_ipmatrix)then !if(j_otype(iout).eq.j_ipreal)then
				if(nrow2.ne.j_o(iout)%i(3))then
					...
					j_err=.true.;return
				endif !if(nrow2.ne.j_o(iout)%i(3))   3119
				i2=0
				do i=irange(1),irange(2)
					i2=i2+1
					!				write(6,*)'imat,iout,ibas,ir1,ir2',imat,iout,ibas,ir1,ir2
					...
					ibas=ibas+nrow
				enddo !i=irange(1),irange(2)   3124
				return
			else !if(j_otype(iout).eq.j_ipreal)then
				write(6,*)'input is not MATRIX or REAL'
				j_err=.true.;return
			endif !if(j_otype(iout).eq.j_ipreal)   3110
			...
			if(isvector)then
				...
				if(get)then
					do iro=irange(1),irange(2)
						...
						enddo !ico=irange(3),irange(4)   3157
 
					enddo !iro=irange(1),irange(2)   3156
				else !if(get)then
					do iro=irange(1),irange(2)
						do ico=irange(3),irange(4)
							ie=ie+1
						...
						enddo !ico=irange(3),irange(4)   3167
					enddo !iro=irange(1),irange(2)   3166
 
				endif !if(get)   3155
			endif !if(isvector)   3141
			return
		else !if(nel2.eq.j_o(iout)%i(1))then
			write(6,*)'output list has wrong size ',j_o(iout)%i(1),' should be ',nel2
			j_err=.true.
		endif !if(nel2.eq.j_o(iout)%i(1))   3140
...
subroutine setelem(iob,io)  ! %%matrix put matrix element made from a(i,j)= -statements
	...
	if(j_otype(imat).eq.j_ipilist)then
		if(arg(1).eq.j_ivzero)then
			if(ivin.ne.j_ivzero)then
				...
				j_err=.true.;return
			endif !if(ivin.ne.j_ivzero)   3211
			j_o(imat)%i(1)=0
			...
			if(inde1.gt.j_o(imat)%i(4))then
				...
				j_err=.true.
			endif !if(inde1.gt.j_o(imat)%i(4))   3217
			j_o(imat)%i2(inde1)=j_v(ivin)
			j_o(imat)%i(1)=max(j_o(imat)%i(1),inde1)
		endif !if(arg(1).eq.j_ivzero)   3210
		return
	endif !if(j_otype(imat).eq.j_ipilist)   3209
	...
	if(j_otype(imat).ne.j_iplist)then !if(j_otype(imat).eq.j_iplist)then
		...
		j_err=.true. ;return
	end if !if(j_otype(imat).ne.j_iplist)   3227
	...
	if(narg1.eq.1)then
		...
		if(inde1.le.0.or.inde1.gt.j_o(imat)%i(3))then
			...
			j_err=.true.
			return
		endif !if(inde1.le.0.or.inde1.gt.j_o(imat)%i(3))   3237
		...
		endif !if(j_otype(imat).eq.j_iplist)   3243
		return
 
	endif !if(narg1.eq.1)   3233
	...
	if(j_otype(ivin).eq.j_ipmatrix)then
		...
		endif !if(j_otype(imat).eq.j_ipmatrix)   3278
	else !if(j_otype(ivin).eq.j_ipmatrix)then
		if(j_otype(imat).eq.j_ipmatrix)then
			j_o(imat)%d(inde1:inde2)=j_v(ivin)
		else !if(j_otype(imat).eq.j_ipmatrix)then
			j_v(j_o(imat)%i2(inde1:inde2))=j_v(ivin)
		endif !if(j_otype(imat).eq.j_ipmatrix)   3284
	endif !if(j_otype(ivin).eq.j_ipmatrix)   3277
	return
...
subroutine ilist(iob,io)
	...
	134			j_o(ivout)%i2(i)=iloc
		...
		if(narg.eq.0.and.noptarg.eq.0)then
			...
			j_err=.true.;return
		endif !if(narg.eq.0.and.noptarg.eq.0)   3390
		if(noptarg.gt.0)then
			...
			if(nextra.gt.0)then
				...
				j_err=.true.;return
			endif !if(nextra.gt.0)   3402
			nin=narg+nextra
			call  j_deflistobject(ivout,' ',iout,nres=nin)
 
			j_o(iout)%i2(1:narg)=j_v(arg)
			j_o(iout)%i(1)=narg
		endif !if(noptarg.gt.0)   3394
 
	endif !if(noptarg.eq.1)   3374
	!subroutine j_deflist(iv,name,list0,ivout) !allocates list object with size list0,
...
subroutine putlist(iob,io)
	...
	if(j_otype(arg(1)).ne.j_iplist.and.j_otype(arg(1)).ne.j_ipilist)then
		...
		j_err=.true.;return
	endif !if(j_otype(arg(1)).ne.j_iplist.and.j_otype(arg(1)).ne.j_ip   3432
	nle=j_o(arg(1))%i(1)
	narg1=narg-1
	nle2=nle+narg1
...
subroutine type(iob, io)  !
	...
	endif !if(j_linkoption(iob,io,j_mcontent).ge.0.and.j_otype(iarg)=   3484
...
subroutine delete_f(iob,io)
	...
	do i=1,narg
		...
		if(j_otype(iv).ne.j_ipchar)then
			...
			j_err=.true.
			cycle
		endif !if(j_otype(iv).ne.j_ipchar)   3515
...
subroutine exist_f(iob,io)
	...
	if(j_otype(iv).ne.j_ipchar)then
		call j_printname('object ',iv ,' is not a character object which could point to a file')
		j_err=.true.;return
	endif !if(j_otype(iv).ne.j_ipchar)   3547
	nu_ = j_nunits(j_nused+1)	! väliaikainen kanava tiedostolle
	if (j_iounit(iv)>0)then !j_o(iv)%i(4)>0) then! aukioleva tiedosto
		nu_ = j_iounit(iv)  !j_o(iv)%i(4)
		call j_closeunit(nu_)
	endif !if (j_iounit(iv)>0)   3552
	call j_getchar(iv,j_filename,le_)
	inquire(file = j_filename(1:le_) , exist=exis)
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	if(exis) then
	...
	if(narg.ge.2)then
		...
		if (j_iounit(iv)>0)then !j_o(iv)%i(4)>0) then! aukioleva tiedosto
			...
			j_v(iv2)=j_0
		endif !if (j_iounit(iv)>0)   3566
	endif !if(narg.ge.2)   3564
	return
...
subroutine exist_o(iob,io)
	...
	if(j_otype(iv).ne.j_ipchar)then
		...
		j_err=.true.;return
	endif !if(j_otype(iv).ne.j_ipchar)   3580
...
subroutine enddo(iob,io)
	...
	if(j_o(iob)%i(iod+8).gt.0)then
		...
		if(j_o(iob)%i(iod+6).gt.j_o(iob)%i(iod+7))then
			...
			io=io+2
			return
		endif !if(j_o(iob)%i(iod+6).gt.j_o(iob)%i(iod+7))   3648
	else !if(j_o(iob)%i(iod+8).gt.0)then
		if(j_o(iob)%i(iod+6).lt.j_o(iob)%i(iod+7))then
 
			io=io+2
			return
		endif !if(j_o(iob)%i(iod+6).lt.j_o(iob)%i(iod+7))   3654
	endif !if(j_o(iob)%i(iod+8).gt.0)   3646
...
subroutine if(iob,io)
	...
	endif !if(j_v(j_o(iob)%i(io+2)).eq.j_0)   3672
	!	write(6,*)'io ',io
...
subroutine which(iob,io)  !which()
	...
	do i=1,narg-imod,2
		...
		end if !if(j_v(j_o(iob)%i(io+1+i)).ne.0.)   3784
	end do !i=1,narg-imod,2   3783
...
subroutine errexit(iob,io)
	...
	do j=1,narg
		...
		if(j_ipc(iv).ne.0)then
			...
			write(6,*)j_v(iv)
		end if !if(j_ipc(iv).ne.0)   3802
	end do !j=1,narg   3800
	j_err=.true.
	return
...
subroutine goto2(iob,io) !select fro all labels
	...
	if(igo.lt.0.or.igo.gt.narg)then
		...
		j_err=.true.
		return
	endif !if(igo.lt.0.or.igo.gt.narg)   3833
...
subroutine POWER(iob,io,ioper)
	...
	if(j_otype(irg).eq.j_ipreal)then
		...
		if(ipower)then
			...
			endif !if(j_v(irg).lt.0.d0)   3908
		endif !if(ipower)   3905
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		if(j_o(iout)%i(3).ne.j_o(irg)%i(3))then
			...
			j_o(iout)%i(1)=nrow
			j_o(iout)%i(2)=ncol
		endif !if(j_o(iout)%i(3).ne.j_o(irg)%i(3))   3922
		if(ipower)then
			...
			if(minva.lt.0.d0)then
				...
				j_err=.true.;return
			endif !if(minva.lt.0.d0)   3932
			j_o(iout)%d=	j_o(irg)%d**pow
		endif !if(ipower)   3928
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   3903
...
subroutine HMULT(iob,io)
	...
	if(j_otype(irg1).ne.j_ipmatrix.or.j_otype(irg2).ne.j_ipmatrix)then
		...
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix.or.j_otype(irg2).ne.j_ipmat   3952
	nrow=j_o(irg1)%i(1)
	ncol=j_o(irg1)%i(2)
	nel=j_o(irg1)%i(3)
	if(nrow.ne.j_o(irg2)%i(1).or.ncol.ne.j_o(irg2)%i(2))then
		...
		j_err=.true.;return
	endif !if(nrow.ne.j_o(irg2)%i(1).or.ncol.ne.j_o(irg2)%i(2))   3959
...
subroutine HDIV(iob,io)
	...
	if(j_otype(irg1).ne.j_ipmatrix.or.j_otype(irg2).ne.j_ipmatrix)then
		write(6,*)' *.  requires matrix arguments'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix.or.j_otype(irg2).ne.j_ipmat   3976
	nrow=j_o(irg1)%i(1)
	ncol=j_o(irg1)%i(2)
	nel=j_o(irg1)%i(3)
	if(nrow.ne.j_o(irg2)%i(1).or.ncol.ne.j_o(irg2)%i(2))then
		write(6,*)'arguments of *.  are not compatible'
		j_err=.true.;return
	endif !if(nrow.ne.j_o(irg2)%i(1).or.ncol.ne.j_o(irg2)%i(2))   3983
	if(irg1.ne.iout.and.irg2.ne.iout)call j_defmatrix(iout,' ',nrow,ncol,j_matreg,ivout)
	do i=1,nel
		...
		endif !if(j_o(irg2)%d(i).eq.j_0)   3989
		j_o(iout)%d(i)=j_o(irg1)%d(i)/j_o(irg2)%d(i)
	enddo !i=1,nel   3988
	return
...
subroutine MULT(iob,io) ! mata*matb
	...
	if(j_otype(irg(1)).eq.j_ipreal.and.j_otype(irg(2)).eq.j_ipreal)then
		...
		return
	endif !if(j_otype(irg(1)).eq.j_ipreal.and.j_otype(irg(2)).eq.j_ip   4016
	...
	if(.not.(ismatrix(1).and.ismatrix(2)))then
		...
		if(j_otype(iout).eq.j_iplist)then
			...
			do i=1,nel
				...
				return
			enddo !i=1,nel   4051
		endif !if(j_otype(iout).eq.j_iplist)   4048
		if(j_otype(iout).eq.j_ipmatrix)then
			if(j_o(iout)%i(1).eq.nrow(imatrix).and.j_o(iout)%i(2).eq.ncol(imatrix))then
				j_o(iout)%d=j_v(ireal)*j_o(irg(imatrix))%d(1:nel)
				return
			endif !if(j_o(iout)%i(1).eq.nrow(imatrix).and.j_o(iout)%i(2).eq.n   4059
 
		endif !if(j_otype(iout).eq.j_ipmatrix)   4058
		call j_defmatrix(iout,' ',nrow(imatrix),ncol(imatrix),j_matreg,ivout)
		j_o(iout)%d=j_v(ireal)*j_o(irg(imatrix))%d(1:nel)
		return
 
	endif !if(.not.(ismatrix(1).and.ismatrix(2)))   4046
	...
	if(nrow(1).eq.1.and.ncol(2).eq.1)then
		...
		return
	endif !if(nrow(1).eq.1.and.ncol(2).eq.1)   4073
	nel=nrow(1)*ncol(2)
	...
	endif !if(nrow(1).eq.ncol(1).and.iout.eq.irg(1).or.iout.eq.irg(2)   4081
	return
...
subroutine DIV(iob,io)
	...
	if(j_otype(irg2).ne.j_ipreal)then
		...
		j_err=.true.
		return
	endif !if(j_otype(irg2).ne.j_ipreal)   4114
	if(j_v(irg2).eq.j_0)then
		write(6,*)'*division by zero'
		j_err=.true.
		return
	endif !if(j_v(irg2).eq.j_0)   4119
...
subroutine BINOP(iob,io,ioper) ! mata+matb !
	...
	if(narg.eq.1)then
		if(j_otype(irg(1)).eq.j_ipmatrix)then
			...
			endif !if(ioper.ne.16.and.ioper.ne.17)   4170
			nco=j_o(irg(1))%i(2)
			nro=j_o(irg(1))%i(1)
			if(nco.eq.1.or.j_linkoption(iob,io,j_many,clear=.true.).ge.0)then
				...
				if(ioper.eq.16)then
					...
					do i=1,nro
						...
						ibas=ibas+nco
					enddo !i=1,nro   4203
				endif !if(ioper.eq.16)   4192
			endif !if(nco.eq.1.or.j_linkoption(iob,io,j_many,clear=.true.).ge   4179
 
 
		endif !if(j_otype(irg(1)).eq.j_ipmatrix)   4168
		return
	endif !if(narg.eq.1)   4167
	!	narg=j_o(iob)%i(io+1)
	...
	if(j_otype(irg(1)).eq.j_ipreal.and.j_otype(irg(2)).eq.j_ipreal)then
		...
		if(j_v(irg(2)).eq.j_0)then
			...
			j_v(iout)=j_v(irg(1))/j_v(irg(2))
		endif !if(j_v(irg(2)).eq.j_0)   4273
		...
		return
	endif !if(j_otype(irg(1)).eq.j_ipreal.and.j_otype(irg(2)).eq.j_ip   4223
	...
	do i=1,2
		...
		if(j_o(irg(i))%i(2).eq.1)then
			...
			colstep(i)=1
		endif !if(j_o(irg(i))%i(2).eq.1)   4333
		if(j_o(irg(i))%i(1).eq.1.or.j_o(irg(i))%i(2).eq.1)then
			...
			matrix(i)=.true.
			imatrix=i
		endif !if(j_o(irg(i))%i(1).eq.1.or.j_o(irg(i))%i(2).eq.1)   4338
	...
	endif !if(vector(1).and.vector(2))   4363
	...
	if(j_otype(iout).eq.j_iplist)then
		...
		listout=.false.
	endif !if(j_otype(iout).eq.j_iplist)   4371
	...
	if(matrix(1).and.matrix(2).or.vector(1).and.vector(2))then
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).eq.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).eq.j_o(irg(2))%d(iel))   4392
		enddo !iel=1,nel   4391
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ne.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ne.j_o(irg(2))%d(iel))   4401
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).le.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).le.j_o(irg(2))%d(iel))   4410
		enddo !iel=1,nel   4409
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).lt.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).lt.j_o(irg(2))%d(iel))   4419
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ge.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ge.j_o(irg(2))%d(iel))   4428
		enddo !iel=1,nel   4427
		...
		do iel=1,nel
			...
			if(j_o(irg(1))%d(iel).gt.j_o(irg(2))%d(iel))then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).gt.j_o(irg(2))%d(iel))   4438
		enddo !iel=1,nel   4436
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0)then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0   4447
		enddo !iel=1,nel   4446
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ne.j_0.or.j_o(irg(2))%d(iel).ne.j_0)then
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ne.j_0.or.j_o(irg(2))%d(iel).ne.j_0)   4457
		...
		do iel=1,nel
			if(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0.or.j_o(irg(1))%d(iel).eq.j_0.and.&
				...
				j_o(iout)%d(iel)=j_0
			endif !if(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0   4468
		enddo !iel=1,nel   4467
		...
		do iel=1,nel
			if(.not.(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).ne.j_0.or.j_o(irg(1))%d(iel).eq.j_0.and.&
				...
				j_o(iout)%d(iel)=j_0
			endif !if(.not.(j_o(irg(1))%d(iel).ne.j_0.and.j_o(irg(2))%d(iel).   4479
	...
	endif !if(matrix(1).and.matrix(2).or.vector(1).and.vector(2))   4384
	...
	endif !if(ivector.gt.0.and.imatrix.gt.0)   4531
	...
	do i=1,nrowout
		do j=1,ncolout
			...
			if(j_o(irg(2))%d(iel2).eq.j_0)then
				...
				diout=j_o(irg(1))%d(iel)/j_o(irg(2))%d(iel2)
			endif !if(j_o(irg(2))%d(iel2).eq.j_0)   4586
			case (16) !select case (ioper)
...
subroutine MINUS(iob,io) ! mata+matb !
	...
	if(j_otype(irg(1)).eq.j_ipreal)then
		...
		j_v(iout)=-j_v(irg(1))
		return
	endif !if(j_otype(irg(1)).eq.j_ipreal)   4649
	...
	if(j_otype(iout).eq.j_iplist)then
		...
		j_err=.true.
		return
	endif !if(j_otype(iout).eq.j_iplist)   4667
		...
		if(j_otype(irg1).eq.j_ipmatrix)then
			...
			do j=2,j_o(irg1)%i(3)
				if(j_o(irg1)%d(j).ne.val1)then
					...
					j_v(iout)=j_0
					return
				endif !if(j_o(irg1)%d(j).ne.val1)   4703
			enddo !j=2,j_o(irg1)%i(3)   4702
			j_v(iout)=j_1
			return
		...
		elseif(j_otype(irg1).eq.j_iplist)then !if(j_otype(irg1).eq.j_ipmatrix)then
			...
			do j=2,j_o(irg1)%i(1)
				if(j_v( j_o(irg1)%i2(j)).ne.val1)then
					j_v(iout)=j_0
					return
				endif !if(j_v( j_o(irg1)%i2(j)).ne.val1)   4715
			enddo !j=2,j_o(irg1)%i(1)   4714
			j_v(iout)=j_1
			return
			...
			j_err=.true.
			return
		endif !if(j_otype(irg1).eq.j_ipmatrix)   4700
	endif !if(narg.eq.1)   4699
	irg2=j_o(iob)%i(io+3)
	if(j_v(irg1).eq.j_v(irg2))then
		j_v(iout)=j_1
	else !if(j_v(irg1).eq.j_v(irg2))then
		j_v(iout)=j_0
	endif !if(j_v(irg1).eq.j_v(irg2))   4729
...
subroutine NE(iob,io)
	...
	if(j_v(irg1).ne.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).ne.j_v(irg2))   4744
...
subroutine LE(iob,io)
	...
	if(j_v(irg1).le.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).le.j_v(irg2))   4760
...
subroutine LT(iob,io)
	...
	if(j_v(irg1).lt.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).lt.j_v(irg2))   4774
...
subroutine GE(iob,io)
	...
	if(j_v(irg1).ge.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).ge.j_v(irg2))   4787
...
subroutine GT(iob,io)
	...
	if(j_v(irg1).gt.j_v(irg2))then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).gt.j_v(irg2))   4800
...
subroutine NOT(iob,io)
	...
	if(j_v(irg1).eq.0.d0)then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).eq.0.d0)   4813
...
subroutine AND(iob,io)
	...
	if(j_v(irg1).ne.0.d0.and.j_v(irg2).ne.0)then
		...
		j_v(iout)=j_0
	endif !if(j_v(irg1).ne.0.d0.and.j_v(irg2).ne.0)   4826
...
subroutine OR(iob,io)
	...
	if(j_v(irg1).ne.0.d0.or.j_v(irg2).ne.0.d0)then
		...
		j_v(iout)=0.d0
	endif !if(j_v(irg1).ne.0.d0.or.j_v(irg2).ne.0.d0)   4839
...
subroutine EQV(iob,io)
	...
	if((j_v(irg1).ne.0.d0.and.j_v(irg2).ne.0.d0).or.(j_v(irg1).eq.0.d0.and.j_v(irg2).eq.0.d0))then
		...
		j_v(iout)=0.d0
	endif !if((j_v(irg1).ne.0.d0.and.j_v(irg2).ne.0.d0).or.(j_v(irg1)   4852
...
subroutine NEQV(iob,io)
	...
	if((j_v(irg1).ne.0.d0.and.j_v(irg2).eq.0.d0).or.(j_v(irg1).eq.0.d0.and.j_v(irg2).ne.0.d0))then
		...
		j_v(iout)=0.d0
	endif !if((j_v(irg1).ne.0.d0.and.j_v(irg2).eq.0.d0).or.(j_v(irg1)   4865
...
subroutine min_(iob,io)
	...
	if(narg.eq.2)then
		if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal)then
			...
			return
		endif !if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal   4878
		if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipmatrix)then
			if(j_o(irg1)%i(1).ne.j_o(irg2)%i(1).or.j_o(irg1)%i(2).ne.j_o(irg2)%i(2))then
				...
				j_err=.true.;return
			endif !if(j_o(irg1)%i(1).ne.j_o(irg2)%i(1).or.j_o(irg1)%i(2).ne.j   4884
			...
			enddo !ii=1,j_o(irg2)%i(3)   4889
			return
		endif !if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipma   4883
		if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipreal)then
			ir=irg1
			irg1=irg2
			irg2=ir
		endif !if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipre   4894
		!now irg2 is matrix
		...
		if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipmatrix)then
			...
			if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)then
				...
				endif !if(j_o(irg2)%i(3).eq.j_o(iout)%i(3))   4904
			...
			endif !if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)   4903
			do ii=1,j_o(irg2)%i(3)
				j_o(iout)%d(ii)=dmin1(j_v(irg1),j_o(irg2)%d(ii))
			enddo !ii=1,j_o(irg2)%i(3)   4913
			return
 
		endif !if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipmatr   4901
 
		write(6,*)'illegal argument types ',j_objecttypes(j_otype(irg1)),j_objecttypes(j_otype(irg1))
		j_err=.true.;return
	endif !if(narg.eq.2)   4877
 
 
	if(j_otype(irg1).ne.j_ipmatrix)then
		write(6,*)'one argument must be MATRIX'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix)   4925
	nrows=j_o(irg1)%i(1)
	ncols=j_o(irg1)%i(2)
	if(j_linkoption(iob,io,j_many).ge.0)then
		ncols=1
		call j_clearoption(iob,io)
	endif !if(j_linkoption(iob,io,j_many).ge.0)   4931
	if(nrows.eq.1.or.ncols.eq.1)then
		...
		do i=1,nrows
			...
			enddo !j=1,ncols   4945
			ibas=ibas+ncols
		enddo !i=1,nrows   4944
 
	endif !if(nrows.eq.1.or.ncols.eq.1)   4935
	return
...
subroutine max_(iob,io)
	...
	if(narg.eq.2)then
		if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal)then
			...
			return
		endif !if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipreal   4963
		if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipmatrix)then
			if(j_o(irg1)%i(1).ne.j_o(irg2)%i(1).or.j_o(irg1)%i(2).ne.j_o(irg2)%i(2))then
				write(6,*)'incompatibe dimensions ',j_o(irg1)%i(1:2),j_o(irg2)%i(1:2)
				j_err=.true.;return
			endif !if(j_o(irg1)%i(1).ne.j_o(irg2)%i(1).or.j_o(irg1)%i(2).ne.j   4969
			if(irg1.ne.iout.and.irg2.ne.iout)call j_defmatrix(iout,' ',j_o(irg2)%i(1),j_o(irg2)%i(2),j_matreg,ivout)
			do ii=1,j_o(irg2)%i(3)
				j_o(iout)%d(ii)=dmax1(j_o(irg1)%d(ii),j_o(irg2)%d(ii))
			enddo !ii=1,j_o(irg2)%i(3)   4974
			return
		endif !if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipma   4968
		if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipreal)then
			ir=irg1
			irg1=irg2
			irg2=ir
		endif !if(j_otype(irg1).eq.j_ipmatrix.and.j_otype(irg2).eq.j_ipre   4979
		!now irg2 is matrix
		!	write(6,*)'<66',irg1,irg2,j_otype(irg1),j_otype(irg2),j_ipmatrix
		if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipmatrix)then
			...
			if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)then
				if(j_o(irg2)%i(3).eq.j_o(iout)%i(3))then
					...
					call j_defmatrix(iout,' ',j_o(irg2)%i(1),j_o(irg2)%i(2),j_matreg,ivout)
				endif !if(j_o(irg2)%i(3).eq.j_o(iout)%i(3))   4989
			else !if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)then
				call j_defmatrix(iout,' ',j_o(irg2)%i(1),j_o(irg2)%i(2),j_matreg,ivout) !could do with one def
			endif !if(iout.ne.irg2.and.j_otype(iout).eq.j_ipmatrix)   4988
			do ii=1,j_o(irg2)%i(3)
				j_o(iout)%d(ii)=dmax1(j_v(irg1),j_o(irg2)%d(ii))
			enddo !ii=1,j_o(irg2)%i(3)   4998
			return
 
		endif !if(j_otype(irg1).eq.j_ipreal.and.j_otype(irg2).eq.j_ipmatr   4986
 
		write(6,*)'illegal argument types ',j_objecttypes(j_otype(irg1)),j_objecttypes(j_otype(irg1))
		j_err=.true.;return
	endif !if(narg.eq.2)   4962
 
	if(j_otype(irg1).ne.j_ipmatrix)then
		write(6,*)'one arguement must be MATRIX'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix)   5009
	nrows=j_o(irg1)%i(1)
	ncols=j_o(irg1)%i(2)
	if(j_linkoption(iob,io,j_many).ge.0)then
		ncols=1
		call j_clearoption(iob,io)
	endif !if(j_linkoption(iob,io,j_many).ge.0)   5015
	if(nrows.eq.1.or.ncols.eq.1)then
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
		...
		do i=1,nrows
			...
			enddo !j=1,ncols   5028
			ibas=ibas+ncols
		enddo !i=1,nrows   5027
 
	endif !if(nrows.eq.1.or.ncols.eq.1)   5019
	return
...
subroutine minloc_(iob,io)
	...
	if(j_otype(irg1).ne.j_ipmatrix)then
		...
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix)   5056
	nrows=j_o(irg1)%i(1)
	ncols=j_o(irg1)%i(2)
	if(j_linkoption(iob,io,j_many,clear=.true.).ge.0)then
		ncols=1
		!	call j_clearoption(iob,io)
	endif !if(j_linkoption(iob,io,j_many,clear=.true.).ge.0)   5062
	if(nrows.eq.1.or.ncols.eq.1)then
		...
		do i=1,ncols
			...
			do j=1,nrows
				...
				if(j_o(irg1)%d(ibas+i).lt.minva)then
					...
					loc=j
				endif !if(j_o(irg1)%d(ibas+i).lt.minva)   5080
				ibas=ibas+ncols
			enddo !j=1,nrows   5078
			j_o(iout)%d(i)=loc
		enddo !i=1,ncols   5076
 
	endif !if(nrows.eq.1.or.ncols.eq.1)   5066
	return
...
subroutine maxloc_(iob,io)
	...
	if(j_otype(irg1).ne.j_ipmatrix)then
		write(6,*)'argument must be MATRIX'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix)   5113
	nrows=j_o(irg1)%i(1)
	ncols=j_o(irg1)%i(2)
	if(j_linkoption(iob,io,j_many).ge.0)then
		ncols=1
		call j_clearoption(iob,io)
	endif !if(j_linkoption(iob,io,j_many).ge.0)   5119
	if(nrows.eq.1.or.ncols.eq.1)then
		...
		do i=1,ncols
			...
			do j=1,nrows
				if(j_o(irg1)%d(ibas+i).gt.maxva)then
					...
					loc=j
				endif !if(j_o(irg1)%d(ibas+i).gt.maxva)   5137
				ibas=ibas+ncols
			enddo !j=1,nrows   5136
			j_o(iout)%d(i)=loc
		enddo !i=1,ncols   5134
 
	endif !if(nrows.eq.1.or.ncols.eq.1)   5123
	return
...
subroutine cumsum(iob,io)
	...
	if(j_otype(irg1).ne.j_ipmatrix)then
		write(6,*)'argument must be MATRIX'
		j_err=.true.;return
	endif !if(j_otype(irg1).ne.j_ipmatrix)   5174
	nrows=j_o(irg1)%i(1)
	ncols=j_o(irg1)%i(2)
	nel=j_o(irg1)%i(3)
	...
	if(nrows.eq.1.or.ncols.eq.1)then
		...
		enddo !i=2,nrows   5197
	endif !if(nrows.eq.1.or.ncols.eq.1)   5185
	return
...
subroutine corrmatrix(iob,io)
	...
	do i=1,ndim
		...
		endif !if(j_o(irg1)%d(ibas+i).lt.j_0)   5240
	...
	do i=1,ndim
		...
		do j=1,ndim
			...
			if(i.ne.j)then
				...
				endif !if(j_dapu.ne.j_0)   5257
 
			endif !if(i.ne.j)   5255
	...
	if(j_linkoption(iob,io,j_msd,clear=.true.).lt.0)then
		...
		enddo !i=1,ndim   5273
	endif !if(j_linkoption(iob,io,j_msd,clear=.true.).lt.0)   5271
		...
		if(iout.ne.irg)then
			if(j_otype(iout).eq.j_ipmatrix)then
				if(j_o(iout)%i(3).eq.j_o(irg)%i(3))then
					...
					goto 80
				endif !if(j_o(iout)%i(3).eq.j_o(irg)%i(3))   5300
			endif !if(j_otype(iout).eq.j_ipmatrix)   5299
			if(j_otype(iout).ne.j_ipreal)call j_del(iout)
			call j_defmatrix(iout,' ',j_o(irg)%i(1),j_o(irg)%i(2),j_matreg,ivout) !matreg?
		endif !if(iout.ne.irg)   5298
80		continue
		j_o(iout)%d=dabs(j_o(irg)%d)
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5293
...
subroutine nint_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5335
...
subroutine int_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5356
...
subroutine ceiling_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5377
...
subroutine floor_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5398
...
6 	if(j_v(irg).lt.j_0)then
		...
		endif !6 	if(j_v(irg).lt.j_0)   5443
		return
...
7 	if(j_v(irg).eq.j_0)then
		...
		elseif(j_v(irg).gt.j_0)Then !if(j_v(irg).eq.j_0)then
			...
			j_v(iout)=-sqrt(-j_v(irg))
		endif !7 	if(j_v(irg).eq.j_0)   5452
		return
		!
8 	if(j_v(irg).lt.0.d0)then
		...
		endif !8 	if(j_v(irg).lt.0.d0)   5461
		return
		!abs
9 	if(j_v(irg).lt.0.d0)then
		...
		endif !9 	if(j_v(irg).lt.0.d0)   5469
		return
10	if(j_v(irg).gt.88.d0)then
		...
		endif !10	if(j_v(irg).gt.88.d0)   5476
		return
		!abs
...
17		if(dabs(j_v(irg)).lt.d-15)then
			...
			j_err=.true.
		endif !17		if(dabs(j_v(irg)).lt.d-15)   5503
...
18	if(dabs(j_v(irg)).lt.d-15)then
			write(6,*)'too close to zero ',j_v(irg)
			j_err=.true.
		endif !18	if(dabs(j_v(irg)).lt.d-15)   5510
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
...
25	if(abs(j_v(irg)).gt.0.d-15)then  !cotan
			...
			j_err=.true.
		endif !25	if(abs(j_v(irg)).gt.0.d-15)   5532
...
26		if(abs(j_v(irg)).gt.0.d-15)then
			...
			write(6,*)'too small argument ',j_v(irg)
			j_err=.true.
		endif !26		if(abs(j_v(irg)).gt.0.d-15)   5541
		return
...
31	j_v(iout)=dabs(j_v(irg))
		...
		return
	endif !if(j_otype(irg).eq.j_ipreal)   5421
	if(j_otype(irg).ne.j_ipmatrix)then
		write(6,*)'argument is not MATRIX or REAL'
		j_err=.true.
 
	endif !if(j_otype(irg).ne.j_ipmatrix)   5568
	...
	endif !if(j_o(iout)%i(3).ne.j_o(irg)%i(3))   5574
...
97 		minva=minval(j_o(irg)%d)
	do i=1,j_o(irg)%i(3)
		...
		elseif(j_o(irg)%d(i).gt.j_0)Then !if(j_o(irg)%d(i).eq.j_0)then
			...
			j_o(iout)%d(i)=-sqrt(-j_o(irg)%d(i))
		endif !if(j_o(irg)%d(i).eq.j_0)   5610
	end do !i=1,j_o(irg)%i(3)   5609
	return
	!
...
subroutine log_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		if(minva.lt.0.d0)then
			...
			allocate( j_o(iout)%d(1:j_o(iout)%i(3)))
			j_o(iout)%d=dlog(j_o(irg)%d)
		endif !if(minva.lt.0.d0)   5821
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5811
...
subroutine log10_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		if(minva.lt.0.d0)then
			...
			allocate( j_o(iout)%d(1:j_o(iout)%i(3)))
			j_o(iout)%d=dlog10(j_o(irg)%d)
		endif !if(minva.lt.0.d0)   5855
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5845
...
subroutine exp_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		if(j_v(irg).gt.88.d0)then
			...
			if(j_otype(iout).ne.j_ipreal)call j_del(iout)
			j_v(iout)=dexp(j_v(irg))
		endif !if(j_v(irg).gt.88.d0)   5880
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		if(minva.lt.0.d0)then
			...
			allocate( j_o(iout)%d(1:j_o(iout)%i(3)))
			j_o(iout)%d=dexp(j_o(irg)%d)
		endif !if(minva.lt.0.d0)   5889
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5879
...
subroutine sin_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5913
...
subroutine sind_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5935
...
subroutine cosd_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   5979
...
subroutine cotan_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		if(dabs(j_v(irg)).lt.d-15)then
			write(6,*)'too close to zero ',j_v(irg)
			j_err=.true.
		endif !if(dabs(j_v(irg)).lt.d-15)   6046
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
		j_v(iout)=dtan(1.d0/j_v(irg))
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		call j_del(iout)
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6045
...
subroutine cotand_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		if(dabs(j_v(irg)).lt.d-15)then
			write(6,*)'too close to zero ',j_v(irg)
			j_err=.true.
		endif !if(dabs(j_v(irg)).lt.d-15)   6072
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
		j_v(iout)=dtan(1.d0/(j_deg*j_v(irg)))
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		call j_del(iout)
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6071
...
subroutine asin_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6097
...
subroutine asind_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6119
...
subroutine acos_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6141
...
subroutine acosd_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6163
...
subroutine acotan_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		if(abs(j_v(irg)).gt.0.d-15)then
			...
			write(6,*)'too small argument ',j_v(irg)
			j_err=.true.
		endif !if(abs(j_v(irg)).gt.0.d-15)   6231
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		if(minva.lt.1.d-15)then
			...
			j_err=.true.
			return
		endif !if(minva.lt.1.d-15)   6248
		j_o(iout)%d=datan(1.d0/j_o(irg)%d)
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6230
...
subroutine acotand_(iob,io)
	...
	if(j_otype(irg).eq.j_ipreal)then
		...
		if(abs(j_v(irg)).gt.0.d-15)then
			...
			write(6,*)'too small argument ',j_v(irg)
			j_err=.true.
		endif !if(abs(j_v(irg)).gt.0.d-15)   6268
		j_v(iout)=dacos(j_v(irg))*j_todeg
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		call j_del(iout)
		j_otype(iout)=j_ipmatrix
		...
		if(minva.lt.1.d-15)then
			...
			j_err=.true.
			return
		endif !if(minva.lt.1.d-15)   6285
		j_o(iout)%d=datan(1.d0/j_o(irg)%d)*j_todeg
	else !if(j_otype(irg).eq.j_ipreal)then
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)   6266
...
! call j_getoption(iob,io,j_msum,-1,0,0,.false.,isum,j_optarg0)
...
subroutine index_v(iob,io)
	...
	if(j_otype(arg(2)).ne.j_iplist)then
		...
		j_err=.true.
		return
	endif !if(j_otype(arg(2)).ne.j_iplist)   6508
	inde=j_v(arg(1))
	...
	endif !if(inde.lt.1.or.inde.gt.j_o(arg(2))%i(1))   6514
	return
...
subroutine find(iob,io)
	...
	if(noprows.ge.0.or.nopcols.ge.0)then
		...
		j_err=.true.
		return
	endif !if(noprows.ge.0.or.nopcols.ge.0)   6588
	!write(6,*)'infind'
...
subroutine matrixstat(iob,io,ifunc)
	...
	endif !if(j_linkoption(iob,io,j_many).ge.0)   6723
		...
		if(ncols.eq.1.or.nrows.eq.1)then
			if(j_o(iwe)%i(3).ne.nel)then
				...
				write(6,*)'weight ',j_oname2(1:j_loname2), 'does not have same size as ',&
					j_oname(1:j_loname)
				j_err=.true. ; return
			endif !if(j_o(iwe)%i(3).ne.nel)   6737
			if(j_otype(ivout).ne.j_ipreal)call j_del(ivout)
			!dcoef=j_1/nel
			!dcoef1=j_1/(nel-j_1)
			...
			endif !if(ifunc.eq.1)   6761
			return
 
		endif !if(ncols.eq.1.or.nrows.eq.1)   6736
		if(j_o(iwe)%i(1).ne.1.and.j_o(iwe)%i(2).ne.1)then
			call j_getname(iwe)
			write(6,*)'weight ',j_oname(1:j_loname),' is not vector but ',j_o(iwe)%i(1:2)
			j_err=.true.
			return
		endif !if(j_o(iwe)%i(1).ne.1.and.j_o(iwe)%i(2).ne.1)   6770
	...
	if(ncols.eq.1.or.nrows.eq.1)then
		...
		endif !if(ifunc.eq.1)   6828
		return
 
	endif !if(ncols.eq.1.or.nrows.eq.1)   6807
...
subroutine class(iob,io)
	...
	if(xmax.le.xmin)then
		...
		j_err=.true.
		return
	endif !if(xmax.le.xmin)   6901
	if(j_v(arg(1)).lt.xmin.or.j_v(arg(1)).gt.xmax)then
		j_v(ivout)=0.d0
		return
	endif !if(j_v(arg(1)).lt.xmin.or.j_v(arg(1)).gt.xmax)   6906
	...
	if(ndx.eq.1)then
		...
		if(dx.le.0.d0)then
			...
			j_err=.true.
			return
		endif !if(dx.le.0.d0)   6915
	endif !if(ndx.eq.1)   6913
	...
	if(nc.gt.0.and.ndx.gt.0)then
		...
		nc=0
	endif !if(nc.gt.0.and.ndx.gt.0)   6925
	call j_clearoption(iob,io) ! subroutine
...
subroutine newdata(iob,io)
	...
	double precision,dimension(:),allocatable :: temp
	logical maketran,isappend,ismatrix,isdata,istime,istrans
	...
	if(isappend)then
		...
		do i=1,narg
			if(j_otype(arg(i)).eq.j_ipmatrix)then
				...
				if(ncols.ne.j_o(ivmat)%i(2))then
					...
					j_err=.true.;return
				endif !if(ncols.ne.j_o(ivmat)%i(2))   7057
			elseif(j_otype(arg(i)).eq.j_ipdata)then !if(j_otype(arg(i)).eq.j_ipmatrix)then
				...
				if(i.eq.1)then
					...
					if(j_otype(arg(1)).ne.j_ipdata)then
						...
						j_err=.true.;return
					endif !if(j_otype(arg(1)).ne.j_ipdata)   7070
					if(j_o(ivmat)%i(2).ne.ncols)then
						...
						j_err=.true.;return
					endif !if(j_o(ivmat)%i(2).ne.ncols)   7077
					...
					if(.not.all(j_o(ivkeep1)%i2(1:ncols).eq.j_o(ivkeep)%i2(1:ncols)))then
						...
						j_err=.true.;return
					endif !if(.not.all(j_o(ivkeep1)%i2(1:ncols).eq.j_o(ivkeep)%i2(1:n   7083
					arg2(i)=ivmat
				endif !if(i.eq.1)   7066
				...
				j_err=.true.
				return
			endif !if(j_otype(arg(i)).eq.j_ipmatrix)   7042
		...
		if(j_reject.or.j_filter)then
			write(6,*)'*fileter-> and reject-> do not work with append->'
			j_err=.true.;return
 
		endif !if(j_reject.or.j_filter)   7117
		...
		if(nextra.gt.0)then
			do ij=1,nextra
				...
				endif !if(iper.gt.0)   7158
			enddo !ij=1,nextra   7156
		endif !if(nextra.gt.0)   7155
		if(j_o(ivkeep)%i(1).lt.nvartot)then
			...
			j_err=.true.; return
		endif !if(j_o(ivkeep)%i(1).lt.nvartot)   7165
		...
		do i=1,narg
			...
			do k=1,j_o(arg2(i))%i(1)
				...
				endif !if(maketran)   7184
 
				ibas2=ibas2+ncols
				ibas=ibas+nkeep
			enddo !k=1,j_o(arg2(i))%i(1)   7183
		...
		do i=1,narg
			...
			endif !if(j_otype(arg(i)).eq.j_ipmatrix)   7224
			ncols=j_o(arg2(i))%i(2)
			if(i.eq.1)then
				nob=j_o(arg2(i))%i(1)
				!write(6,*)'nob ',nob
		...
		do i=1,narg
			...
			if(arg(i).eq.arg2(i))then
				...
				iper=j_putlistobject(ivkeep,list0=nvar0(i),list=j_optarg(nr+1:nr+nvar0(i)))
		...
		if(j_o(ivkeep)%i(1).lt.nvar)then
			...
			write(6,*)'There are duplicated variable names'
			j_err=.true.; return
		endif !if(j_o(ivkeep)%i(1).lt.nvar)   7307
		...
		if(nextra.gt.0)then
			do ij=1,nextra
				...
				if(iper.gt.0)then
					...
					iper=j_putlistobject(ivkeep,single=extra(ij))
				endif !if(iper.gt.0)   7320
			enddo !ij=1,nextra   7318
		endif !if(nextra.gt.0)   7317
		...
		do j=1,nob
			...
			if(j_rejected)then
				...
				j_o(ivmat)%d(ibas+1:ibas+nkeep)=j_v(j_o(ivkeep)%i2(1:nkeep))
				ibas=ibas+nkeep
			endif !if(j_rejected)   7387
 
		enddo !j=1,nob   7348
 
		!write(6,*)'pooo'
 
	endif !if(isappend)   7039
	!write(6,*)'fkfk'
	call j_getname(iout)
	...
	do ifi=1,nin_
		...
		if(ifi.eq.1)then
			...
			if(j_inpr(1:lens).ne.j_cline(1:lens))then
				...
				goto 900
			endif !if(j_inpr(1:lens).ne.j_cline(1:lens))   7534
		endif !if(ifi.eq.1)   7528
		...
		if(ifi.eq.1)then
			...
			do ic1=3,lin
				...
				if(j_inpr(ic:ic).eq.';'.or.ic1.eq.lin)then
					...
					if(ic.gt.ic0+1.and.j_inpr(ic0+1:ic0+1).ne.'!')then
						...
						endif !if(j_inpr(ic-1:ic-1).eq.'?')   7572
						...
						keepv(nkeep)=iread
					endif !if(ic.gt.ic0+1.and.j_inpr(ic0+1:ic0+1).ne.'!')   7570
					ic0=ic
				endif !if(j_inpr(ic:ic).eq.';'.or.ic1.eq.lin)   7568
			enddo !ic1=3,lin   7565
			nkeep=nkeep+1
			...
			if(j_inpr(1:2).eq.'@#'.or.j_inpr(1:6).eq.'jcode:')then
				...
				nobs(ifi)=nobs(ifi)+1
			endif !if(j_inpr(1:2).eq.'@#'.or.j_inpr(1:6).eq.'jcode:')   7617
		endif !if(ifi.eq.1)   7542
		...
		do i=1,10000000
			...
			if(j_tempchar3(1:lens).eq.j_cline(1:lens).or.j_tempchar3(1:6).eq.'jcode:')then
				...
				nobs(ifi)=nobs(ifi)+1
			endif !if(j_tempchar3(1:lens).eq.j_cline(1:lens).or.j_tempchar3(1   7627
		enddo !i=1,10000000   7625
...
66			if(j_inpr(1:2).eq.'@#')cycle
			if(j_inpr(1:6).eq.'jcode:')then
				...
				if(j_stop)then
					...
					call j_stopj()
					return
				endif !if(j_stop)   7659
				if(j_err)then
					call j_closeunit(nu(ifi))
					goto 99
				endif !if(j_err)   7664
				cycle
			endif !if(j_inpr(1:6).eq.'jcode:')   7653
				...
				if(j_inpr(ic:ic).eq.';'.or.ic.gt.lin)then
					...
					if(isread(npp0))then
						...
						if(ic.gt.ic0+1)then
							...
							endif !if(ispe(npp0))   7686
 
						else !if(ic.gt.ic0+1)then
							j_v(readv(nrea))=0.d0
						endif !if(ic.gt.ic0+1)   7685
						...
						! endif
					endif !if(isread(npp0))   7683
 
					ic0=ic
				endif !if(j_inpr(ic:ic).eq.';'.or.ic.gt.lin)   7679
 
			enddo !ic1=2,lin   7676
			...
			if(maketran)then
				...
				call dotrans(ivmaketrans,1)
				if(j_err)return
			endif !if(maketran)   7716
			iobs=iobs+1
			j_o(ivmat)%d((iobs-1)*nkeep+1:iobs*nkeep)=j_v(j_o(ivkeep)%i2(1:nkeep))
 
		enddo !i=2,1000000   7646
	enddo !ifi=1,nin_   7643
900 	continue
...
subroutine classvector(iob,io)
	...
	call j_getoption(iob,io,j_mfirst,-1,0,j_ipreal,.false.,iobsw,j_optarg0)
	...
	if(narg.le.0.and.ilast.lt.0.and.ifirst.lt.0)then
		...
		j_err=.true.
		return
	endif !if(narg.le.0.and.ilast.lt.0.and.ifirst.lt.0)   7929
	if(iw.ge.0.and.nexpand.lt.0)then
		write(6,*)'option ->w requires expand->'
		j_err=.true.;return
 
	endif !if(iw.ge.0.and.nexpand.lt.0)   7934
	...
	nlo=max(narg,1)
		...
		if(narg.gt.0)then
			...
			endif !if(iw.ge.0)   7956
		endif !if(narg.gt.0)   7946
		classv=-1.79d37
		ifi=1
...
!	write(6,*)'i',i
			...
			if((j_v(ivclass).ne.classv.and.i.gt.1).or.i.eq.j_dnobs+1)then
				...
				cmin=1.d60
				cmax=-1.d60
			endif !if((j_v(ivclass).ne.classv.and.i.gt.1).or.i.eq.j_dnobs+1)   7984
			classv=j_v(ivclass)
			...
			endif !if(narg.gt.0)   8022
		enddo ob !	do i=j_dfrom,j_duntil+1   7970
		...
		if(ifirst.ge.0.and.ia.eq.1)then
			if(nexpand.lt.0)then
				...
				do i=2,nclass
					do j=ifi,last0(i)
						call j_putmatrix(ive,i,1,last0(i-1)+1.d0)
					enddo !j=ifi,last0(i)   8051
					ifi=last0(i)+1
				enddo !i=2,nclass   8050
				call j_printname('   done ',ive,' ')
			endif !if(nexpand.lt.0)   8035
		endif !if(ifirst.ge.0.and.ia.eq.1)   8034
		...
		if(ilast.ge.0.and.ia.eq.1)then
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					do j=ifi,last0(i)
						call j_putmatrix(ive,i,1,dble(last0(i)) )
					enddo !j=ifi,last0(i)   8074
					ifi=last0(i)+1
				enddo !i=1,nclass   8073
 
				call j_printname('   done ',ive,' ')
			endif !if(nexpand.lt.0)   8063
		endif !if(ilast.ge.0.and.ia.eq.1)   8062
		...
		if(iobsw.ge.0.and.ia.eq.1)then
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   8093
					ifi=last0(i)+1
				enddo !i=1,nclass   8092
 
				call j_printname('   done ',ive,' ')
			endif !if(nexpand.lt.0)   8085
 
 
		endif !if(iobsw.ge.0.and.ia.eq.1)   8084
		...
		if(imean0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   8125
					ifi=last0(i)+1
				enddo !i=1,nclass   8124
			endif !if(nexpand.lt.0)   8107
			call j_printname('   done ',ive,' ')
		endif !if(imean0.ge.0)   8105
		...
		if(iw.ge.0)then
			...
			do i=1,nclass
				...
				enddo !j=ifi,last0(i)   8140
				ifi=last0(i)+1
			enddo !i=1,nclass   8139
			call j_printname('   done ',ivew,' ')
		endif !if(iw.ge.0)   8136
		...
		if(ivar0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   8173
					ifi=last0(i)+1
				enddo !i=1,nclass   8172
			endif !if(nexpand.lt.0)   8155
			call j_printname('   done ',ive,' ')
		endif !if(ivar0.ge.0)   8153
		...
		if(isd0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   8203
					ifi=last0(i)+1
				enddo !i=1,nclass   8202
			endif !if(nexpand.lt.0)   8185
			call j_printname('   done ',ive,' ')
		endif !if(isd0.ge.0)   8183
		...
		if(isum0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   8233
					ifi=last0(i)+1
				enddo !i=1,nclass   8232
			endif !if(nexpand.lt.0)   8215
			call j_printname('   done ',ive,' ')
		endif !if(isum0.ge.0)   8213
		...
		if(imin0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   8263
					ifi=last0(i)+1
				enddo !i=1,nclass   8262
			endif !if(nexpand.lt.0)   8245
			call j_printname('   done ',ive,' ')
		endif !if(imin0.ge.0)   8243
		...
		if(imax0.ge.0)then
			...
			if(nexpand.lt.0)then
				...
				do i=1,nclass
					...
					enddo !j=ifi,last0(i)   8293
					ifi=last0(i)+1
				enddo !i=1,nclass   8292
			endif !if(nexpand.lt.0)   8275
			call j_printname('   done ',ive,' ')
		endif !if(imax0.ge.0)   8273
 
 
 
	enddo vloop !op: do ia=1,nlo   7944
	!	write(6,*)'tas'
...
subroutine varcomp(iob,io)
	...
	if(ivout.eq.j_ivresult.and.iscov)then
		...
		j_err=.true.
	endif !if(ivout.eq.j_ivresult.and.iscov)   8347
	call j_getdataobject(iob,io)
	!call j_getdatasets(iob)
	...
	!call j_getdataset(j_datasets(k),j_dnobs) !initilize dtaset k, nobs will be the number of observations
		...
		do j=1,narg
			...
			endif !if(abs(j_v(arg(j))).ge.1.7d19)   8378
		enddo !j=1,narg   8377
		ntot=ntot+1
		...
		do j=1,narg
			if(iscov)then
				do j2=1,j-1
					...
					jc=jc+1
				end do !j2=1,j-1   8390
			end if !if(iscov)   8389
			s(jc)=j_v(arg(j))
			jc=jc+1
		end do !j=1,narg   8388
 
		call j_dbw(mets,s,lkm,iobs,nmets,xs,xss,sb,sumwt,xmi,xma,xs2,s2)
 
	enddo obloop !oop:	do i=j_dfrom,j_duntil   8368
	...
	do j=1,narg
		if(iscov)then
			do j2=1,j-1
				...
				endif !if(icorr.ge.0)   8477
				jc=jc+1
			end do !j2=1,j-1   8468
		end if !if(iscov)   8467
		if(icov.ge.0)then
...
subroutine dos(iob,io)  !do loops in input programming  ;do()
	...
	if(j_ndo.ge.j_mxndo)then
		...
		j_ndo=0
		return
	endif !if(j_ndo.ge.j_mxndo)   8666
	if(j_ndo.eq.j_ndoinc(j_ninc))then
		if(j_reacom(j_ninc))then
			...
			endif !if(.not.j_savcom(j_ninc))   8676
			icu=istart
			...
			endif !if(j_inp(1:j_linp).eq.'endsimu')   8701
			call j_puttext(j_ivbuf(j_ninc), j_inp(1:j_linp))
			...
			if(j_inp(j_ialb:j_ialb+2).eq.';do'.and.j_inp(ialb2:ialb2).eq.'(')then
				ndotemp=ndotemp+1
			endif !if(j_inp(j_ialb:j_ialb+2).eq.';do'.and.j_inp(ialb2:ialb2).   8708
			if(j_inp(j_ialb:j_ialb+5).eq.';enddo'.or.j_inp(j_ialb:j_ialb+6).eq.';end do')then
				ndotemp=ndotemp-1
			endif !if(j_inp(j_ialb:j_ialb+5).eq.';enddo'.or.j_inp(j_ialb:j_ia   8711
...
subroutine gotos_(iob,io) ! ;goto  in input programming
	...
	if(j_ninc.lt.2.or.j_ipc(iva).eq.0)then
		...
		return
	end if !if(j_ninc.lt.2.or.j_ipc(iva).eq.0)   8789
 
	if(j_ndo.gt.0)then
		write(6,*)'**open ;do loops closed'
		j_ndo=0
	endif !if(j_ndo.gt.0)   8794
	...
	if(ivb.ne.0.and.j_otype(max(ivb,1)).eq.j_iptext)then !search for labels
			...
			end do !j=0,iva2-iva1   8806
			j_icurl(j_ninc)=iil
			j_reacom(j_ninc)=.false.
			return !adrloop: do i=1,j_o(ivb)%i2(0)
 
		enddo adrloop !loop: do i=1,j_o(ivb)%i2(0)   8802
		sav=.true.
	else !if(ivb.ne.0.and.j_otype(max(ivb,1)).eq.j_iptext)then
		!not saving
		sav=.false.
	end if !if(ivb.ne.0.and.j_otype(max(ivb,1)).eq.j_iptext)   8801
	!like in incl
...
subroutine storematrix(iob,io)
	...
	if(j_o(arg(1))%i(4).ne.j_matreg)then
		...
		j_err=.true.
		return
	endif !if(j_o(arg(1))%i(4).ne.j_matreg)   9014
	nob=j_o(arg(1))%i(1)
	nvar=j_o(arg(1))%i(2)
...
subroutine elementsum(iob,io)   ! %%matrix
	...
	if(j_otype(iarg).ne.j_ipmatrix)then
		...
		j_err=.true. ;return
	endif !if(j_otype(iarg).ne.j_ipmatrix)   9049
	...
	if(ivrow_.gt.0)then
		...
		if(irow.le.0.or.irow.gt.j_o(iarg)%i(1))then
			...
			j_err=.true.  ;return
		endif !if(irow.le.0.or.irow.gt.j_o(iarg)%i(1))   9056
	endif !if(ivrow_.gt.0)   9054
	...
	if(ivcolumn.gt.0)then
		...
		if(icolumn.le.0.or.icolumn.gt.j_o(iarg)%i(2))then
			...
			j_err=.true.  ;return
		endif !if(icolumn.le.0.or.icolumn.gt.j_o(iarg)%i(2))   9064
	endif !if(ivcolumn.gt.0)   9062
	...
	if(ivrow_.gt.0)then
		...
		endif !if(las.gt.j_o(iarg)%i(2).or.las.le.0)   9092
	...
	elseif(ivcolumn.gt.0)then !if(ivrow_.gt.0)then
		...
		end if !if(las.le.0.or.las.gt.j_o(iarg)%i(1))   9104
		su=0
		ii=icolumn
		do i=1,las
			su=su+j_o(iarg)%d(ii)
		...
		if(las.le.0.or.las.gt.j_o(iarg)%i(3))then
			...
			j_err=.true. ;return
		end if !if(las.le.0.or.las.gt.j_o(iarg)%i(3))   9118
		j_v(iout)=sum(j_o(iarg)%d(1:las))
	endif !if(ivrow_.gt.0)   9091
	...
	if(ivrow_.gt.0)then
		if(las.gt.j_o(iarg)%i(2).or.ifi.gt.las.or.ifi.le.0)then
			...
			j_err=.true.
			return
		endif !if(las.gt.j_o(iarg)%i(2).or.ifi.gt.las.or.ifi.le.0)   9129
		ifi=(irow-1)*j_o(iarg)%i(2)
		j_v(iout)=sum(j_o(iarg)%d(ifi+1:ifi+las))
	...
	elseif(ivcolumn.gt.0)then !if(ivrow_.gt.0)then
		if(las.le.0.or.las.gt.j_o(iarg)%i(2))then
			...
			j_err=.true. ;return
		end if !if(las.le.0.or.las.gt.j_o(iarg)%i(2))   9140
		su=0
		ii=icolumn
		do i=1,las
			su=su+j_o(iarg)%d(ii)
		...
		if(ifi.le.0.or.ifi.gt.j_o(iarg)%i(3).or.las.le.0.or.las.gt.j_o(iarg)%i(3))then
			...
			j_err=.true. ;return
		end if !if(ifi.le.0.or.ifi.gt.j_o(iarg)%i(3).or.las.le.0.or.las.gt   9153
		j_v(iout)=sum(j_o(iarg)%d(ifi:las))
	endif !if(ivrow_.gt.0)   9128
...
subroutine dotproduct(iob,io)  !%%matrix
	...
	if(las.le.0.or.las.gt.nele)then
		...
		j_err=.true. ;return
	end if !if(las.le.0.or.las.gt.nele)   9190
	...
	if(ifi.le.0.or.ifi.gt.nele.or.las.le.0.or.las.gt.nele)then
		...
		j_err=.true. ;return
	end if !if(ifi.le.0.or.ifi.gt.nele.or.las.le.0.or.las.gt.nele)   9197
	end select !select case (narg)
...
subroutine loadtrees(iob,io)
	...
	do itre=j_itree1,j_itree2
		...
		enddo !i=1,j_gettreevars(0)   9213
	enddo !itre=j_itree1,j_itree2   9211
	!	io=io+narg+3
	return
...
subroutine assone(iob,io)
	...
	endif !if(j_otype(j_o(iob)%i(io+2)).eq.j_ipreal)   9222
...
subroutine assign(iob,io)
	...
	elseif(narout.eq.1)then !if(narin.eq.1.and.narout.gt.1)then
		if(j_otype(argout(1)).eq.j_ipmatrix.and.j_otype(argin(1)).eq.j_ipreal)then
			...
			endif !if(j_o(argout(1))%i(4).eq.j_matdiag)   9273
			return
		endif !if(j_otype(argout(1)).eq.j_ipmatrix.and.j_otype(argin(1)).   9266
	endif !if(narin.eq.1.and.narout.gt.1)   9251
...
subroutine datawcase(iob,io)
	logical maketran
	! Section datawcase datawcase(): Creates a data object where eac observation has name
	! This function will replace or be alternative for properties function.
	!This function, to be reported later, will be used in factory optimization.
	! endsection
	...
	!character*24 name1
	!		integer,dimension(:),allocatable::linevar
	call j_startfunction(iob,io,0,.true.,narg,j_optarg0,iout)
	if(j_err)return
	call j_getname(iout)
	write(6,*)
	write(6,*)'Making data with cases: ',j_oname(1:j_loname)
	call j_getoption(iob,io,j_mread,1,99999,j_ipreal,.true.,nread,j_optarg)
...
subroutine classify(iob,io)
	...
	if(j_linkoption(iob,io,j_mxrange).lt.0.and.j_linkoption(iob,io,j_many).lt.0)then
		...
		j_err=.true.;return
	endif !if(j_linkoption(iob,io,j_mxrange).lt.0.and.j_linkoption(io   9587
	...
	call j_getoption(iob,io,j_mxrange,-1,2,j_ipreal,.false.,nr,j_optarg0)
	xrange=.true.
	if(nr.lt.0)then
		...
		endif !if(j_linkoption(iob,io,j_mdx).gt.0)   9598
		if(j_linkoption(iob,io,j_mclasses).ge.0)then
			write(6,*)'classes-> requires xrange->'
			j_err=.true.;return
 
		endif !if(j_linkoption(iob,io,j_mclasses).ge.0)   9604
		xrange=.false.
	...
	elseif(nr.eq.0)then !if(nr.lt.0)then
		...
		endif !if(ii1.le.0.or.ii2.le.0)   9618
		xmin=j_v(ii1)
		xmax=j_v(ii2)
	...
	if(xrange)then
		...
		if(nargoc.gt.0)then
			...
			clasn=7.
		endif !if(nargoc.gt.0)   9649
		if(nargoc.ge.2)then
			clasz=j_v(j_o(iob)%i( li+2))
		else !if(nargoc.ge.2)then
			clasz=7.
		endif !if(nargoc.ge.2)   9654
 
		!ivcl=j_igetopt(iob,io,j_mclasses)
 
		call	j_getoption(iob,io,j_mdx,-1,1,j_ipreal,.true.,nargopt,j_optarg0)
		if(j_err)return
		...
		if(nargopt.gt.0)then
			...
			nval=clasn
		end if !if(nargopt.gt.0)   9665
	end if !if(xrange)   9640
	nval2=0
	call	j_getoption(iob,io,j_mzrange,-1,1,j_ipreal,.true.,nargopt,j_optarg0)
	if(iz.gt.0)then
		if(nargopt.lt.2)then
			...
			if(allocated(iperm))then
				if(size(iperm).lt.jnobstot)then
					deallocate(iperm)
					allocate(iperm(1:jnobstot))
				endif !if(size(iperm).lt.jnobstot)   9687
			else !if(allocated(iperm))then
				allocate(iperm(1:jnobstot))
			endif !if(allocated(iperm))   9686
		else !if(nargopt.lt.2)then
			zrange=.true.
			zmin=j_v(j_optarg0(1))
			zmax=j_v(j_optarg0(2))
			call	j_getoption(iob,io,j_mdz,-1,1,j_ipreal,.true.,nargopt,j_optarg0)
			if(nargopt.ge.1)then
				...
				nval2=clasz
			end if !if(nargopt.ge.1)   9699
 
		end if !if(nargopt.lt.2)   9683
	endif !if(iz.gt.0)   9682
	call	j_getoption(iob,io,j_mmean,-1,1,j_ipreal,.true.,igmean,j_optarg0)
	...
	if(minobs.gt.0)then
		...
		endif !if(iz.gt.0)   9717
	endif !if(minobs.gt.0)   9714
	...
	endif !if(.not.xrange.and.iz.gt.0.and..not.zrange)   9730
	...
	do i=j_dfrom,j_duntil
		...
		if(abs(j_v(ix_)).ge.1.7d19)then
			missing=missing+1
			cycle
		endif !if(abs(j_v(ix_)).ge.1.7d19)   9761
		if(iz.gt.0)then
			if(abs(j_v(iz)).ge.1.7d19)then
				missing=missing+1
				cycle
			endif !if(abs(j_v(iz)).ge.1.7d19)   9766
 
		endif !if(iz.gt.0)   9765
		if(iy.gt.0)then
			if(abs(j_v(iy)).ge.1.7d19)then
				missing=missing+1
				cycle
			endif !if(abs(j_v(iy)).ge.1.7d19)   9773
		endif !if(iy.gt.0)   9772
		...
		if(iz.gt.0)then
			if(zrange)then
				...
				enddo !j2=1,nval2   9805
 
			end if !if(zrange)   9797
		end if !if(iz.gt.0)   9796
		...
		endif !if(j2.gt.0)   9825
	enddo !i=j_dfrom,j_duntil   9756
	...
	if(minobs.gt.0)then
		...
		155 if(sumwtx(i0).lt.dinobs.and.sumwtx(i0).gt.0)then
			...
			if((prevc(i0).ne.0.and.xsx(i0)-xsx(prevc(i0)).lt.xsx(nextc(i0))-xsx(i0)).or.&
				...
				i2=nextc(i0)
			end if !if((prevc(i0).ne.0.and.xsx(i0)-xsx(prevc(i0)).lt.xsx(nextc   9870
			! (x-xm)**2=
		...
		if(iz.gt.0)then
			...
			if(igmean.gt.0)then
				...
				if(xrange)then
					do i=1,nval
						call j_putmatrix(ivmat,1,i+1,xmin+(i-0.5)*dx)
					end do !i=1,nval   9939
				else !if(xrange)then
					j_o(ivmat)%d(2:nval+1)=value9(1:nval)
				end if !if(xrange)   9938
				if(zrange)then
					do i=1,nval2
						call j_putmatrix(ivmat,i+1,1,zmin+(i-0.5)*dz)
					end do !i=1,nval2   9946
				else !if(zrange)then
					do i=1,nval2
						call j_putmatrix(ivmat,i+1,1,value2(i))
					end do !i=1,nval2   9950
				end if !if(zrange)   9945
				do i=1,nval
					do j=1,nval2
						call j_putmatrix(ivmat,j+1,i+1,xs(j,i))
					end do !j=1,nval2   9955
			...
			if(isy)then
				...
				do j=0,nval
					if(sumwt(0,j).gt.1.d0.and.xss(0,j).gt.0.d0)then
						...
						haj=0.d0
					end if !if(sumwt(0,j).gt.1.d0.and.xss(0,j).gt.0.d0)   9985
					j2=j
					if(j.eq.0)j2=nval+1
					j_o(ivmat)%d(3*(nval+1)+j2)=haj
				end do !j=0,nval   9984
			end if !if(isy)   9981
		end if !if(iz.gt.0)   9906
		if(isfunc)iy=j_ivresult
		j_o(ivmat)%i2=(/ix_,iz,iy/)
	end if !if(iout.ne.j_ivresult)   9905
	...
	if(iz.gt.0)then
		...
		if(isy)then
			...
			do j=1,nval
				...
				endif !if(xrange)  10034
				write(6,*)' '
			enddo !j=1,nval  10033
		endif !if(isy)  10021
...
4561	format(f12.5,i8,3f12.5)
		if(xrange.and.isy)then  !
			if(j_dprint.ge.2)then
				...
				do j=1,nval
					...
					endif !if(freqtot(0,j).gt.0)  10085
				enddo !j=1,nval  10083
				write(6,*)'---------------'
				...
				enddo !j=1,nval  10096
				write(6,*)'---------------'
				write(6,4561)xsx(0),nobst,xs(0,0)
			endif !if(j_dprint.ge.2)  10080
		...
		elseif(isy)then !if(xrange.and.isy)then
			...
			enddo !j=1,nval  10135
			write(6,*)'-----------'
			write(6,*)'       ',nobst
 
		endif !if(xrange.and.isy)  10079
		!	endif
	end if !if(iz.gt.0)  10001
...
subroutine show(iob,io)
	...
	elseif(j_otype(ivfig).eq.j_ipfigure)then !if(j_otype(ivfig).eq.j_ipchar)then
		...
		j_err=.true.
	endif !if(j_otype(ivfig).eq.j_ipchar)  10204
	call j_clearoption(iob,io)
	return
...
subroutine plot3d(iob,io)
	...
	if(j_otype(imat).ne.j_ipmatrix)then
		...
		j_err=.true.;return
	endif !if(j_otype(imat).ne.j_ipmatrix)  10279
	nrow=j_o(imat)%i(1)
	...
	endif !if(j_o(imat)%i(2).ne.3)  10286
	...
	if(sorted)then
		...
		do i=1,nrow
			...
			endif !if(i.lt.nrow)  10327
			ibas=ibas+3
		enddo !i=1,nrow  10325
	else
		j=(p(1)-1)*3
		do i=1,nrow
			...
			if(i.lt.nrow)then
				...
				j=j2
			endif !if(i.lt.nrow)  10336
		end do !i=1,nrow  10334
	endif !if(sorted)  10323
	deallocate(xi,p)
	call j_closeunit(nu)
...
subroutine clearbits(iob,io)
	...
	do i=2,narg
		...
		if(j.gt.32.or.j.le.0)then
			...
			j_err=.true.
			return
		end if !if(j.gt.32.or.j.le.0)  10375
		!   ii=(j-1)/32+1
		ibit=j-1  ! bit numbering starts from zero
		rw=j_v(ivb)
		iw= ibclr(iw,ibit)
		j_v(ivb)=rw
	end do !i=2,narg  10373
	...
	do i=2,narg
		...
		if(ii.gt.j_o(ivb)%i(0).or.j.le.0)then
			...
			j_err=.true.
			return
		end if !if(ii.gt.j_o(ivb)%i(0).or.j.le.0)  10392
		ibit=j-(ii-1)*64-1   !(ii-1)*32-1  ! bit numbering starts from zero
		rw=j_v(j_o(ivb)%i(ii))
		iw= ibclr(iw,ibit)
		j_v(j_o(ivb)%i(ii))=rw
	end do !i=2,narg  10389
...
subroutine setbits(iob,io)
	...
	do i=2,narg
		...
		if(j.gt.64.or.j.le.0)then
			...
			j_err=.true.
			return
		end if !if(j.gt.64.or.j.le.0)  10427
		!   ii=(j-1)/32+1
		ibit=j-1  ! bit numbering starts from zero
		rw=j_v(ivb)
		iw= ibset(iw,ibit)
		j_v(ivb)=rw
	end do !i=2,narg  10425
	case(j_iplist) !select case(j_otype(ivb))
	do i=2,narg
		j=j_v(j_o(iob)%i2(io+1+i))
		if(j.gt.j_o(ivb)%i(1).or.j.le.0)then
			...
			j_err=.true.
			return
		end if !if(j.gt.j_o(ivb)%i(1).or.j.le.0)  10442
		ii=(j-1)/64+1  !32+1
		if(ii.gt.j_o(ivb)%i(0).or.j.le.0)then
			call j_printname('**setbits: list', ivb,' ')
			...
			j_err=.true.
			return
		end if !if(ii.gt.j_o(ivb)%i(0).or.j.le.0)  10449
		ibit=j-(ii-1)*64-1  !32-1  ! bit numbering starts from zero
		rw=j_v(j_o(ivb)%i(ii))
		iw= ibset(iw,ibit)
		j_v(j_o(ivb)%i(ii))=rw
	end do !i=2,narg  10440
	end select !select case(j_otype(ivb))
	return
...
subroutine getbit(iob,io)
	...
	if(j.gt.64.or.j.le.0)then
		...
		j_err=.true.
		return
	end if !if(j.gt.64.or.j.le.0)  10487
	icurbit=j-1
	rw=j_v(ivb)
	if(btest(iw,icurbit))then
		...
		j_v(iout)=0.
	end if !if(btest(iw,icurbit))  10495
	case(j_iplist) !select case(j_otype(ivb))
	ii=(j-1)/64+1
	if(ii.gt.j_o(ivb)%i(0).or.j.le.0)then
		...
		j_err=.true.
		return
	end if !if(ii.gt.j_o(ivb)%i(0).or.j.le.0)  10502
	icurbit=j-(ii-1)*64-1
	rw=j_v(j_o(ivb)%i(ii))
	if(btest(iw,icurbit))then
		j_v(iout)=j_1
		...
		j_v(iout)=j_0
	end if !if(btest(iw,icurbit))  10510
	end select !select case(j_otype(ivb))
	return
...
subroutine getbitch(iob,io)
	...
	do i=i1,i2
		...
		if(btest(iw,i-1))then
			...
			j_cline(ie:ie)='0'
		end if !if(btest(iw,i-1))  10554
	end do !i=i1,i2  10552
	case(j_iplist) !select case(j_otype(ivb))
	...
	do i=i1,i2
		...
		if(btest(iw,icurbit))then
			...
			j_cline(ie:ie)='0'
		end if !if(btest(iw,icurbit))  10570
	end do !i=i1,i2  10565
	end select !select case(j_otype(ivb))
...
subroutine sort(iob,io)
	...
	if(j_otype(imat).ne.j_ipmatrix)then
		...
		j_err=.true.
		return
	end if !if(j_otype(imat).ne.j_ipmatrix)  10616
	nrow_=j_o(imat)%i(1)
	ncol=j_o(imat)%i(2)
	...
	if(key.le.0.or.key.gt.ncol)then
		...
		j_err=.true.
		!	return
	end if !if(key.le.0.or.key.gt.ncol)  10629
	...
	if(noptarg.eq.2)then
		...
		if(key2.le.0.or.key2.gt.ncol)then
			...
			j_err=.true.
			return
		end if !if(key2.le.0.or.key2.gt.ncol)  10645
...
subroutine thisfile(iob,io) ! returns the character object referring to current include file
	...
	if(j_ninc.le.1)then
		...
		if(ivchar.le.0)then
			...
			j_err=.true.
		endif !if(ivchar.le.0)  10756
		if(j_err) return
 
		call j_asschar2(ivchar,ivout)
	endif !if(j_ninc.le.1)  10748
	return
...
subroutine filestat(iob,io)
	...
	if(j_otype(ifile).ne.j_ipchar)then
		write(6,*)'illegal argument'
		j_err=.true. ;return
	endif !if(j_otype(ifile).ne.j_ipchar)  10775
...
SUBROUTINE envelope(iob,io)
	...
	if(j_o(ivmat)%i(2).ne.2)then
		...
		j_err=.true.
		return
	endif !if(j_o(ivmat)%i(2).ne.2)  10986
	call j_getoption(iob,io,j_mnobs,-1,1,j_ipreal,.true.,narg,arg)
	if(narg.ge.1)then
		n=j_v(arg(1))
	else !if(narg.ge.1)then
		n=j_o(ivmat)%i(1)
	endif !if(narg.ge.1)  10992
	IF (n < 2) then
	...
	DO i = 2, n-1
		...
		ELSE IF (temp > xmax) THEN !IF (temp < xmin) THEN
			vertex(2) = i
			xmax = temp
		END IF !IF (temp < xmin)  11018
	END DO !i = 2, n-1  11016
	...
	IF (xmax == xmin) THEN
		...
		DO i = 2, n-1
			...
			ELSE IF (temp > ymax) THEN !IF (temp < ymin) THEN
				vertex(2) = i
				ymax = temp
			END IF !IF (temp < ymin)  11042
		END DO !i = 2, n-1  11040
	...
	DO i = 1, n
		...
		ELSE IF (dist < zero) THEN !IF (dist > zero) THEN
			...
			IF (dist < dmin) THEN
				next(2) = i
				dmin = dist
			END IF !IF (dist < dmin)  11088
		END IF !IF (dist > zero)  11078
	END DO !i = 1, n  11075
	...
	40 IF (next(j) < 0) THEN
		...
		j = j + 1
		GO TO 40
	END IF !40 IF (next(j) < 0)  11108
 
	jp1 = j + 1
	DO i = nvert, jp1, -1
		vertex(i+1) = vertex(i)
		next(i+1) = next(i)
	END DO !i = nvert, jp1, -1  11115
	jp2 = jp1 + 1
	nvert = nvert + 1
	IF (jp2 > nvert) jp2 = 1
	i1 = vertex(j)
		...
		ELSE !IF (dist > zero) THEN
			...
			IF (dist > zero) THEN
				...
				IF (dist > dmax2) THEN
					next(jp1) = i
					dmax2 = dist
				END IF !IF (dist > dmax2)  11164
			END IF !IF (dist > zero)  11161
		END IF !IF (dist > zero)  11152
		i = iwk(i)
	ELSE !IF (i /= i2save) THEN
		i = i2next
	END IF !60 IF (i /= i2save)  11150
...
100	continue  !call j_nextobs();if(j_err)return
	...
	if(j_rejected)then
		...
		if(ial.gt.j_duntil)then
			...
			j_err=.true.
			return
		endif !if(ial.gt.j_duntil)  11244
		goto 100
	endif !if(j_rejected)  11242
	...
	do i=ial,j_duntil
		...
		if(abs(j_v(iv)).ge.1.7d19)then
			missining=missing+1
			cycle
		endif !if(abs(j_v(iv)).ge.1.7d19)  11259
		nacc=nacc+1
		if(.not.any(value9(1:nval).eq.j_v(iv)))then
			nval=nval+1;value9(nval)=j_v(iv)
		end if !if(.not.any(value9(1:nval).eq.j_v(iv)))  11264
	end do !i=ial,j_duntil  11254
	!	end do !do k=1,jndatasetss
...
subroutine solve(iob,io)
	...
	if(j_otype(arg(1)).eq.j_ipreal)then
		if(j_v(arg(1)).eq.j_0)then
			...
			j_err=.true.;return
		endif !if(j_v(arg(1)).eq.j_0)  11341
		...
		elseif(j_otype(arg(2)).eq.j_ipmatrix)then
			...
			return
		end if !if(j_otype(arg(2)).eq.j_ipreal)  11345
	endif !if(j_otype(arg(1)).eq.j_ipreal)  11340
	...
	if(itarg1.ne.j_ipmatrix.or.itarg2.ne.j_ipmatrix)then
		...
		j_err=.true.;return
	endif !if(itarg1.ne.j_ipmatrix.or.itarg2.ne.j_ipmatrix)  11358
	ndim=j_o(arg(1))%i(1)
	ndim2=j_o(arg(2))%i(2)
	if(ndim.ne.j_o(arg(1))%i(2).or.j_o(arg(2))%i(1).ne.ndim)then
		...
		j_err=.true.
		return
	endif !if(ndim.ne.j_o(arg(1))%i(2).or.j_o(arg(2))%i(1).ne.ndim)  11364
	...
	do i=1,ndim
		...
		enddo !j=1,ndim  11374
 
		do j=1,ndim2
			iel2=iel2+1
			rhs_(i,j)=j_o(arg(2))%d(iel2)
		enddo !j=1,ndim2  11379
	enddo !i=1,ndim  11373
	...
	if(info_.ne.0)then
		...
		do i=1,ndim
			...
			enddo !j=1,ndim2  11395
		enddo !i=1,ndim  11394
	endif !if(info_.ne.0)  11386
...
subroutine regr(iob,io)   !compute a linear regression model using linpack (%%stat)
	...
	endif !if(nstep.ge.1)  11533
 
	call j_getoption_index(iob,io,j_mvar,-1,0,0,&
		.false.,nvar,j_optarg0)
	var=nvar.ge.0
	...
	intcep2=intcep+1
		...
		if(isvariance)then
			...
			sumw=j_0
				...
				if(abs(j_v(iy)).ge.1.7d19)then
					missing=missing+1
					cycle
				endif !if(abs(j_v(iy)).ge.1.7d19)  11576
 
				do j=intcep2,ncoef
					if(abs(j_v(regl(j))).ge.1.7d19)then
						missing=missing+1
						cycle obloopw
					endif !if(abs(j_v(regl(j))).ge.1.7d19)  11582
				enddo !j=intcep2,ncoef  11581
				...
				do j=1,ncoef
					...
					enddo !j2=j,ncoef  11610
				enddo !j=1,ncoef  11607
				sumw=sumw+weight
				sumy=sumy+weight*j_v(iy)
				sumynw=sumynw+j_v(iy)
			enddo obloopw !oopw:do i=j_dfrom,j_duntil  11570
				...
				if(abs(j_v(iy)).ge.1.7d19)then
					missing=missing+1
					cycle
				endif !if(abs(j_v(iy)).ge.1.7d19)  11628
 
				do j=intcep2,ncoef
					if(abs(j_v(regl(j))).ge.1.7d19)then
						missing=missing+1
						cycle obloop1
					endif !if(abs(j_v(regl(j))).ge.1.7d19)  11634
				enddo !j=intcep2,ncoef  11633
				...
				do j=1,ncoef
					...
					enddo !j2=j,ncoef  11654
				enddo !j=1,ncoef  11651
 
				sumy=sumy+j_v(iy)
			enddo obloop1 !oop1:do i=j_dfrom,j_duntil  11622
		endif !if(isvariance)  11567
		if(ntot.lt.ncoef)then
			write(6,*)'*only ',ntot,' observations accepted, estimation not possible'
			j_err=.true.
			return
		endif !if(ntot.lt.ncoef)  11663
		...
		do j=1,ncoef
			if(mat(j,j).lt.1.d-18)then
				...
				goto 445
			endif !if(mat(j,j).lt.1.d-18)  11672
			do j2=1,j-1
				mat(j,j2)=mat(j2,j)
				if(abs(mat(j,j2)/sqrt(mat(j,j)*mat(j2,j2))).gt.0.99999d0)then
					...
					endif !if(j2.eq.1.and.intcep2.gt.1)  11682
					iimin=j
					singutot=.true.
					singucur=.true.
					goto 445
				endif !if(abs(mat(j,j2)/sqrt(mat(j,j)*mat(j2,j2))).gt.0.99999d0)  11681
			enddo !j2=1,j-1  11679
		enddo !j=1,ncoef  11671
		...
		ibas=-j_dnkeep    !dtas
			...
			if(j_rejected.or.abs(j_v(iy)).ge.1.7d19)then
				...
				cycle
			endif !if(j_rejected.or.abs(j_v(iy)).ge.1.7d19)  11721
 
			do j=intcep2,ncoef
				if(abs(j_v(regl(j))).ge.1.7d19)cycle obloop2
			enddo !j=intcep2,ncoef  11727
		...
		if(step)then
			...
			do ii=intcep2,ncoef
				if(abs(t(ii)).lt.tmin0)then
					iimin=ii !REGL
					tmin0=abs(t(ii))
				endif !if(abs(t(ii)).lt.tmin0)  11780
			enddo !ii=intcep2,ncoef  11779
	...
	718 format(1x,a15,2g16.8e1,f11.2)
		...
		endif !if(isvariance)  11812
		if(intcep.gt.0)then
			write(6,*)'F reg (',ncoef-1,idfe,')=',(sst-sse)/(mse_*(ncoef-1.))
		end if !if(intcep.gt.0)  11818
445	if(step.and.tmin0.lt.tmin.or.singucur)then
			if(iimin.lt.ncoef)then
				do ii=iimin,ncoef-1
					regl(ii)=regl(ii+1)
				enddo !ii=iimin,ncoef-1  11823
			endif !if(iimin.lt.ncoef)  11822
 
 
			ncoef=ncoef-1
			ncoef1=ncoef1-1
			cycle steploop
		endif !445	if(step.and.tmin0.lt.tmin.or.singucur)  11821
		exit steploop
 
	enddo steploop !ploop:	do iste=1,ncoef  11554
	!ortonew=0
	...
	if(iout.ne.j_ivresult)then
		...
		if(var)then
			...
			do i=1,ncoef
				...
				enddo !j=1,ncoef  11871
			enddo !i=1,ncoef  11870
		endif !if(var)  11867
		ivcor=0
		if(corr)then
			...
			do i=1,ncoef
				do j=1,ncoef
					...
					endif !if(i.eq.j)  11888
					!		endif
				enddo !j=1,ncoef  11885
			enddo !i=1,ncoef  11884
		endif !if(corr)  11881
		...
		endif !if(getpar)  11936
 
	endif !if(iout.ne.j_ivresult)  11860
...
subroutine coefse(iob,io,ity)  !ity=0 coef, ity=1 se
	...
	do i=1,j_o(irg)%i(0)
		...
		endif !if(j_o(irg)%i(i).eq.inde)  11987
	end do !i=1,j_o(irg)%i(0)  11986
...
subroutine nonlin(iob,io)   !compute a linear regression model using linpack (%%stat)
	...
	call j_getoption(iob,io,j_mpar,1,9999,j_ipreal,.true.,ncoef,par)
	call j_getoption(iob,io,j_minitial,-1,9999,j_ipreal,.true.,ncoef1,initial)
 
 
	call j_getoption(iob,io,j_mmaxiter,-1,1,j_ipreal,.true.,narg,arg)
	maxiter=100
	if(narg.gt.0)maxiter=j_v(arg(1))
	call j_getoption(iob,io,j_mslow,-1,1,j_ipreal,.true.,narg,arg)
	slow=-0.1
	if(narg.gt.0)slow=j_v(arg(1))
	call j_getoption(iob,io,j_mtole,-1,1,j_ipreal,.true.,narg,arg)
	tole=0.1
	if(narg.gt.0)tole=j_v(arg(1))
	! iprint=1
	! call j_getoption(iob,io,j_mprint,-1,1,j_ipreal,.true.,narg,arg)
	! if(narg.gt.0)iprint=j_v(arg(1))
	call j_getoption(iob,io,j_mvar,-1,1,j_ipreal,.false.,narg,arg)
	var=narg.ge.0
	call j_getoption(iob,io,j_mcorr,-1,1,j_ipreal,.false.,narg,arg)
	corr=narg.ge.0
	if(j_err)return
	...
	if(ncoef1.gt.0)then
		if(ncoef.ne.ncoef1)then
			...
			j_err=.true. ;return
		endif !if(ncoef.ne.ncoef1)  12082
 
		j_v(par(1:ncoef1))=j_v(initial(1:ncoef1))
	endif !if(ncoef1.gt.0)  12081
	...
	enddo !ii=1,ncoef  12095
	if(nnz.eq.0)then
		write(6,*)'some initial values should be nonzero'
		j_err=.true.
		return
	endif !if(nnz.eq.0)  12098
	...
	call j_getoption(iob,io,j_mparmin,-1,ncoef,j_ipreal,.true.,narg,arg)
	...
	if(narg.gt.0.)then
		if(narg.ne.ncoef)then
			...
			isparmin=.true.
		endif !if(narg.ne.ncoef)  12115
	endif !if(narg.gt.0.)  12114
	...
	if(narg.gt.0)then
		if(narg.ne.ncoef)then
			...
			isparmax=.true.
		endif !if(narg.ne.ncoef)  12129
	endif !if(narg.gt.0)  12128
	...
	if(narg.gt.0)then
		if(narg.ne.ncoef)then
			...
			isdpar=.true.
		endif !if(narg.ne.ncoef)  12143
	endif !if(narg.gt.0)  12142
	step=1
	call j_getoption(iob,io,j_mstep,-1,1,j_ipreal,.true.,narg,arg) !J added this 6.10.2020
	...
	do i=j_dfrom,j_duntil
		...
		if(abs(j_v(iy)).ge.1.7d19)then
			missing=missing+1
			cycle
		endif !if(abs(j_v(iy)).ge.1.7d19)  12212
		ntot=ntot+1
		...
		if(isvariance)then
			...
			if(variance(i).le.j_0)then
				...
				j_err=.true. ;return
			endif !if(variance(i).le.j_0)  12224
			...
			do j=1,ncoef
				...
				enddo !j2=j,ncoef  12241
			enddo !j=1,ncoef  12238
			...
			do j=1,ncoef
				...
				enddo !j2=j,ncoef  12256
			enddo !j=1,ncoef  12253
			sse0=sse0+resid*resid
			sumf=sumf+j_v(if)
			!	end do !do k=1,jndatasetss
		endif !if(isvariance)  12220
		!	resid=j_v(iy)-j_v(if)
		!	sse0nw=sse0+resid*resid
		if(isloglike)loglike=loglike+j_codevalue(iob,linkloglike)
		if(iregf.gt.0)j_o(j_dimat)%d(ibas+iregf)=j_v(if)
		if(iresid.gt.0)j_o(j_dimat)%d(ibas+iresid)=resid
	end do !i=j_dfrom,j_duntil  12205
	do j=1,ncoef
		do j2=1,j-1
			mat(j,j2)=mat(j2,j)
		enddo !j2=1,j-1  12272
	enddo !j=1,ncoef  12271
	!		write(6,*)'mat',mat
	!	write(6,*)'rhs_',rhs_
	...
	if(iter.gt.maxiter)then
		...
		j_v(par(1:ncoef))=current
		goto 200
	endif !if(iter.gt.maxiter)  12303
	if(iter2.gt.8)then
		j_v(par(1:ncoef))=current
	...
	if(isparmin)then
		do i=1,ncoef
			...
			endif !if(j_v(par(i)).lt.parmin(i))  12320
		enddo !i=1,ncoef  12319
 
	endif !if(isparmin)  12318
	scalemin2=scale
	if(isparmax)then
		do i=1,ncoef
	...
	if(isdpar)then
		...
		enddo !i=1,ncoef  12355
 
	endif !if(isdpar)  12354
	...
	do i=j_dfrom,j_duntil
		...
		if(isvariance)then
			...
			sumf=sumf+j_v(if)
		endif !if(isvariance)  12384
	end do !i=j_dfrom,j_duntil  12375
	!	end do !do k=1,jndatasetss
	...
	if(j_dprint.gt.0)then
		...
		end do !j=1,ncoef  12450
	endif !if(j_dprint.gt.0)  12442
...
589	format(a,g14.6,a,f7.4,a,i7)
		...
		endif !if(isvariance)  12461
	endif !if(j_dprint.gt.0)  12457
	if(isloglike)then
		write(6,*)'loglikehilihood ',loglike
		write(6,*)' '
	endif !if(isloglike)  12469
	if(iout.ne.j_ivresult)then
		if(j_otype(iout).ne.j_ipreal)call j_del(iout)
		...
		if(var)then
			...
			do i=1,ncoef
				do j=1,ncoef
					it=it+1
					j_o(ivvar)%d(it)=rhs_(i,j)*mse_  !laske
				enddo !j=1,ncoef  12490
			enddo !i=1,ncoef  12489
		endif !if(var)  12486
		ivcor=0
		if(corr)then
			call j_defmatrix(iout,'%corr',ncoef,ncoef,j_matreg,ivcor)
			...
			do i=1,ncoef
				do j=1,ncoef
					...
					if(i.eq.j)then
						...
						j_o(ivcor)%d(it)=rhs_(i,j)/sqrt(rhs_(i,i)*rhs_(j,j))
					endif !if(i.eq.j)  12503
				enddo !j=1,ncoef  12501
			enddo !i=1,ncoef  12500
		endif !if(corr)  12497
	...
	endif !if(iout.ne.j_ivresult)  12473
	if(j_depilog.gt.0)call dotrans(j_depilog,1)
	return
	900   write(6,*)'*regr: error in transformations'
	return
...
recursive subroutine value_(iob,io)  ! J-function value(
	...
	if(isz)then  !bilinear interpolation
		...
		if(nrows.lt.3.or.ncols.lt.3.or.narg.lt.3)then
			...
			j_err=.true.
			return
		endif !if(nrows.lt.3.or.ncols.lt.3.or.narg.lt.3)  12585
		arg2=j_v(j_o(iob)%i(io+4))
 
		do icol=3,ncols
			if(arg.le.j_getmatel(ifunc,1,icol))goto 17
 
		enddo !icol=3,ncols  12592
		icol=ncols
						17  continue
	...
	if(irow.le.0.and.j_o(ifunc)%i(4).eq.j_matreg)then
		...
		if(iout.gt.j_namedv)then
			...
			j_err=.true.
			return
		endif !if(iout.gt.j_namedv)  12615
		...
		do i=2,j_o(ifunc)%i(2)
			if(j_o(ifunc)%d(i).ge.arg.or.i.eq.j_o(ifunc)%i(2))then
				...
				end do !irow=2,j_o(ifunc)%i(1)  12624
				goto 900
			end if !if(j_o(ifunc)%d(i).ge.arg.or.i.eq.j_o(ifunc)%i(2))  12622
 
		end do !i=2,j_o(ifunc)%i(2)  12621
		...
		if(irow.le.0.or.irow.gt.j_o(ifunc)%i(1))then
			...
			j_err=.true.
			return
		end if !if(irow.le.0.or.irow.gt.j_o(ifunc)%i(1))  12638
	end if !if(irow.le.0.and.j_o(ifunc)%i(4).eq.j_matreg)  12612
	...
	if(iout.gt.j_mxnamedv+1)then
		...
		nsav=0
	endif !if(iout.gt.j_mxnamedv+1)  12668
	...
	if(inde.le.0.or.inde.gt.j_o(ifunc)%i(1))then
		...
		j_err=.true.
		return
	endif !if(inde.le.0.or.inde.gt.j_o(ifunc)%i(1))  12707
	...
	if(j.gt.j_o(ifunc)%i(1).or.j.le.0)then
		if(j_linkoption(iob,io,j_many).ge.0)then
			...
			!	call j_clearoption(iob,io)  ! subroutine
			goto 900
		endif !if(j_linkoption(iob,io,j_many).ge.0)  12739
		call j_printname('**value: first index has illegal value for bitmatrix ',ifunc, ' ')
		j_err=.true.;return
	endif !if(j.gt.j_o(ifunc)%i(1).or.j.le.0)  12738
	if(narg.gt.2)then
		...
		if(inde2.gt.j_o(ifunc)%i(2).or.inde2.lt.j_o(ifunc)%i(3))then
			if(j_linkoption(iob,io,j_many).ge.0)then
				...
				!	call j_clearoption(iob,io)  ! subroutine
				goto 900
			endif !if(j_linkoption(iob,io,j_many).ge.0)  12750
			call j_printname('**value: second argument has illegal value for bitmatrix ',ifunc, ' ')
			j_err=.true.;return
		endif !if(inde2.gt.j_o(ifunc)%i(2).or.inde2.lt.j_o(ifunc)%i(3))  12749
	endif !if(narg.gt.2)  12747
...
subroutine ran(iob,io) !rando_loopm uniform number netlib ranlib
	...
	end if !if(narg.gt.0)  12807
	if(j_otype(iout).eq.j_ipmatrix)then
		do j=1,j_o(iout)%i(3)
			j_o(iout)%d(j)=ranf()
		enddo !j=1,j_o(iout)%i(3)  12815
	else !if(j_otype(iout).eq.j_ipmatrix)then
 
		j_v(iout)=ranf()
	endif !if(j_otype(iout).eq.j_ipmatrix)  12814
...
subroutine select(iob,io)
	...
	if(iout.eq.j_ivresult)then
		...
		j_err=.true.
	endif !if(iout.eq.j_ivresult)  12848
	...
	if(n.lt.0.or.n.gt.ntot.or.ntot.le.0)then
		...
		j_err=.true.
	endif !if(n.lt.0.or.n.gt.ntot.or.ntot.le.0)  12855
	...
	do i=1,n
		...
		do j=1,ntot
			...
			endif !if(ir.eq.icount)  12876
 
		enddo !j=1,ntot  12874
	enddo !i=1,n  12869
		...
		jloop:		do j=1,j_o(iout)%i(3)
				...
				do i=1,ncol-1
					...
					if(j_o(iarg)%d(ncol+i).ge.ra)then
						...
						cycle jloop
					endif !if(j_o(iarg)%d(ncol+i).ge.ra)  12922
				enddo !i=1,ncol-1  12920
				j_o(iout)%d(j)=j_o(iarg)%d(ncol)
 
				!		j_o(iout)%d(j)=ranf()
			enddo jloop !op:		do j=1,j_o(iout)%i(3)  12917
			...
			do j=1,j_o(iout)%i(3)
				...
				do i=2,ncol
					...
					endif !if(j_o(iarg)%d(ncol+i).ge.ra)  12941
 
 
				enddo !i=2,ncol  12939
				!		j_o(iout)%d(j)=ranf()
			enddo !j=1,j_o(iout)%i(3)  12936
		endif !if(diskr)  12916
	elseif(diskr)then
		do i=1,ncol-1
			!y=y0+(y1-y0)*(x-x0)/(x1-x0)  here x is value of rnadom uniform and y is the xavalue
			if(j_o(iarg)%d(ncol+i).ge.ra)then
				...
				return
			endif !if(j_o(iarg)%d(ncol+i).ge.ra)  12960
		enddo !i=1,ncol-1  12958
		...
		do i=2,ncol
			...
			endif !if(j_o(iarg)%d(ncol+i).ge.ra)  12977
		enddo !i=2,ncol  12975
		!	j_v(iout)=ranf()
	endif !if(j_otype(iout).eq.j_ipmatrix)  12915
...
subroutine rann(iob,io)  !calls function gennor from netlib ranlib
	...
	if(narg.ge.1)then
		...
		if(narg.ge.2)then
			...
			sd=1.
		endif !if(narg.ge.2)  13046
	else !if(narg.ge.1)then
		av=0.
		sd=1.
	endif !if(narg.ge.1)  13044
	if(j_otype(iout).eq.j_ipmatrix)then
		do j=1,j_o(iout)%i(3)
			j_o(iout)%d(j)=gennor(av,sd)
		enddo !j=1,j_o(iout)%i(3)  13056
	else !if(j_otype(iout).eq.j_ipmatrix)then
		j_v(iout)=gennor(av,sd)
	endif !if(j_otype(iout).eq.j_ipmatrix)  13055
...
subroutine ranpoi(iob,io)  !calls function ignpoi from netlib ranlib
	...
	endif !if(rmu.le.0)  13080
	if(j_otype(iout).eq.j_ipmatrix)then
		do j=1,j_o(iout)%i(3)
			j_o(iout)%d(j)=ignpoi(rmu)
		enddo !j=1,j_o(iout)%i(3)  13087
	else !if(j_otype(iout).eq.j_ipmatrix)then
 
		j_v(iout)=ignpoi(rmu)
	endif !if(j_otype(iout).eq.j_ipmatrix)  13086
...
subroutine ranbin(iob,io)
	...
	if(j_otype(iout).eq.j_ipmatrix)then
		...
		elseif(p_.eq.1.)then !if(p_.eq.0.)then
			...
			enddo !j=1,j_o(iout)%i(3)  13157
		endif !if(p_.eq.0.)  13152
		...
		endif !if(p_.eq.0.)  13163
	endif !if(j_otype(iout).eq.j_ipmatrix)  13151
...
subroutine jump(iob,io) ! J-function jump
	...
	endif !if(j_njump.gt.j_mxjump)  13252
	...
	if(ilevel.eq.level.or.ilevel.eq.3)then
		...
		if(j_o(iob)%i(io+1).eq.1)then
			...
			if(j_otype(irg).eq.j_ipchar)then
				...
				return
			endif !if(j_otype(irg).eq.j_ipchar)  13301
		endif !if(j_o(iob)%i(io+1).eq.1)  13298
		call print(iob,io)
	endif !if(ilevel.eq.level.or.ilevel.eq.3)  13296
	return
...
subroutine simulate(iob,io) ! J-function simulate
	...
	elseif(nsel.ge.1)then !if(nsel.eq.0)then
		selected=.true.
		call	j_getoption_index(iob,io,j_mselector,-1,1,j_iptrans,.true.,noptarg,j_optarg0)
	...
	if(ivkeepper.gt.0)then
		...
		if(j_keepper.ge.npermin)then
			...
			endif !if(narg.eq.1)  13382
			write(6,*)j_keepper,' periods stored'
		endif !if(j_keepper.ge.npermin)  13380
	else !if(ivkeepper.gt.0)then
		j_keepper=j_nper
	end if !if(ivkeepper.gt.0)  13378
	...
	if(iout.ne.j_ivresult)then
		...
		if(j_linkoption(iob,io,j_mkeep).gt.0)then
			...
			do i=1,j_o(ivoutput)%i(1)
				...
				end if !if(j_inlistobject(j_o(ivoutput)%i(i),ivtreevars).le.0)  13408
			end do !i=1,j_o(ivoutput)%i(1)  13407
			j_o(ivkeeps)%i(1)=keepl0
		end if !if(j_linkoption(iob,io,j_mkeep).gt.0)  13399
		...
		end if !if(ivunitdata.le.0)  13424
		...
		end if !if(ivunitdataobs.le.0)  13429
				...
				do i=1,j_o(ivtreevars)%i(1)
					...
					if(ipos2.gt.0)then
						if(ipos.gt.0)then
							...
							j_err=.true.
							return
						end if !if(ipos.gt.0)  13479
					endif !if(ipos2.gt.0)  13478
				end do !i=1,j_o(ivtreevars)%i(1)  13475
				j_gettreevars(0)=iio
			end if !if(jivtreedata.le.0.or.j_otype(max(1,jivtreedata)).ne.j_ip  13466
		end if !if(ivtreevars.ne.0)  13464
		if(ivtreevars.ne.0)then
			...
			if(ivmaxtrees.gt.0)then
				...
				maxtrees=100
			end if !if(ivmaxtrees.gt.0)  13496
			do i=1,j_o(ivtreevars)%i(1)
				call j_defmatrix(j_o(ivtreevars)%i2(i),' ',maxtrees,1,j_matreg,ivtemp)
			end do !i=1,j_o(ivtreevars)%i(1)  13501
		end if !if(ivtreevars.ne.0)  13494
		if(isout)then
	...
	if(isout)then
		...
		if(iobc.gt.nobsb)then
			...
			iba=0
			nbuf=nbuf+1
		end if !if(iobc.gt.nobsb)  13558
		do j=1,nvar
			tail%rbuf(iba+j)=j_v(keepl(j))
		end do !j=1,nvar  13566
		iba=iba+nvar
	end if !if(isout)  13556
	510 if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))then
		if(j_iper.le.1)then
			...
			if(isout)then
				...
				end do !j=1,ncvar  13576
				ibac=ibac+ncvar
			end if !if(isout)  13574
			nsw=0
				...
				if(idata.lt.jndatasetss)then
					...
					if(ivtreevars.ne.0)then
						...
						if(jivtreedata.gt.0)then
							call j_getdat(jivtreedata,lopp_,j_ivtreemat,j_ivkeeptree) !,ivtranstree) !,ivvarstree)
							j_itree2=0
						end if !if(jivtreedata.gt.0)  13588
					end if !if(ivtreevars.ne.0)  13586
					iobs=0
				else !if(idata.lt.jndatasetss)then
					goto 900
				end if !if(idata.lt.jndatasetss)  13583
			end if !if(iobs.ge.nobs)  13582
			iobs=iobs+1
			!call j_nextobs()
			if(j_err)return
			if(selected)call selectsim() !out isimul
			...
			if(jivtreedata.gt.0)then
				j_itree1=j_itree2+1
				j_itree2=j_itree1+j_v(j_ivntree)-1
			end if !if(jivtreedata.gt.0)  13603
			goto 1000
		end if !if(j_iper.le.1)  13572
		j_iper=j_iper-1
		goto 510
	end if !510 if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))  13571
	...
	if(j_iper.eq.0)then
		if(iobs.ge.nobs)then
			if(idata.lt.jndatasetss)then
				...
				goto 900
			end if !if(idata.lt.jndatasetss)  13619
		end if !if(iobs.ge.nobs)  13618
		iobs=iobs+1
		!call j_nextobs()
		if(j_err)return
		if(selected)call selectsim() !out isimul
...
900 continue
	...
	if(ixns.gt.0)then
		...
		deallocate(keeplc)
	end if !if(ixns.gt.0)  13671
990 continue !  io=io+narg+3
	if(ivtreevars.ne.0)then
		do i=1,j_o(ivtreevars)%i(0)
			call j_del(j_o(ivtreevars)%i(i))
		enddo !i=1,j_o(ivtreevars)%i(0)  13678
	end if !if(ivtreevars.ne.0)  13677
	return
	...
	subroutine selectsim()
		...
		elseif(j_o(ivsel)%i(1).ne.1)then !if(j_otype(ivsel).ne.j_iplist)then
			...
			if(.not.any(ivsimu2.eq.j_o(iob)%i(io+2:io+1+narg)))then
				...
				j_err=.true.
				return
			endif !if(.not.any(ivsimu2.eq.j_o(iob)%i(io+2:io+1+narg)))  13705
		endif !if(j_otype(ivsel).ne.j_iplist)  13698
		if(ivsimu2.ne.j_ivsimu)then
			call getsim()
		endif !if(ivsimu2.ne.j_ivsimu)  13711
		return
...
subroutine simulator(iob,io) !simulator
	...
	end if !if(ivperiods.le.0)  13737
	notfound=0
	ivkeepper=j_igetopt(iob,io,j_mkeepperiod)
	if(ivkeepper.gt.0)then
		j_keepper=j_v(ivkeepper)
	...
	!
	call j_getoption(iob,io,j_msource,1,1,j_iptrans,.true.,noptarg,j_optarg0)
	if(j_err)return
	...
	if(j_linkoption(iob,io,j_mtreevars).gt.0)then
		...
		ntreevars=0
		ivtreevars=0
	end if !if(j_linkoption(iob,io,j_mtreevars).gt.0)  13780
	if(j_linkoption(iob,io,j_mplotvars).gt.0)then
		...
		nplotvars=0
		ivplotvars=0
	end if !if(j_linkoption(iob,io,j_mplotvars).gt.0)  13789
	matall=.false.
	if(j_linkoption(iob,io,j_mmatrix).gt.0)then
		...
		nm=0
	end if !if(j_linkoption(iob,io,j_mmatrix).gt.0)  13799
	if(j_linkoption(iob,io,j_mlocal).gt.0)then
		...
		do ii=1,nlocm
			...
			if(j_err) return
		enddo !ii=1,nlocm  13812
	else !if(j_linkoption(iob,io,j_mlocal).gt.0)then
		localin=>j_matrix0
		allocate(localout(0:0))
		localout=j_matrix0
	endif !if(j_linkoption(iob,io,j_mlocal).gt.0)  13806
	...
	elseif(ntreevars.gt.0.or.nplotvars.gt.0)then !if(nm+ntreevars+nplotvars.eq.0)then
		...
		end do !i=1,ntreevars  13834
		do i=1,nplotvars
			ii=ii+1
			matrix(ii)=j_o(iob)%i(j_linkoption(iob,io,j_mplotvars)+i)
		end do !i=1,nplotvars  13838
	endif !if(nm+ntreevars+nplotvars.eq.0)  13823
	...
	if(iip.gt.0)then
		if(nonext.and.j_iper.lt.j_keepper)then
			...
			endif !if(j_iper.gt.0)  13869
			goto 980
		endif !if(nonext.and.j_iper.lt.j_keepper)  13867
		if(iip.le.1)then
			write(6,*)'**syntax is node::period'
			goto 980
		end if !if(iip.le.1)  13874
		...
		if(nonext.and.j_iper.lt.j_keepper)then
			...
			if(j_iper.gt.0)then
				call j_printname('in node ',ivt,' ')
			endif !if(j_iper.gt.0)  13905
			goto 980
		endif !if(nonext.and.j_iper.lt.j_keepper)  13903
		if(.not.endsub)then
			last=j_o(ivtrans)%i(0)+1
			call j_putoi(ivtrans,last,jispendtrt)  !????????????
			j_o(ivtrans)%i(0)=last+1  !
		end if !if(.not.endsub)  13910
		do i=1,ngot
			do j=1,j_o(ivtrans)%i(nextloc(i)+1)
				...
				end if !if(istarts_(nextper(i)+1,itr).eq.0)  13918
			end do !j=1,j_o(ivtrans)%i(nextloc(i)+1)  13916
		end do !i=1,ngot  13915
		...
		if(j_linpr2.eq.0)then
			...
			!		ivinputl,ivoutputl,j_matrix0,localin,localout,.false.)
			j_linpr2=0
		endif !if(j_linpr2.eq.0)  13964
 
 
		if(j_err)then
			goto 980
		end if !if(j_err)  13976
		if(j_nextnode.gt.0)then
			if(j_iper.ge.j_nper.and.j_nextnodeb.eq.0)then
				...
				end do !i=1,j_o(ivtrans)%i(j_nextnode+1)  13984
				ngot=ngot+1
				call j_puti(nextloc,ngot,j_nextnode)
				call j_puti(nextper,ngot,j_iper-j_nextnodeb)
			end if !if(j_iper.ge.j_nper.and.j_nextnodeb.eq.0)  13980
		end if !if(j_nextnode.gt.0)  13979
	end if !if(iip.gt.0)  13866
	goto 1
	980 j_err=.true.
	call j_del(ivtrans)
		...
		if(j_keepper.lt.j_nper)then
			...
			if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))then !aa
				...
				if(j_iper.le.j_keepper)then
					io=j_ioret
					return
				end if !if(j_iper.le.j_keepper)  14043
				goto 500
				...
				iob=j_ivsimu    !!!!!!!!!!!!1
				return
			end if !if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))  14041
		else !if(j_keepper.lt.j_nper)then
			io=j_ioret
			return
		end if !if(j_keepper.lt.j_nper)  14039
	end if !if(j_iper.ge.j_nper)  14038
	if(j_nextnodes(j_iper).le.0)then   !nextnodes for period whee next is give
		if(j_iper.eq.j_keepper)then
			io=j_ioret;return
		end if !if(j_iper.eq.j_keepper)  14062
		5000  	continue
		iper1=j_iper-1
		if(iper1.le.0)then
		...
		if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))then
			...
			if(j_iper.le.1)then
				...
				goto 5000
			end if !if(j_iper.le.1)  14074
		end if !if(j_lastpoint(j_iper).ge.j_nextnodes(j_iper-1))  14072
	end if !if(j_nextnodes(j_iper).le.0)  14061
	j_iper=j_iper+1
...
subroutine draw(iob,io) ! draw()
	...
	if(j_o(j_gpiout)%i(5).eq.0)then
		...
		write(nu,*)' '
		write(nu,*)' '
	endif !if(j_o(j_gpiout)%i(5).eq.0)  14164
	...
	endif !if(j_gplexlabel.eq.0)  14176
	...
	end if !if(j_o(j_gpiout)%d(1).eq.j_inf.and.j_o(j_gpiout)%d(2).eq.j  14211
 
	if(j_gpix.le.0)then
		write(6,*)'x-> not given in draw';goto 99
	end if !if(j_gpix.le.0)  14215
	...
	end if !if(j_o(j_gpiout)%d(1).ge.j_o(j_gpiout)%d(2))  14219
	if(j_err)goto 99
...
subroutine drawline(iob,io)
	...
	if(j_gpset.gt.0)then
		if(j_o(j_gpiout)%i(4+j_gpset).eq.0)then
			...
			write(nu,*)' '
		endif !if(j_o(j_gpiout)%i(4+j_gpset).eq.0)  14361
	...
	if(j_gpnarg.eq.2)then
		if(j_otype(j_gparg(1)).eq.j_ipreal.and.j_otype(j_gparg(2)).eq.j_ipreal)then
			if(j_gplelabel.eq.0)then
				...
				j_err=.true.;return
			endif !if(j_gplelabel.eq.0)  14399
			onlylabel=.true.
		endif !if(j_otype(j_gparg(1)).eq.j_ipreal.and.j_otype(j_gparg(2))  14398
	endif !if(j_gpnarg.eq.2)  14397
	...
	if(.not.onlylabel.and.makeplot)then
		...
		endif !if(j_gpstyle.gt.0.and.j_gpmark.ne.j_ivzero)  14434
	...
	endif !if(.not.onlylabel.and.makeplot)  14414
	if(exis)return
	!	iarg=j_gparg(1)
	...
	if(.not.onlylabel)then
		if(j_gpset.eq.0)then
			do ip=1,npoints
				...
				endif !if(isbreak.and.ip.gt.1)  14491
			...
			do ip=1,npoints
				...
				endif !if(isbreak.and.ip.gt.1)  14504
			...
			enddo !ip=1,npoints  14502
			call j_closeunit(nu)  ! nu is unit nuf is the char constant for the name
 
		endif !if(j_gpset.eq.0)  14488
 
	endif !if(.not.onlylabel)  14487
	...
	endif !if(j_gplelabel.gt.0)  14520
	deallocate(j_gpval)
	!if(j_gpshow)call j_showfig(j_gpiout)
	!	io=j_gpnewio
	return
...
subroutine drawline3(iob,io)
	...
	call j_getoption_index(iob,io,j_mmark,-1,1,j_ipchar,.true.,noptarg,j_optarg0)
	if(noptarg.le.0)then
		ismark=.false.
	else !if(noptarg.le.0)then
		call j_getchar(j_optarg0(1),mark,lemark)
		ismark=.true.
	endif !if(noptarg.le.0)  14549
	...
	endif !if(j_otype(iout).ne.j_ipfigure)  14560
	...
	if(append)then
		...
		if(.not.islabel)then
			...
			ili=3
		endif !if(.not.islabel)  14595
	endif !if(append)  14575
	if(narg.eq.1)then
		if(j_otype(arg(1)).ne.j_ipmatrix)then
			write(6,*)'only argument must be matrix'
			j_err=.true.;return
		endif !if(j_otype(arg(1)).ne.j_ipmatrix)  14606
		if(j_o(arg(1))%i(1).ne.2)then
			write(6,*)'argument matrix must have two rows'
			j_err=.true.;return
		endif !if(j_o(arg(1))%i(1).ne.2)  14610
	...
	elseif(narg.eq.2)then !if(narg.eq.1)then
		...
		endif !if(j_otype(arg(1)).eq.j_ipmatrix.and.j_otype(arg(2)).eq.j_  14632
	endif !if(narg.eq.1)  14605
	if(show)then
		...
		enddo !ili=1,j_o(iout)%i(1)  14665
		!	write(nu,'(a)')'e'
		write(nu,'(a)')'replot'
		write(nu,'(a)')'unset title'
...
78		write(6,1000)'<ret>'
		...
		if(ch.ne.' ')then
			...
			if(ch(1:lec).eq.'e')then
				...
				j_err=.true.;return
			endif !if(ch(1:lec).eq.'e')  14683
 
			call j_command(ch(1:lec))
			if(j_stop)then
				call j_stopj()
				return
			endif !if(j_stop)  14689
			if(j_err)then
				j_err=.false.
				write(6,*)'try again, <ret> to continue e => ;return'
 
			endif !if(j_err)  14693
			goto 78
		endif !if(ch.ne.' ')  14680
	endif !if(show)  14661
...
subroutine drawclass(iob,io) ! draw()
	...
	call j_getoption(iob,io,j_mse,-1,0,0,.false.,noptarg,j_optarg0)
	isse=noptarg.ge.0
	call j_getoption(iob,io,j_msd,-1,0,0,.false.,noptarg,j_optarg0)
	issd=noptarg.ge.0
	call j_getoption(iob,io,j_mvar,-1,0,0,.false.,noptarg,j_optarg0)
	isvar=noptarg.ge.0
	call j_getoption(iob,io,j_mfreq,-1,0,0,.false.,noptarg,j_optarg0)
	freq=noptarg.ge.0
 
	call j_getoption(iob,io,j_mhistogram,-1,0,0,.false.,noptarg,j_optarg0)
	histo=noptarg.ge.0
 
	call j_getoption(iob,io,j_marea,-1,0,0,.false.,noptarg,j_optarg0)
	area=noptarg.ge.0
	...
	if(j_gplexlabel.eq.0)then
		...
		call j_replacefig(2,'set xlabel "'//j_gpxlabel(1:j_gplexlabel)//'"')
	endif !if(j_gplexlabel.eq.0)  14796
	...
	endif !if(j_gpleylabel.eq.0)  14806
	...
	if(histo)then
		...
		end do !ip=1,ncol-1  14933
	end if !if(histo)  14837
	!write(6,*)'377373'
	call j_putfig('e')
...
subroutine plotyx(iob,io)   !plotyx()
	...
	if(isfunc)then
		...
		if(j_gpnarg.ne.2)then
			...
			j_err=.true.;return
		endif !if(j_gpnarg.ne.2)  15023
		!		j_fig_ixx=j_o(iob)%i(io+3)
		!		ixy=j_o(iob)%i(io+2)
	endif !if(isfunc)  15015
	...
	if(j_gplexlabel.eq.0)then
		...
		!else
		!	call j_replacefig(2,'set xlabel "'//j_gpxlabel(1:j_gplexlabel)//'"')
	endif !if(j_gplexlabel.eq.0)  15038
	iy=j_gparg(1)
	if(j_gpleylabel.eq.0)then
		call j_getline(j_ivnames,iy,j_varname1,le)
		call j_replacefig(3,'set ylabel "'//j_varname1(1:le)//'"')
		...
		call j_replacefig(3,'set ylabel "'//j_gpylabel(1:j_gpleylabel)//'"')
	endif !if(j_gpleylabel.eq.0)  15046
	...
	end do !i=j_dfrom,j_duntil  15090
	call j_closeunit(nu)
	!	end do !do k=1,jndatasetss
	!	j_fig_np=ntot
	j_v(j_ivaccepted)=ntot
...
subroutine nobs(iob,io) !number of observations in a data-object
	...
	end if !if(j_otype(iv).eq.j_ipregr)  15138
	return
...
subroutine linkdata(iob,io) !linkdata()
	...
	if(j_nargopt(iob,io,j_mdata).le.0)then
		...
		if(j_otype(ivup).ne.j_ipdata)then
			...
			goto 90
		end if !if(j_otype(ivup).ne.j_ipdata)  15194
		ivkeep=j_o(ivup)%i(2)
		ivobsup=j_o(ivup)%i(6)
	end if !if(j_nargopt(iob,io,j_mdata).le.0)  15188
	if(j_nargopt(iob,io,j_msubdata).le.0)then
		...
		if(j_otype(ivsub).ne.j_ipdata)then
			...
			goto 90
		end if !if(j_otype(ivsub).ne.j_ipdata)  15207
	end if !if(j_nargopt(iob,io,j_msubdata).le.0)  15201
	if(j_nargopt(iob,io,j_mclasslink).gt.0)then
		j_err=.true.
		write(6,*)'**linkdata: classlink does not work yet'
		goto 90
	end if !if(j_nargopt(iob,io,j_mclasslink).gt.0)  15212
	if(j_nargopt(iob,io,j_mnobsw).le.0)then
		...
		if(iilink.le.0)then
			...
			if(nobsup*nobw.ne.nobssub)then
				...
				j_err=.TRUE.
				RETURN
			endif !if(nobsup*nobw.ne.nobssub)  15229
...
subroutine index(iob,io)  ! index(data_set,variable)
	...
	if(inde.le.0)then
		if(j_linkoption(iob,io,j_many).le.0)then
			...
			j_err=.true.
		end if !if(j_linkoption(iob,io,j_many).le.0)  15292
	end if !if(inde.le.0)  15291
	j_v(iout)=inde
	...
	if(j_linkoption(iob,io,j_many).gt.0)then
		do j=1,j_o(ivlist)%i(3)
			if(valu.lt.j_o(ivlist)%d(j))then
				exit
			endif !if(valu.lt.j_o(ivlist)%d(j))  15303
		enddo !j=1,j_o(ivlist)%i(3)  15302
		j_v(iout)=j-1
		...
		do j=1,j_o(ivlist)%i(3)
			if(valu.eq.j_o(ivlist)%d(j))then
				j_v(iout)=j
				exit
			endif !if(valu.eq.j_o(ivlist)%d(j))  15310
		enddo !j=1,j_o(ivlist)%i(3)  15309
		call j_printname('**index: matrix ',ivlist,' does not contain value:')
		write(6,*)'value'
		j_err=.true.
	endif !if(j_linkoption(iob,io,j_many).gt.0)  15301
...
subroutine askc(iob,io) ! %%io
	...
	if(ivexit.gt.0)then
		if(j_otype(ivexit).ne.j_ipchar)then
			...
			j_err=.true.
			return
		endif !if(j_otype(ivexit).ne.j_ipchar)  15371
		call j_getchar(ivexit,exitc,lex)
	endif !if(ivexit.gt.0)  15370
	...
	do i=1,max(narg,1)
		if(narg.le.0)then
			...
			if(j_otype(iv).eq.j_ipchar)then
				...
				endif !if(j_o(iv)%i(3).eq.0)  15385
 
			endif !if(j_otype(iv).eq.j_ipchar)  15384
		end if !if(narg.le.0)  15380
		...
		end if !if(j_linkoption(iob,io,j_mq).le.0.or.j_ipc(j_o(iob)%i(j_li  15394
		lef=0
		if(lindef.gt.0)then
			...
			end if !if(j_otype(ide).eq.j_ipchar)  15404
		end if !if(lindef.gt.0)  15402
		if(lef.gt.0)then
			if(ivexit.gt.0)then
		...
		if(line_(2:).eq.' '.or.nc.le.0)then
			...
			end if !if(line_(2:2).eq."'")  15431
			if(ivexit.gt.0)then
				if(line_(nc1+1:nc-1).eq.exitc(1:lex))then
					write(6,*)'**exit from askc due to exit->'
					j_err=.true.
					return
				endif !if(line_(nc1+1:nc-1).eq.exitc(1:lex))  15438
			endif !if(ivexit.gt.0)  15437
			call j_defchar(0,line_(nc1:nc),ivchar)
			if(j_err) return
		end if !if(line_(2:).eq.' '.or.nc.le.0)  15424
		call j_asschar2(ivchar,iv)
	end do !i=1,max(narg,1)  15379
	900 if(j_err)return
	call j_clearoption(iob,io)  ! subroutine
	return
...
subroutine ask(iob,io)  !ask()
	...
	!	io_=io_+narg+3
	!j_getoption_index(iob,moption,minarg,maxarg,iptype,expand,needsarg,noptarg,optarg)
	call j_getoption(iob,io,j_mdefault,-1,narg,j_ipreal,.true.,noptarg,j_optarg0)
	if(noptarg.gt.0.and.noptarg.ne.narg)then
		write(6,*)'default-> should have as many arguments as the functtion has'
		j_err=.true. ;return
 
	endif !if(noptarg.gt.0.and.noptarg.ne.narg)  15495
	...
	if(nchi2.ge.0)then
		...
		if(narg.lt.2)then
			...
			if(idf.le.0)then
				...
				j_err=.true.
				return
			endif !if(idf.le.0)  15615
			if(arg.lt.0)then
				write(6,*)'*cdf(arg,df,chi2->): arg is negative',arg
				j_err=.true.
				return
 
			endif !if(arg.lt.0)  15620
			arg2=idf
		endif !if(narg.lt.2)  15609
...
subroutine close(iob,io)  ! close()
	...
	do i=1,narg
		...
		if(nu.le.0)then
			...
			! j_err=.true.
			return
		endif !if(nu.le.0)  15667
		!		write(6,*)'<9'
		call j_closeunit(nu)
		if(j_err)return
	end do !i=1,narg  15662
	return
...
subroutine read(iob,io)   !read()   %%io
	...
	call j_getoption(iob,io,j_meof,-1,1,j_ipreal,.true.,neof,eof)
	call j_clearoption(iob,io)  ! subroutine
	if(j_err)return
	if(eof(1).gt.0)then
		j_v(eof(1))=0.
	endif !if(eof(1).gt.0)  15789
	if(nu.le.0.and.nu.ne.5)then
		...
		if(j_unitchar(nu).gt.0)then
			...
			j_err=.true.
			return
		endif !if(j_unitchar(nu).gt.0)  15805
		if(ivform.ne.j_ivdollar)call j_getchar2(ivform,j_form_,lef)
		if(j_err)return
	end if !if(nu.le.0.and.nu.ne.5)  15792
	...
	elseif(j_form_(1:1).eq.'B')then !if(j_form_(1:1).eq.'b')then
		...
		if(j_n_dvector.lt.nval)then
			...
			j_n_dvector=nval
		endif !if(j_n_dvector.lt.nval)  15854
		!j_v(j_o(iob)%i(io+1+j)),j=3,narg)
		...
		endif !if(j_form_(lef:lef).eq.'n')  15865
		...
		elseif(ivform.eq.j_ivdollar)then !if(j_form_(lef:lef).eq.'n')then
			...
			endif !if(j_otype(ivarg3).eq.j_ipmatrix)  15879
		else !if(j_form_(lef:lef).eq.'n')then
			write(6,*)'*read: illegal format ',j_form_(1:lef)
			if(j_ninc.eq.1)write(6,*)' file remains open'
			j_err=.true.
			return
		endif !if(j_form_(lef:lef).eq.'n')  15873
	end if !if(j_form_(1:1).eq.'b')  15813
	900	continue
	return
...
subroutine write(iob,io)   !write( ) %%io
	...
	call j_getoption(iob,io,j_mrows,-1,2,j_ipreal,.true.,nargval,arg)
	nrow1=0
	nrow2=0
	...
	elseif(nargval.eq.2)then !if(nargval.eq.1)then
		...
		if(nrow1.lt.1.or.nrow2.lt.nrow1)then
			...
			j_err=.true.
			return
		endif !if(nrow1.lt.1.or.nrow2.lt.nrow1)  16075
	endif !if(nargval.eq.1)  16070
	...
	if(j_otype(ivform).eq.j_ipchar)then
		...
		! bin8=ff.eq.'B'
		chr=ff.eq.'c'
	endif !if(j_otype(ivform).eq.j_ipchar)  16089
	...
	if(ivfile.eq.j_ivdollar)then
		...
		if(nu.le.0)then
			...
			endif !if(j_otype(ivform).eq.j_iptext)  16103
			if(j_err)return
			!nu=j_iounit(ivfile) !j_o(ivfile)%i(4)
			...
			if(j_unitchar(nu).lt.0)then
				...
				j_err=.true.
				return
			endif !if(j_unitchar(nu).lt.0)  16118
		end if !if(nu.le.0)  16101
	...
	end if !if(ivfile.eq.j_ivdollar)  16096
 
	if(ivform.eq.j_ivdollar)then
		if(narg2.eq.0)then
			write(nu,*)' '
			return
		endif !if(narg2.eq.0)  16132
		if(j_writevar(nu,narg2,arg2))return
	endif !if(ivform.eq.j_ivdollar)  16131
...
800	if(ivform.eq.j_ivbuffer)then
		if(j_v(j_ivbuffer).eq.0.)then
				...
				do i=1,lenbu
					if(j_buffer(i:i).ne.' ')then
						...
						if(j0.gt.0.and.j_buffer(i1:i1).eq.' ')then
							...
							line_(j0:j0)=char(9)
						endif !if(j0.gt.0.and.j_buffer(i1:i1).eq.' ')  16151
 
						j0=j0+1
						line_(j0:j0)=j_buffer(i:i)
					endif !if(j_buffer(i:i).ne.' ')  16149
				enddo !i=1,lenbu  16148
				if(j0.gt.0)then
					if(line_(j0:j0).eq.'.')j0=j0-1
				endif !if(j0.gt.0)  16160
			...
			endif !if(tabform)  16145
		end if !if(j_v(j_ivbuffer).eq.0.)  16141
		return
	elseif(j_otype(ivform).eq.j_iptext.or.ivform.eq.j_ivdollar)then !if(ivform.eq.j_ivbuffer)then
		...
		if(narg.eq.3.and.ivfoem.eq.j_ivdollar)then
			...
			ite=ivform
		endif !if(narg.eq.3.and.ivfoem.eq.j_ivdollar)  16176
 
		call j_writetext(nu,ite,0,.true.)
		return
	end if !800	if(ivform.eq.j_ivbuffer)  16140
	if(narg.lt.3)then
		if(ivform.ne.j_ivdollar.and.ivfile.ne.j_ivbuffer.and.j_otype(ivform).eq.j_ipchar)then
			...
			goto 990
		endif !if(ivform.ne.j_ivdollar.and.ivfile.ne.j_ivbuffer.and.j_oty  16186
	endif !if(narg.lt.3)  16185
	...
	if(chr)then
		...
		do ii=4,narg+1
			...
			if(j_otype(iv).ne.j_ipchar)then
				...
				endif !if(le.gt.0)  16208
				le2=le2+le
			endif !if(j_otype(iv).ne.j_ipchar)  16203
		enddo !ii=4,narg+1  16200
	...
	if(narg2.eq.1.and.j_otype(irg).eq.j_ipmatrix)then
		...
		if(ivform.eq.j_ivdollar)then
			if(ncols.eq.1)then
				...
				enddo !ir=nrow1c,nrow2c  16246
				!endif !if(single)then
			endif !if(ncols.eq.1)  16238
			return
		elseif(bin)then !if(ivform.eq.j_ivdollar)then
			...
			if(ncols.eq.1)then
				...
				enddo !ir=nrow1c,nrow2c  16257
			endif !if(ncols.eq.1)  16254
			!endif !if(single)then
			return
		elseif(bin8)then !if(ivform.eq.j_ivdollar)then
			if(ncols.eq.1)then
				...
				do ir=nrow1c,nrow2c
					write(nu,err=990)j_o(irg)%d((ir-1)*ncols+1:ir*ncols)
				enddo !ir=nrow1c,nrow2c  16267
				return
			endif !if(ncols.eq.1)  16264
		...
		elseif(j_otype(ivform).eq.j_ipchar)then !if(ivform.eq.j_ivdollar)then
			...
			if(ncols.eq.1)then
				...
				enddo !ir=nrow1c,nrow2c  16289
			endif !if(ncols.eq.1)  16286
			return
			!endif !if(single)then
		endif !if(ivform.eq.j_ivdollar)  16237
		ismat=.true.
	else !if(narg2.eq.1.and.j_otype(irg).eq.j_ipmatrix)then
		ismat=.false.
		nval=narg-2
	endif !if(narg2.eq.1.and.j_otype(irg).eq.j_ipmatrix)  16223
	...
	if(bin)then
		if(j_n_vector.lt.nval)then
			...
			allocate(j_vector(1:nval))
			j_n_vector=nval
		endif !if(j_n_vector.lt.nval)  16350
	...
	if(nval.gt.0)then !if(ismat)then
		if(j_n_dvector.lt.nval)then
			...
			allocate(j_dvector(1:nval))
			j_n_dvector=nval
		endif !if(j_n_dvector.lt.nval)  16365
		!		write(6,*)'>3>',j_v( j_o(iob)%i(io+4:io+3+nval) )
		j_dvector(1:nval)=j_v( j_o(iob)%i(io+4:io+3+nval) )
	endif !if(nval.gt.0)  16364
	...
	if(bin8.and.nval.gt.0)then
		...
		write(nu,err=990)j_dvector(1:nval)
		return
	endif !if(bin8.and.nval.gt.0)  16375
	...
	if(ivform.eq.j_ivdollar)then !if(bin)then
		...
		end if !if(ivfile.ne.j_ivbuffer)  16382
		...
		if(j_form_(1:1).eq.'(')then
					...
					do i=1,lenbu
						if(line_(i:i).ne.' ')then
							...
							if(j0.gt.0.and.line_(i1:i1).eq.' ')then
								if(line_(j0:j0).ne.'.')j0=j0+1
								line_(j0:j0)=char(9)
							endif !if(j0.gt.0.and.line_(i1:i1).eq.' ')  16402
							j0=j0+1
							line_(j0:j0)=line_(i:i)
						endif !if(line_(i:i).ne.' ')  16400
					enddo !i=1,lenbu  16399
					if(j0.gt.0)then
						if(line_(j0:j0).eq.'.')j0=j0-1
					endif !if(j0.gt.0)  16410
					!	if(nu.ne.6)call j_toutf8(line_(1:max(j0,1)))
					write(6,*)'>12' ,line_(1:max(j0,1))
					write(nu,'(a)',err=990)line_(1:max(j0,1))
				else !if(tabform)then
			...
			do j=3,narg,2
				...
				end if !if(abs(itab).gt.80)  16438
				iv=j_o(iob)%i(io+2+j)
				if(j_otype(iv).eq.j_ipreal)then
					...
					end if !if(wform.and.nint(100.*tab).ne.100*itab)  16444
					...
					j_err=.true.;return
				end if !if(j_otype(iv).eq.j_ipreal)  16442
			...
			if(ivfile.ne.j_ivbuffer)then
					...
					do i=1,lel
						if(line_(i:i).ne.' ')then
							...
							if(j0.gt.0.and.line_(i1:i1).eq.' ')then
								if(line_(j0:j0).ne.'.')j0=j0+1
								line_(j0:j0)=char(9)
							endif !if(j0.gt.0.and.line_(i1:i1).eq.' ')  16484
							j0=j0+1
							line_(j0:j0)=line_(i:i)
						endif !if(line_(i:i).ne.' ')  16482
					enddo !i=1,lel  16481
					if(j0.gt.0)then
						if(line_(j0:j0).eq.'.')j0=j0-1
					endif !if(j0.gt.0)  16492
					write(nu,'(a)',err=990)line_(1:max(j0,1))
				else !if(tabform)then
					write(nu,'(a)',err=990)line_(1:lel)
				endif !if(tabform)  16478
			else !if(ivfile.ne.j_ivbuffer)then
				if(wform)then
					leb=j_v(j_ivbuffer)
				...
				endif !if(wform)  16500
			end if !if(ivfile.ne.j_ivbuffer)  16477
		else !if(j_form_(1:1).eq.'(')then
			write(6,*)'**illegal format in write';j_err=.true.
		end if !if(j_form_(1:1).eq.'(')  16393
	end if !if(ivform.eq.j_ivdollar)  16381
	900 continue !   if(j_err)return
...
subroutine elementprod(iob,io)
	...
	do i=1,2
		...
		if(j_otype( irg(i)).eq.j_ipmatrix)then
			...
			if(j_otype( irg(i)).eq.j_ipreal)then
				...
				j_err=.true.
				return
			endif !if(j_otype( irg(i)).eq.j_ipreal)  16541
		end if !if(j_otype( irg(i)).eq.j_ipmatrix)  16538
	end do !i=1,2  16536
	...
	if(ndim1(1).eq.0.and.ndim1(2).eq.0)then
		...
		j_err=.true.
	end if !if(ndim1(1).eq.0.and.ndim1(2).eq.0)  16553
...
subroutine t(iob,io) ! t(mat)  transpose  %%matrix
	...
	if(j_otype(irg).ne.j_ipmatrix)then
		...
		endif !if(j_otype(irg).eq.j_ipreal)  16593
		return
	end if !if(j_otype(irg).ne.j_ipmatrix)  16592
	...
	if(irg.eq.iout)then
		...
		do i=1,ndim1
			...
			enddo !j=1,ndim2  16613
		enddo !i=1,ndim1  16612
		j_o(iout)%i(1)=ndim2
		j_o(iout)%i(2)=ndim1
...
subroutine inverse(iob,io)  !inverse(mat)
	...
	if(j_otype(irg).ne.j_ipmatrix)then
		if(j_otype(irg).eq.j_ipreal)then
			...
			write(6,*)'#type is:',j_objecttypes( j_otype(irg))
			j_err=.true. ;return
		end if !if(j_otype(irg).eq.j_ipreal)  16649
	end if !if(j_otype(irg).ne.j_ipmatrix)  16648
	ndim1=j_o(irg)%i(1)
	ndim2=j_o(irg)%i(2)
	if(ndim1.ne.ndim2)then
		call j_printname('**argument ',irg,' of inverse is  not a square matrix')
		j_err=.true. ; return
	end if !if(ndim1.ne.ndim2)  16661
	if(j_o(irg)%i(4).eq.j_matdiag)then
		...
		do i=1,ndim1
			if(j_o(irg)%d(ibas+i).eq.j_0)then
				...
				j_err=.true.;return
			endif !if(j_o(irg)%d(ibas+i).eq.j_0)  16669
			j_o(iout)%d(ibas+i)=j_1/j_o(irg)%d(ibas+i)
			ibas=ibas+ndim1
		enddo !i=1,ndim1  16668
		return
	endif !if(j_o(irg)%i(4).eq.j_matdiag)  16665
	...
	do i=1,ndim1
		...
		enddo !j=1,ndim1  16682
		rhs_(i,i)=1.d0
	enddo !i=1,ndim1  16681
	...
	do i=1,ndim1
		...
		enddo !j=1,ndim1  16698
	enddo !i=1,ndim1  16697
...
subroutine qr(iob,io)
	...
	if(j_otype(irg).ne.j_ipmatrix)then
		...
		write(6,*)'#type is:',j_objecttypes( j_otype(irg))
		j_err=.true. ;return
	end if !if(j_otype(irg).ne.j_ipmatrix)  16739
	ndim1=j_o(irg)%i(1);ndim2=j_o(irg)%i(2)
	...
	do i=1,ndim1
		...
		enddo !j=1,ndim2  16749
	enddo !i=1,ndim1  16748
...
subroutine merge(iob,io)  ! merge()
	...
	do i=1,narg
		...
		if(j_otype(lis).eq.j_iplist)then
			...
			nn=nn+1
		end if !if(j_otype(lis).eq.j_iplist)  16817
	end do !i=1,narg  16815
	allocate(temp(0:nn))
	nn=0
	do i=1,narg
		...
		if(j_otype(lis).eq.j_iplist)then
			do j=1,j_o(lis)%i(1)
				...
				end if !if(.not.(any(j_o(lis)%i2(j).eq.temp(1:nn))))  16829
			end do !j=1,j_o(lis)%i(1)  16828
		else !if(j_otype(lis).eq.j_iplist)then
			if(.not.(any(lis.eq.temp(1:nn))))then
				nn=nn+1;temp(nn)=lis
			end if !if(.not.(any(lis.eq.temp(1:nn))))  16834
		end if !if(j_otype(lis).eq.j_iplist)  16827
	end do !i=1,narg  16825
	temp(0)=nn
...
subroutine print(iob,io)  !print-function  %%io  print()
	...
	! Arguments:
	! arg1,…,argn
	! arguments can be any J objects or values of arithmetic or logical expressions or
	...
	if(narg.eq.1)then
		if(j_otype(arg(1)).eq.j_iplist)then
			...
			endif !if(j_linkoption(iob,io,j_mexpand,clear=.true.).ge.0)  17024
 
		endif !if(j_otype(arg(1)).eq.j_iplist)  17021
	endif !if(narg.eq.1)  17020
	...
	endif !if(j_linkoption(iob,io,j_moptions,clear=.true.).ge.0)  17034
 
 
	isappend=j_linkoption(iob,io,j_mappend).gt.0
	call j_getoption_index(iob,io,j_mform,-1,100,j_ipchar,.true.,noptargf,j_optarg0)
 
 
	if(noptargf>0.and.j_optarg0(1).ne.j_ivdollar)then  !format
		call j_getchar(j_optarg0(1),j_form_,lef)
	endif !if(noptargf>0.and.j_optarg0(1).ne.j_ivdollar)  17045
	call j_getoption_index(iob,io,j_mfile,-1,100,j_ipchar,.true.,nfile,file)
	!call j_getchar(ivform,j_form_,lef)
	...
	if(j_linkoption(iob,io,j_mfunc).gt.0)then   !print available functions
		...
		do i=1,j_nfunctions
			...
			if(icur.eq.3.or.i.eq.j_nfunctions_.or.i.eq.j_nfuncs1.or. &
				...
				j_cline=' '
				icur=0
			endif !if(icur.eq.3.or.i.eq.j_nfunctions_.or.i.eq.j_nfuncs1.o  17064
			select case(i)
		...
		enddo !i=1,j_nfunctions  17058
		call j_clearoption(iob,io)  ! subroutine
		goto 900
	endif !if(j_linkoption(iob,io,j_mfunc).gt.0)  17052
	...
	if(j_linkoption(iob,io,j_mmaxlines).gt.0)then
		...
		maxlines=100
	end if !if(j_linkoption(iob,io,j_mmaxlines).gt.0)  17120
...
500		read(nuu,'(a)',err=950,end=888)j_tempchar2
		...
		if(nlin.le.maxlines)then
			...
			endif !if(nfile.gt.0)  17146
		endif !if(nlin.le.maxlines)  17145
...
888		if(.not.isopen)then
			...
			backspace nuu
		endif !888		if(.not.isopen)  17154
		if(isany)then
			write(6,*)'file contains ',nlin, ' lines'
			j_v(j_ivaccepted)=nlin
		endif !if(isany)  17160
		return
	endif !if(narg.eq.1.and.j_otype(arg(1)).eq.j_ipchar)  17131
 
	if(.not.debug)then
		if(j_printvar(nu_,narg,arg))return
	endif !if(.not.debug)  17167
	...
	do j=1,narg  ! o(iob)%i(io+1)
		...
		end if !if(iv.le.j_namedv)  17190
		if(iv.le.j_nv)then
			iotype=j_otype(iv)
		else !if(iv.le.j_nv)then
			iotype=j_ipreal
		end if !if(iv.le.j_nv)  17197
		if(iotype.gt.j_ipreal)then  !vois muuttaa otypet
			if(iotype.eq.j_ipmatrix)then
				...
				do i=1,min(maxlines,j_o(iv)%i(1))
					if(noptargf.gt.0)then
						...
						endif !if(isrow)  17223
					endif !if(noptargf.gt.0)  17215
					iba=iba+j_o(iv)%i(2)
 
				end do !i=1,min(maxlines,j_o(iv)%i(1))  17214
				...
				do jj=1,min(j_o(iv)%i(1),maxlines)
					...
					do kk=j_o(iv)%i(3),min(j_o(iv)%i(2),79+j_o(iv)%i(3))
						...
						if(j_ibittest(iv,jj,kk).ne.0)then
							...
							j_cline(ic:ic)='0'
						end if !if(j_ibittest(iv,jj,kk).ne.0)  17300
					end do !kk=j_o(iv)%i(3),min(j_o(iv)%i(2),79+j_o(iv)%i(3))  17298
					write(nu_,'(a)')j_cline(1:ic)
				enddo !jj=1,min(j_o(iv)%i(1),maxlines)  17296
				...
				do iset=1,nset
					...
					do jj=1,j_o(iv)%i2(nset+iset)
						...
						do k=1,j_o(iv)%i2(iset)
							write(6,*)'<345,iset,jj,k ',iset,jj,k
							irow=irow+1
							...
							if(irow.eq.1)then
								...
								if(j_o(ivrhs2)%d(1).eq.j_0)then
									j_inp(1:5)=' max'
 
								else !if(j_o(ivrhs2)%d(1).eq.0.)then
									j_inp(1:5)=' min'
 
								end if !if(j_o(ivrhs2)%d(1).eq.j_0)  17372
								...
								if(j_o(ivrhs)%d(irow).eq.j_o(ivrhs2)%d(irow))then
									...
									if(j_o(ivrhs)%d(irow).ne.-huge(1.d0))then
										...
										le=le+9
									endif !if(j_o(ivrhs)%d(irow).ne.-huge(1.d0))  17391
									if(j_o(ivrhs2)%d(irow).ne.huge(1.d0))then
										j_inp(le+1:le+10)=' <'//j_chr8(j_o(ivrhs2)%d(irow))
										le=le+10
									endif !if(j_o(ivrhs2)%d(irow).ne.huge(1.d0))  17395
								end if !if(j_o(ivrhs)%d(irow).eq.j_o(ivrhs2)%d(irow))  17385
 
							endif !if(irow.eq.1)  17370
						...
						enddo !k=1,j_o(iv)%i2(iset)  17363
					enddo !jj=1,j_o(iv)%i2(nset+iset)  17357
				enddo !iset=1,nset  17355
			...
			end if !if(iotype.eq.j_ipmatrix)  17203
		else !if(iotype.gt.j_ipreal)then
			if(j_otype(iv).eq.j_ipreal)then
				call j_getname(iv)
				write(nu_,*)' '
				write(nu_,*)j_oname(1:j_loname),'=',j_v(arg(j))
			end if !if(j_otype(iv).eq.j_ipreal)  17443
		end if !if(iotype.gt.j_ipreal)  17202
	end do !j=1,narg  17171
...
subroutine asschar(iob,io) ! %%char assign character variable
	...
	endif !if(j_otype(iout).ne.j_ipchar)  17506
...
subroutine data(iob,io)  !data(
	...
	if(.not.isnobs(1))then
		...
		if(ivdefn.gt.0)then
			...
			nobsb=10000
		end if !if(ivdefn.gt.0)  17877
	endif !if(.not.isnobs(1))  17871
...
459		if(rfcodelink(id).gt.0.and.rfcodelink(id).gt.rfheadlink(id).and.rfcodelink(id).gt.rfreadlink(id))then
			if(iform(id).eq.iformd.or.iform(id).eq.iformb)then
				...
				j_err=.true.;return
			endif !if(iform(id).eq.iformd.or.iform(id).eq.iformb)  17959
			...
			if(j_err)then
				...
				return
			endif !if(j_err)  17969
			rfcodelink(id)=0
 
		endif !459		if(rfcodelink(id).gt.0.and.rfcodelink(id).gt.rfheadli  17958
		...
		if(rfheadlink(id).gt.0.and.rfheadlink(id).gt.rfcodelink(id).and. &
			...
			if(iform(id).eq.iformd.or.iform(id).eq.iformb)then
				...
				j_err=.true.;return
			endif !if(iform(id).eq.iformd.or.iform(id).eq.iformb)  17979
			read(nu(id),'(a)')j_tempchar2
			leco=len_trim(j_tempchar2)
			...
			rfheadlink(id)=0
			goto 459
		endif !if(rfheadlink(id).gt.0.and.rfheadlink(id).gt.rfcodelink(id  17977
		...
		endif !if(iform(id).eq.j_ivb.or.iform(id).eq.j_ivdi.or.iform(id).  17994
 
 
 
 
	enddo !id=1,nlev  17954
	...
	if(.not.sub.and.isnobs(1).and..not.ismaketrans(1).and..not.iskeep(1))then
		...
		elseif(iform(1).eq.j_ivb2)then
			...
			enddo !i=1,nobs(1)  18048
		else !update later
			goto 1952
 
 
		endif !if(iform(1).eq.j_ivb)  18031
	...
	957		write(6,*)'*error reading binary in-> file after reading ',i-1,' observations'
		goto 7000
	endif !if(.not.sub.and.isnobs(1).and..not.ismaketrans(1).and..not  18027
...
1952		maxnobsw=0
	!!!main reading loop in one file
		...
		if(iobc(1).gt.nobsb)then !store in chained list
			...
			endif !if(associated(tail%pnext))  18091
			if(.not.associated(tail%pnext))allocate(tail%pnext)
		...
		end if !if(iobc(1).gt.nobsb)  18089
		100 continue
		...
		if(sub)then
			...
			do ii=1,nsub           !nsub=v(ivnobsw)
				...
				if(iobc(2).gt.nobsb)then !store in chained list
					...
					endif !if(associated(tail2%pnext))  18146
					...
					nbuf(2)=nbuf(2)+1
					if(p)write(6,*)'>22iohere ',nbuf(1)
				end if !if(iobc(2).gt.nobsb)  18144
				!if(idisk2.eq.0)then
				...
				if(dupl)then
					do idu=1,ndup
						...
						!	write(idisk2,rec=iobs2)j_v(keepl2(1:nvar2))
						!	endif
					enddo !idu=1,ndup  18171
 
					nsub2=nsub2+ndup
				endif !if(dupl)  18170
 
			end do !ii=1,nsub  18120
...
7000		continue
	...
	if(.not.isnobs(1))then
		...
		do i=1,nbuf(1)
			...
			nullify(tail%pnext)
			tail=>ptr
		end do !i=1,nbuf(1)  18267
	endif !if(.not.isnobs(1))  18244
	...
	if(isnobs(1).and.nobs(1)>iobs(1))then
		...
		endif !if(iobs(1).eq.0)  18278
 
	endif !if(isnobs(1).and.nobs(1)>iobs(1))  18277
	...
	if(.not.j_err)then
		...
		if(j_dlastdata.eq.iout(1))then
			j_dlastdata=0
			j_dlastdata=0
		endif !if(j_dlastdata.eq.iout(1))  18303
	endif !if(.not.j_err)  18299
	!	j_o(j_ivlastdata)%i(1)=1;j_o(j_ivlastdata)%i2(1)=iout(1)
 
	if(isgaya)then
		write(6,*)'gayatime=',time
		call timegaya()
	end if !if(isgaya)  18310
	...
	subroutine initdata(id)
		...
		if(id.eq.1)then
			...
			if(nu(id).eq.0)then
				...
				endif !if(id.eq.1)  18393
				j_err=.true.;return
 
			endif !if(nu(id).eq.0)  18392
		...
		endif !if(iform(id).gt.j_predefined)  18411
		...
		elseif(j_nread(id).eq.1.and.j_d(id)%readv(1).eq.j_ivdollar)then
			...
			vff(id)=.false.
		endif !if(j_nread(id).eq.0)  18430
		...
		if(rfcodelink(id).gt.0)then
			if(j_o(iob)%i(rfcodelink(id)).gt.0)then
				...
				j_err=.true.;return
			endif !if(j_o(iob)%i(rfcodelink(id)).gt.0)  18456
		endif !if(rfcodelink(id).gt.0)  18455
		if(rfheadlink(id).gt.0)then
			if(j_o(iob)%i(rfheadlink(id)).gt.0)then
				write(6,*)'rfcode-> cannot have arguments'
				j_err=.true.;return
			endif !if(j_o(iob)%i(rfheadlink(id)).gt.0)  18462
		endif !if(rfheadlink(id).gt.0)  18461
		...
		if(isnobs(id))then
			...
			if(nobs(id).le.0)then
				...
				j_err=.true. ;return
			endif !if(nobs(id).le.0)  18480
		else !if(isnobs(id))then
			nobs(id)=10**9
		endif !if(isnobs(id))  18475
		...
		if(ismaketrans(id))then
			if(j_otype(j_optarg0(1)).ne.j_iptrans)then
				...
				j_err=.true.; return
			endif !if(j_otype(j_optarg0(1)).ne.j_iptrans)  18497
			ivmaketrans(id)=j_optarg0(1)
			ivoul=j_trans_output(j_optarg0(1))
 
			noul=j_o(ivoul)%i(1)
		endif !if(ismaketrans(id))  18496
		...
		if(rfreadlink(id).gt.0)then
			...
			rfreadlink(id)=0
			!			goto 459
		endif !if(rfreadlink(id).gt.0)  18509
		!	tassa
		...
		if(j_linkoption(iob,io,lkeep(id)).gt.0)then
			...
			nkeep(id)=j_o(ivkeep(id))%i(1)
			!	write(6,*)nkeep(id)
		end if !if(j_linkoption(iob,io,lkeep(id)).gt.0)  18547
	...
	333 format(a)   ! format(Q,a)
		...
		1000 format(a,$)
			...
			endif !if(nu(id).eq.5)  18636
		endif !if(isincl(id))  18606
		...
		endif !if(isgaya.and.id.eq.2)  18683
		...
		if(ismaketrans(id))then
			...
			if(j_err)goto 51
		end if !if(ismaketrans(id))  18693
		...
		if(isnobs(id)) then
			if(iobs(id)>nobs(id)) then
				...
				j_err=.true. ;goto 51
			endif !if(iobs(id)>nobs(id))  18731
			! if(idisk.gt.0)then
...
subroutine stat_(iob,io)  ! stat(iob,io)
	...
	if(missing)then
		...
		do iobs=j_dfrom,j_duntil
			...
			do j=1,narg
				...
				end if !if(abs(xh).lt.1.7e19)  18923
			end do !j=1,narg  18921
		end do !iobs=j_dfrom,j_duntil  18906
	else !missing
		do iobs=j_dfrom,j_duntil
			...
			!				end do !do j=1,narg
		end do !iobs=j_dfrom,j_duntil  18931
	endif !if(missing)  18904
	...
	if(j_dprint.ne.0)then
		...
		endif !if(lirmse.lt.0)  18973
	end if !if(j_dprint.ne.0)  18969
	rejo=.false.
	if(iout.ne.j_ivresult)then
		...
		!		if(j_err)return
	endif !if(iout.ne.j_ivresult)  18980
	...
	do j=1,narg
		...
		if(j_dprint.ne.0)then
			...
			endif !if(lirmse.lt.0)  19010
			if(j_err)then
				write(6,*)'*j* problem with variable ',iv
				return
			endif !if(j_err)  19015
			...
			endif !if(missing.and.weisu.lt.xws)  19019
		endif !if(j_dprint.ne.0)  19009
		if(iout.eq.j_ivresult)then
		...
		endif !if(iout.eq.j_ivresult)  19028
	end do !j=1,narg  18993
...
subroutine transdata(iob,io)
	...
	do iobs=j_dfrom,j_duntil
		call j_getobs(iobs)
		if(j_err)return
	end do !iobs=j_dfrom,j_duntil  19100
...
subroutine corr(iob,io,itype)  !stat()
	...
	endif !if(j_otype(ivi).eq.j_iplist)  19167
	iout=j_o(iob)%i(io+2+narg)
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	...
	!call j_getdataset(j_datasets(kk),nobs)
		...
		do i=1,narg
			...
			do 4 j=1,i
				...
				4	   				continue !4 j=1,i  19225
		enddo !i=1,narg  19222
	enddo obloop !oop:		do ii=j_dfrom,j_duntil  19203
		...
		do i=1,narg
			...
			do j=1,i-1
				if(xc(i).gt.0..and.xc(j).gt.0.)then
					...
					cor=0.
				endif !if(xc(i).gt.0..and.xc(j).gt.0.)  19261
				j_o(iout)%d((i-1)*narg+j)=cor
				j_o(iout)%d((j-1)*narg+i)= cor
			enddo !j=1,i-1  19260
		enddo !i=1,narg  19256
	endif !if(.not.iscorr)  19247
	if(j_dprint.ge.1)then
		...
		if(j_o(iob)%i(io) .eq.212)then  !not correlation
			...
			enddo !i=1,narg  19281
 
 
 
		endif !if(j_o(iob)%i(io) .eq.212)  19273
 
 
	endif !if(j_dprint.ge.1)  19271
...
subroutine matrix(iob,io) ! matrix()
	...
	if(nva.gt.0)then
		do iva=1,nva
			...
			endif !if(j_otype(j_optarg0(iva)).eq.j_ipmatrix)  19381
		enddo !iva=1,nva  19380
	endif !if(nva.gt.0)  19379
	narg=j_o(iob)%i(io+1)
	if(narg.eq.0)then
	...
	if(isinf.and.j_linkoption(iob,io,j_min).lt.0)then
		...
		j_err=.true.;goto 70
	endif !if(isinf.and.j_linkoption(iob,io,j_min).lt.0)  19407
	if(diag.and.matmat.and.narg.eq.0.and.nva.eq.1)then
		...
		do i=1,ndim
			...
			ibas=ibas+ndim
		enddo !i=1,ndim  19417
		return
 
	endif !if(diag.and.matmat.and.narg.eq.0.and.nva.eq.1)  19411
	...
	if(narg.eq.2)then
		...
		if(diag.and.ndim1.ne.ndim2)then
			...
			j_err=.true.;return
		endif !if(diag.and.ndim1.ne.ndim2)  19458
	endif !if(narg.eq.2)  19456
	isreal=ndim.eq.1.and.ndim2.eq.1
	isany=j_linkoption(iob,io,j_many).ge.0
	!	iarg2=j_o(iob)%i(io+3)
	call j_getoption(iob,io,j_mdo,-1,99999,j_ipreal,.false.,ndo,doval)
	...
	if(narg.eq.0)then
		...
		elseif(ndo.ge.0)then !if(nva.le.0.and.ndo.eq.0)then
			if(ndo.lt.2)then
				...
				j_err=.true. ;goto 70
			endif !if(ndo.lt.2)  19524
			dostep=1.d0
			if(ndo.eq.3)dostep=j_v(doval(3))
			if(dostep.eq.0)then
				write(6,*)'do-step is zero'
				j_err=.true. ; goto 70
			endif !if(dostep.eq.0)  19530
			...
			if(diag)then
				...
				enddo !i=1,nval  19556
			endif !if(diag)  19543
			goto 70
		endif !if(nva.le.0.and.ndo.eq.0)  19500
 
	endif !if(narg.eq.0)  19499
	...
	if(narg.eq.2)then
		if(j_otype(arg(1)).eq.j_iplist)then
			...
			if(inde.lt.1.or.inde.gt.j_o(arg(1))%i(1))then
				...
				j_err=.true.;goto 70
			endif !if(inde.lt.1.or.inde.gt.j_o(arg(1))%i(1))  19571
			irg=j_o(arg(1))%i2(inde)
			if(j_otype(irg).eq.j_ipreal)then
				if(j_otype(iout).ne.j_ipreal)call j_del(iout)
				j_v(iout)=j_v(irg)
				goto 70
			endif !if(j_otype(irg).eq.j_ipreal)  19576
			if(j_otype(irg).ne.j_ipmatrix)then
				call j_printname('*not a matrix:',irg,' ')
				j_err=.true. ;goto 70
			endif !if(j_otype(irg).ne.j_ipmatrix)  19581
			nel=ndim1*ndim2
			...
			!write(6,*)' '
			goto 70
		end if !if(j_otype(arg(1)).eq.j_iplist)  19569
	endif !if(narg.eq.2)  19568
	...
	if(.not.matmat.and..not.isinf)then
		...
		elseif(.not.diag.and.nva.gt.0.and.ndim1*ndim2.ne.nva.and..not.isany.and.nva.ne.1)then !if(diag.and.nva.gt.0.and.nva.ne.ndim1.and.nva.ne.1)then
			write(6,*)'nrows=',ndim1,' and ncols=',ndim2, 'and there are ',nva, &
				'values and no any->'
			j_err=.true.
		endif !if(diag.and.nva.gt.0.and.nva.ne.ndim1.and.nva.ne.1)  19600
		if(j_err)return
 
		call j_defmatrix(iout,' ',ndim1,ndim2,itype,iout)
 
	endif !if(.not.matmat.and..not.isinf)  19599
	...
	if(matmat)then
		...
		if(nva.ne.ndim1*ndim2.and..not.diag)then
			...
			j_err=.true. ;goto 70
		endif !if(nva.ne.ndim1*ndim2.and..not.diag)  19619
		!***********************
		if(diag)then
			nrows=0
			ivastep=1
			...
			do iro=1,ndim1
				...
				if(j_otype(ii).eq.j_ipreal)then
					...
					if(j_otype(ii).ne.j_ipmatrix)then
						...
						call printvalues()
						j_err=.true. ;goto 70
					endif !if(j_otype(ii).ne.j_ipmatrix)  19643
					niro=j_o(ii)%i(1)
					ncols=j_o(ii)%i(2)
					...
					endif !if(ncols.ne.niro)  19651
				endif !if(j_otype(ii).eq.j_ipreal)  19639
				nrows=nrows+niro
			enddo !iro=1,ndim1  19631
			...
			do iro=1,ndim1
				...
				if(j_otype(ii).eq.j_ipreal)then
					...
					do ir0=1,niro
						...
						enddo !ir=1,niro  19680
						!		write(6,*)'iro, ndim1,nrows,ibas',iro,ndim1,nrows,ibas
						ibas=ibas+nrows
					enddo !ir0=1,niro  19679
					ibas=ibas+niro
				endif !if(j_otype(ii).eq.j_ipreal)  19672
 
 
 
			enddo !iro=1,ndim1  19668
		...
		do iro=1,ndim1
			...
			if(j_otype(ii).eq.j_ipreal)then
				...
				if(j_otype(ii).ne.j_ipmatrix)then
					...
					call printvalues()
					j_err=.true. ;goto 70
				endif !if(j_otype(ii).ne.j_ipmatrix)  19709
				niro=j_o(ii)%i(1)
				ncols=j_o(ii)%i(2)
			endif !if(j_otype(ii).eq.j_ipreal)  19705
			nrows=nrows+niro
			do j2=2,ndim2
				...
				if(j_otype(ii).eq.j_ipreal)then
					...
					if(j_otype(ii).ne.j_ipmatrix)then
						...
						call printvalues()
						j_err=.true.;goto 70
					endif !if(j_otype(ii).ne.j_ipmatrix)  19726
					niro2=j_o(ii)%i(1)
					ncol=j_o(ii)%i(2)
				endif !if(j_otype(ii).eq.j_ipreal)  19722
				if(niro2.ne.niro)then
					...
					call printvalues()
					j_err=.true.;goto 70
				endif !if(niro2.ne.niro)  19735
				ncols=ncols+ncol
 
			enddo !j2=2,ndim2  19719
			if(iro.gt.1.and.ncols.ne.ncolsv)then
				write(6,*)'row ',iro,' does not have the same number of columns as previous '
				call printvalues()
				j_err=.true.;goto 70
			endif !if(iro.gt.1.and.ncols.ne.ncolsv)  19746
			ncolsv=ncols
 
		enddo !iro=1,ndim1  19702
		if(inout)then
			...
			do j2=1,ndim2
				...
				if(j_otype(ii).eq.j_ipreal)then
					...
					do iri=1,nrow0
						...
						enddo !jic=1,ncol0  19777
					enddo !iri=1,nrow0  19776
					ncols0=ncols0+ncol0
				endif !if(j_otype(ii).eq.j_ipreal)  19769
			enddo !j2=1,ndim2  19766
			nrows0=nrows0+nrow0
 
 
		enddo !iro=1,ndim1  19764
		if(inout)then
		...
		if(nva.eq.1)then  !constant value
			...
			if(diag)then
				...
				do i=1,min(ndim1,ndim2)
					j_o(iout)%d(iba)=val
					iba=iba+ndim2+1
				enddo !i=1,min(ndim1,ndim2)  19813
			else !if(diag)then
			...
			if(diag)then
				...
				do j=1,min(ndim1,ndim2,nva)
					...
					iba=iba+ndim2+1
				enddo !j=1,min(ndim1,ndim2,nva)  19825
			else !if(diag)then
		...
		if(j_incin)then
			...
			if(isinf)then
				...
				do while(j_o(ivi)%txt(lin+1)(1:1).ne.'/')
					...
					endif !if(lin.gt.j_o(ivi)%i(5))  19847
				enddo !while(j_o(ivi)%txt(lin+1)(1:1).ne.'/')  19845
				ndim1=lin-lin0
				!		write(6,*)'found ',ndim1,' rows'
				if(diag)ndim2=ndim1
 
			endif !if(isinf)  19842
			...
			if(diag)then
				...
				endif !if(j_o(ivi)%txt(lin+1)(1:1).ne.'/')  19867
...
1000 format(a,$)
		...
		if(ndo.eq.0)then !if(ndo.lt.0)then
			if(diag)then
				do i=1,ndim1
					j_o(iout)%d(ibas+i)=i
					ibas=ibas+ndim1
				enddo !i=1,ndim1  19924
 
			else !if(diag)then
				do i=1,ndim1*ndim2
					j_o(iout)%d(i)=i
				enddo !i=1,ndim1*ndim2  19930
		...
		elseif(ndo.eq.1)then !if(ndo.lt.0)then
			if(diag)then
				do i=1,ndim1
					...
					ibas=ibas+ndim1
				enddo !i=1,ndim1  19937
			else !if(diag)then
				do i=1,ndim1*ndim2
					j_o(iout)%d(i)=i+j_v(doval(1))-1
				enddo !i=1,ndim1*ndim2  19942
			endif !if(diag)  19936
		elseif(ndo.eq.2)then !if(ndo.lt.0)then
			...
			if(diag)then
				...
				do i=1,min(ndim1,iup)
					j_o(iout)%d(ibas+i)=i+j_v(doval(1))-1
					ibas=ibas+ndim1
				enddo !i=1,min(ndim1,iup)  19950
 
			else !if(diag)then
				do i=1,min(ndim1*ndim2,iup)
					j_o(iout)%d(i)=i+j_v(doval(1))-1
				enddo !i=1,min(ndim1*ndim2,iup)  19956
			endif !if(diag)  19948
		elseif(ndo.eq.3)then !if(ndo.lt.0)then
			...
			if(diag)then
				do i=1,ndim1
					...
					endif !if(j_v(doval(3)).lt.j_0)  19966
					ibas=ibas+ndim1
				enddo !i=1,ndim1  19963
				...
				do i=1,ndim1*ndim2
					...
					if(j_v(doval(3)).lt.j_0)then
						...
						if(docur.gt.j_v(doval(2))+1.0d-17)exit
					endif !if(j_v(doval(3)).lt.j_0)  19978
 
				enddo !i=1,ndim1*ndim2  19975
 
			endif !if(diag)  19962
 
		endif !if(ndo.eq.0)  19922
	end if !if(nva.ge.1)  19804
	...
	if(j_o(iout)%i(3).eq.1)then
		...
		call j_del(iout)
		j_v(iout)=docur
	endif !if(j_o(iout)%i(3).eq.1)  19995
	if(isreal)then
		docur=j_o(iout)%d(1)
		call j_del(iout)
		j_v(iout)=docur
	endif !if(isreal)  20000
	return
	...
	subroutine printvalues()
		...
		do j=1,nva
			...
			endif !if(j_otype(ii).eq.j_ipmatrix)  20024
		enddo !j=1,nva  20021
		return
...
subroutine trans(iob,io)  ! define transformation set   output=trans()
	...
	if(nin.gt.0)then
		...
		if(j_otype(ivtext).ne.j_iptext)then
			...
			j_err=.true.;return
		endif !if(j_otype(ivtext).ne.j_iptext)  20240
...
subroutine list(iob,io)  !%%list
	...
	if(j_linkoption(iob,io,j_mmask).ge.0)then
		...
		do i=1,narg2
			...
			if(nval.eq.0)nval=-1
			nel2=nel2+abs(nval)
		end do !i=1,narg2  20468
		if(nel.le.0)then
			write(6,*)'*wrn: all mask elements false, no list generated';goto 900
		end if !if(nel.le.0)  20474
		if(nel2.lt.narg)write(6,*)'*wrn* mask shorter, rest rejected'
		if(nel2.gt.narg)write(6,*)'*wrn* mask longer, tail ignored'
		...
		do i=1,narg2
			...
			if(nval.gt.0)then
				...
				end do !j=1,nval  20489
			else if(nval.eq.0)then !if(nval.gt.0)then
				nel2=nel2+1
			else !if(nval.gt.0)then
				nel2=nel2-nval
			end if !if(nval.gt.0)  20488
		end do !i=1,narg2  20486
		j_o(iv2)%i(1)=nel
		call j_clearoption(iob,io)  ! subroutine
		...
		if(narg.eq.1.and.issub)then
			...
			do i_=1,j_namedv
				...
				endif !if(j_tempchar3(1:j_loname+1).eq.j_oname(1:j_loname)//'%')  20528
			enddo !i_=1,j_namedv  20525
			j_o(iout)%i(1)=nfound
			j_o(iout)%i(3)=nfound
			return
		...
		elseif(narg.eq.1.and.j_otype(ivarg_).eq.j_ipchar)then
			...
			if(isq.eq.2.and.le2.eq.3) then  ! '?'  -> all named variables
				...
				!	j_o(iv2)%i(0) =j_namedv
				goto 900
			endif !if(isq.eq.2.and.le2.eq.3)  20542
			different=j_tempchar2(2:2).eq.'-'.and.le2.gt.3
			ledif=le2-3
			isq2=le2+1
			if(isq.gt.0)isq2=j_nextlim(j_tempchar2,isq+1,le2,'?')
			...
			if(isq.gt.0.or.different)then
				...
				17                 do i_=1,j_namedv
					...
					if(different.and.le3.ge.ledif)then  !'!x'
						if(j_tempchar2(3:le2-1).eq.j_tempchar3(1:ledif))cycle
						goto 11
					endif !if(different.and.le3.ge.ledif)  20559
					if(isq.eq.2.and.isq2.eq.le2-1)then    !   '?x?'
						if(index(j_tempchar3(1:le3),j_tempchar2(3:le2-2)).gt.0 )goto 11
						cycle
					endif !if(isq.eq.2.and.isq2.eq.le2-1)  20563
					if(isq.gt.2)then  !there is something before ?     'x?'  or 'x?y'   ~
						if(le3.lt.isq-2)cycle
						if(j_tempchar3(1:isq-2).ne.j_tempchar2(2:isq-1))cycle
 
					endif !if(isq.gt.2)  20567
					loppupit=le2-isq-1
					if(loppupit.gt.0)then  !there is something after ?  '?x'  or 'x?y'
						if(le3.lt.loppupit)cycle
						if(j_tempchar3(le3-loppupit+1:le3).ne.j_tempchar2(isq+1:le2-1))cycle
 
					endif !if(loppupit.gt.0)  20573
 
				11                  nfound=nfound+1
					if(ok)j_o(iv2)%i2(nfound)=i_
 
				enddo !                do i_=1,j_namedv  20556
				...
				nfound=0
				goto 17
			endif !if(isq.gt.0.or.different)  20554
		endif !if(narg.eq.1.and.issub)  20506
		!normal case
		if(j_err) return
		...
		!	do i=1,narg;j_o(iv2)%i(i)=j_o(iob)%i(io+1+i);end do
	end if !if(j_linkoption(iob,io,j_mmask).ge.0)  20464
...
subroutine stempolar(iob,io)  !
	...
	if(j_otype(ispl).ne.j_ipstemspline)then
		...
		j_err=.true.
		return
	end if !if(j_otype(ispl).ne.j_ipstemspline)  20659
	if(j_linkoption(iob,io,j_morigo).gt.0)then
		...
		endif !if(j_o(iob)%i(j_linkoption(iob,io,j_morigo)).ne.1)  20665
		origo=100.*j_v(j_o(iob)%i(j_linkoption(iob,io,j_morigo)+1) )
 
	else !if(j_linkoption(iob,io,j_morigo).gt.0)then
		origo=0.
	endif !if(j_linkoption(iob,io,j_morigo).gt.0)  20664
	...
	do 441 L=1,npo2-1
		...
		if(abs(j_o(ispl)%d(lir+L)-dref).lt.0.001)then
			j_v(iout)=dref
			goto 90
		endif !if(abs(j_o(ispl)%d(lir+L)-dref).lt.0.001)  20692
 
		if(j_o(ispl)%d(lir+L).le.dref)goto 221
 
		441       continue !441 L=1,npo2-1  20687
	...
	if(L.eq.1.and.j_o(ispl)%d(1).le.cotang*(j_o(ispl)%d(lirh)-origo))then
		...
		end if !if(r.gt.0.01.or.-r.gt.j_o(ispl)%d(lirh)-origo)  20757
		...
		end if !IF (R.LT.-0.01 .OR.r.gt.sero)  20769
	end if !if(L.eq.1.and.j_o(ispl)%d(1).le.cotang*(j_o(ispl)%d(lirh)-  20752
	...
	return
		...
		endif !if(j_otype(iverr).ne.j_iptrans)  20793
 
	endif !99 	   if(iverr.gt.0)  20792
	j_err=.true.
	return
...
subroutine valuex(iob,io)   !
	...
	if(arg.le.0.4)then
		...
		endif !if(j_err)  20866
	endif !if(arg.le.0.4)  20862
...
subroutine stemspline(iob,io)     !
	...
	if(narg.eq.2)then
		...
		if(j_otype(ivh_).ne.j_ipmatrix.or.j_otype(ivd_).ne.j_ipmatrix)then
			...
			goto 90
		endif !if(j_otype(ivh_).ne.j_ipmatrix.or.j_otype(ivd_).ne.j_ipmat  20918
		npo=j_o(ivh_)%i(1)
		if(j_o(ivd_)%i(1).ne.npo)then
			write(6,*)'*stemspline:arguments have different dimensions'
			goto 90
		endif !if(j_o(ivd_)%i(1).ne.npo)  20923
		vec=.true.
	else !if(narg.eq.2)then
		if(test)then
			npo=0
			...
			do i=1,npobas
				if(j_v(j_o(iob)%i(io+1+i)).le.0.and.j_v(j_o(iob)%i(io+1+i+npobas)).le.0.)then
					npo=i-1
					goto 756
				endif !if(j_v(j_o(iob)%i(io+1+i)).le.0.and.j_v(j_o(iob)%i(io+1+i+  20933
			enddo !i=1,npobas  20932
			npo=narg/2
			756		continue
	...
	if(sort)then
		...
		if(allocated(iperm))then
			...
			endif !if(size(iperm).lt.npo)  20965
		else !if(allocated(iperm))then
			allocate(iperm(1:npo+1))
		endif !if(allocated(iperm))  20964
		ii=1
		do i=1,npo
			if(vec)then
		...
		do i=2,npo2
			...
			endif !if(j_o(iout)%d(npo+i)-j_o(iout)%d(npo+ii-1).lt.delh)  20998
			ii=ii+1
 
		enddo !i=2,npo2  20997
		...
		do 31 i=1,npo
			...
			if( j_o(iout)%d(ii+npo).lt.0..or.j_o(iout)%d(ii).lt.0.)then
				goto 311
			end if !if( j_o(iout)%d(ii+npo).lt.0..or.j_o(iout)%d(ii).lt.0.)  21032
			if(ii.gt.1)then
				if(j_o(iout)%d(npo+ii).lt.j_o(iout)%d(npo+ii-1))then
					...
					goto 90
				endif !if(j_o(iout)%d(npo+ii).lt.j_o(iout)%d(npo+ii-1))  21036
				if(j_o(iout)%d(npo+ii)-j_o(iout)%d(npo+ii-1).lt.delh)then
					...
					endif !if(i.lt.npo)  21047
					ii=ii-1
				end if !if(j_o(iout)%d(npo+ii)-j_o(iout)%d(npo+ii-1).lt.delh)  21046
			end if !if(ii.gt.1)  21035
			ii=ii+1
 
			31     continue !31 i=1,npo  21022
	...
	endif !if(iprint.gt.2.or.p)  21068
	...
	700    do i=1,npo2-1
		...
		if(testma-dma.gt.dif0)then
			...
			f=0.7*dmi+0.3*dma
			isok=0
		endif !if(testma-dma.gt.dif0)  21103
		if(p)then
		...
		if(isok.le.0.and..false.)then
			...
			goto 90
		endif !if(isok.le.0.and..false.)  21116
 
	enddo !    do i=1,npo2-1  21080
	...
	if(sort)then
		if(allocated(iperm))then
			if(size(iperm).lt.npo)then
				deallocate(iperm)
				allocate(iperm(1:npo+1)) !some reserve
			endif !if(size(iperm).lt.npo)  21242
		else !if(allocated(iperm))then
			allocate(iperm(1:npo+1))
		endif !if(allocated(iperm))  21241
		...
		do i=1,npo-1
			if(tau(i).gt.tau(i+1))then
				...
				j_err=.true.
				return
			endif !if(tau(i).gt.tau(i+1))  21258
		enddo !i=1,npo-1  21257
	endif !if(sort)  21240
...
subroutine eigen(iob,io)
	...
	ELSE IF( ANRM.GT.RMAX ) THEN !IF( ANRM.GT.ZERO .AND. ANRM.LT.RMIN ) THEN
		ISCALE = 1
		SIGMA = RMAX / ANRM
	END IF !IF( ANRM.GT.ZERO .AND. ANRM.LT.RMIN )  21374
	IF( ISCALE.EQ.1 ) &
	...
	IF( ISCALE.EQ.1 ) THEN
		...
		ELSE !IF( INFO.EQ.0 ) THEN
			IMAX = INFO - 1
		END IF !IF( INFO.EQ.0 )  21404
		CALL DSCAL( IMAX, ONE / SIGMA, W, 1 )
	END IF !IF( ISCALE.EQ.1 )  21403
...
recursive subroutine call(iob,io)   !
	...
	endif !if(j_otype(ifunc).ne.j_iptrans)  21488
...
subroutine pause(iob,io)
	...
	endif !if(j_o(iob)%i(io+1).gt.0)  21523
...
subroutine text(iob,io)
	...
	end if !if(iout.eq.j_ivresult)  21610
	...
	1  write(6,1000)'text>'
		...
		if(j_inp(1:2).ne.'//')then
			...
			endif !if(clean)  21630
			goto 1
		end if !if(j_inp(1:2).ne.'//')  21629
...
195		if(j_o(iiv)%txt(irow+1)(1:2).ne.'//')then
			...
			endif !if(clean)  21642
			irow=irow+1
			goto 195
		end if !195		if(j_o(iiv)%txt(irow+1)(1:2).ne.'//')  21640
		j_o(iiv)%i(6)=irow+1
 
	endif !if(j_ninc.eq.1)  21622
...
subroutine txt(iob,io)   !
	...
	end if !if(iout.eq.j_ivresult)  21677
	if(j_linkoption(iob,io,j_min).eq.0)then
		write(6,*)'txt() requires in->'
		j_err=.true.;return
	endif !if(j_linkoption(iob,io,j_min).eq.0)  21680
	if(j_otype(iout).ne.j_ipreal)call j_del(iout)
	...
	if(j_incin)then
		...
		do while(j_o(nu)%txt(lin+1)(1:2).ne.'//')
			...
			endif !if(lin.ge.j_o(nu)%i(5))  21702
		enddo !while(j_o(nu)%txt(lin+1)(1:2).ne.'//')  21700
		ndim1=lin-lin0
		write(6,*)'found ',ndim1,' rows'
		...
		enddo !i=1,ndim1  21718
		j_o(nu)%i(6)=li+1
		j_otype(iout)=j_iptxt
		return
	endif !if(j_incin)  21692
	...
	if(allocated(j_temptxt))then
		...
		endif !if(size(j_temptxt).lt.nlin)  21733
	else !if(allocated(j_temptxt))then
		allocate(j_temptxt(1:nlin))
	endif !if(allocated(j_temptxt))  21732
...
subroutine crossed(iob,io)  !
	...
	do i=2,j_dnobs
		...
		if(class.ne.classv)then
			...
			endif !if(nclass.gt.mxclass)  21845
			classv=class
			inclass(nclass)=0
 
		endif !if(class.ne.classv)  21838
		inclass(nclass)=inclass(nclass)+1
 
	enddo !i=2,j_dnobs  21836
	if(sym)then
		nobc=nobc+inclass(nclass)*(inclass(nclass)+1)/2
	else !if(sym)then
		nobc=nobc+inclass(nclass)*inclass(nclass)
	...
	do i=j_dfrom,j_duntil
		...
		if(iclass.eq.inclass(ncl))then
			...
			iclass=0
			ncl=ncl+1
		endif !if(iclass.eq.inclass(ncl))  21905
 
	enddo !i=j_dfrom,j_duntil  21876
	!enddo !do k=1,jndatasetss
...
subroutine abs(iob,io) ! abs(   absolute value
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		j_err=.true.
		return
	endif !if(j_otype(irg).eq.j_ipreal)  21925
	...
	10		continue
			...
			if(ndim2.lt.0.and.n.gt.0)then  !need to c
				...
				if((ivcolmin.gt.0.or.ndim2.gt.0).and.ivalmin2.lt.icolmin)then
					...
					goto 90
				endif !if((ivcolmin.gt.0.or.ndim2.gt.0).and.ivalmin2.lt.icolmin)  22031
 
				ivalmin=min(ivalmin,ivalmin2)
				ivalmax=max(ivalmax,maxval(ivals(1:n)))
			endif !if(ndim2.lt.0.and.n.gt.0)  22018
		...
		do iobs=1,ndim1
			...
			do k=1,n
				...
				if(ivals(k).lt.icolmin.or.ivals(k).gt.ndim2)then
					...
					return
				endif !if(ivals(k).lt.icolmin.or.ivals(k).gt.ndim2)  22087
				call j_bitset(iout,iobs,ivals(k),1.)
 
			enddo !k=1,n  22080
 
		enddo !iobs=1,ndim1  22071
		deallocate(ivals)
	endif !if(ivin.ge.0)  22070
	if(ivin.eq.0)then
		...
		if(loppu(1:1).ne.'/')then
			...
			j_err=.true.
		end if !if(loppu(1:1).ne.'/')  22104
 
	end if !if(ivin.eq.0)  22101
...
subroutine setvalue(iob,io) !set value for a bitmatrix  %%bitmatrix
	...
	if(j.gt.j_o(irg)%i(1).or.j.le.0)then
		...
		j_err=.true. ;return
	endif !if(j.gt.j_o(irg)%i(1).or.j.le.0)  22143
	...
	if(j_o(irg)%i(2).ne.j_o(irg)%i(3))then
		...
		j_err=.true.;return
	endif !if(j_o(irg)%i(2).ne.j_o(irg)%i(3))  22151
	icol=j_o(irg)%i(2)
	...
	if(icol.lt.j_o(irg)%i(3).or.icol.gt.j_o(irg)%i(2))then
		...
		j_err=.true. ;return
	endif !if(icol.lt.j_o(irg)%i(3).or.icol.gt.j_o(irg)%i(2))  22159
...
subroutine closures(iob,io)             !
	...
	if(j_o(irg)%i(3).ne.1.or.j_o(irg)%i(2).ne.j_o(irg)%i(1))then
		...
		j_err=.true.
		return
	endif !if(j_o(irg)%i(3).ne.1.or.j_o(irg)%i(2).ne.j_o(irg)%i(1))  22199
	ndim=j_o(irg)%i(1)
	...
	do i=1,ndim   !row
		do j=1,ndim
			...
			endif !if(i.eq.j)  22211
		enddo !j=1,ndim  22210
	enddo !i=1,ndim  22209
	lkm=1
		...
		do j=1,ndim
			if(i.eq.j)then
				...
				if(itemp(i,j))then
					...
					endif !if(all(itemp(j,nei(lkm,1:nn))))  22236
				endif !if(itemp(i,j))  22233
			...
			do jj=1,nn2
				...
				if(i.eq.j)then
					...
					if(itemp(key,j))then
						...
						endif !if(all(itemp(j,nei(lkm,1:nn))))  22272
 
					endif !if(itemp(key,j))  22269
				endif !if(i.eq.j)  22264
			enddo !jj=1,nn2  22262
			goto 15
 
		endif !if(key2.gt.0)  22257
 
	enddo !i=1,ndim  22220
...
subroutine difference(iob,io)
	...
	if(j_otype(irg1).ne.j_iplist)then
		...
		j_err=.true.
		return
	endif !if(j_otype(irg1).ne.j_iplist)  22350
	if(j_otype(irg2).ne.j_iplist)then
		...
		if(ii2.lt.j_o(irg1)%i(1))then
			...
			enddo !ij=ii2,j_o(irg1)%i(1)-1  22363
		endif !if(ii2.lt.j_o(irg1)%i(1))  22362
		j_o(irg1)%i(1)=j_o(irg1)%i(1)-1   ! there remains unused elements
		return
	endif !if(j_otype(irg2).ne.j_iplist)  22355
...
subroutine interpolate(iob,io)
	...
	if(narg.gt.2.and.2*np.eq.narg)then
		...
		j_err=.true.
		return
	endif !if(narg.gt.2.and.2*np.eq.narg)  22432
	iout=j_o(iob)%i(io+2+narg)
	if(narg.gt.3)then
	...
	elseif(narg.eq.2.and.j_otype(argx(1)).eq.j_ipmatrix)then !if(narg.eq.5)then
		if(j_o(argx(1))%i(1).lt.2.or.j_o(argx(1))%i(2).lt.2)then
			...
			j_err=.true.
			return
		endif !if(j_o(argx(1))%i(1).lt.2.or.j_o(argx(1))%i(2).lt.2)  22495
		irow=j_igetopt(iob,io,j_mrow)
		if(irow.le.0)then
			ibasy=j_o(argx(1))%i(2)
		else !if(irow.le.0)then
			ibasy=(irow-1)*j_o(argx(1))%i(2)
		endif !if(irow.le.0)  22501
		ismatrix=.true.
		iy=argx(1)
		nval=j_o(argx(1))%i(2)
	elseif(narg.eq.3)then !if(narg.eq.5)then
		if(j_otype(argx(1)).ne.j_ipmatrix.or.j_otype(argx(2)).ne.j_ipmatrix)then
			...
			j_err=.true.
			return
		endif !if(j_otype(argx(1)).ne.j_ipmatrix.or.j_otype(argx(2)).ne.j  22510
		iy=argx(2)
		ibasy=0
		...
		endif !if(j_o(argx(1))%i(1).gt.1.and.j_o(argx(1))%i(2).gt.1.o  22518
 
 
 
 
	endif !if(narg.eq.5)  22447
	found=.false.
	if(ismatrix)then
		...
		do i=1,nval-1
			if(arg.ge.j_o(argx(1))%d(i).and.arg.le.j_o(argx(1))%d(i+1).or. &
				...
				found=.true.
				exit
			endif !if(arg.ge.j_o(argx(1))%d(i).and.arg.le.j_o(argx(1))%d(i+1)  22534
 
		enddo !i=1,nval-1  22533
		...
		do i=1,np-1
			if(arg.ge.j_v(argx(i)).and.arg.le.j_v(argx(i+1)).or. &
				...
				exit
			endif !if(arg.ge.j_v(argx(i)).and.arg.le.j_v(argx(i+1)).o  22551
		enddo !i=1,np-1  22550
 
		if(found)j_v(iout)=j_rlinter(j_v(argx(i)),j_v(argx(i+1)),&
			j_v(argy(i)),j_v(argy(i+1)),arg)
 
	endif !if(ismatrix)  22531
	if(.not.found)then
		write(6,*)'no part found for ',arg
		j_err=.true.
 
	endif !if(.not.found)  22563
...
subroutine sign(iob,io)  !
	...
	if(j_v(j_o(iob)%i(io+2)).ge.0)then
		...
		j_v(iout)=-1.
	endif !if(j_v(j_o(iob)%i(io+2)).ge.0)  22580
	!io=io+narg+3
	return
...
subroutine getobs(iob,io)   !this is j_function
	...
	if(j_otype(ivdata).ne.j_ipdata)then
		...
		j_err=.true.
		return
	endif !if(j_otype(ivdata).ne.j_ipdata)  22615
	...
	if(iobs.le.0.or.iobs.gt.nobs )then
		...
		j_err=.true.
		return
	endif !if(iobs.le.0.or.iobs.gt.nobs )  22621
	...
	if(ivtrans.gt.0)then
		if(j_otype(ivtrans).ne.j_iptrans)then
			...
			j_err=.true.
			return
		endif !if(j_otype(ivtrans).ne.j_iptrans)  22631
		call dotrans(ivtrans,1)
		if(j_err)return
		call j_clearoption(iob,io)  ! subroutine
	endif !if(ivtrans.gt.0)  22630
...
subroutine der(iob,io)
	...
	do while(.true.)
		...
		endif !if(io2.eq.iexitloc)  22737
...
136					isilo2=il2.gt.0
			...
			endif !if(isvar2)  22767
		endif !if(isarg2)  22763
...
subroutine itraceoff(iob,io)
	...
	do i=1,narg
		...
		endif !if(il.le.0)  23050
	enddo !i=1,narg  23048
...
subroutine trace(iob,io)
	...
	if(imi.gt.0)then
		...
		elseif(narg.ne.nmin.and.nmin.ne.0)then !if(nmin.eq.1)then
			...
			j_err=.true.
			return
		endif !if(nmin.eq.1)  23080
		ismin=iexit
	endif !if(imi.gt.0)  23077
	...
	if(imi.gt.0)then
		...
		elseif(narg.ne.nmax.and.nmax.ne.0)then !if(nmax.eq.1)then
			...
			j_err=.true.
			return
		endif !if(nmax.eq.1)  23096
		ismax=iexit
	endif !if(imi.gt.0)  23094
	if(nmax.lt.0.and.nmin.lt.0.and.iexit.gt.1)then
		write(6,*)'trace:, errexit must be with min-> or max->'
 
	endif !if(nmax.lt.0.and.nmin.lt.0.and.iexit.gt.1)  23106
	...
	if(ivlevel.gt.0)then
		...
		endif !if(tracelevel.lt.0.or.tracelevel.gt.2)  23118
 
	endif !if(ivlevel.gt.0)  23116
	...
	do i=1,narg
		...
		endif !if(nmax.ge.0)  23139
	enddo !i=1,narg  23124
...
subroutine itrace(iob,io)
	...
	endif !if(j_ivtracevars.eq.0)  23165
	...
	endif !if(j_ivtracevars.eq.0.or.j_otype(max(j_ivtracevars,1)).ne.  23169
	...
	if(j_linkoption(iob,io,j_mmin).gt.0)then
		...
		elseif(narg.ne.nmin)then !if(nmin.eq.1)then
			...
			j_err=.true.
			return
		endif !if(nmin.eq.1)  23196
		ismin=iexit
	endif !if(j_linkoption(iob,io,j_mmin).gt.0)  23193
	...
	if(j_linkoption(iob,io,j_mmax).gt.0)then
		...
		elseif(narg.ne.nmax)then !if(nmax.eq.1)then
			...
			j_err=.true.
			return
		endif !if(nmax.eq.1)  23210
		ismax=iexit
	endif !if(j_linkoption(iob,io,j_mmax).gt.0)  23208
	...
	if(ivlevel.gt.0)then
		...
		if(tracelevel.lt.0.or.tracelevel.gt.2)then
			write(6,*)';trace: illegal level-> ' ,tracelevel
		endif !if(tracelevel.lt.0.or.tracelevel.gt.2)  23235
 
	endif !if(ivlevel.gt.0)  23233
	...
	endif !if(ntr.gt.j_o(j_ivtracelevel)%i(3))  23259
	call j_clearoption(iob,io)  ! subroutine
	return
...
subroutine tracenow(iob,io)   !io?
	...
	if(j_o(j_ivtraceminstatus)%d(ii).gt.0)then
		...
		endif !if(j_otype(iv).ne.j_ipreal)  23283
		if(j_v(iv).lt.j_o(j_ivtracemin)%d(ii))then
			j_err=j_o(j_ivtraceminstatus)%d(ii).ge.2
			goto 20
		endif !if(j_v(iv).lt.j_o(j_ivtracemin)%d(ii))  23289
	endif !if(j_o(j_ivtraceminstatus)%d(ii).gt.0)  23282
	...
	if(j_o(j_ivtracemaxstatus)%d(ii).gt.0)then
		if(j_otype(iv).ne.j_ipreal)then
			...
			j_err=.true.
			goto 20
		endif !if(j_otype(iv).ne.j_ipreal)  23296
 
		if(j_v(iv).gt.j_o(j_ivtracemax)%d(ii))then
			j_err=j_o(j_ivtracemaxstatus)%d(ii).ge.2
			goto 20
		endif !if(j_v(iv).gt.j_o(j_ivtracemax)%d(ii))  23302
	endif !if(j_o(j_ivtracemaxstatus)%d(ii).gt.0)  23295
	...
	if(lin.gt.0)then
		...
		endif !if(j_otype(iv).eq.j_ipreal)  23317
		...
		endif !if(j_otype(iv).eq.j_ipreal)  23324
		! if(j_ninc.gt.1)then
		! write(6,*)'**input line: ',j_inp(1:j_linp)
		! endif !if(j_ninc.gt.1)then
	endif !if(lin.gt.0)  23314
...
subroutine tracetest(iob,io)   !
	...
	do i=1,narg
		...
		if(j_otype(iset).ne.j_iptraceset)then
			...
			j_err=.true.
			return
		endif !if(j_otype(iset).ne.j_iptraceset)  23346
		do j=1,j_o(iset)%i(0)
			ii=j_o(iset)%i(j)
			...
			endif !if(j_o(j_ivtracecount)%d(ii).le.0)  23353
		enddo !j=1,j_o(iset)%i(0)  23351
	enddo !i=1,narg  23344
...
subroutine existobject(iob,io)  !  vapaa
	...
	if(j_otype(irg).ne.j_ipchar)then
		...
		j_err=.true.
		return !goto 90 !return
	endif !if(j_otype(irg).ne.j_ipchar)  23375
	...
	endif !if(j_linkoption(iob,io,j_mobject).ge.0)  23381
	call j_clearoption(iob,io)  ! subroutine
...
subroutine exist(iob,io)  !  exist(
	...
	if(j_otype(irg).ne.j_ipchar)then
		...
		j_err=.true.
		return !goto 90 !return
	endif !if(j_otype(irg).ne.j_ipchar)  23403
	call j_getchar(irg,j_filename,le)
	if(j_linkoption(iob,io,j_mobject).ge.0)then
		...
		inquire(file=j_filename(1:le),exist=ex)
		if(ex)j_v(iout)=1.d0
	endif !if(j_linkoption(iob,io,j_mobject).ge.0)  23409
...
subroutine laaspoly(iob,io)   !
	...
	endif !if(j_otype(iout).le.j_ipreal)  23465
...
subroutine negbin(iob,io)  !
	...
	endif !if(arg.lt.j_0.or.amyy.lt.j_0.or.ak.lt.j_0)  23553
	if(amyy.eq.j_0)then
		if(arg.eq.j_0)then
			j_v(iout)=j_1
		else
			j_v(iout)=j_0
		endif !if(arg.eq.j_0)  23561
	endif !if(amyy.eq.j_0)  23560
	if(ak.eq.j_0)then
		iarg=arg
		if(dble(iarg).ne.iarg)then
			...
			j_err=.true.
			return
		endif !if(dble(iarg).ne.iarg)  23569
		! kert=j_1
		! do i=1,iarg
		! kert=kert+kert*i
		! enddo !do i=1,iarg
...
subroutine density(iob,io)
	...
	if(linkfunc.gt.0)then
		...
		if(isdisc)then
			...
			enddo !i=1,ncol  23671
			do i=1,ncol
				j_o(iout)%d(ncol+i)=j_o(iout)%d(ncol+i)/sum
			enddo !i=1,ncol  23678
			...
			enddo !i=2,ncol  23690
			do i=1,ncol
				j_o(iout)%d(ncol+i)=j_o(iout)%d(ncol+i)/sum
			enddo !i=1,ncol  23697
		endif !if(isdisc)  23667
		return
	...
	elseif(narg.eq.1)then
		if(j_o(arg(1))%i(4).ne.j_matclass)then
			...
			j_err=.true. ;return
			!goto 900
		endif !if(j_o(arg(1))%i(4).ne.j_matclass)  23704
		nrow_=j_o(iarg)%i(1)
		ncol=j_o(arg(1))%i(2)
...
subroutine bin(iob,io)  !binomial distribution
	...
	endif !if(k.lt.0.d0)  23760
	ios=1
	do i=k2+1,n
		ios=ios*i
	enddo !i=k2+1,n  23767
	in=1
	do i=1,nk
		in=in*i
	enddo !i=1,nk  23771
...
subroutine mod_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)  23805
...
subroutine fraction_(iob,io)
	...
	elseif(j_otype(irg).eq.j_ipmatrix)then !if(j_otype(irg).eq.j_ipreal)then
		...
		write(6,*)'illegal argument'
		j_err=.true.
	endif !if(j_otype(irg).eq.j_ipreal)  23825
